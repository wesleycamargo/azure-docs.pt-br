...
* **Disco temporário.** A VM é criada com um disco temporário (a unidade `D:` no Windows). Esse disco é armazenado em uma unidade física no computador host. Ele *não* é salvo no armazenamento do Azure e pode desaparecer durante as reinicializações e outros eventos de ciclo de vida da VM. Use esse disco somente para dados temporários, como arquivos de paginação ou de permuta.
...
* Esse endereço IP público pode ser dinâmico ou estático. O padrão é dinâmico.
  ...
  * Reserve um [endereço IP estático][static-ip] se precisar de um endereço IP fixo que não mudará &mdash;, por exemplo, se você precisar criar um registro A no DNS ou se precisar do endereço IP para colocar na lista de permissões.
...
* Você pode escalar uma VM vertical ou horizontalmente, [alterando o tamanho da VM][vm-resize]. 
...
* **Interrompendo uma VM.**  O Azure faz uma distinção entre os estados "Parado" e "Desalocado". Você receberá uma cobrança quando o status da VM for "Parado". Você não receberá uma cobrança quando a VM for desalocada.
    ...
    O botão **Parar** no portal do Azure também desaloca a VM. No entanto, se você desligar por meio do sistema operacional enquanto estiver conectado, a VM será interrompida, mas *não* desalocada e, portanto, você ainda será cobrado.
...
A implantação de exemplo fornecida nestas diretrizes usa três diferentes [blocos de construção de modelo][blocks] para criar:

* uma rede virtual (VNet)
* um NSG (grupo de segurança de rede)
* uma VM (máquina virtual)

Essa arquitetura de referência usa um único grupo de recursos que você pode implantar clicando no botão abaixo e aceitar os valores padrão para todos os parâmetros.
...
Se você precisar alterar a implantação para atender às suas necessidades, siga as instruções da página [diretrizes para única vm][readme]. 
...
[Redimensionar máquinas virtuais]: https://azure.microsoft.com/en-us/blog/resize-virtual-machines/
