...
-   [Noções básicas do Service Bus][Noções básicas do Service Bus]
-   [Filas][Filas]
-   [Tópicos][Tópicos]
...
O Service Bus é um serviço de nuvem de diversos clientes, o que significa que o serviço é compartilhado por vários usuários. Cada usuário, como um desenvolvedor de aplicativos, cria um *namespace*, em seguida, define os mecanismos de comunicação necessárias dentro desse namespace. A [Figura 1][Figura 1] mostra isso.

<a name="Fig1"></a>![Diagrama do Service Bus do Azure][Diagrama do Service Bus do Azure]
...
Suponha que você queira conectar dois aplicativos usando uma fila do Service Bus. [Figura 2][Figura 2] ilustra essa opção.

<a name="Fig2"></a>![Diagrama do Service Bus (API, REST)][Diagrama do Service Bus (API, REST)]

**Figura 2: Filas do Service Bus fornecem o enfileiramento de mensagens assíncronas unidirecional.**

O processo é simples: um remetente enviar uma mensagem para uma fila do Service Bus e um receptor pega essa mensagem em um momento posterior. Uma fila pode ter apenas um único receptor, como [Figura 2][Figura 2] mostra ou vários aplicativos podem ler a partir da mesma fila. Na última situação, cada mensagem normalmente é lido por um receptor de filas não fornecem um serviço vários cast.
...
Útil como estão, filas nem sempre são a solução certa. Às vezes, os tópicos do Service Bus são melhores. [Figura 3][Figura 3] ilustra a ideia.

<a name="Fig3"></a>![Diagrama dos tópicos e das assinaturas do Service Bus][Diagrama dos tópicos e das assinaturas do Service Bus]
...
<a name="Fig4"></a>![Retransmissão do Diagrama do Service Bus][Retransmissão do Diagrama do Service Bus]
...
Conectando aplicativos sempre fez parte da criação de soluções completas e é difícil ver esse problema nunca deixará de existir. Fornecendo tecnologias baseadas em nuvem para fazer isto por meio de filas, tópicos e retransmissões, o Service Bus tem como objetivo fazer com essa função essencial mais fácil e mais amplamente disponíveis.
  ...
  [Figura 1]: #Fig1
  [Diagrama do Service Bus do Azure]: ./media/hybrid-solutions/SvcBus_01_architecture.png
  [Figura 2]: #Fig2
  [Diagrama do Service Bus (API, REST)]: ./media/hybrid-solutions/SvcBus_02_queues.png
  [Figura 3]: #Fig3
  [Diagrama dos tópicos e das assinaturas do Service Bus]: ./media/hybrid-solutions/SvcBus_03_topicsandsubscriptions.png
  [Figura 4]: #Fig4
  [Retransmissão do Diagrama do Service Bus]: ./media/hybrid-solutions/SvcBus_04_relay.png
