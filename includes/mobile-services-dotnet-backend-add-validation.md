...
5. Clique com o botão direito do mouse no projeto do serviço e clique em **Publicar**. Publique o serviço móvel na conta do Microsoft Azure usando as configurações de publicação utilizadas anteriormente no tutorial [Introdução] ou [Introdução aos dados](mobile-services-dotnet-backend-windows-store-dotnet-get-started-data.md).
 
     >[AZURE.NOTE]  Também é possível testar usando o serviço hospedado localmente no IIS Express. Para obter mais informações, consulte o tutorial [Introdução aos dados](mobile-services-dotnet-backend-windows-store-dotnet-get-started-data.md).
