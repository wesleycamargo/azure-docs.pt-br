...
1. Após ter se conectado à máquina virtual usando a Área de trabalho remota, abra o Internet Explorer no menu **Iniciar**.
2. Selecione o botão **Ferramentas** no canto superior direito.  Em **Opções da Internet**, selecione a guia **Segurança** e depois selecione o ícone **Sites Confiáveis** e finalmente clique no botão **Sites**.Adicione *http://\*.mongodb.org* à lista de sites confiáveis.
3. Acesse [Downloads- MongoDB] [MongoDownloads].
4. Localize a liberação mais recente na seção **Liberação de Produção (Recomendado)** e clique no link ***2008+** na coluna Windows de 64 bits.  Clique em **Salvar como** e salve o arquivo zip na área de trabalho.
5. Clique com o botão direito no arquivo zip e selecione **Extrair tudo...**    Especifique "C:\" e clique em **Extrair**.Depois que os arquivos forem extraídos, você poderá renomear a pasta de instalação para algo mais simples.Como "MongoDB", por exemplo.
6. Crie dados MongoDB e diretórios de log no disco de dados (unidade **F:**, por exemplo), que você criou nas etapas acima. Em **Iniciar**, selecione **Prompt de comando** para abrir uma janela de prompt de comando.Tipo:
...
7. Para executar o banco de dados, execute: 
	...
	Todas as mensagens de log serão direcionadas ao arquivo *F:\MongoLogs\mongolog.log* assim que o servidor mongod.exe inicia e pré-aloca arquivos de diário. Pode levar alguns minutos para que o MongoDB pré-aloque os arquivos de diário e comece a detectar conexões.
...
11. No painel esquerdo, selecione **Regras de Entrada**.  No painel **Ações** à direita, selecione **Nova Regra...**.
	...
	Selecione **TCP** e em seguida **Portas locais específicas**.  Especifique uma porta de "27017" (a porta MongoDB escuta) e clique em **Avançar**.
