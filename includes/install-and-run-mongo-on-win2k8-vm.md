...
2. Selecione o botão **Ferramentas** no canto superior direito.  Em **Opções da Internet**, selecione a guia **Segurança**, selecione o ícone **Sites confiáveis** e, por fim, clique no botão **Sites**. Adicione _https://\*.mongodb.org_ à lista de sites confiáveis.
...
5. Geralmente, o MongoDB é instalado em C:\Arquivos de Programas\MongoDB. Procure pelas Variáveis de Ambiente na área de trabalho e adicione o caminho dos binários do MongoDB à variável PATH. Por exemplo, você pode localizar os binários em C:\Arquivos de Programas\MongoDB\Server\3.2\bin em seu computador.

6. Crie diretórios de dados e de log do MongoDB no disco de dados (unidade **F:**, por exemplo) criado nas etapas anteriores. No menu **Iniciar**, selecione **Prompt de Comando** para abrir uma janela do prompt de comando.  Tipo:
...
> [AZURE.NOTE] A porta 27017 é a porta padrão usada pelo MongoDB. Você pode alterar essa porta padrão especificando o parâmetro `--port` ao iniciar o servidor mongod.exe. Forneça o mesmo número de porta no firewall e no ponto de extremidade "Mongo" nas instruções anteriores.
