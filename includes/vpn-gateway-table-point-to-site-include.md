---
title: Arquivo de inclusão
description: Arquivo de inclusão
services: vpn-gateway
author: cherylmc
ms.service: vpn-gateway
ms.topic: include
ms.date: 04/19/2018
ms.author: cherylmc
ms.custom: include file
ms.openlocfilehash: 2c4eef702ed204141ea038773eedc5bdb20169a4
ms.sourcegitcommit: fa493b66552af11260db48d89e3ddfcdcb5e3152
ms.translationtype: HT
ms.contentlocale: pt-BR
ms.lasthandoff: 04/23/2018
---
**Autenticação de certificado nativo do Azure**

**Método/modelo de implantação** | **Portal do Azure** | **PowerShell** |
|---|---|---|
| Gerenciador de Recursos | [Artigo](../articles/vpn-gateway/vpn-gateway-howto-point-to-site-resource-manager-portal.md) | [Artigo](../articles/vpn-gateway/vpn-gateway-howto-point-to-site-rm-ps.md)|
| Clássico | [Artigo](../articles/vpn-gateway/vpn-gateway-howto-point-to-site-classic-azure-portal.md) | Com suporte |

**Autenticação RADIUS**

**Método/modelo de implantação** | **Portal do Azure** | **PowerShell** |
|---|---|---|
| Gerenciador de Recursos | Com suporte | [Artigo](../articles/vpn-gateway/point-to-site-how-to-radius-ps.md)|
| Clássico | Sem suporte | Sem suporte |