...
2. No painel de navegação esquerdo do portal, clique em **+ Criar um recurso**, depois em **Integração Corporativa** e em **Retransmissão**.
...
1. Clique em **Todos os recursos**, depois clique em no nome do namespace recém-criado.
2. Na janela Namespace da retransmissão, clique em **Políticas de acesso compartilhado**.
3. Na janela **Políticas de acesso compartilhado**, clique em **RootManageSharedAccessKey**.
   
    ![informações de conexão][connection-info]
4. Na tela **Política: RootManageSharedAccessKey**, clique no botão **Copiar** ao lado da **Chave primária - Cadeia de conexão** para copiar a cadeia de conexão na área de transferência para uso posterior. Cole esse valor no Bloco de notas ou em outro local temporário.
