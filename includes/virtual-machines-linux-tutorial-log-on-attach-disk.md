...
8. Você deve criar o sistema de arquivos na nova partição. Como exemplo, digite o seguinte comando para criar o sistema de arquivo e, em seguida, digite a senha da conta:
   ...
   > [!NOTE]
   > Observe que nos sistemas SUSE Linux Enterprise 11 só há acesso somente leitura para sistemas de arquivos ext4. Para esses sistemas, é recomendável formatar o novo sistema de arquivos como ext3 em vez de ext4.
   > 
   > 
...
11. Adicione a nova unidade ao /etc/fstab:
    ...
    > [!NOTE]
    > O blkid pode não exigir acesso sudo em todos os casos, no entanto, pode ser mais fácil executar com `sudo -i` em algumas distribuições, se /sbin ou /usr/sbin não estiverem em seu `$PATH`.
    > 
    > 
