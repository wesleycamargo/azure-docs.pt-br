...
> [AZURE.NOTE] Este tutorial usa uma única conta de armazenamento para gravar todas as mensagens recuperadas do Hub IoT. Para decidir se você precisará usar várias contas do Armazenamento do Azure em sua solução, veja as [Diretrizes de escalabilidade do Armazenamento do Azure].
...
9. Modifique o método **Main** na classe **Program** conforme mostrado abaixo. Substitua a cadeia de conexão **iothubowner** do Hub IoT (do tutorial [Introdução ao Hub IoT]), a cadeia de conexão de armazenamento e a cadeia de conexão do Barramento de Serviço com permissões **Enviar** para a fila chamada **d2ctutorial**:
    ...
    > [AZURE.NOTE] Para simplificar, este tutorial usa uma única instância da classe [EventProcessorHost]. Para saber mais, veja o [Guia de programação de Hubs de Eventos].

## Receber mensagens interativas
Nesta seção, você escreverá um aplicativo de console do Windows que recebe mensagens interativas da fila do Barramento de Serviço. Para saber mais sobre como arquitetar uma solução usando o Barramento de Serviço, veja [Criar aplicativos multicamadas com o Barramento de Serviço][].
