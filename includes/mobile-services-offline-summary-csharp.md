...
* Para executar pull de dados de uma tabela no servidor para o aplicativo, chamamos  `IMobileServiceSyncTable.PullAsync`.
    ...
    Uma pull sempre envia um push primeiro. Isso é para garantir que todas as tabelas no armazenamento local com relações permaneçam consistentes.
...
* Para habilitar a sincronização incremental, passe uma ID de consulta para `PullAsync()`. A ID de consulta é usada para armazenar o último carimbo de hora atualizado dos resultados da última operação de recepção. A ID da consulta deve ser uma cadeia de caracteres descritiva que é exclusiva para cada consulta lógica em seu aplicativo. Se a consulta tiver um parâmetro, então o mesmo valor de parâmetro deve ser parte da ID da consulta.
