...
4. Adicione o seguinte código que implementa o método POST no controlador:
   ...
   > [!NOTE]
   > Este método POST pode ser chamado por qualquer cliente que tenha a chave do aplicativo, que não é segura. Para proteger o ponto de extremidade, aplique o atributo `[AuthorizeLevel(AuthorizationLevel.User)]` ao método ou classe para exigir a autenticação.
   > 
   > 
