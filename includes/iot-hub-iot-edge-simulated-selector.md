...
Este passo a passo do [Exemplo de Upload para a Nuvem de Dispositivo Simulado] mostra como usar o [Edge IoT do Azure][lnk-sdk] para enviar a telemetria do dispositivo para a nuvem ao Hub IoT por meio de dispositivos simulados.
...
A amostra de Upload para a Nuvem de Dispositivo Simulado mostra como usar o Edge IoT para criar um gateway que envia a telemetria dos dispositivos simulados para um hub IoT. Os dispositivos simulados não podem se conectar diretamente ao Hub IoT, porque:
...
O gateway do Edge IoT resolve esses problemas para os dispositivos simulados das seguintes maneiras:
...
> [!NOTE]
> Os módulos não transmitem mensagens diretamente entre si. Os módulos publicam mensagens em um agente interno, que entrega as mensagens para outros módulos usando um mecanismo de assinatura, conforme mostrado no diagrama abaixo. Para obter mais informações, consulte [Introdução ao Edge IoT do Azure][lnk-gw-getstarted].
> 
> 
...
1. Ele cria dados de temperatura simulados. Observe que se você estiver usando dispositivos reais, o módulo lerá os dados desses dispositivos físicos.
...
Este módulo usa mensagens com uma identidade de dispositivo Hub IoT atribuída pelo módulo anterior e envia o conteúdo da mensagem ao Hub IoT usando o HTTPS. HTTP é um dos três protocolos compreendidos pelo Hub IoT.

Em vez de abrir uma conexão com o Hub IoT de cada aplicativo de dispositivo simulado, este módulo abre uma única conexão HTTP do gateway para o hub IoT e multiplexa as conexões de todos os dispositivos simulados por essa conexão. Isso permite que um único gateway conecte um número muito maior de dispositivos, simulados ou não, do que seria possível se ele abrisse uma conexão exclusiva para cada dispositivo.
