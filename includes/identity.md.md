...
-   [Executando o Active Directory do Windows Server em VMs][]

-   [Usando o Azure Active Directory][]

-   [Usando o Access Control do Active Directory do Azure][]

## <a name="adinvm"></a>Executando o Active Directory do Windows Server em VMs

Executar o AD do Windows Server em VMs do Azure é muito semelhante a executá-lo localmente. A [Figura 1][] mostra um exemplo típico de como isso acontece.

![Active Directory do Azure na Máquina Virtual][]
...
No exemplo mostrado aqui, o AD do Windows Server está em execução em VMs criadas usando Máquinas Virtuais do Azure, a tecnologia da plataforma IaaS. Essas VMs e algumas outras são agrupadas em uma rede virtual (VNET) conectada a um datacenter local usando a Rede Virtual do Azure. O VNET forma um grupo das VMs da nuvem que interage com a rede local por meio de uma conexão de rede virtual privada (VPN). Isso permite que essas VMs do Azure se pareçam exatamente outra sub-rede para o datacenter local. Como mostra a figura, duas dessas VMs estão executando controladores de domínio do AD do Windows Server. As outras VMs na VNET podem estar executando aplicativos, como o SharePoint, ou podem estar sendo usadas de alguma outra maneira, como para desenvolvimento e teste. O data center local também está executando dois controladores de domínio do AD do Windows Server.
...
A [Figura 2][] ilustra a primeira dessas duas opções, na qual o Active Directory do Azure é tudo que é necessário.

![Active Directory do Azure na Máquina Virtual][1]
...
## <a name="ac"></a>Usando o Access Control do Active Directory do Azure
...
Uma vez que o navegador do usuário tem um token do IdP do IdP escolhido, ele envia esse token para o Access Control (etapa 3). O Access Control valida o token, certificando-se de que ele realmente foi emitido por esse IdP, e cria um novo token de acordo com as regras definidas para este aplicativo. Assim como o Active Directory do Azure, o Access Control é um serviço multilocatário, mas os inquilinos são aplicativos em vez das empresas dos clientes. Cada aplicativo pode obter seu próprio namespace, como mostra a figura, e pode definir várias regras sobre autorização e muito mais.
...
David Chappell é diretor da Chappell & Associates [www.davidchappell.com][] em São Francisco, Califórnia. Por meio de sua fala, escrita e consultoria,

  [Executando o Active Directory do Windows Server em VMs]: #adinvm
  [Usando o Azure Active Directory]: #ad
  [Usando o Access Control do Active Directory do Azure]: #ac
  [Figura 1]: #fig1
  [Active Directory do Azure na Máquina Virtual]: ./media/identity/identity_01_ADinVM.png
  [clique aqui]: http://msdn.microsoft.com/en-us/library/windowsazure/jj156090.aspx
  [Figura 2]: #fig2
  [1]: ./media/identity/identity_02_AD.png
  [Apresentando o OData (a página pode estar em inglês)]: http://download.microsoft.com/download/E/5/A/E5A59052-EE48-4D64-897B-5F7C608165B8/IntroducingOData.pdf
  [Figura 3]: #fig3
  [Active Directory do Windows Azure na Máquina Virtual]: ./media/identity/identity_03_AD.png
  [Identidade baseada em declarações para Windows: tecnologias e cenários]: http://www.davidchappell.com/writing/white_papers/Claims-Based_Identity_for_Windows_v3.0--Chappell.docx
  [Figura 4]: #fig4
