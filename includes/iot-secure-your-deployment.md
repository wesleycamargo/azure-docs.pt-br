...
Os aceleradores de solução de IoT protegem os dispositivos de IoT com os dois métodos a seguir:
...
* MQTT: o pacote CONNECT usa `{deviceId}` como o `{ClientId}`, `{IoThubhostname}/{deviceId}` no campo **Nome de usuário** e token SAS no campo **Senha**.
