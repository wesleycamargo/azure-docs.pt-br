<properties 
   pageTitle="Provisionar os clusters de Hadoop do Linux no HDInsight | Azure" 
...
- Chaves SSH. Se você quiser colocar em remoto em um cluster do Linux usando o SSH. Para obter instruções sobre como gerar chaves SSH consulte os seguintes artigos
	-  De um computador Linux - [Use SSH com o HDInsight baseado em Linux (Hadoop) em Linux, Unix ou OS X](../hdinsight-hadoop-linux-use-ssh-unix).
	-  Em um computador Windows - [Use SSH com o HDInsight baseado em Linux (Hadoop) do Windows](../hdinsight-hadoop-linux-use-ssh-windows).

## Neste artigo

* [Opções de configuração](#configuration)
* [Opções para provisionar clusters HDInsight Linux](#options)
...
Provisionar os clusters do Linux de um computador executando esse sistema operacional | Usando o Portal de Gerenciamento do Azure | Usando CLI Multiplataforma | Usando o SDK do .NET | Usando o PowerShell
-----------------| ------------------------| -------------------| ---------- | ---------
...
3. Na página **Detalhes do Cluster**, digite ou escolha os valores a seguir:
    ...
    <table border='1'>
		...
		<tr><td>Sistema operacional</td>
			<td>Selecione <b>Ubuntu 12.04 LTS Preview</b> para provisionar cluster HDInsight no Linux. Para provisionar um cluster do Windows, consulte <a href="http://azure.microsoft.com/ documentation/articles/hdinsight-provision-clusters/" target="_blank">Provisionar clusters Hadoop do Windows no HDInsight</a>.</td></tr>
...
5. Na página **Configurar Usuário de Cluster**, forneça os seguintes valores:
    ...
    <table border='1'>
		...
		<tr><td>Chave pública SSH</td>
			<td>Se você escolher uma chave como tipo de autenticação, especifique a chave pública SSH que você já deve ter gerado. Quando você inicia uma sessão SSH com uma observação no cluster do Linux, usará a chave privada associada com essa chave pública.<br>
...
#### <a id="clilin"></a>Configurar a linha de comando entre plataformas para Linux
...
#### <a id="cliwin"></a>Configurar a linha de comando entre plataformas para Windows
...
#### <a id="cliprovision"></a>Provisionar clusters HDInsight usando a linha de comando entre plataformas do Azure
...
Se você já tiver uma conta de armazenamento, mas não souber o nome e a chave da conta, poderá usar os seguintes comandos para recuperar as informações:
	...
	-- Mostra as informações de uma conta de armazenamento
	mostrar conta de armazenamento do Azure <StorageAccountName>

	-- Lista as chaves para uma conta de armazenamento
	lista de chaves da conta de armazenamento do Azure <StorageAccountName>
...
- [Trabalhando com o HDInsight no Linux](../hdinsight-hadoop-linux-information) Familiarize-se com as nuances de trabalhar com um cluster HDInsight no Linux.
- [Gerenciar clusters HDInsight usando Ambari](../hdinsight-hadoop-manage-ambari). Saiba como monitorar e gerenciar seu Hadoop baseado em Linux no cluster HDInsight usando Ambari Web ou a API REST do Ambari. 
- [Usar o MapReduce com o HDInsight][hdinsight-use-mapreduce]. Saiba mais sobre as diferentes maneiras de executar trabalhos MapReduce em um cluster.
- [Usar o Hive com o HDInsight][hdinsight-use-hive]. Saiba mais sobre as diferentes formas de executar uma consulta de Hive em um cluster.
...
- [Usar o armazenamento de blob do Azure com o HDInsight](../hdinsight-use-blob-storage). Saiba como o HDInsight usa o armazenamento de Blob do Azure para armazenar dados para clusters HDInsight.
