---
title: Introdução à autenticação (Xamarin.Android) - serviços móveis
...
1. No Xamarin Studio, abra o projeto criado quando você concluiu o tutorial [Introdução aos Serviços Móveis].
2. No menu **Executar**, clique em **Iniciar depuração** para iniciar o aplicativo. Verifique se uma exceção sem tratamento com um código de status de 401 (não autorizado) é acionada depois que o aplicativo é iniciado.
...
2. Adicione o método a seguir à classe **ToDoActivity**:
   ...
   > [!NOTE]
   > Se você estiver usando um provedor de identidade que não seja da Microsoft, altere o valor passado para o método **login** acima para um dos seguintes: *Facebook*, *Google*, *Twitter*, ou *WindowsAzureActiveDirectory*.
   > 
   > 
