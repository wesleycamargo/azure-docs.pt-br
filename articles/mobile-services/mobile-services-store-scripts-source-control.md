---
title: Armazenar o código do projeto back-end JavaScript no controle de origem | Microsoft Docs
...
1. Navegue até a pasta `.\service` do seu repositório Git local, abra o arquivo package.json em um editor de texto e adicione o campo a seguir ao objeto **dependencies**:
   
        "node-uuid": "~1.4.3"
   
   > [!NOTE]
   > Essa atualização no arquivo package.json causará uma reinicialização no seu serviço móvel depois que a confirmação for enviada por push.
   > 
   > 
2. Agora, navegue até a subpasta .\\service\\table, abra o arquivo todoitem.insert.js e modifique-o da seguinte maneira:
...
3. Assim como na seção anterior, digite o comando a seguir no prompt de comando do Git:
