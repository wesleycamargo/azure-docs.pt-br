...
Este tutorial se baseia nas etapas e no aplicativo de exemplo do tutorial anterior, [Introdução aos dados]. Antes de começar este tutorial, primeiro conclua a [Introdução aos dados].  
...
4. Localize o método **OnAdd** em **TodoListViewController.cs**. Atualize o método para garantir que o <code>índice</code>  retornado não seja <code>-1</code> como é retornado no tratamento de exceções em **InsertTodoItemAsync**. Nesse caso, não desejamos adicionar uma nova linha a <code>TableView</code>.
