---
title: Controle de acesso baseado em função nos Serviços Móveis usando o .NET e o Active Directory do Azure (Windows Store) | Microsoft Docs
...
9. No AuthorizeAadRole.cs, atualize o método `GetAADToken` na classe `AuthorizeAadRole` com o seguinte código. Este método usa as configurações de aplicativo armazenadas no serviço móvel para obter um token de acesso para o AAD do ADAL.
   ...
   > [!NOTE]
   > O ADAL para .NET inclui um cache de token na memória por padrão para ajudar a minimizar o tráfego de rede adicional no seu Active Directory. No entanto, você pode escrever sua própria implementação de cache ou desabilitar totalmente o cache. Para saber mais, consulte [ADAL para .NET].
   > 
   > 
...
11. No AuthorizeAadRole.cs, atualize o método `OnAuthorization` na classe `AuthorizeAadRole` com o seguinte código. Esse código espera que o usuário que chame o Serviço Móvel tenha se autenticado no AAD. Ele obtém a id de objeto do AAD do usuário e verifica a associação de grupo do Active Directory que corresponde à função.
    ...
    > [!NOTE]
    > É possível pesquisar o grupo do Active Directory por nome. No entanto, em muitos casos a melhor prática é armazenar a id do grupo como uma configuração de aplicativo do serviço móvel. Isso porque o nome do grupo pode mudar, mas a ID permanece a mesma.
    > 
    > 
