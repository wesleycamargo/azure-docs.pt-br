---
title: Adicionar notificações por push ao seu aplicativo de Serviços Móveis (Xamarin.Forms) — Serviços Móveis
...
3. [Adicionar notificações por push ao aplicativo Xamarin.Forms](#iOS)
4. [Adicionar notificações por push ao aplicativo Xamarin.Forms.Android](#Android)
5. [Adicionar notificações por push ao aplicativo Xamarin.Forms.Windows](#Windows)
6. [Atualizar o script de inserção de tabela do Azure para enviar notificações por push a todos os aplicativos](#all-apps)
...
1. No portal clássico do Azure, clique em **Serviços Móveis** e, em seguida, clique no serviço móvel que você acabou de criar.
...
2. No [Portal clássico do Azure], clique em **Serviços Móveis** e, em seguida, clique no serviço móvel. Clique na guia **Painel** e anote a **URL do Site**. Em seguida, clique em **Gerenciar Chaves** e anote a **Chave do Aplicativo**. Você precisará desses valores quando acessar o serviço móvel do seu código de aplicativo.
...
> [!IMPORTANT]
> Devido aos requisitos do APNS (Serviço de Notificação por Push da Apple), você deve implantar e testar as notificações por push em um dispositivo compatível com o iOS (iPhone ou iPad) em vez de usar o emulador.
> 
> 
...
1. Em **AppDelegate**, substitua o evento **RegisteredForRemoteNotifications**:
...
2. Em **AppDelegate**, substitua o evento **ReceivedRemoteNotification**:
...
No [portal clássico do Azure], clique na guia **Dados** e, em seguida, clique na tabela **TodoItem**.

   ![][21]

1. Em **todoitem**, clique na guia **Script** e selecione **Inserir**.
   
   ![][22]

Isso exibe a função que é chamada quando ocorre uma inserção na tabela **TodoItem**.

1. Substitua a função de inserção com o seguinte código e **Salvar**:
...
3. Abra o arquivo de projeto MainActivity.cs e adicione a seguinte instrução using à classe:
...
1. No projeto ToDoAzure.Droid, crie uma nova classe no projeto chamada `GcmService`.
2. Adicione as seguintes instruções de uso à classe **GcmService**:
...
4. No arquivo de projeto **GcmService.cs**, adicione a seguinte classe:
...
5. No arquivo de projeto GcmService.cs, adicione o seguinte código que define a classe **GcmService**:
...
1. Adicione o código a seguir à classe **GcmService** que substitui o manipulador de eventos **OnRegistered**.
...
2. Substitua o método **OnMessage** em **GcmService** pelo seguinte código:
...
3. Adicione as substituições de método a seguir para **OnUnRegistered()** e **OnError()**, necessárias à compilação do projeto.
...
4. No aplicativo, adicione uma nova tarefa.
5. Passe o dedo para baixo da parte superior da tela para abrir a Central de Notificação do dispositivo para ver a notificação.
...
2. Adicione o seguinte ao App.xaml.cs:
   ...
   > [!NOTE]
   > Neste tutorial, o serviço móvel envia uma notificação de toast ao dispositivo. Ao enviar uma notificação de bloco, você deve chamar o método **BindToShellTile** no canal.
   > 
   > 
...
5. No Gerenciador de Soluções, expanda **Propriedades**, abra o arquivo WMAppManifest.xml, clique na guia **Funcionalidades** e verifique se a funcionalidade **ID\_CAP\_PUSH\_NOTIFICATION** está marcada.
...
1. Clique na guia **Enviar por Push**, selecione **Habilitar as notificações por push sem autenticação** e, em seguida, clique em **Salvar**.
   
    Isso permite que o serviço móvel se conecte ao MPNS no modo não autenticado para enviar notificações por push.
   
   > [!NOTE]
   > Este tutorial usa MPNS no modo não autenticado. Nesse modo, o MPNS limita o número de notificações que podem ser enviadas a um canal do dispositivo. Para remover essa restrição, você deve gerar e carregar um certificado clicando em **Carregar** e selecionando o certificado. Para obter mais informações sobre como gerar o certificado, consulte [Configurando um serviço Web autenticado para enviar notificações por push ao Windows Phone].
   > 
   > 
...
1. No Visual Studio, pressione F5 para executar o aplicativo.
   ...
   > [!NOTE]
   > Você pode encontrar um RegistrationAuthorizationException 401 Não autorizado ao testar o emulador do Windows Phone. Isto pode ocorrer durante a chamada de `RegisterNativeAsync()` devido a forma que o emulador do Windows Phone sincroniza seu relógio com o PC anfitrião. Pode resultar em um token de segurança que será rejeitado. Para resolver isto basta definir manualmente o relógio no emulador antes de realizar o teste.
   > 
   > 
2. No aplicativo, crie uma nova tarefa com o título **Hello push** e clique imediatamente no botão Iniciar ou Voltar para sair do aplicativo.
