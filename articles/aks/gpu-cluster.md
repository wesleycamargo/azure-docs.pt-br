---
title: GPUs no Serviço de Contêiner do Azure (AKS)
description: Usar GPUs no Serviço de Contêiner do Azure (AKS)
...
É recomendável um tamanho mínimo de `Standard_NC6` para os nós do AKS (Serviço de Contêiner do Azure).
...
Descreva um dos nós para confirmar se as GPUs são agendáveis. Isso pode ser encontrado na seção `Capacity`. Por exemplo, `alpha.kubernetes.io/nvidia-gpu:  1`.
...
O manifesto de trabalho a seguir inclui um limite de recurso de `alpha.kubernetes.io/nvidia-gpu: 1`. As ferramentas de depuração e bibliotecas CUDA apropriadas estarão disponíveis no nó em `/usr/local/nvidia` e devem ser montadas no pod utilizando a especificação de volume apropriada, conforme mostrado abaixo.
...
Use o comando [kubectl create][kubectl-create] para executar o trabalho. Esse comando analisa o arquivo de manifesto e cria objetos Kubernetes definidos.
```
...
Interessado em executar cargas de trabalho de Machine Learning no Kubernetes? Consulte a documentação do Kubeflow para obter mais detalhes.

> [!div class="nextstepaction"]
> [Guia do usuário do Kubeflow][kubeflow-docs]
