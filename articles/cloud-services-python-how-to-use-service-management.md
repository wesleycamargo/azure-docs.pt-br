<properties linkid="develop-python-service-management" urlDisplayName="Gerenciamento de Serviços" pageTitle="Como usar a API de Gerenciamento de Serviços (Python) - guia do recurso" metaKeywords="" description="Saiba como executar tarefas comuns de Gerenciamento de Serviços de forma programática no Python." metaCanonical="" services="cloud-services" documentationCenter="Python" title="Como usar o Gerenciamento de Serviços no Python" authors="" solutions="" manager="" editor="" />
...
* [Como listar os sistemas operacionais disponíveis][]
* [Como criar uma imagem do sistema operacional][]
* [Como excluir uma imagem do sistema operacional][]
* [Como criar uma máquina virtual][]
* [Como excluir uma máquina virtual][]
* [Próximas etapas][]

## <a name="WhatIs"> </a>O que é Gerenciamento de Serviços
A API de Gerenciamento de Serviços fornece acesso programático a grande parte da funcionalidade de gerenciamento de serviços disponível por meio do [portal de gerenciamento][management-portal]. O SDK do Azure para Python permite que você gerencie os serviços de nuvem, as contas de armazenamento e os grupos de afinidade.

Para usar a API de Gerenciamento de Serviços, será necessário [criar uma conta do Azure](http://www.windowsazure.com/pt-br/pricing/free-trial/). 

## <a name="Concepts"> </a>Conceitos
O SDK do Azure para Python encapsula a [API de Gerenciamento de Serviços do Azure][svc-mgmt-rest-api], que é uma API REST. Todas as operações da API são executadas por meio de SSL e mutuamente autenticadas usando certificados X.509 v3. O serviço de gerenciamento pode ser acessado em um serviço em execução no Azure ou diretamente pela Internet, em qualquer aplicativo que possa enviar uma solicitação HTTPS e receber uma resposta HTTPS.

## <a name="Connect"> </a>Como conectar-se ao gerenciamento de serviços
Para conectar-se ao ponto de extremidade do Gerenciamento de Serviços, você precisa da ID da sua assinatura do Azure e de um certificado de gerenciamento válido. Você pode obter sua ID de assinatura por meio do [portal de gerenciamento][management-portal].

### Certificados de gerenciamento no Windows
...
### Certificados gerenciamento em Mac/Linux
Você pode usar o [OpenSSL](http://www.openssl.org/) para criar seu certificado de gerenciamento.  Na verdade, você precisa criar dois certificados, um para o servidor (um arquivo `.cer`) e outro para o cliente (um arquivo `.pem`). Para criar o arquivo `.pem`, execute:
...
Quando você cria um aplicativo e o executa no Azure, o código e a configuração, em conjunto, são chamados de [serviço de nuvem] do Azure (conhecido anteriormente como *serviço hospedado* em versões anteriores do Azure). O método **create\_hosted\_service** permite que você crie um novo serviço hospedado, fornecendo um nome de serviço hospedado (que deve ser exclusivo no Azure), um rótulo (automaticamente codificado em base64), uma descrição e um local. Você pode especificar um grupo de afinidade em vez de um local para o serviço. 
...
É possível listar todos os serviços hospedados por sua assinatura com o método **list\_hosted\_serviços**:
...
Se desejar obter informações sobre um determinado serviço hospedado, você pode fazê-lo passando o nome do serviço hospedado para o método **get\_hosted\_service_properties**:
...
Depois que você criar um serviço de nuvem, você pode implantar seu código no serviço com o método **create\_deployment**.

## <a name="DeleteCloudService"> </a>Como excluir um serviço de nuvem

É possível excluir um serviço de nuvem passando o nome do serviço para o método **delete\_hosted\_service**:

	sms.delete_hosted_service('myhostedservice')

Observe que para você poder excluir um serviço, todas as implantações para o serviço devem ser excluídas primeiro. (Consulte [Como excluir uma implantação](#DeleteDeployment) para obter detalhes.)

## <a name="CreateDeployment"> </a>Como criar uma implantação

O método **create\_deployment** carrega um novo [pacote de serviço] e cria uma nova implantação no ambiente de teste ou de produção. Os parâmetros desse método são os seguintes:

* **name**: o nome do serviço hospedado.
* **deployment\_name**: o nome da implantação.
* **slot**: uma cadeia de caracteres que indica o slot de `preparo` ou de `produção`.
* **package_url**: a URL do pacote de implantação (um arquivo .cspgk). O arquivo do pacote deve ser armazenado em uma conta de Armazenamento de Blob do Azure sob a mesma assinatura que o serviço hospedado para o qual o pacote está sendo carregado. Você pode criar um pacote de implantação com os [cmdlets do PowerShell do Azure] ou com a [ferramenta de linha de comando cspack].
...
No exemplo acima, observe que o status da operação **create\_deployment** pode ser recuperada passando o resultado retornado por **create\_deployment** para o método **get\_operation\_status**.

Você pode acessar propriedades de implantação com o slot **get\_deployment_by\_slot** ou os métodos **get\_deployment_by\_name**. O exemplo a seguir recupera uma implantação especificando o slot de implantação. O exemplo também itera por todas as instâncias da implantação:
...
## <a name="UpdateDeployment"> </a>Como atualizar uma implantação

Uma implantação pode ser atualizada usando o método **change\_deployment_configuration** ou o método **update\_deployment_status**.
...
No exemplo acima, observe que o status da operação **change\_deployment\_configuration** pode ser recuperada passando o resultado retornado por **change\_deployment\_configuration** para o método **get\_operation\_status**.
...
Um [serviço de armazenamento] fornece acesso a [Blobs][azure-blobs], [Tabelas][azure-tables] e [Filas][azure-queues] do Azure. Para criar um serviço de armazenamento, você precisa de um nome para o serviço (com 3 a 24 caracteres minúsculos e exclusivo no Azure), uma descrição, um rótulo (até 100 caracteres, automaticamente codificado em base64) e um local ou um grupo de afinidade. O exemplo a seguir mostra como criar um serviço de armazenamento especificando um local. Se desejar usar um grupo de afinidade, você precisará primeiro criar um grupo de afinidade (consulte [Como criar um grupo de afinidade](#CreateAffinityGroup)) e defini-lo com o parâmetro **affinity\_group**.
...
No exemplo acima, observe que o status da operação **create\_storage\_account** pode ser recuperada passando o resultado retornado por **create\_storage\_account** para o método **get\_operation\_status**.  
...
É possível excluir um serviço de armazenamento passando o nome do serviço de armazenamento para o método **delete\_storage\_account**. A exclusão de um serviço de armazenamento excluirá todos os dados armazenados no serviço (blobs, tabelas e filas).
...
Depois de ter criado um grupo de afinidade, você pode especificar o grupo (em vez de um local) ao [criar um serviço de armazenamento](#CreateStorageService).
...
Para criar uma máquina virtual, você precisa primeiro criar um [serviço de nuvem](#CreateCloudService).  Em seguida, crie a implantação da máquina virtual usando o método **create\_virtual\_machine\_deployment**:
...
O serviço de nuvem pode então ser excluído, usando o método **delete\_hosted\_service**:
...
-   Consulte a Referência do MSDN: [Serviços de Nuvem][]
-   Consulte a Referência do MSDN: [Máquinas Virtuais][]
...
[Como listar os sistemas operacionais disponíveis]: #ListOperatingSystems
[Como criar uma imagem do sistema operacional]: #CreateVMImage
[Como excluir uma imagem do sistema operacional]: #DeleteVMImage
[Como criar uma máquina virtual]: #CreateVM
[Como excluir uma máquina virtual]: #DeleteVM
[Próximas etapas]: #NextSteps
[management-portal]: https://manage.windowsazure.com/
[svc-mgmt-rest-api]: http://msdn.microsoft.com/pt-br/library/windowsazure/ee460799.aspx


[download-SDK-Python]: https://www.windowsazure.com/pt-br/develop/python/common-tasks/install-python/
...
[azure-blobs]: https://www.windowsazure.com/pt-br/develop/python/how-to-guides/blob-service/
[azure-tables]: https://www.windowsazure.com/pt-br/develop/python/how-to-guides/table-service/
[azure-queues]: https://www.windowsazure.com/pt-br/develop/python/how-to-guides/queue-service/
[Esquema de configuração de serviço do Azure (.cscfg)]: http://msdn.microsoft.com/pt-br/library/windowsazure/ee758710.aspx
[Serviços de Nuvem]: http://msdn.microsoft.com/pt-br/library/windowsazure/jj155995.aspx
