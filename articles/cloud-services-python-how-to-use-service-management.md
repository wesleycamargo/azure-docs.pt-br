<properties 
	pageTitle="Como usar a API de gerenciamento de serviços (Python) - guia de recursos" 
...
* [Conceitos][]
* [Como: Conectar-se ao gerenciamento de serviço][]
...
* [Como: Atualizar uma implementação][]
* [Como: Mover as implantações entre o preparo e a produção][]
...
* [Como: Lista os sistemas operacionais disponíveis][]
* [Como: Cria uma imagem de sistema operacional][]
* [Como: Exclui uma imagem de sistema operacional][]
* [Como: Criar uma máquina virtual][]
* [Como: Exclui uma máquina virtual][]
* [Próximas etapas][]

## <a name="WhatIs"> </a>O que é o Gerenciamento de Serviços
A API de Gerenciamento de Serviços fornece acesso programático a grande parte da funcionalidade do gerenciamento de serviços disponível por meio do [Portal de Gerenciamento][management-portal]. O SDK do Azure para Python permite que você gerencie os serviços de nuvem, as contas de armazenamento e os grupos de afinidade.
...
> [AZURE.NOTE] Uma vez que o SDK do Azure para Python versão 0.8.0, é agora possível usar certificados criados com o OpenSSL quando for executado no Windows.  Isso requer o Python 2.7.4 ou posterior.
...
O comando criará o arquivo  `.cer` e o instalará no armazenamento de certificados **Pessoal**. Para obter mais detalhes, consulte [Criar e carregar um certificado de gerenciamento para Azure (a página pode estar em inglês)](http://msdn.microsoft.com/library/windowsazure/gg551722.aspx).
...
Você também pode usar o [OpenSSL](http://www.openssl.org/) para criar o certificado de gerenciamento.  Na verdade, você precisa criar dois certificados, um para o servidor (um arquivo  `.cer`) e um para o cliente (um arquivo  `.pem`). Para criar o arquivo  `.pem`, execute:
...
Quando você cria um aplicativo e o executa no Azure, o código e a configuração, juntos, são chamados de [serviço de nuvem] do Azure (conhecido como  *hosted service* em versões anteriores do Azure). O método **create\_hosted\_service** permite que você crie um novo serviço hospedado, fornecendo um nome de serviço hospedado (que deve ser exclusivo no Azure), um rótulo (automaticamente codificado em base64), uma descrição e um local. Você pode especificar um grupo de afinidade em vez de um local para o serviço. 
...
Você pode listar todos os serviços hospedados para sua assinatura com o método **list\_hosted\_services**:
...
O método **create\_deployment** carrega um novo [pacote de serviço] e cria uma nova implantação no ambiente de preparo ou de produção. Os parâmetros para esse método são os seguintes:
...
* **configuração**: o arquivo de configuração do serviço (arquivo .cscfg) codificado em base64.
* **label**: o rótulo do nome do serviço hospedado (automaticamente codificado em base64).

O exemplo a seguir cria uma nova implantação `v1` for a hosted service called `myhostedservice`:
...
Observe no exemplo acima que o status da operação **create\_deployment** pode ser recuperado passando o resultado retornado por **create\_deployment** para o método **get\_operation\_status**.

Você pode acessar propriedades de implantação com os métodos **get\_deployment\_by\_slot** ou **get\_deployment\_by\_name**. O exemplo a seguir recupera uma implantação especificando o slot de implantação. O exemplo também reitera por todas as instâncias da implantação:
...
Observe no exemplo acima, o status da operação **change\_deployment\_configuration** pode ser recuperado passando o resultado retornado por **change\_deployment\_configuration** para o método **get\_operation\_status**.

O método **update\_deployment\_status** permite que você defina um status de implantação como EM EXECUÇÃO ou SUSPENSO. O exemplo a seguir demonstra como definir o status para EM EXECUÇÃO para uma implantação chamada `v1` of a hosted service called `myhostedservice`:
...
Um [serviço de armazenamento] fornece acesso a [Blobs][azure-blobs], [Tabelas][azure-tables] e [Filas][azure-queues] do Azure. Para criar um serviço de armazenamento, você precisa de um nome para o serviço (com 3 a 24 caracteres minúsculos e exclusivo no Azure), uma descrição, um rótulo (até 100 caracteres, automaticamente codificado em base64) e um local ou um grupo de afinidade. O exemplo a seguir mostra como criar um serviço de armazenamento especificando um local. Se quiser usar um grupo de afinidade, você precisará primeiro criar um grupo de afinidade (consulte [Como: Criar um grupo de afinidade](#CreateAffinityGroup)) e defini-lo com o parâmetro **affinity\_group**.
...
Observe no exemplo acima que o status da operação **create\_storage\_account** pode ser recuperado passando o resultado retornado por **create\_storage\_account** para o método **get\_operation\_status**.  
...
Você pode excluir um serviço de armazenamento passando o nome do serviço de armazenamento para o método **delete\_storage\_account**. A exclusão de um serviço de armazenamento excluirá todos os dados armazenados no serviço (blobs, tabelas e filas).
...
Para adicionar uma imagem de sistema operacional ao repositório de imagens, use o método **add\_os\_image**:
...
Para listar as imagens do sistema operacional que estão disponíveis, use o método **list\_os\_images**. Isso inclui todas as imagens da plataforma e imagens do usuário:
...
Para criar uma máquina virtual, você precisa primeiro criar um [serviço de nuvem](#CreateCloudService).  Em seguida, crie a implantação de máquina virtual usando o método **create\_virtual\_machine\_deployment**:
...
Para excluir uma máquina virtual, você primeiro exclui a implantação usando o método **delete\_deployment**:
...
-   Consulte a referência de MSDN: [Serviços de Nuvem][]
-   Consulte a referência de MSDN: [Máquinas Virtuais][]
...
[pacote de serviço]: http://msdn.microsoft.com/library/windowsazure/jj155995.aspx
...
[Implantando um Serviço do Azure]: http://msdn.microsoft.com/library/windowsazure/gg433027.aspx
...
[Serviços de Nuvem]: http://msdn.microsoft.com/library/windowsazure/jj155995.aspx
[Máquinas Virtuais]: http://msdn.microsoft.com/library/windowsazure/jj156003.aspx
