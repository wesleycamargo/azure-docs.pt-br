...
> [WACOM.NOTE] Este artigo descreve o uso do Portal de Gerenciamento do Azure, o PowerShell do Azure e a Interface de Linha de Comando entre Plataformas do Azure para trabalhar com logs de diagnóstico. Para obter informações sobre como trabalhar com logs de diagnóstico usando o Visual Studio, consulte [Solucionando problemas em Sites do Azure no Visual Studio][Solucionando problemas em Sites do Azure no Visual Studio].

## Sumário

-   [O que é: o diagnóstico de site?][O que é: o diagnóstico de site?]
-   [Como: Habilitar diagnósticos][Como: Habilitar diagnósticos]
-   [Como: Baixe logs][Como: Baixe logs]
-   [Como: Logs de streaming][Como: Logs de streaming]
-   [Como: Compreendendo os logs de diagnóstico][Como: Compreendendo os logs de diagnóstico]
-   [Próximas etapas][Próximas etapas]

<a name="whatisdiag"></a>

## O que é o diagnóstico de site?
...
-   **Log de Servidor da Web** - registra todas as transações HTTP em um site, usando o [formato W3C estendido de arquivo de log][formato W3C estendido de arquivo de log]. Este relatório é útil para determinar as métricas gerais do site, como o número de solicitações manipuladas e quantas solicitações existem vindas de um endereço IP específico

### Diagnóstico de aplicativo

O diagnóstico de aplicativo permite que você capture informações produzidas por um aplicativo da Web. Os aplicativos ASP.NET podem usar a classe [Rastreamento.de.Diagnóstico.de.Sistema][Rastreamento.de.Diagnóstico.de.Sistema] para registrar informações no log de diagnóstico do aplicativo. Por exemplo:
...
Para obter informações sobre como trabalhar com Diagnóstico de Aplicativo usando o Visual Studio, consulte [Solucionando problemas em Sites do Azure no Visual Studio][1].
...
## Como: habilitar diagnóstico

O diagnóstico pode ser habilitado ao visitar a página **Configurar** no site do seu Azure no [Portal de Gerenciamento do Azure][Portal de Gerenciamento do Azure]. Na página **Configurar**, use as seções **diagnóstico de aplicativo** e **diagnóstico de site** para habilitar o log.
...
> [WACOM.NOTE] As informações armazenadas em **armazenamento de tabela** ou **armazenamento de blob** apenas podem ser acessadas usando um cliente de armazenamento ou um aplicativo que possa trabalhar diretamente com estes sistemas de armazenamento. Por exemplo, o Visual Studio 2013 contém um Gerenciador de Armazenamento que pode ser usado para explorar armazenamentos de tabela ou de blob e o HDInsight pode acessar dados gravados no armazenamento de blob. Você também pode gravar um aplicativo que acesse o Armazenamento do Azure usando um dos [SDKs do Azure][SDKs do Azure].
...
-   **Sistema de arquivos** - armazena as informações de diagnóstico de aplicativo no sistema de arquivos do site. Estes arquivos podem ser acessados por FTP ou baixados como um arquivo Zip usando o PowerShell do Azure ou as Ferramentas de Linha de Comando do Windows Azure.
...
> [WACOM.NOTE] O diagnóstico também pode ser habilitado a partir do PowerShell do Azure usando o cmdlet **Set-AzureWebsite**. Se você não instalou o PowerShell do Azure ou não o configurou para usar sua Assinatura do Azure, consulte [Como usar o PowerShell do Azure][Como usar o PowerShell do Azure].
...
-   **Logs do Web Server** - /LogFiles/http/RawLogs. Esta pasta contém um ou mais arquivos de texto que foram formatados usando o [formato W3C estendido de arquivo de log][formato W3C estendido de arquivo de log].
...
> [WACOM.NOTE] Se você não instalou o PowerShell do Azure ou não o configurou para usar sua Assinatura do Azure, consulte [Como usar o PowerShell do Azure][Como usar o PowerShell do Azure].
...
> [WACOM.NOTE] Se você não instalou as Ferramentas da Linha de Comando ou não configurou-as para usar a sua Assinatura do Windows Azure, consulte [Como usar as ferramentas da linha de comando do Windows Azure][Como usar as ferramentas da linha de comando do Windows Azure].

<a name="streamlogs"></a>

## Como: transmitir logs
...
Isto conectará ao site especificado pelo parâmetro **-Nome** e iniciará o streaming de informações para a janela do PowerShell, enquanto eventos de log ocorrem no site. Qualquer informação escrita para os arquivos com terminação .txt ou .htm que sejam armazenadas no diretório /LogFiles (d:/home/LogFiles) será transmitida à console local.
...
Isto conectará ao site chamado 'websitename' e iniciará o streaming de informações para a janela, enquanto eventos de log ocorrem no site. Qualquer informação escrita para os arquivos com terminação .txt ou .htm que sejam armazenadas no diretório /LogFiles (d:/home/LogFiles) será transmitida à console local.
...
## Como: compreender os logs de diagnóstico
...
<td style="border:1px solid black;vertical-align:top">
Data/hora do evento no formato aaaaMMddHH
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
Um valor de GUID que identifica exclusivamente esta entidade
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
A data e hora em que o evento ocorreu, em formato de escala (maior precisão)
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
Nível de evento (por exemplo, erro, aviso ou informação)
...
<td style="border:1px solid black;vertical-align:top">
A ID deste evento<br />
é padronizada como 0, caso nada seja especificado
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
A instância do site onde o evento ocorreu
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
A ID do thread que produziu o evento
...
<td style="border:1px solid black;vertical-align:top">
A mensagem com detalhes do evento
...
| ApplicationName     | O nome do site                                                               |
| InstanceId          | A instância do site onde o evento ocorreu                                    |
| EventTickCount      | A data e hora em que o evento ocorreu, em formato de escala (maior precisão) |
| EventId             | A ID deste evento<br />é padronizada como 0, caso nada seja especificado     |
| Pid                 | ID do Processo                                                               |
...
O rastreamento de solicitações falhas é armazenado em arquivos XML chamados **fr\#\#\#\#\#\#.xml**. Para facilitar a visualização das informações registradas, uma folha de estilos XML chamada **freb.xsl** é fornecida no mesmo diretório dos arquivos XML. A abertura de um dos arquivos XML no Internet Explorer usará a folha de estilos XSL para fornecer uma exibição de formato da informação rastreada. Isto aparecerá semelhante ao seguinte:

![solicitação falha visualizada no navegador][solicitação falha visualizada no navegador]
...
Os logs do servidor da Web são formatados usando o [formato W3C estendido de arquivo de log][formato W3C estendido de arquivo de log]. Esta informação pode ser lida usando um editor de texto ou analisada usando ferramentas como o [Analisador de Log][Analisador de Log].

> [WACOM.NOTE] Os logs produzidos pelos sites do Azure não oferecem suporte aos campos **s-computername**, **s-ip**, ou **cs-version**.
...
-   [Tutorial - solucionando problemas com sites][Tutorial - solucionando problemas com sites]
-   [Solucionando problemas de sites do Azure no Visual Studio][Solucionando problemas em Sites do Azure no Visual Studio]
-   [Analisar logs de sites em HDInsight][Analisar logs de sites em HDInsight]

  [Solucionando problemas em Sites do Azure no Visual Studio]: /pt-BR/develop/net/tutorials/troubleshoot-web-sites-in-visual-studio/
  ...
  [formato W3C estendido de arquivo de log]: http://msdn.microsoft.com/library/windows/desktop/aa814385.aspx
  [Rastreamento.de.Diagnóstico.de.Sistema]: http://msdn.microsoft.com/pt-BR/library/36hhw2t6.aspx
  [1]: http://www.windowsazure.com/pt-BR/develop/net/tutorials/troubleshoot-web-sites-in-visual-studio/
  [Portal de Gerenciamento do Azure]: https://manage.microsoft.com
  [SDKs do Azure]: http://www.windowsazure.com/pt-BR/downloads/#
  [Como usar o PowerShell do Azure]: http://www.windowsazure.com/pt-BR/develop/nodejs/how-to-guides/powershell-cmdlets/
  [Como usar as ferramentas da linha de comando do Windows Azure]: http://www.windowsazure.com/pt-BR/develop/nodejs/how-to-guides/command-line-tools/
  [solicitação falha visualizada no navegador]: ./media/web-sites-enable-diagnostic-log/tws-failedrequestinbrowser.png
  [Analisador de Log]: http://go.microsoft.com/fwlink/?LinkId=246619
