---
title: 'Tutorial: Criar uma API personalizada usando um aplicativo Web ASP.NET no PowerApps e em Fluxos Lógicos | Microsoft Docs'
...
> [!IMPORTANT]
> Este tópico foi movido para powerapps.microsoft.com em [Tutorial: criar uma API Web protegida por AAD personalizada para PowerApps e fluxo](https://powerapps.microsoft.com/tutorials/customapi-web-api-tutorial/). Vá para o PowerApps a fim de obter a versão mais recente. Esse link do Azure está sendo arquivado.
> 
> 
...
2. Na próxima tela, selecione o modelo de API Web e **Sem Autenticação**: ![](./media/powerapps-web-api-tutorial/noauth.png "Sem Autorização")
   ...
   > [!IMPORTANT]
   > Certifique-se de definir a autenticação como “Sem Autenticação”.
   > 
   > 
...
4. Em seguida, gere um arquivo do Swagger para a API Web. É possível fazer isso com facilidade abrindo o **Console do Gerenciador de Pacotes** e instalando o **Swashbuckle**: ![](./media/powerapps-web-api-tutorial/swashbuckle-console.png "Console do Swashbuckle")
...
7. Extraia o JSON do Swagger navegando até ***https://\<URL-do-aplicativo-Web-do-Azure>/swagger/docs/v1***.
   ...
   > [!IMPORTANT]
   > Um documento do Swagger com ID(s) de operação duplicada(s) é inválido. Se você estiver usando o modelo do C# de exemplo, a ID da operação “Values\_Get” será repetida duas vezes. Altere uma instância para “Value\_Get”.
   > 
   > 
...
3. Para **webAPI**, use a seguinte configuração:
   ...
   3. URL de resposta: ***https://\<a-URL-raiz>/.auth/login/aad/callback***
      ...
      > [!IMPORTANT]
      > Você precisará da ID do Cliente desse aplicativo mais tarde, portanto, anote-a.
      > 
      > 
