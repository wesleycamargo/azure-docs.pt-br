...
Conecte-se ao Microsoft Translator para traduzir texto, detectar um idioma e muito mais.

O Microsoft Translator pode ser usado do PowerApps Enterprise e de aplicativos lógicos.
...
Quando você adiciona o Microsoft Translator ao PowerApps Enterprise, você insere os valores **ID do cliente ** e **Segredo do Cliente** do seu aplicativo do Microsoft Translator. Se não tiver um aplicativo do Translator, você pode criar um:
...
Copie/cole agora os valores de **ID do Cliente** e **Segredo do Cliente** na sua configuração do Translator no Portal do Azure.


## Referência da API REST do Swagger

### Detectar o idioma    
Detecta o idioma de origem de um dado texto. ```GET: /Detect```
...
### Texto em fala    
Converte um determinado texto em fala como um fluxo de áudio no formato wave. ```GET: /Speak```
...
### Obter linguagens    
Recupera todos os idiomas com suporte no Microsoft Translator. ```GET: /TranslatableLanguages```
...
Recupera os idiomas disponíveis para síntese de fala. ```GET: /SpeakLanguages```
...
## Próximas etapas
Depois de adicionar a API do Microsoft Translator ao PowerApps Enterprise, [conceda permissões aos usuários](../power-apps/powerapps-manage-api-connection-user-access.md) para usar a API em seus aplicativos.
