...
O Active Directory do Azure (AD do Azure) é um serviço moderno e baseado em REST que fornece recursos de gerenciamento de identidade e de Controle de Acesso para seus aplicativos de nuvem. O AD do Azure integra-se facilmente com serviços de nuvem, bem como o Azure, o Microsoft Office 365, o Dynamics CRM Online e o Windows Intune. As implantações existentes do Active Directory no local também podem tirar total proveito do AD do Azure. Para saber mais, consulte a [página Identidade][página Identidade] em [windowsazure.com][windowsazure.com].
...
O aplicativo de exemplo incluído neste tutorial pode ser [baixado aqui][baixado aqui]. O exemplo pode ser executado sem alterações, mas talvez seja necessário alterar a [atribuição da porta no Visual Studio][atribuição da porta no Visual Studio] para usar https. Siga as instruções no link, mas defina o protocolo de ligação como "https" na seção vinculações do arquivo applicationHost.config. Todos os trechos de código nas etapas abaixo foram tirados do exemplo.
...
-   [WCF Data Services para OData][WCF Data Services para OData]

### Sumário

-   [Introdução][Introdução]
-   [Parte 1: Obter uma ID de cliente para acessar o AD do Azure][Parte 1: Obter uma ID de cliente para acessar o AD do Azure]
-   [Parte 2: Habilitar os clientes para entrarem usando o AD do Azure][Parte 2: Habilitar os clientes para entrarem usando o AD do Azure]
-   [Parte 3: Habilitar Logon Único][Parte 3: Habilitar Logon Único]
-   [Parte 4: Acessar o AD Graph do Azure][Parte 4: Acessar o AD Graph do Azure]
-   [Parte 5: Publicar seu aplicativo][Parte 5: Publicar seu aplicativo]
-   [Resumo][Resumo]

## <a name="getclientid"></a>Parte 1: Obter uma ID de cliente para acessar o AD do Azure
...
Para desenvolver e publicar aplicativos que se integram com o AD do Azure, você deve se inscrever para uma conta do [Painel do Vendedor da Microsoft][Painel do Vendedor da Microsoft]. Em seguida, você receberá uma solicitação para [criar um perfil de conta][criar um perfil de conta] como uma empresa ou como um indivíduo. Esse perfil é usado para publicar aplicativos no Azure Marketplace ou em outros marketplaces e é necessário para gerar uma ID do Cliente e um Segredo do Cliente.
...
Uma ID do Cliente e um Segredo do Cliente são necessários para integrar seu aplicativo com o AD do Azure. Uma ID do Cliente é o identificador exclusivo para o seu aplicativo e é usada principalmente para identificar um aplicativo para logon único ou para chamadas de autenticação para o AD Graph do Azure. Para obter mais informações sobre como obter uma ID do Cliente e um Segredo do Cliente, consulte [Criar IDs e Segredos de clientes no Painel do Vendedor da Microsoft][Criar IDs e Segredos de clientes no Painel do Vendedor da Microsoft].
...
1.  No nó **appSettings**, substitua os valores de "clientId" e "SymmetricKey" pela sua ID do Cliente, Segredo do Cliente e seu domínio de aplicativo:
...
2.  No nó **audienceUris** de **system.identityModel**, insira sua ID do Cliente após "spn:":
...
Para gerar a solicitação de redirecionamento no passo 2 acima, você deve acrescentar os parâmetros da cadeia de caracteres de consulta à seguinte URL para a página de autorização do AD do Azure: *<http://activedirectory.windowsazure.com/Consent/AuthorizeApplication.aspx>*
...
Durante o desenvolvimento, essas permissões são usadas para testar aplicativos não publicados. Para aplicativos publicados, esse parâmetro será ignorado. Em vez disso, as permissões solicitadas no detalhe de seu aplicativo serão usadas. Consulte a Parte 5 para obter mais informações sobre essa listagem.
Há três valores possíveis para este parâmetro:
...
### Etapa 2: Manipulando a resposta de consentimento
...
<div class="dev-callout"><strong>Observa&ccedil;&atilde;o</strong><p>Depois que o consentimento for concedido, o AD do Azure pode levar algum tempo at&eacute; que o acesso ao SSO e ao Graph sejam provisionados. O primeiro usu&aacute;rio de cada organiza&ccedil;&atilde;o que se inscreve em seu aplicativo pode ver erros de entrada at&eacute; que o provisionamento seja conclu&iacute;do.</p></div>
...
### Etapa 3: Obter um locatário do AD do Azure para testar seu aplicativo

Para testar a capacidade de integração do aplicativo com o AD do Azure, você precisará de um locatário do AD do Azure. Se você já tiver um locatário usado para testar outro aplicativo, você poderá reutilizá-lo. É recomendável obter pelo menos dois locatários para garantir que seu aplicativo possa ser testado e usado por vários locatários. Não é recomendável usar um locatário de produção para essa finalidade. [Obter um locatário do AD do Azure][Obter um locatário do AD do Azure]
...
<div class="dev-callout"><strong>Observa&ccedil;&atilde;o</strong><p>Se seus clientes se inscreverem para um novo locat&aacute;rio do AD do Azure, pode levar algum tempo para que o locat&aacute;rio seja totalmente provisionado. Os usu&aacute;rios podem ver erros na p&aacute;gina de consentimento at&eacute; que o provisionamento seja conclu&iacute;do.</p></div>

## <a name="enablesso"></a>Parte 3: Habilitar Logon Único
...
### Etapa 1: Procurar a ID do locatário
...
2.  No arquivo **FederationMetadata.xml**, localize a entrada **Entity Descriptor**. A ID do locatário é incluída como parte da propriedade **entityID** e seguida a "<https://sts.windows.net>", conforme mostrado abaixo:
    ...
    Nesse caso, o valor TenantID é **a7456b11-6fe2-4e5b-bc83-67508c201e4b**.
...
### Etapa 2: Gerar a solicitação de entrada

Quando um cliente entrar no seu aplicativo, por exemplo, clicando no botão de entrada, a solicitação de entrada deve ser gerada usando a ID de locatário do cliente e sua ID de cliente do aplicativo. No aplicativo de exemplo, esta solicitação é gerada pelo método *GenerateSignInMessage* da classe *Microsoft.IdentityModel.WAAD.Preview.WebSSO.URLUtils* Esse método verifica se a TenantID do cliente representa uma organização que autorizou seu aplicativo e gera a URL de destino para o botão de entrada, conforme mostrado abaixo:
...
1.  Usar uma classe de proxy gerada para chamar o AD Graph do Azure
...
<div class="dev-callout"><strong>Observa&ccedil;&atilde;o</strong><p>A biblioteca auxiliar do aplicativo de exemplo Microsoft.IdentityModel.WAAD.Preview j&aacute; cont&eacute;m uma classe de proxy gerada automaticamente (criada pela adi&ccedil;&atilde;o de uma refer&ecirc;ncia de servi&ccedil;o a https://graph.windows.net/your-domain-name chamada GraphService). O aplicativo usar&aacute; essa classe de proxy para chamar o servi&ccedil;o do AD Graph do Azure.</p></div>

### Etapa 1: Usar uma classe de proxy para chamar o AD Graph do Azure
...
No aplicativo de exemplo, essas chamadas ao API são tratadas pelo método GraphInterface na classe *Microsoft.IdentityModel.WAAD.Preview.Graph.GraphInterface*, conforme mostrado no seguinte código.
...
### Etapa 2: Adquirir um token de acesso usando a Biblioteca de Autenticação do Azure
...
<div class="dev-callout"><strong>Observa&ccedil;&atilde;o</strong><p>A AAL est&aacute; dispon&iacute;vel como um pacote NuGet e pode ser instalada no Visual Studio.</p></div>
...
### Etapa 3: Chamar o AD Graph do Azure para obter uma lista de usuários
...
## <a name="publish"></a>Parte 5: Publicar seu aplicativo
...
<div class="dev-callout"><strong>Observa&ccedil;&atilde;o</strong><p>Seu aplicativo &eacute; respons&aacute;vel por gerenciar qualquer rela&ccedil;&atilde;o de cobran&ccedil;a com seus clientes. O Azure Marketplace fornece apenas links para o site de seu aplicativo e informa&ccedil;&otilde;es sobre ele.</p></div>

### Etapa 1: Criando um manifesto e o detalhe do aplicativo
    ...
    <AppRequiredPermissions>
      <AppPermissionRequests Policy="AppOnly">
        <AppPermissionRequest Right="Directory Reader" Scope="http://directory">
          <Reason culture="pt-BR" value="Needs to read the app"/>
...
Usando sua nova ID do Cliente e o manifesto do aplicativo, você pode criar um detalhe do aplicativo seguindo as instruções em [Adicionar aplicativos no Painel do Vendedor da Microsoft][Adicionar aplicativos no Painel do Vendedor da Microsoft]. Ao criar um detalhe do aplicativo, certifique-se de selecionar o tipo de aplicativo AD do Azure. Ao concluir seu detalhe do aplicativo, clique em "Enviar" para publicar o aplicativo no Azure Marketplace. Você precisará aguardar até que o aplicativo seja aprovado para que a publicação seja concluída.

<div class="dev-callout"><strong>Observa&ccedil;&atilde;o</strong><p>Se voc&ecirc; receber uma solicita&ccedil;&atilde;o para &quot;adicionar informa&ccedil;&otilde;es de imposto e pagamento&quot;, poder&aacute; ignorar essa etapa porque voc&ecirc; est&aacute; vendendo seu aplicativo diretamente ao cliente e n&atilde;o por meio da Microsoft.</p></div>

### Etapa 2: Finalizar o teste e tornar seu aplicativo público
...
A integração com o AD do Azure permite que seus clientes inscrevam-se e entrem em seu aplicativo usando um sistema de gerenciamento de identidades que ele já mantém, o que reduz ou elimina a necessidade de separar as tarefas de gerenciamento de identidades com seu aplicativo. Essa funcionalidade dá a seus clientes uma experiência mais transparente ao usar seu aplicativo e libera o tempo gasto com tarefas de gerenciamento.
  ...
  [Parte 1: Obter uma ID de cliente para acessar o AD do Azure]: #getclientid
  ...
  [logon]: ./media/active-directory-dotnet-integrate-multitent-cloud-applications/login.png
