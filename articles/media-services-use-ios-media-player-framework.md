...
* Para o primeiro clipe, **appendTo** é -1. E quando chamamos `[framework scheduleClip:adpodInfo1 atTime:adLinearTime forType:PlaylistEntryType_Media andGetClipId:&adIndex]`, `adIndex` recebe um valor exclusivo que indica o fim desse primeiro clipe no pacote de anúncios. Em seguida, para o segundo clipe no pacote de anúncios, alinhe o início do segundo anúncio com o fim do primeiro definindo **appendTo** como `adpodInfo2.appendTo = adIndex;`, que especifica a posição final do primeiro como o local para iniciar o segundo clipe. 
