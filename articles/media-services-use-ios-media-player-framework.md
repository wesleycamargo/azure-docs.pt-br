<properties linkid="develop-media-services-how-to-guides-ios-media-player-framework" urlDisplayName="iOS Media Player Framework" pageTitle="Usar o iOS Media Player Framework com os Serviços de Mídia do Azure" metaKeywords="" description="Saiba como usar a biblioteca do iOS Media Player Framework com os Serviços de Mídia para criar aplicativos sofisticados e dinâmicos." metaCanonical="" services="media-services" documentationCenter="" title="Como usar o iOS Media Player Framework com os Serviços de Mídia do Azure" authors="" solutions="" manager="" editor="" />



#Como usar o iOS Media Player Framework com os Serviços de Mídia do Azure
...
Geralmente, é relativamente complexo criar um aplicativo que pode criar fluxos de conteúdo que resultam de uma interação entre o aplicativo e o usuário - normalmente, você precisa criar o fluxo inteiro a partir do zero e armazená-lo, com antecedência, no servidor. Usando o iOS Media Player Framework, você pode criar aplicativos cliente que podem fazer tudo isso sem a necessidade de controle ou modificar o fluxo de conteúdo principal. Você pode:

- Agende fluxos de conteúdo com antecedência no dispositivo cliente.
- Agende anúncios ou inserções de pre-roll.
- Agende anúncios ou inserções de post-roll.
- Agende anúncios ou inserções de mid-roll para criar pacotes de anúncios.
- Controle se o anúncio ou a inserção de mid-roll é reproduzido cada vez que a linha do tempo do conteúdo é retrocedida ou se ele é executado uma vez e, em seguida, removido da linha do tempo.
- Insira conteúdo dinamicamente diretamente na linha do tempo como resultado de qualquer evento, no caso de o usuário pressionar um botão ou o aplicativo receber uma notificação de um serviço - por exemplo, um programa de conteúdo de notícias pode enviar notificações de últimas notícias e o aplicativo pode "pausar" o conteúdo principal para carregar dinamicamente um fluxo de últimas notícias. 
...
3. Esta é a estrutura do player de exemplo:
...
![Controlador da barra de busca](http://mingfeiy.com/wp-content/uploads/2013/01/controller.png)
...
* O controle de **Tempo do Player** mostra dois horários (`Label:playerTime`), como 00h23/02h10. Nesse caso, 00h23 é a hora da reprodução atual e 02h10 é a duração total da mídia. 
...
Fazer isso seguindo o código anterior agenda dois fluxos de conteúdo na linha do tempo do conteúdo principal. O primeiro é agendado com base em `URLWithString:url` e o segundo conteúdo é agendado com base em `URLWithString:secondContent`. Para o segundo, o conteúdo inicia a partir de um ponto em 30 segundos do fluxo de mídia e termina em 80 segundos dentro dele. 
...
* Todo **LinearTime.duration** precisa ser 0 ao agendar um anúncio.
* Quando **clipEndMediaTime** for mais longo que a duração do anúncio, o anúncio terminará depois que for concluído e nenhuma exceção será gerada. É aconselhável verificar se a duração natural do anúncio está dentro do tempo de renderização (**clipEndMediaTime**) para que você não perca uma oportunidade de anunciar. 
* Anúncios de pre-roll, mid-roll e post-roll são suportados. Pre-roll pode ser agendado apenas no início de todo o conteúdo. Por exemplo, você não pode agendar um pre-roll para o segundo conteúdo em um cenário RCE (edição de corte irregular). 
* Anúncios adesivos e de uma única reprodução são suportados e podem ser usados em conjunto com o anúncio de pre-roll, mid-roll ou post-roll.
* O formato do anúncio pode ser .Mp4 ou HLS.

### Como agendar anúncios de pre-roll, mid-roll e post-roll e pacotes de anúncios.
...
#### Agendamento pacotes de anúncios
...
* Para o primeiro clipe, o **appendTo** é -1. E quando chamamos `[framework scheduleClip:adpodInfo1 atTime:adLinearTime forType:PlaylistEntryType_Media andGetClipId:&adIndex]`, `adIndex` recebe um valor exclusivo que indica o fim desse primeiro clipe no pacote de anúncios. Em seguida, para o segundo clipe no pacote de anúncios, alinhe o início do segundo anúncio com o fim do primeiro definindo **appendTo** como `adpodInfo2.appendTo = adIndex;`, o que especifica a posição final do primeiro como o local para iniciar o segundo clipe. 
