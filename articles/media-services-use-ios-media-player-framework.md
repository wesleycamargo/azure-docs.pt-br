...
2.  A lógica de aplicativo principal reside no **SamplePlayerViewController.m** na pasta `Shared`. A maior parte dos trechos de código descritos abaixo está localizada nesse arquivo.
...
-   Os botões **Seleção de URL** permitem que você selecione URLs alternativas na lista de URLs de mídia.
...
-   Os botões **SkipFroward e SkipBackward**atualmente não funcionam como o esperado. Uma versão atualizada será lançada em breve.

-   Pressionar o **botão Agendar Agora** enquanto o conteúdo principal está sendo reproduzido insere um anúncio (você pode definir a URL de origem do anúncio em code-behind). Observação: Na versão atual, você não pode agendar um anúncio enquanto outro anúncio está sendo reproduzido.
...
-   **clipEndMediaTime** representa a hora de término da reprodução de um vídeo. Se **clipEndMediaTime**=100, a reprodução do vídeo termina no 100º segundo do videoclipe.
...
**Observação:** Sempre agende o conteúdo principal antes de agendar qualquer anúncio (inclusive o anúncio de pre-roll).
...
-   Quando **clipEndMediaTime** for mais longo que a duração do anúncio, o anúncio terminará depois de sua conclusão e nenhuma exceção será gerada. É aconselhável verificar se a duração natural do anúncio está dentro do tempo de renderização (**clipEndMediaTime**) para que você não perca uma oportunidade de anunciar.
-   Há suporte para anúncios de pre-roll, mid-roll e post-roll. Pre-roll pode ser agendado apenas no início de todo o conteúdo. Por exemplo, você não pode agendar um pre-roll para o segundo conteúdo em um cenário RCE (edição de corte irregular).
-   Anúncios adesivos e de uma única reprodução têm suporte e podem ser usados em conjunto com o anúncio de pre-roll, mid-roll ou post-roll.
...
-   Você deve definir o valor **appendTo** como -1, se não for um Ad Pod.
-   O valor **type** pode ser pre-roll, mid-roll, post-roll ou ad pod. Para pre-roll ou post-roll, como não há nenhum tempo associado, especifique o tipo.
...
A única diferença deste agendamento de anúncio para o de pre-roll é `postAdInfo.type = AdType_Postroll;`. O código anterior agendou um anúncio de cinco segundos como post-roll.
...
### Consulte o [wiki do Media Player Framework do Azure][wiki do Media Player Framework do Azure] para obter mais informações.
  ...
  [wiki do Media Player Framework do Azure]: https://github.com/WindowsAzure/azure-media-player-framework/wiki
