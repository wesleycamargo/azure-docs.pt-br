<properties 
	pageTitle="Como usar o Apache Giraph com o Azure HDInsight" 
	description="Saiba como usar o Apache Giraph para realizar o processamento de gráficos com o Azure HDInsight" 
	services="hdinsight" 
	documentationCenter="" 
	authors="blackmist" 
	manager="paulettm" 
...
O [Apache Giraph][giraph] permite que você realize processamento de tabelas usando o Hadoop, além de poder ser utilizado com o HDInsight do Azure. 
...
* [Solucionar problemas que você eventualmente encontrar](#tshoot)
...
* [Maven](http://maven.apache.org/) 3 ou posterior
...
1. Atualmente (14/07/2014), o Giraph requer um patch para funcionar com o armazenamento de arquivos WASB utilizado pelo HDInsight. O patch foi enviado ao projeto Apache Giraph, mas ainda não foi aceito. Baixe o patch da seção __anexos__ do [GIRAPH-930](https://issues.apache.org/jira/browse/GIRAPH-930) e salve-o no drive local como __giraph-930.diff__.
...
4. Carregue o arquivo JAR de exemplo no armazenamento primário para seu cluster HDInsight usando o [PowerShell do Azure][aps] e o [HDInsight-Tools][tools].
...
A SimpleShortestPathsComputation demonstra a implementação do [Pregel](http://people.apache.org/~edwardyoon/documents/pregel.pdf) básico para encontrar a rota mais curta entre objetos em uma tabela. Use as etapas a seguir para carregar os dados amostrais, execute um trabalho usando o exemplo SimpleShortestPathsComputation e, então, visualize os resultados.

> [AZURE.NOTE] A origem para esse e outros exemplos está disponível na [ramificação versão-1.1](https://github.com/apache/giraph/tree/release-1.1) do [repositório GitHub](https://github.com/apache/giraph).
...
1. Crie um novo arquivo chamado __tiny\_graph.txt__. Ele deve conter as linhas a seguir.
	...
	Esses dados descrevem uma relação entre objetos em uma [tabela direcionada](http://en.wikipedia.org/wiki/Directed_graph), usando o formato `[source_id, source_value,[[dest_id], [edge_value],...]]`. Cada linha representa uma relação entre um objeto __source\_id__ e um ou mais objetos __dest\_id__. O __edge\_value__ (ou peso) pode ser pensada como a intensidade ou a distância da conexão entre o __source\_id__ e o __dest\_id__. 
...
2. Carregue o arquivo __tiny\_graph.txt__ no armazenamento primário no seu cluster HDInsight usando o [Azure PowerShell][aps] e as [HDInsight-Tools][tools].
...
3. Use o PowerShell a seguir para executar o exemplo __SimpleShortstPathsComputation__, usando o arquivo __tiny\_graph.txt__ como entrada. Isso requer que você tenha instalado e configurado o [PowerShell do Azure][aps].
...
Isso criará a estrutura de diretórios __example/output/shortestpaths__ no diretório atual e baixará os arquivos começando por __parte__. O cmdlet __Cat__ exibirá os conteúdos dos arquivos, que devem ser semelhantes ao exibido a seguir.
...
> [AZURE.NOTE] A linha de comando do Hadoop só está disponível ao se conectar ao cluster HDInsight, usando a área de trabalho remota.
> 
...
1. Usando o [Portal de Gerenciamento do Azure](https://manage.windowsazure.com), selecione seu cluster HDInsight e, então, selecione __Configuração__.
...
Agora que você aprendeu como usar o Giraph com o HDInsight, tente fazer [Pig][] e [Hive][] com o HDInsight.
