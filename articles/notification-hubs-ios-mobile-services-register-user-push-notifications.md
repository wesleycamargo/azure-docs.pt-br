<properties urlDisplayName="Notify iOS app users by using Mobile Services" pageTitle="Registrar o usuário atual para notificações por push usando um serviço móvel - Hubs de notificação" metaKeywords=" do aplicativo de registro do Azure Hubs de Notificação, notificações por push do Azure, aplicativo iOS com notificação por push" description="Learn how to request push notification registration in an iOS app with Azure Notification Hubs when registeration is performed by Azure Mobile Services." metaCanonical="" services="mobile-services,notification-hubs" documentationCenter="" title="Register the current user for push notifications by using a mobile service" authors="yuaxu" solutions="" manager="dwrede" editor="" />
