<properties
   pageTitle="Atualizando para o SDK .NET da Pesquisa do Azure versão 1.0.0-preview | Microsoft Azure | Serviço de pesquisa de nuvem hospedada"
   description="Atualizando para o SDK .NET da Pesquisa do Azure versão 1.0.0-preview"
   services="search"
   documentationCenter=""
...
# Atualizando para o SDK .NET da Pesquisa do Azure versão 1.0.0-preview

Se você estiver usando a versão 0.13.0-preview ou anterior do [SDK .NET da Pesquisa do Azure](https://msdn.microsoft.com/library/azure/dn951165.aspx), este artigo ajudará você a atualizar o seu aplicativo para usar a versão mais recente, 1.0.0-preview.
...
A versão 1.0.0-preview do SDK .NET da Pesquisa do Azure contém várias alterações significativas da versão anterior (0.13.0-preview). A maioria das alterações é leve e, portanto, a alteração do seu código não deve exigir muito. Confira [Etapas da atualização](#UpgradeSteps) para obter instruções sobre como alterar o seu código para usar a nova versão do SDK.

<a name="WhatsNew"></a>
## Novidades no 1.0.0-preview

A versão 1.0.0-preview destina-se à mesma versão de API REST que as versões mais antigas do SDK .NET da Pesquisa do Azure (28-02-2015); portanto, não há nenhum recurso novo do serviço nesta versão. No entanto, há um novo recurso do lado do cliente para usuários avançados do SDK.
...
Primeiro, atualize a sua referência NuGet para `Microsoft.Azure.Search` usando o NuGet Package Manager Console ou clicando duas vezes em suas referências do projeto e selecionando "Gerenciar pacotes NuGet..." no Visual Studio.

> [AZURE.NOTE]Verifique se você é capaz de ver os pacotes de pré-lançamento ao selecionar "Incluir Pré-lançamento", se estiver usando o Visual Studio, ou ao usar a opção `-IncludePrerelease` se estiver usando o Package Manager Console.
...
A próxima etapa é corrigir os erros de compilação individualmente. A maioria exigirá a alteração de alguns nomes de classe e de método que foram renomeados no SDK. A [Lista de alterações significativas no 1.0.0-preview](#ListOfChanges) contém uma lista dessas alterações de nome.

Se estiver usando classes personalizadas para modelar os seus documentos e essas classes tiverem propriedades de tipos primitivos não anuláveis (por exemplo, `int` ou `bool` em C#), há uma correção de bug na versão 1.0.0-preview do SDK da qual você deve estar ciente. Confira [Correções de bugs no 1.0.0-preview](#BugFixes) para obter mais detalhes.

Finalmente, depois de solucionar possíveis erros de compilação, você pode fazer alterações no seu aplicativo para tirar proveito da nova funcionalidade, se desejar. O recurso de serialização personalizada no novo SDK é detalhado em [Novidades no 1.0.0-preview](#WhatsNew).

<a name="ListOfChanges"></a>
## Lista de alterações significativas no 1.0.0-preview
...
Cada operação no SDK .NET da Pesquisa do Azure é exposta como um conjunto de sobrecargas de método para chamadores síncronos e assíncronos. As assinaturas e a fatoração dessas sobrecargas de método mudou na versão 1.0.0-preview.
...
As assinaturas de método para a mesma operação na versão 1.0.0-preview têm esta aparência:
...
O SDK .NET da Pesquisa do Azure a partir da versão 1.0.0-preview organiza os métodos de operação de maneira diferente:
...
<a name="BugFixes"></a>
## Correções de bugs no 1.0.0-preview
...
Corrigimos o problema na versão 1.0.0-preview do SDK. Agora, se você tiver uma classe de modelo como esta:
