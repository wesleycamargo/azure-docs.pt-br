...
A versão atual da API REST de Gerenciamento da Pesquisa do Azure é `api-version=2015-02-28`. A versão anterior, `api-version=2014-07-31-Preview`, foi preterida. Embora ela continue funcionando nos próximos meses, incentivamos a transição para a nova versão assim que possível.
...
Observe que você não pode alterar o nome, o local e a SKU. A alteração de qualquer uma dessas propriedades exigirá a criação de um novo serviço.
...
> [AZURE.NOTE] Se você usar um PUT para atualizar o serviço, deverá usar o mesmo corpo da solicitação usado em uma solicitação [Criar serviço](#CreateService).
