...
## <a name="about-peering"></a>Sobre o emparelhamento 

Considere as seguintes restrições antes de emparelhar redes virtuais:
...
- Um emparelhamento é estabelecido entre duas redes virtuais. Os emparelhamentos não são provisórios. Se você criar emparelhamentos entre:
    - Rede virtual 1 e rede virtual 2
    - Rede virtual 2 e rede virtual 3

  Não há nenhum emparelhamento entre a rede virtual 1 e a rede virtual 3 por meio da rede virtual 2. Se você quiser configurar um emparelhamento entre a rede virtual 1 e a rede virtual 3, você precisa criar um emparelhamento entre a rede virtual 1 e a rede virtual 3.
...
- Você pode emparelhar redes virtuais implantadas por meio de diferentes modelos de implantação que estão na mesma assinatura. A capacidade de emparelhar redes virtuais implantadas por meio de diferentes modelos de implantação em assinaturas diferentes está em versão prévia.
- As assinaturas que ambas as redes virtuais estão devem ser associadas ao mesmo locatário do Azure Active Directory. Se você ainda não tiver um locatário do AD, [crie um](../active-directory/develop/active-directory-howto-tenant.md?toc=%2fazure%2fvirtual-network%2ftoc.json#start-from-scratch) rapidamente. 
- <a name="roles-permissions"></a>Sua conta deve ter a função necessários ou as permissões para criar um emparelhamento. Por exemplo, se você pretende emparelhar duas redes virtuais, chamadas *VNet1* e *VNet2*, sua conta deverá receber a seguinte função ou permissões mínimas para cada rede virtual:
...
## <a name="create-peering"></a>Criar um emparelhamento

>[!NOTE]
>Há vários requisitos, restrições e considerações para criar um emparelhamento com êxito. Antes de criar um emparelhamento, certifique-se de estar familiarizado com a lista de consideração na seção [Sobre emparelhamento](#about-peering) deste artigo.
>

1. Faça logon no [portal](https://portal.azure.com) com uma conta que tenha as [funções e permissões](#roles-permissions) necessárias.
...
## <a name="change-subnet"></a>Exibir ou alterar as configurações de emparelhamento
...
## <a name="delete-subnet"></a>Excluir um emparelhamento
...
- Criar um hub e spoke de topologia de rede | [Vários](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke?toc=%2fazure%2fvirtual-network%2ftoc.json#vnet-peering)|
