...
# Atribuir vários endereços IP a máquinas virtuais
...
2. Altere os "valores" das seguintes $Variables para o nome que você deseja dar à NIC, o [grupo de recursos](../resource-group-overview.md#resource-groups) ao qual você deseja atribuí-la e a [localização](https://azure.microsoft.com/regions) do Azure onde você deseja criá-la.
...
3. <a name="subnet"></a>A NIC deve ser conectada a uma sub-rede em uma VNet (Rede Virtual) do Azure existente na mesma localização e [assinatura](../azure-glossary-cloud-terminology.md#subscription) que a NIC. Se você não estiver familiarizado com VNets, leia o artigo [Visão geral da rede virtual](virtual-networks-overview.md) para saber mais sobre elas ou leia o artigo [Criar uma VNet](virtual-networks-create-vnet-arm-ps.md) para saber como criar uma. Altere os seguintes "valores" das $Variables para o nome da VNet e da sub-rede à qual você deseja conectar a NIC e o nome do grupo de recursos em que a VNet está.
	...
	Digite os seguintes comandos para obter o nome das sub-redes na VNet e substitua *Subnet1* acima pelo nome de uma sub-rede:
...
9. <a name="os"></a>Adicione manualmente todos os endereços IP privados secundários (endereços IP com *False* na coluna **Primário** da saída na etapa anterior) à configuração de TCP/IP no sistema operacional. O endereço IP privado atribuído à *IPConfig-1* na etapa 5 é atribuído automaticamente ao sistema operacional por meio do DHCP do Azure, porque ela é a configuração *Primária*.
	...
	**Windows**
	...
	2. Abra as propriedades do adaptador de rede.
	3. Abra as propriedades do **Protocolo IP versão 4**
		- Clique em **Usar o seguinte endereço IP** e insira os seguintes valores:
			...
			- **Gateway padrão**: o primeiro endereço IP na sub-rede. Se sua sub-rede for 10.0.0.0/24m, o endereço IP do gateway será 10.0.0.1.
		...
		- Clique em **OK** para fechar as configurações de TCP/IP e, em seguida, em **OK** novamente para fechar as configurações do adaptador.
	4. Em um prompt de comando, digite *ipconfig /all*. Todos os endereços IP que você adicionou são mostrados e o DHCP está desativado.
...
## <a name="add"></a>Adicione os endereços IP a uma VM existente
...
3. Crie uma variável e defina-a para a NIC digitando o seguinte comando:
