...
Para o SDK, a história é semelhante. A criação e o provisionamento de um cluster usando o SDK estão documentados em [Usando o SD .NET do HDInsight](../hdinsight-provision-clusters/#sdk). A configuração de um headnote muito grande exige a adição do parâmetro `HeadNodeSize = NodeVMSize.ExtraLarge` para o método `ClusterCreateParameters()` usado no código.
...
- [ZooKeeper][zookeeper]
- [Conectar a clusters HDInsight usando o RDP](../hdinsight-administer-use-management-portal/#rdp)
