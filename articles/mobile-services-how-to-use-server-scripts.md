<properties linkid="register-for-facebook-auth" urlDisplayName="Serviços Móveis" pageTitle="Trabalhar com scripts de servidor em Serviços Móveis" metaKeywords="scripts de servidor, dispositivos móveis, Azure, agendador" description="Fornece exemplos de como definir, registrar e usar scripts de servidor em Serviços Móveis do Azure." metaCanonical="" services="" documentationCenter="Mobile" title="Trabalhar com scripts de servidor em Serviços Móveis" authors="ricksal" solutions="" manager="" editor="" />


# Trabalhar com scripts de servidor em Serviços Móveis
 
Este artigo fornece informações detalhadas e exemplos de como trabalhar com scripts de servidor em Serviços Móveis do Azure. Este tópico é dividido nas seguintes seções:
...
+ [API personalizada][Custom API anchor]
...
+ [Agendador de Trabalhos]
	+ [Como: definir scripts de trabalho agendados]
...
Nos Serviços Móveis, você pode definir lógica de negócios personalizada como código JavaScript armazenado e executado no servidor. Esse código de script do servidor é atribuído a um dos seguintes recursos de servidor:
...
>[WACOM.NOTE]Uma função registrada na operação de exclusão deve ser nomeada _del_ porque a exclusão é uma palavra reservada em JavaScript. 
...
Quando a função **executar** é chamada, o valor `item`, [consulta][query object], ou o `id` passado como primeiro argumento para a função de script é usado para realizar a operação. Para uma operação de inserção, atualização ou consulta, você pode modificar o item ou a consulta antes de chamar **executar**: 
...
###<a name="work-with-users"></a>Como: trabalhar com os usuários
...
Quando funções de API personalizadas são chamadas pelo tempo de execução dos Serviços Móveis, tanto um objeto de [solicitação][objeto de solicitação] quanto de [resposta][objeto de resposta] são fornecidos. Esses objetos expõem a funcionalidade da [biblioteca de express.js], que pode ser aproveitada pelos seus scripts. A seguinte API personalizada chamada **hello** é um exemplo muito simples que retorna _Hello, world!_ em resposta a uma solicitação POST:
...
+ `https://<serviço>.azure-mobile.net/api/calculator/add?a=1&b=2`

		{"result":3}

+ `https://<serviço>.azure-mobile.net/api/calculator/sub?a=3&b=5`
...
>[WACOM.NOTE]Se você tiver habilitado o controle do código-fonte, é possível editar os arquivos de script do trabalho agendado diretamente na subpasta .\service\scheduler em seu repositório git. Para obter mais informações, consulte [Como: compartilhar código usando o controle do código-fonte].
...
+ **crypto**: oferece o recurso de criptografia do OpenSSL. Para obter mais informações, consulte a [Documentação do Node.js][crypto API].
...
+ Para módulos publicados e instalados por npm, use o arquivo package.json para declarar quais pacotes você deseja que o serviço móvel instale. Dessa forma, o serviço sempre tem acesso à versão mais recente dos pacotes necessários. O arquivo package.json permanece no diretório `.\service`. Para obter mais informações, consulte [Suporte para package.json nos Serviços Móveis do Azure].

+ Para módulos particulares ou personalizados, você pode usar npm para instalar manualmente o módulo no diretório `.\service\node_modules` do controle do código-fonte. Para obter um exemplo de como carregar um módulo manualmente, consulte [Utilizar código compartilhado e módulos do Node. js em seus scripts de servidor].

	>[WACOM.NOTE]Quando `node_modules` já estiver na hierarquia do diretório, NPM criará o subdiretório `\node-uuid` nela, em vez de criar um novo `node_modules` no repositório. Nesse caso, basta excluir o diretório `node_modules` existente.
...
O exemplo de código completo e o debate complementar podem ser encontrados nesta [postagem no blog](http://blogs.msdn.com/b/jpsanders/archive/2013/03/20/server-script-to-insert-table-items-in-windows-azure-mobile-services.aspx). Se você usar esse código, é possível adaptá-lo à sua situação específica e testá-lo integralmente.
...
Para gravar nos logs, use o [objeto de console] global. Use a função **log** ou **info** para registrar avisos de nível de informação. As funções **warning** e **error** registram seus respectivos níveis, que são chamados nos logs. 
...
[Como: trabalhar com os usuários]: #work-with-users
[Como: definir scripts de trabalho agendados]: #scheduler-scripts
...
[Como: acessar os cabeçalhos de solicitação de API personalizada]: #get-api-headers
...
[objeto de console]: http://msdn.microsoft.com/pt-br/library/windowsazure/jj554209.aspx
