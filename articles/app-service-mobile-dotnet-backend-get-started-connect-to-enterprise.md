...
2. No gerenciador de pacotes NuGet, clique em **Online**. Digite **Microsoft.Azure.Mobile.Server.AppService** como um termo de pesquisa. Em seguida, clique em **Instalar** para instalar o pacote [Extensão de Serviço de Aplicativo do back-end .NET de aplicativos móveis]. Esse pacote fornece métodos de extensão para trabalhar com informações sobre o usuário conectado no momento.

2. No projeto de Código de Aplicativo Móvel, crie uma nova classe chamada SharePointUploadContext. Adicione uma instrução `using Microsoft.Azure.Mobile.Server.AppService;` ao arquivo. Em seguida, adicione o seguinte à classe:
