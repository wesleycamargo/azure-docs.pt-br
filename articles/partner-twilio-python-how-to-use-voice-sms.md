<properties linkid="develop-php-how-to-twilio-sms-service" urlDisplayName="Serviço de voz/SMS do Twilio" pageTitle="Como usar o Twilio para voz e SMS (PHP) - Azure" metaKeywords="Twilio PHP do Azure, chamadas telefônicas do Azure, chamadas telefônicas do Azure, twilio do Azure, SMS do Azure, SMS do Azure, chamadas de voz do Azure, chamadas de voz do azure, mensagens de texto do Azure, mensagens de texto do Azure" description="Saiba como fazer uma chamada telefônica e enviar uma mensagem SMS com o serviço de API do Twilio no Azure. Exemplos de código escritos em PHP." metaCanonical="" services="" documentationCenter="" title="Como usar o Twilio para recursos de voz e SMS no PHP" authors="" solutions="" manager="" editor="" />





# Como usar o Twilio para recursos de voz e SMS no PHP
...
* [Como: enviar uma mensagem de SMS](#howto_send_sms)
* [Como: fornecer respostas TwiML de seu próprio site](#howto_provide_twiml_responses)
...
Para obter mais informações sobre os verbos do Twilio, seus atributos e o TwiML, consulte o [TwiML] [twiml]. Para obter informações adicionais sobre a API do Twilio, consulte [API do Twilio] [twilio_api].
...
Um aplicativo PHP que usa o serviço da Twilio e está em execução no Azure não é diferente de qualquer outro aplicativo PHP que usa o serviço da Twilio. Embora os serviços da Twilio sejam baseados em REST e possam ser chamados no PHP de várias maneiras, este artigo se concentra em como usar os serviços da Twilio com a [Biblioteca da Twilio para PHP no Github][twilio_php]. Para obter mais informações sobre como usar a biblioteca do Twilio para PHP, consulte [http://readthedocs.org/docs/twilio-php/en/latest/index.html][twilio_lib_docs].
...
1. Baixe a biblioteca do Twilio para PHP no Github ([https://github.com/twilio/twilio-php][twilio_php]) e adicione o diretório **Services** ao seu aplicativo.
...
O seguinte mostra como enviar uma mensagem SMS usando a classe **Services_Twilio**. O **de** número é fornecido por Twilio para contas de avaliação para enviar mensagens SMS. O número **Para** deve ser verificado para sua conta de Twilio antes de executar o código.
...
Depois de definir a página PHP para fornecer respostas TwiML, use a URL da página PHP como a URL passada para o método  `Services_Twilio->account->calls->create`. Por exemplo, se você tiver um aplicativo Web denominado **MyTwiML** implantado em um serviço hospedado do Azure e o nome da página PHP for **mytwiml.php**, a URL poderá ser passada para **Services_Twilio->account->calls->create**, conforme mostrado no exemplo a seguir:
...
<h2><a id="AdditionalServices"></a>Como: usar serviços adicionais do Twilio</h2>
