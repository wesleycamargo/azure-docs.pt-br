<properties
   pageTitle="Como configurar as funções para um Serviço de Nuvem do Azure com o Visual Studio | Microsoft Azure"
...
1. Para configurar cada função em seu projeto do Azure por meio do **Gerenciador de Soluções**, abra o menu de atalho para a função no projeto do Azure e escolha **Propriedades**.

    Uma página com o nome da função é exibida no editor do Visual Studio. A página exibe os campos para a guia **Configuração**.

1. Na lista **Configuração do Serviço**, escolha o nome da configuração de serviço que você deseja editar.

    Se quiser fazer alterações em todas as configurações de serviço para essa função, você pode escolher **Todas as Configurações**.
...
1. Escolha o botão **Salvar** na barra de ferramentas para salvar essas alterações no arquivo de configuração de serviço.

## Gerenciar cadeias de conexão para contas de armazenamento

Você pode adicionar, remover ou modificar cadeias de conexão para suas configurações de serviço. Talvez você queira cadeias de conexão diferentes para configurações de serviço diferentes. Por exemplo, você pode querer uma cadeia de conexão local para uma configuração de serviço local com um valor de `UseDevelopmentStorage=true`. Você também pode querer definir uma configuração de serviço de nuvem que use uma conta de armazenamento no Azure.
...
1. Selecione **Cadeia de Conexão** na lista suspensa **Tipo**.
...
1. Para usar o emulador de conta de armazenamento local, selecione o botão de opção **Emulador de armazenamento do Microsoft Azure** e escolha o botão **OK**.

1. Para usar uma conta de armazenamento no Azure, selecione o botão de opção **Sua assinatura** e selecione a conta de armazenamento desejada.

1. Para usar credenciais personalizadas, escolha o botão de opção **Credenciais inseridas manualmente**. Digite o nome da conta de armazenamento e a chave primária ou secundária. Para obter informações sobre como criar uma conta de armazenamento e como inserir os detalhes da conta de armazenamento na caixa de diálogo **Criar Cadeia de Conexão de Armazenamento**, consulte [Configurando serviços necessários para publicar um serviço de nuvem por meio do Visual Studio](vs-azure-tools-setting-up-services-required-to-publish-a-cloud-service-from-visual-studio.md).

1. Para excluir uma cadeia de conexão, selecione a cadeia de conexão e escolha o botão **Remover Configuração**.

1. Escolha o ícone **Salvar** na barra de ferramentas para salvar essas alterações no arquivo de configuração de serviço.

1. Para acessar a cadeia de conexão no arquivo de configuração de serviço, você precisa obter o valor da configuração. O código a seguir mostra um exemplo em que um armazenamento de blob é criado e dados são carregados usando uma cadeia de conexão `MyConnectionString` por meio do arquivo de configuração de serviço quando um usuário escolhe **Button1** na página Default.aspx na função web de um serviço de nuvem do Azure. Adicione o seguinte usando instruções a Default.aspx.cs:
...
1. Abra Default.aspx.cs no modo de exibição de Design e adicione um botão da caixa de ferramentas. Adicione o seguinte código ao método `Button1_Click`. Esse código usa `GetConfigurationSettingValue` para obter o valor do arquivo de configuração de serviço para a cadeia de conexão. Será criado na conta de armazenamento um blob que é referenciado na cadeia de conexão `MyConnectionString` e, por fim, o programa adiciona texto ao blob.
...
1. Na lista **Configuração do Serviço**, escolha a configuração de serviço que você deseja atualizar.
    ...
    >[AZURE.NOTE]Você pode atualizar cadeias de caracteres para uma configuração de serviço específica, mas se precisar adicionar ou excluir uma cadeia de caracteres, você precisa selecionar **Todas as Configurações**.
...
1. Selecione **Cadeia de Caracteres** na lista suspensa **Tipo**.

1. Para adicionar ou alterar o valor da cadeia de caracteres na caixa de texto **Valor**, digite o novo valor.

1. Para excluir uma cadeia de caracteres, selecione a cadeia de caracteres e escolha o botão **Remover Configuração**.
...
1. Adicione o seguinte código para ao método `Button1_Click` para acessar essa cadeia de caracteres da mesma maneira como acessa uma cadeia de conexão. Seu código pode, então, executar alguns códigos específicos com base no valor da cadeia de configurações para o arquivo de configuração de serviço usado.
...
1. Na lista **Configuração de Serviço**, escolha **Todas as Configurações**.
...
1. Na caixa de texto **Tamanho**, digite o tamanho em MB que você precisa ter no armazenamento local.

1. Para remover os dados no armazenamento local quando a máquina virtual para esta função for reciclada, marque a caixa de seleção **Limpar na reciclagem de função**.
...
1. Para salvar essas alterações nos arquivos de configuração de serviço, escolha o ícone **Salvar**.

1. Para acessar o armazenamento local que você adicionou no arquivo de configuração de serviço, você deve obter o valor da configuração de recurso local. Use as seguintes linhas de código para acessar esse valor, crie um arquivo chamado **MyStorageTest.txt** e escreva uma linha de dados de teste nesse arquivo. Você pode adicionar o código ao método `Button_Click` que você usou nos procedimentos anteriores:
...
1. Adicione o seguinte código ao método `Button1_Click`. Isso cria o arquivo no armazenamento local e grava dados de teste no arquivo.
...
1. (Opcional) Para exibir esse arquivo que você criou quando executou seu serviço de nuvem localmente, use as seguintes etapas:
  ...
  1. Execute a função web e clique em **Button1** para certificar-se de que o código dentro de `Button1_Click` seja chamado.

  1. Na área de notificação, abra o menu de atalho para o ícone do Azure e escolha **Mostrar Interface do Emulador de Computação**. A caixa de diálogo **Emulador de Computação do Azure** é exibida.

  1. Selecione a função web.

  1. Na barra de menus, escolha **Ferramentas**, **Abrir repositório local**. É exibida uma janela do Windows Explorer.

  1. Na barra de menus, digite **MyStorageTest.txt** na caixa de texto **Pesquisar** e selecione **Enter** para iniciar a pesquisa.
...
1. Na lista **Configuração do Serviço**, escolha a configuração de serviço que você deseja atualizar ou escolha **Todas as Configurações**.
...
1. Para habilitar o diagnóstico, marque a caixa de seleção **Habilitar Diagnóstico**.
...
1. Para usar uma conta de armazenamento para a cadeia de conexão local, escolha a opção **Credenciais inseridas manualmente**.
    ...
    Para obter mais informações sobre como criar uma conta de armazenamento e como inserir os detalhes da conta de armazenamento na caixa de diálogo Criar Cadeia de Conexão de Armazenamento, consulte [Configurando serviços necessários para publicar um serviço de nuvem por meio do Visual Studio](vs-azure-tools-setting-up-services-required-to-publish-a-cloud-service-from-visual-studio.md).

1. Escolha a conta de armazenamento que você deseja usar em **Nome da conta**.
...
1. Para usar a conta de armazenamento fornecida por você como a cadeia de conexão para diagnóstico (e cache) quando você publica seu serviço de nuvem no Azure, marque a caixa de seleção **Atualizar cadeias de conexão do armazenamento de desenvolvimento para Diagnóstico e Cache com credenciais de conta de armazenamento do Azure ao publicar no Azure**.

1. Escolha o botão **Salvar** na barra de ferramentas para salvar essas alterações no arquivo de configuração de serviço.

## Alterar o tamanho da máquina virtual usada para cada função

Você pode definir o tamanho da máquina virtual para cada função. Só é possível definir o tamanho para todas as configurações de serviço. Se você selecionar um tamanho de máquina menor, menos núcleos de CPU, memória e armazenamento em disco local serão alocados. A largura de banda alocada também será menor. Para obter mais informações sobre os tamanhos e os recursos alocados, consulte [Configurar tamanhos para serviços de nuvem](https://msdn.microsoft.com/library/azure/ee814754).

Os recursos necessários para cada máquina virtual no Azure afetam o custo da execução do seu serviço de nuvem no Azure. Para obter mais informações sobre a cobrança do Azure, consulte [Entenda sua fatura do Azure](billing-understand-your-bill.md).
...
1. Para selecionar o tamanho da máquina virtual para esta função, escolha o tamanho apropriado na lista **Tamanho da VM**.
...
1. Para adicionar um certificado a essa função, escolha o botão **Adicionar Certificado**. Uma nova entrada é adicionada à lista.

1. Na caixa de texto **Nome**, digite o nome do certificado.

1. Na lista **Local do Repositório**, escolha um local para o certificado que você deseja adicionar.

1. Na lista **Nome do Repositório**, escolha o repositório que você deseja usar para selecionar o certificado.

1. Para adicionar o certificado, escolha o botão de reticências (...). A caixa de diálogo **Segurança do Windows** é exibida.

1. Escolha o certificado que você deseja usar na lista e selecione o botão **OK**.
...
1. Para excluir um certificado, escolha o certificado e selecione o botão **Remover Certificado**.
...
1. Escolha o tipo de ponto de extremidade de que você precisa na lista **Tipo**.

1. Escolha o protocolo para o ponto de extremidade de que você precisa na lista **Protocolo**.

1. Se for um ponto de extremidade de entrada, digite na caixa de texto **Porta Pública** a porta pública a ser usada.

1. Na caixa de texto **Porta Privada**, digite a porta privada a ser usada.
