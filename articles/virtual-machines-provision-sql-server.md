<properties linkid="manage-windows-commontask-install-sql-server" urlDisplayName="Instalar o SQL Server" pageTitle="Provisionar uma máquina virtual do SQL Server no Azure " metaKeywords="Tutorial do Azure de criação do SQL Server, vm do SQL Server, configurando o SQL Server" description="Um tutorial que ensina como criar e configurar uma máquina virtual do SQL Server no Azure." metaCanonical="" services="virtual-machines" documentationCenter="" title="Provisionando uma máquina virtual do SQL Server no Azure" authors=""  solutions="" writer="selcint" manager="clairt" editor="tyson"  />
...
1. Faça logon no [Portal de Gerenciamento do Azure (a página pode estar em inglês)](http://manage.windowsazure.com) usando sua conta. Se você não tiver uma conta do Azure, visite [Avaliação gratuita do Azure](http://www.windowsazure.com/pt-br/pricing/free-trial/)
...
3. Na página **Criar uma Máquina Virtual**, selecione uma imagem de máquina virtual que contenha o SQL Server e clique na seta de avanço na parte inferior à direita da página. Para obter as informações mais atualizadas sobre as imagens do SQL Server com suporte no Azure, consulte o tópico [Introdução ao SQL Server em Máquinas Virtuais do Azure (a página pode estar em inglês)](http://go.microsoft.com/fwlink/p/?LinkId=294720) no conjunto de documentação [SQL Server em Máquinas Virtuais do Azure](http://go.microsoft.com/fwlink/p/?LinkId=294719). 

    >[WACOM.NOTE] Se você tiver uma máquina virtual criada usando a imagem da plataforma da edição de avaliação do SQL Server 2012 que estava disponível durante o período de Visualização, não será possível atualizá-la para uma imagem de edição paga por hora na galeria. Você pode escolher uma das duas opções a seguir:
    > - Criar uma nova máquina virtual usando a edição do SQL Server paga por hora da galeria e migrar seus arquivos de banco de dados para essa nova máquina virtual, seguindo as etapas em [Como migrar os arquivos e o esquema de banco de dados do SQL Server entre máquinas virtuais no Azure usando discos de dados](http://go.microsoft.com/fwlink/p/?LinkId=294738) **Ou**

    > - Atualizar uma instância existente da Edição de Avaliação do SQL Server 2012 para outra edição do SQL Server 2012 sob o contrato [License Mobility por meio do Software Assurance no Azure](http://www.windowsazure.com/pt-br/pricing/license-mobility/) seguindo as etapas em [Atualizar para outra edição do SQL Server 2012 (o site pode estar em inglês)](http://msdn.microsoft.com/library/cc707783.aspx). Para obter informações sobre como adquirir a cópia licenciada do SQL Server, consulte [Como comprar o SQL Server (a página pode estar em inglês)](http://www.microsoft.com/pt-br/sqlserver/get-sql-server/how-to-buy.aspx).
...
4. Na página **Configuração de Máquina Virtual**, forneça as seguintes informações:
	...
	>[WACOM.NOTE] O tamanho da máquina virtual é especificado durante o provisionamento:
 	>- Médio é o menor tamanho recomendado para cargas de trabalho de produção. 
    >- O tamanho mínimo recomendado para uma máquina virtual é Grande ao usar o SQL Server Enterprise Edition.
    >- Selecione Grande ou maior ao usar o SQL Server Enterprise Edition. 
   	>- Selecione A6 ao usar o SQL Server 2012 Enterprise para imagem de Data warehouse. 
   	>- Selecione A7 ao usar o SQL Server 2014 Evaluation para imagem de Data warehouse. 
   	>- O tamanho selecionado limita o número de discos de dados que você pode configurar. Para obter informações mais atualizadas sobre tamanhos de máquina virtual disponíveis e o número de discos de dados que você pode anexar a uma máquina virtual, consulte [Tamanhos de Máquina Virtual para o Azure (a página pode estar em inglês)](http://go.microsoft.com/fwlink/p/?LinkId=294819).
...
1. Quando a configuração for concluída, clique no nome da máquina virtual para ir para a página PAINEL. Na parte inferior da página, clique em **Conectar**.
	...
	![Selecionar a página Painel][Image5b]
...
4. Na primeira vez que você fizer logon nessa máquina virtual, talvez seja necessário concluir vários processos, incluindo a configuração da sua área de trabalho, atualizações do Windows e a conclusão das tarefas da configuração inicial do Windows (sysprep). Depois que o Windows sysprep é concluído, a configuração do SQL Server conclui as tarefas de configuração. Essas tarefas podem causar um pequeno atraso enquanto são concluídas. A opção `SELECIONAR @@SERVERNAME` pode não retornar o nome correto até a conclusão da instalação do SQL Server.
...
- [Conectando-se ao Mecanismo de Banco de Dados de seu aplicativo] (#cdea)
...
3. Próximo à parte superior da página, selecione a página **PONTOS DE EXTREMIDADE** e, na parte inferior da página, clique em **ADICIONAR PONTO DE EXTREMIDADE**.
	...
	![Clicar em ADICIONAR PONTO DE EXTREMIDADE][Image28]
...
4. Na página **Adicionar Ponto de Extremidade à Máquina Virtual**, clique em **Adicionar Ponto de Extremidade** e, em seguida, clique na seta de avanço para continuar.
	...
	![Clicar em Adicionar ponto de extremidade][Image29]
...
Abrir portas adicionais para outros componentes conforme necessário. Para obter mais informações, consulte [Configurando o Firewall do Windows para permitir acesso ao SQL Server (a página pode estar em inglês)](http://msdn.microsoft.com/pt-br/library/cc646023.aspx).
...
Para obter mais informações sobre logons do SQL Server, consulte [Criar um logon (a página pode estar em inglês)](http://msdn.microsoft.com/pt-br/library/aa337562.aspx).
...
- Tutorial: Grupos de disponibilidade AlwaysOn no Hybrid IT
...
- Usar o PowerShell para criar uma VM do Azure com o SQL Server BI e o SharePoint 2010
...
- [White paper: Implantar um Microsoft SQL Server Business Intelligence em Máquinas Virtuais do Azure](http://msdn.microsoft.com/library/windowsazure/dn321998.aspx)
