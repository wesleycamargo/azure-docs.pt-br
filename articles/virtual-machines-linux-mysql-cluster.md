...
Estamos prontos para uma simulação de failover automático. Existem duas maneiras de fazer isso: fácil e difícil. A maneira fácil é usar a função de desligamento do cluster: `` crm_standby -U `uname -n` -v on ``. Usando isso no mestre, o escravo assumirá. Não se esqueça de desativá-lo (ou crm\_mon informará que um nó está em espera)
