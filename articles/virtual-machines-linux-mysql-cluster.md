...
-   [Configurando o Corosync][Configurando o Corosync]
-   [Configurando o Pacemaker][Configurando o Pacemaker]
-   [Testando][Testando]
-   [STONITH][STONITH]
...
Existem outras arquiteturas possíveis para MySQL, inclusive NBD Cluster, Percona e Galera, bem como diversas soluções de middleware, inclusive pelo menos uma disponível como uma VM em [Repositório de VM][Repositório de VM]. Como essas soluções podem replicar em unicast X multicast ou difusão e não dependem de armazenamento compartilhado ou de várias interfaces de rede, os cenários devem ser fáceis de implantar no Microsoft Azure.
...
Em `hadb01`, agora estamos prontos para montar os recursos DRBD. Debian e derivativos usam `/var/lib/mysql` como o diretório de dados do MySQL. Como não instalamos o MySQL, iremos criar o diretório e montar o recurso DRBD. No `hadb01`:
...
Você também precisa habilitar a rede para MySQL caso queira fazer consultas externas às VMs, que é a finalidade deste guia. Em ambas as VMs, abra `/etc/mysql/my.cnf` e navegue até `bind-address`, alterando-o de 127.0.0.1 para 0.0.0.0. Depois de salvar o arquivo, emita um `sudo service mysql restart` na primário atual.
...
Estamos prontos para uma simulação de failover automático. Existem duas maneiras de fazer isso: fácil e difícil. A maneira fácil é usar a função de desligamento do cluster: `` crm_standby -U `uname -n` -v on ``. Usando isso no mestre, o subordinado assumirá. Não se esqueça de desativá-lo (ou crm\_mon informará que um nó está em espera)
...
Código de exemplo para o recurso disponível em [GitHub][GitHub]. Você precisa alterar a configuração do cluster adicionando o seguinte a `sudo crm configure`:
...
-   O desempenho da gravação dependerá da interconexão da VM no comutador virtual porque esse é o mecanismo usado pelo DRBD para replicar o dispositivo
  ...
  [Repositório de VM]: http://vmdepot.msopentech.com
