<properties pageTitle="O que &eacute; o SDK de Trabalhos Web do Azure" metaKeywords="Azure Azure WebJobs SDK, Azure storage, Azure queues, Azure tables" description="Uma introdu&ccedil;&atilde;o ao SDK de Trabalhos Web do Azure. Explica o que &eacute; o SDK, cen&aacute;rios t&iacute;picos em que ele &eacute; &uacute;til e exemplos de c&oacute;digo." metaCanonical="" services="web-sites,storage" documentationCenter=".NET" title="O que &eacute; o SDK de Trabalhos Web do Azure" authors="tdykstra" solutions="" manager="wpickett" editor="jimbe" />
...
-   [Usando o SDK de Trabalhos Web fora de Trabalhos Web][Usando o SDK de Trabalhos Web fora de Trabalhos Web]
-   [Usando o SDK de Trabalhos Web para chamar qualquer função][Usando o SDK de Trabalhos Web para chamar qualquer função]
...
## <span id="scenarios"></span></a>Cenários
...
-   Outras tarefas de execução longa que você deseja executar em um thread em segundo plano, como [enviar emails][enviar emails].

## <span id="code"></span></a> Exemplos de código
...
O SDK de Trabalhos Web oferece muitas maneiras de trabalhar com o Armazenamento do Azure. Por exemplo, se o parâmetro decorado por você com o atributo `QueueTrigger` for uma matriz de bytes ou um tipo personalizado, ele será desserializado automaticamente de JSON. É possível usar um atributo `BlobTrigger` para disparar um processo sempre que um novo blob é criado em uma conta do Armazenamento do Azure. (Embora `QueueTrigger` encontre novas mensagens da fila em questão de segundos, `BlobTrigger` pode demorar até 20 minutos para detectar um novo blob. `BlobTrigger` examina se há blobs sempre que o `JobHost` é iniciado e verifica periodicamente os logs do Armazenamento do Azure para detectar novos blobs.)

## <span id="workerrole"></span></a>Usando o SDK de Trabalhos Web fora de Trabalhos Web
...
-   Para obter mais informações, consulte [Como chamar manualmente uma função][Como chamar manualmente uma função] e [Como escrever logs][Como escrever logs]

## <span id="nextsteps"></span></a>Próximas etapas

Para obter mais informações sobre o SDK de Trabalhos Web, consulte [Recursos recomendados para Trabalhos Web do Azure][Recursos recomendados para Trabalhos Web do Azure].
  ...
  [Introdução ao SDK de Trabalhos Web]: ../websites-dotnet-webjobs-sdk-get-started/
  [enviar emails]: https://github.com/victorhurdugaci/AzureWebJobsSamples/tree/master/SendEmailOnFailure
  ...
  [Como chamar manualmente uma função]: ../websites-dotnet-webjobs-sdk-storage-queues-how-to/#manual
  [Como escrever logs]: ../websites-dotnet-webjobs-sdk-storage-queues-how-to/#logs
