<properties urlDisplayName="Website with Storage" pageTitle="Site do Node.js com armazenamento de tabela | Microsoft Azure" metaKeywords="Azure table storage Node.js, Azure Node.js application, Azure Node.js tutorial, Azure Node.js example" description="Um tutorial que ensina a usar o servi&ccedil;o Tabela do Azure para armazenar dados de um aplicativo de n&oacute; hospedado em um site do Azure." metaCanonical="" services="web-sites,storage" documentationCenter="nodejs" title="Aplicativo Web Node.js que usa o Servi&ccedil;o Tabela do Azure" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
Este tutorial mostra como usar o serviço de Tabela fornecido pelo Gerenciamento de Dados do Azure para armazenar e acessar dados em um aplicativo de [nó][nó] hospedado no Azure. Este tutorial pressupõe que você tenha alguma experiência anterior usando nó e [Git][Git].
...
> [WACOM.NOTE] Muitas das etapas abaixo mencionam o uso da linha de comando. Para essas etapas, use a linha de comando de seu sistema operacional, como **cmd.exe** (Windows) ou **Bash** (Unix Shell). Nos sistemas OS X, você pode acessar a linha de comando por meio do aplicativo Terminal.

## Pré-requisitos

Antes de seguir as instruções deste artigo, verifique se você tem os seguintes itens instalados:

-   [node][nó] versão 0.10.24 ou superior
...
1.  Abra seu navegador e vá ate o [Portal do Azure][Portal do Azure]. Se solicitado, faça login com as informações de assinatura do Azure.

2.  Clique em **+ NOVO** na parte inferior do portal e selecione **Conta de Armazenamento**.

    ![+novo][+novo]

    ![Conta de Armazenamento][Conta de Armazenamento]

3.  Selecione **Criação Rápida** e insira a URL e a Região/Afinidade da conta de armazenamento. Já que esse é um tutorial e não precisa ser replicado globalmente, desmarque **Habilitar Replicação Geográfica**. Por fim, clique em "Criar Conta de Armazenamento".
...
2.  Digite o seguinte para instalar o comando express.
    ...
    > [WACOM.NOTE] O parâmetro '-g' usado durante a instalação do módulo expresso o instala globalmente. Isso é feito para que possamos acessar o comando **express** para gerar o scaffolding do site da Web sem precisar digitar informações adicionais do caminho.\*\*
...
2.  Em seguida, digite o seguinte comando para instalar os módulos [azure][Azure], [node-uuid], [nconf] e [async] localmente, bem como para salvar uma entrada para eles no arquivo **package.json**:
...
2.  No início do arquivo, adicione o seguinte para carregar o módulo azure, defina o nome da tabela, partitionKey e defina as credenciais de armazenamento usadas por este exemplo:
    ...
    > [WACOM.NOTE] nconf carregará os valores de configuração de uma das variáveis de ambiente ou o arquivo **config.json**, que será criado posteriormente.
...
3.  No arquivo app.js, role para baixo até onde for capaz de visualizar a seguinte linha:
    ...
    Substitua as linhas acima pelo código mostrado abaixo. Isso inicializará uma instância de **Tarefa** com uma conexão à sua conta de armazenamento. Isso é transmitido a **TaskList**, que a usará para se comunicar com o serviço de Tabela:
...
1.  Baixe e extraia os arquivos para a [Twitter Bootstrap][1]. Copie o arquivo **bootstrap.min.css** da pasta **bootstrap\\dist\\css** para o diretório **public\\stylesheets** de seu aplicativo de lista de tarefas.
...
3.  Abra um navegador da web e navegue até http://127.0.0.1:3000. Isso deve exibir uma página web semelhante à seguinte:
...
Se você não tiver uma conta do Azure, será possível se inscrever em uma [conta gratuita][Portal do Azure]. Após a inscrição, siga estas etapas para continuar esse tutorial.
...
> [WACOM.NOTE] Para obter mais informações, consulte [Instalar e configurar a interface de linha de comando entre plataformas do Azure][Instalar e configurar a interface de linha de comando entre plataformas do Azure];

> [WACOM.NOTE] Embora as ferramentas de linha de comando tenham sido criadas principalmente para usuários do Mac e do Linux, elas se baseiam em Node.js e devem funcionar em qualquer sistema capaz de executar nós.
...
2.  Use o comando a seguir para criar um novo site do Azure
    ...
    Você será solicitado a fornecer o nome do site e o datacenter onde o site estará localizado. Forneça um nome exclusivo e selecione o datacenter geograficamente próximo à sua localidade.
    ...
    > [WACOM.NOTE] Se este for o primeiro site do Azure de sua assinatura, você será instruído a usar o portal para criá-lo. Para obter mais informações, consulte [Criar e implantar um aplicativo Node.js em um site do Azure].
...
3.  Na seção **configurações do aplicativo**, digite **STORAGE\_NAME** no campo **CHAVE** e o nome da sua conta de armazenamento no campo **VALOR**. Clique na marca de seleção para mover para o próximo campo. Repita este processo para as chaves e os valores a seguir:

    -   **STORAGE\_KEY** - a chave de acesso de sua conta de armazenamento
...
Embora as etapas neste artigo descrevam como usar o Serviço de Tabela para armazenar informações, você também pode usar o MongoDB. Consulte [Aplicativo da web Node.js com o MongoDB][Aplicativo da web Node.js com o MongoDB] para obter mais informações.

## Recursos adicionais

[Ferramenta de linha de comando do Azure para Mac e Linux] [Criar e implantar um aplicativo Node.js em sites do Azure]: /pt-br/documentation/articles/web-sites-nodejs-develop-deploy-mac/
[Publicando sites do Azure com o Git][Publicando sites do Azure com o Git]: /pt-br/documentation/articles/web-sites-publish-source-control/
[Centro de Desenvolvedores do Azure]: /pt-br/develop/nodejs/

  [nó]: http://nodejs.org
  [Git]: http://git-scm.com
  [Uma página da Web que exibe uma lista de tarefas vazia]: ./media/storage-nodejs-use-table-storage-web-site/table_todo_empty.png
  [Portal do Azure]: http://windowsazure.com
  [+novo]: ./media/storage-nodejs-use-table-storage-web-site/plus-new.png
  [Conta de Armazenamento]: ./media/storage-nodejs-use-table-storage-web-site/new-storage.png
  [Criação Rápida]: ./media/storage-nodejs-use-table-storage-web-site/quick-storage.png
  [teclas de acesso]: ./media/storage-nodejs-use-table-storage-web-site/manage-access-keys.png
