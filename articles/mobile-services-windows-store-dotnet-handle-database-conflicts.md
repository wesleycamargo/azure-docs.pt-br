...
3. No MainPage.xaml.cs, adicione a seguinte diretiva `using` à parte superior da página.
...
3. No MainPage.xaml.cs, adicione a definição do método `ResolveConflict()` referenciado em `UpdateToDoItem()`. Observe que para resolver o conflito, você define a versão do item local como a versão atualizada do servidor antes de confirmar a decisão do usuário. Caso contrário, você encontrará o conflito continuamente.
...
##Tratando automaticamente a resolução de conflitos em scripts de servidor
...
Este tutorial demonstrou como habilitar um aplicativo da Windows Store para tratar conflitos de gravação ao trabalhar com dados nos Serviços Móveis. Em seguida, considere a opção de concluir um dos seguintes tutoriais da Windows Store:
...
[Adicionar autenticação ao seu aplicativo]: /develop/mobile/tutorials/get-started-with-users-dotnet
