...
2. No Gerenciador de Soluções do Visual Studio, abra MainPage.xaml e substitua a definição de `ListView` pela `ListView` mostrada abaixo e salve a alteração.
...
3. Em MainPage.xaml.cs, adicione o seguinte `usando a diretiva` na parte superior da página.

		using System.Threading.Tasks;


4. No Gerenciador de Soluções do Visual Studio, abra MainPage.xaml.cs. Adicione o manipulador de eventos à MainPage para o evento `LostFocus` de TextBox como mostrado abaixo.
...
4. Em MainPage.xaml.cs, adicione a definição do método `UpdateToDoItem()` da MainPage referenciado no manipulador de eventos, conforme mostrado abaixo.
...
5. Execute o aplicativo **todolist** nas duas máquinas. Altere o `text` de TodoItem no último item na instância 2. Em seguida, clique em outra caixa de texto para que o manipulador de eventos `LostFocus` atualize o banco de dados.
...
6. Na instância 1 do aplicativo, insira um valor diferente para a propriedade do último texto. 	Em seguida, clique em outra caixa de texto para que o manipulador de evento `LostFocus` tente atualizar o banco de dados com uma propriedade `__version` incorreta.
