...
1. Confirme se o serviço está sendo executado em seu próprio espaço executando `azds space list` novamente. Primeiro, você observará que uma instância de `mywebapi` agora está em execução no espaço `scott` (a versão em execução no `default` ainda está em execução, mas não está listada). Em segundo lugar, a URL de ponto de acesso de `webfrontend` é prefixada com o texto "scott.s.". Essa URL é exclusiva para o espaço `scott`. A URL especial significa que solicitações enviadas para a "URL de scott" tentarão primeiro ser encaminhadas para os serviços do espaço `scott`, mas, em caso de falha, recorrerão aos serviços do espaço `default`.
