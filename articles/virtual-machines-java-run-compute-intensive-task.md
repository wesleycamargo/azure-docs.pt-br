...
Observe que a funcionalidade do Barramento de Serviço do Azure requer que o certificado de Baltimore CyberTrust Root seja instalado como parte do armazenamento **cacerts** do JRE. Este certificado é automaticamente incluído no JRE usado por este tutorial. Se você não tem este certificado no armazenamento **cacerts** do JRE, consulte [Adicionar um certificado no repositório de certificados de autoridade de certificação Java][add_ca_cert] para obter informações sobre como adicioná-lo (bem como informações sobre como exibir os certificados no seu armazenamento cacerts).
...
1. No seu computador de desenvolvimento (que não tem de ser a máquina virtual que você criou), faça o download do [SDK do Azure para Java](http://azure.microsoft.com/develop/java/).
