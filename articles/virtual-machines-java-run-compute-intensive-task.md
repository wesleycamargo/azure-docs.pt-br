<properties urlDisplayName="TSP on Virtual Machine" pageTitle="Aplicativo Java que requer muitos recursos computacionais em uma máquina virtual - Azure" metaKeywords="Azure virtual machine Java, Azure Java app, Azure Java application" description="Saiba como criar uma máquina virtual do Azure que execute aplicativos Java que requerem muitos recursos computacionais e podem ser monitorados por outro aplicativo Java." metaCanonical="" services="virtual-machines" documentationCenter="Java" title="How to run a compute-intensive task in Java on a virtual machine" authors="robmcm" videoId="" scriptId="" solutions="" manager="wpickett" editor="mollybos" />
...
4. Clique em **Avançar**.
4. Na caixa de diálogo **Configuração da máquina virtual**: Especifique um nome para a máquina virtual. Especifique o tamanho a ser usado para a máquina virtual. Digite um nome para o administrador no campo **Nome de Usuário**.Lembre-se do nome e da senha que você digitará a seguir, você vai usá-los ao fazer logon remotamente na máquina virtual. Digite uma senha no campo **Nova senha** e insira-a novamente no campo **Confirmar**.Esta é a senha da conta do Administrador. Clique em **Avançar**.
5. Na próxima caixa de diálogo **Configuração da máquina virtual**: Para **Serviço de nuvem**, use o padrão **Criar um novo serviço de nuvem**. O valor de **Nome DNS do Serviço de Nuvem** deve ser exclusivo no cloudapp.net.Se necessário, modifique esse valor para que o Azure indique que ele é exclusivo. Especifique uma região, um grupo de afinidade ou uma rede virtual. Para o objetivo deste tutorial, especifique uma região, como **Oeste dos Estados Unidos**. Para **Conta de Armazenamento**, selecione **Usar uma conta de armazenamento gerada automaticamente**. Para **Conjunto de Disponibilidade**, selecione **(Nenhuma)**. Clique em **Avançar**.
5. Na caixa de diálogo **Configuração da máquina virtual** final: Aceite as entradas de ponto de extremidade padrão. Clique em **Concluído**.
...
3.   No painel superior esquerdo do Portal de Gerenciamento, clique no nó do **Barramento de Serviço** e, em seguida, clique no botão **Novo**.  
    ![Service Bus Node screenshot][svc_bus_node]
4.   Na caixa de diálogo **Criar um novo namespace de serviço**, digite um **Namespace** e, em seguida, para certificar-se de que ele é exclusivo, clique no botão **Verificar disponibilidade**.  
...
1. No seu computador de desenvolvimento (que não tem de ser a máquina virtual que você criou), faça o download do [Azure SDK para Java](http://www.windowsazure.com/pt-br/develop/java/).
