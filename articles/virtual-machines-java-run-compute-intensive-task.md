<properties linkid="dev-java-compute-load" urlDisplayName="TSP na máquina virtual" pageTitle="Aplicativo Java de computação intensiva em uma MV - Azure" metaKeywords="Máquina virtual Java do Azure, app Java do Azure, aplicativo Java do Azure" description="Aprenda como criar uma máquina virtual Azure que executa um aplicativo Java de computação intensiva que pode ser monitorado por outro aplicativo Java." metaCanonical="" services="virtual-machines" documentationCenter="Java" title="Como executar uma tarefa com uso intenso de computação em Java em uma máquina virtual" authors="waltpo" videoId="" scriptId="" solutions="" manager="bjsmith" editor="mollybos" />
...
* Como fazer logon remotamente em sua máquina virtual.
*  Como criar um espaço de nomes do Service Bus.
...
![Solver de problemas do Caixeiro Viajante][solver_output]
...
4. Na caixa de diálogo **Configuração da máquina virtual**:
    ...
    4. Digite uma senha no campo **Nova senha** e digite-a novamente no campo **Confirmar**. Esta é a senha da conta do Administrador.
    5. Clique em **Avançar**.
5. Na próxima caixa de diálogo **Configuração da máquina Virtual**:
    1. Para **Serviço em nuvem**, use o padrão **Criar um novo serviço de nuvem**.
    2. O valor para o **Nome DNS do serviço de nuvem** deve ser exclusivo no cloudapp.net. Se necessário, modifique esse valor para que o Azure indique que ele é exclusivo.
    ...
    3. Para **Conjunto de Disponibilidade**, selecione **(Nenhum)**.
    4. Clique em **Avançar**.
5. Na caixa de diálogo final **Configuração da máquina Virtual**:
    1. Aceite as entradas do ponto de extremidade padrão.
...
Para começar a usar filas do Service Bus no Azure, você deve primeiro criar um espaço de nomes de serviço. Um namespace de serviço fornece um contêiner de escopo para endereçamento dos recursos do Service Bus em seu aplicativo.
...
3.  No painel superior esquerdo do Portal de Gerenciamento, clique no nó do **Service Bus** e, em seguida, clique no botão **Novo**.  
...
5.  Depois de verificar se o nome do namespace está disponível, escolha o país ou a região na qual o namespace deve estar hospedado e, em seguida, clique no botão **Criar o Namespace**.  
      
    O namespace que você criou irá aparecer no Portal de Gerenciamento e leva um tempo para ser ativado. Aguarde até que o status esteja **Ativo** para continuar com a próxima etapa.

## Obter as credenciais do gerenciamento padrão do Namespace

Para executar operações de gerenciamento, como a criação de uma fila no novo namespace, você precisa obter as credenciais de gerenciamento para o namespace.

1.  No painel de navegação esquerdo, clique no nó **Service Bus**, para exibir a lista de namespaces disponíveis:   
    ![Imagem de tela do Namespaces disponível][avail_namespaces]
2.  Selecione o namespace que você acabou de criar na lista abaixo:   
    ![Imagem de tela da lista do Namespace][namespace_list]
3.  O painel direito **Propriedades** listará as propriedades para o novo namespace:   
    ![Captura de tela do painel Propriedades][properties_pane]
4.  A **Chave padrão** está oculta. Clique no botão **Exibição** para exibir as credenciais de segurança:   
...
1. Na sua máquina de desenvolvimento (que não tem de ser a máquina virtual que você criou), faça o download do [Azure SDK para Java](http://www.windowsazure.com/pt-br/develop/java/).
2. Crie um aplicativo de console Java usando o código de exemplo no final desta seção. Para fins deste tutorial, usaremos **TSPSolver.java** como o nome de arquivo Java. Modificar os espaços reservados **your\_service\_bus\_namespace**, **your\_service\_bus\_owner** e **your\_service\_bus\_key** para usar o **namespace** do Service Bus, os valores do **emissor padrão** e da **chave padrão**, respectivamente.
...
Execute os aplicativos de computação intensiva, primeiro para criar a fila, depois para solucionar o problema do Caixeiro Viajante, que adicionará a melhor rota atual para a fila do Service Bus. Enquanto o aplicativo de computação intensiva está em execução (ou depois), execute o cliente para exibir os resultados da fila do Service Bus.

### Como executar o aplicativo de computação intensiva
...
7. Você precisará criar a fila do Service Bus antes de executar as permutações do solver TSP. Execute o seguinte comando para criar a fila do Service Bus:

        java -jar TSPSolver.jar createqueue

8. Agora que a fila está criada, você pode executar as permutações do solver TSP. Por exemplo, execute o seguinte comando para executar o solver para 8 cidades. 

        java -jar TSPSolver.jar 8

 Se você não especificar um número, ele será executado para 10 cidades. Como o solver encontra rotas atuais mais curtas, ele irá adicioná-las à fila.

> [WACOM.NOTE]
> Quanto maior o número que você especificar, mais tempo o solver será executado. Por exemplo, a execução de 14 cidades pode levar vários minutos, e a execução de 15 cidades pode levar várias horas. Aumentando a 16 ou mais cidades pode resultar em dias de tempo de execução (eventualmente semanas, meses e anos). Isso ocorre porque o rápido aumento do número de permutações avaliado pelo solver, como o número de cidades, aumenta.
 
### Como executar o aplicativo do cliente de monitoramento
1. Faça logon no seu computador onde você executará o aplicativo do cliente. Isso não precisa ser na mesma máquina executando o aplicativo **TSPSolver**, embora possa ser.
2. Crie uma pasta onde você executará seu aplicativo. Por exemplo, **c:\TSP**.
3. Copie **TSPClient.jar** para **c:\TSP**,
4. Certifique-se de que a pasta da lixeira do JRE esteja na variável do ambiente PATH.
5. Em um prompt de comando, altere os diretórios para c:\TSP.
6. Execute o seguinte comando:

        java -jar TSPClient.jar

    Opcionalmente, especifique o número de minutos de espera entre a verificação da fila, passando um argumento de linha de comando. O período de suspensão padrão para a verificação da fila é de 3 minutos, que serve para que nenhum argumento de linha de comando seja passado para **TSPClient**. Se você quiser usar um valor diferente para o intervalo de suspensão, por exemplo, um minuto, execute:

	    java -jar TSPClient.jar 1

    O cliente será executado até ver a mensagem de uma fila do "Concluído". Observe que, se você executar várias ocorrências do solver sem executar o cliente, você precisará executar o cliente várias vezes para esvaziar completamente a fila. Como alternativa, você pode excluir a fila e depois criá-la novamente. Para excluir a fila, execute o seguinte comando **TSPSolver** (não **TSPClient**):

        java -jar TSPSolver.jar deletequeue

    O solver será executado até terminar de examinar todas as rotas. 

## Como impedir os aplicativos Java
Para os aplicativos cliente e solver, você pode pressionar **Ctrl+C** para sair se você desejar finalizar antes da conclusão normal.
