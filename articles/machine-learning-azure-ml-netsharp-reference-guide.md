...
Uma estrutura de rede neural consiste de ***nós*** organizados em ***camadas*** e conexões ***ponderadas*** (ou ***limites***) entre os nós. As conexões são direcionais, cada conexão tendo um nó de ***origem*** e um a ***nó de*** destino.
...
-   Em uma declaração de camada de entrada, o número de nós é o número especificado pelo valor do parâmetro para **Número de nós ocultos**.
...
-   [][4]<http://www.cs.toronto.edu/~hinton/absps/imagenet.pdf></a> (Seção 3.4)
...
**Alerta:** Os nós na camada de destino correspondem a neurônios que são os nós centrais dos kernels. Por exemplo, se KernelShape[d] é ímpar, então KernelShape[d]/2 corresponde ao nó central do kernel. Se KernelShape[d] é par, o nó central fica em KernelShape[d]/2 - 1. Portanto, se **Padding**[d] é falso, o primeiro e último nós KernelShape[d]/2 não têm nós correspondentes na camada de destino. Para evitar essa situação, defina **Padding** como [true, true, …, true].
...
-   A primeira linha define a camada de entrada chamada de Mydata, que conta com 100 nós, cada um deles representando um recurso nos exemplos de entrada.
...
-   O número total de nós pode ser calculado usando a dimensionalidade declarada da camada, [50, 5, 5] conforme descrito a seguir: **MapCount** * **NodeCount**\[0] * **NodeCount**\[1] * **NodeCount**\[1] = 10 \* 5 \* 5 \* 5
