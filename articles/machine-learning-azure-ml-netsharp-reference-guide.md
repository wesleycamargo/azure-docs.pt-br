...
<!-- ESSE CONTEÚDO FOI REMOVIDO ATÉ QUE ESSE RECURSO TENHA SUPORTE NO PRODUTO
...
Imediatamente após definir a camada treinável, você precisa declarar conexões entre as camadas que definiu.  A declaração do grupo de conexões começa pela palavra-chave **from**, seguida do nome da camada de origem do grupo e do tipo de grupo de conexões a ser criado.   
...
**Alerta**:  Os nós na camada de destino correspondem a neurônios que são os nós centrais dos kernels.  Por exemplo, se KernelShape[d] for ímpar, então KernelShape[d]/2 corresponde ao nó do kernel central.  Se KernelShape[d] for par, o nó central está em KernelShape[d]/2 - 1.  Portanto, se **Padding**[d] for Falso, o primeiro e o último nós KernelShape[d]/2 não têm nós correspondentes na camada de destino.  Para evitar essa situação, defina **Padding** como [true, true, â€¦, true].  
...
-	**Alpha**:  (obrigatório) Especifica um valor de ponto flutuante que corresponde a ***Î±*** na fórmula anterior. 
-	**Beta**:  (obrigatório) Especifica um valor de ponto flutuante que corresponde a ***Î²*** na fórmula anterior. 
