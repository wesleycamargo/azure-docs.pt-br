---
title: "Configurar política SSL no Gateway de Aplicativo do Azure PowerShell | Microsoft Docs"
...
Saiba como configurar versões de política SSL e cipher suites no Gateway de Aplicativo. Você pode selecionar um [lista de políticas predefinidas](#predefined-ssl-policies) que contêm configurações diferentes de versões de política SSL e habilitado os conjuntos de codificação. Você também tem a capacidade de definir um [política personalizada do SSL](#configure-a-custom-ssl-policy) com base em suas necessidades.

## <a name="get-available-ssl-options"></a>Obter as opções de SSL disponíveis

O `Get-AzureRMApplicationGatewayAvailableSslOptions` cmdlet fornece uma lista de políticas predefinidas disponíveis, conjuntos de codificação disponíveis e versões de protocolo que podem ser configuradas. O exemplo a seguir mostra um exemplo de saída da execução do cmdlet.
...
O gateway de aplicativo vem com três políticas predefinidas que podem ser usadas. O cmdlet `Get-AzureRmApplicationGatewaySslPredefinedPolicy` recupera essas políticas. Cada política tem versões de protocolo diferente e conjuntos de codificação habilitados. Essas políticas predefinidas podem ser usadas para configurar rapidamente uma política SSL no seu gateway de aplicativo. Por padrão **AppGwSslPolicy20170401** será selecionada se nenhuma política SSL específica é definida.

Veja a seguir um exemplo do `Get-AzureRmApplicationGatewaySslPredefinedPolicy` em execução.
