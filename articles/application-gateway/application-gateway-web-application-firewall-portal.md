...
1. Na página **Noções Básicas**, insira esses valores para as seguintes configurações de gateway de aplicativo:
   ...
   - Selecione *WAF* para a camada do Gateway de Aplicativo.![Criar um Gateway de Aplicativo](./media/application-gateway-web-application-firewall-portal/application-gateway-create.png)
...
O pool de back-end é usado para rotear solicitações aos servidores back-end que atendem à solicitação. Os pools de back-end podem ter NICs, conjuntos de dimensionamento de máquinas virtuais, IPs públicos, IPs internos, FQDN (nomes de domínio totalmente qualificados) e back-ends multilocatário como Serviço de Aplicativo do Azure. Adicione os destinos de back-end a um pool de back-end.

Neste exemplo, você usa máquinas virtuais como o back-end de destino. Você pode criar máquinas virtuais ou usar as existentes. Aqui, você cria duas máquinas virtuais que o Azure usa como servidores de back-end para o Gateway de Aplicativo. Para fazer isto:

1. Crie uma nova sub-rede, *myBackendSubnet*, na qual as novas VMs serão criadas. 
2. Crie duas novas VMS, *myVM* e *myVM2*, para serem usadas como servidores back-end.
