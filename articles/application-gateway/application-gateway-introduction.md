...
Por exemplo, as solicitações de `http://contoso.com/video/*` são encaminhadas para VideoServerPool, e de `http://contoso.com/images/*` são encaminhadas para ImageServerPool. O DefaultServerPool será selecionado se nenhum dos padrões de caminho forem compatíveis.
...
As solicitações de `http://contoso.com` são encaminhadas para ContosoServerPool, e de `http://fabrikam.com` são encaminhadas para FabrikamServerPool.
...
## <a name="session-affinity"></a>Afinidade da sessão
