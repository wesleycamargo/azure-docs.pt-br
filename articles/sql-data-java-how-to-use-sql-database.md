<properties linkid="develop-java-sql-azure" urlDisplayName="Banco de Dados SQL" pageTitle="Como usar o SQL Azure (Java) - guia de recursos do Azure" metaKeywords="" description="Saiba como usar o Banco de Dados SQL do Azure a partir do código Java. " metaCanonical="" services="sql-database" documentationCenter="Java" title="Como usar o Banco de Dados SQL do Azure em Java" authors="waltpo" solutions="" manager="" editor="mollybos" />

# Como usar o Banco de Dados SQL do Azure em Java

As etapas a seguir mostram como usar o Banco de Dados SQL do Azure com Java. Exemplos de linha de comando são mostrados para simplificar, mas etapas muito semelhantes podem ser apropriadas para aplicativos Web, independentemente de estarem hospedados no local, no Azure ou em outros ambientes. Este guia aborda a criação de um servidor e a criação de um banco de dados a partir do [Portal de Gerenciamento do Azure](https://windows.azure.com) Para obter informações sobre como realizar essas tarefas a partir do portal de produção, consulte [Usando o Banco de Dados SQL com Java (a página pode estar em inglês)](http://msdn.microsoft.com/pt-br/library/windowsazure/hh749029.aspx).
...
WHERE também pode ser usada ao recuperar contagens, atualizar linhas ou excluir linhas.
