<properties urlDisplayName="Get started with notification hubs" pageTitle="Introdução aos Hubs de Notificação do Azure" metaKeywords="" description="Learn how to use Azure Notification Hubs to push notifications." metaCanonical="" services="notification-hubs" documentationCenter="Mobile" title="Getting Started with Notification Hubs" authors="glenga" solutions="" manager="dwrede" editor="dwrede" />
...
Neste tutorial, você cria um aplicativo da Windows Store em branco que recebe notificações por push usando o serviço WNS (Notificação por Push do Windows). Ao concluir, você poderá difundir notificações por push a todos os dispositivos que executam seu aplicativo usando o hub de notificação.
...
+ Microsoft Visual Studio Express 2013 para Windows com Atualização 2<br/>Esta versão do Visual Studio é necessária para criar um projeto do aplicativo universal. Se você apenas quer criar um aplicativo da Windows Store, você precisa do Visual Studio 2012 Express para Windows 8.
...
2. Procure `WindowsAzure.Messaging.Managed` e clique em **Instalar**, selecione todos os projetos na solução e aceite os termos de uso. 
...
5. Na parte superior do manipulador de eventos **OnLaunched** no App.xaml.cs, adicione a seguinte chamada ao novo método **InitNotificationsAsync**:
...
7. Em seguida, adicione a seguinte linha em seu método **Main**:
...
8. Com o conjunto do aplicativo de console como projeto de inicialização, pressione a tecla **F5** para executar o aplicativo. 
	...
	Você receberá uma notificação do sistema em todos os dispositivos registrados. Clicar ou tocar na barra de notificação do sistema carrega o aplicativo.
