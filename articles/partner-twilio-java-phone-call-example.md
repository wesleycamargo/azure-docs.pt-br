<properties urlDisplayName="How to Make a Phone Call from Twilio in Java" pageTitle="Como fazer uma chamada telefônica por meio da Twilio (Java) - Azure" metaKeywords="chamada da Twilio no Azure, site de chamada do Twilio, Azure Twilio Java" description="Saiba como fazer uma chamada telefônica de uma página da Web usando o Twilio em um aplicativo Java no Azure." metaCanonical="" services="" documentationCenter="Java" title="How to Make a Phone Call Using Twilio in a Java Application on Azure" authors="MicrosoftHelp@twilio.com; robmcm" videoId="" scriptId="" solutions="" manager="twilio" editor="mollybos" />
...
6. Verifique se seu keystore de cacerts contém o certificado da Equifax Secure Certificate Authority com impressão digital MD5 67:CB:9D:C0:13:24:8A:82:9B:B2:17:1E:D1:1B:EC:D4 (o número de série é 35:DE:F4:CF e a impressão digital SHA1 é D2:32:09:AD:23:D3:14:23:21:74:E4:0D:7F:9D:62:13:97:86:63:3A). Esse é o certificado da CA (autoridade de certificação) do serviço [https://api.twilio.com][twilio_api_service], que é chamado quando você usa APIs da Twilio. Para obter informações sobre como adicionar esse certificado de autoridade de certificação a seu repositório de cacerts de JDK, consulte [Adicionando um certificado ao repositório de certificados de autoridade de certificação do Java][add_ca_cert].
...
Quando estiver pronto para implantar no Azure, recompile para implantação na nuvem, implante no Azure e execute http://*your_hosted_name*.cloudapp.net/TwilioCloud/callform.jsp no navegador (substitua se valor por *your_hosted_name*).
...
* Você pode usar **RoleEnvironment.getConfigurationSettings** para recuperar a ID da conta da Twilio e o token de autenticação de definições de configuração da implantação, em vez de embutir os valores no makecall.jsp. Para obter informações sobre a classe **RoleEnvironment**, consulte [Usando a biblioteca de tempo de execução do Azure no JSP][azure_runtime_jsp] e a documentação do pacote de tempo de execução do serviço Azure em [http://dl.windowsazure.com/javadoc][azure_javadoc].
