<properties linkid="dev-nodejs-getting-started" urlDisplayName="Serviço de Nuvem" pageTitle="Guia de introdução ao Node.js - Tutorial do Azure" metaKeywords="Introdução ao node. js do Azure, tutorial do Node.js do Azure, tutorial do Node.js do Azure" description="Um tutorial completo que ajuda você a desenvolver um aplicativo da Web Node.js somples e implantá-lo no Azure." metaCanonical="" services="cloud-services" documentationCenter="Node.js" title="Criar e implantar um aplicativo Node.js para um Serviço de Nuvem do Azure" authors=""  solutions="" writer="larryfr" manager="" editor=""  />
...
# Criar e implantar o aplicativo Node.jc para um Serviço de Nuvem do Azure

Na conclusão deste guia, você terá um aplicativo simples do Node.js em execução
em um Serviço de Nuvem do Azure. Serviços de Nuvem são os blocos de construção de
aplicativos de nuvem dimensionáveis no Azure. Eles permitem a separação e o gerenciamento
independente e o dimensionamento dos componentes de front-end e back-end de seu aplicativo.  Os serviços de nuvem
fornecem uma máquina virtual exclusiva robusta para hospedar cada função confiável.

Para obter mais informações sobre serviços de nuvem e como eles são comparados aos Sites do Azure e máquinas virtuais, consulte [Sites do Azure, serviços de nuvem e VMs: quando usar o quê?](http://msdn.microsoft.com/pt-br/library/windowsazure/jj218759.aspx)

<div class="dev-callout"><strong>Procurando desenvolver um site simples?</strong>
<p>Se seu cenário envolve apenas um site de front-end simples, considere <a href="../create-a-website-(mac)">usar um Site do Azure leve.</a> Você pode atualizar facilmente para um serviço de nuvem conforme o site cresce e suas necessidades mudam.</p>
</div>
<br />

Seguindo este tutorial, você irá criar um aplicativo da Web simples hospedado dentro de uma função Web. Você
usará o emulador de computação para testar o aplicativo localmente e, em seguida,
implantará usando ferramentas de linha de comando do PowerShell.
...
1. No **Menu Iniciar** ou **Tela Iniciar**, pesquise por **PowerShell do Azure**. Por fim, clique com o botão direito no **PowerShell do Azure** e selecione **Executar como Administrador**.
	...
	![Ícone do Azure PowerShell][powershell-menu]
...
3.  Insira o seguinte cmdlet para criar uma nova solução:
	...
	Para obter mais informações sobre esses arquivos, consulte
        [Visão geral da criação de um serviços hospedado para o Azure][].
...
Uma das ferramentas instaladas pelo SDK do Azure é o emulador de computação
do Azure, que permite testar seu aplicativo localmente. O
emulador de computação simula o ambiente de que seu aplicativo será executado
quando ele é implantado na nuvem. Execute as seguintes etapas para testar o aplicativo no emulador.
...
1.  Publicar o aplicativo usando o cmdlet **Publish-AzureServiceProject**,
...
3.  Cria um novo **serviço de nuvem** se não existir. Um **serviço de nuvem** é o contêiner no qual seu aplicativo é hospedado quando é implantado no Azure. Para obter mais informações, consulte [Visão geral da criação de um serviços hospedado para o Azure][].
...
Depois de implantar seu aplicativo, convém desativá-lo para que você possa evitar custos extras. O Azure cobra as instâncias da função Web por hora do tempo do servidor consumido. O tempo do servidor é consumido quando seu aplicativo é implantado, mesmo se as instâncias não estiverem sendo executadas e estiverem no estado parado.
