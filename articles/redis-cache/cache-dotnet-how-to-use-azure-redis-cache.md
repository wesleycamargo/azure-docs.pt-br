...
Cada camada é diferente em termos de recursos e preços. Para saber mais sobre preços, consulte [Detalhes de preços do cache][Detalhes de preços do cache].

Este guia mostra como usar o cliente [StackExchange.Redis][StackExchange.Redis] usando o código C#. Os cenários abordados incluem a **criação e configuração de um cache**, a **configuração de clientes de cache** e a **adição e remoção de objetos do cache**. Para obter mais informações sobre como usar o Cache Redis do Azure, consulte a seção [Próximas etapas][Próximas etapas]. Para obter um tutorial passo a passo da complicação de um aplicativo Web ASP.NET MVC com o Cache Redis, confira [Como criar um Aplicativo Web com o Cache Redis](cache-web-app-howto.md).
...
> [!IMPORTANT]
> Aviso: nunca armazene credenciais no código-fonte. Para manter esse exemplo simples, eu estou mostrando-lhes no código-fonte. Consulte [Como cadeias de caracteres de aplicativo e cadeias de caracteres de conexão funcionam][Como cadeias de caracteres de aplicativo e cadeias de caracteres de conexão funcionam] para obter informações sobre como armazenar credenciais.
> 
> 
...
> [!NOTE]
> A porta não SSL é desabilitada por padrão para novos caches. Para obter instruções sobre como habilitar a porta não SSL, consulte as [Portas de acesso](cache-configure.md#access-ports).
> 
> 
...
* Confira a [documentação do cliente do cache StackExchange.Redis][documentação do cliente do cache StackExchange.Redis].
  * O Cache Redis do Azure pode ser acessado por meio de muitos clientes Redis e linguagens de desenvolvimento. Para saber mais, confira [http://redis.io/clients][http://redis.io/clients].
* O Cache Redis do Azure também pode ser usado com ferramentas como Redsmin e Redis Desktop Manager.
  * Para obter mais informações sobre o Redsmin, confira [Como recuperar uma cadeia de conexão do Redis do Azure e usá-la com Redsmin][Como recuperar uma cadeia de conexão do Redis do Azure e usá-la com Redsmin].
