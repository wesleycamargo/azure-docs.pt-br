...
Este tutorial mostra como adicionar o CDN do Azure para um [aplicativo web no serviço de aplicativo do Azure](../app-service/app-service-web-overview.md). [A Rede de distribuição de conteúdo (CDN) do Azure](cdn-overview.md) armazena em cache conteúdo Web estático em locais estrategicamente posicionados para fornecer uma taxa de transferência máxima para a distribuição de conteúdo aos usuários. O CDN do Azure também reduz a carga do servidor no seu aplicativo web. 
...
Para concluir a etapa de domínio personalizado deste tutorial, você precisa possuir um domínio personalizado e ter acesso ao registro DNS do seu provedor de domínio. Por exemplo, para adicionar entradas DNS para contoso.com e www.contoso.com, você deve ter acesso para definir as configurações de DNS para o domínio raiz contoso.com.

Se você ainda não tiver um domínio de nome, considere concluir o [Tutorial de domínio do serviço de aplicativo](../app-service/custom-dns-web-site-buydomains-web-app.md) para adquirir um domínio usando o portal do Azure. 
...
Se você não tiver um domínio personalizado, considere concluir o [Tutorial de domínio do serviço de aplicativo](../app-service/custom-dns-web-site-buydomains-web-app.md) para adquirir um domínio usando o portal do Azure. 
...
Na página **Adicionar um domínio personalizado**, você poderá visualizar o nome de host do ponto de extremidade para usar na criação de um registro CNAME. O nome de host é derivado da sua URL de ponto de extremidade do CDN: **&lt;EndpointName>.azureedge.net**. 
...
Navegue até o site do registrador do domínio e localize a seção para criar registros DNS. Por exemplo, esta seção pode ser denominada **Nome de domínio**, **DNS**, ou **Nomear Gerenciamento de Servidor**.
...
Retorne à página **Adicionar domínios personalizados** e insira seu domínio personalizado, incluindo o subdomínio, na caixa de diálogo. Por exemplo, digite *cdn.contoso.com*.   
...
Em um navegador, navegue até o arquivo *index.html* usando o seu domínio personalizado (por exemplo, cdn.contoso.com/index.html) para verificar se o resultado é o mesmo de quando você acessa diretamente &lt;endpointname&gt;azureedge.net/index.html.
