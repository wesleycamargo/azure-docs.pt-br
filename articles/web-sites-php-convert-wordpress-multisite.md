<properties linkid="develop-php-tutorials-convert-wordpress-to-multisite" urlDisplayName="Converte um site WordPress para um multisite" pageTitle="Converte um site WordPress para um multisite" metaKeywords="WordPress, Multisite" description="Saiba como pegar um website no WordPress existente criado através de uma galeria no Azure e convertê-lo em um multisite do WordPress" metaCanonical="" services="web-sites" documentationCenter="PHP" title="Converte um site WordPress para um multisite" authors="" solutions="" manager="" editor="" />





# Converte um site WordPress para um multisite
...
Primeiro, você precisa ativar multissite através do `wp-config.php` arquivo com o **WP\_ALLOW\_MULTISSITE** constante. Há dois métodos para editar os arquivos de site da web: a primeira é por meio de FTP e o segundo até gito. Se você não estiver familiarizado com a instalação de qualquer um desses métodos, consulte os seguintes tutoriais:
...
Abrir o `wp-config.php` de arquivos com o editor de sua preferência e adicione o seguinte acima o `/* que é tudo, parar a edição! Blogs feliz. */` linha.
...
A rede agora está configurada no banco de dados, mas há uma etapa restante para habilitar a funcionalidade de rede. Finalizar o `wp-config.php` configurações e certifique-se de `Web. config` rotas corretamente a cada site.


Depois de clicar na **instalar** no botão o *o programa de instalação de rede* página, WordPress tentará atualizar o `wp-config.php` e `Web. config` arquivos. No entanto, você sempre deve verificar os arquivos para garantir que as atualizações foram bem-sucedidas. Caso contrário, esta tela apresentará as atualizações necessárias. Editar e salvar os arquivos.
...
* Ponto *awverify* para *awverify.O_DOMAIN.azurewebsites.net*
...
É importante observar aqui que Azure sites da web aceita CNAME e registros, no entanto você *deve* usar um registro para habilitar o mapeamento de domínio correto. Um CNAME não pode ser encaminhado para outro CNAME, que é o Azure criada para você com o_DOMAIN.azurewebsites.net.
