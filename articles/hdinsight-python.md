...
> [AZURE.NOTE] As etapas neste artigo se aplicam ao cluster do HDInsight versões 2.1, 3.0 e 3.1.
...
2. A instrução  **SELECT TRANSFORM ... USING 'D:\Python27\python.exe streaming.py'** seleciona dados da **hivesampletable**, , e passa as variáveis clientid, devicemake e devicemodel para o script **streaming.py**.
	...
	> [AZURE.NOTE] A cláusula **USING** especifica o caminho completo para o python.exe, já que ele não está no caminho.
...
2. Use  Use o script PowerShell a seguir para carregar os arquivos **streaming.py** e **jython.py** no servidor. Substitua o nome do seu cluster HDInsight do Azure e o caminho para os arquivos **streaming.py** e **jython.py** nas três primeiras linhas do script.
	...
	> [AZURE.NOTE] É possível encontrar outros métodos para carregar os scripts no documento [Carregar dados para trabalhos do Hadoop no HDInsight](/pt-br/documentation/articles/hdinsight-upload-data/).
...
1. Depois de fazer o upload do arquivo, abra um navegador e navegue até https://YourClusterName.azurehdinsight.net/. Quando for solicitado a informar as credenciais, digite o nome de usuário e a senha de administrador para seu cluster.
	...
	> [AZURE.NOTE] Você também pode usar o link **Gerenciar Cluster** na parte inferior do **Painel** do HDInsight no portal de gerenciamento do Azure para executar o painel do Hive.
