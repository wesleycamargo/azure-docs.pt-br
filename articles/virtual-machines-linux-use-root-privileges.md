...
2. **Somente a chave SSH** - a máquina virtual foi provisionada com um certificado (arquivo `.cer` ou `.pem`) ou com chave SSH, mas sem senha.  Neste caso, o `sudo` **não** solicitará a senha do usuário antes de executar o comando.
...
Nesse caso, o usuário **não** será solicitado para uma senha. Depois de pressionar `<enter>`, `sudo` executará o comando com privilégios de `root`.
