...
-   OpenSSL 1.0 +
-   Openssh 5.3+
-   Utilitários do Filesystem: sfdisk, fdisk, mkfs
...
-   serialconsole: Configura GRUB para marcar ttyS0 (a primeira porta serial) como
     o console de inicialização. Isso garante que os logs de inicialização do kernel são enviados para a
...
Um arquivo de configuração função Web é mostrado abaixo:
...
As várias opções de configuração são descritas em detalhes abaixo.
As opções de configuração são de três tipos: Booliano, Sequência de caracteres e Inteiro.
As opções de configuração Booliano podem ser especificadas como “y” ou “n”.
A palavra-chave especial "Nenhum" pode ser usada para algumas entradas de configuração de tipo de sequência, conforme detalhado abaixo.
...
### Exemplo do arquivo de topologia de função
  ...
  [Repositório de software livre do Github para o agente Linux Azure]: https://github.com/WindowsAzure/WALinuxAgent
