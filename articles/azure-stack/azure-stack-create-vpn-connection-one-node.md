...
Este artigo descreve como criar uma conexão de VPN site a site entre duas redes virtuais em dois ambientes separados do Kit de desenvolvimento na pilha do Azure (ASDK). Quando você configura as conexões, você aprenderá como funcionam os gateways VPN no Azure Stack.
...
* Dois servidores e outros pré-requisitos que atendem aos requisitos de hardware ASDK, conforme descrito em [guia de início rápido: Avaliar o Kit de desenvolvimento do Azure Stack](azure-stack-deploy-overview.md).
...
Para concluir a configuração de conexão, você deve implantar dois ambientes ASDK.

> [!NOTE]
> Para cada ASDK que você implanta, execute as [instruções de implantação](azure-stack-run-powershell-script.md). Neste artigo, são chamados de ambientes de ASDK **POC1** e **POC2**.
...
A tabela a seguir resume a configuração de rede para ambos os ambientes de ASDK. Use o procedimento que aparece após a tabela para adicionar o endereço externo BGPNAT que seja específico para sua rede.

### <a name="network-configuration-table"></a>Tabela de configuração de rede
...
> [!NOTE]
> Os endereços IP BGPNAT externos no ambiente de exemplo são 10.16.167.195 para POC1 e 10.16.169.131 para POC2. Use o procedimento a seguir para determinar os endereços IP BGPNAT externos para seus hosts ASDK e, em seguida, adicioná-los para a tabela anterior de configuração de rede.
...
Agora você pode criar os recursos de rede POC1 que você precisa para definir seus gateways. As instruções a seguir descrevem como criar os recursos usando o portal do usuário do Azure Stack. Você também pode usar o código do PowerShell para criar os recursos.
...
4. Por padrão, o nome da sub-rede é definido como **GatewaySubnet**. As sub-redes de gateway são especiais. Para funcionar corretamente, eles devem usar o **GatewaySubnet** nome.
...
5. Selecione o **rede Virtual** item para escolher uma rede virtual. Selecione **VNET-01** na lista.
6. Selecione o **endereço IP público** item de menu. Quando o **escolher endereço IP público** janela Abrir, selecione **criar novo**.
...
### <a name="sign-in-as-a-tenant-again"></a>Entre novamente como um locatário

Um administrador de serviço pode entrar como um locatário para testar os planos, ofertas e assinaturas que seus locatários podem usar. Se você ainda não tiver uma, [criar uma conta de locatário](azure-stack-add-new-user-aad.md) antes de entrar no.

### <a name="create-virtual-network-and-vm-subnet"></a>Criar rede virtual e sub-rede VM
...
### <a name="create-gateway-subnet"></a>Criar sub-rede de gateway
...
4. O nome da sub-rede é definido como **Sub-rede de Gateway** por padrão. Sub-redes de gateway são especiais e devem ter esse nome específico para funcionar corretamente.
5. No **intervalo de endereços** campo, verifique se o endereço está **10.0.21.0/24**.
6. Selecione **Okey** para criar a sub-rede de gateway.

### <a name="create-virtual-network-gateway"></a>Criar gateway de rede virtual
...
### <a name="create-local-network-gateway-resource"></a>Criar recurso de gateway de rede local

1. No portal do usuário POC2, selecione **+ criar um recurso**.
2. Vá para **Marketplace**e, em seguida, selecione **Networking**.
3. Na lista de recursos, selecione **gateway de rede Local**.
4. Na **nome**, insira **POC1-GW**.
5. Na **endereço IP**, insira o endereço externo BGPNAT POC1 listado anteriormente na tabela de configuração de rede.
6. Na **espaço de endereço**, em POC1, digite o **10.0.10.0/23** espaço de endereço da **VNET-01**.
...
9. Na **chave compartilhada (PSK)**, insira **12345**. Se você escolher um valor diferente, lembre-se de que ele deve corresponder ao valor da chave compartilhada que você criou em POC1. Selecione **OK**.
10. Examine os **resumo** folha e, em seguida, selecione **Okey**.

## <a name="create-a-virtual-machine"></a>Criar uma máquina virtual

Agora crie uma máquina virtual em POC2 e colocá-lo em sua sub-rede VM em sua rede virtual:
...
Como o ASDK é independente e isolada da rede na qual o host físico é implantado, o *externo* rede VIP que os gateways estão conectados não é realmente externa. Em vez disso, a rede VIP está oculto por trás de um roteador que executa a conversão de endereço de rede.

O roteador é uma máquina virtual do Windows Server, chamada **AzS-bgpnat01**, que executa a função de roteamento e os serviços de acesso remoto (RRAS) na infraestrutura de ASDK. Você deve configurar NAT na máquina virtual AzS-bgpnat01 para permitir a conexão de VPN site a site conectar-se em ambas as extremidades.
...
> [!IMPORTANT]
> Você deve concluir este procedimento para ambos os ambientes de ASDK.
...
3. Copie e edite o seguinte script do PowerShell. Para configurar o NAT em cada Kit de desenvolvimento do Azure Stack, execute o script em um Windows PowerShell ISE elevado. No script, adicione valores para o `External BGPNAT address` e `Internal IP address` espaços reservados:
...
Agora que a conexão site a site é estabelecida, você deve validar que você pode obter o tráfego flua através dela. Para validar, entrar em uma das máquinas virtuais que você criou em ambiente ASDK. Em seguida, execute ping a máquina virtual que você criou em outro ambiente.
...
6. Abrir com privilégios elevados **Windows PowerShell** janela.
7. Enter **ipconfig /all**.
8. Um endereço IPv4 é exibido abrangido **10.0.20.0/24**. No ambiente de exemplo, é o endereço **10.0.20.4**, mas seu endereço pode ser diferente.
9. Para criar uma regra de firewall que permite que a máquina virtual para responder a pings, execute o seguinte comando do PowerShell:
...
10. Da máquina virtual em POC2, execute ping a máquina virtual em POC1, através do túnel. Para fazer isso, você pode executar ping o DIP que registrou na VM01. No ambiente de exemplo, isso é **10.0.10.4**, mas não se esqueça de executar ping no endereço que você anotou no laboratório. Você verá um resultado que se parece com o exemplo a seguir:
...
3. No **Conexão** janela, as estatísticas de **dados no** e **dados de saída** aparecem. Na captura de tela a seguir, os números grandes são atribuídos à transferência de arquivo adicionais. Você deve ver alguns valores de diferentes de zero.
