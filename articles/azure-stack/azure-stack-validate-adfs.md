...
ms.locfileid: "49947383"
---
# <a name="validate-adfs-integration-for-azure-stack"></a>Validar a integração do AD FS para o Azure Stack

Use a ferramenta de verificador de preparação do Azure Stack (AzsReadinessChecker) para verificar se seu ambiente está preparado para a integração do AD FS com o Azure Stack. Você deve validar a integração do AD FS antes de começar a integração de data center ou antes de uma implantação do Azure Stack.

Valida o verificador de preparação:

* O *metadados de Federação* contém os elementos XML válidos para a federação.
* O *certificado SSL do ADFS* podem ser recuperados e cadeia de confiança pode ser criada. No carimbo de data / ADFS devem confiar na cadeia de certificados do SSL. O certificado deve ser assinado pelo mesmo *autoridade de certificação* como os certificados de implantação do Azure Stack ou por um parceiro de autoridade raiz confiável. Para obter a lista completa dos parceiros de autoridade raiz confiável, consulte: https://gallery.technet.microsoft.com/Trusted-Root-Certificate-123665ca
* *Certificado de autenticação do ADFS* é confiável e não está de expiração.
...
* A URL de metadados de Federação do AD FS. Por exemplo, `https://adfs.contoso.com/FederationMetadata/2007-06/FederationMetadata.xml`
* O arquivo XML de metadados de Federação. Por exemplo, federationmetadata
...
1. No prompt do PowerShell, execute o seguinte para iniciar a validação. Especifique o valor para **- CustomADFSFederationMetadataEndpointUri** como o URI para os metadados de Federação:

     `Invoke-AzsADFSValidation -CustomADFSFederationMetadataEndpointUri https://adfs.contoso.com/FederationMetadata/2007-06/FederationMetadata.xml`

1. Depois que a ferramenta é executada, examine a saída. Confirme que o status for Okey para os requisitos de integração do AD FS. Uma validação bem-sucedida aparece semelhante ao seguinte.
...
Em ambientes de produção, cadeias de certificados de confiança de uma estação de trabalho de operadores de teste não pode ser considerado totalmente uma indicação da postura de relação de confiança PKI na infraestrutura do Azure Stack. Rede do VIP público do carimbo de pilha do Azure precisa a conectividade com a CRL para a infraestrutura de PKI.
...
**Causa** -PowerShell Autoload Falha ao carregar o módulo de preparação verificador corretamente.

**Resolução** – módulo de importação preparação verificador explicitamente. Copie e cole o código a seguir no PowerShell e atualizar o \<versão\> com o número de versão para a versão atualmente instalada.
