...
As versões de serviço do Azure podem ser diferentes entre o Azure e o Azure Stack. Cada recurso requer o atributo apiVersion, que define os recursos oferecidos. Para garantir a compatibilidade de versão de API no Azure Stack TP2, estas são as versões de API válidas para cada Provedor de Recursos:
...
Algumas funções não estão disponíveis no Azure Stack Technical Preview 2 para a criação de modelos e não devem ser usadas. Essas funções são:

* Skip
* Take

## <a name="resource-location"></a>Local do recurso
Os modelos do Resource Manager usam um atributo de localização para colocar recursos durante a implantação. No Azure, localização é uma região, como o oeste dos EUA ou a América do Sul. No Azure Stack, as localizações são diferentes, pois ele está em seu datacenter.  Para garantir que os modelos sejam transferíveis entre o Azure e o Azure Stack, você deve referenciar a localização do grupo de recursos enquanto implanta recursos individuais. É possível fazer isso usando [resourceGroup().Location](https://github.com/Azure/AzureStack-QuickStart-Templates/blob/master/101-simple-windows-vm/azuredeploy.json#L54), para garantir que os recursos herdem a localização do grupo de recursos.  O seguinte trecho de modelo do Resource Manager é um exemplo do uso dessa função ao implantar uma conta de armazenamento:
