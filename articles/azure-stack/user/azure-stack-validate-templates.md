...
Para validar um modelo, você deve primeiro criar um arquivo de recursos de nuvem e, em seguida, execute a ferramenta de validação. Você usar os seguintes módulos do PowerShell de ferramentas do Azure Stack:

- No **CloudCapabilities** pasta: `AzureRM.CloudCapabilities.psm1` cria um arquivo JSON de recursos de nuvem que representam os serviços e as versões em uma nuvem do Azure Stack.
- No **TemplateValidator** pasta: `AzureRM.TemplateValidator.psm1` usa um arquivo JSON de recursos de nuvem para testar modelos para implantação no Azure Stack.
...
3. Use o `Get-CloudCapabilities` cmdlet para recuperar versões de serviço e crie um arquivo JSON de recursos de nuvem. Se você não especificar **- OutputPath**, o arquivo AzureCloudCapabilities.Json é criado no diretório atual. Use seu real local do Azure:
...
Erros ou avisos de validação de modelo são exibidos no console do PowerShell e gravados em um arquivo HTML no diretório de origem. Captura de tela a seguir está um exemplo de um relatório de validação:
