...
| Rede de virtual do Azure Stack | ApplicationvNet<br>10.100.100.0/23 | ApplicationSubnet <br>10.100.100.0/24 |  |
|  |  | GatewaySubnet <br>10.100101.0/24 |  |
| Gateway de rede Virtual do Azure | Gateway do Azure |  |  |
| Gateway de rede Virtual do Azure Stack | Gateway AzureStack |  |  |
| IP público do Azure | Azure GatewayPublicIP |  | Determinado no momento da criação |
| IP público do Azure Stack | GatewayPublicIP AzureStack |  | Determinado no momento da criação |
| Gateway de rede Local do Azure | S2SGateway AzureStack<br>   10.100.100.0/23 |  | Valor IP público do Azure Stack |
...
4. O **nome** da sub-rede será automaticamente preenchida com o valor 'GatewaySubnet'. Esse valor é necessário para o Azure reconheça a sub-rede como a sub-rede de gateway.
...
5. Selecione **criar** para criar o gateway VPN. As configurações de gateway são validadas e o bloco "Gateway de rede Virtual implantado" é mostrado em seu painel.
...
4. Na **criar gateway de rede local**, especifique os valores para o gateway de rede local, usando nossa **valores de exemplo Tutorial**. Inclua os seguintes valores adicionais.
    ...
    - **Definir as configurações BGP**: Use somente quando configurar o BGP. Caso contrário, não selecione essa opção.
    - **Assinatura**: Verifique se que a assinatura correta está mostrando.
    ...
    - **Local**: selecione o local em que esse objeto será criado. Talvez você queira selecionar que sua rede virtual reside no mesmo local, mas não é necessário fazê-lo.
