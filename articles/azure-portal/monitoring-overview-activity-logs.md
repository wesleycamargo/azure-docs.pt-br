...
O Log de Atividades difere dos [Logs de Diagnóstico](../monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs.md), que são todos os logs emitidos por um recurso. Esses logs fornecem dados sobre a operação desse recurso, em vez de operações nesse recurso.
...
* [Criar um alerta de email ou webhook que dispara um evento do Log de Atividades.](../monitoring-and-diagnostics/insights-auditlog-to-webhook-email.md)
* [Salvá-lo em uma **Conta de Armazenamento** para inspeção manual ou arquivamento](../monitoring-and-diagnostics/monitoring-archive-activity-log.md). Você pode especificar o tempo de retenção (em dias) usando os **Perfis de Log**.
...
* [Transmiti-lo para um **Hub de Eventos**](../monitoring-and-diagnostics/monitoring-stream-activity-logs-event-hubs.md) para ingestão por um serviço de terceiro ou solução de análise personalizada, como o Power BI.
