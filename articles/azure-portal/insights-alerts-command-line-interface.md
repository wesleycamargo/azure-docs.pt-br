<properties
	pageTitle="Usar a CLI (Interface de linha de comando) de plataforma cruzada a fim de criar alertas para serviços do Azure | Microsoft Azure"
	description="Use a interface de linha de comando para criar alertas do Azure, que podem disparar notificações ou automação quando as condições especificadas são atendidas."
	authors="rboucher"
	manager=""
	editor=""
...
# Usar a CLI (Interface de linha de comando) de plataforma cruzada a fim de criar alertas para serviços do Azure 
...
Este artigo mostra como configurar alertas do Azure usando a CLI (Interface de linha de comando).
...
## Criar regras de alerta usando a CLI

1. Conclua os pré-requisitos e entre no Azure. Confira [Exemplos da CLI do Azure Insights](insights-cli-samples.md). Em resumo, instale a CLI e execute os comandos. Eles conectam você, mostram qual assinatura está usando e se preparam para executar comandos do Insights.
...
3. Para criar uma regra, primeiro precisa ter várias informações importantes.
	...
	_PT1M_ é a granularidade da medição disponível (intervalos de 1 minuto). O uso de diferentes granularidades oferece opções diferentes de métrica.
...
5. Para criar o webhook ou enviar email quando um alerta é disparado, primeiro crie o email e/ou os webhooks. Crie a regra imediatamente a seguir. Você não pode associar webhook ou emails a regras já criadas usando a CLI.
...
6. Para criar um alerta que é disparado em uma condição específica no log de atividades, use a forma:
...
7. Você pode verificar se os alertas foram criados corretamente examinando uma regra individual.

	```console
    azure insights alerts rule list myresourcegroup --ruleName myrule
	```

8. Para excluir regras, use um comando com esta forma:
