...
<div class="dev-center-tutorial-selector sublanding"> 
        <a href="/pt-br/documentation/articles/notification-hubs-windows-dotnet-notify-users/" title="Windows Universal">Windows Universal</a><a href="/pt-br/documentation/articles/notification-hubs-aspnet-backend-ios-notify-users/" title="iOS">iOS</a>
...
> [AZURE.NOTE] Este tutorial presume que você criou e configurou seu hub de notificação conforme descrito em [Introdução aos Hubs de Notificação (Android)][Introdução aos Hubs de Notificação (Android)].
...
1.  Siga o tutorial [Introdução aos Hubs de Notificação (Android)][Introdução aos Hubs de Notificação (Android)] para criar e configurar o aplicativo para receber notificações por push do GCM.

2.  Abra o arquivo res/layout/activity\_main.xml e substitua o conteúdo pelo seguinte:
...
3.  Abra o arquivo res/values/strings.xml e adicione as seguintes linhas:
...
4.  Agora, crie uma classe **RegisterClient** no mesmo pacote que a classe **MainActivity**. Certifique-se de ter substituído `{backend endpoint}` pelo ponto de extremidade de back-end obtido na seção anterior.
    ...
    Esse componente implementa as chamadas do REST necessárias para entrar em contato com o back-end do aplicativo, para se registrar para as notificações por push. Ele também armazena localmente os *registrationIds* criados pelo Hub de Notificação, conforme detalhado em [Registrando-se a partir do back-end do seu aplicativo][Registrando-se a partir do back-end do aplicativo]. Observe que ele utiliza um token de autorização armazenado no repositório local quando você clica no botão **Fazer logon e registrar-se**.

5.  Na classe **MainActivity**, remova os campos privados para **NotificationHub** e adicione um campo para **RegisterClient**:

        //private NotificationHub hub;
        private RegisterClient registerClient;

6.  Em seguida, no método **onCreate**, remova a inicialização do campo **hub** e o método **registerWithNotificationHubs**. Depois, adicione as linhas a seguir, que inicializam uma instância da classe **RegisterClient**. O método deve conter as linhas a seguir:
...
7.  Em seguida, adicione os métodos a seguir, certificando-se de ter substituído `{backend endpoint}` pelo ponto de extremidade de back-end obtido na seção anterior.
...
4.  Na interface do usuário do aplicativo Android, clique em **Logon**. Em seguida, clique em **Enviar push**.
  ...
  [Introdução aos Hubs de Notificação (Android)]: /pt-br/documentation/articles/notification-hubs-android-get-started/
