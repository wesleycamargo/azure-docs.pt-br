...
Se sua VM do Azure não estiver disponível para seleção quando se habilita a replicação, pode ser devido à configuração obsoleta da Recuperação de Site deixada na VM do Azure. A configuração obsoleta pode ser deixada em uma VM do Azure nos seguintes casos:
