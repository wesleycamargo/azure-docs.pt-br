<properties
	pageTitle="Conectar-se a cargas de trabalho após o failover com o Azure Site Recovery | Microsoft Azure"
	description="Este artigo discute opções para se conectar a cargas de trabalho após o failover com o Azure Site Recovery"
...
#  Conectar-se a cargas de trabalho após o failover com o Azure Site Recovery
...
Como parte de sua estratégia de BCDR, você precisará certificar-se de que você pode se conectar a cargas de trabalho após o failover. Este artigo descreve estratégias para fazer isso.
...
- Este diagrama supõe que:
	...
	**Antes do failover**
	...
	![Antes do failover](./media/site-recovery-connect-after-failover/network-design2.png)

	**Após o failover**

	![Após o failover](./media/site-recovery-connect-after-failover/network-design3.png)
...
2. Se o administrador define o mesmo pool de endereços IP que é usado no site principal para a rede no local de recuperação a Recuperação de Site alocará o mesmo endereço IP que está alocado para a máquina virtual principal para o endereço de IP de máquina virtual de réplica O endereço de IP é reservado no VMM, mas não está definido como o endereço IP de failover. O endereço IP de failover é definido apenas antes do failover. Esta captura de tela mostra o failover de configurações de TCP/IP para a máquina virtual de réplica (no console do Hyper-V). Essas configurações são replicadas antes do início de failover para a máquina virtual.
	...
	![Definir endereço IP](./media/site-recovery-connect-after-failover/network-design4.png)
...
- Há uma rede lógica baseada em VLAN (rede de aplicativos) que foi criada no servidor VMM. ![Rede lógica](./media/site-recovery-connect-after-failover/network-design5.png)
- Uma rede VM (rede de VM do aplicativo) foi criada usando a rede lógica. ![Rede VM](./media/site-recovery-connect-after-failover/network-design6.png)
...
![Redes do Azure](./media/site-recovery-connect-after-failover/network-design10.png)
...
- Um failover e algumas atualizações DNS manterão os aplicativos acessíveis. Se o servidor DNS estiver configurado para permitir atualizações dinâmicas então as máquinas virtuais seria registrariam-se usando o novo endereço IP quando eles iniciarem após um failover.
	...
	![Redes do Azure](./media/site-recovery-connect-after-failover/network-design11.png)
...
- As máquinas virtuais atualizarão o servidor DNS que estiverem usando depois que forem iniciadas. As entradas DNS normalmente precisam ser alteradas ou liberadas em toda a rede e as entradas em cache nas tabelas de rede precisam ser atualizadas ou liberadas, portanto, não é incomum enfrentar tempo de inatividade enquanto ocorrem essas alterações de estado. Isso pode ser reduzido:
	...
	- Usando [Azure Traffic Manager com a Recuperação de Site](https://azure.microsoft.com/blog/2015/03/03/reduce-rto-by-using-azure-traffic-manager-with-azure-site-recovery/) para aplicativos baseados em Internet.
...
[Esta postagem de blog](https://azure.microsoft.com/blog/2014/09/04/networking-infrastructure-setup-for-microsoft-azure-as-a-disaster-recovery-site/) fornece uma explicação de como configurar a infraestrutura necessária de rede do Azure quando reter endereços IP não for um requisito. Ele mostra como configurar a rede local e no Azure. Depois conclui com instruções para executar um teste de failover e um failover planejado.
