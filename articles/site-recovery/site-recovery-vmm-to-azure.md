<properties
	pageTitle="Configurar a proteção entre um site VMM local e o Azure | Microsoft Azure"
...
Verifique se que você tem os seguintes pré-requisitos em vigor.
...
O diagrama a seguir mostra os diferentes canais e portas de comunicação usados pelo Azure Site Recovery para coordenação e replicação.
...
1. Entre no [Portal do Azure](https://portal.azure.com) por meio do servidor VMM que deseja registrar.
...
5. Em **Região**, selecione a região geográfica para o cofre. As regiões geográficas disponíveis incluem Leste da Ásia, Oeste da Europa, Oeste dos EUA, Leste dos EUA e Sudeste da Ásia.
...
1. Na página **Serviços de Recuperação**, clique no cofre para abrir na página de **Início Rápido**. O **Início Rápido** também pode ser aberto a qualquer momento usando o ícone.

	![Ícone de Inicialização Rápida](./media/site-recovery-vmm-to-azure/ASRE2AVMM_QuickStartIcon.png)

2. No menu de atalho, selecione **Entre um site do Hyper-V local e o Microsoft Azure**.
...
3. Na **Verificação dos pré-requisitos**, selecione para parar o serviço VMM para começar a configuração do Provedor. O serviço para e reiniciará automaticamente quando a configuração finaliza. Se estiver instalando em um cluster do VMM, você deverá parar a função de Cluster.
	...
	![Pré-requisitos](./media/site-recovery-vmm-to-azure/ASRE2AVMM_ProviderPrereq.png)
...
4. No **Microsoft Update** você pode optar por atualizações. Com esta configuração de Provedor habilitada, a atualização será instalada de acordo com a política do Microsoft Update.
	...
	![Atualizações da Microsoft](./media/site-recovery-vmm-to-azure/ASRE2AVMM_ProviderUpdate.png)
...
8. Em **Nome do servidor**, especifique um nome amigável para identificar o servidor VMM no cofre. Em uma configuração de cluster, especifique o nome de função de cluster do VMM.
	...
	![Registros do servidor](./media/site-recovery-vmm-to-azure/ASRE2AVMM_ProviderRegKeyServerName.png)

8. Na sincronização dos **Metadados de nuvem inicial**, selecione se você deseja sincronizar os metadados para todas as nuvens no servidor VMM com o cofre. Esta ação só precisa acontecer uma vez em cada servidor. Se você não quiser sincronizar todas as nuvens, pode deixar essa configuração desmarcada e sincronizar cada nuvem individualmente nas propriedades da nuvem no console VMM.

9. Em **Criptografia de dados para VMs protegidas para o Azure**, especifique um local para salvar um certificado SSL automaticamente gerado para criptografia de dados. Esse certificado é usado se você habilitar a criptografia de dados para uma nuvem protegida pelo Azure no portal de Recuperação de Site do Azure. Mantenha esse certificado protegido. Quando você executa um failover para o Azure, selecione-o para decodificar dados criptografados.

	![Registros do servidor](./media/site-recovery-vmm-to-azure/ASRE2AVMM_ProviderSyncEncrypt.png)

8. Clique em **Registrar** para concluir o processo. Após o registro, os metadados do servidor VMM é recuperado pela Recuperação de Site do Azure. O servidor é exibido na guia **Recursos** da página **Servidores** no cofre.
...
## Etapa 5: instalar o agente dos Serviços de Recuperação do Azure
...
4. Na página **Configurações de instalação**, especifique onde você deseja instalar o Agente e selecione a localização do cache no qual os metadados de backup serão instalados. Então clique em <b>Instalar</b>.
...
2. Na guia **Itens Protegidos**, clique na nuvem que deseja configurar e vá para a guia **Configuração**.
...
5. Defina <b>Criptografar dados armazenados</b> como <b>Desligado</b>. Essa configuração especifica que os dados devem ser criptografados entre o site local e o Azure.
...
Depois de salvar as configurações, é iniciado um trabalho para acompanhar o progresso do mapeamento, que pode ser monitorado na guia **Trabalhos**. As máquinas virtuais de réplica existentes que corresponderem à rede VM de origem serão conectadas às redes do Azure de destino. As máquinas virtuais que estiverem conectadas à rede VM de origem serão conectadas à rede do Azure mapeada após a replicação. Se você modificar um mapeamento existente com uma nova rede, as máquinas virtuais de réplica serão conectadas usando as novas configurações.
...
4. Na guia Configurar da máquina virtual, você pode modificar as seguintes propriedades de rede.
	...
	- Rede da máquina virtual de destino: a rede à qual a máquina virtual se conecta é determinada pelo mapeamento da rede da máquina virtual de origem. No caso de a máquina virtual de origem ter mais de um adaptador de rede, e as redes de origem estarem mapeadas para diferentes redes no destino, o usuário precisa escolher entre uma das redes de destino.
	...
	1. IP de destino: se o adaptador de rede da máquina virtual de origem estiver configurado para usar IP estático, o usuário poderá fornecer o IP da máquina virtual de destino. O usuário pode usar essa funcionalidade para reter o IP da máquina virtual de origem após um failover. Se nenhum IP for fornecido, qualquer IP disponível seria fornecido ao adaptador de rede no momento do failover. Caso o IP de destino fornecido pelo usuário já estar sendo usado por outra máquina virtual em execução no Azure, o failover falhará.
...
2. Salve o nome dos endereços IP que foram alocados à máquina virtual de DNS em estado de failover.
...
1. Após o failover, você poderá ver a réplica de teste da máquina virtual no portal do Azure. Se tiver configurado para máquinas virtuais de acesso a rede local, você pode iniciar uma conexão de área de trabalho remota para a máquina virtual.

    a. Verificar se as máquinas virtuais foram iniciadas com êxito.
...
<p>Na guia **Trabalhos** você exibe os trabalhos, realiza busca detalhada nos detalhes do trabalho e erros, executa consultas de trabalho para recuperar trabalhos que combinam com os critérios específicos, exporta trabalhos para o Excel e reinicia os trabalhos que falharam.</p>

<p>Em **Painel**, você pode baixar as últimas versões dos arquivos de instalação do Provedor e do Agente, obter informações de configuração para o cofre, consultar o número de máquinas virtuais com proteção gerenciada pelo cofre, ver os trabalhos recentes, gerenciar o certificado do cofre e sincronizar novamente as máquinas virtuais.</p>
