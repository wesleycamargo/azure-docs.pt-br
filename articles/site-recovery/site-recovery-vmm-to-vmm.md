...
3. Em **Microsoft Update**, você pode aceitar as atualizações para que as atualizações do Provedor sejam instaladas de acordo com a política do Microsoft Update.
...
5. Quando a instalação terminar, clique em **Registrar** para registrar o servidor no cofre.
...
10. Em **Método de replicação inicial**, se você estiver replicando pela rede, especifique se deseja iniciar a replicação inicial ou agendá-la. Para economizar largura de banda de rede, convém agendá-la fora dos seus horários mais ocupados. Em seguida, clique em **OK**.
...
3. Habilite a replicação para a máquina virtual primária. O Azure Site Recovery e o VMM garantirão que o mesmo host de réplica e a mesma máquina virtual sejam detectados, e o Azure Site Recovery reutilizará e restabelecerá a replicação usando as configurações especificadas.
...
3. Em **Failover de Teste**, selecione **Nenhum**. Com essa opção, você testará que a VM faz o failover conforme o esperado, mas a VM replicada não será conectada a nenhuma rede.
   
    ![Failover de teste](./media/site-recovery-vmm-to-vmm/test-failover1.png)
4. Clique em **OK** para iniciar o failover. Você pode acompanhar o andamento clicando na VM para abrir suas propriedades ou no trabalho **Failover de Teste** em **Configurações** > **Trabalhos** > **trabalhos da Recuperação de Site**.
...
7. Depois de verificar que as VMs iniciam com êxito, clique em **O failover de teste está concluído**. Neste ponto, todos os elementos criados automaticamente pela Recuperação de Site durante o failover de teste são excluídos.
