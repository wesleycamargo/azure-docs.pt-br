...
O Azure Site Recovery exige que o usuário tenha permissão para criar o cofre de chaves nas regiões de destino e para copiar as chaves para a região.
...
Para gerenciar as permissões, navegue para o recurso de cofre de chaves no portal e adicione as permissões necessárias ao usuário. Por exemplo: o guia passo a passo abaixo mostra como habilitá-la para o cofre de chaves "ContosoWeb2Keyvault", que está na região de origem.


-  Navegue até "Página Inicial > Cofres de chaves > ContosoWeb2KeyVault > Políticas de acesso"

![Permissões de cofre de chaves](./media/azure-to-azure-how-to-enable-replication-ade-vms/key-vault-permission-1.png)



- Não há nenhuma permissão de usuário, portanto, adicione a permissão mencionada acima clicando em "Adicionar novo" e o usuário e as permissões
...
Veja [este artigo](#trusted-root-certificates-error-code-151066) para ver a solução de problemas de permissão.
>[!NOTE]
...
9. **Cofres de chaves de criptografia de disco**: por padrão, o Azure Site Recovery cria um novo cofre de chaves na região de destino com nome com sufixo "asr", baseado nas chaves de criptografia de disco da VM de origem. Caso o cofre de chaves criado pelo Azure Site Recovery já exista, ele é reutilizado. Se necessário, escolha um cofre de chaves diferente na lista.
...
Nos cenários abaixo, você será solicitado a atualizar as configurações de criptografia da VM de destino.
  - Você habilitou a replicação do Site Recovery na VM e habilitou o ADE (Azure Disk Encryption) na VM de origem em uma data posterior
  - Você habilitou a replicação do Site Recovery na VM e alterou a chave de criptografia de disco e/ou a chave de criptografia de chaves na VM de origem em uma data posterior
...
## <a name="trusted-root-certificates-error-code-151066"></a>Solucionar problemas de permissões de cofre de chaves durante a replicação de VM do Azure para o Azure

**Causa 1:** Talvez você tenha selecionado um cofre de chaves já criado da região de destino que não tem as permissões necessárias.
Você pode estar selecionando um cofre de chaves já criado na região de destino em vez de permitir a criação pelo Azure Site Recovery. Verifique se o cofre de chaves tem as permissões necessárias, conforme mencionado acima.</br>
*Por exemplo*: Um usuário tenta replicar uma VM que tem um cofre de chaves na região de origem, o "ContososourceKeyvault".
O usuário tem todas as permissões sobre o cofre de chaves da região de origem, mas durante a proteção, ele seleciona um cofre de chaves já criado, "ContosotargetKeyvault", para o qual ele não tem permissão. Então, a proteção gera um erro.</br>
**Como corrigir:** Vá para "Página Inicial > Cofres de chaves > ContososourceKeyvault > Políticas de acesso" e adicione as permissões conforme mostrado acima.

**Causa 2:** Talvez você tenha selecionado um cofre de chaves já criado da região de destino que não tem as permissões de descriptografar-criptografar.
Você pode estar selecionando um cofre de chaves já criado na região de destino em vez de permitir a criação pelo Azure Site Recovery. Verifique se o usuário tem as permissões de descriptografar-criptografar caso você também esteja criptografando a chave na região de origem.</br>
*Por exemplo*: Um usuário tenta replicar uma VM que tem um cofre de chaves na região de origem, o "ContososourceKeyvault".
O usuário tem todas as permissões sobre o cofre de chaves da região de origem, mas durante a proteção, ele seleciona um cofre de chaves já criado, "ContosotargetKeyvault", para o qual ele não tem permissão para descriptografar e criptografar.</br>
**Como corrigir:** Vá para "Página Inicial > Cofres de chaves > ContososourceKeyvault > Políticas de acesso" e adicione as permissões em Permissões de chave > Operações criptográficas.
