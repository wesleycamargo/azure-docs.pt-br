...
description: Este artigo fornece uma visão geral dos componentes e da arquitetura usada ao replicar VMs VMware locais para o Azure com o Azure Site Recovery
...
ms.locfileid: "29677738"
---
# <a name="vmware-to-azure-replication-architecture"></a>Arquitetura de replicação de VMware para o Azure

Este artigo descreve a arquitetura e os processos usados quando você faz a replicação, o failover e a recuperação de VMs (máquinas virtuais) de VMware entre um site VMware local e o Azure usando o [Azure Site Recovery](site-recovery-overview.md).
...
2. No cofre dos Serviços de Recuperação, especifique as configurações de replicação de origem. Como parte desse processo, configure o servidor de configuração local. Para implantar o servidor como uma VM do VMware, faça o download um modelo do OVF preparado e importe-o na VMware para criar a VM.
...
4. Os computadores são replicados de acordo com a política de replicação e uma cópia inicial dos dados de VM é replicada para o Armazenamento do Microsoft Azure.
...
1. É possível fazer failover de uma única máquina ou criar planos de recuperação para fazer failover de várias VMs.
...
2. Depois que os componentes estão em vigor, o failback ocorre em três estágios:
    ...
    a. Estágio 1: proteja novamente as VMs do Azure para que elas comecem a replicação para as VMs de VMware locais.

    b. Estágio 2: execute um failover para o site local.

    c. Etapa 3: depois que as cargas de trabalho fizeram failback, você reabilitará a replicação para as VMs locais.
