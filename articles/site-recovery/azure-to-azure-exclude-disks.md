...
- Você tem o PowerShell `Az` módulo. Se precisar instalar ou atualizar o Azure PowerShell, siga este [Guia para instalar e configurar o Azure PowerShell](/powershell/azure/install-az-ps).
- Você já criou o cofre dos Serviços de Recuperação e fez a proteção de máquinas virtuais, pelo menos, uma vez. Se não, em seguida, fazer isso usando a documentação mencionada [aqui](azure-to-azure-powershell.md).
