...
# Replicação do Hyper-V com o Azure Site Recovery
...
## Configurando o ambiente de origem para replicação entre o local e o Azure
...
### Finalizar proteção na máquina virtual
...
| Categoria | Cenários |
|---------------------------|----------------------------------------------|
| Erro não recuperável | Nenhuma nova tentativa é feita. O status de replicação da máquina virtual é **Crítico** e há necessidade de intervenção do administrador. Alguns exemplos incluem: <ul><li>Cadeia de VHD quebrada</li><li>A máquina virtual de réplica em um estado inválido</li><li>Erro de autenticação de rede</li><li>Erro de autorização</li><li>Máquina virtual que não é encontrada, no caso de um servidor Hyper-V autônomo</li></ul>|
...
## Ciclo de vida de recuperação e de proteção da máquina virtual do Hyper-V
