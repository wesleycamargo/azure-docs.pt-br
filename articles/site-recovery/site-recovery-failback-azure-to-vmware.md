---
title: "Executar failback de máquinas virtuais VMware e servidores físicos para o site local | Microsoft Docs"
...
* Se você realizou o failover de servidores físicos, o failback sempre ocorrerá para uma nova VM do VMware.
  * Antes de realizar failback em um computador físico, observe que:
    * O computador físico protegido voltará como uma Máquina virtual após o failover do Azure para o VMware. Um computador com Windows Server 2008 R2 SP1 se protegido e com failover no Azure não pode passar por failback.
...
5. Se você selecionou uma rede de *implantação clássica* , será necessário criar uma nova VM por meio da galeria do Azure e instalar o servidor em processo nela.
   ...
   2. Instalar o Servidor em processo de acordo com as etapas [fornecidas aqui](site-recovery-vmware-to-azure-classic.md#step-5-install-the-management-server)
...
> [!NOTE]
> Se você quiser instalar um servidor de destino mestre no Linux, siga as instruções do próximo procedimento. Verifique se você usa somente o sistema operacional mínimo CentOS 6.6, para o SO MT.
> 
> 
...
3. Se as máquinas de origem que você está protegendo estiverem executando o sistema de arquivos Linux wit Reiser ou XFS no dispositivo de inicialização ou raiz, baixe e instale outros pacotes da seguinte maneira:
   ...
   * # <a name="yum-install-device-mapper-multipath---this-is-required-to-enable-multipath-packages-on-the-mt-server"></a>yum install device-mapper-multipath Isso é necessário para habilitar pacotes de vários caminhos no servidor do MT.
...
5. Se CS não for acessível do servidor de processo, você poderá verificar a conectividade de seu servidor de processo ao CS por – Telnet ao computador CS na porta 443. Você também pode tentar executar ping do CS do computador PS. Um servidor PS também deve ter uma pulsação quando ele é conectado ao CS.
6. Se você estiver tentando realizar failback para um vCenter alternativo, verifique se seu novo vCenter está descoberto, com o servidor de destino mestre também descoberto. Como sintoma típico, você verá que os armazenamentos de dados não estão acessíveis/visíveis na caixa de diálogo Proteger Novamente.
7. Um computador WS2008R2SP1 protegido como um computador local físico ou virtual não pode passar por failback do Azure para o local.
