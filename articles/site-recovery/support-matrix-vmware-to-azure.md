...
description: "Resume os sistemas operacionais e componentes compatíveis para replicar VMs da VMware no Azure usando o Azure Site Recovery."
...
>
> - Atualizações de versões principais de uma distribuição Linux em uma máquina virtual do VMware protegida pelo Site Recovery ou em um servidor físico não são compatíveis. Ao atualizar o sistema operacional em versões principais (por exemplo, CentOS 6.* para CentOS 7.\*), desabilite a replicação para a máquina, atualize o sistema operacional nela, em seguida, habilite a replicação novamente.
>
...
**Contagem de discos do sistema operacional** | 1 | A verificação de pré-requisitos falha quando não há suporte para ela.
**Contagem de discos de dados** | A contagem será 64 ou menos se você estiver replicando *VMs VMware para o Azure*. A contagem será 16 ou menos se você estiver replicando *VMs Hyper-V para o Azure*. | A verificação de pré-requisitos falha quando não há suporte para ela.
...
**Tipo de VM** | Geração 1.<br/><br/> Geração 2--Windows. | VMs da Geração 2 com um tipo de disco básico de SO (que inclui um ou dois volumes de dados formatados como VHDX) e suporte para menos de 300 GB de espaço em disco.<br></br>Não há suporte para VMs Linux da Geração 2. Para obter mais informações, consulte [Recuperação de desastres para Azure aprimorada](https://azure.microsoft.com/blog/2015/04/28/disaster-recovery-to-azure-enhanced-and-were-listening/).|
