---
title: Replicar máquinas virtuais e servidores físicos do VMware no Azure com o Azure Site Recovery (herdado) | Microsoft Docs
...
> [!NOTE]
> Selecione Standard DS4 ao configurar a proteção para cargas de trabalho que exigem alto desempenho de E/S consistente e baixa latência para hospedar as cargas de trabalho intensivas de E/S usando a [Conta de Armazenamento Premium](../storage/storage-premium-storage.md).
> 
> 
...
6. Se você estiver executando o Linux:
   ...
   4. Extraia os arquivos do instalador compactado executando **tar –xvzf Microsoft-ASR\_UA\_8.4.0.0\_RHEL6-64*** ![Servidor de destino mestre Linux](./media/site-recovery-vmware-to-azure-classic-legacy/linux-tar.png)
...
> [!NOTE]
> Pode demorar até 15 minutos após a conclusão do registro para que o servidor de destino mestre seja listado no portal. Para atualizar imediatamente, clique em **Atualizar** na página **Servidores de Configuração**.
> 
> 
...
5. Observe que talvez você precise reiniciar o servidor para concluir a instalação. Em **Servidor de Configuração** > **Detalhes do Servidor**, verifique se o servidor de processo aparece e foi registrado com êxito no cofre.
...
4. Em **Especificar Recursos de Destino**, selecione os servidores de destino mestre e o armazenamento a serem usados para replicação e selecione se as configurações devem ser usadas para todas as cargas de trabalho. Selecione [Conta de Armazenamento Premium](../storage/storage-premium-storage.md) quando configurar a proteção para cargas de trabalho que exigem alto desempenho de E/S consistente e baixa latência para hospedar as cargas de trabalho intensivas de E/S. Se você quiser usar uma conta de Armazenamento Premium para os discos de carga de trabalho, você precisa usar o Destino mestre da série DS. Mas você não pode usar discos de Armazenamento Premium com Destino mestre que não pertençam às séries DS.
   ...
   > [!NOTE]
   > Não há suporte para a movimentação das contas de armazenamento criadas usando o [novo portal do Azure](../storage/storage-create-storage-account.md) entre os grupos de recursos.
   > 
   > 
...
2. É possível modificar o nome que será dado ao computador no Azure após o failover e o tamanho da máquina virtual do Azure. Você também pode selecionar a rede do Azure à qual o computador será conectado após o failover.
   ...
   > [!NOTE]
   > [Migration of networks](../resource-group-move-resources.md) nos grupos de recursos dentro da mesma assinatura ou nas assinaturas não tem suporte para as redes usadas para a implantação do Site Recovery.
   > 
   > 
