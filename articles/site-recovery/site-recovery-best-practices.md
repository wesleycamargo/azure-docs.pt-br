...
**Máquinas virtuais** | Você precisará de pelo menos uma VM no servidor Hyper-V de origem. As VMs que estão replicando no Azure devem estar em conformidade com os [pré-requisitos de máquina virtual do Azure](#azure-virtual-machine-requirements). | Pelo menos uma VM no servidor Hyper-V de origem. As VMs que estão replicando no Azure devem estar em conformidade com os [pré-requisitos de máquina virtual do Azure](#azure-virtual-machine-requirements). | Pelo menos uma VM na nuvem VMM de origem.
...
**Armazenamento do Azure** | Você precisará de uma [conta de armazenamento do Azure](../storage/storage-redundancy.md#geo-redundant-storage) para armazenar os dados replicados no Azure. Os dados replicados são armazenados no armazenamento do Azure e as VMs do Azure se adaptam quando ocorre failover. | Não aplicável | Você precisará de uma [conta de armazenamento do Azure](../storage/storage-redundancy.md#geo-redundant-storage) para armazenar os dados replicados no Azure. Os dados replicados são armazenados no armazenamento do Azure e as VMs do Azure se adaptam quando ocorre failover.
...
**Replicação SAN** | Não aplicável | Não aplicável | Se você quiser replicar entre dois sites VMM locais usando a replicação SAN, poderá usar o ambiente SAN existente. Veja [supported SAN arrays (matrizes SAN com suporte)](http://social.technet.microsoft.com/wiki/contents/articles/28317.deploying-azure-site-recovery-with-vmm-and-san-supported-storage-arrays.aspx).
...
**Armazenamento do Azure** | Você precisará de uma [conta de armazenamento do Azure](../storage/storage-redundancy.md#geo-redundant-storage) para armazenar os dados replicados no Azure. Os dados replicados são armazenados no armazenamento do Azure e as VMs do Azure se adaptam quando ocorre failover. | Não aplicável
