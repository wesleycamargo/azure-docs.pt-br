...
> [AZURE.NOTE] Este tutorial assume que você criou e configurou o seu hub de notificação conforme descrito em[Introdução aos Hubs de Notificação (Android)](notification-hubs-android-get-started.md). 
> Se você estiver usando Serviços Móveis como seu serviço de back-end, consulte a [Versão dos Serviços Móveis](../mobile-services-javascript-backend-android-push-notifications-app-users.md) deste tutorial.
...
1. Siga o tutorial [Introdução aos Hubs de Notificação (Android)](notification-hubs-android-get-started.md) para criar e configurar seu aplicativo para receber notificações por push da GCM.
...
6. Em seguida, adicione os métodos a seguir, certificando-se de ter substituído  `{backend endpoint}` pelo ponto de extremidade de back-end obtido na seção anterior.
	...
	O retorno de chamada para **Fazer logon** gera um token de autenticação básico com base no nome de usuário e na senha inseridos (observe que isso representa qualquer token que seu esquema de autenticação usar) e usa  `RegisterClient` para chamar o back-end.  O retorno de chamada para **Enviar por push** chama o back-end para disparar uma notificação segura a todos os dispositivos desse usuário. 
