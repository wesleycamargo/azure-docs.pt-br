...
##Como usar modelos de plataforma cruzada
...
Esse requisito força o back-end do aplicativo a produzir cargas diferentes para cada plataforma e torna o back-end responsável por parte da camada de apresentação do aplicativo. Algumas questões abrangem localização e layouts gráficos (especialmente para aplicativos da Windows Store que englobam notificações para vários tipos de blocos).
...
O modelo correspondente para o aplicativo da Windows Store é:
...
##Como usar modelos para personalização
...
O modelo para a previsão de um dia com graus Celsius temperaturas é como segue:
...
##Como registrar modelos

Para registrar usando modelos de instalação (preferencial) ou o modelo de registro, consulte a seção [Gerenciamento de registro](notification-hubs-registration-management.md).

##Linguagem de expressão do modelo
...
| $(prop, n) | Conforme consta acima, mas o texto é explicitamente cortado em n caracteres, por exemplo, $(title, 20) corta o conteúdo da propriedade de título em 20 caracteres. |
| . (prop, n) | Conforme consta acima, mas o texto é sufixado com três pontos quando é cortado. O tamanho total de caracteres cortados e o sufixo não excedem n caracteres .(title, 20) com uma propriedade de entrada de “Esta é a linha de título” resulta em **Este é o título...** |
| %(prop) | Semelhante ao $(name), exceto que a saída é codificada para URI. |
| #(prop) | Usada em modelos JSON (por exemplo, para modelos iOS e Android).<br><br>Essa função funciona exatamente como $(prop) anteriormente especificada, exceto quando usada em modelos JSON (por exemplo, modelos Apple). Nesse caso, se essa função não for delimitada por “{‘,’}” (por exemplo, ‘myJsonProperty’: ‘#(name)’) e for avaliada como um número em formato Javascript, por exemplo, regexp: (0|(&#91;1-9&#93;&#91;0-9&#93;*))(.&#91;0-9&#93;+)?((e|E)(+|-)?&#91;0-9&#93;+)?, então, a saída JSON será um número.<br><br>Por exemplo, ‘badge : ‘#(name)’se torna 'badge': 40 (e não ‘40‘). | | ‘text’ ou “text” | A literal. Os literais contêm texto arbitrário entre aspas simples ou duplas. | | expr1 + expr2 | O operador de concatenação junta duas expressões em uma única cadeia de caracteres.
...
Conforme explicado acima, ao usar concatenação, as expressões devem ser colocadas entre chaves. Por exemplo:
