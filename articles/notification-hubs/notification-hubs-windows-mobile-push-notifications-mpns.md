...
ms.locfileid: "54452472"
---
# <a name="tutorial-push-notifications-to-windows-phone-apps-by-using-azure-notification-hubs"></a>Tutorial: Enviar notificações por push para aplicativos do Windows Phone usando Hubs de Notificação do Azure
...
5. Adicione o seguinte código na parte superior do método `Application_Launching` no `App.xaml.cs`:
   ...
   > [!NOTE]
   > O valor `MyPushChannel` é um índice usado para pesquisar um canal existente na coleção [HttpNotificationChannel](https://msdn.microsoft.com/library/windows/apps/microsoft.phone.notification.httpnotificationchannel.aspx). Se não houver uma, crie uma nova entrada com esse nome.
