...
---
# <a name="tutorial-push-notifications-to-chrome-apps-with-azure-notification-hubs"></a>Tutorial: Enviar notificações por push para aplicativos Chrome com os Hubs de Notificação do Azure
...
> [!NOTE]
> As notificações por push do aplicativo Chrome não são notificações genéricas do navegador - elas são específicas do modelo de extensibilidade do navegador (veja [Visão geral de aplicativos Chrome] para obter detalhes). Além do navegador da área de trabalho, os aplicativos Chrome são executados em celulares (Android e iOS) por meio do Apache Cordova. Para saber mais, confira [Aplicativos do Chrome para celular].
...
3. Forneça um **Nome de Projeto** apropriado e clique no botão **Criar**.
...
5. Anote o **Número do Projeto** na página **Projetos** do projeto que você criou. Você usa o número de projeto como a **ID de Remetente do GCM** no Aplicativo Chrome para se registrar no GCM.

    ![Console do Google Cloud - número do projeto](media/notification-hubs-chrome-get-started/gcm-project-number.png)
6. No painel, selecione **Ir para a visão geral de APIs**.

    ![Vá para o botão de visões gerais de API](media/notification-hubs-chrome-get-started/go-to-apis-overview-button.png)
7. Na página de API e Serviços, selecione **Habilitar APIs e Serviços**.

    ![Habilitar APIs e Serviços](media/notification-hubs-chrome-get-started/enable-apis-and-services.png)
8. Pesquise a lista com a palavra-chave **mensagens em nuvem**. Selecione **Google Cloud Messaging** na lista filtrada.

    ![API do Google Cloud Messaging](media/notification-hubs-chrome-get-started/google-cloud-messaging-api.png)
9. Na página **Google Cloud Messaging**, selecione **Habilitar**.

    ![Habilitar GCM](media/notification-hubs-chrome-get-started/enable-gcm.png)
10. Na página **API e Serviços**, alterne para a guia **Credenciais**. Selecione **Criar credenciais**e, em seguida, selecione **Chave de API**.

    ![Criar botão de chave de API](media/notification-hubs-chrome-get-started/create-api-key-button.png)
11. Na caixa de diálogo **Chave de API criada**, selecione o botão de cópia para copiar a chave para a área de transferência. Salve-a em algum lugar. Você usa esse valor para configurar o hub de notificação na próxima seção para habilitá-lo para enviar notificações por push para o GCM.
...
&emsp;&emsp;6. Selecione **Google (GCM)** na categoria **CONFIGURAÇÕES DE NOTIFICAÇÃO**, insira a **Chave API** para o projeto do GCM e clique em **Salvar**.

&emsp;&emsp;&emsp;&emsp;![Hubs de Notificação do Azure - Google (GCM)](media/notification-hubs-chrome-get-started/configure-gcm-api-key.png)
...
6. Crie um arquivo chamado `register.js` com o código nesta etapa. Esse arquivo especifica o script por trás de `register.html`. Os Aplicativos Chrome não permitem a execução embutida, por isso é necessário criar um script de backup separado para a sua interface do usuário.
   ...
   * `registerWithGCM` é o primeiro manipulador de clique de botão, que faz a chamada `chrome.gcm.register` ao GCM para registrar a instância atual do Aplicativo Chrome.
   * `registerCallback` é a função de retorno de chamada que é chamada quando a chamada de registro do GCM é retornada.
   ...
   * `splitConnectionString` e `generateSaSToken` são os auxiliares que representam a implementação de JavaScript da criação de um token SaS que deve ser enviado em todas as chamadas à API REST. Para saber mais, veja [Conceitos comuns](https://msdn.microsoft.com/library/dn495627.aspx).
...
> [!NOTE]
> Você pode enviar notificações por push com os Hubs de Notificação de qualquer back-end por meio da [interface REST](https://msdn.microsoft.com/library/windowsazure/dn223264.aspx) pública. Confira o [portal de documentação](https://azure.microsoft.com/documentation/services/notification-hubs/) para obter mais exemplos de plataforma cruzada.
...
3. Na janela da console, execute o seguinte comando:
   ...
   Uma referência ao SDK do Barramento de Serviço do Azure com o [pacote NuGet WindowsAzure.ServiceBus é adicionada automaticamente ao projeto.](https://nuget.org/packages/WindowsAzure.ServiceBus/)
...
[Aplicativos do Chrome para celular]: https://developer.chrome.com/apps/chrome_apps_on_mobile
[Criar um registro da API REST NH]: https://msdn.microsoft.com/library/azure/dn223265.aspx
