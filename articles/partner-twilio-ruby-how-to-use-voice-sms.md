<properties linkid="develop-ruby-how-to-twilio-sms-voice-service" urlDisplayName="Serviço de voz/SMS do Twilio" pageTitle="Como usar o Twilio para voz e SMS (Ruby) - Azure" metaKeywords="Twilio Ruby do Azure, chamadas telefônicas do Azure, chamadas telefônicas do Azure, twilio do Azure, SMS do Azure, SMS do Azure, chamadas de voz do Azure, chamadas de voz do azure, mensagens de texto do Azure, mensagens de texto do Azure" description="Saiba como fazer uma chamada telefônica e enviar uma mensagem SMS com o serviço de API do Twilio no Azure. Exemplos de código escritos em Ruby." metaCanonical="" services="" documentationCenter="Ruby" title="Como usar o Twilio para recursos de voz e SMS no PHP" authors="" solutions="" manager="" editor="" />
...
* [Como: adicionar serviços do Twilio](#additional_services)
...
Neste ponto, já é possível executar o comando `ruby web.rb -p 5000`. Isso formará um servidor web pequeno na porta 5000. Você poderá acessar esse aplicativo no navegador, visitando a URL configurada para a sua VM do Azure. Depois que conseguir acessar seu aplicativo web no navegador, você estará pronto para iniciar a compilação de um aplicativo do Twilio.
...
Se você abrir `http://yourdomain.cloudapp.net/make_call` em um navegador, isso disparará a chamada para a API do Twilio para fazer a chamada telefônica. Os dois primeiros parâmetros em `client.account.calls.create` são autoexplicativos: é o número da chamada é `from` e o número da chamada é `to`. 
