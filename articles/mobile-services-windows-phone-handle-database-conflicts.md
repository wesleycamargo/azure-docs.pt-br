...
<a href="/pt-br/documentation/articles/mobile-services-windows-store-javascript-handle-database-conflicts/" title="JavaScript da Windows Store">JavaScript da Windows Store</a>
...
-   [SDK do Windows Phone 8][SDK do Windows Phone 8] em execução no Windows 8.
...
2.  No Gerenciador de Soluções do Visual Studio, abra MainPage.xaml e substitua a definição `phone:LongListSelector` pela ListBox mostrado abaixo e salve a alteração.
...
3.  No Gerenciador de Soluções do Visual Studio, abra o arquivo MainPage.xaml.cs e adicione a seguinte diretiva `using`.
...
1.  No MainPage.xaml.cs, atualize a definição da classe **TodoItem** com o código a seguir para incluir a classe a propriedade do sistema **version** ativando o suporte para a detecção de conflitos de gravação:
...
6.  No emulador da esquerda, atualize o `text` do último TodoItem para **Test Write 1**, clique em outra caixa de texto para que o manipulador de eventos `LostFocus` atualize o banco de dados. A captura de tela abaixo mostra um exemplo.
...
9.  No aplicativo do emulador da direita, tente atualizar o texto do mesmo TodoItem e disparador o evento `LostFocus`. Em resposta ao conflito, o script o resolveu recusando a atualização porque o item já estava concluído.
