...
>[AZURE.NOTE]Este artigo se aplica a conexões ponto a site para redes virtuais criadas no modo clássico de implantação. Neste momento, não há suporte para conexões ponto a site para uma rede virtual criada no modo de implantação do Gerenciador de Recursos do Azure.
...
1. [Criar uma rede virtual e um gateway de VPN](#create-a-virtual-network-and-a-vpn-gateway).
2. [Criar seus certificados](#create-your-certificates).
3. [Configurar o seu cliente VPN](#configure-your-vpn-client).

## Criar uma rede virtual e um gateway de VPN



Uma conexão ponto a site requer uma rede virtual com um gateway de roteamento dinâmico. As etapas a seguir guiará você pela criação de ambos.
...
Os certificados são usados para autenticar clientes VPN para VPNs ponto a site. Este procedimento tem várias etapas. Use os links a seguir para concluir cada etapa na ordem.

1. [Gerar um certificado raiz autoassinado](#generate-a-self-signed-root-certificate) - Somente os certificados raiz autoassinados têm suporte atualmente.
2. [Carregar o arquivo do certificado raiz no Portal do Azure](#upload-the-root-certificate-file-to-the-management-portal).
...
3. O comando no exemplo a seguir criará e instalará um certificado raiz no repositório de certificados Pessoais do seu computador e também criará um arquivo *.cer* correspondente, que você posteriormente carregará para o Portal do Azure.
4. Vá para o diretório no qual deseja que o arquivo .cer fique localizado e execute o comando a seguir, em que *RootCertificateName* é o nome que você deseja usar para o certificado. Se você executar o exemplo a seguir sem alterações, o resultado será um certificado raiz e o arquivo *RootCertificateName.cer* correspondente.
...
### Carregar o certificado raiz no Portal do Azure

1. Quando você gerou um certificado raiz autoassinado no procedimento anterior, criou também um arquivo *.cer*. Agora você vai carregar esse arquivo no Portal do Azure. Observe que o arquivo.cer não contém a chave privada do certificado raiz.
...
1. No Portal do Azure, na página **Painel** da sua rede virtual, navegue até o menu de visualização rápida no canto direito e clique no pacote VPN que pertence ao cliente que você deseja conectar à sua rede virtual. Há suporte para os seguintes sistemas operacionais clientes:
...
1. Copie o arquivo de configuração localmente no computador que você deseja conectar à sua rede virtual e clique duas vezes no arquivo .exe. Depois que o pacote tiver sido instalado, você pode iniciar a conexão VPN. Observe que o pacote de configuração não está assinado pela Microsoft. Convém assinar o pacote usando o serviço de assinatura de sua organização, ou assiná-lo você mesmo usando a ferramenta [SignTool](https://msdn.microsoft.com/library/windows/desktop/aa387764(v=vs.85).aspx). Não há problemas em usar o pacote sem assinatura. No entanto, se o pacote não estiver assinado, será exibido um aviso quando você instalar o pacote.
...
Você pode saber mais sobre a conectividade entre locais da rede virtual neste artigo: [Sobre conectividade segura entre locais da rede virtual](vpn-gateway-cross-premises-options.md).
