...
Este passo a passo presume que você instalou e configurou a [CLI do Azure v. 2.0](/cli/azure/install-az-cli2). Você também precisa ter uma chave pública RSA SSH em `~/.ssh/id_rsa.pub`. Se você não tiver uma, consulte as etapas para [OS X e Linux](../virtual-machines/linux/mac-create-ssh-keys.md) ou para o [Windows](../virtual-machines/linux/ssh-from-windows.md).
