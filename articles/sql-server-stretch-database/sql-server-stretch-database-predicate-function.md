<properties
	pageTitle="Selecione linhas para migrar usando uma função de filtro (Stretch Database) | Microsoft Azure"
...
# Selecionar linhas a migrar pelo uso de uma função de filtro (Stretch Database)
...
>   [AZURE.NOTE] Se você fornecer uma função de filtro que apresente um desempenho ruim, a migração de dados também terá um desempenho ruim. O Stretch Database aplica a função de filtro à tabela usando o operador CROSS APPLY.
...
### Passando nomes de coluna à função de filtro
...
## <a name="addafterwiz"></a>Adicione uma função de filtro após executar o Assistente  
...
## Exemplos de funções de filtro que não são válidas
