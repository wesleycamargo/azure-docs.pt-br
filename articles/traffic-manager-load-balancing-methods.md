<properties 
   pageTitle="Sobre os métodos de balanceamento do Gerenciador de Tráfego"
   description="Este artigo o ajudará a entender os diferentes métodos de balanceamento de carga usados pelo Gerenciador de Tráfego"
...
# Sobre os métodos de balanceamento de carga do Gerenciador de Tráfego

Há três métodos de balanceamento de carga disponíveis no Gerenciador de Tráfego. Cada perfil do Gerenciador de Tráfego pode usar apenas um método de balanceamento de carga por vez, embora você possa selecionar um método de balanceamento de carga diferente para seu perfil a qualquer momento.

É importante observar que todos os métodos de balanceamento de carga incluem monitoramento de ponto de extremidade. Depois de configurar seu perfil do Gerenciador de Tráfego para especificar o método de balanceamento de carga que atende melhor às suas necessidades, defina as configurações de monitoramento. Quando o monitoramento estiver configurado corretamente, o Gerenciador de Tráfego monitorará o estado de seus pontos de extremidade, consistindo em serviços de nuvem e sites, e não enviará tráfego a pontos de extremidade que ele achar que não estão disponíveis. Para obter informações sobre o monitoramento do Gerenciador de Tráfego, consulte [Sobre o monitoramento do Gerenciador de Tráfego](traffic-manager-monitoring.md). 

Os três métodos de balanceamento de carga do Gerenciador de Tráfego são:

- **Failover**: Selecione Failover quando você tiver pontos de extremidade nos mesmos data centers do Azure ou em data centers diferentes (conhecidos como regiões no Portal de Gerenciamento) e quiser usar um ponto de extremidade primário para todo o tráfego, mas fornecer backups caso os pontos de extremidade primário ou de backup estejam indisponíveis. Para obter mais informações, consulte [Método de balanceamento de carga de Failover](#failover-load-balancing-method).

- **Round Robin**: Selecione Round Robin quando desejar distribuir a carga entre um conjunto de pontos de extremidade no mesmo data center ou em data centers diferentes. Para obter mais informações, consulte [Método de balanceamento de carga de Round Robin](#round-robin-load-balancing-method).

- **Desempenho**: Selecione Desempenho quando tiver pontos de extremidade em locais geográficos diferentes e você quiser solicitar aos clientes que usem o ponto de extremidade "mais próximo" em termos de menor latência. Para obter mais informações, consulte [Método de balanceamento de carga de Desempenho](#performance-load-balancing-method).

Observe que os sites do Azure já fornecem failover e funcionalidade de balanceamento de carga de round-robin para sites em um data center, independentemente do modo do site. O Gerenciador de Tráfego permite que você especifique o balanceamento de carga failover e round-robin para sites em data centers diferentes.
...
### Método de balanceamento de carga de failover
...
Ao se configurar o método de balanceamento de carga de Failover, a ordem dos pontos de extremidade selecionados é importante. Usando o Portal de Gerenciamento, você pode configurar a ordem de failover na página Configuração para o perfil.

A Figura 1 mostra um exemplo do método de balanceamento de carga de Failover para um conjunto de pontos de extremidade.
...
### Método de balanceamento de carga de round robin
...
A Figura 2 mostra um exemplo do método de balanceamento de carga de Round Robin para um conjunto de pontos de extremidade.
...
O balanceamento de carga de Round Robin também dá suporte à distribuição ponderada de tráfego de rede. A Figura 3 mostra um exemplo do método de balanceamento de carga de Round Robin ponderado para um conjunto de pontos de extremidade.
...
- Atualização gradual de aplicativo: Alocar um percentual do tráfego para roteá-lo para um novo ponto de extremidade e aumentar gradualmente o tráfego até que ele chegue a 100%.
- Migração de aplicativo para o Azure: Criar um perfil com pontos de extremidade do Azure e externos e especificar o peso do tráfego que é roteado para cada ponto de extremidade.
- Expansão da nuvem para capacidade adicional: Expandir rapidamente uma implantação local para a nuvem colocando-a por trás de um perfil do Gerenciador de Tráfego. Quando precisar de capacidade extra na nuvem, você poderá adicionar ou habilitar mais pontos de extremidade e especificar qual parte do tráfego vai para cada ponto de extremidade.

No momento, você não pode usar o Portal de Gerenciamento para configurar o balanceamento de carga ponderado. O Azure fornece acesso programático a esse método usando a API REST do Gerenciamento de Serviço e os cmdlets do PowerShell do Azure associados.
...
- Se o seu perfil contiver vários pontos de extremidade no mesmo data center, o tráfego direcionado para esse data center será distribuído uniformemente pelos pontos de extremidade que estão disponíveis e íntegros, de acordo com o monitoramento do ponto de extremidade.
- Se todos os pontos de extremidade de um data center estiverem indisponíveis (de acordo com o monitoramento de ponto de extremidade), o tráfego para os pontos de extremidade será distribuído entre todos os outros pontos de extremidade disponíveis especificados no perfil, não para os mais próximos. Isso é para ajudar a evitar uma potencial falha em cascata que pode ocorrer se o ponto de extremidade mais próximo ficar sobrecarregado.
