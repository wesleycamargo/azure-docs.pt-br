...
<a id="pipeline-only"></a>__*__ O Azure Databricks e Azure Data Lake Analytics __somente__ podem ser usados em um pipeline. 

>Você cria os destinos de computação para pipelines de aprendizado de máquina conforme mostrado neste artigo, mas usa esses serviços de computação nas etapas de pipeline, em vez de nos métodos listados aqui.  Além disso, apenas algumas etapas de pipeline usam a configuração de execução descrita neste artigo.  Para obter mais informações sobre como usar destinos de computação em um pipeline, veja [Criar e executar um pipeline de aprendizado de máquina](how-to-create-your-first-pipeline.md).
...
A configuração inicial do backup de um novo ambiente pode levar vários minutos, dependendo do tamanho das dependências necessárias. Desde que a lista de pacotes permaneça inalterada, o conjunto de tempo configuração ocorrerá apenas uma vez.
...
Para ambientes gerenciados pelo usuário, você é responsável por configurar seu ambiente e instalar todos os pacotes de que seu script de treinamento precisa no destino de computação. Se seu ambiente de treinamento já estiver configurado (como em seu computador local), você poderá ignorar a etapa configurar definindo `user_managed_dependencies` como True. O Conda não verificará seu ambiente nem instalará nada para você.
...
É possível criar a Computação do Azure Machine Learning como um destino de computação em tempo de execução. A computação é criada automaticamente para a sua execução. O cluster é dimensionado até o número de **max_nodes** que você especificar na sua configuração de execução. A computação é excluída automaticamente depois de a execução ser concluída.
...
1. **Criar**: Crie uma DSVM antes de usá-la para treinar seu modelo. Para criar esse recurso, veja [Provisionar a Máquina Virtual de Ciência de Dados para Linux (Ubuntu)](https://docs.microsoft.com/en-us/azure/machine-learning/data-science-virtual-machine/dsvm-ubuntu-intro).
...
* [Reutilizar os destinos de computação existentes](#portal-reuse)
...
### <a id="portal-reuse"></a>Reutilizar os destinos de computação existentes
...
* Exiba a referência do SKD da [classe RunConfiguration](https://docs.microsoft.com/en-us/python/api/azureml-core/azureml.core.runconfig.runconfiguration?view=azure-ml-py).
