...
A Máquina Virtual de Ciência de Dados da Microsoft (DSVM) é um poderoso ambiente de desenvolvimento de ciência de dados que permite executar várias tarefas de exploração e modelagem de dados. O ambiente já vem compilado e agrupado com várias ferramentas populares de análise de dados que facilitam começar rapidamente sua análise. A DSVM trabalha de forma muito próxima com muitos serviços do Azure e é capaz de ler e processar dados que já estão armazenados no Azure, no Azure SQL Data Warehouse, Azure Data Lake, Armazenamento do Azure ou Banco de Dados de Documentos. Ela também aproveita as ferramentas de análise como o Aprendizado de Máquina do Azure e o Azure Data Factory.
...
**Pré-requisitos** Você também precisará de uma assinatura do Azure. Você pode se inscrever para uma versão de avaliação gratuita [aqui](https://azure.microsoft.com/free/). Instruções para provisionamento de uma Máquina Virtual de Ciência de Dados no Portal do Azure estão disponíveis em [Criando uma máquina virtual](https://ms.portal.azure.com/#create/microsoft-ads.standard-data-science-vmstandard-data-science-vm).
...
Para R, você pode usar um IDE chamado "Revolution R Enterprise 8.0", que pode ser encontrado no menu Iniciar ou na área de trabalho. A Microsoft forneceu bibliotecas adicionais sobre o Open Source/CRAN-R para permitir a análise escalonável e a capacidade de analisar dados maiores que o tamanho da memória permitido fazendo análise paralela em partes. Veja uma captura de tela de como usar o IDE do R.

![IDE do R](./media/machine-learning-data-science-vm-do-ten-things/RevoIDE.png)

Para Python, você pode usar um IDE como o Visual Studio Community Edition, que tem a extensão PTVS (Ferramentas Python para o Visual Studio) pré-instalada. Por padrão, somente um Python 2.7 básico (sem nenhuma biblioteca de análise como SciKit, Pandas) é configurado no PTVS. Para habilitar o Anaconda Python 2.7 e 3.5, é preciso fazer o seguinte:
...
Agora você está pronto para abrir um projeto e começar a trabalhar!

## 2\. Usando um Notebook Jupyter para explorar e modelar seus dados com o Python ou R

O Notebook Jupyter é um poderoso ambiente que fornece um "IDE" baseado em navegador para exploração e modelagem de dados. É possível usar o Python 2, Python 3 ou R (ambos Open Source e Microsoft R Server). Para iniciar o notebook Jupyter, clique no ícone do menu Iniciar/ícone da área de trabalho chamado **Notebook Jupyter**. Na DSVM, também é possível navegar até "https://localhost:9999/" para acessar o notebook Jupyter. Se for solicitada uma senha, use as instruções encontradas na [página de documentação da DSVM](machine-learning-data-science-provision-vm.md/#how-to-create-a-strong-password-on-the-jupyter-notebook-server) para criar uma senha forte e acessar o notebook Jupyter.

A SER DEFINIDA: CAPTURA DE TELA

Assim que estiver no notebook, você verá um diretório que contém alguns notebooks de exemplo que são pré-empacotados com a DSVM. Você pode clicar no notebook e ver o código. É possível executar cada célula pressionando **SHIFT-ENTER**. Você pode executar o notebook inteiro clicando em **Célula** -> **Executar**.

É possível criar um novo notebook clicando no ícone Jupyter (canto superior esquerdo), clicando no botão **Novo** à direita e escolhendo a linguagem do notebook (também conhecida como kernels). Atualmente, oferecemos suporte ao Python 2.7, Python 3.5 e R. O kernel do R permite programação no Open Source R, bem como no Microsoft R Server escalonável corporativo. Uma vez no notebook, você pode explorar seus dados, criar o modelo e testar o modelo usando suas bibliotecas escolhidas.


## 3\. Criar modelos usando R e Python e operacionalizá-lo no Aprendizado de Máquina do Azure

Depois de criar e validar o modelo, a próxima etapa geralmente é implantá-lo na produção. Isso permite aos aplicativos cliente invocar as previsões do modelo em tempo real ou em lote. O Aprendizado de Máquina do Azure fornece um mecanismo para operacionalizar o modelo criado no R ou Python. Quando você operacionaliza seu modelo no Aprendizado de Máquina do Azure, um serviço Web é exposto, o que permite aos clientes fazer chamadas REST que são passadas nos parâmetros de entrada e recebem previsões do modelo como saídas. Se ainda não tiver se inscrito no AzureML, você poderá obter um acesso de convidado gratuito de 8 horas ou um espaço de trabalho gratuito visitando a página inicial [AzureML Studio](https://studio.azureml.net/) e clicando em "Introdução".

### Criar e operacionalizar modelos criados usando Python  

Carregue o notebook "IrisClassifierPyMLWebService" no Jupyter. Veja um modelo simples criado no Python usando a aprendizagem SciKit, conforme encontrado no notebook.
...
O método usado para implantar os modelos do Python no AzureML é encapsular a previsão de um modelo em uma função e decorá-la com atributos fornecidos pela biblioteca do AzureML, indicando sua ID de espaço de trabalho no AzureML, a Chave de API, os parâmetros de entrada e os parâmetros de retorno.
...
OBSERVAÇÃO: atualmente, a biblioteca do AzureML é compatível apenas com o Python 2.7.

### Criar e operacionalizar modelos R

Você pode implantar modelos R criados na Máquina Virtual de Ciência de Dados ou em algum lugar do AzureML de uma maneira semelhante à realizada para o Python. Primeiramente, crie um arquivo settings.json, conforme abaixo, para fornecer sua ID de espaço de trabalho e o token de autenticação. Escreva um wrapper para a função de previsão do modelo. Em seguida, chame ```publishWebService``` na biblioteca do AzureML passando o wrapper de função. Veja um trecho de código que pode ser usado para publicar um modelo como um serviço Web no AzureML.
...
#### Criar um modelo em R e publicá-lo no AzureML
...
Para administrar os recursos de nuvem e a assinatura do Azure, você pode usar o navegador e apontar para o [Portal do Azure](portal.azure.com). Também é possível usar o Azure PowerShell para administrar os recursos e a assinatura do Azure por meio de um script. Você pode executar o Azure PowerShell de um atalho na área de trabalho ou clicar em "Microsoft Azure Powershell" no menu Iniciar. Veja a [documentação do Microsoft Azure Powershell](../powershell-azure-resource-manager.md) para obter mais informações sobre como você pode administrar seus recursos e assinatura do Azure usando os scripts do Windows PowerShell.


## 5\. Estender o espaço de armazenamento com um sistema de arquivos compartilhado

Os cientistas de dados podem compartilhar grandes conjuntos de dados, códigos, etc., dentro da equipe. O DSVM em si tem cerca de 70 GB de espaço disponível. Para estender o armazenamento, você pode usar o Serviço de Arquivos do Azure e montá-lo na DSVM ou acessá-lo por meio da API REST. O espaço máximo do compartilhamento do Serviço de Arquivos do Azure é de 5 TB e o limite do tamanho de arquivo individual é 1 TB.
...
Agora, você pode acessar essa unidade como qualquer unidade normal na VM.
...
No Visual Studio, você pode fazer a mesma operação de clonagem. Veja a captura de tela abaixo para acessar ferramentas Git e Github no Visual Studio.

![Git no Visual Studio](./media/machine-learning-data-science-vm-do-ten-things/VSGit.PNG)


Mais informações sobre como usar Git para trabalhar com o repositório Github usando vários recursos disponíveis são encontradas em github.com. Você pode achar o [roteiro](https://training.github.com/kit/downloads/github-git-cheat-sheet.pdf) uma referência útil.
...
- ****Verifique se a ferramenta AzCopy pré-instalada, encontrada em ```C:\Program Files (x86)\Microsoft SDKs\Azure\AzCopy\azcopy.exe```, está adicionada à variável do seu ambiente. Para obter mais informações sobre AzCopy, veja a [documentação do AzCopy](../storage/storage-use-azcopy.md)


- **Inicie o Gerenciador de Armazenamento do Azure [aqui](https://azurestorageexplorer.codeplex.com/).**
...
Para mover dados entre seus arquivos locais e o Armazenamento de Blobs, você pode usar o AzCopy na linha de comando ou no PowerShell: `AzCopy /Source:C:\myfolder /Dest:https://<mystorageaccount>.blob.core.windows.net/<mycontainer> /DestKey:<storage account key> /Pattern:abc.txt`

Substitua **C:\\myfolder** pelo caminho em que seu arquivo está armazenado, **mystorageaccount** pelo nome da conta de Armazenamento de Blobs, **mycontainer** pelo nome do contêiner, **chave da conta de armazenamento** pela sua chave de acesso de Armazenamento de Blobs. Você pode encontrar as credenciais da conta de armazenamento no [Portal do Azure](http://portal.azure.com).

![StorageAccountCredential\_v2](./media/machine-learning-data-science-vm-do-ten-things/StorageAccountCredential_v2.png)

Execute o comando AzCopy no PowerShell ou no prompt de comando. Veja um exemplo de uso do comando AzCopy:
...
No Estúdio do Aprendizado de Máquina do Azure, é possível usar um **Módulo de leitor** para ler dados do blob.
...
No Notebook IPython **Disputa de dados de NYC usando o Notebook IPython e o Armazenamento de Blobs do Azure**, você pode usar o pacote **pyodbc** para ler dados diretamente no blob.
...
Você pode ver que os dados são lidos em um quadro de dados:
...
- Crie sua análise do Azure Data Lake no [Portal do Azure](http://portal.azure.com).
...
- Instale o **Gateway de Gerenciamento de Dados** seguindo este [documento](../data-factory/data-factory-move-data-between-onprem-and-cloud.md).

![Azure\_Data\_Gateway\_v2](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Gateway_v2.PNG)


- Compartilhe a pasta local em que seus dados estão armazenados com todos para que o Gateway de Dados do Azure possa acessá-la.
...
**Mover dados da VM para o Data Lake: Azure Data Factory**

O Azure Data Factory é um serviço para criar, agendar e gerenciar pipelines de dados. Podemos usar o Azure Data Factory para mover dados entre armazenamentos diferentes. Crie o [Azure Data Factory](https://azure.microsoft.com/services/data-factory/) no [Portal do Azure](http://portal.azure.com):

![Azure\_Data\_Factory\_Create](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Factory_Create.PNG)

Depois que ele for criado, você poderá criar pipelines para mover dados entre armazenamentos diferentes. Em **Criar e implantar**, você pode especificar os conjuntos de dados a serem transferidos e configurar os pipelines.
...
As principais etapas para mover dados da Máquina Virtual para o Azure Data Lake encontram-se a seguir. Os detalhes sobre como mover dados usando o Azure Data Factory podem ser encontrados [aqui](../data-factory/data-factory-data-movement-activities.md). Os arquivos JSON abaixo estarão na VM do Data Science.

1. **Criar serviços vinculados**
	- Clique em **Novo Repositório de Dados**, escolha **Armazenamento do Azure Data Lake** e insira seus parâmetros e credenciais no arquivo JSON.
	- Clique em **Novo Repositório de Dados**, escolha **Sistema de Arquivos** e insira seus parâmetros e credenciais no arquivo JSON.
2. **Criar conjuntos de dados**
	- Clique em **Novo conjunto de dados**, escolha **Azure Data Lake** e insira o nome do **Serviço Vinculado** e o **caminho da pasta** no arquivo JSON.
	- Clique em **Novo conjunto de dados**, escolha **Arquivo local** e insira o **esquema do conjunto de dados**, o nome do **Serviço Vinculado**, o **nome do arquivo** e o **caminho da pasta** no arquivo JSON.
3. **Criar pipelines**
	- Clique em **Novo pipeline**, especifique os **conjuntos de dados de entrada e saída**, o **tipo de atividade**, etc. no arquivo JSON. 
4. **Criar gateways de dados**
	- Registre a chave do gateway e verifique se o registro está **Registrado** e o status é **Iniciado**.
	- Clique em **Novo gateway de dados** e preencha o **nome do gateway de dados **; a parte **Configurar** será definida automaticamente se seu gateway de dados for instalado corretamente nas etapas anteriores.
...
Os arquivos JSON usados nas etapas acima são:

**Serviços vinculados: AzureDataLakeStoreLinkedService**
...
**Serviços vinculados: OnPremisesFileServerLinkedService**
...
**Conjuntos de dados: OnPremisesFile**
...
**Conjuntos de dados: weiglakenew1**
...
**Conjuntos de dados: vmtodatalake\_tripdata**
...
Depois que o pipeline é criado, você pode examinar o pipeline no **Diagrama**, no painel do Azure Data Factory.


![](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Factory_Find_Diagram.PNG)

Clique na caixa **Diagrama** para ver o pipeline:

![](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Factory_PipeLine_Diagram.PNG)

Você pode monitorar o pipeline de dados em **Conteúdo**->**Conjuntos de dados**->**Monitoramento**

![](./media/machine-learning-data-science-vm-do-ten-things/Azure_Data_Factory_Monitor_v2.PNG)


Também é possível verificar se os dados são movidos para o Azure Data Lake usando o **Gerenciador do Azure Data Lake** no **Visual Studio**.
...
Se os dados residirem no Armazenamento de Blobs, você poderá lê-los diretamente no blob do armazenamento do Azure na consulta U-SQL. Antes de compor a consulta U-SQL, verifique se sua conta de Armazenamento de Blobs está vinculada ao Azure Data Lake. Acesse o **Portal do Azure**, encontre o painel de Análise do Azure Data Lake, clique em **Adicionar Fonte de Dados**, escolha o tipo de armazenamento como **Armazenamento do Azure** e insira o Nome e a Chave da Conta de Armazenamento do Azure. Em seguida, você poderá fazer referência aos dados armazenados na conta de armazenamento.

![](./media/machine-learning-data-science-vm-do-ten-things/Link_Blob_to_ADLA_v2.PNG)


No Visual Studio, você pode ler dados no blob, manipular alguns dados, engenharia de recurso, bem como gerar os dados resultantes para o Azure Data Lake ou Armazenamento de Blobs do Azure. Quando fizer referência aos dados no Armazenamento de Blobs, use ****wasb://**; quando fizer referência aos dados no Azure Data Lake, use ****swbhdfs://**
...
- Personalize os Clusters HDInsight Hadoop do Azure no [Portal do Azure](machine-learning-data-science-customize-hadoop-cluster.md)
...
- Ou você pode seguir este [passo a passo](machine-learning-data-science-process-hive-walkthrough.md) para carregar os dados sobre Táxis de NYC no cluster HDI. As principais etapas incluem:
...
Agora vamos preparar uma amostra de dados (1%) para modelagem. Podemos usar esses dados no módulo de leitor do AML.
...
Você também pode usar o módulo de **leitor** no estúdio para acessar o banco de dados no cluster Hadoop. Insira as credenciais dos clusters HDI e da Conta de Armazenamento do Azure e você poderá criar modelos de aprendizado de máquina usando o banco de dados nos clusters HDI.
...
Depois de importar os dados, você pode ir para Jupyter e abrir o notebook ```DocumentDBSample```, que contém o código python para acessar o Banco de Dados de Documentos e fazer algumas consultas básicas. Aprenda mais sobre o Banco de Dados de Documentos ao visitar a [página de documentação](https://azure.microsoft.com/documentation/learning-paths/documentdb/) do serviço


## 8\. Criar relatórios e painel usando o Power BI Desktop 

Vamos visualizar o arquivo JSON, Volcano, que vimos no exemplo acima do Banco de Dados de Documentos no Power BI para obter informações visuais dos dados. As etapas detalhadas são encontradas no [artigo sobre o Power BI](../documentdb/documentdb-powerbi-visualize.md). As etapas principais são as seguintes:
...
2. Você verá os registros JSON importados como uma lista
3. Em seguida, converta a lista em uma tabela para que o Power BI possa trabalhar com ela
...
5. Você verá que o local é um campo "Registro". Expanda o registro e selecione apenas as coordenadas. As coordenadas são uma coluna de lista
6. Em seguida, você adicionará uma nova coluna para converter a coluna de coordenadas da lista em uma coluna LatLong separada por vírgula concatenando os dois elementos no campo da lista coordenada usando a fórmula ```Text.From([coordinates]{1})&","&Text.From([coordinates]{0})```. 
7. Por fim, converta a coluna ```Elevation``` para Decimal e clique em Fechar e Aplicar.
...
É possível iniciar a criação de relatórios e as visualizações usando o modelo de dados. Você pode seguir as etapas deste [artigo sobre o Power BI](../documentdb/documentdb-powerbi-visualize.md#build-the-reports) para criar um relatório. O resultado final será um relatório semelhante ao que se segue.


A ser definido: ausência da URL da imagem do Relatório sobre Mapas de Vulcões - incorreto.

## 9\. Dimensionar dinamicamente a DSVM para atender às suas necessidades de projeto

Você pode escalar verticalmente a DSVM para atender às necessidades do seu projeto. Se não precisar usar a VM à noite ou nos fins de semana, você poderá desligá-la no [Portal do Azure](https://portal.azure.com). OBSERVAÇÃO: isso incorrerá em cobranças de computação se você usar apenas o botão de desligamento do sistema operacional na VM.

Se precisar lidar com análise em larga escala e precisar de mais capacidade de CPU e/ou memória e/ou disco, você poderá encontrar uma ampla opção de tamanhos de VM em termos de núcleos de CPU, capacidade de memória e tipos de disco (incluindo unidades de estado sólido) que atendam às suas necessidades de computação e orçamentárias. A lista completa de VMs e seus preços de computação por hora estão disponíveis na página [Preços de Máquinas Virtuais do Azure](https://azure.microsoft.com/pricing/details/virtual-machines/).

Da mesma forma, se sua necessidade de capacidade de processamento de VM for reduzida (por exemplo: você moveu uma grande carga de trabalho para um cluster Hadoop ou um Spark), você pode reduzir verticalmente o cluster no [Portal do Azure](https://portal.azure.com) e acessar as configurações da sua instância de VM. Veja uma captura de tela.
...
Empacotamos várias ferramentas que acreditamos que poderão atender a muitas das necessidades de análise comum de dados e pouparão tempo evitando a instalação e a configuração de cada ambiente, pagando apenas por aquilo que você usar. Você pode aproveitar outros serviços de análise e dados do Azure, conforme visto anteriormente neste artigo para aprimorar seu ambiente de análise. Entendemos que, em alguns casos, suas necessidades podem exigir ferramentas adicionais, incluindo algumas ferramentas de terceiros. Você tem acesso administrativo completo na máquina virtual para instalar novas ferramentas necessárias. Também é possível instalar pacotes adicionais no Python e no R que não foram pré-instalados. Para Python, você pode usar ```conda``` ou ```pip```. Para R, você pode usar o ```install.packages()``` no console do R ou usar o IDE e escolher "Pacotes -> Instalar Pacotes...".

Essas são apenas algumas amostras do que você pode fazer na Máquina Virtual de Ciência de Dados da Microsoft. Há muito mais coisas que você fazer para torná-la um ambiente eficaz de análise.
