...
Os dados usados neste artigo, juntamente com os scripts de R, podem ser [baixados aqui][download]. Esses dados foram originalmente sintetizados com base nas informações disponibilizadas pela Universidade de Wisconsin em http://future.aae.wisc.edu/tab/production.html.
...
Usarei o RStudio para desenvolver, testar e depurar o código R em pequena escala. Esse código é então recortado e colado em um módulo [Executar Script R][execute-r-script] no Estúdio de Aprendizado de Máquina, pronto para ser executado.

###O módulo Executar Script R

No Estúdio de Aprendizado de Máquina, os scripts R são executados no módulo [Executar Script R][execute-r-script]. Um exemplo do módulo [Executar Script R][execute-r-script] no Estúdio de Aprendizado de Máquina é ilustrado na Figura 1.
...
Consultando a Figura 1, vejamos algumas das principais partes do ambiente do Estúdio de Aprendizado de Máquina para trabalhar com o módulo [Executar Scripts R][execute-r-script].
...
Naturalmente, abordaremos o módulo [Executar Script R][execute-r-script] em mais detalhes no restante deste documento.
...
Qualquer código R no módulo [Executar Script R][execute-r-script] será executado quando você clicar no botão **Executar** para iniciar o teste. Quando a execução for concluída, uma marca de seleção aparecerá no ícone [Executar Script R][execute-r-script].

####Codificação R defensiva para o Aprendizado de Máquina do Azure
...
####Depurar e testar R no Studio de Aprendizado de Máquina
...
Esse código não foi executado resultando em uma condição de erro. Quando clico em **Exibir log de erro** no **painel de propriedades**, é produzida a exibição mostrada na Figura 2:
...
####Pacotes no Estúdio de Aprendizado de Máquina
...
Uma introdução com tutorial para o RStudio está disponível em https://support.rstudio.com/hc/sections/200107586-Using-RStudio.
...
###Carregar e verificar dados no Studio de Aprendizado de Máquina
...
- Você deve ver os novos conjuntos de dados clicando na guia **Conjuntos de dados**.
...
- Dê um título ao seu teste. Chamarei o teste de **Análise de derivados de leite da Califórnia**.

- Procure o conjunto de dados que você acabou de carregar.
...
- Conecte a saída do conjunto de dados **csdairydata.csv dataset** à entrada à esquerda (**Dataset1**) de [Executar Script R][execute-r-script].
...
Vamos dar uma olhada nos dados que carregou em nosso teste. No teste, clique duas vezes na saída do conjunto de dados **cadairydata.csv dataset** e selecione **visualizar**. Você deve ver algo semelhante à Figura 4.

![Resumo do conjunto de dados cadairydata.csv][4]

*Figura 4. Resumo de conjunto de dados cadairydata.csv.*

Nessa exibição, vemos muitas informações úteis. A linha **Tipo de Recurso** mostra quais tipos de dados o Estúdio de Aprendizado de Máquina do Azure atribui às colunas de nosso conjunto de dados. Também vemos as primeiras linhas do conjunto de dados. Dar uma olhada rápida como esta é boa uma verificação de integridade para ser feita antes de começar qualquer trabalho sério.
...
Vamos conferir as entradas do módulo [Executar Script R][execute-r-script]. Neste exemplo, vamos ler os dados dos produtos derivados de leite da Califórnia no módulo [Executar Script R][execute-r-script].
...
4.	Arraste e solte o arquivo zip dos **conjuntos de dados** para as **telas do Estúdio de Aprendizado de Máquina**.
...
Uma vez concluídas essas etapas, o módulo [Executar Script R][execute-r-script] executará o script R no arquivo zip quando o teste for executado. Agora seu teste deve ser semelhante à Figura 5.
...
Agora que podemos ler os dados dos derivados de leite da Califórnia no código R no módulo [Executar Script R][execute-r-script], precisamos garantir que os dados nas colunas têm o tipo e o formato desejados.
...
Vamos executar esse código e examinar a saída. A saída de **Visualizar** no menu do Dispositivo R é mostrada na Figura 9.

![Resumo do dataframe com uma variável de fator][10]
...
Execute novamente o teste e use a opção **Visualizar** para a saída da porta do Dispositivo R com os resultados mostrados na Figura 10.

![Resumo do dataframe com o número correto de níveis de fator][11]
...
Execute esse código em seu teste e verifique o resultado com **Visualizar** na porta do Dispositivo R. Esses resultados são mostrados na Figura 11.

![Resumo do dataframe com duas colunas removidas][12]
...
Agora, execute o teste atualizado e use **Visualizar** na porta do Dispositivo R para exibir os resultados. Esses resultados são mostrados na Figura 12.

![Resumo do dataframe com a coluna adicional][13]
...
3. As verificações são executadas nos argumentos para a função. Em cada caso, se for detectado um erro, será retornado um valor padrão, e uma mensagem será produzida pela função `warming()`. Estou usando `warning()` em vez de `stop()`, pois essa segunda opção encerrará a execução, justamente o que estou tentando evitar. Observe que eu escrevi este código em um estilo de procedimento, pois nesse caso uma abordagem funcional me parece complexa e obscura.
...
Executar esse código R em seu teste e veja a saída impressa no output.log. Agora, você verá os valores transformados das quatro colunas usando **Visualizar** no menu do Dispositivo R, conforme mostrado na Figura 13.

![Resumo dos valores de transformados no dataframe][14]
...
###Objetos de série temporal em R
...
Vamos começar com o nosso exemplo. Arraste e solte um módulo **novo** [Executar Script R][execute-r-script] em seu teste. Conecte a porta de saída do Conjunto de Dados de Resultado 1 do módulo [Executar Script de R][execute-r-script] existente à porta de entrada do Conjunto de Dados de Resultado 1 do novo módulo [Executar Script R][execute-r-script].
...
![Resumo do dataframe no módulo Executar Script R][15]
...
####Criando um objeto de série temporal
...
![Resumo do dataframe com um objeto de série temporal][16]
...
![Lista de objetos ccf de análise da correlação emparelhados][19]
...
Observe que os nomes das linhas estão na coluna do dataframe. Isso preserva os nomes de linha quando eles são a saída de [Executar Script R][execute-r-script].
...
Comece adicionando um **novo** módulo [Executar Script R][execute-r-script] à sua experiência. Conecte a saída do **Conjunto de Dados de Resultado** do módulo [Executar Script R][execute-r-script] existente à entrada do **Conjunto de Dados 1** do novo módulo. O resultado deve ser semelhante a Figura 20.
...
Vamos fazer outra plotagem de série de tempo dos dados de produção de derivados de leite da Califórnia para ver até que ponto o modelo sazonal está funcionando. Adicionei o seguinte código ao modelo [Executar Script R][execute-r-script] do Aprendizado de Máquina do Azure (não o RStudio) para criar o modelo e fazer uma plotagem:
...
3.	Depurando
	...
	O RStudio inclui recursos avançados de depuração. A documentação para esses recursos está em https://support.rstudio.com/hc/articles/200713843-Debugging-with-RStudio.

	Os recursos de solução de problemas do ponto de interrupção são documentados em https://support.rstudio.com/hc/articles/200534337-Breakpoint-Troubleshooting.
...
- R in Action, por Robert Kabacoff, é outro livro introdutório útil. O site complementar Quick R é um recurso útil, em http://www.statmethods.net/.

- R Inferno, de Patrick Burns, é um livro surpreendentemente bem-humorado que lida com inúmeros tópicos complicados e difíceis que podem ser encontrados ao se programar em R. O livro está disponível gratuitamente em http://www.burns-stat.com/documents/books/the-r-inferno/.

- Se você quiser se aprofundar em tópicos avançados em R, consulte o livro Advanced R, de Hadley Wickham. A versão online desse livro está disponível gratuitamente em http://adv-r.had.co.nz/.
...
- Mais 60 recursos de R listados em http://www.computerworld.com/article/2497464/business-intelligence-60-r-resources-to-improve-your-data-skills.html
