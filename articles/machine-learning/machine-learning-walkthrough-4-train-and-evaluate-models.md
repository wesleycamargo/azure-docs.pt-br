...
3.  Conecte a saída esquerda do módulo (“Conjunto de dados de resultado”) t [Executar script R][execute-r-script] à porta de entrada direita ("Conjunto de Dados") do módulo [Treinar Modelo][train-model].

    > [AZURE.TIP] Não precisamos de duas das entradas e uma das saídas do módulo [Executar Script R][execute-r-script] para este teste; portanto, apenas as deixaremos desconectadas. 

4.  Selecione o módulo [Treinar modelo][train-model]. No painel **Propriedades**, clique em **Iniciar seletor de coluna**, selecione **Todos os tipos** na lista suspensa em **Colunas disponíveis** e insira "Risco de crédito" no campo de texto. Selecione **Todos os tipos** na lista suspensa em **Colunas selecionadas**. Selecione "Risco de crédito" e clique no botão de seta realçado para movê-lo para **Colunas selecionadas**. 
5.  Clique em **Salvar**.
...
Primeiro, uma pequena explicação sobre o SVM. Árvores de decisão aumentadas funcionam bem com recursos de qualquer tipo. No entanto, como o módulo SVM gera um classificador linear, o modelo que ele gera apresenta o melhor erro de teste quando todos os recursos numéricos possuem a mesma escala. Para converter todos os recursos numéricos para a mesma escala, usamos a Transformação “Tanh” (com o módulo [Normalizar dados][normalize-data]). Isso transforma nossos números no intervalo [0,1]. Os recursos de cadeia de caracteres são convertidos pelo módulo SVM para recursos categóricos e, em seguida, para recursos binários 0/1, portanto, não precisamos transformar manualmente os recursos de cadeia de caracteres. Também não desejamos transformar a coluna Risco de Crédito (coluna 21) – ela é numérica, mas esse é o valor que estamos treinando para o modelo prever, portanto, precisamos deixá-la intacta.  
...
2.  Clique com o botão direito do mouse no módulo [Treinar modelo][train-model], selecione **Copiar**, depois clique com o botão direito do mouse na tela e selecione **Colar**. A cópia do módulo [Treinar modelo][train-model] tem a mesma seleção de coluna que o original.
3.  Conecte a saída do módulo SVM à porta de entrada esquerda ("Modelo não treinado") do segundo módulo [Treinar Modelo][train-model] module.
...
2.  Conecte o módulo [Treinar modelo][train-model] que esteja conectado ao módulo [Árvore de decisão aumentada de duas classes][two-class-boosted-decision-tree] à porta de entrada da esquerda do módulo [Modelo de pontuação][score-model] module.
3.  Conecte a porta de entrada direita do módulo [Modelo de pontuação][score-model] à saída esquerda do módulo [Executar script R][execute-r-script] direito.
    ...
    O módulo [Modelo de Pontuação][score-model] agora pode pegar as informações de crédito dos dados de teste, executá-las por meio do modelo e comparar as previsões que o modelo gera com a coluna de risco de crédito real nos dados de teste.
...
Para avaliar os dois resultados de pontuação usaremos um módulo [Avaliar modelo][evaluate-model].  
