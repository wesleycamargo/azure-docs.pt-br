<properties 
	pageTitle="Depurar seu modelo no aprendizado de máquina do Azure | Azure" 
...
* o módulo Modelo de Pontuação produz resultados incorretos 

## O módulo Modelo de Treinamento gera um erro

![image1](./media/machine-learning-debug-models/train_model-1.png)

O módulo Modelo de Treinamento espera que as seguintes 2 entradas:
...
2. O conjunto de dados não contém nenhuma coluna de Recurso.  Por exemplo, se o conjunto de dados de entrada tem apenas uma coluna, que é marcada como a coluna Rótulo, não haveria nenhum recurso com o qual compilar o modelo.  Nesse caso, o módulo Modelo de Treinamento gerará um erro.
...
Em um gráfico típico do treinamento/teste de aprendizado supervisionado, o módulo Divisão divide o conjunto de dados original em duas partes: a parte que é usada para treinar o modelo e a parte que é reservada para pontuação da mesma forma que o modelo treinado é executado em dados para os quais ele não foi treinado.  O modelo treinado, em seguida, é usado para pontuar os dados de teste e, depois, os resultados são avaliados para determinar a precisão do modelo.

O módulo Modelo de Pontuação requer duas entradas:

1. Uma saída de modelo treinado do módulo Modelo de treinamento
2. Um conjunto de dados de pontuação no qual o modelo não foi treinado.

Isso pode ocorrer mesmo que o teste tenha sido bem-sucedido, o módulo Modelo de pontuação produz resultados incorretos.  Vários cenários podem causar isso:

1. Se o rótulo especificado for categórico, e um modelo de regressão for treinado nos dados, uma saída incorreta poderá ser produzida pelo módulo Modelo de pontuação.  Isso ocorre porque a regressão requer uma variável de resposta contínua.  Nesse caso, deve ser mais adequado usar um modelo de classificação. 
...
3. Se o conjunto de dados de pontuação não contiver todos os recursos usados para treinar o modelo, o Modelo de pontuação produzirá um erro.
4. O Modelo de Pontuação não produzirá qualquer saída correspondente a uma linha no conjunto de dados de pontuação que contenha um valor ausente ou um valor infinito para qualquer um dos seus recursos.
5. O Modelo de Pontuação pode produzir resultados idênticos para todas as linhas no conjunto de dados de pontuação.  Isso pode ocorrer, por exemplo, ao tentar fazer a classificação usando Florestas de Decisão se o número mínimo de amostras por nó folha for escolhido como sendo maior que o número de exemplos de treinamento disponível.
