...
2. Abra o arquivo *Web.config* do aplicativo no projeto ContosoAdsWeb.
	...
	A cadeia de conexão de armazenamento se chama AzureWebJobsStorage, porque esse é o nome usado por padrão pelo SDK de Trabalhos Web. O mesmo nome é usado aqui, de modo que você só precisa definir um valor de cadeia de conexão no ambiente do Azure.
...
&lt;/configuration&gt;</pre>
	...
	Por padrão, o SDK de Trabalhos Web procura cadeias de conexão chamadas AzureWebJobsStorage e AzureWebJobsDashboard. Como alternativa, é possível [armazenar a cadeia de conexão, por mais que você queira passá-la explicitamente para o objeto `JobHost`](websites-dotnet-webjobs-sdk-storage-queues-how-to.md/#config).

1. Substitua ambas as cadeias de conexão de armazenamento pela cadeia de conexão que você copiou anteriormente.
...
1. Para iniciar o front-end Web do aplicativo, pressione CTRL+F5. 
	...
	O navegador padrão é aberto na home page. (The web project runs because you've made it the startup project.)
...
2. Insira alguns dados de teste e selecione uma imagem para carregar e depois clique em **Criar**.
	...
	Enquanto isso, depois de uma curta espera, uma mensagem de registro em log na janela do aplicativo de console mostra que uma mensagem da fila foi recebida e processada.   
...
9. Na caixa de diálogo **Criar aplicativo Web no Microsoft Azure**, insira um nome exclusivo na caixa **Nome do aplicativo Web**.
	...
	A URL completa consistirá no que você digitar aqui, acrescido do domínio .azurewebsites.net (conforme exibido ao lado da caixa de texto **Nome do aplicativo Web**). Por exemplo, se o nome do aplicativo Web for ContosoAds, a URL será ContosoAds.azurewebsites.net.

9. Na lista suspensa **Região**, escolha a mesma região selecionada para a conta de armazenamento.

	Essa configuração especifica em qual datacenter do Azure o aplicativo Web será executado. A manutenção do aplicativo Web e da conta de armazenamento no mesmo datacenter minimiza os encargos de dados egressos e com latência.
...
4. Na etapa **Visualização**, clique em **Iniciar Visualização**.
	...
	A janela de visualização mostra que binários e arquivos de configuração do projeto de Trabalho Web serão copiados para a pasta *app_data\jobs\continuous* do aplicativo da Web.
...
Para este aplicativo de exemplo, a atividade de front-end do aplicativo Web sempre precede a criação de uma mensagem de fila, portanto, não há nenhum problema se o aplicativo Web entrar em suspensão e finalizar o Trabalho Web devido a um longo período de inatividade. Quando uma solicitação chega, o aplicativo Web é ativado e o Trabalho Web é reiniciado.

Para Trabalhos Web que você desejar manter em execução mesmo quando o aplicativo Web estiver inativo por um longo período de tempo, você pode usar o recurso [AlwaysOn](http://weblogs.asp.net/scottgu/archive/2014/01/16/windows-azure-staging-publishing-support-for-web-sites-monitoring-improvements-hyper-v-recovery-manager-ga-and-pci-compliance.aspx).
...
2.  Clique em **OK**.
	...
	* Adicionou um arquivo *webjob-publish-settings.json* na pasta Propriedades do projeto do Trabalho Web.
...
13. Encontre o pacote do *Azure Storage* e, em seguida, clique em **Gerenciar**.
...
3. No projeto ContosoAdsWeb, adicione os seguintes arquivos do projeto baixado.
	...
	- Na pasta *Views\Shared*: <em>arquivo _Layout.cshtml</em>. 
...
Código que é chamado por meio do método `Application_Start` cria um contêiner de blob *images* e uma fila *images*, se eles ainda não existirem. Isso garante que, sempre que você começar a usar uma nova conta de armazenamento, o contêiner do blob e a fila obrigatórios serão criados automaticamente.
...
* Os dois atributos `Blob` fornecem objetos vinculados a blobs: um para o blob de imagem existente e outro para um novo blob de miniatura que o método cria. 
	...
	Os nomes dos blobs vêm das propriedades do objeto `BlobInformation` recebidas na mensagem da fila (`BlobName` e `BlobNameWithoutExtension`). Para obter toda a funcionalidade da SCL, é possível usar a classe `CloudBlockBlob` para trabalhar com blobs. Se quiser reutilizar código escrito para funcionar com objetos `Stream`, você pode usar a classe  `Stream`. 

Para obter mais informações sobre como escrever funções que usam atributos SDK de Trabalhos Web, consulte os recursos a seguir:
