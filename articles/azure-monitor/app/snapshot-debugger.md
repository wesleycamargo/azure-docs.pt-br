...
3. Examine as opções padrão que o pacote adicionou a [ApplicationInsights.config](../../azure-monitor/app/configuration-with-applicationinsights-config.md):
...
Se um instantâneo estiver disponível para uma determinada ID de problema ou exceção, um botão **Abrir Instantâneo de Depuração** aparecerá na [exceção](../../azure-monitor/app/asp-net-exceptions.md) no portal do Application Insights.

![Botão Abrir Instantâneo de Depuração na exceção](./media/snapshot-debugger/snapshot-on-exception.png)
