...
Há suporte para apenas os recursos que oferecem suporte a coletar métricas com o diagnóstico do Azure e enviam diretamente para o Log Analytics.  Para uma lista mais detalhada, reveja [Coletar logs e métricas do serviço do Azure para uso no Log Analytics](collect-azure-metrics-logs.md) 
...
5. Selecione a assinatura apropriada na lista se você tiver mais de um, digitando o valor correto.<br><br> ![Selecione a assinatura retornada pelo script](./media/collect-azurepass-posh/script-select-subscription.png)<br> Caso contrário, ele seleciona automaticamente a assinatura única disponível.
6. Em seguida, o script retorna uma lista de workspaces de Log Analytics registrados na assinatura.  Selecione um apropriado da lista.<br><br> ![Selecione o workspace retornado pelo script](./media/collect-azurepass-posh/script-select-workspace.png)<br> 
7. Selecione os recursos do Azure dos quais você gostaria de habilitar a coleta. Por exemplo, se você digitar 5, você habilita coleta de dados para bancos de dados do SQL Azure.<br><br> ![Tipo de recurso selecione retornado pelo script](./media/collect-azurepass-posh/script-select-resource.png)<br>
