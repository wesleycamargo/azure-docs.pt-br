...
3. Daemons (serviços) [etcd](https://github.com/coreos/etcd) e [frota](https://github.com/coreos/fleet) integrados para gerenciamento e comunicação dinâmica da máquina virtual e do cluster
...
Como usar as instruções em [Como usar SSH com Linux no Azure](http://azure.microsoft.com/pt-br/documentation/articles/virtual-machines-linux-use-ssh-key/) para criar uma chave pública e privada para SSH. As etapas básicas estão descritas nas instruções abaixo. Você vai usar essas chaves para conectar-se às máquinas virtuais no cluster e verificar se elas estão funcionando e podem comunicar-se umas com as outras.

> [WACOM.NOTE] Este tópico parte do pressuposto de que você não tem essas chaves e precisa criar arquivos **`myPrivateKey.pem`** e **`myCert.pem`** para fins de clareza. Se você já tiver um par de chave pública e privada salvo em **`~/.ssh/id_rsa`**, pode simplesmente digitar `openssl req -x509 -key ~/.ssh/id_rsa -nodes -days 365 -newkey rsa:2048 -out myCert.pem` para obter o arquivo .pem que precisa carregar para o Azure.
...
> [WACOM.NOTE] Lembre-se de digitar `cat etcdid` para recuperar a id de descoberta etcd do arquivo `etcdid` criado acima e substituir **`<token>`** no seguinte arquivo **cloud-config.yaml** pelo número gerado do seu arquivo `etcdid`.Se não puder validar o cluster ao final, é possível que você tenha pulado uma das etapas.
...
(Para obter informações mais completas sobre o arquivo cloud-config, consulte [Usando Cloud-Config](https://coreos.com/docs/cluster-management/setup/cloudinit-cloud-config/) na documentação do CoreOS.)
...
2. Localize sua imagem CoreOS. No momento, há apenas uma imagem -- **`2b171e93f07c4903bcad35bda10acf22__CoreOS-Alpha-475.1.0`** --, mas, para localizar as imagens disponíveis a qualquer momento, digite `azure vm image list | grep .*CoreOS.*` para ver um ou mais resultados com esta aparência:
...
`azure service create <cloud-service-name>`, em que *cloud-service-name* é o nome do seu serviço de nuvem CoreOS. O nome desta amostra é **`coreos-cluster`**. Você precisará reutilizar o nome escolhido na criação das suas instâncias de máquina virtual CoreOS no serviço de nuvem. 

Observação: Se você observar seu trabalho até o momento no [novo portal](https://portal.azure.com), verá que o nome do serviço de nuvem é tanto um grupo quanto um domínio de recursos, como mostra a imagem a seguir:
...
> [WACOM.NOTE] Se já estiver usando a chave **~/.ss h/id_rsa**, adicione-a com `ssh-add ~/.ssh/id_rsa`.
...
[Tutorial de CoreOS de Tim Park]: https://github.com/timfpark/coreos-azure
