...
| Azure Document DB | <ul><li>[Usar consultas SQL parametrizadas no Azure Cosmos DB](#sql-docdb)</li></ul> | 
...
| Tecnologias aplicáveis | Genérico |
| Atributos              | N/D  |
| Referências              | [Anunciando a parametrização de SQL no Cosmos DB](https://azure.microsoft.com/blog/announcing-sql-parameterization-in-documentdb/) |
| Etapas | Embora o Cosmos DB dê suporte apenas a consultas somente leitura, a injeção de SQL ainda será possível se as consultas forem construídas concatenando com a entrada do usuário. É possível que um usuário obtenha acesso a dados que não deveria acessar na mesma coleção criando consultas SQL mal-intencionadas. Use consultas SQL parametrizadas se as consultas forem construídas com base na entrada do usuário. |
