...
O objetivo deste artigo é fornecer uma visão geral dos recursos de segurança centrais do Azure que ajudam com o gerenciamento de identidades. Também fornecemos links para artigos que apresentarão detalhes de cada recurso para que você possa saber mais.
...
SSO (logon único) significa poder acessar todos os aplicativos e recursos que você precisa para fazer negócios, iniciando a sessão apenas uma vez usando uma única conta de usuário. Depois de conectado, você pode acessar todos os aplicativos sem a exigência de autenticação (por exemplo, digite uma senha) uma segunda vez.
...
O Azure AD estende o Active Directory local para a nuvem, permitindo que os usuários usem suas contas organizacionais primárias não apenas para entrar em seus dispositivos e recursos da empresa ingressados no domínio, mas também todos os aplicativos SaaS e Web necessários para seu trabalho.
...
O Proxy de Aplicativo do Azure AD permite que você publique aplicativos locais, como sites do [SharePoint](https://support.office.com/article/What-is-SharePoint-97b915e6-651b-43b2-827d-fb25777f446f?ui=pt-BR&rs=pt-BR&ad=US), o [Outlook Web App](https://technet.microsoft.com/library/jj657718.aspx) e aplicativos baseados no [IIS](http://www.iis.net/), na sua rede privada e fornece acesso seguro aos usuários fora da sua rede. O Proxy de Aplicativo fornece acesso removo e SSO (logon único) para diversos tipos de aplicativos Web locais com milhares de aplicativos SaaS para os quais o Azure AD já dá suporte. Os funcionários podem fazer logon em seus aplicativos de casa, em seus próprios dispositivos, e realizar a autenticação por meio desse proxy baseado em nuvem.
...
O registro de dispositivo do Azure AD é a base para cenários de [acesso condicional](../active-directory/active-directory-conditional-access-on-premises-setup.md) com base no dispositivo. Quando um dispositivo é registrado, o registro de dispositivo do Azure Active Directory fornece o dispositivo com uma identidade que é usada para autenticar o dispositivo quando o usuário faz logon. O dispositivo autenticado e os atributos desse dispositivo podem, em seguida, ser usados para impor políticas de acesso condicional para aplicativos locais e hospedados em nuvem.
...
- [Hybrid identity white paper](http://download.microsoft.com/download/D/B/A/DBA9E313-B833-48EE-998A-240AA799A8AB/Hybrid_Identity_White_Paper.pdf) (White paper sobre identidade híbrida)
