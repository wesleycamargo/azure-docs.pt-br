...
| Armazenamento do Azure | <ul><li>[Garantir que as comunicações para o Armazenamento do Azure sejam feitas via HTTPS](#comm-storage)</li><li>[Validar o hash MD5 depois de baixar o blob se o HTTPS não puder ser habilitado](#md5-https)</li><li>[Usar um cliente compatível com SMB 3.0 para garantir a criptografia de dados em trânsito para os compartilhamentos de arquivos do Azure](#smb-shares)</li></ul> |
...
| Referências              | [Armazenamento de Arquivos do Azure](../storage/storage-files-introduction.md), [Como montar um compartilhamento de arquivos do Azure no Windows](../storage/storage-file-how-to-use-files-windows.md) |
| Etapas | O Armazenamento de Arquivos do Azure dá suporte a HTTPS ao usar a API REST, mas é mais frequentemente usado como um compartilhamento de arquivos SMB conectado a uma VM. O SMB 2.1 não é compatível com a criptografia, de modo que as conexões só são permitidas dentro da mesma região no Azure. No entanto, o SMB 3.0 dá suporte à criptografia e pode ser usado com o Windows Server 2016, o Windows Server 2012 R2, o Windows 8.1 e o Windows 10, permitindo o acesso entre regiões e até mesmo à área de trabalho. |
...
| Componente               | Armazenamento do Azure | 
| Fase do SDL               | Compilação |  
| Tecnologias aplicáveis | Genérico, Windows Mobile |
| Atributos              | N/D  |
| Referências              | [Anexação de chave pública e certificado](https://www.owasp.org/index.php/Certificate_and_Public_Key_Pinning#.Net) |
| Etapas | <p>A anexação de certificado protege contra ataques MITM (Man-In-The-Middle) baseados em TLS. A anexação é o processo de associar uma conexão TLS a um certificado x.509 ou chave pública esperado. Essa associação pode ocorrer após a primeira conexão bem-sucedida ("confiança no primeiro uso") ou antes disso (o que exigiria pré-carregamento).</p><p>Quando um adversário tentar executar um ataque MITM TLS, o cliente verificará para garantir que o certificado/chave recebido corresponde ao valor esperado. Portanto, mesmo que o adversário tivesse um certificado de outra forma válido e confiável, a conexão ainda falhará.</p><p>A anexação de certificado pode ser implementada no .NET por meio da implementação do delegado [HttpWebRequest.ServerCertificateValidationCallback](https://docs.microsoft.com/dotnet/api/system.net.httpwebrequest.servercertificatevalidationcallback) (preferencial) ou [ServicePointManager.ServerCertificateValidationCallback](https://docs.microsoft.com/dotnet/api/system.net.servicepointmanager.servercertificatevalidationcallback). No exemplo a seguir, podemos validar uma chave pública codificada e o algoritmo, mas em um aplicativo do mundo real, esses valores devem ser armazenados em uma área de configuração segura e ser atualizados conforme o necessário.</p>|
