...
- O Azure Disk Encryption para Linux requer 7GB de RAM na VM para habilitar a criptografia de disco do sistema operacional em [suporte para imagens](azure-security-disk-encryption-faq.md#bkmk_LinuxOSSupport). Quando o processo de criptografia de disco do sistema operacional for concluído, a VM pode ser configurada para executar com menos memória.
- Antes de habilitar a criptografia, os discos de dados a serem criptografados precisam ser listados corretamente em /etc/fstab. Use um nome de dispositivo de bloco persistente para essa entrada, como dispositivo de nomes no formato "/ DES/sdX" não podem ser considerados para ser associado com o mesmo disco entre as reinicializações, principalmente depois que a criptografia é aplicada. Para obter mais detalhes sobre esse comportamento, consulte: [alterações de nome de dispositivo de solucionar problemas de VM do Linux](../virtual-machines/linux/troubleshoot-device-names-problems.md)
- Certifique-se de que o /etc/fstab estão configuradas corretamente para montagem. Para definir essas configurações, execute o comando mount - a ou reinicie a VM e disparar a remontagem dessa forma. Quando terminar, verifique a saída do comando lsblk para verificar se a unidade desejada ainda está montada. 
    - Se o arquivo /etc/fstab não montar a unidade corretamente antes de habilitar a criptografia, o Azure Disk Encryption será não é possível montá-lo corretamente.
...
-  Política do BitLocker em máquinas virtuais de domínio associado com a política de grupo personalizado deve incluir a seguinte configuração: [configurar o armazenamento de usuário das informações de recuperação do bitlocker -> permitir 256 bits chave de recuperação](https://docs.microsoft.com/windows/security/information-protection/bitlocker/bitlocker-group-policy-settings). O Azure Disk Encryption falhará quando as configurações de diretiva de grupo personalizado para o Bitlocker são incompatíveis. Em computadores que não tinham a configuração de política correta, aplicar a nova política, forçar a nova política de atualização (gpupdate.exe /force) e, em seguida, reiniciando pode ser necessária.  


## <a name="bkmk_PSH"></a>PowerShell do Azure
[O Azure PowerShell](/powershell/azure/overview) fornece um conjunto de cmdlets que usam o modelo [Azure Resource Manager](../azure-resource-manager/resource-group-overview.md) para gerenciar os recursos do Azure. Você pode usá-lo em seu navegador com [Azure Cloud Shell](../cloud-shell/overview.md), ou você pode instalá-lo em seu computador local usando as instruções abaixo para usá-lo em qualquer sessão do PowerShell. Se você já tiver instalado localmente, verifique se que você usar a versão mais recente do SDK do Azure PowerShell para configurar o Azure Disk Encryption. Baixe a última versão do [Azure PowerShell](https://github.com/Azure/azure-powershell/releases).
...
3. Verifique se as versões instaladas dos módulos.
      ...
      ```
4. Faça logon Azure usando o [Connect-AzureRmAccount](/powershell/module/azurerm.profile/connect-azurermaccount) cmdlet.
...
Para executar os seguintes comandos, obtenha e use o [módulo do Azure AD PowerShell](/powershell/azure/active-directory/install-adv2). 
...
2. Crie uma nova entidade de serviço.
     ...
     ```
3.  O appId retornado é o ClientID do Azure AD usado em outros comandos. É também o SPN que você usará para definir uma política de keyvault. A senha é o segredo do cliente que você deve usar posteriormente para ativar a criptografia de disco do Azure. Proteja adequadamente o segredo do cliente no Azure AD.
