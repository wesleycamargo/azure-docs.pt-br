...
| Azure Document DB | <ul><li>[Usar tempos de vida mínimos de tokens para tokens de Recurso gerados](#resource-tokens)</li></ul> |
| ADFS | <ul><li>[Implementar o logoff apropriado usando métodos WsFederation ao usar o ADFS](#wsfederation-logout)</li></ul> |
| Servidor de identidade | <ul><li>[Implementar o logoff adequado ao usar o Servidor de identidade](#proper-logout)</li></ul> |
...
| API Web | <ul><li>[Atenuar ataques CSRF (solicitação intersite forjada) em APIs da Web ASP.NET](#csrf-api)</li></ul> |

## <a name="a-idlogout-adalaimplement-proper-logout-using-adal-methods-when-using-azure-ad"></a><a id="logout-adal"></a>Implementar o logoff apropriado usando métodos ADAL ao usar o Azure AD
...
## <a name="a-idfinite-tokensause-finite-lifetimes-for-generated-sas-tokens"></a><a id="finite-tokens"></a>Usar tempos de vida finitos para tokens SaS gerados
...
## <a name="a-idresource-tokensause-minimum-token-lifetimes-for-generated-resource-tokens"></a><a id="resource-tokens"></a>Usar tempos de vida mínimos de tokens para tokens de Recurso gerados
...
## <a name="a-idwsfederation-logoutaimplement-proper-logout-using-wsfederation-methods-when-using-adfs"></a><a id="wsfederation-logout"></a>Implementar o logoff apropriado usando métodos WsFederation ao usar o ADFS
...
## <a name="a-idproper-logoutaimplement-proper-logout-when-using-identity-server"></a><a id="proper-logout"></a>Implementar o logoff adequado ao usar o Servidor de identidade
...
| Tecnologias aplicáveis | Genérico |
| Atributos              | N/D  |
| Referências              | [Saída federada do IdentityServer3](https://identityserver.github.io/Documentation/docsv2/advanced/federated-signout.html) |
| Etapas | O IdentityServer oferece suporte à federação com provedores de identidade externos. Quando um usuário sai de um provedor de identidade upstream, dependendo do protocolo usado, talvez seja possível receber uma notificação após a desconexão do usuário. Isso permitiria que o IdentityServer notificasse seus clientes para que eles também pudessem desconectar o usuário. Consulte a documentação na seção de referências para obter detalhes sobre a implementação.|

## <a name="a-idhttps-secure-cookiesaapplications-available-over-https-must-use-secure-cookies"></a><a id="https-secure-cookies"></a>Os aplicativos disponíveis via HTTPS devem usar cookies seguros
...
```
Essa configuração é aplicada mesmo se HTTP for usado para acessar o aplicativo. Se HTTP for usado para acessar o aplicativo, essa configuração interromperá o aplicativo, pois os cookies serão definidos com o atributo secure, e o navegador não os enviará de volta ao aplicativo.
...
## <a name="a-idcookie-definitionaall-http-based-application-should-specify-http-only-for-cookie-definition"></a><a id="cookie-definition"></a>Todo aplicativo baseado em http deve especificar http somente para definição de cookie
...
| Etapas | Para atenuar o risco de divulgação de informações com um ataque XSS (script entre sites), um novo atributo - httpOnly - foi introduzido aos cookies e é suportado por todos os principais navegadores. Esse atributo especifica que um cookie não pode ser acessado por script. Usando cookies HttpOnly, um aplicativo Web reduz a possibilidade de roubo de informações confidenciais contidas no cookie por meio de script e o envio dessas informações ao site do invasor. |
...
## <a name="a-idcsrf-aspamitigate-against-cross-site-request-forgery-csrf-attacks-on-aspnet-web-pages"></a><a id="csrf-asp"></a>Atenuar ataques CSRF (solicitação intersite forjada) em páginas Web ASP.NET
...
| Atributos              | N/D  |
| Referências              | N/D  |
| Etapas | Solicitação intersite forjada (CSRF ou XSRF) é um tipo de ataque em que um invasor pode executar ações no contexto de segurança de sessão estabelecida de um usuário diferente com um site, por exemplo, modificar ou excluir conteúdo, se o site de destino depender exclusivamente de cookies de sessão para autenticar a solicitação recebida. Um invasor pode explorar essa vulnerabilidade fazendo um navegador diferente de um usuário carregar uma URL com um comando de um site vulnerável no qual o usuário já está conectado. Há muitas maneiras de um invasor fazer isso, como hospedando um site diferente que carrega um recurso do servidor vulnerável ou fazendo o usuário clicar em um link. Esse tipo de ataque pode ser evitado se o servidor enviar um token adicional ao cliente, exigir que o cliente inclua esse token em todas as solicitações futuras e verificar se todas as solicitações futuras incluem um token que pertence à sessão atual, por exemplo, usando o ASP.NET AntiForgeryToken ou ViewState. |
...
```
Este é um filtro de autorização que verifica se:
* A solicitação de entrada tem um cookie chamado __RequestVerificationToken
* A solicitação de entrada tem uma entrada Request.Form chamada __RequestVerificationToken
* Esses cookies e os valores de Request.Form correspondem. Supondo que tudo esteja certo, a solicitação passará normalmente. Mas, se não estiver, uma falha de autorização com a mensagem "Um token antifalsificação necessário não foi fornecido ou era inválido". 
...
## <a name="a-idinactivity-lifetimeaset-up-session-for-inactivity-lifetime"></a><a id="inactivity-lifetime"></a>Configurar sessão para tempo de vida de inatividade
...
## <a name="a-idproper-app-logoutaimplement-proper-logout-from-the-application"></a><a id="proper-app-logout"></a>Implementar o logoff apropriado do aplicativo
...
## <a name="a-idcsrf-apiamitigate-against-cross-site-request-forgery-csrf-attacks-on-aspnet-web-apis"></a><a id="csrf-api"></a>Atenuar ataques CSRF (solicitação intersite forjada) em APIs da Web ASP.NET
