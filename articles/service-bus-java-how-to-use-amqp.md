<properties linkid="develop-java-how-to-guides-service-bus-amqp" urldisplayname="Service Bus AMQP" headerexpose="" pageTitle="Como usar o AMQP 1.0 com a API do Service Bus do Java - Azure" metakeywords="Java Messsage AMQP, Service Bus AMQP, download AMQP JMS library" footerexpose="" description="Saiba como usar o Java Message Service (JMS) com o Service Bus do Azure e o Protocolo de Enfileiramento de Mensagens Avançadas (AMQP) 1.0." umbraconavihide="0" disquscomments="1" metaCanonical="" title="Como usar a API do Java Message Service (JMS) com Service Bus e AMQP 1.0" authors=""  solutions="" documentationCenter="Java" writer="sethm" manager="dwrede" editor="mattshel" />


# Como usar a API do Java Message Service (JMS) com o Service Bus e AMQP 1.0

# Introdução

O Protocolo de Enfileiramento de Mensagens Avançadas (AMQP, Advanced Message Queuing Protocol) 1.0 é um protocolo de mensagens eficiente, confiável e conectado que pode ser usado para criar aplicativos de mensagens avançados entre plataformas. Suporte ao AMQP 1.0 foi adicionado ao Service Bus do Azure como um recurso de visualização em outubro de 2012. Espera-se que a transição para Disponibilidade Geral (GA) seja feita no primeiro semestre de 2013.
...
Este Guia de Instruções explica como usar os recursos de mensagens agenciadas do Service Bus (tópicos sobre filas e publicação/assinatura) de aplicativos Java usando o popular padrão de API do Java Message Service (JMS).
...
A entrada usada para definir um **ConnectionFactory** no Provedor de JNDI de arquivo de propriedades do Qpid tem o seguinte formato:

	connectionfactory.[jndi_name] = [ConnectionURL]

Onde [jndi_nome] e [ConnectionURL] têm os seguintes significados:
...
<table>
  ...
  <tr>
    <td>[ConnectionURL]</td>
    <td>Uma URL que fornece à biblioteca do JMS as informações necessárias para o agenciador do AMQP.</td>
  </tr>
</table>

O formato de **ConnectionURL** é o seguinte:

	amqps://[nome de usuário]:[senha]@[namespace].servicebus.windows.net
...
<table>
  ...
  <tr>
    <td>[Senha]</td>
    <td>A forma codificada da URL da chave do emissor do Service Bus é obtida no Portal de Gerenciamento do Azure.</td>
  </tr>
</table>

**Nota**: Você deve executar uma codificação de URL da senha manualmente. Um utilitário útil de codificação de URL está disponível em [http://www.w3schools.com/tags/ref_urlencode.asp](http://www.w3schools.com/tags/ref_urlencode.asp).
...
A entrada usada para definir um destino no Provedor de JNDI do arquivo de propriedades do Qpid tem o seguinte formato:
...
O programa de exemplo a seguir envia TextMessages do JMS para uma fila do Service Bus com o nome lógico da JNDI da FILA e recebe as mensagens de volta.
...
##Mensagens em plataformas cruzadas entre o JMS e o .NET
...
* Inicie a amostra do aplicativo Java com o argumento de linha de comando â€œsendonlyâ€. Nesse modo, o aplicativo não receberá mensagens da fila; ele somente as enviará.
...
<p><strong>Saída do aplicativo do .NET</strong></p>
...
* Inicie a amostra do aplicativo .NET com o argumento de linha de comando â€œsendonlyâ€. Nesse modo, o aplicativo não receberá mensagens da fila; ele somente as enviará.
* Inicie a amostra do aplicativo Java sem nenhum argumento de linha de comando.
...
* Apenas um **MessageProducer** ou **MessageConsumer** é permitido por **Session**. Se precisar criar vários **MessageProducers** ou **MessageConsumers** em um aplicativo, crie uma **Session** dedicada para cada um deles.
...
Este guia de instruções explicou como usar os recursos de mensagens agenciadas do Service Bus (tópicos sobre filas e publicação/assinatura) do Java usando a API popular JMS e AMQP 1.0.

Você também pode usar o AMQP 1.0 do Service Bus de outras linguagens, incluindo o .NET, C, Python e PHP. Os componentes criados com estas diferentes linguagens podem trocar mensagens de forma confiável e com total fidelidade usando o suporte do AMQP 1.0 no Service Bus. Para obter mais informações, consulte o [Guia do Desenvolvedor do AMQP 1.0 do Service Bus](http://msdn.microsoft.com/pt-br/library/windowsazure/jj841071.aspx).
