<properties
   pageTitle="Introdução à CLI do Lote do Azure | Microsoft Azure"
...
* Alterne para o **modo do Resource Manager**: `azure config mode arm`

>[AZURE.TIP] Recomendamos que você atualize frequentemente sua instalação da CLI do Azure para aproveitar as atualizações e aprimoramentos do serviço.
...
> [AZURE.NOTE] O nome da conta do Lote deve ser exclusivo na região do Azure em que a conta é criada. Ele pode conter somente caracteres alfanuméricos minúsculos e deve ter de três a 24 caracteres. Você não pode usar caracteres especiais, como `-` ou `_`, em nomes de conta do Lote.

### Conta de armazenamento vinculada (armazenamento automático)

Você pode (opcionalmente) vincular uma conta de armazenamento de **Finalidade geral** à sua conta do Lote durante a criação. O recurso [pacotes de aplicativos](batch-application-packages.md) do Lote usa o armazenamento de blobs em uma conta do Armazenamento de finalidade Geral vinculada, como faz a biblioteca [.NET de Convenções de Arquivo do Lote](batch-task-output.md). Esses recursos opcionais ajudarão você a implantar os aplicativos executados por suas tarefas do Lote, persistindo os dados que eles produzem.
...
## Gerenciar as chaves de acesso da conta
...
>[AZURE.TIP] Liste suas chaves com `azure batch account keys list`, e exiba o ponto de extremidade da conta com `azure batch account show`.
...
>[AZURE.NOTE] Se você especificar um arquivo JSON durante a criação de um recurso, todos os outros parâmetros especificados na linha de comando desse recurso serão ignorados.
...
Conforme mencionado na **Visão geral do Lote**, você tem duas opções ao selecionar um sistema operacional para os nós no pool: [Configuração de Máquina Virtual](batch-api-basics.md#pool) e **Configuração dos Serviços de Nuvem**. Use as opções `--image-*` para criar pools de Configuração de Máquina Virtual, e `--os-family` para criar pools de Configuração de Serviços de Nuvem. Não é possível especificar as opções `--os-family` e `--image-*`.
...
>[AZURE.TIP] Verifique a [lista de imagens de máquina virtual](batch-linux-nodes.md#list-of-virtual-machine-images) em busca dos valores apropriados para as opções `--image-*`.
...
## Listar pools, trabalhos, tarefas e outros recursos
...
### Listagem de recursos com eficiência
...
* `--filter-clause [filter-clause]` Retorna apenas as entidades que correspondem à expressão especificada de OData
* `--expand-clause [expand-clause]` Obtenha as informações de entidade em uma única chamada REST subjacente. No momento, a cláusula expand oferece suporte apenas à propriedade `stats`.
...
[batch_forum]: https://social.msdn.microsoft.com/forums/azure/pt-BR/home?forum=azurebatch
