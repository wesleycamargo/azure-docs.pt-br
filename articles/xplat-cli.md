...
* [Como criar script para a interface de linha de comando entre plataformas do Azure](#script)
...
>[WACOM.NOTE] Talvez você precise usar `sudo` para executar o comando __npm__ com êxito.
...
> [WACOM.NOTE] Node.js pode ser instalado em <a href="http://nodejs.org/">http://nodejs.org/</a>.
...
Para fazer logon usando uma conta institucional, use o seguinte comando:
	...
	azure login [nome de usuário] [senha]
...
Isso abrirá o navegador padrão e você deverá entrar no Portal de Gerenciamento do Azure. Depois da entrada, um arquivo `.publishsettings` será baixado. Anote onde esse arquivo está salvo.
...
Em seguida, importe o arquivo `.publishsettings` executando o seguinte comando, substituindo `[caminho do arquivo .publishsettings]` pelo caminho do arquivo `.publishsettings`:

	azure account import [caminho do arquivo .publishsettings]

> [WACOM.NOTE] Quando você importa configurações de publicação, as informações para acessar a assinatura do Azure são armazenadas em um diretório `.azure` localizado no diretório `user`. O diretório `user` é protegido pelo sistema operacional. No entanto, é recomendável seguir etapas adicionais para criptografar o diretório `user`. Você pode fazer isso das seguintes maneiras:
>
> * No Windows, modifique as propriedades do diretório ou use BitLocker.
...
Na lista acima, a coluna **Atual** indica a assinatura padrão atual como Azure-sub-1. Para alterar a assinatura padrão, use o comando `azure account select` e especifique a assinatura que você deseja que seja a padrão. Por exemplo:

	azure account select Azure-sub-2

Isso irá alterar a assinatura padrão para Azure-sub-2. 

> [WACOM.NOTE] A alteração da assinatura padrão entra em vigor imediatamente, e é uma alteração global. Novos comandos xplat, executados na mesma instância de linha de comando ou em outra instância, usarão a nova assinatura padrão.
...
A maioria dos comandos é formatada como `azure <command> <operation> [parâmetros]` e realiza uma operação em um serviço ou objeto, como a configuração da conta. Outros comandos fornecem subcomandos e seguem o formato `azure <command> <subcommand> <operation> [parâmetros]`. Os seguintes são comandos de exemplo que funcionam com a configuração de sua conta:

* Para exibir as assinaturas importadas, use:
...
Para dar suporte ao novo Gerenciador de Recursos, a xplat-cli permite alternar esses 'modos' de gerenciamento usando-se o comando `azure config mode`.
...
Para obter mais informações sobre como trabalhar com o Gerenciador de Recursos usando a xplat-cli, consulte [Usando a interface de linha de comando da plataforma cruzada do Microsoft Azure com o Gerenciador de Recursos][xplatarm].
...
1. Use o comando a seguir para criar um novo site do Azure. Substitua **mywebsite** por um nome exclusivo.
	...
	> [WACOM.NOTE] Se usar Git no controle do código-fonte do projeto, você poderá especificar o parâmetro `--git` para criar um repositório Git no Azure para esse site. Isso também irá inicializar um repositório Git no diretório do qual o comando foi executado, caso ele ainda não exista. Ele também criará um Git remoto chamado __azure__, que pode ser usado para enviar por push as implantações para o site do Azure usando o comando `git push azure master`.
...
<h2><a id="script"></a>Como criar script para a interface de linha de comando entre plataformas do Azure</h2>
...
Embora a xplat-cli registre informações de erro para STDERR, informações adicionais sobre erros também podem ser registradas em log em um arquivo **azure.err** no diretório no qual o script foi executado. Se as informações forem armazenadas nesse arquivo, o seguinte será gravado para STDOUT:
	...
	info:    foram registradas informações sobre o erro em azure.err
...
* Se você tiver problemas ao usar a xplat-cli ou o Azure, visite os [fóruns do Azure](http://social.msdn.microsoft.com/Forums/windowsazure/pt-br/home).

* Para obter mais informações sobre o Azure, consulte [http://www.windowsazure.com/](http://www.windowsazure.com).
