...
Parâmetros: \* evento \(cadeia de caracteres\) – "userlogin" \* usuário \(cadeia de caracteres\) – a identificação exclusiva do usuário. <script> se \(typeof AzureMLRecommendationsEvent = = "undefined"\) {AzureMLRecommendationsEvent = [];} AzureMLRecommendationsEvent.push \({evento: "userlogin", usuário: "ABCD10AA" }\); </script>
