...
4.  [Adicionar um carimbo de data/hora para CompleteDate][Adicionar um carimbo de data/hora para CompleteDate]
...
2.  No MainPage.xaml.cs, substitua o método **InsertTodoItem** existente pelo código a seguir:
...
1.  No Gerenciador de Soluções do Visual Studio, no projeto cliente todolist, abra o arquivo MainPage.xaml e substitua o elemento **StackPanel** pela definição abaixo. Em seguida, salve o arquivo. Isso altera o manipulador de eventos em **CheckBoxComplete** para que tratemos do `click` evento. Também podemos adicionar um bloco de texto ao lado da caixa de seleção e vinculá-lo ao carimbo de data/hora da data de conclusão.
...
3.  No arquivo MainPage.xaml.cs, substitua a classe **TodoItem** existente pela seguinte definição que inclui a nova propriedade **CompleteDate** como um tipo anulável.
    ...
    > [WACOM.NOTE] O `DataMemberAttribute` informa ao cliente para mapear a nova propriedade `CompleteDate` no aplicativo para a coluna `CompleteDate` definida na tabela TodoItem. Usando esse atributo, seu aplicativo pode ter nomes de propriedades em objetos que são diferentes dos nomes das colunas no Banco de Dados SQL.
