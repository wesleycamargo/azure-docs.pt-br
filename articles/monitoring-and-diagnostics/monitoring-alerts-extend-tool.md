---
title: Iniciar alertas de extensão do OMS para o Azure | Microsoft Docs
...
---
# <a name="initiate-extending-alerts-from-oms-into-azure"></a>Iniciar alertas de extensão do OMS para o Azure
...
Para iniciar voluntariamente a extensão de alertas do OMS para o Azure, siga as etapas listadas abaixo.

1. Na página Visão geral do Operations Management Suite (OMS), vá até Configurações e, em seguida, a seção Alertas. Clique no botão "Estender no Azure", como destacado na ilustração a seguir.

    ![Página de configurações de alerta do OMS com a opção de extensão](./media/monitor-alerts-extend/ExtendInto.png)
...
2. Depois que o botão é clicado, um Assistente de 3 etapas será mostrado, com a primeira etapa fornecendo detalhes do processo. Pressione Avançar para continuar.
    ...
    ![Estender Alertas do OMS para o Azure – Etapa 1](./media/monitor-alerts-extend/ExtendStep1.png)

3. Na segunda etapa, o sistema mostrará um resumo da alteração proposta listando os [Grupos de Ação](monitoring-action-groups.md) apropriados para os alertas do OMS. Se ações semelhantes forem vistas em mais de um alerta, o sistema proporá associar a todos eles um único grupo de ação.  O grupo de ação proposto segue a convenção de nomenclatura: *Espaçodetrabalho_GrupodeAção_Número*. Para continuar, clique em Avançar.
Uma tela de exemplo abaixo.

    ![Estender Alertas do OMS para o Azure – Etapa 2](./media/monitor-alerts-extend/ExtendStep2.png)


4. Na última etapa do assistente, você pode pedir para o OMS agendar a extensão de todos os alertas no Azure criando novos grupos de ação e associá-los a alertas, conforme mostrado na imagem anterior. Para prosseguir, clique em Concluir e confirme o aviso para iniciar o processo. Opcionalmente, os clientes também podem fornecer endereços de email para os quais eles gostariam que o OMS enviasse um relatório para terminar o processamento.

    ![Estender Alertas do OMS para o Azure – Etapa 3](./media/monitor-alerts-extend/ExtendStep3.png)

5. Depois que o assistente for concluído, o controle retornará para a página de configurações de alerta e a opção "Estender no Azure" será removida. Em segundo plano, o OMS agendará alertas no OMS para serem estendidos para o Azure. Isso pode levar algum tempo e quando a operação é iniciada por um breve período, os alertas do OMS não estarão disponíveis para modificação. O status atual será mostrado por meio de faixa e, se os endereços de email tiverem sido fornecidos durante a etapa 4, eles serão informados quando o processo em segundo plano conseguir estender todos os alertas para o Azure. 

6. Os alertas continuarão a ser listados no OMS, mesmo depois de serem estendidos para o Azure.

    ![Depois de estender alertas do OMS para o Azure](./media/monitor-alerts-extend/PostExtendList.png)


## <a name="option-2---using-api"></a>Opção 2: Uso da API
Para os clientes que quiserem controlar ou automatizar programaticamente o processo de estender alertas do OMS para o Azure. A Microsoft forneceu uma nova API AlertsVersion no Log Analytics.
...
> [!NOTE]
> A chamada de GET à API não fará os alertas do OMS serem estendidos para o Azure. Ele apenas fornecerá como resposta o resumo das alterações propostas. Para confirmar que essas alterações serão feitas para estender os alertas no Azure, uma chamada POST precisa ser feita para a API.
...
Para iniciar o agendamento da extensão dos alertas no OMS para o Azure, inicie um POST para a API. Fazer essa chamada/comando confirma a intenção do usuário, bem como a aceitação para que seus alertas no OMS sejam estendidos para o Azure e que as alterações sejam executadas conforme o indicado na resposta da chamada GET para a API. Opcionalmente, o usuário pode fornecer uma lista de endereços de email para o qual o OMS enviará um relatório, quando o processo em segundo plano agendado de estender os alertas no OMS para o Azure for concluído com êxito.
...
> [!NOTE]
> O resultado de estender alertas do OMS para o Azure pode variar do resumo fornecido por GET devido a qualquer alteração feita no sistema. Depois do agendamento, os alertas do OMS estarão temporariamente indisponíveis para edição/modificação embora novos alertas possam ser criados. 
