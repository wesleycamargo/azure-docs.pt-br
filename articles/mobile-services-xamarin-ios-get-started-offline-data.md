<properties 
	pageTitle="Usando dados offline nos serviços móveis (Xamarin iOS) | Centro de desenvolvimento de Serviços Móveis" 
...
1. Abra `QSTodoService.cs`. Edite os métodos `InsertTodoItemAsync()` e `CompleteItemAsync()` e comente as chamadas para `SyncAsync()`.
...
3. Para alterar a inicialização da interface do usuário para que ela não seja sincronizada durante a inicialização do aplicativo, edite o método `QSTodoListViewController.ViewDidLoad()`. Remova a chamada para `RefreshAsync()` no final do método e a substitua por uma chamada para `ReloadData()`:
...
<!-- URLs. -->
[Tratando conflitos com o suporte offline para os Serviços Móveis]: /en-us/documentation/articles/mobile-services-xamarin-ios-handling-conflicts-offline-data/ 
[Introdução aos dados]: /en-us/documentation/articles/partner-xamarin-mobile-services-ios-get-started-data/
[Introdução aos Serviços Móveis]: /en-us/documentation/articles/partner-xamarin-mobile-services-ios-get-started/
[Como usar o cliente do componente Xamarin para os serviços móveis do Azure]: /en-us/documentation/articles/partner-xamarin-mobile-services-how-to-use-client-library/
