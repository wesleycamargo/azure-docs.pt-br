...
2. Especifique um nome de canal; o campo de descrição é opcional. Em Configurações de Canal, selecione **Standard** para a opção de Codificação Ativa, com o Protocolo de Entrada definido como **RTP (MPEG-TS)**. Você pode deixar todas as outras configurações como estão.
...
###<a id=configure_elemental_rtp></a>Configurar o codificador Elemental Live 
...
1. Navegue até a interface Web **Elemental Live** e configure o codificador para streaming **UDP/TS**. 

2. Depois de criar um novo evento, role para baixo até os grupos de saída e adicione o grupo de saída **UDP/TS**.

3. Crie uma nova saída selecionando **Novo Fluxo** e clicando em **Adicionar Saída**.
...
5. Role para baixo até a “Transmissão 1" que acabou de criar, clique na guia **Vídeo** à esquerda e expanda a seção de configurações **Avançadas**. 
...
6. Obter URL de entrada do canal.
	...
	Navegue de volta para a ferramenta AMSE e verifique o status de conclusão do canal. Depois que o estado é alterado de **Iniciando** para **Em execução**, você pode obter a URL de entrada.
...
>[AZURE.IMPORTANT]Antes de clicar em **Iniciar** na interface Web do Elemental Live, é **essencial** que o canal esteja pronto. Além disso, lembre-se de não deixar o Canal em um estado pronto sem um evento por mais de 15 minutos.
