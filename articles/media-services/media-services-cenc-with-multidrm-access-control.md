...
A Microsoft sempre promoveu ativamente DASH e CENC juntamente com alguns principais vetores da indústria. Os Serviços de Mídia do Microsoft Azure dão suporte a DASH e a CENC. Para obter comunicados recentes, consulte os blogs da Mingfei: [Announcing Google Widevine license delivery services public preview in Azure Media Services](http://azure.microsoft.com/blog/announcing-google-widevine-license-delivery-services-public-preview-in-azure-media-services/) e [Azure Media Services adds Google Widevine packaging for delivering multi-DRM stream](http://azure.microsoft.com/blog/azure-media-services-adds-google-widevine-packaging-for-delivering-multi-drm-stream/).

###Visão geral deste artigo

Neste artigo, "várias DRM" abordam o seguinte:
...
**DRM**|**SO nativo/Aplicativo nativo**|**Suporte para navegadores de área de trabalho**|**Suporte para navegadores de dispositivos móveis**|**Suporte para formatos de streaming**
----|------|----|-----|----
**PlayReady**|Windows, Windows Phone, Xbox|IE11 no Windows 8.1+, Microsoft Edge|Windows Phone 8.1+|DASH, Smooth Streaming
**Widevine**|Android|Chrome 34+ em todos os PCs e Macs|Android 4.3+|DASH
...
Muitos-para-1|Várias chaves de conteúdo são necessárias para cada ativo. <br/><br/>Por exemplo, se você precisar aplicar proteção CENC dinâmica com vários DRM para MPEG-DASH e criptografia dinâmica AES-128 para HLS, precisará de duas chaves de conteúdo separadas, cada uma com seu próprio ContentKeyType. (Para a chave de conteúdo usada na proteção de CENC dinâmica, deve-se usar ContentKeyType.CommonEncryption; já para a chave de conteúdo usada na criptografia dinâmica do AES-128, deve-se usar ContentKeyType.EnvelopeEncryption.)<br/><br/>Outro exemplo: na proteção de CENC de conteúdo DASH, teoricamente, uma chave de conteúdo pode ser usada para proteger a transmissão de vídeo e outra chave de conteúdo, para proteger a transmissão de áudio. 
...
1. Criar um player usando a [API do Azure Media Player](http://amp.azure.net/libs/amp/latest/docs/). [A API ProtectionInfo do Azure Media Player](http://amp.azure.net/libs/amp/latest/docs/) permite que você especifique qual tecnologia DRM usar em diferentes plataformas DRM.
1. Matriz de teste:
...
George Trifonov, da equipe de Serviços de Mídia do Azure, escreveu um blog fornecendo as etapas detalhadas a fim de configurar o Active Directory do Azure para um aplicativo de player MVC do ASP.NET: [Integrar o OWNIN MVC dos Serviços de Mídia do Azure baseado em aplicativos com o Active Directory do Azure e restringir a entrega de chave de conteúdo baseado em declarações JWT](http://gtrifonov.com/2015/01/24/mvc-owin-azure-media-services-ad-integration/).

George também escreveu um blog sobre [Autenticação de token JWT nos Serviços de Mídia do Azure e Criptografia Dinâmica](http://gtrifonov.com/2015/01/03/jwt-token-authentication-in-azure-media-services-and-dynamic-encryption/). E aqui está seu [exemplo de integração do AD do Azure com a distribuição de chaves dos Serviços de Mídia do Azure](https://github.com/AzureMediaServicesSamples/Key-delivery-with-AAD-integration).
...
1. **Emissor** A URL deve terminar com **"/"**.  

	**Público** deve ser o ID de cliente do aplicativo player; você também deve adicionar **"/"** no final da URL do emissor.

		<add key="ida:audience" value="[Application Client ID GUID]" />
		<add key="ida:issuer" value="https://sts.windows.net/[AAD Tenant ID]/" /> 

	Em [Decodificador JWT](http://jwt.calebb.net/), você deve ver **aud** e **iss** como mostrado abaixo no token JWT:
...
##Tópicos adicionais no design e na implementação
...
1. Aquisição de licença do DRM pelo cliente que precisa estar em HTTPS.
...
As informações detalhadas sobre a substituição de chave do AD do Azure podem ser encontradas no documento: [Informações importantes sobre substituição de chave de assinatura no AD do Azure](http://msdn.microsoft.com/library/azure/dn641920.aspx).

Entre os [par de chaves pública-privada](https://login.windows.net/common/discovery/keys),
...
Se você der uma olhada em como um aplicativo Web chama um aplicativo de API em [Identidade do aplicativo com concessão de credenciais de cliente OAuth 2.0](active-directory-authentication-scenarios.md#web-application-to-web-api), o fluxo de autenticação é o seguinte:

1.	Um usuário está conectado ao AD do Azure no aplicativo Web (confira [Navegador da Web para aplicativo Web](active-directory-authentication-scenarios.md#web-browser-to-web-application)).
...
###Usando EME (extensões de mídia criptografada) para PlayReady

Em um navegador moderno com suporte a EME/PlayReady, como o IE 11 no Windows 8.1 e posteriores e navegador Microsoft Edge no Windows 10, o PlayReady será o DRM subjacente para EME.
...
![Usuários não qualificados](./media/media-services-cenc-with-multidrm-access-control/media-services-unentitledusers.png.png)

###Executando STS (Serviço de Token Seguro) personalizado

Para o cenário de execução de STS personalizado, o token JWT será emitido pelo STS personalizado usando a chave simétrica ou assimétrica.
