...
1. Salve o arquivo de predefinições modificado no disco rígido local e usar código da seguinte maneira para codificar com a predefinição personalizada:
	...
	// Carregar arquivo e criar ativo IAtivo ativo = CreateAssetAndUploadSingleFile(AssetCreationOptions.None, @"C:\\TEMP\\Original.mp4");
	...
	// Obter uma referência de processador de mídia e passar para ela o nome do processador a ser usado para a tarefa específica. IMediaProcessor processor = GetLatestMediaProcessorByName("Codificador de Mídia do Azure"); Console.WriteLine("Got MP " + processor.Name + ", ID : " + processor.Id + ", versão: " + processor.Version);
	 
	// Ler os dados de configuração em uma configuração string. string = File.ReadAllText(inputPresetFile);
	 
	// Criar uma tarefa com os detalhes de codificação, usando uma predefinição de cadeia de caracteres. ITask task = job.Tasks.AddNew("Codificar tarefa para " + asset.Name + ", codificado usando " + presetName, processor, configuration, Microsoft.WindowsAzure.MediaServices.Client.TaskOptions.None);
	 
	// Especifique o ativo de entrada a ser codificado. task.InputAssets.Add(asset);
	 
	// Adicione um ativo de saída para conter os resultados do trabalho. task.OutputAssets.AddNew("Ativo de saída para " + asset.Name + ", codificado usando " + presetName, AssetCreationOptions.None);
	 
	// Inicie o job. job.Submit();
