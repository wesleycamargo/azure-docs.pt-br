...
> [!NOTE]
> Você pode criptografar cada ativo com vários tipos de criptografia (AES-128, PlayReady, Widevine, FairPlay). Ver [tipos de criptografia e protocolos de Streaming](content-protection-overview.md#streaming-protocols-and-encryption-types), para ver o que faz sentido para combinar.
...
* Obter as credenciais necessárias para usar as APIs de serviços de mídia, seguindo [APIs de acesso](access-api-cli-how-to.md)

## <a name="download-code"></a>Código de download

Clone um repositório do GitHub que contém o exemplo de .NET completo discutido neste tópico em sua máquina usando o comando a seguir:
...
> [!NOTE]
> O exemplo cria recursos exclusivos toda vez que você executar o aplicativo. Normalmente, você vai reutilizar recursos existentes como transformações e políticas (se existentes recursos têm configurações necessárias). 
...
O [Ativo](https://docs.microsoft.com/rest/api/media/assets) de saída armazena o resultado do trabalho de codificação. Depois que a codificação estiver pronta, O ativo de saída é publicado usando a criptografia AES (ClearKey).  
...
Neste tutorial, criamos a entrada do trabalho com base em um arquivo que é ingerido diretamente de um [URL de origem HTTPs](job-input-from-http-how-to.md).
...
Quando um fluxo é solicitado por um player, os Serviços de Mídia usam a chave especificada para criptografar dinamicamente o conteúdo usando a criptografia AES (neste caso, usando criptografia AES.) Para descriptografar o fluxo, o player solicita a chave do serviço de distribuição de chaves. Para determinar se o usuário está autorizado a obter a chave, o serviço avalia as políticas de autorização que você especificou para a chave.

[!code-csharp[Main](../../../media-services-v3-dotnet-tutorials/AMSV3Tutorials/EncryptWithAES/Program.cs#GetOrCreateContentKeyPolicy)]

## <a name="get-a-token"></a>Obter um token
        
Neste tutorial, podemos especificar para a política de chave de conteúdo para ter uma restrição de token. A política restrita do token deve ser acompanhada por um token emitido por um Serviço de Token de Segurança (STS). Os Serviços de mídia oferecem suporte a tokens no formato [Token Web JSON](https://msdn.microsoft.com/library/gg185950.aspx#BKMK_3) (JWT) e isso é o que nós configuramos no exemplo.
