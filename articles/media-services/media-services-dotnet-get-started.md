<properties
	pageTitle="Introdução ao fornecimento de VoD (Vídeo sob Demanda) usando SDK .NET"
...
# Introdução ao fornecimento de VoD (Vídeo sob Demanda) usando SDK .NET
...
- Sistemas operacionais: Windows 7, Windows 2008 R2, Windows 8 ou mais recente.
...
2. Usar o pacote NuGet [windowsazure.mediaservices.extensions](https://www.nuget.org/packages/windowsazure.mediaservices.extensions) para instalar **extensões do SDK do .NET dos Serviços de Mídia do Azure**. As Extensões do SDK do .NET dos Serviços de Mídia do Azure são um conjunto de métodos de extensão e funções auxiliares que simplificarão seu código e tornarão mais fácil desenvolver com os Serviços de Mídia. Instalar esse pacote também instala o **SDK do .NET dos Serviços de Mídia** e adiciona todas as outras dependências necessárias.
