...
A maneira mais rápida para adicionar a biblioteca a seu projeto no **Eclipse** é `Right click on your project -> Android Tools -> Add Support Library...`.
...
-   Especifique um ícone usado para notificações (tanto nas de aplicativo quanto nas de sistema), copiando e editando a seção a seguir (entre as marcas `<application>` e `</application>`):

			<meta-data android:name="engagement:reach:notification:icon" android:value="<name_of_icon_WITHOUT_file_extension_and_WITHOUT_'@drawable/'>" />

> [AZURE.IMPORTANT] Esta seção é **obrigatória** se você planeja usar notificações de sistema durante a criação de campanhas do Reach. O Android impede que as notificações de sistema sem ícones sejam exibidas. Portanto, se você omitir esta seção, os usuários finais não poderão recebê-las.

-   Se você criar campanhas com notificações de sistema usando o quadro geral, você precisa adicionar as seguintes permissões (após a marca `</application>`) se estiverem faltando:
...
**Seu aplicativo agora está pronto para receber e exibir as campanhas do Reach!**

##Como lidar com o envio de dados

### Integração

Se desejar que seu aplicativo seja capaz de receber envios de dados do Reach, você precisa criar uma subclasse de `com.microsoft.azure.engagement.reach.EngagementReachDataPushReceiver` e fazer referência a ela no arquivo `AndroidManifest.xml` (entre as marcas `<application>` e/ou `</application>`):
...
O parâmetro de categoria é opcional quando você cria uma campanha de envio de dados, e permite que você filtre esses envios. Isso é útil se você tiver vários receptores de difusão lidando com tipos diferentes de envios de dados, ou se você deseja enviar por push diferentes tipos de dados "Base64" e deseja identificar tipo deles antes de analisá-los.
...
-   Um receptor de difusão deve retornar `null` no retorno de chamada se ele não souber como lidar com um envio de dados. Você deve usar a categoria para determinar se seu receptor de difusão deve lidar com o envio de dados ou não.
-   Se um receptor de difusão aceita o envio de dados, esse receptor deve retornar `true` no retorno de chamada.
-   Se um receptor de difusão reconhece o envio de dados mas o descarta por qualquer motivo, esse receptor deve retornar `false` no retorno de chamada. Por exemplo, retornar `false` quando os dados recebidos são inválidos.
-   Se um receptor de difusão retorna `true` enquanto outro um retorna `false` para o mesmo envio de dados, o comportamento será indefinido; portanto, você nunca deve fazer isso.
...
Ao seguir o procedimento de integração descrito acima, o serviço do Engagement conecta-se aos servidores do Engagement somente quando as estatísticas precisam ser reportadas (mais um tempo limite de 1 minuto). Consequentemente, **campanhas do Reach só podem ser recebidas durante uma sessão de usuário**. Felizmente, o Engagement pode ser configurado para **permitir que seu aplicativo receba campanhas do Reach a qualquer momento**, inclusive quando o dispositivo estiver dormindo (o dispositivo deve, é claro, ter uma conexão de rede ativa; as mensagens são atrasadas enquanto o dispositivo está offline).
...
Se você não desejar que a sobreposição seja adicionada à sua atividade e se você não incluir o layout de notificação em seu próprio layout, você pode desabilitar a sobreposição para essa atividade no `AndroidManifest.xml` pela adição de uma seção `meta-data`, como no exemplo a seguir:
...
Quando você modifica os layouts fornecidos, pode modificar também a aparência de todas as notificações. Categorias permitem que você defina diversas aparências de destino (possivelmente comportamentos) para notificações. Uma categoria pode ser especificada quando você cria uma campanha do Reach. Tenha em mente que categorias também permitem personalizar anúncios e pesquisas, e isso é descrito mais adiante neste documento.
...
> [AZURE.IMPORTANT] Leia o aviso sobre o atributo android: process \<android-sdk-engagement-process\> no tópico Como integrar o Engagement no Android.
...
Ele é passada como um parâmetro `String` ou indiretamente em um objeto `EngagementReachContent` que tem um método `getCategory()`.

Você pode alterar a maior parte do processo de criação de notificação redefinindo métodos em `EngagementDefaultNotifier`; para uma personalização mais avançada, sinta-se livre para dar uma olhada na documentação técnica e no código-fonte.
...
Se você deseja compilar o objeto `Notification` do zero, você pode retornar `false` para o método e chamar você mesmo o `notify` no `NotificationManager`. Nesse caso, é importante que você mantenha um `contentIntent`, um `deleteIntent` e o identificador de notificação usado por `EngagementReachReceiver`.
...
O gerenciamento do clique em um comunicado exclusivamente de notificação só pode ser personalizado substituindo-se `EngagementDefaultNotifier.onNotifAnnouncementIntentPrepared` para modificar o `Intent` preparado. Usar esse método permite ajustar os sinalizadores facilmente.
...
Ao usar a categoria padrão, alguns métodos de ciclo de vida são chamados no objeto `EngagementReachInteractiveContent` para reportar estatísticas e atualizar o estado da campanha:
...
-   Se a notificação é liberada, o método `exitNotification` é chamado, a estatística é reportada e próximas campanhas agora podem ser processadas.
-   Se a notificação é clicada, `actionNotification` é chamado, a estatística é reportada e a intenção associada é iniciada.
...
-   Você não estende `EngagementDefaultNotifier`, por exemplo, você implementou o tratamento de categoria a partir do zero.
...
-   Para notificações no aplicativo, você substituiu `prepareInAppArea`, certifique-se de mapear pelo menos `actionNotification` para um dos seus controles de IU.
...
Esses arquivos compartilham dois layouts comuns para a área de título e a área do botão. O layout para o título é `engagement_content_title.xml` e usa o arquivo epônimo sorteável para a tela de fundo. O layout dos botões de ação e de saída é `engagement_button_bar.xml` e usa o arquivo epônimo sorteável para a tela de fundo.

Em uma pesquisa, o layout de pergunta e suas opções são dinamicamente infladas com várias vezes o arquivo de layout `engagement_question.xml` para as perguntas e o arquivo `engagement_choice.xml` para as opções.

#### Categorias

##### Layouts alternativos

Assim como as notificações, a categoria de campanha pode ser usada para ter layouts alternativos para seus anúncios e pesquisas.
...
Em seguida, você precisa implementar `MyCustomTextAnnouncementActivity`; se você quiser apenas alterar o layout (mas manter os mesmos identificadores de exibição), você só precisa definir a classe como no exemplo a seguir:
...
Assim como para personalização da notificação avançada, é recomendável examinar o código-fonte da implementação padrão.

Eis aqui algumas coisas que se deve manter em mente: O Reach iniciará a atividade com uma intenção específica (correspondente ao filtro de intenções), além de um parâmetro extra que é o identificador de conteúdo.
...
Para estatísticas, você deve reportar o conteúdo que é exibido no evento `onResume`:
...
Em seguida, não se esqueça de chamar  `actionContent(this)` ou então `exitContent(this)` no objeto de conteúdo antes que a atividade entre em segundo plano.

Se você não chamar  `actionContent` ou então `exitContent`, as estatísticas não serão enviadas (ou seja, não haverá análise da campanha) e, mais importante, as próximas campanhas não serão notificadas até o processo do aplicativo ser reiniciado.

A orientação ou outras alterações de configuração podem mudar o código, tornando difícil determinar se a atividade entra em segundo plano ou não; a implementação padrão garante que o conteúdo seja relatado como encerrado se o usuário sair da atividade (pressionando `HOME` ou então `BACK`), mas isso não ocorre se a orientação for alterada.
...
Como você pode ver, se você tiver chamado `actionContent(this)` e então terminado a atividade, `exitContent(this)` pode ser chamado com segurança sem ter qualquer efeito.
