...
1. Se você tiver um cluster do Windows, faça com que ele seja remoto em um cluster do HDInsight que tenha acesso a uma conta de Repositório do Data Lake. Para instruções, consulte [Conectar-se a clusters usando RDP](hdinsight-administer-use-management-portal.md#connect-to-clusters-using-rdp). No cluster de área de trabalho, abra a linha de comando do Hadoop.

	Se você tiver um cluster do Linux, use SSH para se conectar ao cluster. Consulte [Conectar-se a um cluster HDInsight baseado em Linux](hdinsight-hadoop-linux-use-ssh-unix.md#connect-to-a-linux-based-hdinsight-cluster). Execute os comandos do prompt de SSH.
