...
## <a name="the-agent-operation-failed-because-of-a-communication-error-with-the-dpm-agent-coordinator-service-on-server"></a>A operação do agente falhou devido a um erro de comunicação com o serviço Coordenador de Agentes do DPM no Servidor 
...
| Backup | Proteger novamente a VM do VMWare em um novo servidor MABS não aparece como disponível para adicionar. | Propriedades do VMWare são apontadas para um servidor MABS antigo e obsoleto. Para resolver esse problema: no VCenter (equivalente do SC-VMM), vá para a guia 'Resumo' e 'Atributos Personalizados'.  Exclua o antigo nome do servidor MABS do valor 'DPMServer'.  Volte para o novo servidor MABS e modifique o PG.  Depois de usar o botão 'Atualizar', a VM verá uma caixa de seleção disponível para adicionar à proteção. |
...
| Backup | Falha em trabalhos de criação de ponto de recuperação online para VM do VMware. O DPM encontrou um erro de VMware ao tentar obter informações de ChangeTracking. ErrorCode - FileFaultFault (ID 33621 ) |  1. Redefinir o ctk no VMWare para as VMs afetadas <br/> 2. Verificar se o disco independente não está em vigor no VMWare <br/> 3. Interromper a proteção para as VMs afetadas e proteger novamente com o botão Atualizar <br/> 4. Executar uma CC para as VMs afetadas|
...
| Operação | Detalhes do erro | Solução alternativa |
| --- | --- | --- |
| Tentar configurar notificações por email usando a conta do Office365. | Obtendo a ID do erro: 2013| **Causa:**<br/> Tentativa de usar a conta do Office 365 <br/> **Ação recomendada:**<br/> 1. A primeira coisa que se deve assegurar é que "Permitir retransmissão anônima em um conector de recebimento" para o servidor DPM esteja configurada no Exchange. Confira [este link](http://technet.microsoft.com/en-us/library/bb232021.aspx) sobre como configurá-la.  <br/> 2. Se você não pode usar uma retransmissão SMTP interna e precisa configurar usando o servidor do Office 365, você pode configurar o IIS para ser uma retransmissão para isso. Você precisa configurar o servidor DPM para ser [capaz de retransmitir o SMTP para o O365 usando o IIS](https://technet.microsoft.com/en-us/library/aa995718(v=exchg.65).aspx) <br/><br/> Observação importante: na etapa 3 -> g -> ii, certifique-se de usar o formato user@domain.com e NÃO domínio\usuário <br/> Aponte o DPM para usar o nome do servidor local como servidor SMTP, a porta 587 e o email do usuário do qual os emails devem vir. <br/> O nome de usuário e a senha na página de instalação de SMTP do DPM devem ser uma conta de domínio no domínio em que o DPM está. <br/><br/> OBSERVAÇÃO: ao alterar o endereço do servidor SMTP, faça a alteração para as novas configurações, feche a caixa de configurações e, em seguida, reabra-a para ter certeza de que ela reflete o novo valor.  Apenas alterar e testar não utilizará sempre as novas configurações, então a melhor prática é testar dessa maneira. <br/><br/> A qualquer momento durante esse processo, você pode limpar essas configurações fechando o console do DPM e editando as seguintes chaves do Registro:<br/> HKLM\SOFTWARE\Microsoft\Microsoft Data Protection Manager\Notification\ <br/> Exclua as chaves SMTPPassword e SMTPUserName. <br/> Você pode adicioná-las novamente na interface do usuário quando você iniciá-lo novamente.
