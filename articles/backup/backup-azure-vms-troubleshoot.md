...
| A VM não está em um estado que permite todos os backups. |<ul><li>Se a VM estiver em um estado transitório entre **Execução** e **Desligada**, aguarde a alteração do estado para mudar. Em seguida, dispare o trabalho de backup. <li> Se a VM é uma VM do Linux e usa o módulo de kernel do Linux Security-Enhanced, excluir o caminho do agente Linux do Azure **/var/lib/waagent** da política de segurança e verifique se a extensão de Backup do Azure está instalada.  |
| Uma máquina virtual do Azure não foi encontrada. |Esse erro ocorre quando a VM primária é excluída, mas a política de backup ainda parece para uma VM excluída. Corrija esse erro, da seguinte maneira: <ol><li>Recrie a máquina virtual com o mesmo nome e com o mesmo nome do grupo de recursos **nome do serviço de nuvem**, <br>**ou o** <li> Desative a proteção para esta VM para que os trabalhos de backup não sejam criados. </ol> |
...
| O trabalho não está em um estado cancelável: <br>Aguarde até que o trabalho seja concluído. <br>**ou o**<br> O trabalho selecionado não está em um estado cancelável: <br>Aguarde a conclusão do trabalho. |É provável que o trabalho esteja quase terminando. Aguarde até o trabalho ser concluído.|
...
## <a name="troubleshoot-backup-of-encrypted-vms"></a>Solucionar problemas de backup de VMs criptografadas

### <a name="azure-backup-doesnt-have-permissions-for-key-vault-access"></a>O Backup do Azure não tem permissões para acesso de Cofre de chaves
- **Erro de código**: UserErrorKeyVaultPermissionsNotConfigured
- **Mensagem de erro**: O Serviço de Backup do Azure não tem permissões suficientes para o cofre da chave para Backup de máquinas virtuais criptografadas.
- **Resolução**: Atribuir permissões de Backup do Azure para Key Vault na [portal](backup-azure-vms-encryption.md#provide-permissions), ou com [PowerShell](backup-azure-vms-automation.md#enable-protection)

### <a name="the-vm-cant-be-restored-because-the-associated-key-vault-doesnt-exist"></a>A VM não pode ser restaurada porque o Cofre de chaves associado não existe
- **Resolução**: Certifique-se de que você [criou um Key Vault](../key-vault/quick-create-portal.md#create-a-vault).
- **Resolução**: Siga [estas instruções](backup-azure-restore-key-secret.md) para restaurar uma chave e o segredo, mesmo se não existirem no cofre de chaves.

### <a name="the-vm-cant-be-restored-because-the-associated-key-doesnt-exist"></a>A VM não pode ser restaurada porque a chave associada não existe
- **Erro de código**: UserErrorKeyVaultKeyDoesNotExist
- **Mensagem de erro**: Não é possível restaurar essa VM criptografada pois a chave associada a essa VM não existe.
- **Resolução**: Siga [estas instruções](backup-azure-restore-key-secret.md) para restaurar uma chave e o segredo, mesmo se não existirem no cofre de chaves.

### <a name="the-vm-cant-be-restored-because-azure-backup-doesnt-have-authorization"></a>A VM não pode ser restaurada porque o Backup do Azure não tem autorização
- **Erro de código**: ProviderAuthorizationFailed/UserErrorProviderAuthorizationFailed
- **Mensagem de erro**: O Serviço de Backup não tem autorização para acessar recursos em sua assinatura.
- **Resolução**: Restaure discos conforme recomendado. [Saiba mais](backup-azure-vms-encryption.md#restore-an-encrypted-vm). 


## <a name="networking"></a>Rede
Como todas as extensões, a extensão de Backup do Azure precisa acessar a internet pública para trabalhar. A falta de acesso à Internet Pública pode se manifestar de várias formas:
