...
> [!div class="checklist"]
> * Verificar os pré-requisitos para fazer backup de uma Instância do SQL Server.
...
1. Garanta que você tenha uma Instância do SQL Server em execução no Azure. Você pode [criar uma Instância do SQL Server rapidamente](../sql-database/sql-database-get-started-portal.md) no marketplace.
2. Examine as limitações da versão prévia pública abaixo.
3. Examine o suporte ao cenário.
4. [Examine as perguntas comuns](faq-backup-sql-server.md) sobre esse cenário.


## <a name="preview-limitations"></a>Limitações de visualização

Esta versão prévia pública tem várias limitações.

- A VM que executa o SQL Server exige conectividade com a Internet para acessar os endereços IP públicos do Azure. 
- É possível fazer backup de até 2.000 bancos de dados do SQL Server em um cofre. Caso você tenha mais, crie outro cofre. 
- Os backups de [grupos de disponibilidade distribuída](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/distributed-availability-groups?view=sql-server-2017) não funcionam por completo.
- Não há suporte para FCIs (Instâncias do Cluster de Failover) Always On do SQL Server.
- O backup do SQL Server deve ser configurado no portal. No momento, não é possível configurar o backup com o Azure PowerShell, a CLI ou as APIs REST.
...
**Áreas geográficas com suporte** | ASE (Sudeste da Austrália); BRS (Sul do Brasil); CNC (Canadá Central); CE (Leste do Canadá); CUS (Centro dos EUA); EA (Ásia Oriental); AE (Leste da Austrália); EUS (Leste dos EUA); EUS2 (Leste dos EUA 2); INC (Índia Central); INS (Sul da Índia); JPE (Leste do Japão); JPW (Oeste do Japão); KRC (Coreia Central); KRS (Sul da Coreia); NCUS (Centro-Norte dos EUA); NE (Europa Setentrional); SCUS (Centro-Sul dos EUA); SEA (Sudeste Asiático); UKS (Sul do Reino Unido); UKW (Oeste do Reino Unido); WCUS (Centro-Oeste dos EUA); WE (Europa Ocidental); WUS (Oeste dos EUA) e WUS 2 (Oeste dos EUA 2)
...
Para todas as operações, a máquina virtual (VM) do SQL Server precisa ter conectividade com os endereços IP públicos do Azure. As operações de VM (descoberta de banco de dados, configuração de backups, agendamento de backups, restauração de pontos de recuperação etc.) falharão sem conectividade com os endereços IP públicos. Estabeleça a conectividade com uma destas opções:

- **Permitir os intervalos de IP do datacenter do Azure**: Permita os [intervalos de IP](https://www.microsoft.com/download/details.aspx?id=41653) no download. Para obter acesso em um NSG (Grupo de Segurança de Rede), use o cmdlet **Set-AzureNetworkSecurityRule**.
...
9. Selecione a VM que executa o banco de dados do SQL Server > **Descobrir BDs**. 
10. O Backup do Azure descobre todos os bancos de dados do SQL Server na VM. Durante a descoberta, ocorre o seguinte em segundo plano:
...
- Alternativamente, você pode habilitar a proteção automática. A proteção automática protege os bancos de dados existentes de uma só vez e protege automaticamente os novos bancos de dados adicionados à instância do grupo de disponibilidade.


Configure o backup da seguinte maneira:

1. No painel do cofre, selecione **Backup**. 
...
5. Em **Meta de Backup**, selecione **Configurar Backup**.
...
6. Selecione todos os bancos de dados que deseja proteger > **OK**.

    ![Proteção do banco de dados](./media/backup-azure-sql-database/select-database-to-protect.png)

    - Todas as Instâncias do SQL Server são mostradas (autônomas e grupos de disponibilidade).
    - Selecione a seta para baixo à esquerda do nome da instância/do grupo de disponibilidade para filtrá-lo.

7. No menu **Backup**, selecione **Política de backup**. 

    ![Selecionar a Política de backup](./media/backup-azure-sql-database/select-backup-policy.png)

8. Em **Escolher política de backup**, selecione uma política e, em seguida, clique em **OK**.
...
10. Acompanhe o progresso da configuração na área **Notificações** do portal.
...
- Não há nenhum limite no número de bancos de dados que podem ser selecionados para a proteção automática de uma só vez.
...
Caso precise desabilitar a proteção automática, clique no nome da instância em **Configurar Backup** e selecione **Desabilitar Proteção Automática** para a instância. Todos os bancos de dados continuarão sendo copiados em backup. No entanto, os bancos de dados futuros não serão protegidos automaticamente.
...
3. A conta de serviço virtual do Windows **NT SERVICE\AzureWLBackupPluginSvc** foi criada durante a fase de descoberta do SQL e do registro da máquina virtual. Insira o nome da conta, conforme mostrado em **Inserir o nome do objeto a ser selecionado**. Selecione **Verificar Nomes** para resolver o nome. Clique em **OK**.

    ![Selecione Verificar Nomes para resolver o nome de serviço desconhecido](./media/backup-azure-sql-database/check-name.png)


4. Em **Funções de Servidor**, verifique se a função **sysadmin** está selecionada. Clique em **OK**. As permissões necessárias agora devem existir.

    ![Verifique se a função de servidor sysadmin está selecionada](./media/backup-azure-sql-database/sysadmin-server-role.png)

5. Agora, associe o banco de dados ao cofre dos Serviços de Recuperação. No portal do Azure, na lista **Servidores Protegidos**, clique com o botão direito do mouse no servidor que está em um estado de erro > **Redescobrir BDs**.

    ![Verifique se o servidor tem as permissões apropriadas](./media/backup-azure-sql-database/check-erroneous-server.png)

6. Verifique o progresso na área **Notificações**. Quando os bancos de dados selecionados são descobertos, é exibida uma mensagem de êxito.
