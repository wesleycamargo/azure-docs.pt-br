...
| Consistência do sistema de arquivos | Sim - para computadores executando Windows | Há dois cenários nos quais o ponto de recuperação pode ser *consistente com o sistema de arquivos*:<ul><li>fazer o backup de VMs Linux no Azure, já que o Linux não tem uma plataforma equivalente para VSS.<li>Falha do VSS durante backup para VMs do Windows no Azure.</li></ul> Em ambos os casos, o melhor que se pode fazer é garantir que: <ol><li>* a VM seja iniciada*. <li>Não haja *corrupção*.<li>Não haja *perda de dados*.</ol> Os aplicativos precisam implementar seu próprio mecanismo de "correção" nos dados restaurados.|
