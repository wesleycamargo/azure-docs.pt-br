...
A execução transacional de JavaScript e com linguagem integrada do Banco de Dados de Documentos permite que os desenvolvedores gravem **procedimentos armazenados**, **gatilhos** e **funções definidas pelo usuário (UDFs)** nativamente em JavaScript. Isso permite que os desenvolvedores gravem a lógica de aplicativo, que pode ser enviada e executada diretamente nas partições de armazenamento do banco de dados.
...
-   **Lógica de procedimento:** JavaScript, enquanto uma linguagem de programação de alto nível, oferece uma interface rica e familiar para expressar a lógica de negócios. Você pode realizar sequências complexas de operações de maneira mais próxima aos dados.

-   **Transações atômicas:** O Banco de Dados de Documentos garante que as operações do banco de dados sejam realizadas dentro de um único procedimento armazenado ou gatilho a fim de que sejam atômicas. Isso permite que um aplicativo combine operações relacionadas em um único lote para que todas ou nenhuma delas seja bem-sucedida.

-   **Desempenho:** O fato de que JSON é intrinsicamente mapeado para o sistema de tipos de linguagem JavaScript e também é a unidade básica de armazenamento no Banco de Dados de Documentos que permite uma série de otimizações, como a materialização lenta de documentos JSON no conjunto de buffer e sua disponibilização sob demanda ao código de execução. Há mais benefícios de desempenho associados ao envio da lógica de negócios ao banco de dados:
...
A criação e execução de gatilhos, procedimentos armazenados e operadores de consulta personalizados são suportadas por meio da API REST e dos SDKs clientes em diversas plataformas, incluindo .NET, Node.js e JavaScript. **Esse tutorial utiliza o SDK Node.js** para ilustrar a sintaxe e o uso de procedimentos armazenados, gatilhos e UDFs.
...
O próximo trecho mostra como usar o objeto de contexto para interagir com entidades do Banco de Dados de Documentos.
...
Esse procedimento armazenado assume como entrada documentToCreate, o corpo de um documento a ser criado na coleção atual. Todas essas operações são assíncronas e dependem de retornos de chamada de função do JavaScript. A função de retorno de chamada possui dois parâmetros, um para o objeto de erro no caso de falhas na operação e um para o objeto criado. Dentro da chamada de retorno, os usuários podem lidar com a exceção ou lançar um Erro. Caso uma chamada de retorno não seja fornecida e haja um erro, o tempo de execução do Banco de Dados de Documento lançará um Erro.

No exemplo acima, a chamada de retorno lança um Erro se a operação tiver falhado. Caso contrário, ela definirá a ID do documento criada como o corpo da resposta ao cliente. A seguir, mostramos como esse procedimento armazenado é executado com parâmetros de entrada.
...
O exemplo descrito demonstra como usar procedimentos armazenados. Iremos discutir os gatilhos e funções definidas pelo usuário (UDFs) posteriormente no tutorial. Primeiramente, vamos observar as características gerais do suporte a script do Banco de Dados de Documentos.

## Suporte de tempo de execução

O SDK do lado do servidor de JavaScript do Banco de Dados de Documentos oferece suporte para a maioria dos principais recursos de linguagem JavaScript conforme o padrão [ECMA-262][ECMA-262].
...
## Gatilhos e funções definidas pelo usuário
...
O procedimento armazenado é registrado executando uma solicitação POST em relação à URI dbs/sehcAA==/colls/sehcAIE2Qy4=/sprocs com o corpo contendo o procedimento armazenado a ser criado. Gatilhos e UDFs podem ser registrados da mesma forma emitindo uma solicitação POST em relação a /triggers e /udfs, respectivamente.
Esse procedimento armazenado pode, então, ser executado emitindo uma solicitação POST em relação ao seu link de recursos:
...
Além do cliente Node.js, o Banco de Dados de Documentos oferece suporte a SDKs de .NET, JavaScript e Python. Os procedimentos armazenados, gatilhos e UDFs também podem ser criados e executados usando qualquer um desses SDKs. O exemplo a seguir mostra como criar e executar um procedimento armazenado usando o cliente .NET. Observe como os tipos .NET são transferidos para o procedimento armazenado como JSON e lidos novamente.
...
Essa amostra mostra como usar o SDK .NET para criar um pré-gatilho e um documento com o gatilho habilitado.
...
E o exemplo a seguir mostra como criar uma função definida pelo usuário (UDF) e utilizá-la em uma consulta SQL do Banco de Dados de Documentos.
...
3.  JavaScript – sistema de tipos JSON [][2]<http://www.json.org/js.html></a>

4.  Extensibilidade segura e portátil do banco de dados - [][3]<http://dl.acm.org/citation.cfm?id=276339></a>

5.  Arquitetura do banco de dados orientada a serviços - [][4]<http://dl.acm.org/citation.cfm?id=1066267&coll=Portal&dl=GUIDE></a>

6.  Hospedando o tempo de execução .NET no Microsoft SQL server - [][5]<http://dl.acm.org/citation.cfm?id=1007669></a>
