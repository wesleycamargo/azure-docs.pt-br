<properties linkid="develop-net-tutorials-compute-intensive-task-on-a-virtual-machine" urlDisplayName="Computar tarefa do .NET intensiva" pageTitle="Computar tarefa do .NET intensiva em uma máquina virtual - Azure" metaKeywords="implantando aplicativo do .NET de computação, aplicativo do .NET VM, monitoramento da fila do Barramento de Serviço, monitoramento remoto" description="Saiba como implantar e executar um aplicativo do .NET que exija muita computação em uma máquina virtual Azure e usar filas do Barramento de Serviço para monitorar o progresso remotamente." metaCanonical="" services="virtual-machines" documentationCenter=".NET" title="Como executar uma tarefa que exija muita computação no .NET em uma máquina virtual Azure" authors="waltpo" solutions="" manager="" editor="" />
...
7. Insira um nome para o administrador no campo **Nome do Usuário**. Lembre-se do nome e da senha que você digitará a seguir, você irá usá-los ao fazer logon remotamente na máquina virtual.
8. Insira uma senha no campo **Nova senha** e insira-a novamente no campo **Confirmar**.
...
<h2>Como criar um namespace do barramento de serviço</h2>

Para começar a usar filas do barramento de serviço no Azure, você deve primeiro criar um espaço de nomes de serviço. Um namespace de serviço fornece um contêiner de escopo para endereçamento dos recursos do Barramento de Serviço em seu aplicativo.

Para criar um namespace de serviço:

1.  Faça logon no [Portal de Gerenciamento do Azure](https://manage.windowsazure.com).
2.  No painel de navegação à esquerda do Portal de Gerenciamento, clique em **Service Bus**.
...
5.  Depois de verificar se o nome do namespace está disponível, escolha a região em que o namespace deve ser hospedado (não se esqueça de usar a mesma região em que a máquina virtual está hospedada).
    ...
    <p>Escolha a **mesma região** que você usa ou pretende usar na máquina virtual. Isso proporcionará o melhor desempenho.</p>
    </div>
...
7. Clique na marca de seleção. O sistema agora cria o namespace de serviço e o habilita. Talvez você precise aguardar vários minutos enquanto o sistema provisiona recursos para sua conta.
...
1.  No painel de navegação à esquerda, clique no nó **Service Bus**, para
    ...
    ![Captura de tela de namespaces disponíveis][available_namespaces]
...
3. Clique em **Chave de Acesso**.   
    ![Botão da chave de acesso][access_key_button]
...
<h2>Como executar os aplicativos do .NET.</h2>
...
<h2>Como parar os aplicativos do .NET</h2>
...
<h2>Alternativa à criação e à exclusão da fila com TSPSolver</h2>
