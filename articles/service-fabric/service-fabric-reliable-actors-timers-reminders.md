...
O período seguinte do temporizador é iniciado depois que o retorno de chamada concluir a execução. Isso implica a interrupção do temporizador enquanto o retorno de chamada estiver em execução e ele é iniciado quando o retorno de chamada for concluído.

No caso de o ator ser um ator com monitoração de estado, o tempo de execução de atores salva o estado do ator quando o retorno de chamada é concluído, como no exemplo anterior. Se ocorrer um erro ao salvar o estado, esse objeto de ator será desativado e uma nova instância será ativada. Um método de retorno de chamada que não modifica o estado de ator pode ser registrado como um retorno de chamada do temporizador no modo somente leitura, especificando o atributo Readonly no retorno de chamada do temporizador, conforme descrito na seção sobre [métodos do Readonly](service-fabric-reliable-actors-introduction.md#readonly-methods).

Todos os temporizadores são interrompidos quando o ator é desativado como parte da coleta de lixo e nenhum retorno de chamada do temporizador é chamado depois disso. Além disso, o tempo de execução de atores não retém todas as informações sobre os temporizadores que estavam em execução antes de desativação. Cabe ao ator registrar todos os temporizadores necessários quando ele for reativado no futuro. Para obter mais informações, consulte a seção sobre [coleta de lixo de ator](service-fabric-reliable-actors-lifecycle.md).

## Lembretes de ator
Os lembretes são um mecanismo para acionar chamadas de retorno persistentes em um ator em horários específicos. Sua funcionalidade é semelhante à dos temporizadores, mas, ao contrário dos temporizadores, os lembretes são acionados em todas as circunstâncias até que o lembrete seja registrado explicitamente pelo ator. Especificamente, os lembretes são acionados em todas as desativações e failovers ator porque o tempo de execução de atores retém as informações sobre os lembretes do ator.

Os lembretes têm suporte apenas para atores com monitoração de estado. Atores sem monitoração de estado não podem usar lembretes. Os provedores de estado de atores são responsáveis por armazenar informações sobre os lembretes que foram registradas por atores.
...
No exemplo acima, `"Pay cell phone bill"` é o nome do lembrete, que é uma cadeia de caracteres usada pelo ator para identificar exclusivamente um lembrete. `BitConverter.GetBytes(amountInDollars)` é o contexto que está associado ao lembrete. Ele será devolvido para o ator como um argumento para o retorno de chamada do lembrete, ou seja, `IRemindable.ReceiveReminderAsync`.
...
Quando um lembrete é disparado, o tempo de execução dados atores da malha chamará o método `ReceiveReminderAsync` no ator. Um ator pode registrar vários lembretes e o método `ReceiveReminderAsync` é chamado sempre que qualquer um desses lembretes é disparado. O ator pode usar o nome de lembrete que é passado para o método `ReceiveReminderAsync` para descobrir qual lembrete foi acionado.

O tempo de execução dos atores salva o estado do ator quando a chamada de `ReceiveReminderAsync` é concluída. Se ocorrer um erro ao salvar o estado, esse objeto de ator será desativado e uma nova instância será ativada. Para especificar que o estado não precisa ser salvo após a conclusão do retorno de chamada do lembrete, o sinalizador `ActorReminderAttributes.ReadOnly` pode ser definido no parâmetro dos `attributes` quando o método `RegisterReminder` for chamado para criar o lembrete.

Para cancelar o registro de um lembrete, o método `UnregisterReminder` deve ser chamado, conforme mostrado no exemplo a seguir.
