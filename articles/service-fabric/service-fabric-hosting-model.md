...
Para entender o modelo de hospedagem, vamos ver um exemplo. Vamos supor que temos uma *ApplicationType* “MyAppType”, que tem um *ServiceType* “MyServiceType”.  “MyServiceType” é fornecido pelo *ServicePackage* “MyServicePackage”, que tem um *CodePackage* “MyCodePackage”. “MyCodePackage” registra *ServiceType* “MyServiceTypeB” quando ele é executado.

Vamos supor que temos um cluster de 3 nós e que criamos um *aplicativo* **fabric:/App1** do tipo “MyAppType”. Nesse *aplicativo* **fabric:/App1**, criamos um serviço **fabric:/App1/ServiceA** do tipo “MyServiceType”, que tem duas partições (digamos **P1** & **P2**) e três réplicas por partição. O diagrama a seguir mostra a exibição desse aplicativo ao final de sua implantação em um nó.

<center>
![Exibição do nó do aplicativo implantado][node-view-one]
</center>

O Service Fabric ativou “MyServicePackage”, que iniciou “MyCodePackage”, que está hospedando réplicas de ambas as partições.  Por exemplo, **P1** & **P2**. Todos os nós do cluster terão a mesma exibição, já que escolhemos que o número de réplicas por partição seja igual ao número de nós no cluster. Vamos criar outro serviço **fabric:/App1/ServiceB** no aplicativo **fabric:/App1** que tem uma partição (digamos **P3**) e três réplicas por partição. O seguinte diagrama mostra a nova exibição do nó:

<center>
![Exibição do nó do aplicativo implantado][node-view-two]
</center>

Como você pode ver, o Service Fabric colocou a nova réplica da partição **P3** do serviço **fabric:/App1/ServiceB** na ativação existente de “MyServicePackage”. Agora permite cria outro *aplicativo* **fabric:/App2** do tipo “MyAppType”. Dentro de **fabric:/App2**, crie um serviço **fabric:/App2/Services** que tem duas partições (digamos **P4** & **P5**) e três réplicas por partição. Os seguintes diagramas mostram a nova exibição do nó:

<center>
![Exibição do nó do aplicativo implantado][node-view-three]
</center>

O Service Fabric ativa uma nova cópia de “MyServicePackage”, que inicia uma nova cópia de “MyCodePackage”. Réplicas de ambas as partições do serviço **fabric:/App2/Services** (por exemplo, **P4** & **P5**) são colocados nesta cópia nova 'MyCodePackage'.
...
```
Continuando com nosso exemplo anterior, vamos criar outro serviço **fabric:/App1/ServiceC** no aplicativo **fabric:/App1** que tem duas partições (digamos **P6** & **P7**) e três réplicas por partição com **ServicePackageActivationMode** definido como “ExclusiveProcess”. O seguinte diagrama mostra a nova exibição do nó:
...
>
>- O modelo de hospedagem do **Processo Exclusivo** corresponde a **ServicePackageActivationMode** definida para **ExclusiveProcess** e precisa ser especificado explicitamente no momento da criação do serviço. 
>
...
Uma cópia ativa de um *ServicePackage* em um nó é conhecido como um [pacote de serviço implantado][p3]. Conforme mencionado anteriormente, quando o modelo de **Processo Exclusivo** é usado para criar serviços, para determinado *aplicativo*, é possível que vários pacotes de serviço sejam implantados no mesmo *ServicePackage*. Ao executar operações específicas a um pacote de serviço implantado (por exemplo, [relatório de integridade de um pacote de serviço implantado][p4] ou [reinicialização do pacote de códigos de um pacote de serviço implantado][p5]), a **ServicePackageActivationId** deve ser fornecida para identificar um pacote de serviço implantado específico.
...
>
> - No modelo de hospedagem do **Processo Exclusivo**, em um determinado *nó* de determinado *aplicativo*, uma ou mais cópias de um *ServicePackage* podem estar ativas. Cada ativação tem uma *ServicePackageActivationId* **não vazia**, especificada durante a execução de operações relacionadas ao pacote de serviço implantado. 
>
...
- Serviço **fabric:/SpecialApp/ServiceA** do tipo “MyServiceTypeA” com duas partições (digamos **P1** e **P2**) e três réplicas por partição.
- Serviço **fabric:/SpecialApp/ServiceB** do tipo “MyServiceTypeB” com duas partições (digamos **P3** e **P4**) e três réplicas por partição.
...
Na ativação de “MultiTypeServicePackge” na réplica da partição **P1** do serviço **fabric:/SpecialApp/ServiceA**, “MyCodePackageA” está hospedando a réplica e “MyCodePackageB” está ativa e em execução. Da mesma forma, na ativação de “MultiTypeServicePackge” na réplica da partição **P3** do serviço **fabric:/SpecialApp/ServiceB**, “MyCodePackageB” está hospedando a réplica e “MyCodePackageA” está apenas em execução e assim por diante. Portanto, quanto maior o número de *CodePackages* (registrando diferentes *ServiceTypes*) por *ServicePackage*, maior será o uso de recursos redundantes. 
 
 Contudo, se criarmos os serviços **fabric:/SpecialApp/ServiceA** e **fabric:/SpecialApp/ServiceB** com o modelo de **Processo Compartilhado**, o Service Fabric ativará somente uma cópia de “MultiTypeServicePackge” para o *aplicativo* **fabric:/SpecialApp** (como vimos anteriormente). “MyCodePackageA” hospedará todas as réplicas para o serviço **fabric:/SpecialApp/Services** (ou de qualquer serviço do tipo “MyServiceTypeA” para ser mais preciso). “MyCodePackageB” hospedará todas as réplicas para o serviço **fabric:/SpecialApp/ServiceB** (ou de qualquer serviço do tipo “MyServiceTypeB” para ser mais preciso). O seguinte diagrama mostra a exibição do nó nesta configuração: 

<center>
![Exibição do nó do aplicativo implantado][node-view-six]
</center>

No exemplo anterior, talvez você pense que se “MyCodePackageA” registrar “MyServiceTypeA” e “MyServiceTypeB” e não houver nenhum “MyCodePackageB”, não haverá nenhum *CodePackage* redundante em execução. Isso está correto, no entanto, como mencionado anteriormente. Esse modelo de aplicativo não se alinha com o modelo de hospedagem de **Processo Exclusivo**. Se a meta é colocar cada réplica em seu próprio processo dedicado, registrar os dois *ServiceTypes* do mesmo *CodePackage* não é necessário. Colocar cada *ServiceType* em seu próprio *ServicePackage* é uma escolha mais natural.
