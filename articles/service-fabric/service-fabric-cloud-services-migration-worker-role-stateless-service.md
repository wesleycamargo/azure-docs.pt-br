<properties
   pageTitle="Guia para converter funções de trabalho e Web em serviços sem estado do Service Fabric | Microsoft Azure"
...
## Projeto de Serviço de Nuvem para projeto de aplicativo Service Fabric
...
Parar VM | `OnStop()` | N/D
Abrir escuta para solicitações de cliente | N/D | <ul><li> `CreateServiceInstanceListener()` quando sem estado</li><li>`CreateServiceReplicaListener()` quando com estado</li></ul>
...
Há várias diferenças importantes entre o ciclo de vida e a vida útil das funções de trabalho e do Service Fabric:
 ...
 - **Tempo de vida:** uma instância de função de trabalho reciclará se o método `Run` sair. O método `RunAsync` em um serviço do Service Fabric, no entanto, pode executar até a conclusão, e a instância do serviço permanecerá ativa.
...
Informações de ponto de extremidade | `RoleInstance` <ul><li>Instância atual: `RoleEnvironment.CurrentRoleInstance`</li><li>Outras funções e instâncias: `RoleEnvironment.Roles`</li> | <ul><li>`NodeContext` Para o endereço de nó atual</li><li>`FabricClient` e `ServicePartitionResolver` para descoberta de ponto de extremidade de serviço</li> 
...
--- | --- | ---
Configuração local | ServiceDefinition.csdef | ServiceManifest.xml
Privilégios | "limitados" ou "elevados" | qualquer conta de usuário ou computador
...
## Uma observação sobre o ambiente de desenvolvimento
