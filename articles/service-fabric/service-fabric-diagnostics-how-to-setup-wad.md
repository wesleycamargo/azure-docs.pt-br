...
Para implantar o Diagnóstico nas VMs do cluster como parte da criação do cluster, é usado o painel Configurações de Diagnóstico, mostrado na imagem abaixo. A configuração padrão para *Logs de Suporte* é **Habilitado** e para *Diagnóstico de Aplicativos* é **Desabilitado**. Depois que o cluster tiver sido criado, essas configurações não poderão ser alteradas usando o portal.
...
Como alternativa você pode baixar o exemplo de Gerenciador de Recursos, fazer suas alterações e criar um cluster com o modelo modificado usando o comando `New-AzureResourceGroupDeployment` em uma janela do Azure PowerShell. Veja as informações abaixo para os parâmetros que você precisará para passar para o comando. Para obter informações detalhadas sobre como implantar um Grupo de Recursos usando o PowerShell, consulte o artigo [Deploy a Resource Group with Azure Resource Manager template](../resource-group-template-deploy.md)
...
Após modificar o arquivo **template.json** conforme descrito, republique o modelo ARM. Se o modelo foi exportado, a execução do arquivo **deploy. ps1** republicará o modelo. Após implantar, verifique se o status de *ProvisioningState* é *Com êxito*.
