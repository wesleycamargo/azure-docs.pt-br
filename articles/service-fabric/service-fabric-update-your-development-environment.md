...
# Atualizar o ambiente de desenvolvimento da Malha do Serviço
 ...
 O Service Fabric do Azure fornece periodicamente novas versões do tempo de execução, SDK e das ferramentas para uso no desenvolvimento local. Mantendo seu ambiente de desenvolvimento local atualizado com essas versões, você pode assegurar que terá sempre acesso aos recursos, correções de bug e melhorias de desempenho mais recentes ao compilar e testar seus aplicativos localmente.
...
## Limpar o cluster local
 ...
 2. Navegue até o diretório de instalação de cluster usando `cd "$env:ProgramW6432\Microsoft SDKs\Service Fabric\ClusterSetup"`.

 3. Execute `.\CleanCluster.ps1`.
...
## Atualizar o tempo de execução, o SDK e as ferramentas
 ...
 2. Após a conclusão, abra uma nova janela do PowerShell como administrador e navegue até o diretório de instalação de cluster usando `cd "$ENV:ProgramFiles\Microsoft SDKs\ServiceFabric\ClusterSetup"`.
...
>[AZURE.NOTE]A estrutura do projeto padrão foi alterada nesta versão. Você poderá abrir e executar projetos existentes no Visual Studio. No entanto, se você encontrar problemas com a criação, implantação ou depuração de seus aplicativos, considere criar um novo projeto e migrar seu código para ele.
