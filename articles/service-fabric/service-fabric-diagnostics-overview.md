...
---
# <a name="monitoring-and-diagnostics-in-azure-service-fabric"></a>Monitoramento e diagnóstico no Azure Service Fabric

O monitoramento e o diagnóstico são fundamentais para desenvolver, testar e implantar em ambientes de produção com êxito. As soluções do Service Fabric funcionam melhor quando você implementa o monitoramento e o diagnóstico do princípio, ajudando a garantir que os serviços funcionam perfeitamente em ambientes de desenvolvimento local e em setups de cluster de produção do mundo real.
...
O monitoramento e o diagnóstico são importantes para garantir que tudo está funcionando conforme o esperado e permitem que você responda às situações com interrupção mínima do serviço. 

O fluxo de trabalho geral do monitoramento e diagnóstico consiste em três etapas: 
1. **Geração de eventos**: isso inclui eventos (logs, rastreamentos, eventos personalizados) nos níveis de serviço/aplicativos e infraestrutura (cluster) 
2. **Agregação de eventos**: os eventos gerados precisam ser coletados e agregados antes que possam ser exibidos. Isso pode feito ser por meio de tabelas de armazenamento como definido pelo Diagnóstico do Microsoft Azure ou criando um pipeline EventFlow
3. **Análise**: os eventos precisam ser visualizado e estar acessíveis em algum formato, para permitir a análise e a exibição conforme necessário. Você pode optar por usar ferramentas como ApplicationInsights, Operations Management Suite, Kibana e vários outros

Embora existam vários produtos disponíveis que cobrem essas três áreas, muitos clientes escolhem tecnologias diferentes para cada um desses aspectos. É importante que as partes funcionem em conjunto para fornecer uma solução de monitoramento de ponta a ponta para o aplicativo.

Dependendo das suas necessidades, você poderá estender ainda mais o diagnóstico e o monitoramento para incluir alertas automatizados e mitigação, que normalmente vêm integrados às ferramentas de análise que você pode optar por usar. 

## <a name="event-generation"></a>Geração de evento

Eventos, logs e rastreamentos podem ser gerados da camada de infraestrutura (do cluster, das máquinas ou das ações do Service Fabric) ou da camada de aplicativo (qualquer instrumentação adicionada aos aplicativos e serviços implantados no cluster).

### <a name="infrastructure-monitoring-the-cluster"></a>Infraestrutura: monitorando o cluster

O Service Fabric pode ajudar a manter um aplicativo em execução durante falhas de infraestrutura, mas você ainda precisa entender se o erro está ocorrendo no aplicativo ou na infraestrutura subjacente. Você também precisa monitorar a infraestrutura para o planejamento de capacidade, para que você saiba quando adicionar ou remover infraestruturas. É importante monitorar e solucionar problemas da infraestrutura e do aplicativo que compõem uma implantação do Service Fabric. Mesmo se um aplicativo está disponível para os clientes, a infraestrutura ainda pode se uma potencial fonte de problemas. Para se certificar de que o cluster está se comportando da maneira esperada, o Service Fabric lhe prepara com cinco canais de registro em log prontos para uso:

1. Canal operacional: operações de alto nível executadas pelo Service Fabric e pelo cluster, inclui eventos para um nó surgindo, um novo aplicativo sendo implantado ou uma reversão de atualização do SF etc. 
2. Canal de informações do cliente: relatórios de integridade e as decisões de balanceamento de carga
3. Eventos do Reliable Services: eventos específicos do modelo de programação
4. Eventos do Reliable Actors: contadores de desempenho e eventos específicos do modelo de programação
5. Logs de suporte: logs do sistema gerados pelo Service Fabric apenas para serem usados por nós ao fornecer suporte

É altamente recomendável que, na criação do cluster, "Diagnóstico" seja habilitado. Isso pode ser feito no portal, conforme mostrado abaixo ou por meio de um modelo do Azure Resource Manager que inclui o diagnóstico. 

![Criação de cluster de Portal do Azure com o diagnóstico habilitado](./media/service-fabric-diagnostics-overview/azure-enable-diagnostics.png)

Como visto acima, há também um campo opcional para adicionar uma chave de instrumentação do Application Insights (AppInsights). Se você optar por usar a AppInsights para qualquer análise de eventos (a AppInsights é uma das soluções recomendadas), inclua o recurso instrumentationKey (GUID) do AppInsights aqui.
...
### <a name="application-instrumenting-code-for-custom-events"></a>Aplicativo: instrumentar o código para eventos personalizados
...
- Eventos **ETW** emitidos como parte do canal operacional
...
A parte final do monitoramento é visualizar o fluxo de eventos, informar sobre o desempenho do serviço e alertar ao detectar um problema. Você pode usar diferentes soluções para esse aspecto do monitoramento. AppInsights e OMS (Operations Management Suite) para alerta com base no fluxo de eventos. Você pode usar o Microsoft Power BI ou uma solução de terceiros, como [Kibana](https://www.elastic.co/products/kibana) ou [Splunk](https://www.splunk.com/) para visualizar os dados.

### <a name="appinsights"></a>AppInsights

O AppInsights é uma das ferramentas recomendadas para monitoramento de aplicativos e serviços. O AI.SDK atualizado faz um excelente trabalho com eventos do Service Fabric e além de fornecer boas visualizações de dados e uma ferramenta de consulta (por meio da Análise do AppInsights), ele também pode criar um AppMap preciso que pode ajudá-lo a rastrear as dependências entre processos em um aplicativo ou cluster.

Configure um recurso AppInsights pesquisando por "Application Insights" no Azure Marketplace. Depois de criar um, vá para *Propriedades* para localizar a Chave de Instrumentação do AI (na forma de um GUID). Isso é usado para:
* Integrar o AppInsights para receber eventos de nível de infraestrutura de um cluster do Service Fabric diretamente por meio de um modelo do Azure Resource Manager ou por meio do Portal do Azure ao criar um cluster, supondo que o Diagnóstico foi habilitado
* Configurar o EventFlow (eventFlowConfig.json) para produzir dados para o Application Insights, conforme mostrado na seção acima

### <a name="oms"></a>OMS

O OMS é outra ferramenta recomendada para diagnóstico e monitoramento de clusters do Service Fabric. A solução Service Fabric pode ser adicionada a qualquer espaço de trabalho e tem um painel para mostrar os diferentes tipos de eventos do Service Fabric. Os espaços de trabalho do OMS também têm uma ferramenta de consulta de log poderosa no Log Analytics.

Para configurar um espaço de trabalho do OMS, confirme que o diagnóstico foi habilitado para o cluster. Adicione “Análise do Service Fabric” do Azure Marketplace a um espaço de trabalho do OMS existente ou crie um novo. Configure as fontes de dados do espaço de trabalho para se conectarem às tabelas de Armazenamento do Azure nas quais o cluster está gravando eventos. 

Para o OMS coletar eventos personalizados, você também precisa se certificar de que a instrumentação adicionada aos aplicativos também devem gravar nas mesmas tabelas de Armazenamento ou quaisquer outras que também estão configuradas para serem fontes do espaço de trabalho. 

O OMS também é o caminho recomendado a ser seguido para análise e visualização de dados se você precisar de monitoramento e diagnóstico para contêineres, uma vez que você pode adicionar uma solução de Contêineres ao espaço de trabalho que funciona bem com contêineres orquestrados do Service Fabric. Para exibir um breve guia sobre como configurar isso, acesse [aqui](service-fabric-diagnostics-containers-windowsserver.md).
...
Um watchdog é um serviço separado que pode inspecionar a integridade e a carga entre serviços e informar a integridade de qualquer coisa na hierarquia de modelo de integridade. Isso pode ajudar a evitar erros que não seriam detectados com base no modo de exibição de um único serviço. Os watchdogs também são um bom local para hospedar os códigos que podem realizar ações de correção para condições conhecidas sem qualquer interação com o usuário. Você pode encontrar uma implementação de serviço de watchdog de exemplo [aqui](https://github.com/Azure-Samples/service-fabric-watchdog-service).
