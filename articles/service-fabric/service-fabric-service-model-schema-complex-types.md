...
Especifique uma conta de usuário padrão para todos os pacotes de código do serviço que não têm uma RunAsPolicy específica definida na seção ServiceManifestImport.
|Atributo|Valor|
|---|---|
...
O certificado do cliente de função de administrador padrão usado para proteger a comunicação entre o cliente e o servidor.
|Atributo|Valor|
|---|---|
...
O certificado do cliente de função de usuário padrão usado para proteger a comunicação entre o servidor e o cliente.
|Atributo|Valor|
|---|---|
...
Um ponto de entrada privilegiado que por padrão é executado com as mesmas credenciais do que o Service Fabric (normalmente, a conta NetworkService) antes de qualquer outro ponto de entrada. A presença de um ponto de entrada de instalação separado evita a necessidade de executar o host de serviço com altos privilégios por longos períodos de tempo. No manifesto do aplicativo, você pode alterar as permissões de segurança para executar o script de inicialização em uma conta de sistema local ou uma conta de administrador.
|Atributo|Valor|
|---|---|
...
Modo de isolamento do contêiner. Os valores válidos são default, process ou hyperv (com suporte somente para contêineres do Windows). Em hosts do no Windows 10, somente o modo de isolamento do Hyper-V tem suporte, assim o contêiner é executado no modo de isolamento do Hyper-V, independentemente de sua configuração de modo de isolamento.
|Atributo|Valor|
|---|---|
|Nome|Isolamento|
|uso|opcional|
...
Descreve as substituições de recursos para pontos de extremidade nos recursos de manifesto do serviço.
...
Especifica limites de recurso para um pacote de códigos.
|Atributo|Valor|
|---|---|
...
#### <a name="placementconstraints"></a>PlacementConstraints
Usado para controlar em quais nós no cluster um serviço pode ser executado. Um par chave-valor que descreve o nome da propriedade do nó e os requisitos de serviços para o valor. Instruções individuais podem ser agrupadas com lógica booliana simples para criar a restrição necessária. Por exemplo, "(FirmwareVersion>12  && InDMZ == True)".
|Atributo|Valor|
|---|---|
|Nome|PlacementConstraints|
|Tipo|xs:string|
