...
```
em que `<path to ClusterConfig.json>` e `<path to MicrosoftAzureServiceFabric.cab>` são os caminhos para a configuração do cluster e o arquivo .cab de tempo de execução, respectivamente.
