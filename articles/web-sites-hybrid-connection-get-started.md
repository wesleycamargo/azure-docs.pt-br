...
6. Quando o processo for concluído, a área de notificações no portal informará que a conexão foi criada com sucesso. <!-- TODO

    Tudo falha nesta etapa. Não consigo criar um serviço BizTalk no portal dogfood. Volto para o portal antigo (portal completo) e crio o serviço BizTalk, mas ele não parece permitir que você estabeleça a conexão - Ao concluir a etapa Criar conexão híbrida, você obtém o seguinte erro: Falha ao criar conexão híbrida RelecIoudHC. O tipo de recurso não foi encontrado no namespace 'Microsoft.BizTaIkServices para a api versão 2014-06-01'.
	
	O erro indica que não foi encontrado o tipo; não se trata da instância. ![Notificação de sucesso][CreateHCSuccessNotification] -->
