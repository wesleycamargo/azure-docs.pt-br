---
title: "Encaminhar o status do trabalho e os fluxos de trabalho da Automação para o Log Analytics (OMS) | Microsoft Docs"
...
1. Uma assinatura do OMS. Para obter informações adicionais, confira [Introdução ao Log Analytics](../log-analytics/log-analytics-get-started.md).  
   ...
   > [!NOTE]
   > O espaço de trabalho do OMS e a conta de Automação precisam estar na mesma assinatura do Azure para que essa configuração funcione corretamente.
   >
   >
...
## <a name="setup-integration-with-log-analytics"></a>Configurar a integração com o Log Analytics
...
| resourceId |Especifica o tipo de recurso no Azure.  Para a Automação, o valor é a conta da Automação associada ao runbook. |
| operationName |Especifica o tipo de operação realizada no Azure.  Para a Automação, o valor será Trabalho. |
| resultType |O status do trabalho de runbook.  Os valores possíveis são:<br>- InProgress |
