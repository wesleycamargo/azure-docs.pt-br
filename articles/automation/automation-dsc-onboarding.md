...
2. Compie o script a seguir localmente. Esse script contém uma configuração DSC do PowerShell para criar metaconfigurações e um comando para dar início à criação de metaconfigurações.
   ...
   # <a name="the-dsc-configuration-that-will-generate-metaconfigurations"></a>A configuração DSC que vai gerar metaconfigurações
...
1. Preencha a chave de registro e a URL para sua conta de Automação, bem como os nomes dos computadores a carregar. Todos os outros parâmetros são opcionais. Para encontrar a chave de registro e a URL de registro de sua conta da Automação, confira a seção [**Proteger o registro**](#secure-registration) abaixo.
...
3. Baixe, da conta de Automação da qual você deseja carregar nós, as metaconfigurações do DSC do PowerShell para as máquinas que você deseja carregar:
   ...
   # <a name="define-the-parameters-for-get-azurermautomationdsconboardingmetaconfig-using-powershell-splatting"></a>Definir os parâmetros de Get-AzureRmAutomationDscOnboardingMetaconfig usando o nivelamento do PowerShell
     $Params = @{
   ...
   # <a name="use-powershell-splatting-to-pass-parameters-to-the-azure-automation-cmdlet-being-invoked"></a>Usar o nivelamento do PowerShell para passar parâmetros ao cmdlet da Automação do Azure que está sendo chamado
   # <a name="for-more-info-about-splatting-run-get-help--name-aboutsplatting"></a>Para saber mais sobre nivelamento, execute: Get-Help-Name about_Splatting
