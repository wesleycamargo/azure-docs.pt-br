---
title: Introdução à Automação do Azure
...
Talvez você queira um runbook que esteja em execução em um Hybrid Runbook Worker para executar tarefas de gerenciamento em relação a outros serviços ou computadores em seu ambiente. Nesse cenário, o runbook também precisará acessar outras portas. Se suas políticas de segurança de TI não permitirem que os computadores em sua rede se conectem à internet, examine [Gateway de OMS](../log-analytics/log-analytics-oms-gateway.md). O Gateway do OMS atua como um proxy para o Hybrid Runbook Worker. Ele Coleta o status de trabalho e recebe informações de configuração da sua conta de Automação do Azure.
...
Para o Hybrid Runbook Worker conectar e registrar com Log Analytics, ele deve ter acesso ao número da porta e às URLs descritas nesta seção. Além das [portas e URLs necessárias para o Microsoft Monitoring Agent](../log-analytics/log-analytics-windows-agent.md) conectar o Log Analytics. 
...
|Método | DESCRIÇÃO |
|-------|-------------|
| Selecione **Automação e Controle** no Azure Marketplace | Uma oferta do Microsoft Azure Marketplace cria uma conta de Automação e um espaço de trabalho do Log Analytics que estão vinculados e no mesmo grupo de recursos e região. A integração com o Log Analytics também inclui o benefício de usá-lo para monitorar e analisar o status do trabalho do runbook e os fluxos de trabalho ao longo do tempo. Você também pode usar os recursos avançados no Log Analytics para escalar ou investigar problemas. A oferta também implanta as soluções de **Controle de Alterações** e **Gerenciamento de Atualizações**, que são habilitadas por padrão. |
| Selecione **Automação** no Marketplace | Esse método cria uma conta de Automação em um grupo de recursos novo ou existente que não está vinculado a um espaço de trabalho do Log Analytics. Ele não inclui nenhuma solução disponível da oferta de **Automação e Controle**. Esse método é uma configuração básica que apresenta a Automação do Azure a você. Ele pode ajudá-lo a aprender a gravar runbooks e configurações DSC, e aprender a usar os recursos do serviço. |
| Selecione soluções de **Gestão** | Se você selecionar uma solução **Gerenciamento**, incluindo [Gerenciamento de Atualizações](../operations-management-suite/oms-solution-update-management.md), [Iniciar/parar VMs durante horários inativos](automation-solution-vm-management.md) ou [Controle de Alterações](../log-analytics/log-analytics-change-tracking.md), a solução solicitará que você selecione uma conta de Automação existente e o espaço de trabalho do Log Analytics. A solução oferece a opção de criar uma conta de Automação e espaço de trabalho do Log Analytics, conforme necessário para a solução ser implantada na sua assinatura. |

### <a name="create-an-automation-account-thats-integrated-with-log-analytics"></a>Criar uma conta de Automação do Azure integrada com o Log Analytics
A automação integrada é recomendável para que você selecione a oferta de **Automação e Controle** no Marketplace. Usar esse método cria uma conta de Automação e estabelece a integração com um espaço de trabalho do Log Analytics. Quando você usa esse método, você também tem a opção de instalar as soluções de gerenciamento que estão disponíveis com a oferta.  

[Criar uma conta de Automação independente](automation-create-standalone-account.md) orienta você pelo processo de criação de uma conta de Automação e espaço de trabalho do Log Analytics, integrando a oferta de **Automação e Controle**. Você pode aprender a criar uma conta de Automação do Azure independente para testar ou visualizar o serviço.  

Para criar uma conta de Automação e espaço de trabalho do Log Analytics usando o Marketplace de **Automação e Controle**, oferecendo:
...
5. Em **Automação e controle**, selecione **Espaço de trabalho do OMS**. Em **Espaços de trabalho do OMS**, selecione um espaço de trabalho do Log Analytics vinculado à assinatura do Azure onde a conta de Automação esteja. Se você não tiver um espaço de trabalho do Log Analytics, selecione **Criar novo espaço de trabalho**. Em **Espaço de trabalho do OMS**: 
  ...
  5. Selecione **Conta de Automação**.  Se você criar um novo espaço de trabalho do Log Analytics, também deverá criar uma conta de Automação associada ao novo espaço de trabalho do Log Analytics. Inclua sua assinatura do Azure, grupo de recursos e região. 
    ...
    Todas as outras opções são preenchidas automaticamente com base no espaço de trabalho do Log Analytics selecionado. Você não pode modificar essas opções. 
    ...
    4. Caso contrário, selecione uma conta Executar como de Automação existente. A conta selecionada não pode estar vinculada a outro espaço de trabalho do Log Analytics. Caso esteja, será exibida uma mensagem de notificação. Se a conta já estiver vinculada a um espaço de trabalho do Log Analytics, selecione uma conta de Automação Executar Como diferente ou crie uma.
...
8. Para continuar com a Automação de integração e um espaço de trabalho do Log Analytics, selecione **Criar**. Todas as configurações são validadas e, em seguida, o Azure tenta implantar a oferta em sua assinatura. Esse processo pode levar alguns segundos. Para acompanhar seu progresso, no menu, selecione **Notificações**. 
