...
Quando uma Configuração da DSC é compilada, uma ou mais configurações de nó são produzidas, dependendo dos blocos de Nó na configuração. Uma configuração de nó é o mesmo que um "MOF" ou um "documento de configuração" (se você estiver familiarizado com esses termos da DSC do PS) e representa uma "função", como servidor Web ou trabalhador, o estado desejado que um ou mais nós devem assumir ou verificar se há conformidade. Nomes de configurações de nó no DSC de Automação do Azure assumem a forma de “Configuration Name.NodeConfigurationBlockName”.
...
O DSC de Automação do Azure facilita a integração de nós para gerenciamento pelo DSC de Automação do Azure e permite a alteração da configuração de nó atribuída a cada nó do servidor. Assim, na próxima vez que um nó verificar o servidor para obter instruções, ele assumirá uma função diferente e alterará sua configuração e o status de conformidade que deve seguir de forma correspondente.
