<properties
   ...
   description="Este artigo fornece uma visão geral da segurança e dos diferentes métodos de autenticação disponíveis para Contas de Automação na Automação do Azure."
   services="automation"
   documentationCenter=""
   authors="MGoedtel"
   manager="jwhit"
...
A Automação do Azure permite automatizar tarefas em relação a recursos no Azure, locais e com outros provedores de nuvem, como AWS (Amazon Web Services). Para que um runbook execute as ações necessárias, ele deve ter permissões para acessar os recursos com os quais trabalha com segurança e apenas com os direitos mínimos necessários na assinatura. Este artigo abordará os vários cenários de segurança com suporte na Automação do Azure e mostrará como iniciar com base no(s) ambiente(s) que você precisa gerenciar.
