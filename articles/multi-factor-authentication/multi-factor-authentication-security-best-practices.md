...
## <a name="best-practices-for-a-cloud-deployment"></a>Práticas recomendadas para uma implantação de nuvem
...
* Não crie um provedor de autenticação por autenticação multifator. Se você fizer isso, você pode acabar pagar por solicitações de verificação de usuários que já têm licenças.
...
Se você não tiver licenças que incluem o Azure MFA, a primeira etapa para adotar o Azure MFA na nuvem é criar um provedor de autenticação de MFA. MFA está disponível por padrão para os administradores globais que têm o Azure Active Directory, mas os provedores de autenticação multifator ampliam os recursos de verificação em duas etapas para todos os usuários. 

Ao criar o Provedor de Autenticação, você precisará selecionar um diretório e considerar os seguintes detalhes:

* Você não precisa de um diretório do Azure AD para criar um provedor de autenticação multifator, mas você obter mais funcionalidade nesse caso. Os seguintes recursos são habilitados quando você associa o provedor de autenticação um diretório do Azure AD:  
...
* O DirSync ou o AAD Sync só será necessário se você estiver sincronizando o ambiente do Active Directory local com um diretório do Azure AD. Se você usa apenas um diretório do AD do Azure que não está sincronizado com uma instância local do Active Directory, não haverá necessidade do DirSync ou AAD Sync.
...
* Certifique-se de que sua equipe de suporte técnico é treinada para lidar com cenários em que o usuário não pode entrar porque o aplicativo móvel ou o telefone não recebe uma notificação ou chamada telefônica. O suporte técnico pode [habilitar um desvio único](multi-factor-authentication-whats-next.md#one-time-bypass) para permitir que um usuário faça a autenticação uma única vez ao “desviar” a verificação em duas etapas. O desvio é temporário e expira após um número especificado de segundos.
...
![Padrão de componentes do servidor MFA: console, o mecanismo de sincronização, o portal de gerenciamento, o serviço de nuvem](./media/multi-factor-authentication-security-best-practices/server.png)
\*não é instalado por padrão \** instalado mas não está habilitado por padrão
...
Se sua organização for federada para o SSO com o Azure AD e você vai usar o Azure MFA, em seguida, esteja ciente do seguinte ao usar senhas de aplicativo:
...
Use a lista abaixo para outras considerações e orientação para cada componente que será implantado no local:
