<properties linkid="develop-dotnet-rest-service-using-web-api" urlDisplayName="Serviço REST usando a API Web" pageTitle="Serviço REST do .NET usando a API Web - Tutorial do Azure" metaKeywords="Site do tutorial do Azure, site da API do ASP.NET, VS do Azure" description="Um tutorial que ensina a implantar um aplicativo que usa a API Web ASP.NET em um site do Azure usando o Visual Studio." metaCanonical="" services="web-sites" documentationCenter=".NET" title="Serviço REST usando a API Web ASP.NET e o Banco de Dados SQL" authors="riande" solutions="" manager="" editor="" />
...
* [Adicionar uma interface de API Web Restful][addwebapi]
...
2. Clique em **CRIAÇÃO PERSONALIZADA**.
	...
	![Criar link do banco de dados no Portal de Gerenciamento](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rr6.PNG)

	O assistente **Novo Site - Criação Personalizada** é aberto. 

3. Na etapa **Novo Site** do assistente, insira uma cadeia de caracteres na caixa **URL** a ser usada como a URL exclusiva de seu aplicativo. A URL completa consistirá no que você digitar aqui mais o sufixo que você vê abaixo da caixa de texto. A ilustração mostra "contactmgr11", mas essa URL provavelmente já é utilizada e, portanto, você precisa escolher outra.
...
1. Na lista suspensa **Banco de Dados**, escolha **Criar um banco de dados SQL de 20 MB gratuito**.
	...
	![Etapa Criar um novo site do assistente Novo Site - Criar com Banco de Dados](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rrCWS.png)
...
11. Clique na marca de seleção na parte inferior da caixa para indicar que você terminou.

	![Etapa Configurações do Banco de Dados do assistente Novo Site - Criar com Banco de Dados][setup007]

	 A imagem a seguir mostra o uso de um SQL Server e logon existentes.
	
	![Etapa Configurações do Banco de Dados do assistente Novo Site - Criar com o Banco de Dados][rxPrevDB]
...
1. Na caixa de diálogo **Alterar Autenticação**, clique em **Sem Autenticação** e em **OK**.
...
1. Substitua o conteúdo do arquivo *_Layout.cshtml* pelo seguinte código:
...
14. O navegador padrão é aberto automaticamente na URL do site implantado.
...
A próxima tarefa é habilitar o recurso [Codificar primeiras migrações (a página pode estar em inglês)](http://curah.microsoft.com/55220) para criar o banco de dados com base no modelo de dados criado.
...
2. Na janela **Console do Gerenciador de Pacotes**, digite o seguinte comando:
	...
	O comando **add-migration Initial** gera uma classe chamada **&lt;date_stamp&gt;Initial** que cria o banco de dados. O primeiro parâmetro ( *Initial* ) é arbitrário e é usado para criar o nome do arquivo. Você pode ver os novos arquivos de classe no **Gerenciador de Soluções**.
...
3. No navegador, acrescente **/api/contacts** à URL.
	...
	**Aviso de Segurança**: a esta altura, o aplicativo é inseguro e vulnerável a ataques CSRF. Mais à frente no tutorial, removeremos essa vulnerabilidade. Para obter mais informações, consulte [Evitando ataques de solicitação intersite forjada (CSRF)][prevent-csrf-attacks].

<h2><a name="xsrf"></a><span class="short-header">XSRF</span>Adicionar proteção XSRF</h2>
...
1. Atualize a seção *Scripts* do arquivo *Views\Home\Index.cshtml* arquivo para incluir o código a fim de obter os tokens XSRF.
