...
2. Use o seguinte para criar uma nova tabela chamada **log4jLogs**.
    ...
    * **-d** - como `- G` não é usado, a solicitação padrão é o método POST. `-d` especifica os valores de dados que são enviados com a solicitação
        ...
        *  **execute** - as instruções do HiveQL para executar
...
6. Use as instruções a seguir para criar uma nova tabela "interna" chamada **errorLogs**.
    ...
    * **CREATE TABLE IF NOT EXISTS** - cria uma tabela, se ela ainda não existir. Como a palavra-chave **EXTERNAL** não é utilizada, esta é uma tabela "interna", que é armazenada no data warehouse do Hive e é gerenciada totalmente pelo Hive
...
Para obter mais informações sobre a interface REST usada nesse artigo, consulte a <a href="https://cwiki.apache.org/confluence/display/Hive/WebHCat+Reference" target="_blank">Referência WebHCat</a>.
