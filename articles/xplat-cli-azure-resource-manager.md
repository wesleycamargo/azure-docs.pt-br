...
> [WACOM.NOTE] Se você ainda não tiver instalado e configurado xplat cli, consulte [Instalar e configurar a interface de linha de comando da plataforma cruzada do Microsoft Azure][Instalar e configurar a interface de linha de comando da plataforma cruzada do Microsoft Azure] para obter mais etapas sobre como instalar, configurar e usar o xplat-cli.

## Gerenciador de Recursos

O Gerenciador de Recursos permite gerenciar um grupo de *recursos* (entidades gerenciadas pelo usuário, como um servidor de banco de dados, um banco de dados ou um site) como uma unidade lógica única ou um *grupo de recursos*. Por exemplo, um grupo de recursos pode conter um site e recursos de banco de dados SQL.
...
Quando um modelo é usado para modificar ou criar um grupo, uma *implantação* é criada e, então, aplicada ao grupo.
...
Para obter mais informações sobre como se autenticar usando uma conta organizacional, consulte [[Instalar e configurar a interface de linha de comando da plataforma cruzada do Microsoft Azure][Instalar e configurar a interface de linha de comando da plataforma cruzada do Microsoft Azure].
...
6.  Depois de salvar o arquivo **params.json**, use o comando a seguir para criar um novo grupo de recursos com base no modelo. O parâmetro `-e` especifica o arquivo **params.json** criado na etapa anterior.
...
7.  Para exibir o grupo, use o comando a seguir.
...
> [WACOM.NOTE] Ao usar os comandos `azure resource` em vez do comando `list`, você deve especificar a versão da API do recurso com que estiver trabalhando usando o parâmetro `-o`. Se você não tiver certeza sobre a versão da API a ser usada, consulte o arquivo de modelo e localize o campo **apiVersion** do recurso.

1.  Para listar todos os recursos em um grupo, use o comando a seguir.

        azure resource list MyGroupName

2.  Para exibir recursos individuais, como o site, dentro do grupo, use o comando a seguir.
