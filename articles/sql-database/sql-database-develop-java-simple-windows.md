...
O exemplo de código cria um objeto `Connection` usando uma cadeia de conexão. Você pode encontrar a cadeia de conexão usando o [portal do Azure](http://portal.azure.com/). Para obter detalhes sobre como localizar a cadeia de conexão, confira [Criar seu primeiro Banco de Dados SQL do Azure](sql-database-get-started.md).
