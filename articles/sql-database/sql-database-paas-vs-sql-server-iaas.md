...
* [SQL Server em Máquinas Virtuais do Azure](https://azure.microsoft.com/services/virtual-machines/sql-server/): SQL Server instalado e hospedado na nuvem em VMs (Máquinas Virtuais) do Windows Server em execução no Azure, também conhecido como IaaS (infraestrutura como serviço).
  O SQL Server em máquinas virtuais do Azure é o mesmo software do SQL Server que você instala em seu servidor local. A única diferença é que este servidor agora é uma VM hospedada no Azure. O SQL Server no Azure é otimizado para migrar aplicativos existentes do SQL Server, e todas as versões e edições do SQL Server estão disponíveis. Ele permite hospedar tantos bancos de dados conforme necessário; oferece suporte a execuções de transações entre bancos de dados e oferece controle total da sua instância do SQL Server e do sistema operacional Windows.
...
|  | Equipes que desejam que a Microsoft gerencie suas atualizações de banco de dados, alta disponibilidade e recuperação de desastres. |Equipes que podem configurar e gerenciar a alta disponibilidade, recuperação de desastres e aplicação de patch para o SQL Server. Alguns recursos automatizados fornecidos simplificam muito isso. | |
...
Atualmente, o **Banco de Dados SQL** está disponível em várias camadas de serviço, todas as quais são cobradas por hora a uma taxa fixa com base na camada de serviço e no nível de desempenho que você escolher. Além disso, você será cobrado pelo tráfego de Internet de saída a [taxas de transferência de dados](https://azure.microsoft.com/pricing/details/data-transfers/)regulares. As camadas de serviço Básico, Standard, Premium, Uso Geral e Crítico são projetados para fornecer desempenho previsível com vários níveis de desempenho para atender aos requisitos de pico do aplicativo. Você pode alterar entre as camadas de serviço e os níveis de desempenho para atender às necessidades de produtividade variadas do seu aplicativo. Para obter as últimas informações sobre as camadas de serviço atuais com suporte, consulte [Camadas de Serviço do Banco de Dados SQL do Azure](sql-database-service-tiers.md). Você também pode criar [pools elásticos](sql-database-elastic-pool.md) para compartilhar recursos de desemprenho entre instâncias de banco de dados.

> [!IMPORTANT]
> Se o banco de dados tiver um alto volume de transações e precisar dar suporte a muitos usuários simultâneos, são recomendáveis as camadas de serviço Premium ou Crítico. Para minimizar a latência entre o aplicativo e o banco de dados SQL, localize o aplicativo na mesma região que o banco de dados e, em seguida, teste o desempenho - aumentando a camada de serviço e o nível de desempenho, conforme necessário.
