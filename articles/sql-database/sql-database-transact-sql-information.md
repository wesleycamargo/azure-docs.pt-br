...
Por exemplo, o AlwaysOn é substituído pela Replicação Geográfica Ativa. Por esse motivo, não há suporte para as instruções Transact-SQL relacionadas aos grupos de disponibilidade no Banco de Dados SQL nem para as exibições de gerenciamento dinâmico relacionadas ao AlwaysOn.  
...
Geralmente, a sintaxe preterida no SQL Server não tem suporte no Banco de Dados SQL.
...
O Banco de Dados SQL dá suporte a alguns, mas não a todos os argumentos existentes nas instruções Transact-SQL correspondentes do SQL Server 2016. Por exemplo, a instrução `CREATE PROCEDURE` está disponível; porém, todas as opções de `CREATE PROCEDURE` não estão disponíveis. Descrever a sintaxe completa aqui seria confuso e redundante. Consulte os tópicos de sintaxe vinculados para obter detalhes sobre as áreas de cada instrução para as quais há suporte.

- Bancos de dados: [CREATE](https://msdn.microsoft.com/library/dn268335.aspx)/[ALTER DATABASE](https://msdn.microsoft.com/library/ms174269.aspx)   
...
- Tabelas: [CREATE](https://msdn.microsoft.com/library/dn305849.aspx)/[ALTER TABLE](https://msdn.microsoft.com/library/ms190273.aspx)   
