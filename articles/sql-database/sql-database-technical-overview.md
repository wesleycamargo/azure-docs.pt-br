...
Este artigo é uma introdução aos principais conceitos e recursos do Banco de Dados SQL relacionados ao desempenho, escalabilidade e gerenciabilidade, com links para explorar os detalhes. Se você estiver pronto para iniciar tutoriais práticos, acesse [Criar seu primeiro banco de dados SQL](sql-database-get-started.md) ou [Criar um pool elástico](sql-database-elastic-pool-manage-portal.md). Para obter uma breve demonstração em vídeo, assista a este vídeo.
