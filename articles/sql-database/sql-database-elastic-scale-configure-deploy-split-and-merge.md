<properties 
	...
	pageTitle="Tutorial de serviço de divisão e mesclagem SQL do Azure" 
	description="Divisão e mesclagem com dimensionamento Elástico" 
...
#Tutorial de serviço de divisão e mesclagem da Escala Elástica
...
1. Crie um banco de dados do Banco de Dados SQL do Azure que será usado como o banco de dados de status de Divisão-Mesclagem. Vá para o [Portal de Gerenciamento do Azure](https://manage.windowsazure.com). Na parte inferior esquerda, clique em **Novo** e em **Serviços de dados** -> **Banco de dados SQL** -> **Personalizar criação**. Preencha o nome do banco de dados e crie novos usuário e senha. Certifique-se de registrar o nome e a senha para uso posterior.

2. Certifique-se de que o servidor de Banco de Dados SQL do Azure permite que os Serviços do Microsoft Azure se conectem a ele. Vá para o [Portal de gerenciamento do Azure](https://manage.windowsazure.com), no painel esquerdo, clique em **Bancos de dados SQL**. Em seguida, clique em **Servidores** na faixa de opções na parte superior da tela e selecione seu servidor. Em seguida, clique em **Configurar** na parte superior e verifique se a configuração dos **Serviços do Windows Azure** estão definidas como **Sim**.
...
5.    A configuração do destino para os logs de diagnóstico requer uma Conta de armazenamento no Azure. A configuração de Divisão/Mesclagem exige a cadeia de caracteres de conexão para sua conta de Armazenamento do Azure. A cadeia de conexão deve estar no formato:

        "DefaultEndpointsProtocol=https;AccountName=myAccountName;AccountKey=myAccessKey" 
    
Para determinar a chave de acesso, acesse o [portal de Gerenciamento do Azure](https://manage.windowsazure.com), clique na guia **Armazenamento**no lado esquerdo, selecione o nome que corresponde à sua conta de armazenamento e clique em **Gerenciar chaves de acesso** na parte inferior. Clique no botão **Copiar** para a **Chave de acesso primária**.
...
5. Na caixa de diálogo, digite um rótulo de implantação. Para ambos 'Package' e 'Configuration', clique em 'From Local' e escolha o arquivo **SplitMergeService.cspkg** e seu arquivo .cscfg configurado anteriormente.
...
* Certifique-se de que o servidor de Banco de Dados SQL do Azure permite que os Serviços do Microsoft Azure se conectem a ele. Para fazer isso, abra https://manage.windowsazure.com, clique em "Bancos de dados SQL" à esquerda, clique em "Servidores" na parte superior e selecione seu servidor. Clique em **Configurar** na parte superior e verifique se a configuração dos **Serviços do Windows Azure** estão definidas como "Sim". (Consulte a seção Pré-requisitos na parte superior deste artigo).

* Reveja os logs de diagnóstico para a instância do serviço de Divisão/Mesclagem. Abra uma instância do Visual Studio e, na barra de menus, clique em **Modo de exibição** e em **Gerenciador de Servidores**. Clique no ícone do **Microsoft Azure** para conectar-se à sua assinatura do Azure. Em seguida, navegue até Microsoft Azure -> Armazenamento -> <your storage account> -> Tabelas -> WADLogsTable. Para mais informações, consulte [Procurando recursos de armazenamento com o Gerenciador de Servidores](http://msdn.microsoft.com/library/azure/ff683677.aspx) 
...
4. **ShardManagement.psm1**  - o script auxiliar que envolve a API ShardManagement
...
Esse erro significa que o certificado SSL não está corretamente configurado. Siga as instruções na seção 'Connecting with a web browser'.
