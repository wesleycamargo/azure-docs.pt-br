...
Se você estiver usando failover de replicação geográfica de opções de restauração geográfica para recuperar seu aplicativo de uma interrupção, você deverá certificar-se de que a conectividade com os novos bancos de dados esteja configurada corretamente para que o funcionamento normal do aplicativo possa ser retomado. Esta é uma lista de verificação de tarefas para preparar para produção o seu banco de dados recuperado.
...
Você deve verificar se todos os logons usados pelo aplicativo existem no servidor que está hospedando o banco de dados recuperado. Para obter mais informações, consulte Como gerenciar a segurança durante a recuperação de desastre. Para obter mais informações, consulte [Configuração de segurança para replicação geográfica](sql-database-geo-replication-security-config.md)

>[AZURE.NOTE] Se estiver usando a opção de restauração geográfica para recuperação de uma interrupção, você deverá configurar as regras de firewall de servidor e logons durante a análise de recuperação de desastre para verificar se o servidor primário está disponível para recuperar sua configuração. Já que a restauração geográfica usa os backups de banco de dados, a configuração no nível do servidor pode não estar disponível durante a interrupção. Após a análise, você pode remover os bancos de dados restaurados mas manter o servidor e sua configuração prontos para o processo de recuperação. Para obter mais informações sobre análises de recuperação de desastre, consulte [Executando análises de recuperação de desastre](sql-database-disaster-recovery-drills.md).
