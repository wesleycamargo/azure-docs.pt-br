...
No diagrama a seguir, os usuários se conectam a uma URL que é especificada para o Gerenciador de Tráfego (__http://myATMURL.trafficmanager.net__) e que abstrai as URLs do site real (__http://app1URL.cloudapp.net__ e \___http://app2URL.cloudapp.net__). Dependendo de como você configura os critérios de quando rotear os usuários, estes serão enviados para o site real correto quando a política determinar. As opções de política são round robin, desempenho ou failover. Neste artigo, abordaremos somente a opção de failover.
