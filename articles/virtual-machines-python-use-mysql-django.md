<properties linkid="develop-python-web-app-with-django-and-mysql" urlDisplayName="Web com Django + MySQL" pageTitle="App da Web do Python com Django e MySQL - Tutorial do Azure" metaKeywords="app da web django Azure, Azure Django MySQL, Azure django Python" description="Um tutorial que ensina você a usar o MySQL com o Django em uma máquina virtual do Azure. Exemplos de código escritos no Python." metaCanonical="" services="virtual-machines" documentationCenter="Python" title="Django Hello World - MySQL Windows Edition" authors=""  solutions="" writer="" manager="" editor=""  />
...
Este tutorial descreve como usar o MySQL em conjunto com o Django em uma única máquina virtual do Azure. Este guia pressupõe que você tem alguma experiência anterior com o Azure e o Django. Para obter uma introdução do Azure e do Django, consulte [Django Hello World] [djangohelloworld]. Este guia também pressupõe que você tem algum conhecimento do MySQL. Para obter uma visão geral do MySQL, consulte o [site do MySQL][mysqldoc].
...
* Instalar um [driver do MySQL] [mysqlpy] para o Python.
...
Você expandirá o exemplo [Olá, Mundo do Django] [djangohelloworld] utilizando um banco de dados do MySQL, hospedado em uma VM do Azure, para encontrar um substituto melhor para *Mundo*. O substituto, por sua vez, será determinado através de um aplicativo *contador* do Django garantido pelo MySQL. Como foi o caso do exemplo Olá Mundo, esse aplicativo do Django será novamente hospedado em uma máquina virtual do Azure.
...
1. Siga as instruções fornecidas [aqui ][preview-portal-vm]para criar uma máquina virtual do Azure da distribuição do *Windows Server 2008 R2*.

1. Abra uma porta do TCP para transações do MySQL na máquina virtual:
 ...
 * Adicione um ponto de extremidade com NAME = **mysql**, PROTOCOL = **TCP**, PUBLIC PORT = **3306**, PRIVATE PORT = **3306**..

1. Adicione outro ponto de extremidade com NAME = **web**, PROTOCOL = **TCP**, PUBLIC PORT = **80**, PRIVATE PORT = **80**.  Isso redirecionará as solicitações externas da Internet para a porta na qual o Django é executado, isto é, *80*.

1. Use a *Área de Trabalho Remota* do Windows para fazer logon remotamente na máquina virtual recém-criada do Azure.

1. Abra a porta TCP **80** na máquina virtual:
 * No menu **Iniciar**, selecione **Ferramentas do Administrador** e **Windows Firewall com Segurança Avançada**. 
...
1. Instale a versão mais recente do [MySQL Community Server] [mysqlcommunity] para Windows na máquina virtual:  

	**Observação**: é recomendável instalar o banco de dados em uma partição de dados diferente do SO. 

 * Execute o *Windows (x86, 64 bits), o link do MSI Installer* [aqui ] [mysqlcommunity] e baixe o instalador adequado do MSI do espelhamento de download mais próximo de você. Algumas dicas úteis:
 ...
 * Instale um [banco de dados "mundo"] [mysqlworld] exemplo (versão do MyISAM):
     * Baixe [este] [mysqlworlddl] arquivo zip na Máquina Virtual do Azure.
...
1. Instale o pacote de cliente do MySQL Python. Você pode instalá-lo diretamente [deste link] [mysqlpydl]. Depois de concluído, execute o comando a seguir para verificar a sua instalação:

  ![][1]


## Expanda o aplicativo Django Hello World
1. Siga as instruções fornecidas no tutorial do [Django Hello World] [djangohelloworld] para criar um aplicativo da Web "Olá Mundo" no Django.

1. Abra **C:\django\helloworld\helloworld\settings.py** no editor de texto de sua preferência.  Modifique o dicionário global **DATABASES** para ler:
  ...
  **Observação:** você **deve** alterar a chave *HOST* para corresponder ao endereço IP **permanente** da VM do seu Azure (MySQL). A esta altura, o *HOST* deve ser definido para o que quer que os relatórios de comando *ipconfig* do Windows estejam indicando. 
...
1. Acrescente o texto a seguir em **C:\django\helloworld\counter\models.py**:
  ...
  Tudo o que fizemos aqui foi definir uma subclasse do *Contador* do Django de nome de classe *Model* com um único campo inteiro, *count*. Esse modelo de contador trivial gravará o número de acessos ao nosso aplicativo Django. 

1. Em seguida, vamos tornar o Django ciente da existência do *Contador*:
 * Edite o **C:\django\helloworld\helloworld\settings.py** novamente. Adicione *'counter'* à tupla *INSTALLED_APPS*.
...
1.  Volte para a janela do PowerShell do Windows e digite os comandos a seguir para implantar seu site do Django publicamente:
    ...
    O parâmetro **runserver** instrui o Django para executar nosso site *helloworld* na porta TCP *80*. Os resultados desse comando devem ser semelhantes a:
...
1. No seu navegador da Web local, abra **http://*yourVmName*.cloudapp.net** (onde *yourVmName* é qualquer nome que você usou na etapa de criação da máquina virtual). Você deverá ver "Hello ... !" conforme mostrado na captura de tela abaixo. Isso indica que o Django está em execução na máquina virtual e funcionando corretamente.

    ![][5]

  Atualize o navegador da Web algumas vezes para ver a alteração da mensagem de *"Hello **&lt;country abc&gt;**"* para *"Hello **&lt;outro país&gt;**"*.
...
[djangohelloworld]: http://windowsazure.com/pt-br/documentation/articles/virtual-machines-python-django-web-app-windows-server
...
[preview-portal-vm]: /pt-br/manage/windows/tutorials/virtual-machine-from-gallery/

[O status do comando Stop-AzureService]: ../Media/django-helloworld-ps-stop.png
[O status do comando Remove-AzureService]: ../Media/django-helloworld-ps-remove.png

[Guia de instalação]: http://windowsazure.com/pt-br/documentation/articles/python-how-to-install
