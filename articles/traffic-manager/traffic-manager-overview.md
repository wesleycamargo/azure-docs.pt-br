...
5. **Crie seu perfil e defina as configurações**. Você pode usar APIs REST, Windows PowerShell ou o Portal do Azure para criar seu perfil do Gerenciador de Tráfego e definir as configurações. Para obter mais informações, consulte [Como definir as configurações do Gerenciador de Tráfego](#how-to-configure-traffic-manager-settings). As etapas a seguir supõem que você usará a **Criação Rápida** no Portal do Azure. 
   - **Crie seu perfil do Gerenciador de Tráfego** - para criar um perfil usando a Criação Rápida no Portal do Azure, consulte [Gerenciar perfis do Gerenciador de Tráfego](traffic-manager-manage-profiles.md).
...
Você pode definir as configurações do Gerenciador de Tráfego usando o Portal do Azure, com APIs REST e cmdlets do Windows PowerShell.

Embora cada elemento de API REST não esteja visível no Portal do Azure, muitas configurações estão disponíveis usando qualquer um dos métodos. Para obter mais informações sobre o uso de APIs REST, consulte [Operações no Gerenciador de Tráfego (referência de API REST)](http://go.microsoft.com/fwlink/p/?LinkId=313584).
...
>[AZURE.NOTE]Atualmente, não há suporte para a configuração de pontos de extremidade externos (tipo = 'Qualquer'), ponderações para o método de roteamento de tráfego Round Robin e perfis aninhados com o Portal do Azure. Você deve usar o REST (consulte [Criar definição](http://go.microsoft.com/fwlink/p/?LinkId=400772)) ou o Windows PowerShell (consulte [Add-AzureTrafficManagerEndpoint](https://msdn.microsoft.com/library/azure/dn690257.aspx)).

### Definindo configurações no Portal do Azure
...
Você pode definir as seguintes configurações no Portal do Azure:

- **Prefixo DNS** – um prefixo exclusivo criado por você. Os perfis são exibidos no Portal do Azure por prefixo.
...
- **Perfil** – um perfil contém um prefixo de nome de domínio que você cria. Cada perfil corresponde à sua assinatura. Você pode criar vários perfis por assinatura. O nome do perfil está visível no Portal do Azure. O nome que você cria, que está contido no perfil, é conhecido como o Domínio do Gerenciador de Tráfego.
- **Definição** – uma definição contém configurações de política e de monitor. Uma definição corresponde a um perfil. Você pode ter apenas uma definição por perfil. A definição em si não é visível no Portal do Azure, embora muitas das configurações contidas na definição sejam visíveis e possam ser configuradas no Portal do Azure.
- **Opções de DNS** – em cada definição, há opções de DNS. É onde a TTL do DNS é configurada.
- **Monitores** – em cada definição, há configurações de monitor. É onde o protocolo, a porta e o caminho relativo e nome de arquivo são configurados. As configurações de monitor são visíveis e podem ser definidas no Portal do Azure. Para obter mais informações, consulte [Monitoramento do Gerenciador de Tráfego](traffic-manager-monitoring.md).
- **Política** – em cada definição, há configurações de política. A política é onde os métodos de roteamento de tráfego e os pontos de extremidade são especificados. A política em si não é visível no Portal do Azure, embora algumas das configurações de política sejam visíveis e possam ser definidas no Portal do Azure. Para obter mais informações, consulte [Sobre os métodos de roteamento de tráfego do Gerenciador de Tráfego](traffic-manager-load-balancing-methods.md).
...
Para adicionar um perfil do Gerenciador de Tráfego como um ponto de extremidade e configurar o número mínimo de pontos de extremidade íntegros, você deve usar REST (consulte [Criar definição](http://go.microsoft.com/fwlink/p/?LinkId=400772)) ou o Windows PowerShell (consulte [Add-AzureTrafficManagerEndpoint](https://msdn.microsoft.com/library/azure/dn690257.aspx)). Você não pode usar o portal do Azure:
