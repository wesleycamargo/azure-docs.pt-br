<properties
   ...
   description="Mostra o esquema do gerenciador de recursos para a criação de uma atribuição de função durante a implantação."
...
| apiVersion | enum | Sim | **01-07-2015** | A versão da API a ser usada para criar o recurso. |  
...
| Nome | Tipo | Obrigatório | Valores permitidos | Descrição |
| ------- | ---- | ---------------- | -------- | ----------- |
| roleDefinitionId | string | Sim | **/subscriptions/{subscription-id}/providers/Microsoft.Authorization/roleDefinitions/{role-definition-id}** | O identificador de uma definição de função existente a ser usado na atribuição de função. |
| principalId | string | Sim | Identificador global exclusivo | O identificador de uma entidade existente. Isso mapeia para a ID dentro do diretório e pode apontar para um usuário, uma entidade de serviço ou um grupo de segurança. |
| scope | string | Sim | Para o grupo de recursos:<br />**/subscriptions/{id-da assinatura}/resourceGroups/{nome-do-grupo-de-recursos}**<br /><br />Para o recurso:<br />**/subscriptions/{id-da assinatura}/resourceGroups/{nome-do-grupo-de-recursos}/providers/{namespace-do-provedor}/{tipo-de-recurso}/{nome-do-recurso}** | O escopo ao qual essa atribuição de função se aplica. |
