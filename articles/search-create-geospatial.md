...
Para obter mais visão geral, assista a este vídeo no Canal 9 sobre [Pesquisa do Azure e Dados Geoespaciais](http://channel9.msdn.com/Shows/Data-Exposed/Azure-Search-and-Geospatial-Data)
...
+ Um serviço de Pesquisa do Azure. Você precisará do nome do serviço de Pesquisa, mais a chave de administrador. Consulte [Introdução à Pesquisa do Azure](search-get-started.md) para obter detalhes.
...
+	**StoreIndexer** cria um índice de Pesquisa do Azure e carrega dados.
+	**AdventureWorksWebGeo** é um aplicativo baseado em MVC4 que consulta o índice de Pesquisa do Azure e mostra locais e armazenamento em um mapa do Bing.

[WACOM.INCLUDE [Você de uma conta do Azure para concluir este tutorial:](../includes/free-trial-note.md)]


<h2 id="sub-2">Bing Mapas</h2>
...
<h3>Criar uma conta para o Bing Mapas</h3>

1. Acesse o [Portal do Bing Mapas](https://www.bingmapsportal.com/) e crie uma nova conta. Insira os detalhes para criar a conta.
...
<h2 id="sub-3">Geocodificação de endereços no C# usando a API Bing Mapas DataFlow</h2>

Nesta etapa, usamos a API DataFlow do Bing Mapas para geocodificar alguns endereços para várias lojas de bicicletas no mundo todo. 
...
1. Abra a solução AdventureWorksGeo no Visual Studio, expanda o projeto **StoreIndexer** no Gerenciador de Soluções e abra o Program.cs. Visto que já cobrimos a criação do índice na [Pesquisa do Azure - Demonstração do Adventure Works](http://azuresearchadventureworksdemo.codeplex.com/), não iremos discutir como isso funciona no Program.cs.

2. Vá até a função **Main** e observe que ela chama **ApplyStoreData**.  Mova para essa função e explore o código.  

3.	**ApplyStoreData** carrega dados de um arquivo CSV chamado "store_locations.csv" para um System.Data.DataTable.  
...
<h2 id="sub-4">Adicionar o mapeamento para um aplicativo MVC4 usando a Pesquisa do Azure e o Bing Mapas</h2>
...
<h2 id="sub-5">Explorar o AdventureWorksWebGeo</h2>
...
2.	Abra Index.cshtml em **Visualizações** | **Início**.

3.	Esse arquivo segue a forma típica que você adicionaria o Bing Mapas a um aplicativo Web, no entanto, alguns pontos a destacar são:
...
+	A função **Pesquisa** recupera os locais de lojas que são então recebidos como PushPins para o Bing Mapas. 
...
2. Ao prompt PM>, insira "Update-package -reinstall Microsoft.AspNet.Mvc"
...
+ Áreas de seleção desenhadas pelo usuário que permitem aos usuários selecionar regiões específicas para serem pesquisadas ao desenhar uma área no mapa. A área é então filtrada pela Pesquisa do Azure usando a API geo-intersect e traçada no mapa.
