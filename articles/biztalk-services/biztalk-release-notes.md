...
Você 'Compilou' a solução com êxito no Visual Studio. Em seguida, você 'Recompila' ou 'Limpa' a solução. Depois disso, quando você recompilar ou limpar novamente, ocorrerá o seguinte erro: Não é possível copiar o arquivo <Caminho para DLL> para “bin\\Debug\\FileName.dll”. O processo não pode acessar o arquivo 'bin\\Debug\\FileName.dll' porque ele está sendo usado por outro processo.
