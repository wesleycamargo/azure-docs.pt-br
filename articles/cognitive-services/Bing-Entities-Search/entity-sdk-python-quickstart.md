...
```
Instale as dependências do SDK de Pesquisa de Entidade do Bing:
```
cd mytestenv
...
```
Pesquise uma única entidade (Gibralter) e imprima uma breve descrição:
```
...
```
Pesquise e lide com os resultados de desambiguação para uma consulta ambígua (William Gates).
```
...
```
Pesquise um único repositório (Microsoft Store) e imprima o seu número de telefone.
```
...
```
Pesquise uma lista de restaurantes (restaurantes em Seattle) e imprima seus nomes e números de telefone.
```
...
```
Disparar uma solicitação incorreta e leia a resposta de erro.
```
