---
title: Análise de vídeo em tempo real com a API de Detecção Facial | Microsoft Docs
titleSuffix: Microsoft Cognitive Services
description: Use a API de Detecção Facial dos Serviços Cognitivos para executar a análise em tempo quase real em quadros obtidos de um fluxo de vídeo ao vivo.
...
ms.locfileid: "35363716"
---
# <a name="how-to-analyze-videos-in-real-time"></a>Como analisar vídeos em tempo real
Este guia demonstrará como executar a análise em tempo quase real em quadros obtidos de um fluxo de vídeo ao vivo. Os componentes básicos de um sistema desse tipo são:
...
Essas amostras são escritas em C# e o código pode ser encontrado no GitHub aqui: [https://github.com/Microsoft/Cognitive-Samples-VideoFrameAnalysis](https://github.com/Microsoft/Cognitive-Samples-VideoFrameAnalysis/).

## <a name="the-approach"></a>A abordagem
Há várias maneiras de solucionar o problema de execução da análise em tempo quase real em fluxos de vídeo. Começaremos descrevendo três abordagens em níveis crescentes de sofisticação.

### <a name="a-simple-approach"></a>Uma abordagem simples
O design mais simples para um sistema de análise em tempo quase real é um loop infinito, no qual em cada iteração capturamos um quadro, analisamos esse quadro e, em seguida, consumimos o resultado:
```CSharp
while (true)
{
    Frame f = GrabFrame();
...
```
Também temos um thread consumidor, que está removendo as tarefas da fila, aguardando sua conclusão e exibindo o resultado ou acionando a exceção gerada. Usando a fila, podemos assegurar que os resultados sejam consumidos um de cada vez, na ordem correta, sem limitar a taxa máxima de quadros do sistema.
```CSharp
// Consumer thread. 
while (true)
{
...
1. Obtenha chaves de API para as APIs da Pesquisa Visual em [Assinaturas](https://azure.microsoft.com/try/cognitive-services/). Para a análise de quadros de vídeo, as APIs aplicáveis são:
...
3. Abra a amostra no Visual Studio 2015, compile e execute os aplicativos de exemplo:
...
Neste guia, você aprendeu como executar a análise em tempo quase real em fluxos de vídeo ao vivo usando as APIs de Detecção Facial, de Pesquisa Visual Computacional e de Detecção de Emoções e como você pode usar nosso código de exemplo para começar.  Comece criando seu aplicativo com as chaves de API gratuitas na [página de inscrição dos Serviços Cognitivos da Microsoft](https://azure.microsoft.com/try/cognitive-services/). 
