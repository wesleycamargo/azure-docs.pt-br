---
title: "Tipos de compilação e qualidade do modelo: API de recomendações | Microsoft Docs"
...
---
# <a name="build-types-and-model-quality"></a>Tipos de compilação e qualidade do modelo
...
### <a name="recommendation-build-type"></a>Tipo de build de recomendação
...
#### <a name="recommendations-build-parameters"></a>Recomendações de parâmetros de compilação
...
| *ItemCutOffLowerBound* |Define o número mínimo de pontos de uso nos quais um item deve estar para que seja considerado parte do modelo. |Inteiro,  <br> 2 ou mais <br>  (2) |
| *ItemCutOffUpperBound* |Define o número máximo de pontos de uso nos quais um item deve estar para que seja considerado no modelo. |Inteiro,  <br>2 ou mais<br>  (2147483647) |
| *UserCutOffLowerBound* |Define o número mínimo de transações que um usuário deve ter realizado para ser considerado parte do modelo. |Inteiro,  <br> 2 ou mais <br>  (2) |
| *UserCutOffUpperBound* |Define o número máximo de transações que um usuário deve ter realizado para ser considerado no modelo. |Inteiro,  <br>2 ou mais <br>  (2147483647) |
...
| *SplitterStrategy* |Se habilitar informações de modelagem estiver definido como *true*, essa será a maneira como os dados deverão ser divididos para fins de avaliação. |String, *RandomSplitter* ou *LastEventSplitter* <br>Padrão: RandomSplitter |

<a name="FBTBuild"></a>

### <a name="fbt-build-type"></a>Tipo de build FBT
...
| *FbtMaxItemSetSize* |Limita o número de itens em um conjunto frequente. |Número inteiro    <br> 2-3 <br>  (2) |
| *FbtMinimalScore* |Pontuação mínima que um conjunto frequente deve ter para ser incluído nos resultados retornados. Quanto maior, melhor. |Duplo <br> 0 e acima <br>  acima (0) |
| *FbtSimilarityFunction* |Define a função de semelhança a ser usada pela compilação. **Lift** favorece o acaso, **co-occurrence** favorece a previsibilidade e **Jaccard** é a interseção. |Cadeia de caracteres,   <br>  <i>cooccurrence, lift, jaccard</i><br> Padrão: <i>jaccard</i> |

<a name="SelectBuild"></a>

## <a name="build-evaluation-and-selection"></a>Avaliação e seleção de compilação
...
O objetivo de uma avaliação offline é prever a precisão (número de usuários que comprarão um dos itens recomendados) e a diversidade de recomendações (o número de itens que são recomendados).
Como parte da avaliação de métricas de precisão e a diversidade, o sistema encontra um exemplo de usuários e divide as transações desses usuários em dois grupos: o conjunto de dados de treinamento e o conjunto de dados de teste.

> [!NOTE]
> Para usar métricas offline, você deve ter carimbos de da/hora em seus dados de uso.
> Os dados de hora são necessários para dividir o uso corretamente entre conjuntos de dados de treinamento e teste.
> 
...
#### <a name="percentage"></a>Percentual
...
#### <a name="users-in-test"></a>Usuários no teste
...
#### <a name="users-not-considered"></a>Usuários não considerados
...
#### <a name="percentile-buckets"></a>Buckets de percentil
...
* Opcionalmente, selecione *splitterStrategy* (*RandomSplitter* ou *LastEventSplitter*).
  *RandomSplitter* divide os dados de uso nos conjuntos de dados de teste e de treinamento com base na porcentagem de teste *randomSplitterParameters* determinada e em valores de semente aleatórios.
  *LastEventSplitter* divide os dados de uso nos conjuntos de dados de treinamento e de teste com base na última transação de cada usuário.
