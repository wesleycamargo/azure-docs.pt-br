...
* Reconhecer uma declaração breve, como um comando, sem resultados intermediários
...
A API oferece suporte a marcas SSML (Idioma com Marcação de Síntese de Fala), para que você possa especificar a pronúncia fonética exata de palavras problemáticas. O SSML também pode indicar características de fala (incluindo ênfase, taxa, volume, gênero e tom) diretamente no texto.
...
Se você precisar de um dialeto sem suporte ou quiser apenas uma voz exclusiva para o seu aplicativo, a API de Conversão de Texto em Fala dá suporte a modelos de voz personalizados.
