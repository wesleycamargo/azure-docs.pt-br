<properties
 pageTitle="Guia do desenvolvedor do Hub IoT do Azure | Microsoft Azure"
 description="Guia do desenvolvedor do Hub IoT do Azure abordando pontos de extremidade do Hub IoT, segurança, registro de identidade de dispositivo e mensagens"
 services="iot-hub"
...
- [Registro de identidade do dispositivo](#identityregistry). Esta seção descreve quais informações são armazenadas no registro de identidade de dispositivo de cada Hub IoT, e como elas podem ser acessadas e modificadas.
- [Segurança](#security). Esta seção descreve o modelo de segurança para dispositivos e componentes da nuvem usados para conceder acesso aos recursos do Hub IoT.
...
O Hub IoT do Azure é um serviço multilocatário que expõe funcionalidades a diversos atores. A figura abaixo mostra os vários pontos de extremidade expostos pelo Hub IoT do Azure.

![Pontos de extremidade do Hub IoT][img-endpoints]

Veja uma descrição dos pontos de extremidade:

* **Provedor de recursos** - o provedor de recursos do Hub IoT expõe uma interface do [Gerenciador de Recursos do Azure][lnk-arm], que permite aos proprietários de assinatura do Azure criar Hubs IoT, atualizar propriedades do Hub IoT e excluir Hubs IoT. As propriedades do Hub IoT regem as políticas de segurança no nível do hub (ao contrário do controle de acesso no nível do dispositivo, consulte [Controle de acesso](#accesscontrol)) e as opções funcionais para sistema de mensagens da nuvem para o dispositivo e do dispositivo para a nuvem. O provedor de recursos também permite [Exportar identidades de dispositivo](#importexport).
* **Gerenciamento de identidade de dispositivos** -cada Hub IoT expõe um conjunto de pontos de extremidade HTTP REST para gerenciamento de identidades de dispositivo (por exemplo, criar, recuperar, atualizar e excluir). As identidades de dispositivo são usadas para controle de acesso e autenticação do dispositivo. Consulte [Registro de identidade do dispositivo](#identityregistry) para saber mais.
* **Pontos de extremidade do dispositivo** - para cada dispositivo provisionado no registro de identidade de dispositivo, o Hub IoT expõe um conjunto de pontos de extremidade que serão usados para comunicação nesse dispositivo. Atualmente, esses pontos de extremidade são expostos em HTTP e [AMQP][lnk-amqp]\:
    - *Enviar mensagens do dispositivo para a nuvem*. Esse ponto de extremidade é usado para enviar mensagens do dispositivo para a nuvem. Consulte [Sistema de mensagens do dispositivo para a nuvem](#d2c) para saber mais.
    - *Receber mensagens da nuvem para o dispositivo*. Esse ponto de extremidade é usado pelo dispositivo para receber mensagens da nuvem para o dispositivo direcionadas a ele. Consulte [Sistema de mensagens da nuvem para o dispositivo](#c2d) para saber mais.
...
Ao usar os SDKs (ou integrações de produtos) que não estejam cientes do Hub IoT, é necessário recuperar um ponto de extremidade compatível com os Hubs de Eventos e o nome do hub de evento das configurações do Hub IoT no [Portal de Visualização do Azure]\:
...
> [AZURE.NOTE]Às vezes, o SDK exige um **Nome de host** ou um **Namespace**. Quando esse for o caso, remova o esquema do **Ponto de extremidade compatível com o Hub de Eventos**. Por exemplo, se o ponto de extremidade compatível com o Hub de Eventos for `sb://iothub-ns-myiothub-1234.servicebus.windows.net/`, o **Hostname** será `iothub-ns-myiothub-1234.servicebus.windows.net` e o **Namespace** será `iothub-ns-myiothub-1234`.

Dessa forma, você pode usar qualquer política de segurança de acesso compartilhado com permissões **ServiceConnect** para conectar-se ao Hub de Eventos acima.

Caso você tenha que construir uma cadeia de conexão do Hub de Eventos com as informações acima, use o seguinte padrão:

        Endpoint={Event Hub-compatible endpoint};SharedAccessKeyName={iot hub policy name};SharedAccessKey={iot hub policy key}


Veja uma lista de SDKs e integrações que podem ser usados com o Hub IoT:

* [Cliente Java dos Hubs de Eventos](https://github.com/hdinsight/eventhubs-client)
* [Spout do Apache Storm](../hdinsight/hdinsight-storm-develop-csharp-event-hub-topology.md), encontre o link para a origem do spout [aqui](https://github.com/apache/storm/tree/master/external/storm-eventhubs)
* [Integração do Apache Spark](../hdinsight/hdinsight-apache-spark-csharp-apache-zeppelin-eventhub-streaming.md)

## Registro de identidade do dispositivo <a id="identityregistry"></a>
...
Em um alto nível, o registro de identidade do dispositivo é uma coleção RESTful de recursos de identidade do dispositivo. As seções a seguir detalham as propriedades do recurso de identidade do dispositivo e as operações em identidades permitidas pelo registro.

> [AZURE.NOTE]Você pode consultar [SDKs e APIs do Hub IoT][lnk-apis-sdks] para saber mais detalhes sobre o protocolo HTTP e os SDKs disponíveis para interagir com o registro de identidade do dispositivo.
...
| connectionStateUpdatedTime | somente leitura | Data e hora da ultima atualização do estado da conexão. |
...
O registro de identidade de dispositivo do Hub IoT do Azure expõe as seguintes operações:
...
Todas as operações acima permitem o uso da simultaneidade otimista como especificado na [RFC7232][lnk-rfc7232].
...
O registro de identidade de dispositivo do hub não contém metadados de aplicativo, pode ser acessado como um dicionário usando o **deviceId** como a chave e não tem suporte para consultas expressivas. Qualquer solução de IoT terá um repositório específico para a solução contendo metadados específicos do aplicativo, como o espaço implantado para um sensor de temperatura em uma solução inteligente de construção.
...
1. Durante um processo de orquestração de provisionamento. Consulte [Orientação do Hub IoT do Azure - Provisionamento][lnk-guidance-provisioning] para saber mais.
2. Se, por algum motivo, você considerar um dispositivo comprometido ou temporariamente não autorizado.
...
Você pode exportar identidades de dispositivo em massa do registro de identidade de um hub IoT. Isso pode ser feito com operações assíncronas no ponto de extremidade do provedor de recursos do Hub IoT do Azure ([Pontos de extremidade](#endpoints)).

Estas são as operações possíveis em trabalhos de exportação:
...
| includeKeysInExport | opcional | Se for **true**, as chaves serão incluídas na saída da exportação; caso contrário, as chaves serão exportadas como null. Padrão: **false**. |
...
Os trabalhos de exportação usam, como um parâmetro, um URI de Assinatura de Acesso Compartilhado de blob que concede acesso de gravação a um contêiner de blob. Isso é usado para mostrar o resultado da operação.

Os resultados são gravados no contêiner de blob especificado em um arquivo chamado `job_{job_id}_devices.txt`. Esse arquivo conterá identidades de dispositivo serializadas como JSON, conforme especificado nas [Propriedades de identidade de dispositivo](#deviceproperties). Os materiais de segurança serão definidos como **null** caso **includeKeysInExport** seja definido como **false**.
...
O Hub IoT do Azure usa o seguinte conjunto de *permissões* para conceder acesso ao ponto de extremidade de cada hub. As permissões limitam o acesso a um hub IoT com base na funcionalidade.

* **RegistryRead**. Essa permissão concede acesso de leitura ao registro de identidade do dispositivo. Consulte [Registro de identidade do dispositivo](#identityregistry) para saber mais.
* **RegistryReadWrite**. Essa permissão concede acesso de leitura e gravação ao registro de identidade do dispositivo. Consulte [Registro de identidade do dispositivo](#identityregistry) para saber mais.
* **ServiceConnect**. Essa permissão concede acesso aos pontos de extremidade de comunicação e de monitoramento voltados para o serviço de nuvem. Por exemplo, ela concede permissão para que os serviços de nuvem recebam mensagens do dispositivo para a nuvem, enviem mensagens da nuvem para o dispositivo e obtenham as confirmações de entrega correspondentes.
* **DeviceConnect**. Essa permissão concede acesso aos pontos de extremidade de comunicação voltados para o dispositivo. Por exemplo, ela concede permissão de envio de mensagens do dispositivo para a nuvem e de recebimento de mensagens da nuvem para o dispositivo. Essa permissão é usada principalmente por dispositivos.

As permissões são concedidas das seguintes maneiras:

* **Políticas de acesso compartilhado no nível do Hub**. As *políticas de acesso compartilhado* podem conceder qualquer combinação das permissões listadas acima. Você pode definir políticas no [Portal de Gerenciamento do Azure][lnk-management-portal] ou programaticamente usando as [APIs do provedor de recursos do Hub IoT do Azure][lnk-resource-provider-apis]. Um hub IoT recém-criado tem as seguintes políticas padrão:
...
* **Credenciais de segurança de acordo com o dispositivo**. Cada Hub IoT contém um [registro de identidade do dispositivo](#identityregistry). É possível configurar as credenciais de segurança para cada dispositivo nesse registro concedendo permissões de **DeviceConnect** com escopo nos pontos de extremidade correspondentes do dispositivo.

**Exemplo**. Em uma solução IoT, normalmente há um componente de gerenciamento de dispositivo que usa a política *registryReadWrite*, um componente processador de eventos e um componente de lógica de negócios de dispositivo em tempo de execução que usam a política de *serviço*. Os dispositivos individuais se conectam usando as credenciais armazenadas no registro de identidade do hub.

Para obter orientação sobre os tópicos de segurança do Hub IoT, consulte a seção [Orientação sobre o Hub IoT do Azure][lnk-guidance-security].

### Autenticação
O Hub IoT do Azure concede acesso aos pontos de extremidade, verificando um token com base nas políticas de acesso compartilhado e nas credenciais de segurança de registro de identidade do dispositivo. Credenciais de segurança, como chaves simétricas, nunca são enviadas na conexão.
...
| Valor | Descrição |
| ----- | ----------- |
| {signature} | Uma assinatura HMAC-SHA256 da cadeia de caracteres: `{URL-encoded-resourceURI} + "\n" + expiry` |
...
Quando usa [AMQP][lnk-amqp], o Hub IoT do Azure dá suporte ao [SASL PLAIN][lnk-sasl-plain] e à [Segurança baseada em declarações AMQP][lnk-cbs].

No caso da segurança baseada em declarações AMQP, o padrão especifica como transmitir os tokens listados acima.

Para SASL PLAIN, o nome de usuário pode ser: * `{policyName}&commat;sas.root.{iothubName}` no caso de tokens no nível do hub. * `{deviceId}` no caso de tokens no escopo do dispositivo.

Em ambos os casos, o campo de senha contém o token conforme descrito na seção [Formato do token](#tokenformat).

> [AZURE.NOTE]Os [SDKs do Hub IoT do Azure ][lnk-apis-sdks] geram tokens automaticamente durante a conexão com o serviço. Em alguns casos, os SDKs apresentam limitações com relação ao protocolo com suporte ou ao método de autenticação disponível. Consulte a documentação dos [SDKs do Hub IoT do Azure][lnk-apis-sdks] para saber mais.
...
Esse é um mecanismo parecido com a [política de editor de Hubs de Eventos][lnk-event-hubs-publisher-policy] e permite a implementação de métodos de autenticação personalizados, conforme é explicado na seção de segurança da [Orientação sobre o Hub IoT do Azure][lnk-guidance-security].

## Mensagens
O Hub IoT do Azure fornece primitivos do sistema de mensagens para a comunicação entre um back-end de aplicativo (*serviço* ou *nuvem*) e dispositivos, e vice-versa. Chamamos essas funcionalidades de [Dispositivo para a Nuvem](#d2c) e [Nuvem para o Dispositivo](#c2d).
...
| -------- | ----------- |
| MessageId | Um identificador definido pelo usuário para a mensagem, normalmente usado para padrões de solicitação-resposta. Formato: uma cadeia de caracteres que diferencia maiúsculas de minúsculas (com até 128 caracteres) de caracteres alfanuméricos ASCII 7 bits + `{'-', ':',’.', '+', '%', '_', '#', '*', '?', '!', '(', ')', ',', '=', '@', ';', '$', '''}`. |
...
| ConnectionAuthMethod | Definida pelo Hub IoT nas mensagens D2C. Informações sobre o método de autenticação usado para autenticar o dispositivo que envia a mensagem. Consulte [Anti-falsificação D2C](#antispoofing) para saber mais.|

### Escolhendo seu protocolo de comunicação <a id="amqpvshttp"></a>
O Hub IoT dá suporte para os protocolos [AMQP][lnk-amqp] e HTTP/1 para comunicações do lado do dispositivo. Veja uma lista de considerações sobre suas utilizações.

* **Padrão da nuvem para o dispositivo**. O HTTP/1 não tem uma maneira eficiente de implementar o envio do servidor. Ao usar HTTP/1, os dispositivos estão sondando o Hub IoT do Azure em busca de mensagens da nuvem para o dispositivo. Isso é muito ineficiente para o dispositivo e para o Hub IoT. As diretrizes atuais indicam que, ao usar HTTP/1, é necessário definir um intervalo de sondagem inferior a uma vez a cada 25 minutos para cada dispositivo. Por outro lado, AMQP dá suporte ao envio do servidor ao receber mensagens da nuvem para o dispositivo, e isso habilita imediatamente os envios de mensagens do Hub IoT para o dispositivo. Se a latência de entrega for uma preocupação, AMQP será a melhor opção de protocolo. Por outro lado, para dispositivos conectados raramente, o HTTP/1 funciona bem.
...
* **Dispositivos com poucos recursos**. As bibliotecas HTTP/1 são consideravelmente menores do as do AMQP. Caso o dispositivo tenha poucos recursos (por exemplo, menos de 1 Mb de RAM), HTTP/1 poderá ser a única implementação de protocolo disponível.
* **Percurso da rede**. O padrão AMQP escuta na porta 5672. Isso pode causar problemas em redes fechadas para protocolos diferentes de HTTP. Observe que essa limitação é temporária, pois o Hub IoT implementará AMQP sobre WebSockets.
...
O Hub IoT do Azure implementa o envio de mensagens do dispositivo para a nuvem de uma maneira bastante semelhante aos [Hubs de Eventos][lnk-event-hubs], com mensagens do dispositivo para a nuvem do Hub IoT apresentando mais semelhanças aos *eventos* dos Hubs de Eventos do que as *mensagens* do [Barramento de Serviço][lnk-servicebus].
...
* Assim como nos Hubs de Eventos, os clientes que leem mensagens do dispositivo para a nuvem precisam lidar com partições e pontos de verificação, consulte [Hubs de Eventos - consumindo eventos][lnk-event-hubs-consuming-events].
* Assim como os eventos dos Hubs de Eventos, as mensagens do dispositivo para a nuvem podem ter no máximo 256 Kb e podem ser agrupadas em lotes para otimizar os envios. Os lotes podem ter no máximo 256 Kb em tamanho e no máximo 500 mensagens.
...
Em muitos casos, os dispositivos não estão apenas enviando pontos de dados de telemetria para o back-end de aplicativo, mas também mensagens e solicitações *interativas* que exigem a execução e a manipulação da camada de lógica de negócios do aplicativo. Bons exemplos são alertas críticos que precisam disparar uma ação específica no back-end, ou respostas do dispositivo aos comandos.

Consulte a seção [Processamento do dispositivo para a nuvem][lnk-guidance-d2c-processing] da Orientação do Hub IoT para saber mais sobre a melhor maneira de processar esse tipo de mensagem.
...
* **Contagem de partição**. Essa propriedade pode ser definida no momento da criação e define o número de partições para inclusão do evento D2C.
...
Todas as propriedades acima podem ser modificadas no [Portal de Gerenciamento do Azure][lnk-management-portal] ou programaticamente por meio do [Hub IoT do Azure - APIs do provedor de recursos][lnk-resource-provider-apis].
...
As duas primeiras contêm a **deviceId** e a **generationId** do dispositivo de origem, conforme as [Propriedades de identidade do dispositivo](#deviceproperties). A propriedade **ConnectionAuthMethod** contém um objeto JSON serializado com as seguintes propriedades:
...
Consulte [Introdução às mensagens da nuvem para o dispositivo do Hub IoT do Azure][lnk-getstarted-c2d-tutorial] para obter um tutorial sobre mensagens da nuvem para o dispositivo, e [SDKs e APIs do Hub IoT][lnk-apis-sdks] para obter referencias sobre como diferentes APIs e SDKs expõem a funcionalidade da nuvem para o dispositivo.
...
| Propriedade | Descrição |
| -------- | ----------- |
| EnqueuedTime | Carimbo de data e hora da saída da mensagem. Por exemplo, o dispositivo foi concluído ou a mensagem expirou. |
...
| Descrição | Valores de cadeia de caracteres para os resultados acima. |
...
**Importante**. O serviço precisa especificar um **MessageId** para a mensagem da nuvem para o dispositivo a fim de poder correlacionar seus comentários com a mensagem original.

**Exemplo**. Veja um exemplo de corpo de uma mensagem de comentários.
...
O Hub IoT permitirá até 1100 atualizações de dispositivo (criar, atualizar e excluir) por unidade (independentemente da SKU) por dia.

### Restrições de operação
As restrições de operação são limites de taxa aplicados em intervalos de minutos, e têm como objetivo evitar o abuso. O Hub IoT tenta evitar o retorno de erros sempre que possível, mas ele começará a retornar exceções se a restrição for violada por muito tempo. Veja a lista de restrições impostas. Os valores referem-se a um hub individual.
...
**Observação**. A qualquer momento, é possível aumentar as cotas ou restrições aumentando as unidades provisionadas em um Hub IoT.
...
[lnk-arm]: https://azure.microsoft.com/pt-BR/documentation/articles/resource-group-overview/
[lnk-azure-resource-manager]: https://azure.microsoft.com/pt-BR/documentation/articles/resource-group-overview/
[lnk-cbs]: https://www.oasis-open.org/committees/download.php/50506/amqp-cbs-v1%200-wd02%202013-08-12.doc
[lnk-createuse-sas]: https://azure.microsoft.com/pt-BR/documentation/articles/storage-dotnet-shared-access-signature-part-2/
[lnk-event-hubs-publisher-policy]: https://code.msdn.microsoft.com/Service-Bus-Event-Hub-99ce67ab
[lnk-event-hubs]: http://azure.microsoft.com/services/event-hubs/
[lnk-event-hubs-consuming-events]: https://azure.microsoft.com/pt-BR/documentation/articles/event-hubs-programming-guide/#consume-events-event-consumers
[lnk-guidance-d2c-processing]: iot-hub-csharp-csharp-process-d2c.md
[lnk-management-portal]: https://portal.azure.com
[lnk-rfc7232]: https://tools.ietf.org/html/rfc7232
[lnk-sas-java]: https://msdn.microsoft.com/pt-BR/library/azure/Hh875756.aspx
