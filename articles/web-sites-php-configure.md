<properties title="Como configurar o PHP nos Sites do Azure" pageTitle="Como configurar o PHP nos Sites do Azure" metaKeywords="Azure, Sites do Azure, configuração, PHP" description="Saiba como configurar a instalação padrão do PHP ou adicione uma instalação personalizada nos Sites do Azure." services="Web Sites" documentationCenter="PHP" />
...
1. No Portal do Azure, navegue para o painel do seu site e clique em **Configurar**.
	...
	![Configurar a guia no painel dos sites][configure]

1. Clique em PHP 5.5.

	![Selecione a versão do PHP][select-php-version]
...
1. Na seção **configurações do aplicativo**, crie uma chave **PHP_EXTENSIONS** e um valor **bin\your-ext-file**. Para habilitar várias extensões, inclua uma lista de arquivos `.dll` separada por vírgulas.
	...
	![Habilitar extensões em configurações do aplicativo][app-settings]

1. Clique em **Salvar** na parte inferior da página.

	![Salvar definições de configuração][save-button]

<h2><a name="UseCustomPHP"></a>Como: usar um tempo de execução personalizado do PHP</h2>
...
1. Na seção **mapeamentos de manipulador**, adicione `*.php` à EXTENSÃO e adicione o caminho ao `php-cgi.exe` executável. Se você colocar o seu tempo de execução do PHP no diretório `bin` na raiz do seu aplicativo, o caminho será `D:\home\site\wwwroot\bin\php\php-cgi.exe`.
