<properties title="How to Configure PHP in Azure Websites" pageTitle="Como configurar o PHP nos Sites do Azure" metaKeywords="Azure, Azure Web Sites, configuration, PHP" description="Learn how to configure the default PHP installation or add a custom PHP installation in Azure Websites." services="Web Sites" documentationCenter="PHP" authors="tomfitz" manager="wpickett" />
...
Uma alternativa para o uso de um arquivo é usar a função [ini_set()] em scripts para definir opções de configuração que não sejam diretrizes de nível de sistema apenas.
...
1. Coloque arquivos de extensão `.dll` no diretório `bin` (por exemplo, `php_mongo.dll`). Certifique-se de que as extensões sejam compatíveis com a versão padrão do PHP (que, no momento deste texto, é PHP 5.4), que sejam não thread-safe e compatíveis com a versão VC9.
1. Implante seu aplicativo.
1. No Portal do Microsoft Azure, navegue para o painel do seu site e clique em **Configurar**.
