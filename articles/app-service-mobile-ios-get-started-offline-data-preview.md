<properties 
	...
	description="Aprenda a usar o Aplicativo Móvel do Serviço de Aplicativo para cache e sincronização de dados offline no aplicativo iOS" 
...
3. Antes de qualquer operação de tabela poder ser executada, o armazenamento local deve ser inicializado.  Este é o código relevante no método `QSTodoService.init`:
    ...
    Isso cria um repositório local usando a interface `MSCoreDataStore`, que é fornecida no SDK do aplicativo móvel.  Você pode, em vez disso, fornecer um repositório local diferente implementando o protocolo `MSSyncContextDataSource`.
...
4. Os métodos `pullData` e `syncData` realizam a operação de sincronização em si: `syncData` primeiro envia novas alterações, em seguida, chama `pullData` para obter dados de back-end remoto.
    ...
    O segundo parâmetro para `pullWithQuery` é uma ID de consulta que é usada para *sincronização incremental*.  A sincronização incremental recupera somente os registros modificados desde a última sincronização, usando o carimbo de data/hora `UpdatedAt` do registro (chamado `ms_updatedAt` no armazenamento local).  A ID da consulta deve ser uma cadeia de caracteres descritiva que é exclusiva para cada consulta lógica em seu aplicativo.  Se desejar sair da sincronização incremental, passe `nil` como a ID da consulta.  Observe que isso pode ser potencialmente ineficiente, já que irá recuperar todos os registros de cada operação de recepção.

<!--     >[AZURE.NOTE] Para remover registros de armazenamento local do dispositivo quando eles tiverem sido excluídos do banco de dados do serviço móvel, você deve habilitar a [Exclusão Reversível].  Caso contrário, seu aplicativo deve chamar periodicamente `MSSyncTable.purgeWithQuery` para limpar o armazenamento local.
 -->
...
- Ao usar o recurso de sincronização offline, você deve definir as tabelas do sistema, conforme mostrado abaixo.
    ...
    **TodoItem**
    ...
    |-----------   |  ------ | -------------------------------------------------------|
    | ID           | Cadeia de caracteres  | chave primária no armazenamento remoto                            |
    | concluído     | Booliano | campo todo item                                        |
    | texto         | Cadeia de caracteres  | campo todo item                                        |
    | ms_createdAt | Data    | (opcional) é mapeado para a propriedade do sistema __createdAt         |
    | ms_updatedAt | Data    | (opcional) é mapeado para a propriedade do sistema _updatedAt         |
    | ms_version   | Cadeia de caracteres  | (opcional) usado para detectar conflitos, é mapeado para _version |
...
1. Em **QSTodoListViewController.m**, altere o método **viewDidLoad** para remover a chamada para `[autoatualização]` no final do método.  Agora, os dados não serão sincronizados com o servidor na inicialização do aplicativo, mas em vez disso, se tornarão o conteúdo do repositório local.

2. Em **QSTodoService.m**, modifique a definição de `addItem` para que ele não sincronize após o item ser inserido.  Remova o bloco `self syncData` e substitua-o pelo seguinte:
...
Nesta seção, você desligará o Wi-Fi no simulador para criar um cenário offline.  Quando você adicionar itens de dados, eles serão mantidos no armazenamento local de dados principais, mas não sincronizados com o back-end móvel.
...
3. Exiba o conteúdo da tabela TodoItem remota:
   ...
   - Para o back-end do .NET, exiba o conteúdo da tabela com uma ferramenta SQL como o SQL Server Management Studio, ou então com um cliente REST, como o Fiddler ou Postman.
    ...
    Verifique se os novos itens foram *not* sincronizados com o servidor:
...
Para dar suporte a esse recurso de sincronização offline, usamos a `MSSyncTable` interface e inicializamos `MSClient.syncContext` com um repositório local.  Nesse caso, o repositório local era um banco de dados baseado em dados principais. 
...
*  Para executar push das mudanças no servidor, chamamos  `Review the Core Data model`.  Esse método é um membro de `MSSyncContext` em vez da tabela de sincronização porque enviará as alterações por push a todas as tabelas.
...
<!-- * Para remover registros de armazenamento local do dispositivo quando eles tiverem sido excluídos do banco de dados do serviço móvel, você deve habilitar a [Exclusão Reversível].  Caso contrário, seu aplicativo deve chamar periodicamente `MSSyncTable.purgeWithQuery` para remover registros do banco de dados local, caso eles tenham sido excluídos no serviço remoto.
 -->
...
[Criar um aplicativo do iOS]: app-service-mobile-dotnet-backend-ios-get-started.md
...
[Compilar uma Interface do Usuário]: https://developer.apple.com/library/mac/documentation/ToolsLanguages/Conceptual/Xcode_Overview/Edit_User_Interfaces/edit_user_interface.html
...
[Como usar a biblioteca de cliente dos Serviços Móveis para iOS]: mobile-services-ios-how-to-use-client-library.md
