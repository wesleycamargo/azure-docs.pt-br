...
-   Como os modelos do ASP.NET MVC 5 usam o framework de CSS Bootstrap para aprimorar a exibição em dispositivos móveis
...
3. 	Na caixa de diálogo **Propriedades de Mvc5Mobile.zip**, escolha o botão **Desbloquear**. (Desbloquear impede a exibição de um aviso de segurança que normalmente ocorre quando você tenta usar um arquivo .zip que você baixou da Web.)
...
9.	Digite o nome da conta do Azure e clique em **Continuar**.
...
*bootstrap.js* e *bootstrap.css*. O código a seguir mostra as duas últimas instruções do método `RegisterBundles` após a alteração:
...
Observe que a barra de navegação recolhível é agora apenas uma lista simples e desordenada. Clique em **Procurar por marca** novamente, então clique em **ASP.NET**.
...
O Bootstrap não é específico para o ASP.NET MVC 5, e você pode aproveitar esses recursos em qualquer aplicativo Web. Mas ele agora está incorporado ao
modelo de projeto ASP.NET MVC 5, para que seu aplicativo Web do MVC 5 possa tirar proveito do Bootstrap por padrão.

Para obter mais informações sobre o Bootstrap, vá para o
...
Você pode substituir qualquer modo de exibição (inclusive layouts e modos de exibição parciais) para navegadores de dispositivos móveis em geral, para um navegador móvel individual ou para qualquer navegador específico. Para fornecer um modo de exibição específico para dispositivos móveis, você pode copiar um arquivo de modo de exibição e adicionar *.Mobile* ao nome do arquivo. Por exemplo, para criar um modo de exibição
*Índice* móvel, você pode copiar *Views\\Home\\Index.cshtml* para
*Views\\Home\\Index.Mobile.cshtml*.

Nesta seção, você criará um arquivo de layout específico para dispositivos móveis.

Para iniciar, copie *Views\\Shared\\\_Layout.cshtml* para
*Views\\Shared\\\_Layout.Mobile.cshtml*. Abra *\_Layout.Mobile.cshtml* e altere o título de **Aplicativo MVC5** para **Aplicativo MVC5 (Móvel)**.

Em cada chamada `Html.ActionLink` para a barra de navegação, remova "Procurar por" em cada link
*ActionLink*. O código a seguir mostra a marca completa `<ul class="nav navbar-nav">` do arquivo de layout móvel.
...
Navegue até a página de marcas usando um navegador de desktop e o emulador do navegador móvel. O emulador do navegador móvel mostra as duas alterações que você fez (o título de *\_Layout.Mobile.cshtml* e o título de
...
Este código define um novo modo de exibição denominado "iPhone", que será comparado a todas as solicitações de entrada. Se a solicitação de entrada coincide com a condição definida (ou seja, se o agente do usuário contém a cadeia de caracteres
"iPhone"), o MVC do ASP.NET procurará por modos de exibição cujo nome contém o
sufixo "iPhone".
...
Você pode desabilitar globalmente um modo de exibição padrão (não móvel) de ser renderizado dentro de um layout para dispositivos móveis, definindo `RequireConsistentDisplayMode` como
'true' no arquivo *Views\\\_ViewStart.cshtml*, assim:
...
No entanto, a principal vantagem do framework de Bootstrap CSS é o layout responsivo, o que significa que uma única folha de estilos pode ser aplicada em área de trabalho, telefone e navegadores de tablet para criar aparência e sensação consistentes. Na próxima seção, você verá como aproveitar o Bootstrap para criar modos de exibição para dispositivos móveis.

##<a name="bkmk_Improvespeakerslist"></a> Aprimorar a lista de alto-falantes

Como você acabou de ver, o modo de exibição *Speakers* está legível, mas os links são pequenos e difíceis de tocar em um dispositivo móvel. Nesta seção, você tornará o modo de exibição *AllSpeakers* acessível para dispositivos móveis, que exibe links grandes, fáceis de tocar e que contém uma caixa de pesquisa para localizar alto-falantes rapidamente.

É possível usar o estilo do [grupo de listas vinculadas][] do Bootstrap para aprimorar o modo de exibição *Speakers*. Em *Views\\Home\\AllSpeakers.cshtml*, substitua o conteúdo do arquivo Razor pelo código abaixo.
...
estilo de lista do Bootstrap, e o atributo `class="input-group-item"` aplica estilos de item de lista do Bootstrap a cada link.
...
Observe que as marcas `<form>` e `<input>` ambas possuem os estilos Bootstrap aplicados a elas. O elemento '<span>' adiciona um
[glyphicon][] do Bootstrap à caixa de pesquisa.
...
Também será preciso incluir o filter.js em seus pacotes registrados. Abra
*App\_Start\\BundleConfig.cs* e altere os primeiros pacotes. Altere a primeira instrução `bundles.Add` (para o pacote **jquery**) para incluir
*Scripts\\filter.js*, da seguinte maneira:
...
##<a name="bkmk_improvetags"></a> Aprimorar a lista de marcas

Como acontece com o modo de exibição *Speakers*, o modo de exibição *Tags* é legível, mas os links são pequenos e difíceis de tocar em um dispositivo móvel. Você pode corrigir o modo de exibição *Tags* da mesma maneira que você corrige o modo de exibição *Speakers* se você usar as alterações de código descritas anteriormente, mas com a seguinte sintaxe de método `Html.ActionLink` em *Views\\Home\\AllTags.cshtml*:
...
##<a name="bkmk_improvedates"></a> Aprimorar a lista de datas

Você pode aprimorar o modo de exibição *Dates* do mesmo modo que você aprimorou os modos de exibição *Speakers* e
*Marcas* se você usar as alterações de código descritas anteriormente, mas com a seguinte sintaxe de método `Html.ActionLink` em *Views\\Home\\AllDates.cshtml*:
...
Você pode aprimorar ainda mais o modo de exibição de *Dates* organizando os valores data-hora por data. Isso pode ser feito com o estilo
[painéis][] do Bootstrap. Substitua o conteúdo do arquivo *Views\\Home\\AllDates.cshtml* pelo código a seguir:
...
##<a name="bkmk_improvesessionstable"></a> Aprimorar o modo de exibição SessionsTable
...
*Views\\Home\SessionsTable.cshtml* e, em seguida, substitua o conteúdo do arquivo pelo seguinte código:
...
-   usa o [grupo personalizado de lista vinculada][] do Bootstrap
    para formatar as informações da sessão verticalmente, para que todas essas informações possam ser lidas em um navegador móvel (usando classes como lista/grupo/item/texto)
...
##<a name="bkmk_improvesessionbycode"></a> Aprimorar o modo de exibição SessionByCode

Finalmente, você corrigirá o modo de exibição *SessionByCode*, para torná-lo acessível a dispositivos móveis.
...
Escolha o link **Construindo um aplicativo de página única com o ASP.NET e o
AngularJS**.
...
A nova marcação usa o estilo de painéis do Bootstrap para aprimorar o modo de exibição no dispositivo móvel. 
...
-   [9 princípios básicos de design da Web com resposta ágil](http://blog.froont.com/9-basic-principles-of-responsive-web-design/)
-   [Bootstrap][BootstrapSite]
-   [Blog oficial do Bootstrap][]
-   [Tutorial do Twitter Bootstrap do Tutorial Republic][]
-   [O Playground do Bootstrap][]
...
[Framework do CSS Bootstrap]: #bkmk_bootstrap
...
[Aprimorar a lista de alto-falantes]: #bkmk_Improvespeakerslist
[Aprimorar a lista de marcas]: #bkmk_improvetags
[Aprimorar a lista de datas]: #bkmk_improvedates
[Aprimorar o modo de exibição SessionsTable]: #bkmk_improvesessionstable
[Aprimorar o modo de exibição SessionByCode]: #bkmk_improvesessionbycode
