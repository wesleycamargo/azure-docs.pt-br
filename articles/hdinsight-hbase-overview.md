<properties 
	pageTitle="Uma visão geral do HBase no HDInsight | Azure" 
	description="Uma introdução ao HBase no HDInsight, casos de uso e uma comparação com outras soluções de banco de dados." 
	services="hdinsight" 
	documentationCenter="" 
	authors="bradsev" 
	manager="paulettm" 
...
O HBase é um banco de dados NoSQL de código aberto Apache baseado no Hadoop que fornece acesso aleatório e uma sólida consistência para grandes quantidades de dados não estruturados e semiestruturados. Ele foi criado com base no BigTable do Google e é um banco de dados de coluna agrupado em famílias. Os dados são armazenados nas linhas de uma tabela e os dados contidos nas linhas são agrupados por famílias de colunas. O HBase é um banco de dados sem esquema, no aspecto em que nem as colunas nem os tipos de dados armazenados nelas precisam ser definidos antes da utilização. O código-fonte aberto foi lançado pela primeira vez por Mike Cafarella em 2007. Ele é dimensionado linearmente para manipular petabytes de dados em milhares de nós. Ele pode fazer uso da redundância de dados, do processamento em lote e de outros recursos que são fornecidos por aplicativos distribuídos no ecossistema do Hadoop.
...
Dados podem ser gerenciados no HBase usando os comandos  `create` `get`, `put`, e `scan` do shell do HBase. Os dados são gravados no banco de dados usando `put` e lidos usando os comandos `get`. O comando `scan` é utilizado para obter dados de múltiplas linhas em uma tabela. Os dados também podem ser gerenciados utilizando a API C# do HBase, que oferece uma biblioteca de cliente sobre a API REST do HBase. Um banco de dados do HBase também pode ser consultado usando o Hive. Para ver uma introdução a esses modelos de programação, consulte [Introdução ao HBase com o Hadoop no HDInsight][hbase-get-started]. Coprocessadores também estão disponíveis para permitir o processamento de dados nos nós que hospedam o banco de dados.
...
[Utilizar o Maven para criar aplicativos Java que usam o HBase com o HDInsight (Hadoop)][hbase-build-java-maven]

[SDK do HBase com C#][cs-hbase-sdk]
...
[hbase-get-started]: http://azure.microsoft.com/pt-br/documentation/articles/hdinsight-hbase-get-started/
