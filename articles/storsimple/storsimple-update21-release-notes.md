<properties 
   pageTitle="Notas de versão da Atualização 2.1 para o StorSimple 8000 Serie | Microsoft Azure"
   description="Descreve os novos recursos, problemas e soluções alternativas da Atualização 2.1 para o StorSimple 8000 Series."
   services="storsimple"
...
# Notas de versão da Atualização 2.1 para o StorSimple 8000 Series  

## Visão geral

As notas de versão a seguir descrevem os novos recursos e identificam os problemas críticos abertos da Atualização 2.1 para o StorSimple 8000 Series. Elas também contêm uma lista das atualizações de software do StorSimple incluídas nesta versão.

A Atualização 2.1 pode ser aplicada a qualquer dispositivo do StorSimple que executa a Versão (GA) ou a Atualização 0.1 à 2. A versão do dispositivo associada à Atualização 2.1 é 6.3.9600.17705.

Examine as informações contidas nas notas de versão antes de implantar a atualização em sua solução do StorSimple.

>[AZURE.IMPORTANT]
> 
> - A Atualização 2.1 tem somente as atualizações de software. São necessárias cerca de 1,5 a 2 horas para instalar essa atualização. 
...
## Novidades na Atualização 2.1

Os seguintes aprimoramentos importantes foram feitos na Atualização 2.1.
...
- **Aprimoramentos de desempenho de instantâneo** – A Atualização 2.1 melhorou o tempo para processar uma nuvem de instantâneo em determinados cenários, nos quais grandes volumes estão sendo usados e há pouca, ou nenhuma, rotatividade de dados. Um cenário que se beneficiaria desse aprimoramento seria o de volumes de arquivamento.


- **Proteção da coleta do pacote Suporte** – Houve melhorias na maneira como o pacote Suporte é coletado e carregado nesta versão.


- **Melhorias de confiabilidade de Atualização** – Esta versão tem correções de bugs que resultam em uma confiabilidade de Atualização aprimorada.

  
 

## Problemas corrigidos na Atualização 2.1

A tabela a seguir fornece um resumo dos problemas que foram corrigidos nas Atualizações 2.1.
...
| 7 | WMI (Instrumentação de Gerenciamento do Windows) | Nas versões anteriores do software, havia várias instâncias de falha de proxy da web com a exceção “Falha ao carregar o provedor <ManagementException>”. Esse bug foi atribuído a um vazamento de memória da WMI, e agora está corrigido. | Sim | Não |
...
## Problemas conhecidos na Atualização 2.1
...
| 17 | Volumes afixados localmente | O trabalho de restauração expõe o backup do instantâneo temporário no Catálogo de Backup, mas apenas pelo tempo que durar o trabalho de restauração. Além disso, ele expõe um grupo de discos virtuais com o prefixo **tmpCollection** na página **Políticas de Backup**, mas apenas enquanto durar o trabalho de restauração. | Isso poderá ocorrer se o trabalho de restauração tiver apenas volumes fixados localmente ou uma combinação de volumes fixados e em camadas. Se o trabalho de restauração incluir apenas volumes em camadas, esse comportamento não ocorrerá. Nenhuma intervenção do usuário é necessária. | Sim | Não |
...
## Atualizações de controlador e firmware na Atualização 2.1
...
Para obter mais informações sobre como instalar o driver Storport, Spaceport e as atualizações de firmware de disco, confira [instalar a Atualização 2.1](storsimple-install-update-21.md) em seu dispositivo StorSimple.

 
## Atualizações do dispositivo virtual na Atualização 2.1

Esta atualização não pode ser aplicada ao dispositivo virtual. Novos dispositivos virtuais precisarão ser criados.

## Próxima etapa

Saiba como [instalar a Atualização 2.1](storsimple-install-update-21.md) no seu dispositivo StorSimple.
