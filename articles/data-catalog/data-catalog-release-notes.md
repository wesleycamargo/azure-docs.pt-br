...
Registrando e se conectando ao banco de dados Oracle. Ao se conectar com as fontes de dados do banco de dados Oracle, os usuários devem ter instalado os drivers corretos do Oracle que correspondem ao número de bits (32 bits ou 64 bits) do software que está sendo usado.
...
-	Ao se conectar a fontes de dados Oracle usando o Excel em um computador executando a versão de 32 bits do Microsoft Office, inclusive no Windows de 64 bits, os drivers do Oracle de 32 bits serão usados
-	Ao se conectar a fontes de dados Oracle usando o Excel em um computador executando a versão de 64 bits do Microsoft Office, os drivers do Oracle de 64 bits serão usados.

### Registrando e se conectando ao SQL Server Reporting Services

Suporte para fontes de dados do SQL Server Reporting Services (SSRS) na versão de teste inicial do Catálogo de Dados do Azure está limitado somente aos servidores de modo nativo. Suporte para o SSRS no modo SharePoint será adicionado em uma versão posterior.
...
Ao registrar tabelas e exibições que contêm BLOB (Objeto Binário Grande) e colunas de UDT (Tipo de Dados Definido pelo Usuário) e selecionando incluir visualização dos ativos de dados, essas colunas não serão incluídas na visualização.
...
Para localizar o arquivo RegistrationTool.exe.config, inicie a ferramenta de registro e, em seguida, abra o utilitário Gerenciador de Tarefas do Windows. Na guia Detalhes do Gerenciador de tarefas, clique em RegistrationTool.exe e escolha Abrir local do arquivo no menu pop-up.
