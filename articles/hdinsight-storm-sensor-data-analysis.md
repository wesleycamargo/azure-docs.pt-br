...
> [AZURE.NOTE] Java, o JDK, Maven e Git também estão disponíveis por meio do gerenciador de pacotes [Chocolatey NuGet][Chocolatey NuGet].

## Criar o painel

O painel é usado para exibir informações do sensor em tempo quase real. Nesse caso, o painel está em um aplicativo ASP.NET hospedado no site da Web do Azure. O principal objetivo do aplicativo é servir como um hub [SignalR][SignalR] que recebe informações da topologia do Storm conforme processa mensagens.

O site da Web também contém um arquivo index.html estático, que também se conecta ao SignalR e usa D3.js para plotar os dados transmitidos pela topologia do Storm.

> [WACOM.NOTE] Embora você também possa usar WebSockets bruto, em vez de SignalR, WebSockets não fornece um mecanismo de escala integrado se precisar colocá-lo em escala para o site da Web. O SignalR pode ser colocado em escala usando o Barramento de Serviço do Azure (<http://www.asp.net/signalr/overview/performance/scaleout-with-windows-azure-service-bus>).
>
> Para um exemplo de uso da topologia do Storm para comunicar-se com um site da Web Python usando WebSockets brutos, consulte o projeto [Visualização do Storm Tweet Sentiment D3][Visualização do Storm Tweet Sentiment D3].
...
2.  Na janela **Novo Projeto ASP.NET**, selecione o modelo de aplicativo **Vazio**. Na seção **Microsoft Azure**, selecione **Host na nuvem** e **site da Web**. Por fim, clique em **OK**.

    > [AZURE.NOTE] Se solicitado, entre com a sua assinatura do Azure.

3.  No diálogo **Configurar o Microsoft Azure Site**, insira um **Nome do site** e **Região** para seu site da Web e clique em **OK**. Isso criará o site da Web do Azure que hospedará o painel.

4.  No **Gerenciador de Soluções**, clique com o botão direito no projeto e selecione **Adicionar | Classe do Hub SignalR (v2)**. Atribua à classe o nome **DashHub.cs** e adicione-a ao projeto. Isso conterá o hub do SignalR que é usado para comunicar dados entre o HDInsight e a página da web do painel.

    > [AZURE.NOTE] Se você estiver usando o Visual Studio 2012, o modelo da **Classe de Hub do SignalR (v2)** não estará disponível. É possível adicionar uma **Classe** simples chamada DashHub em vez disso. Você também precisará instalar manualmente o pacote SignalR abrindo **Ferramentas | Gerenciador de Pacote de Biblioteca | Console do Gerenciador de Pacote** e executando o seguinte comando:
    >
...
6.  No **Gerenciador de Soluções**, clique com o botão direito no projeto e selecione **Adicionar | Classe de Inicialização do OWIN**. Nomeie a nova classe como **Startup.cs**.
...
7.  Substitua os conteúdos do **Startup.cs** pelo seguinte.
...
9.  No **Gerenciador de Soluções**, clique com o botão direito em **index.html** e selecione **Definir como Página Inicial**.
...
10. Substitua o código no arquivo **index.html** pelo seguinte.
    ...
    > [AZURE.NOTE] Uma versão posterior dos scripts SignalR pode ser instalada pelo gerenciador de pacote. Verifique se as referências de script abaixo correspondem às versões dos arquivos de script no projeto (elas serão diferentes e você tiver adicionado SignalR usando NuGet, em vez de adicionar um hub.)
...
15. Quando o site tiver sido publicado, uma página da Web se abre exibindo uma linha do tempo móvel.
...
1.  No [Portal do Azure][Portal do Azure], selecione **NOVO | Barramento de Serviço | Hub de Eventos | Criação Personalizada**.
...
5.  Selecione **Configurar** e crie duas novas políticas de acesso usando as seguintes informações.
    ...
    Depois de criar permissões, selecione o ícone **Salvar** na parte inferior da página. Isso cria as políticas de acesso compartilhado que serão usadas para enviar mensagens a hub e enviar mensagens a ele.
...
1.  No Visual Studio, crie um novo projeto da **Área de Trabalho do Windows** e selecione o modelo de projeto do **Aplicativo de Console**. Nomeie o projeto **SendEvents** e depois clique em **OK**.

2.  No **Gerenciador de Soluções**, clique com o botão direito em **SendEvents** e selecione **Gerenciar Pacotes NuGet**.
...
3.  Em **Gerenciar Pacotes NuGet**, pesquise e instale os seguintes pacotes.
    ...
    Depois da instalação dos pacotes, **Feche** o gerenciador de pacotes.

4.  Substitua os conteúdos do **Program.cs** pelo seguinte.
...
5.  No arquivo **Program.cs**, defina o valor das seguintes variáveis no início do arquivo para os valores correspondentes recuperados do seu Hub de Eventos no Portal de Gerenciamento do Azure.
    ...
    | Defina isto...         | Para fazer isto...                                                    |
    |------------------------|-----------------------------------------------------------------------|
    | eventHubName           | Nome do seu hub de eventos. Por exemplo, **temperature**.             |
    | eventHubNamespace      | O namespace do seu hub de eventos. Por exemplo, **sensors-ns**.       |
    | sharedAccessPolicyName | A política criada com acesso de envio. Por exemplo, **dispositivos**. |
    | sharedAccessPolicyKey  | A chave para a política com acesso de envio.                          |

6.  No **Gerenciador de Soluções**, clique com o botão direito em **SendEvents** e **Adicionar | Classe**. Nomeie a nova classe como **Event.cs**. Isso descreverá a mensagem enviada ao Hub de Eventos.

7.  Substitua os conteúdos do **Event.cs** pelo seguinte.
    ...
    Essa classe descreve os dados que estamos enviando - o TimeStamp, um DeviceID e um valor de Temperature.
...
Para a topologia em execução no Storm comunicar-se diretamente com o HBase, você deve provisionar ambos os servidores na Rede Virtual do Azure.
...
5.  Insira 1 para o número de **Nós de dados** a usar para esse cluster. Para **Região/Rede virtual**, selecione a Rede virtual do Azure criada anteriormente. Para **Sub-redes da Rede Virtual**, selecione **Sub-rede-2**.
...
7.  Para **Conta de Armazenamento**, selecione **Criar Novo Armazenamento** ou selecione uma conta de armazenamento existente. Selecione ou insira o **Nome da conta** e o **Contêiner padrão** a serem utilizados. Clique no ícone de seleção na parte inferior esquerda para criar o cluster Storm.
    ...
    > [WACOM.NOTE] Você deve usar um contêiner diferente do usado para o cluster Storm.
...
Depois de criar a área de trabalho remota, selecione **Conectar** na parte inferior da página. Siga os avisos pra conectar-se ao cluster.
...
> [WACOM.NOTE] As etapas nesta seção devem ser realizadas no ambiente de desenvolvimento local.
...
1.  Use a área de trabalho remota para conectar-se ao seu cluster Storm, então copie o arquivo **%STORM\_HOME%\\examples\\eventhubspout\\eventhubs-storm-spout-0.9-jar-with-dependencies.jar** para seu ambiente de desenvolvimento local. Isso contém o **events-storm-spout**.
...
Para enviar mensagens ao painel ASP.NET, use o [SDK do cliente SignalR para Java][SDK do cliente SignalR para Java].
...
4.  Altere os diretórios para o diretório **java-client\\signalr-client-sdk** e compile o projeto para um arquivo JAR usando os seguintes comandos.
    ...
    > [WACOM.NOTE] Se você receber um erro dizendo que a dependência **gson** não pode ser baixada, remova as seguintes linhas do arquivo **java-client\\signalr-client-sdk\\pom.xml**.
    >
    > ``` <repositories>
    > <repository>
    > <id>central</id>
...
3.  Use o seguinte comando Maven para criar o scaffolding básico para seu aplicativo.
    ...
    -   Crie um novo diretório usando o *artifactId* especificado. Neste caso, **Temperature**.
...
A seguir, modifique o **pom.xml** para fazer referência às dependências para esse projeto, bem como os plug-ins do Maven a usar ao criar e empacotar.
...
2.  Ao fim do arquivo **pom.xml**, logo antes da entrada **\</project\>**, adicione o seguinte.
    ...
    -   Inclua o arquivo de recurso **/conf/Config.properties**. Esse arquivo será criado mais tarde, mas conterá informações de configuração para conectar-se ao Hub de Eventos do Azure.
    ...
    -   Use o **maven-shade-plugin** para criar um uberjar ou fat jar, que contém esse projeto e as dependências requeridas.
    -   Use o **exec-maven-plugin**, que lhe permite executar o aplicativo localmente sem um cluster Hadoop.

### Adicionar arquivos de configuração

**eventhubs-storm-spout** lê informações de configuração de um arquivo **Config.properties**. Isso diz a que hub de eventos se conectar. Embora você possa especificar um arquivo de configuração ao iniciar a topologia em um cluster, incluir um no projeto fornece uma configuração padrão conhecida.

1.  No diretório **Temperature**, crie um novo diretório com nome **conf**.

2.  No diretório **conf**, crie dois novos arquivo:
...
3.  Use o seguinte como conteúdo para o arquivo **Config.properties**.
    ...
    Substitua a **senha** pela chave para a política do **storm** criada anteriormente no Hub de Eventos. Substitua **namespace** pelo namespace do seu Hub de Eventos.
...
5.  No arquivo **hbase-site.xml**, substitua o valor do **sufixo** para as entradas zookeeper pelo sufixo DNS recuperado anteriormente para o HBase. Por exemplo, **zookeeper0.mycluster.b4.internal.cloudapp.net, zookeeper1.mycluster.b4.internal.cloudapp.net, zookeeper2.mycluster.b4.internal.cloudapp.net**.
...
1.  No diretório **\\temperaturemonitor\\src\\main\\java\\com\\microsoft\\examples**, crie um novo diretório chamado **helpers**.

2.  No diretório **helpers**, crie dois novos arquivo:
...
2.  No diretório **bolts**, crie dois novos arquivo:

    -   **ParserBolt.java** - analisa a mensagem recebida do Hub de Eventos em campos individuais, então emite dois fluxos
    -   **DashboardBolt.java** - registre em log informações para o painel da Web via SignalR
...
4.  Use o seguinte como conteúdo do arquivo **DashboardBolt.java**.
    ...
    Substitua `http://yourwebsiteaddress` pelo endereço do site da Web do Azure em que você publicou o painel anteriormente. Por exemplo, http://mydashboard.azurewebsites.net.
...
2.  Abra o arquivo **Temperature.java** e use o seguinte como conteúdo.
    ...
    > [AZURE.NOTE] Observe que as linhas para **HBaseBolt** têm os comentários removidos. Isso ocorre porque a próxima etapa é executar a topologia localmente. Uma vez que o HBaseBolt conversa diretamente com o HBase, isso retornará erros se estiver habilitado. A menos que você tenha configurado uma rede virtual com um servidor DNS e unido sua máquina local à rede virtual também.

### Testar a topologia localmente

Para compilar e testar o arquivo na máquina de desenvolvimento, use as seguintes etapas.
...
No seu ambiente de desenvolvimento, use as seguintes etapas para executar a topologia Temperature no cluster Storm do HDInsight.
...
2.  Na máquina de desenvolvimento loca, inicie o aplicativo .NET **SendEvents** de modo que haja alguns eventos a ler.

3.  Conecte-se ao seu cluster Storm do HDInsight Storm usando a área de trabalho remota e copie o arquivo **TemperatureMonitor-1.0-SNAPSHOT.jar** para o diretório **c:\\apps\\dist\\storm\<version number\>**.
...
-   Para obter mais informações sobre o Apache Storm, consulte <https://storm.incubator.apache.org/>

-   Para obter mais informações sobre HBase com HDInsight, consulte a [Visão geral do HBase com HDInsight][Visão geral do HBase com HDInsight]

-   Para obter mais informações sobre SignalR, consulte [ASP.NET SignalR][ASP.NET SignalR]

-   Para obter mais informações sobre D3.js, consulte [D3.js - Documentos Conduzidos por Dados][D3.js - Documentos Conduzidos por Dados]

-   Para informações sobre a criação de topologias no .NET, consulte [Desenvolver aplicativos de dados de streaming com SCP.NET e C# em Storm no HDInsight][Desenvolver aplicativos de dados de streaming com SCP.NET e C# em Storm no HDInsight]

  [SDK do Microsoft Azure para .NET]: http://azure.microsoft.com/pt-br/downloads/archive-net-downloads/
  ...
  [Visualização do Storm Tweet Sentiment D3]: https://github.com/P7h/StormTweetsSentimentD3Viz
  [Portal do Azure]: https://manage.windowsazure.com
  [Portal de Gerenciamento do Azure]: https://manage.windowsazure.com/
  [SDK do cliente SignalR para Java]: https://github.com/SignalR/java-client
  [Visão geral do HBase com HDInsight]: http://azure.microsoft.com/pt-br/documentation/articles/hdinsight-hbase-overview/
  [ASP.NET SignalR]: http://signalr.net/
  [D3.js - Documentos Conduzidos por Dados]: http://d3js.org/
  [Desenvolver aplicativos de dados de streaming com SCP.NET e C# em Storm no HDInsight]: /pt-br/documentation/articles/hdinsight-hadoop-storm-scpdotnet-csharp-develop-streaming-data-processing-application/
