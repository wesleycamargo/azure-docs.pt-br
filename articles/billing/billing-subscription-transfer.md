...
* **Tudo é transferido? Incluindo grupos de recursos, VMs, discos e outros serviços em execução?**
  ...
  Sim, todos os seus recursos, como VMs, discos, e sites são transferidos para o novo proprietário. No entanto, todas as [funções de administrador](billing-add-change-azure-subscription-administrator.md) e políticas [RBAC (Controle de Acesso Baseado em Função)](../active-directory/role-based-access-control-configure.md) que você configurou não são transferidas. 
...
* **Como faço para migrar dados e serviços da minha assinatura do Azure para uma nova assinatura?**
  ...
  Se não for possível transferir a propriedade, você pode migrar seus recursos manualmente. Consulte [Mover recursos para um novo grupo de recursos ou uma nova assinatura](../azure-resource-manager/resource-group-move-resources.md).
