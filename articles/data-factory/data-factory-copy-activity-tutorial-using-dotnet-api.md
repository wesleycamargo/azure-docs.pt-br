...
1. Adicione o código a seguir, que **cria e ativa um pipeline** no método **Main**. Essa pipeline tem uma **CopyActivity** que usa **BlobSource** como fonte e **BlobSink** como coletor.
...
2. Adicione o seguinte código ao método **Principal** para obter o status de uma fatia de dados do conjunto de dados de saída. Há apenas uma fatia esperada neste exemplo.   
...
3. Adicione o seguinte código para obter detalhes de execução de uma fatia de dados ao método **Principal** .
           ...
           Console.ReadKey();
4. Adicione o método auxiliar a seguir usado pelo método **Main** na classe **Program**.  
...
5. No Gerenciador de Soluções, expanda o projeto (**DataFactoryAPITestApp**), clique com o botão direito do mouse em **Referências** e clique em **Adicionar Referência**. Marque a caixa de seleção do assembly "**System.Configuration**" e clique em **OK**. 
6. Compile o aplicativo de console. Clique no menu **Compilar** e clique em **Solução de Compilação**. 
7. Confirme se há pelo menos um arquivo no contêiner **adftutorial** no seu armazenamento de BLOBs do Azure. Caso contrário, crie o arquivo **Emp.txt** no bloco de notas com o seguinte conteúdo e carregue-o no contêiner adftutorial.
   
       John, Doe
       Jane, Doe
8. Execute o exemplo, clicando em **Depurar** -> **Iniciar Depuração** no menu. Quando você vir **Obter detalhes de execução de uma fatia de dados**, aguarde alguns minutos e pressione **ENTER**. 
9. Use o Portal do Azure para verificar se a data factory **APITutorialFactory** foi criada com os seguintes artefatos: 
   * Serviço vinculado: **LinkedService_AzureStorage** 
   * Conjunto de dados: **DatasetBlobSource** e **DatasetBlobDestination**.
   * Pipeline: **PipelineBlobSample** 
10. Verifique se os dois registros de funcionários são criados na tabela "**emp**" do banco de dados SQL do Azure especificado.
