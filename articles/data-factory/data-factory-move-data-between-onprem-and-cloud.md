...
Este artigo fornece uma visão geral da integração de dados entre armazenamentos de dados locais e armazenamentos de dados na nuvem e o processamento de nuvem usando o Data Factory. Baseia-se no artigo [Atividades de movimentação de dados](data-factory-data-movement-activities.md) e outros artigos de conceitos principais de data factory. Você deve estar familiarizado com conceitos de data factory como pipelines, atividades, conjuntos de dados e a atividade de cópia.

## Como o Azure Data Factory funciona com o Gateway de Gerenciamento de Dados 
E se você pudesse mover dados continuamente de e para o local para a nuvem? O Azure Data Factory (um agente que você instala localmente) fornece os recursos de gerenciamento de gateway de dados de que você precisa, incluindo um aplicativo para definir credenciais de gateway.

## O que o gateway de dados faz
...
1.	As versões de **Sistema Operacional** com suporte são Windows 7, Windows 8/8.1, Windows 10, Windows Server 2008 R2, Windows Server 2012, Windows Server 2012 R2. Instalação do Gateway de Gerenciamento de Dados em um controlador de domínio não tem suporte atualmente.
...
### Instalação
O Gateway de Gerenciamento de Dados pode ser instalado baixando-se um pacote de instalação MSI do [Centro de Download da Microsoft](https://www.microsoft.com/download/details.aspx?id=39717). O MSI também pode ser usado para atualizar o Gateway de Gerenciamento de Dados existente para a versão mais recente, com todas as configurações preservadas. Você pode encontrar o link para o pacote MSI do Portal do Azure seguindo a explicação passo a passo abaixo.
...
Por exemplo, para copiar de **um repositório de dados local para um coletor do Banco de Dados SQL do Azure ou um coletor do SQL Data Warehouse do Azure**, você precisa permitir a comunicação **TCP** de saída na porta **1433** para o Firewall do Windows e o firewall corporativo e definir as configurações de firewall do SQL Server do Azure para adicionar o endereço IP do computador do gateway à lista de endereços IP permitidos.
...
3.	No **Editor JSON**, faça o seguinte:
	...
	4. Se você estiver usando a autenticação SQL:
		...
		As credenciais serão **criptografadas** usando um certificado que o serviço do Data Factory possui. Em vez disso, se você quiser usar o certificado associado ao Gateway de Gerenciamento de Dados, confira [Definir credenciais com segurança](#set-credentials-and-security).
...
5. Clique no botão **Criptografar** na barra de comandos para iniciar o aplicativo de clique único **Gerenciador de Credenciais**. Você verá a caixa de diálogo **Definindo Credenciais**. ![Caixa de diálogo de Configurando Credenciais](./media/data-factory-move-data-between-onprem-and-cloud/setting-credentials-dialog.png)
...
4. No Azure PowerShell, alterne para a pasta **C:\\Program Files\\Microsoft Data Management Gateway\\1.0\\PowerShellScript** e execute o script **RegisterGateway.ps1** associado à variável local **$Key**, conforme mostrado no comando a seguir, para registrar o agente cliente instalado no computador com o gateway lógico criado anteriormente.
