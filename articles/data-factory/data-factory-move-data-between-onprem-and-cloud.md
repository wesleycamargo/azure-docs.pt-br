...
## <a name="walkthrough:-copy-on-prem-data-to-cloud"></a>Passo a passo: copiar os dados locais para a nuvem
...
1. Depois de fazer logon [portal do Azure](https://portal.azure.com), clique em **NOVO** no canto inferior esquerdo, selecione **Análises de dados** na folha **Criar** e clique em **Data Factory** na folha ** Análises de dados** folha.
...
2. Na folha **Nova data factory** :
   ...
   1. Insira **ADFTutorialOnPremDF** for the **nome**.
...
3. Observe que a opção **Adicionar ao Quadro Inicial** está marcada na folha **Novo data factory**.
...
4. Na folha **Novo data factory**, clique em **Criar**.
...
5. Procure as notificações do processo de criação clicando no botão **Notificações** na barra de título, conforme mostrado na imagem a seguir. Clique novamente para fechar a janela de notificações. 
   
    ![Hub de NOTIFICAÇÕES](./media/data-factory-move-data-between-onprem-and-cloud/OnPremNotificationsHub.png)
6. Depois que a criação estiver concluída, você verá a folha **Data Factory** como mostrado na imagem abaixo:
...
2. Em Editor do Data Factory, clique em **...(reticências)** na barra de ferramentas, e em seguida, clique em **Novo gateway de dados**. 
   
   ![Novo gateway de dados na barra de ferramentas](./media/data-factory-move-data-between-onprem-and-cloud/NewDataGateway.png)
3. Na folha **Criar**, digite **adftutorialgateway** como **nome** e clique em **OK**.    
   
    ![Folha Criar Gateway](./media/data-factory-move-data-between-onprem-and-cloud/OnPremCreateGatewayBlade.png)
4. Na folha **Configurar**, clique em **Instalar diretamente neste computador**. Essa ação baixará o pacote de instalação para o gateway, instalará, configurará e registrará o gateway no computador.  
...
6. Aguarde:
   ...
   1. **Nome do gateway** está definido como **adftutorialgateway**.
   2. A barra de status inferior exibe a mensagem **Conectado ao serviço de nuvem** junto com uma **marca de seleção verde**.
      
      Na guia **início**, também é possível fazer as seguintes operações:      - **Registrar** um gateway com uma chave do portal do Azure usando o botão de registro. 
...
10. Você deve consultar **adftutorialgateway** em **Gateways de Dados** no modo de exibição de árvore à esquerda.  Se você clicar nisso, verá o JSON associado. 
...
2. No **Editor JSON**, siga estas etapas: 
   ...
   2. Se você está usando a Autenticação do Windows:
      ...
      2. Especifique o nome de usuário e senha para as propriedades **userName** e **password**.  
   ...
   3. Se você estiver usando a autenticação SQL:
...
3. Clique em **Implantar** na barra de comandos para implantar o serviço vinculado do SQL Server. 
...
1. Insira algum exemplo na tabela: 
...
### <a name="create-input-table"></a>Criar tabela de entrada
...
3. Clique em **Implantar** na barra de comandos para implantar o conjunto de dados (a tabela é um conjunto de dados retangular). Confirme que você vê uma mensagem na barra de título que diz **TABELA IMPLANTADA COM ÊXITO**. 

### <a name="create-output-table"></a>Criar tabela de saída
...
1. Clique em **Implantar** na barra de comandos para implantar o conjunto de dados (a tabela é um conjunto de dados retangular). Confirme que você vê uma mensagem na barra de título que diz **TABELA IMPLANTADA COM ÊXITO**.
...
3. Substitua o JSON no painel direito pelo texto a seguir: 
...
4. Clique em **Implantar** na barra de comandos para implantar o conjunto de dados (a tabela é um conjunto de dados retangular). Confirme que você vê uma mensagem na barra de título que diz **PIPELINE IMPLANTADO COM ÊXITO**.  
...
13. (opcional) Clique em **Pipelines**, clique em **ADFTutorialOnPremDF** e execute uma consulta drill-through nas tabelas de entrada (**Consumed**) ou nas tabelas de saída (**Produced**).
14. Use ferramentas como o **Explorador de Armazenamento do Azure** para verificar a saída.
