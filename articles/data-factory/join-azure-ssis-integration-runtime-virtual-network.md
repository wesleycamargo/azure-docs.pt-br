...
Se o catálogo do SSIS estiver hospedado no Banco de Dados SQL do Azure com pontos de extremidade de serviço de rede virtual/Instância Gerenciada (versão prévia), você poderá ingressar o IR do Azure-SSIS para:
...
A rede virtual pode ser implantada através do modelo de implantação clássico ou do modelo de implantação do Azure Resource Manager. Se você estiver planejando ingressar o IR do Azure-SSIS na *mesma rede virtual* que já está ingressada na Instância Gerenciada (versão prévia), verifique se o IR do Azure-SSIS está em uma *sub-rede diferente* daquela usada para a Instância Gerenciada (versão prévia).   
...
O IR do Azure-SSIS precisa criar determinados recursos de rede no mesmo grupo de recursos como rede virtual, incluindo um balanceador de carga do Azure, um endereço IP público do Azure e um grupo de segurança de trabalho de rede.
...
5. Na página **Configurações Gerais** da janela **Configuração do Integration Runtime**, selecione **Avançar**. 
...
8. Agora, você pode iniciar o IR usando o botão **Iniciar** na coluna **Ações** para o IR do Azure-SSIS. Demora aproximadamente de 20 a 30 minutos para iniciar um IR do Azure-SSIS. 
