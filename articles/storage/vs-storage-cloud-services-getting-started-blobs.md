...
- Confira [Como usar o Armazenamento de Blob do .NET](storage-dotnet-how-to-use-blobs.md) para obter mais informações sobre como manipular blobs programaticamente.
...
**OBSERVAÇÃO:** algumas APIs que executam chamadas para o armazenamento do Azure no ASP.NET são assíncronas. Confira [Programação assíncrona com Async e Await](http://msdn.microsoft.com/library/hh191443.aspx) para saber mais. O código a seguir pressupõe que você está usando os métodos de programação assíncrona.

Para criar um contêiner na sua conta de armazenamento, tudo o que você precisa fazer é adicionar uma chamada para `CreateIfNotExistsAsync` como no código a seguir:
...
Quando você chama no contêiner `ListBlobs` (como no exemplo anterior), a coleção retornada conterá objetos `CloudBlobDirectory` e `CloudBlockBlob` que representam os diretórios e os blobs contidos no nível superior. Esta seria a saída resultante:
