...
Há duas versões do AzCopy que podem ser baixadas. AzCopy no Windows oferece opções de linha de comando de estilo do Windows. [AzCopy no Linux](storage-use-azcopy-linux.md) tem como alvo plataformas Linux oferta estilo POSIX opções de linha de comando. Este artigo aborda o AzCopy no Windows.

## <a name="download-and-install-azcopy-on-windows"></a>Baixar e instalar o AzCopy no Windows

### <a name="latest-version-v81"></a>Versão mais recente (8.1)
Baixe a [versão mais recente do AzCopy no Windows](https://aka.ms/downloadazcopy).

#### <a name="azcopy-on-windows-81-release-notes"></a>Notas de versão do AzCopy no Windows 8.1
- Não há suporte para a versão mais recente do serviço de tabela. Se você usar o recurso de exportação de tabela, baixe a versão 7.3 AzCopy.
- Criado com o .NET Core 2.1 e todas as dependências do .NET Core agora são empacotadas na instalação.
- Adicionado suporte de autenticação OAuth. Use ```azcopy login``` para fazer logon usando o Active Directory do Azure.

### <a name="azcopy-with-table-support-v73"></a>Azcopy com o suporte de tabela (v 7.3)
Baixe o [AzCopy 7.3 com suporte de tabela](https://aka.ms/downloadazcopynet).

### <a name="post-installation-step"></a>Etapas pós-instalação
...
Quando você copia um blob em uma conta de armazenamento, uma operação de [cópia no servidor](https://blogs.msdn.com/b/windowsazurestorage/archive/2012/06/12/introducing-asynchronous-cross-account-copy-blob.aspx) é realizada.
...
Quando você copia um blob entre contas de armazenamento, uma operação de [cópia no servidor](https://blogs.msdn.com/b/windowsazurestorage/archive/2012/06/12/introducing-asynchronous-cross-account-copy-blob.aspx) é realizada.
...
`/SyncCopy` pode gerar custo de saída adicionais, comparado à cópia assíncrona, a abordagem recomendada é usar essa opção em uma VM do Azure que está na mesma região que sua conta de armazenamento de origem para evitar o custo de saída.
...
Se a origem especificada for um compartilhamento de arquivos do Azure, você deverá especificar o nome exato do arquivo, (*por exemplo*, `abc.txt`) para baixar um único arquivo ou especificar a opção `/S` para baixar todos os arquivos do compartilhamento de maneira recursiva. A tentativa de especificar um padrão de arquivo e a opção `/S` simultaneamente resulta em um erro.
...
## <a name="upload-files-to-an-azure-file-share"></a>Carregar arquivos para um compartilhamento de arquivos do Azure
...
Vamos examinar vários modos de copiar arquivos em um compartilhamento de arquivos do Azure usando AzCopy.
...
```
Quando você copia um arquivo de um blob para um compartilhamento de arquivos, uma operação [cópia do servidor](https://blogs.msdn.com/b/windowsazurestorage/archive/2012/06/12/introducing-asynchronous-cross-account-copy-blob.aspx) é executada.
...
O arquivo de dados JSON gerado segue o formato de carga para metadados mínimos. Para obter detalhes sobre esse formado de carga, confira [Formato de carga para operações do serviço Tabela](https://msdn.microsoft.com/library/azure/dn535600.aspx).
...
Suponha que um contêiner de Blob contém o seguinte: Um arquivo JSON que representa uma Tabela do Azure, e seu arquivo de manifesto.
...
AzCopy determina o tipo de conteúdo de um blob com base em um arquivo JSON que armazena o tipo de conteúdo para o mapeamento de extensão de arquivo. Esse arquivo JSON é chamado AzCopyConfig.json e está localizado no diretório AzCopy. Se você tiver um tipo de arquivo que não está na lista, você pode acrescentar o mapeamento para o arquivo JSON:
...
* Para obter ajuda detalhada de linha de comando para o AzCopy: `AzCopy /?`
* Para obter ajuda detalhada sobre qualquer parâmetro do AzCopy: `AzCopy /?:SourceKey`
...
**Aplicável a:** Blobs, arquivos e tabelas
...
Calcula um hash MD5 para dados baixados e certifica que o hash MD5 armazenado no blob, ou a propriedade Content-MD5 do arquivo, corresponde ao hash calculado. Se os valores não corresponderem, o AzCopy falhará em baixar os dados. Como, por padrão, a verificação MD5 permanece desativada, você deve especificar essa opção para realizar a verificação MD5 ao baixar os dados.

O Azure Storage não assegura que o hash MD5 armazenado para o blob ou o arquivo esteja atualizado. É de responsabilidade do cliente atualizar o MD5 sempre que o blob ou o arquivo é modificado. No caso de imagens de disco (discos gerenciados ou não gerenciados), as VMs do Azure não atualizam o valor de MD5 conforme os conteúdos do disco se alteram, portanto, /CheckMD5 gera um erro ao fazer o download de imagens de disco.

O AzCopy v8 sempre define a propriedade Content-MD5 para um blob ou um arquivo do Azure depois de carregá-lo no serviço.  
...
* [Como usar o armazenamento de BLOBs do .NET](../blobs/storage-dotnet-how-to-use-blobs.md)
...
* [Apresentando a visualização de biblioteca de movimentação de dados de armazenamento do Azure](https://azure.microsoft.com/blog/introducing-azure-storage-data-movement-library-preview-2/)
* [AzCopy: Introdução a cópia síncrona e tipo de conteúdo personalizado](https://blogs.msdn.com/b/windowsazurestorage/archive/2015/01/13/azcopy-introducing-synchronous-copy-and-customized-content-type.aspx)
* [AzCopy: Anunciando a disponibilidade geral do AzCopy 3.0 plus pré-lançamento do AzCopy 4.0 com o suporte de tabela e arquivo](https://blogs.msdn.com/b/windowsazurestorage/archive/2014/10/29/azcopy-announcing-general-availability-of-azcopy-3-0-plus-preview-release-of-azcopy-4-0-with-table-and-file-support.aspx)
* [AzCopy: Otimizado para cenários de cópia em larga escala](https://go.microsoft.com/fwlink/?LinkId=507682)
* [AzCopy: Suporte para armazenamento com redundância geográfica de acesso de leitura](https://blogs.msdn.com/b/windowsazurestorage/archive/2014/04/07/azcopy-support-for-read-access-geo-redundant-account.aspx)
* [AzCopy: Transferir dados com o modo reiniciável e token SAS](https://blogs.msdn.com/b/windowsazurestorage/archive/2013/09/07/azcopy-transfer-data-with-re-startable-mode-and-sas-token.aspx)
* [AzCopy: Usando a cópia de Blob entre contas](https://blogs.msdn.com/b/windowsazurestorage/archive/2013/04/01/azcopy-using-cross-account-copy-blob.aspx)
* [AzCopy: Upload/download de arquivos para Blobs do Azure](https://blogs.msdn.com/b/windowsazurestorage/archive/2012/12/03/azcopy-uploading-downloading-files-for-windows-azure-blobs.aspx)
