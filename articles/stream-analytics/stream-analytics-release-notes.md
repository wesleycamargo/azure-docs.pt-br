...
Title | Descrição
---|---
Atualização de versão da API REST | A versão da API REST foi atualizada para 2015-10-01. Detalhes podem ser encontrados no MSDN em [Referência da API REST de gerenciamento do Stream Analytics](https://msdn.microsoft.com/library/azure/dn835031.aspx).
Integração de Aprendizado de Máquina do Azure | Com essa versão, é fornecido suporte para funções definidas pelo usuário do Aprendizado de Máquina do Azure. Um tutorial pode ser encontrado [aqui](stream-analytics-machine-learning-tutorial.md), bem como o comunicado do blog geral [aqui](http://blogs.msdn.com/b/streamanalytics/).
