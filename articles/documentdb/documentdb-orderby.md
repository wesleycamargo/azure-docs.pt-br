...
---
# <a name="sorting-azure-cosmos-db-data-using-order-by"></a>Classificando dados do Azure Cosmos DB usando Order By
O Microsoft Azure Cosmos DB dá suporte à consulta de documentos usando o SQL em documentos JSON. Os resultados de consulta podem ser ordenados usando a cláusula ORDER BY nas instruções de consulta SQL.
...
Para obter uma referência completa sobre consultas SQL, consulte o [tutorial de Consultas do Azure Cosmos DB](documentdb-sql-query.md).

## <a name="how-to-query-with-order-by"></a>Como consultar com Order By
Assim como ocorre no ANSI-SQL, agora você pode incluir uma cláusula opcional Order By em instruções SQL ao consultar o Cosmos DB. A cláusula pode incluir um argumento ASC/DESC opcional para especificar a ordem na qual os resultados devem ser recuperados. 
...
O Cosmos DB dá suporte à ordenação com uma propriedade numérica simples, de cadeia de caracteres ou booliana por consulta, com tipos de consulta adicionais que chegarão em breve. Consulte [Qual é o próximo passo](#Whats_coming_next) para obter mais detalhes.

## <a name="configure-an-indexing-policy-for-order-by"></a>Configurar uma política de indexação para Order By
Lembre-se de que o Cosmos DB dá suporte a dois tipos de índices (Hash e Intervalo), que podem ser definidos para propriedades/caminhos específicos, tipos de dados (cadeias de caracteres/números) e em valores de precisão diferentes (precisão máxima ou um valor de precisão fixa). Como o Cosmos DB usa como padrão a indexação Hash, é necessário criar uma nova coleção com uma política de indexação personalizada com Intervalo em números, cadeias de caracteres ou ambos, para usar Order By. 
...
Para obter detalhes, consulte [Políticas de indexação do Azure Cosmos DB](documentdb-indexing-policies.md).
...
Como Order By utiliza o índice do Cosmos DB para pesquisas, o número de unidades de solicitação consumidas por consultas Order By será semelhante às consultas equivalentes sem Order By. Como qualquer outra operação no Cosmos DB, o número de unidades de solicitação depende do tamanho e da forma dos documentos, bem como da complexidade da consulta. 
...
**Como fazer para consultar meus dados existentes no Cosmos DB usando Order By?**
...
* [Referência de consulta do Azure Cosmos DB](documentdb-sql-query.md)
* [Referência de política de indexação do Azure Cosmos DB](documentdb-indexing-policies.md)
* [Referência do SQL do Azure Cosmos DB](https://msdn.microsoft.com/library/azure/dn782250.aspx)
* [Amostras de Order By do Azure Cosmos DB](https://github.com/Azure/azure-documentdb-dotnet/tree/master/samples/code-samples/Queries)
