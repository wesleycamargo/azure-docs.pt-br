...
Este artigo fornece uma visão geral dos vários casos de uso comuns do Cosmos DB.  As recomendações deste artigo servem como ponto de partida ao desenvolver seu aplicativo com o Cosmos DB.   

Após ler este artigo, você poderá responder as perguntas a seguir: 

* Quais são os casos de uso comuns do Cosmos DB?
* Quais são os benefícios do uso do Cosmos DB para aplicativos de varejo?
* Quais são os benefícios do uso do Cosmos DB como um armazenamento de dados para sistemas IoT (Internet das Coisas)?
* Quais são os benefícios do uso do Cosmos DB para aplicativos Web e móveis?

## <a name="introduction"></a>Introdução
O [Azure Cosmos DB](../cosmos-db/introduction.md) é um serviço de banco de dados distribuído globalmente da Microsoft. O serviço foi projetado para permitir aos clientes dimensionar de forma elástica (e independente) a produtividade e o armazenamento em qualquer quantidade de regiões geográficas. O Cosmos DB é o primeiro serviço de banco de dados distribuído globalmente no mercado hoje a oferecer [contratos de nível de serviço](https://azure.microsoft.com/support/legal/sla/documentdb/v1_1/) completos que abrangem produtividade, latência, disponibilidade e consistência. 

O projeto do Cosmos DB começou em 2011 como um “Projeto Florence” para resolver os problemas de desenvolvedores enfrentados por aplicativos grandes de escala da Internet na Microsoft. Ao observar que esses problemas não são exclusivos aos aplicativos da Microsoft, em 2015, decidimos disponibilizar o Cosmos DB para desenvolvedores externos na forma do [Azure DocumentDB](https://azure.microsoft.com/blog/documentdb-moving-to-general-availability/). O serviço é amplamente usado internamente na Microsoft e é um dos serviços de crescimento mais rápido utilizados por desenvolvedores do Azure externamente. 

O Azure Cosmos DB é um multimodelo de banco de dados distribuído globalmente que é usado em uma ampla variedade de aplicativos e casos de uso. É uma boa escolha para qualquer aplicativo que precisa de tempos de resposta baixos na ordem de milissegundos e ser expandido de forma rápida e global. Ele dá suporte a vários modelos de dados (chave-valor, documentos, gráficos e colunas) e várias APIs para acesso a dados, incluindo [MongoDB](documentdb-protocol-mongodb.md), [SQL do DocumentDB](documentdb-introduction.md), [Gremlin](../cosmos-db/graph-introduction.md) e [Tabelas do Azure](../cosmos-db/table-introduction.md) nativamente, bem como de maneira extensível. 

Veja a seguir alguns atributos do Cosmos DB que o tornam adequado para aplicativos de alto desempenho no âmbito global.
...
![Arquitetura de referência de IoT do Azure Cosmos DB](./media/documentdb-use-cases/iot.png)

Picos de dados podem ser processados por Hubs de eventos do Azure que oferecem inclusão de dados de alta taxa de transferência com baixa latência. Dados consumidos que precisem ser processados para obter informações em tempo real poderão ser encaminhados para o Azure Stream Analytics para análise em tempo real. Os dados podem ser carregados no Cosmos DB para consultas ad hoc. Depois que os dados são carregados no Cosmos DB, eles estão prontos para serem consultados.  Os dados no Cosmos DB podem ser usados como dados de referência como parte da análise em tempo real. Além disso, os dados podem ser refinados ainda mais e processados com a conexão dos dados do Cosmos DB ao HDInsight para trabalhos do Pig, Hive ou Map/Reduce.  Depois, os dados refinados são carregados novamente no Cosmos DB para relatórios.   

Para obter uma solução de IoT de exemplo que usa o Cosmos DB, os Hubs de Eventos e o Storm, consulte o [repositório hdinsight-storm-examples no GitHub](https://github.com/hdinsight/hdinsight-storm-examples/).
...
O Cosmos DB é usado por jogos como [The Walking Dead: No Man's Land](https://azure.microsoft.com/blog/the-walking-dead-no-mans-land-game-soars-to-1-with-azure-documentdb/) da [Next Games](http://www.nextgames.com/) e [Halo 5: Guardians](https://azure.microsoft.com/blog/how-halo-5-guardians-implemented-social-gameplay-using-azure-documentdb/). O Cosmos DB oferece os seguintes benefícios aos desenvolvedores de jogos:

* O Cosmos DB permite que o desempenho seja aumentado ou reduzido de forma elástica. Isso permite que os jogos lidem com a atualização de perfis e estatísticas de dezenas a milhões de jogadores simultâneos fazendo uma única chamada à API.
* O Cosmos DB dá suporte a leituras e gravações em milissegundos para ajudar a evitar retardos durante as partidas.
* A indexação automática do Cosmos DB permite a filtragem em várias propriedades diferentes em tempo real, por exemplo, localização de jogadores por suas IDs internas de jogador, do GameCenter, do Facebook, das IDs do Google ou de consulta baseada em associação do jogador a uma comunidade. Isso é possível sem a criação de indexação complexa ou infraestrutura de fragmentação.
...
* O Cosmos DB como uma PaaS (plataforma como serviço) gerenciada exigiu trabalho mínimo de configuração e gerenciamento para permitir a iteração rápida e reduzir o tempo de colocação no mercado.

![Arquitetura de referência de jogos do Azure Cosmos DB](./media/documentdb-use-cases/gaming.png)

## <a name="web-and-mobile-applications"></a>Aplicativos Web e móveis
O Cosmos DB é normalmente usado em aplicativos Web e móveis e é especialmente adequado para modelagem de interações sociais, integração a serviços de terceiros e desenvolvimento de experiências personalizadas avançadas. Os SDKs do Cosmos DB podem ser usados para criar aplicativos avançados do iOS e do Android usando a conhecida [estrutura do Xamarin](documentdb-mobile-apps-with-xamarin.md).  

### <a name="social-applications"></a>Aplicativos sociais
Um caso de uso comum do Cosmos DB é armazenar e consultar o UGC (conteúdo gerado pelo usuário) de aplicativos Web e móveis, particularmente aplicativos de mídia social. Alguns exemplos de UGC são sessões de bate-papo, tweets, postagens em blogs, classificações e comentários. Muitas vezes, o UGC em aplicativos de redes sociais é uma mistura de texto livre, propriedades, rótulos e relações que não se limitam a uma estrutura rígida. Conteúdo como bate-papos, comentários e postagens pode ser armazenado no Cosmos DB sem a necessidade de transformações ou de objetos complexos em camadas de mapeamento relacional.  Propriedades de dados podem ser adicionadas ou modificadas facilmente para atender aos requisitos conforme os desenvolvedores iteram sobre o código do aplicativo, portanto, promovendo um desenvolvimento rápido.  
...
![Arquitetura de referência de aplicativo Web do Azure Cosmos DB](./media/documentdb-use-cases/apps-with-global-reach.png)
...
![Arquitetura de referência de aplicativo Web do Azure Cosmos DB](./media/documentdb-use-cases/personalization.png)

## <a name="next-steps"></a>Próximas etapas
Para obter uma introdução ao Azure Cosmos DB, siga nossos [inícios rápidos](../cosmos-db/create-documentdb-dotnet.md), que mostrarão um passo a passo para criar uma conta e uma introdução ao Cosmos DB. 
...
* [Johnson Controls](http://www.johnsoncontrols.com). Johnson Controls é líder global em vários setores e tecnologias diversificadas que atende a uma ampla variedade de clientes em mais de 150 países.
* [Microsoft Windows, Universal Store, Hub IoT do Azure, Xbox Live e outros serviços de escala da Internet](https://azure.microsoft.com/blog/how-azure-documentdb-planet-scale-nosql-helps-run-microsoft-s-own-businesses/). Como a Microsoft cria serviços altamente escalonáveis usando o Azure DocumentDB.
...
* [Next Games](https://azure.microsoft.com//blog/the-walking-dead-no-mans-land-game-soars-to-1-with-azure-documentdb/). O jogo The Walking Dead: No Man's Land alcança o primeiro lugar com o apoio do Azure Cosmos DB.
