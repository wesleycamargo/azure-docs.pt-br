...
description: "Use este tutorial para saber mais sobre o conector do Spark ao Azure Cosmos DB que permite a conexão do Apache Spark ao Azure Cosmos DB para realizar agregações distribuídas e ciências de dados no sistema de banco de dados multilocatário distribuído globalmente da Microsoft projetado para a nuvem."
...
O conector do Spark ao Azure Cosmos DB permite que o Azure Cosmos DB atue como uma fonte de entrada ou um coletor de saída para trabalhos do Apache Spark. A conexão do [Spark](http://spark.apache.org/) ao [Azure Cosmos DB](https://azure.microsoft.com/services/documentdb/) acelera sua capacidade de resolver os problemas de ciência de dados em constante mudança, nos quais os dados podem ser persistidos rapidamente e consultados com o Cosmos DB. O conector do Spark ao Azure Cosmos DB utiliza com eficiência os índices nativos gerenciados pelo Cosmos DB e habilita colunas atualizáveis ao executar a análise, a aplicação de filtragem de predicado em dados distribuídos globalmente em constante mudança, que variam de IoT, ciência de dados e cenários de análise. 

## <a name="download"></a>Baixar

Comece baixando o conector do Spark ao Azure Cosmos DB (versão prévia) no repositório [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark/) do GitHub.

## <a name="connector-components"></a>Componentes do conector

O conector usa os seguintes componentes:

* [Azure Cosmos DB](http://documentdb.com), um [sistema de banco de dados multilocatário distribuído globalmente](documentdb-distribute-data-globally.md) da Microsoft projetado para a nuvem. O Azure Cosmos DB permite que os clientes provisionem e dimensionem de forma elástica a produtividade e o armazenamento em qualquer quantidade de regiões geográficas. O serviço oferece garantia de 99% de baixa latência, uma garantia de 99,99% de alta disponibilidade e [vários modelos de consistência bem definidos](documentdb-consistency-levels.md) para desenvolvedores.
...
![Fluxo de dados do Spark ao Cosmos DB por meio do pyCosmos DB](./media/documentdb-spark-connector/azure-documentdb-spark-pydocumentdb.png)
...
1. A conexão é feita no nó mestre do Spark com o nó de gateway do Cosmos DB por meio do pyCosmos DB.  Observe que o usuário especifica apenas as conexões do Spark e do Cosmos DB. O fato de que ele se conecta ao respectivo mestre e nós de gateway é transparente para o usuário.
2. A consulta é feita no Cosmos DB (por meio do nó de gateway), no qual a consulta executa a consulta posteriormente em partições da coleção nos nós de dados. A resposta para essas consultas é enviada de volta ao nó de gateway e esse conjunto de resultados retorna ao nó mestre do Spark.
...
O detalhe importante é que a comunicação entre o Spark e o Cosmos DB é limitada ao nó mestre do Spark e aos nós de gateway do Cosmos DB.  As consultas são mais ágeis, pois a camada de transporte ocorre entre esses dois nós.
...
### <a name="connecting-spark-to-cosmos-db-via-pycosmos-db"></a>Conectando o Spark ao Cosmos DB por meio do pyCosmos DB 
Graças à simplicidade do transporte da comunicação, a execução de uma consulta do Spark para o Cosmos DB com o pyCosmos DB é relativamente simples.
...
A conexão do Spark ao Cosmos DB usando o pyCosmos DB é normalmente usada em cenários em que:
...
O conector do Spark ao Cosmos DB utiliza o [SDK do Java do Azure Cosmos DB](https://github.com/Azure/azure-documentdb-java) e move dados entre os nós de trabalho do Spark e do Cosmos DB, conforme mostra o seguinte diagrama:

![Fluxo de dados no conector do Spark ao Cosmos DB](./media/documentdb-spark-connector/azure-documentdb-spark-connector.png)

O fluxo de dados é o seguinte:

1. Uma conexão é feita no nó mestre do Spark com o nó de gateway do Cosmos DB para obter o mapa de partições.  Observe que o usuário especifica apenas as conexões do Spark e do Cosmos DB. O fato de que ele se conecta ao respectivo mestre e nós de gateway é transparente para o usuário.
2. Essas informações são fornecidas de volta ao nó mestre do Spark.  Neste ponto, você deve conseguir analisar a consulta para determinar quais partições (e suas localizações) no Cosmos DB você precisa acessar.
3. Essas informações são transmitidas para os nós de trabalho do Spark...
4. Permitindo que os nós de trabalho do Spark se conectem diretamente às partições do Cosmos DB para extrair os dados necessários e trazer os dados de volta para as partições do Spark nos nós de trabalho do Spark.

O detalhe importante é que a comunicação entre o Spark e o Cosmos DB é consideravelmente mais rápida, pois a movimentação de dados ocorre entre os nós de trabalho do Spark e os nós de dados do Cosmos DB (partições).

### <a name="building-the-spark-to-cosmos-db-connector"></a>Criando o conector do Spark ao Cosmos DB
...
### <a name="connecting-spark-to-cosmos-db-using-the-connector"></a>Conectando o Spark ao Cosmos DB usando o conector
Embora o transporte de comunicação seja um pouco mais complicado, a execução de uma consulta do Spark no Cosmos DB com o conector é consideravelmente mais rápida.
...
Esta seção fornece alguns exemplos de como você pode fazer agregações distribuídas e análises usando o Apache Spark e o Azure Cosmos DB juntos.  Observe que o Azure Cosmos DB já tem suporte para agregações, conforme abordado no [blog Planet scale aggregates with Azure Cosmos DB](https://azure.microsoft.com/blog/planet-scale-aggregates-with-azure-documentdb/) (Agregações em escala mundial com o Azure Cosmos DB). Portanto, veja como elevá-lo ao próximo patamar com o Apache Spark.

Observe que essas agregações fazem referência ao [bloco de anotações do Conector do Spark ao Cosmos DB](https://github.com/Azure/azure-documentdb-spark/blob/master/samples/notebooks/Spark-to-Cosmos DB_Connector.ipynb).

### <a name="connecting-to-flights-sample-data"></a>Conexão com dados de exemplo de voos
Para esses exemplos de agregações, estamos acessando alguns dados de desempenho de voo armazenados em nosso banco de dados do Cosmos DB **DoctorWho**.  Para se conectar a ele, você precisa utilizar o trecho de código a seguir:
...
Com esse trecho, também vamos executar uma consulta base que transfere o conjunto de dados filtrado desejado do Cosmos DB para o Spark (em que o último pode executar agregações distribuídas).  Nesse caso, estamos pedindo voos partindo de Seattle (SEA).
...
Neste próximo conjunto, agora podemos executar facilmente consultas **GROUP BY** em nosso banco de dados do Cosmos DB:
...
Se você ainda não baixou, baixe o conector do Spark ao Cosmos DB no repositório GitHub [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark) e explore os recursos adicionais no repositório:
