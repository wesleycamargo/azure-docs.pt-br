...
## Bancos de dados
Um banco de dados individual do Banco de Dados de Documentos pode conter praticamente uma quantidade ilimitada de armazenamento de documentos agrupada por coleções. As coleções fornecem isolamento de desempenho — cada coleção pode ser provisionada com produtividade que não é compartilhada com outras coleções no mesmo banco de dados ou conta. Um banco de dados do Banco de Dados de Documentos é elástico em termos de tamanho, variando de GBs a TBs de armazenamento de documento com suporte de SSD e provisão de produtividade. Diferentemente de um banco de dados RDBMS tradicional, um banco de dados do Banco de Dados de Documentos não é vinculado a um único computador e pode abranger vários computadores ou clusters.

Com o Banco de Dados de Documentos, conforme precisar dimensionar seus aplicativos, você pode criar mais coleções ou bancos de dados, ou ambos.
...
>[AZURE.NOTE] A linha de base de uma unidade de solicitação para um documento de 1 KB corresponde a um GET simples do documento com Consistência de Sessão.
