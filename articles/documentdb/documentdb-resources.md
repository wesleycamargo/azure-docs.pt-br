...
- Como eu trabalho com UDFs, gatilhos e procedimentos armazenados?
...
Você pode provisionar uma ou mais contas do Banco de Dados de Documentos usando sua assinatura do Azure. Para sua conta de banco de dados, você pode adquirir unidades empilháveis de produtividade e armazenamento de documentos com suporte SSD sob a forma de unidades de capacidade, conforme mostrado no diagrama a seguir.    
...
<tr>
<td valign="top" ><p>CurrentMediaStorageUsageInMB (LEITURA)</p></td>
...
<td valign="top" ><p>O número total de unidades de capacidade provisionadas na conta de banco de dados.</p></td>
</tr>

<tr>
<td valign="top" ><p>CapacityUnitsConsumed (LEITURA)</p></td>
<td valign="top" ><p>O número total de unidades de capacidade consumidas atualmente na conta de banco de dados.</p></td>
</tr>

<tr>
<td valign="top" ><p>ProvisionedDocumentStorageInMB (LEITURA)</p></td>
<td valign="top" ><p>A quantidade total de armazenamento de documentos (em MB) provisionada entre todas as unidades de capacidade na conta do banco de dados.</p></td>
</tr>

<tr>
<td valign="top" ><p>ReservedDocumentStorageInMB (LEITURA)</p></td>
<td valign="top" ><p>A quantidade total de armazenamento de documentos (em MB) reservada entre todas as unidades de capacidade na conta do banco de dados. Diz-se que o armazenamento dos documentos está reservado quando uma coleção contém documentos. </p></td>
</tr>

<tr>
<td valign="top" ><p>ConsumedDocumentStorageInMB (LEITURA)</p></td>
<td valign="top" ><p>A quantidade total de armazenamento de documentos (em MB) consumida, de fato, entre todas as unidades de capacidade na conta do banco de dados. Esse é o tamanho dos seus documentos reais e do índice combinados entre várias coleções em uma conta do banco de dados.</p></td>
</tr>
...
Assim como com outros recursos do modelo de recursos do Banco de Dados de Documentos, os bancos de dados podem ser criados, substituídos, excluídos, lidos ou enumerados facilmente usando as [APIs REST do Banco de Dados de Documentos do Azure](https://msdn.microsoft.com/library/azure/dn781481.aspx) ou qualquer [SDK do cliente](https://msdn.microsoft.com/library/azure/dn781482.aspx). O Banco de Dados de Documentos garante uma forte consistência para a leitura ou consulta de metadados de um recurso do banco de dados. Excluir um banco de dados automaticamente garante que você não possa acessar qualquer uma das coleções ou usuários contidos nele. Embora o Banco de Dados de Documentos recupere o armazenamento e a produtividade provisionados como parte do banco de dados excluído em segundo plano, o armazenamento provisionado e a produtividade para o banco de dados excluído estão disponíveis imediatamente para uso.   

##Coleções
Uma coleção do Banco de Dados de Documentos é um contêiner para seus documentos JSON. Uma coleção também é uma unidade de dimensionamento para transações e consulta. Também é possível expandir um banco de dados do Banco de Dados de Documentos adicionando mais coleções. Se o seu aplicativo precisar ser mais escalado, é possível provisionar mais armazenamento de documento com suporte de SSD (e produtividade) e distribuí-lo entre as coleções de um ou mais bancos de dados em sua conta do banco de dados.
...
Uma UDF pode ser especificada como parte da consulta SQL e é usada como uma maneira de estender a linguagem central de consulta [SQL do Banco de Dados de Documentos](https://msdn.microsoft.com/library/azure/dn782250.aspx).   
