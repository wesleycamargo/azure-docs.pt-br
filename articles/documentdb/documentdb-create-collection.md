...
Para usar o Microsoft Azure DocumentDB, você deve ter uma conta do [DocumentDB](documentdb-create-account.md), um banco de dados, uma coleção e documentos. Este tópico descreve como criar uma coleção do DocumentDB no Portal do Azure.

Não sabe direito o que é uma coleção? Confira [O que é uma coleção do DocumentDB?](#what-is-a-documentdb-collection).
...
1. No [portal do Azure](https://portal.azure.com/), na barra de atalhos, clique em **DocumentDB (NoSQL)** e, na folha **DocumentDB (NoSQL)**, escolha a conta à qual você deseja adicionar uma coleção. Se você não tiver contas listadas, você precisará [criar uma conta do DocumentDB](documentdb-create-account.md).

   ![Captura de tela realçando as contas do DocumentDB no Jumpbar, a conta na folha do DocumentDB e o banco de dados na folha da conta do DocumentDB, nas lentes dos Bancos de Dados](./media/documentdb-create-collection/docdb-database-creation-1-2.png)

   Se **DocumentDB (NoSQL)** não estiver visível na barra de atalhos, clique em **Mais Serviços** e em **DocumentDB (NoSQL)**. Se você não tiver contas listadas, você precisará [criar uma conta do DocumentDB](documentdb-create-account.md).
2. Na folha **Conta do DocumentDB** da conta selecionada, clique em **Adicionar Coleção**.

    ![Captura de tela realçando as contas do DocumentDB no Jumpbar, a conta na folha do DocumentDB e o banco de dados na folha da conta do DocumentDB, nas lentes dos Bancos de Dados](./media/documentdb-create-collection/docdb-database-creation-3.png)
3. Na folha **Adicionar Coleção**, na caixa **ID da Coleção**, insira a ID da nova coleção. Os nomes de coleção devem ter entre 1 e 255 caracteres e não podem conter `/ \ # ?` nem espaços à direita. Quando o nome for validado, uma marca de seleção verde será exibida na caixa de identificação.
    ...
    ![Captura de tela realçando o botão Adicionar Coleção na folha do Banco de Dados, as configurações na folha Adicionar Coleção e o botão OK — portal do Azure para DocumentDB — criador do banco de dados baseado na nuvem para bancos de dados JSON NoSQL](./media/documentdb-create-collection/docdb-collection-creation-5-8.png)
...
9. A nova coleção agora aparece na lente **Coleções** na folha **Visão geral**.
    ...
    ![Captura de tela da nova coleção na folha Banco de Dados — Portal do Azure para DocumentDB — criador do banco de dados baseado na nuvem para bancos de dados JSON NoSQL](./media/documentdb-create-collection/docdb-collection-creation-9.png)
...
## <a name="what-is-a-documentdb-collection"></a>O que é uma coleção do DocumentDB?
...
As coleções são particionadas automaticamente em um ou mais servidores físicos pelo DocumentDB. Ao criar uma coleção, você pode especificar a produtividade provisionada em termos de unidades de solicitação por segundo e uma propriedade de chave de partição. O valor dessa propriedade será usado pelo DocumentDB para distribuir documentos entre partições e rotear solicitações como consultas. O valor de chave de partição também atua como o limite de transação para procedimentos armazenados e gatilhos. Cada coleção tem uma quantidade reservada de produtividade específica, que não é compartilhada com outras coleções na mesma conta. Portanto, você pode escalar horizontalmente o aplicativo em termos de armazenamento e taxa de transferência.

Coleções não são o mesmo que tabelas em bancos de dados relacionais. As coleções não impõem esquemas. Na verdade, o DocumentDB não impõe nenhum esquema. Trata-se de um banco de dados livre de esquema. Sendo assim, você pode armazenar tipos diferentes de documentos com vários esquemas na mesma coleção. É possível optar por usar coleções para armazenar objetos de um único tipo, assim como acontece com as tabelas. O melhor modelo depende apenas de como os dados aparecem juntos em consultas e transações.

## <a name="other-ways-to-create-a-documentdb-collection"></a>Outra formas de criar uma coleção do DocumentDB
