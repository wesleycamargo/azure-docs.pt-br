---
title: "Diretrizes para implementação de API | Microsoft Docs"
...
---
# <a name="api-implementation-guidance"></a>Diretrizes para implementação de API
...
## <a name="considerations-for-implementing-a-restful-web-api"></a>Considerações sobre implementação de uma API da Web RESTful
...
## <a name="considerations-for-implementing-request-routing"></a>Considerações sobre a implementação de roteamento de solicitações
Em um serviço implementado usando a API Web ASP.NET, cada solicitação é roteada para um método em uma classe *controller* . A estrutura da API da Web fornece duas opções principais para implementar o roteamento; roteamento *baseado em convenções* e roteamento *baseado em atributos*. Considere os pontos a seguir ao determinar a melhor maneira para rotear solicitações de sua API da Web:
...
* **Compreenda as limitações e requisitos do roteamento baseado em convenções**.
    ...
    As rotas podem ser genéricas, incluindo argumentos literais como *api* e variáveis como *{controller}* e *{id}*. O roteamento baseado em convenções permite que alguns elementos da rota sejam opcionais. A estrutura da API da Web determina qual método invocar no controlador, fazendo a correspondência entre o método HTTP na solicitação e a parte inicial do nome do método na API e, em seguida, combinando quaisquer eventuais parâmetros opcionais. Por exemplo, se um controlador chamado *orders* contiver os métodos *GetAllOrders()* ou *GetOrderByInt(int id)*, então a solicitação GET *http://www.adventure-works.com/api/orders/* será direcionada para o método *GetAlllOrders()* e a solicitação GET *http://www.adventure-works.com/api/orders/99* será roteada para o método *GetOrderByInt(int id)*. Se não houver nenhum método correspondente disponível que comece com o prefixo Get no controlador, a estrutura da API Web responde com uma mensagem HTTP 405 (Método Não Permitido). Além disso, o nome do parâmetro (id) especificado na tabela de roteamento deve ser igual ao nome do parâmetro para o método *GetOrderById* ; caso contrário, a estrutura da API da Web envia uma resposta HTTP 404 (Não Encontrado).

    As mesmas regras se aplicam a solicitações HTTP POST, PUT e DELETE; uma solicitação PUT que atualiza os detalhes do pedido 101 deve ser direcionada para o URI *http://www.adventure-works.com/api/orders/101*; o corpo da mensagem conterá os novos detalhes do pedido e essas informações serão passadas como um parâmetro para um método no controlador “orders”, com um nome começando com o prefixo *Put*, como *PutOrder*.

    A tabela de roteamento padrão não corresponderá a uma solicitação que faz referência a recursos filho em uma API Web RESTful, como *http://www.adventure-works.com/api/customers/1/orders* (localize os detalhes de todos os pedidos feitos pelo cliente 1). Para tratar desses casos, você poderá adicionar rotas personalizadas à tabela de roteamento:
    ...
    Essa rota direciona as solicitações que correspondem ao URI para o método *GetOrdersForCustomer* no controlador *Customers*. Este método deve aceitar um parâmetro único chamado *custI*:
...
* **Prefira roteamento baseado em atributos**.
    ...
    Roteamento baseado em atributos também tem o efeito colateral útil de atuar como documentação para os desenvolvedores que precisam manter o código no futuro; é imediatamente clara a compreensão de qual método pertence a qual rota, e o atributo *HttpGet* esclarece o tipo de solicitação HTTP à qual o método responde.

    O roteamento baseado em atributos permite que você defina as restrições que restringem o modo como é realizada a correspondência entre os parâmetros. Restrições podem especificar o tipo do parâmetro e, em alguns casos, também podem indicar o intervalo aceitável de valores do parâmetro. No exemplo a seguir, o parâmetro id para o método *FindCustomerByID* deve ser um número inteiro não negativo. Se um aplicativo envia uma solicitação HTTP GET com um número de cliente negativo, a estrutura da API da Web responderá com uma mensagem HTTP 405 (Método Não Permitido):
...
* **Identifique os métodos que não devem ser roteados**.
    ...
    Se você estiver usando roteamento baseado em convenções, indique os métodos que não correspondem às ações HTTP identificando-os com o atributo *NonAction* . Isso geralmente se aplica aos métodos auxiliares definidos para uso por outros métodos em um controlador, e esse atributo impedirá tanto que esses métodos sejam chamados por uma solicitação HTTP errônea, quanto que a correspondência desses métodos seja feita por tal solicitação.
* **Considere as vantagens e desvantagens de colocar a API em um subdomínio**.

    Por padrão, a API Web ASP.NET organiza as APIs no diretório */api* em um domínio, como *http://www.adventure-works.com/api/orders*. Esse diretório reside no mesmo domínio que qualquer outro serviço exposto pelo mesmo host. Pode ser vantajoso dividir a API Web em seu próprio subdomínio em execução em um host separado, com URIs como *http://api.adventure-works.com/orders*. Essa separação permite a você particionar e dimensionar a API da Web com mais eficiência, sem afetar outros serviços ou aplicativos da Web em execução no domínio *www.adventure-works.com*.

    No entanto, colocar uma API da Web em um subdomínio diferente também pode levar a problemas de segurança. Qualquer aplicativo ou serviço Web hospedado em *www.adventure-works.com* que invoca uma API da Web sendo executada em outro lugar pode violar a política de mesma origem de muitos navegadores da Web. Nessa situação, será necessário habilitar compartilhamento de CORS (recursos entre origens) entre os hosts. Para obter mais informações, consulte o documento Diretrizes de Segurança de API.
...
* **Forneça links para oferecer suporte a descoberta de recursos e navegação estilo HATEOAS**.
  ...
  * Uma solicitação HTTP GET para o URI *http://adventure-works.com/customers/2* a fim de coletar os detalhes do cliente (novamente). Os dados podem ser retornados como XML ou JSON.
  * Uma solicitação HTTP PUT para o URI *http://adventure-works.com/customers/2* a fim de modificar os detalhes do cliente. Os novos dados devem ser fornecidos na mensagem de solicitação no formato x-www-form-urlencoded.
  * Uma solicitação HTTP DELETE para o URI *http://adventure-works.com/customers/2* a fim de excluir os detalhes do cliente. A solicitação não espera nenhuma informação adicional nem dados de retorno no corpo da mensagem de resposta.
  * Uma solicitação HTTP GET para o URI *http://adventure-works.com/customers/2/orders* a fim de encontrar todos os pedidos do cliente. Os dados podem ser retornados como XML ou JSON.
  * Uma solicitação HTTP PUT para o URI *http://adventure-works.com/customers/2/orders* a fim de criar um novo pedido do cliente. Os dados devem ser fornecidos na mensagem de solicitação no formato x-www-form-urlencoded.
...
* **Capture as exceções e retorne uma resposta significativa para os clientes**.
  ...
  > [!TIP]
  > Não inclua informações que possam ser úteis para um invasor tentando penetrar em sua API da Web. Para obter mais informações, visite a página [Tratamento de Exceções em API Web ASP.NET](http://www.asp.net/web-api/overview/error-handling/exception-handling), no site da Microsoft.
  >
  > [!NOTE]
...
* **Ofereça suporte a paginação para solicitações que podem retornar um grande número de objetos**.
    ...
    Um aplicativo cliente pode emitir uma solicitação para recuperar 30 pedidos começando com deslocamento 50, usando o URI *http://www.adventure-works.com/api/orders?limit=30&offset=50*.
...
* **Forneça Suporte Assíncrono Para Solicitações de Execução Longa**.
  ...
  > [!NOTE]
  > Para obter mais informações sobre como usar WebJobs com o site do Azure, visite a página [Utilizar WebJobs para executar tarefas em segundo plano em sites do Microsoft Azure](app-service-web/web-sites-create-web-jobs.md) , no site da Microsoft.
  >
  >
  ...
  8. Usar um Hub de Notificação do Azure para enviar respostas assíncronas para aplicativos cliente. A página [Notificação de Usuários nos Hubs de Notificação do Azure](notification-hubs/notification-hubs-aspnet-backend-windows-dotnet-wns-notification.md) no site da Microsoft fornece mais detalhes.
  ...
  10. Usar o SignalR para enviar dados em tempo real do servidor Web ao cliente por uma conexão de rede persistente. O SignalR está disponível para aplicativos Web ASP.NET como um pacote do NuGet. Você pode encontrar mais informações no site [SignalR ASP.NET](http://signalr.net/) .

      > [!NOTE]
      > Comet e SignalR ambos utilizam conexões de rede persistentes entre o servidor Web e o aplicativo cliente. Isso pode afetar a escalabilidade, já que um grande número de clientes pode exigir um número igualmente grande de conexões simultâneas.
      >
      >
...
> [!NOTE]
> O exemplo de API da Web disponível com essas diretrizes inclui um projeto de teste, que mostra como executar testes de unidade em operações selecionadas.
>
>
...
* Verifique todos os links e URIs nas mensagens de resposta. Por exemplo, uma mensagem HTTP POST deve retornar o URI do recurso recém-criado. Todos os links HATEOAS devem ser válidos.
  ...
  > [!IMPORTANT]
  > Se você publicar a API da Web por meio de um Serviço de Gerenciamento de API, esses URIs deve refletir a URL do Serviço de Gerenciamento e não a URL do servidor Web que hospeda a API da Web.
  >
  >
...
## <a name="publishing-and-managing-a-web-api-by-using-the-azure-api-management-service"></a>Publicação e gerenciamento uma API da Web usando o Serviço de Gerenciamento de API do Azure
O Azure fornece o [Serviço de Gerenciamento de API](https://azure.microsoft.com/documentation/services/api-management/) , que você pode usar para publicar e gerenciar uma API da Web. Usando esse recurso, você pode gerar um serviço que atua como uma fachada para uma ou mais APIs da Web. O serviço é por si próprio um serviço Web escalonável, que você pode criar e configurar usando o Portal de Gerenciamento do Azure. Você pode usar esse serviço para publicar e gerenciar uma API da Web conforme descrito a seguir:
...
## <a name="supporting-developers-building-client-applications"></a>Oferecendo suporte desenvolvedores que criam aplicativos cliente
...
### <a name="documenting-the-rest-operations-for-a-web-api"></a>Documentando as operações REST para uma API da Web
...
### <a name="implementing-a-client-sdk"></a>Implementando um SDK de cliente
...
A página [Application Insights - Introdução ao ASP.NET](application-insights/app-insights-asp-net.md) no site da Microsoft fornece mais informações.
...
* A página [Executar tarefas em segundo plano com WebJobs](app-service-web/web-sites-create-web-jobs.md) , no site da Microsoft, fornece informações e exemplos sobre como usar os WebJobs para realizar operações em segundo plano em um site do Azure.
...
* A página [Métodos de roteamento do Gerenciador de Tráfego](traffic-manager/traffic-manager-routing-methods.md) , no site da Microsoft, resume como o Gerenciador de Tráfego do Azure pode ser usado para realizar o balanceamento de carga de solicitações entre várias instâncias de um site que hospeda uma API da Web.
* A página [Application Insights - introdução ao ASP. NET](application-insights/app-insights-asp-net.md) , no site da Microsoft, fornece informações detalhadas sobre como instalar e configurar o Application Insights em um projeto de API Web ASP.NET.
