...
4. Na folha **Visão geral** do laboratório, selecione **+ Máquina Virtual**. ![Botão Adicionar VM](./media/devtest-lab-add-vm-with-artifacts/devtestlab-home-blade-add-vm.png)
...
13. Selecione **Artefatos** e, na lista de artefatos, selecione e configure os artefatos que você deseja adicionar à imagem base. **Observação:** se você for iniciante em Laboratórios de Desenvolvimento/Teste ou na configuração de artefatos, vá para a seção [Adicionar um artefato existente a uma VM](#add-an-existing-artifact-to-a-vm) e volte aqui quando terminar.
14. Se quiser exibir ou copiar o modelo do Azure Resource Manager, pule para a seção [Salvar modelo do Azure Resource Manager](#save-arm-template) e retorne para cá quando terminar.
