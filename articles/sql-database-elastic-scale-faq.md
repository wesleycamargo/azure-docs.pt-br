...
O objeto de informações de esquema só é usado para dividir os cenários de mesclagem. Se um aplicativo for basicamente o único locatário, ele não exige o serviço de divisão e mesclagem e, portanto, não é necessário para preencher o objeto de informações de esquema.
...
Consulte **[Adicionar um fragmento a um aplicativo de Escala elástica](http://azure.microsoft.com/documentation/articles/sql-database-elastic-scale-add-a-shard/)**. 

#### Quanto custa a Escala elástica?
O uso da biblioteca de Escala elástica não acarretará nenhum custo. Custos se acumulam apenas para os bancos de dados do SQL Azure que você usa para fragmentos e o Gerenciador de mapa de fragmento, bem como as funções web/de trabalho configuradas para o serviço de divisão e mesclagem.
...
#### O serviço de divisão e mesclagem provisiona (ou exclui) um banco de dados durante uma operação de divisão ou mesclagem? 
