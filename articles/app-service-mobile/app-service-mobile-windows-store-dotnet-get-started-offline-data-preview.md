...
* [SDK dos Serviços móveis do Azure versão 2.0.0 (ou posterior)][Nuget do SDK do aplicativo móvel do Azure]
* [Armazenamento do Azure Mobile Services SQLite versão 1.0.2 (ou posterior)][Armazenamento do SQLite Nuget]
...
Os recursos offline do aplicativo móvel do Azure permitem que você interaja com um banco de dados local quando estiver em um cenário offline com seu Serviço Móvel. Para usar esses recursos em seu aplicativo, você deve inicializar `MobileServiceClient.SyncContext` para um armazenamento local. Então, faça referência à sua tabela por meio da interface `IMobileServiceSyncTable`. Neste tutorial, usamos o SQLite para o repositório local.
...
2. No Visual Studio, abra o projeto que você concluiu no tutorial [Criar um aplicativo do Windows]. Instale o pacote **WindowsAzure.MobileServices.SQLiteStore** do NuGet para o tempo de execução dos projetos do Windows 8.1 e do Windows Phone 8.1.
...
3. No Gerenciador de soluções, clique com o botão direito em **Referências** para o tempo de execução dos projetos da plataforma Windows 8.1 e Windows Phone 8.1 e adicione uma referência para o SQLite, que está localizado na seção **Extensões**.
...
10. Adicione manipuladores de exceção ao método `SyncAsync`:
    ...
    Observe que o `MobileServicePushFailedException` pode ocorrer tanto para uma operação de push quanto de pull. Ele pode ocorrer por uma recepção porque a operação de recepção executa internamente um envio para certificar-se de que todas as tabelas, juntamente com quaisquer relacionamentos serão consistentes.
...
1. Abra o App.xaml.cs no projeto compartilhado. Remova o comentário de sua inicialização anterior de `MobileServiceClient` para adicionar novamente a chave de aplicativo, a URL de gateway e a URL do back-end do aplicativo móvel corretos.

2. Pressione a tecla **F5** para recompilar e executar o projeto. Observe que os dados têm a mesma aparência do cenário offline mesmo que agora o aplicativo esteja conectado ao back-end do aplicativo móvel. Isso ocorre porque esse aplicativo sempre funciona com o `IMobileServiceSyncTable` que está apontado para o repositório local.

3. Faça logon no portal do Azure e examine o banco de dados para o back-end do aplicativo móvel.

    No Visual Studio, vá para **Server Explorer** -> **Azure** -> **Bancos de Dados SQL**. Clique com o botão direito do mouse em seu Banco de Dados e selecione **Abrir no Gerenciador de Objetos do SQL Server**.
...
*  Para executar push das mudanças no servidor, chamamos  `IMobileServiceSyncContext.PushAsync()`. Esse método é um membro de `IMobileServicesSyncContext` em vez da tabela de sincronização porque enviará as alterações por push a todas as tabelas.
