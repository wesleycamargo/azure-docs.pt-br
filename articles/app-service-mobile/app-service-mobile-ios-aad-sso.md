...
Neste tutorial, você adiciona a autenticação ao projeto de início rápido usando uma Biblioteca de autenticação do Active Directory (ADAL). Você também pode habilitar a autenticação com menos configuração apenas usando o SDK dos Aplicativos Móveis, como abordamos no tutorial [Adicionar autenticação ao seu aplicativo]. Com esse tópico em ADAL você tem uma experiência mais integrada de autenticação para usuários finais e o ADAL fornece recursos mais avançados para acessar outros recursos protegidos de AAD.
