...
# <a name="how-to-use-the-azure-mobile-apps-node.js-sdk"></a>Como usar o SDK do Node.js para Aplicativos Móveis do Azure
...
## <a name="<a-name="introduction"></a>introduction"></a><a name="Introduction"></a>Introdução
...
### <a name="<a-name="howto-cmdline-basicapp"></a>how-to:-create-a-basic-node.js-backend-using-the-command-line"></a><a name="howto-cmdline-basicapp"></a>Como criar um back-end de Node.js básico usando a linha de comando
...
### <a name="<a-name="howto-vs2015-basicapp"></a>how-to:-create-a-node-backend-with-visual-studio-2015"></a><a name="howto-vs2015-basicapp"></a>Como criar um back-end de nó com o Visual Studio 2015
...
### <a name="<a-name="create-node-backend-portal"></a>how-to:-create-a-node.js-backend-using-the-azure-portal"></a><a name="create-node-backend-portal"></a>Como criar um back-end do Node.js usando o Portal do Azure
...
### <a name="<a-name="download-quickstart"></a>how-to:-download-the-node.js-backend-quickstart-code-project-using-git"></a><a name="download-quickstart"></a>Como baixar o projeto de código de início rápido do back-end de Node.js usando Git
...
2. Siga as etapas em [Habilitar o repositório de aplicativos do Serviço de Aplicativo](../app-service-web/app-service-deploy-local-git.md#Step3) para habilitar o repositório Git para seu site de back-end, anotando o nome de usuário e a senha de implantação.
...
### <a name="<a-name="howto-publish-to-azure"></a>how-to:-publish-your-node.js-backend-to-azure"></a><a name="howto-publish-to-azure"></a>Como publicar seu back-end de Node.js no Azure
...
### <a name="<a-name="howto-enable-homepage"></a>how-to:-enable-a-home-page-for-your-application"></a><a name="howto-enable-homepage"></a>Como habilitar uma home page para seu aplicativo
...
## <a name="<a-name="tableoperations"></a>table-operations"></a><a name="TableOperations"></a>Operações de tabela 
...
| POST /tables/_tablename_ | Criar um registro na tabela |
| PATCH /tables/_tablename_/:id | Atualizar um registro na tabela |
...
### <a name="<a-name="howto-dynamicschema"></a>how-to:-define-tables-using-a-dynamic-schema"></a><a name="howto-dynamicschema"></a>Como definir tabelas usando um esquema dinâmico
...
### <a name="<a-name="howto-staticschema"></a>how-to:-define-tables-using-a-static-schema"></a><a name="howto-staticschema"></a>Como definir tabelas usando um esquema estático
...
### <a name="<a-name="howto-sqlexpress-setup"></a>how-to:-use-sql-express-as-a-development-data-store-on-your-local-machine"></a><a name="howto-sqlexpress-setup"></a>Como usar o SQL Express como um armazenamento de dados de desenvolvimento em sua máquina local
...
### <a name="<a-name="howto-config-localdev"></a>how-to:-configure-your-project-for-local-development"></a><a name="howto-config-localdev"></a>Como configurar seu projeto para desenvolvimento local

Os Aplicativos Móveis do Azure leem um arquivo JavaScript chamado _azureMobile.js_ no sistema de arquivos local.  Não use esse arquivo para configurar o SDK dos Aplicativos Móveis do Azure em produção; em vez disso, use as Configurações de Aplicativo no [portal do Azure] .  O arquivo _azureMobile.js_ deve exportar um objeto de configuração.  As configurações mais comuns são:

- Configurações do banco de dados
- Configurações de registro em log de diagnóstico
- Configurações de CORS alternativas

Um exemplo de arquivo _azureMobile.js_ implementando as configurações de banco de dados fornecidas anteriormente é dado abaixo:
...
### <a name="<a-name="howto-appsettings"></a>how:-configure-app-settings-for-your-mobile-app"></a><a name="howto-appsettings"></a>Como definir configurações de aplicativo para seu aplicativo móvel
...
| Configurações de Aplicativo                 | _azureMobile.js_   | Descrição                               | Valores Válidos                                |
| :-------------------------- | :------------------------ | :---------------------------------------- | :------------------------------------------ |
...
### <a name="<a-name="howto-use-sqlazure"></a>how-to:-use-sql-database-as-your-production-data-store"></a><a name="howto-use-sqlazure"></a>Como usar o Banco de Dados SQL como o armazenamento de dados de produção
...
> [AZURE.NOTE] Se já houver um banco de dados no mesmo local do back-end do aplicativo móvel, você poderá escolher **Usar um banco de dados existente** e selecionar esse banco de dados. O uso de um banco de dados em um local diferente não é recomendado devido a latências maiores.
...
### <a name="<a-name="howto-tables-auth"></a>how-to:-require-authentication-for-access-to-tables"></a><a name="howto-tables-auth"></a>Como exigir autenticação para acesso às tabelas
...
### <a name="<a-name="howto-tables-getidentity"></a>how-to:-use-authentication-claims-with-your-tables"></a><a name="howto-tables-getidentity"></a>Como usar declarações de autenticação com as tabelas
...
### <a name="<a-name="howto-tables-disabled"></a>how-to:-disable-access-to-specific-table-operations"></a><a name="howto-tables-disabled"></a>Como desabilitar o acesso a operações de tabela específicas
...
### <a name="<a-name="howto-tables-query"></a>how-to:-adjust-the-query-that-is-used-with-table-operations"></a><a name="howto-tables-query"></a>Como ajustar a consulta usada em operações de tabela
...
### <a name="<a-name="howto-tables-softdelete"></a>how-to:-configure-soft-delete-on-a-table"></a><a name="howto-tables-softdelete"></a>Como configurar a exclusão reversível em uma tabela
...
### <a name="<a-name="howto-tables-seeding"></a>how-to:-seed-your-database-with-data"></a><a name="howto-tables-seeding"></a>Como propagar seu banco de dados com dados
...
### <a name="<a-name="swagger"></a>how-to:-enable-swagger-support"></a><a name="Swagger"></a>Como habilitar o suporte para o Swagger
...
## <a name="<a-name="push">push-notifications"></a><a name="push">Notificações por Push
...
### <a name="</a><a-name="send-push"></a>how-to:-send-push-notifications"></a></a><a name="send-push"></a>Como enviar notificações por push
...
###<a name="<a-name="push-user"></a>how-to:-send-push-notifications-to-an-authenticated-user-using-tags"></a><a name="push-user"></a>Como enviar notificações por push para um usuário autenticado usando marcações
...
## <a name="<a-name="customapi"></a>-custom-apis"></a><a name="CustomAPI"></a> APIs personalizadas

###  <a name="<a-name="howto-customapi-basic"></a>how-to:-define-a-custom-api"></a><a name="howto-customapi-basic"></a>Como definir uma API personalizada
...
Vamos pegar uma API de exemplo que retorna a data do servidor usando o método _Date.now()_ .  Eis o arquivo api/date.js:
...
### <a name="<a-name="howto-customapi-auth"></a>how-to:-require-authentication-for-access-to-a-custom-api"></a><a name="howto-customapi-auth"></a>Como solicitar autenticação para acesso a uma API personalizada
...
### <a name="<a-name="howto-customapi-auth"></a>how-to:-handle-large-file-uploads"></a><a name="howto-customapi-auth"></a>Como manipular transferências de arquivos grandes
...
### <a name="<a-name="howto-customapi-sql"></a>how-to:-execute-custom-sql-statements"></a><a name="howto-customapi-sql"></a>Como executar instruções SQL personalizadas
...
## <a name="<a-name="debugging"></a>debugging,-easy-tables,-and-easy-apis"></a><a name="Debugging"></a>Depuração, Tabelas Fáceis e APIs Fáceis

### <a name="<a-name="howto-diagnostic-logs"></a>how-to:-debug,-diagnose,-and-troubleshoot-azure-mobile-apps"></a><a name="howto-diagnostic-logs"></a>Como depurar, diagnosticar e solucionar problemas dos Aplicativos Móveis do Azure
...
### <a name="<a-name="in-portal-editing"></a><a-name="work-easy-tables"></a>how-to:-work-with-easy-tables-in-the-azure-portal"></a><a name="in-portal-editing"></a><a name="work-easy-tables"></a>Como trabalhar com Tabelas Fáceis no portal do Azure
...
###<a name="<a-name="work-easy-apis"></a>how-to:-work-with-easy-apis-in-the-azure-portal"></a><a name="work-easy-apis"></a>Como trabalhar com APIs fáceis no portal do Azure
...
###<a name="<a-name="online-editor"></a>how-to:-edit-code-in-the-app-service-editor"></a><a name="online-editor"></a>Como editar o código no Editor de Serviço de Aplicativo
...
[Início rápido do cliente HTML/Javascript]: app-service-html-get-started.md
...
[Criar um serviço de aplicativo do Azure]: ../app-service-web/
...
[exemplo de esquema estático no GitHub]: https://github.com/azure/azure-mobile-apps-node/tree/master/samples/static-schema
