...
Usando declarações [XDT][XDT] (XML Document Transformation), você pode transformar o arquivo [ApplicationHost.config][ApplicationHost.config] nos sites do Windows Azure. Também é possível usar declarações XDT para adicionar extensões de site particulares a fim de habilitar ações de administração de site personalizadas. Este artigo inclui uma extensão de site do PHP Manager de exemplo que permite o gerenciamento das configurações de PHP por meio de uma interface da Web.

<!-- MINI TOC -->

-   [Transformar a configuração do site em ApplicationHost.config][Transformar a configuração do site em ApplicationHost.config]
-   [Estender o site][Estender o site]
    -   [Visão geral das extensões de site particular][Visão geral das extensões de site particular]
    -   [Exemplo de extensão do site: Gerenciador de PHP][Exemplo de extensão do site: Gerenciador de PHP]
        -   [O aplicativo Web Gerenciador de PHP][O aplicativo Web Gerenciador de PHP]
        -   [O arquivo applicationHost.xdt][O arquivo applicationHost.xdt]
    -   [Implantação da extensão do site][Implantação da extensão do site]

## <span id="transform"></span></a>Transformar a configuração do site em ApplicationHost.config

A plataforma Sites do Azure fornece flexibilidade e controle para configuração do site. Embora o arquivo de configuração ApplicationHost.config do IIS padrão não esteja disponível para edição direta nos Sites do Windows Azure, a plataforma dá suporte a um modelo de transformação ApplicationHost.config declarativo com base no XDT (XML Document Transformation).

Para aproveitar essa funcionalidade de transformação, você cria um arquivo ApplicationHost.xdt com conteúdo XDT e o coloca na raiz do site. Em seguida, na página **Configurar** do Portal do Windows Azure, você define a configuração de aplicativo `WEBSITE_PRIVATE_EXTENSIONS` como 1 (talvez você precise reiniciar o site).
...
Para ver exemplos adicionais, consulte [][]<https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions></a>
...
## <span id="extend"></span></a>Estender o site

### <span id="overview"></span></a>Visão geral das extensões de site particular
...
1.  Extensão do site **Conteúdo**: cria qualquer aplicativo web com suporte pelos sites do Azure
2.  Extensão do site **Declaração**: cria um arquivo ApplicationHost.xdt
3.  Extensão do site **implantação**: coloca conteúdo na pasta SiteExtensions em `root`
4.  Extensão do site **habilitação**: define as configurações do aplicativo `WEBSITE_PRIVATE_EXTENSIONS` para 1
...
**Observação**: informações adicionais sobre esses elementos-chave estão disponíveis em [][]<https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions></a>. Um exemplo detalhado está incluído para ilustrar as etapas para criar e habilitar uma extensão de site particular. O código-fonte do exemplo Gerenciador de PHP a seguir pode ser baixado em [][1]<https://github.com/projectkudu/PHPManager></a>

### <span id="SiteSample"></span></a>Exemplo de extensão do site: Gerenciador de PHP
...
#### <span id="PHPwebapp"></span></a>O aplicativo Web Gerenciador de PHP
...
#### <span id="XDT"></span></a>O arquivo applicationHost.xdt
...
### <span id="deploy"></span></a>Implantação da extensão do site

Para instalar a extensão do site, é possível usar o FTP para copiar todos os arquivos do aplicativo Web para a pasta `\SiteExtensions\[your-extension-name]` na qual você deseja instalar a extensão. Não se esqueça de copiar o arquivo ApplicationHost.xdt para esse local também.

Em seguida, no Portal de Sites do Windows Azure, vá até a guia **Configurar** do site que tem a extensão. Na seção **configurações do aplicativo**, adicione a chave `WEBSITE_PRIVATE_EXTENSIONS` e atribua a ela um valor igual a `1`.

![TransformSiteappSettings][TransformSiteappSettings]

Por fim, no Portal do Windows Azure, reinicie o site da web para habilitar sua extensão.
...
<!-- IMAGES -->
  ...
  [XDT]: http://msdn.microsoft.com/pt-BR/library/dd465326.aspx
