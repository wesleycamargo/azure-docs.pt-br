<properties 
	pageTitle="Transformar e estender o site" 
	description="TBD" 
	authors="cephalin" 
	writer="cephalin" 
	editor="mollybos" 
...
Usando declarações [XDT](http://msdn.microsoft.com/library/dd465326.aspx) (XML Document Transformation), você pode transformar o arquivo [ApplicationHost.config](http://www.iis.net/learn/get-started/planning-your-iis-architecture/introduction-to-applicationhostconfig) nos sites do Microsoft Azure. Também é possível usar declarações XDT para adicionar extensões de site particulares a fim de habilitar ações de administração de site personalizadas. Este artigo inclui uma extensão de site do PHP Manager de exemplo que permite o gerenciamento das configurações de PHP por meio de uma interface da Web.
...
* [Estender o site](#extend)
	...
	* [Exemplo de extensão do site: Gerenciador de PHP](#SiteSample)
		* [O aplicativo Web do Gerenciador de PHP](#PHPwebapp)
		* [O arquivo applicationHost.](#XDT)
...
A plataforma Sites do Azure fornece flexibilidade e controle para configuração do site. Embora o arquivo de configuração ApplicationHost.config do IIS padrão não esteja disponível para edição direta nos Sites do Microsoft Azure, a plataforma dá suporte a um modelo de transformação ApplicationHost.config declarativo com base no XDT (XML Document Transformation).

Para aproveitar essa funcionalidade de transformação, você cria um arquivo ApplicationHost.xdt com conteúdo XDT e o coloca na raiz do site. Em seguida, na página **Configurar** do Portal do Microsoft Azure, você define a configuração de aplicativo  `WEBSITE_PRIVATE_EXTENSIONS` como 1 (talvez você precise reiniciar o site). 
...
Para obter exemplos adicionais, consulte [https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions](https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions).
...
2. **Declaração** de extensão do site: cria um arquivo applicationHost.
3. **Implantação** de extensão do site: coloca o conteúdo na pasta SiteExtensions em `root`
4.  **Habilitação** de extensão do site: define a configuração do aplicativo `WEBSITE_PRIVATE_EXTENSIONS` para 1
...
Em seguida, no Portal de Sites do Microsoft Azure, vá até a guia **Configurar** do site que tem a extensão. Na seção **configurações do aplicativo**, adicione a chave  `WEBSITE_PRIVATE_EXTENSIONS` e atribua a ela um valor igual a `1`.

![TransformSiteappSettings][TransformSiteappSettings]

Por fim, no Portal do Microsoft Azure, reinicie o site da web para habilitar sua extensão.
