...
Este tutorial se baseia nas etapas e no código de exemplo do tutorial anterior [Introdução][Introdução] ou [Introdução aos dados][Introdução aos dados]. Antes de iniciar este tutorial, você deve primeiro concluir o tutorial [Introdução][Introdução] ou [Introdução aos dados][Introdução aos dados].
...
1.  No Gerenciador de Soluções do Visual Studio, no projeto cliente todolist, abra o arquivo MainPage.xaml e substitua o elemento **CheckBoxComplete** pelas definições abaixo. Em seguida, salve o arquivo. Isso altera o manipulador de eventos em **CheckBoxComplete** para que tratemos do `click` evento. Também podemos adicionar um bloco de texto ao lado da caixa de seleção e vinculá-lo ao carimbo de data/hora da data de conclusão.
