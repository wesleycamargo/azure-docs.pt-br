<properties linkid="manage-services-hdinsight-howto-upload-data-to-hdinsight" urlDisplayName="Carregar dados" pageTitle="Carregar dados no HDInsight | Azure" metaKeywords="" description="Saiba como carregar e acessar dados no HDInsight usando o Gerenciador de Armazenamento do Azure, o console interativo, a linha de comando do Hadoop ou o Sqoop." metaCanonical="" services="" documentationCenter="" title="Carregar dados no HDInsight" authors=""  solutions="" writer="jgao" manager="paulettm" editor="cgronlun"  />



#Carregar dados no HDInsight

O Azure HDInsight oferece um HDFS (Sistema de Arquivos Distribuído) do Hadoop completo no Armazenamento de Blob do Azure. Ele foi projetado como uma extensão HDFS para fornecer uma experiência perfeita aos clientes, permitindo que o conjunto completo de componentes no ecossistema do Hadoop opere diretamente nos dados que ele gerencia. O Armazenamento de Blob do Azure e o HDFS são sistemas de arquivos distintos que são otimizados para armazenamento de dados e computação nesses dados. Para obter os benefícios do uso do Armazenamento de Blob do Azure, consulte [Usar o Armazenamento de Blob do Azure com HDInsight][hdinsight-storage] 

Os clusters do Azure HDInsight normalmente são implantados para executar trabalhos MapReduce e são descartados quando esses trabalhos são concluídos. A manutenção dos dados nos clusters HDFS após a conclusão dos cálculos seria uma maneira cara de armazenar esses dados. O Armazenamento de Blob do Azure é altamente disponível, escalonável, com alta capacidade, baixo custo e opção de armazenamento para dados a serem processados usando o HDInsight. O armazenamento de dados em um Blob permite que os clusters HDInsight usados para computação sejam liberados sem que ocorra perda de dados. 

O Armazenamento de Blob do Azure pode ser acessado por meio de [AzCopy][azure-azcopy], [PowerShell do Azure][azure-powershell], [Biblioteca de Cliente de armazenamento do Azure para .NET][azure-storage-client-library] ou por meio de ferramentas do explorer. Estas são algumas das ferramentas disponíveis:
...
* [Carregar dados no Armazenamento de Blob usando o Azure Explorer](#storageexplorer)
* [Carregar dados no Armazenamento de Blob usando a linha de comando do Hadoop](#commandline)
* [Importar dados do Banco de Dados SQL do Azure para armazenamento de Blob usando o Sqoop](#sqoop)

##<a id="azcopy"></a>Carregar dados no Armazenamento de Blob usando o AzCopy##

O AzCopy é um utilitário de linha de comando desenvolvido para simplificar a tarefa de transferir dados de e para uma conta de Armazenamento do Azure. Você pode usá-lo como uma ferramenta independente ou incorporar esse utilitário em um aplicativo existente. [Baixe o AzCopy][azure-azcopy-download].

A sintaxe do AzCopy é:

	AzCopy <Source> <Destination> [filePattern [filePattern...]] [Opções]

Para obter mais informações, consulte [AzCopy - Carregando e baixando arquivos para Blobs do Azure][azure-azcopy]

##<a id="powershell"></a>Carregar dados no Armazenamento de Blob usando o PowerShell do Azure##

O PowerShell do Azure é um ambiente de script poderoso que você pode usar para controlar e automatizar a implantação e o gerenciamento de suas cargas de trabalho no Azure. Você pode usar o PowerShell Azure para carregar dados no Armazenamento de Blob, para que os dados possam ser processados por trabalhos MapReduce. Para obter informações sobre como configurar sua estação de trabalho para executar o PowerShell do Azure, consulte [instalar e configurar o PowerShell do Azure][powershell-install-configure].
...
- No caso de pastas vazias, eles servem como um marcador da existência da pasta. O Armazenamento de blob é inteligente o suficiente para saber que se existir um blob denominado foo/bar, haverá uma pasta chamada foo. Mas se você desejar ter uma pasta vazia chamada foo, a única maneira de indicar isso é tendo esse arquivo de 0 byte especial no lugar.
- Eles contêm alguns metadados especiais necessários para o sistema de arquivos Hadoop, especialmente as permissões e os proprietários das pastas.
...
##<a id="storageexplorer"></a>Carregar dados no Armazenamento de Blob usando o Azure Explorer

O *Azure Storage Explorer* é uma ferramenta útil para inspecionar e alterar os dados em seu Armazenamento do Azure. É uma ferramenta gratuita que pode ser baixada em [http://azurestorageexplorer.codeplex.com/](http://azurestorageexplorer.codeplex.com/ "Azure Storage Explorer")

Antes de usar a ferramenta, você deve conhecer sua chave de conta e nome de conta do Armazenamento do Azure. Para obter instruções sobre como obter essas informações, consulte a seção "Como exibir, copiar e regenerar chaves de acesso de armazenamento [Gerenciar contas de armazenamento][azure-storage-account]  
...
12. O exemplo a seguir demonstra como copiar o arquivo davinci.txt do sistema de arquivos local no nó principal HDInsight no diretório /example/data.

		hadoop dfs -copyFromLocal C:\temp\davinci.txt /example/data/davinci.txt

	Como o sistema de arquivos padrão está no armazenamento de Blob do Azure, /example/datadavinci.txt, na verdade, está no armazenamento de Blob do Azure.  Você também pode fazer referência ao arquivo como:
...
Antes de importar dados, você deve saber o nome do servidor de Banco de Dados SQL do Azure, o nome da conta do banco de dados, a senha da conta e o nome do banco de dados. 
...
3. Cole o script na janela do console do PowerShell do Azure para executá-lo. Consulte [Introdução ao HDInsight][hdinsight-getting-started] para obter um exemplo do PowerShell para recuperar o arquivo de dados no Armazenamento de Blob do Azure.

Para obter mais informações sobre como usar o Sqoop, consulte [Usar o Sqoop com o HDInsight][hdinsight-sqoop].
...
[azure-storage-account]: /pt-br/manage/services/storage/how-to-manage-a-storage-account/
