...
> [!NOTE]
> O procedimento descrito neste artigo só será relevante se você não tiver selecionado a instalação da edição de comunidade do RStudio Server ao provisionar o cluster.  Se ele tiver sido adicionado durante o provisionamento, acesse-o clicando no bloco **Painéis do Servidor R** na entrada do Portal do Azure de seu cluster e, em seguida, no bloco **R Studio Server**. 
...
7. Crie um túnel SSH para o cluster, mapeando `ssh -L localhost:8787:localhost:8787 USERNAME@CLUSTERNAME-ed-ssh.azurehdinsight.net` no cluster HDInsight para o computador cliente. Você deve criar um túnel SSH antes de abrir uma nova sessão do navegador.
