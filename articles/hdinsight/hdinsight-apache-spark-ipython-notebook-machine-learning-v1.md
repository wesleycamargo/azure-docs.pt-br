...
# Criar aplicativos de Aprendizado de Máquina usando o Apache Spark no Azure HDInsight (Windows)

> [AZURE.NOTE]O HDInsight agora fornece clusters do Spark no Linux. Para obter informações sobre como criar um aplicativo de aprendizado de máquina em clusters do Spark no HDInsight no Linux, confira [Criar aplicativos de Aprendizado de Máquina usando o Apache Spark no Azure HDInsight (Linux)](hdinsight-apache-spark-ipython-notebook-machine-learning.md).
...
![Instantâneo de dados de HVAC](./media/hdinsight-apache-spark-ipython-notebook-machine-learning-v1/hdispark.ml.show.data.png "Instantâneo dos dados de HVAC")
...
2. Crie um novo bloco de anotações. Clique em **Novo** e, em seguida, clique em **Python 2**.
	...
	![Criar um novo bloco de anotações do Jupyter](./media/hdinsight-apache-spark-ipython-notebook-machine-learning-v1/hdispark.note.jupyter.createnotebook.png "Criar um novo bloco de anotações do Jupyter")
...
3. Um novo bloco de anotações é criado e aberto com o nome Untitled.pynb. Clique no nome do bloco de anotações na parte superior e digite um nome amigável.
	...
	![Fornecer um nome para o bloco de anotações](./media/hdinsight-apache-spark-ipython-notebook-machine-learning-v1/hdispark.note.jupyter.notebook.name.png "Fornecer um nome para o bloco de anotações")
...
3. Comece a criar seu aplicativo de aprendizado de máquina. Neste aplicativo, usamos um pipeline ML do Spark para executar uma classificação de documento. No pipeline, vamos dividir o documento em palavras, converter as palavras em um vetor de recurso numérico e, finalmente, criar um modelo de previsão usando as etiquetas e vetores de recurso.
	...
	Toda vez que você executar um trabalho no Jupyter, o título da janela do navegador da Web mostrará um status **(Ocupado)** junto com o título do bloco de anotações. Você também verá um círculo preenchido ao lado do texto **Python 2** no canto superior direito. Depois que o trabalho for concluído, isso será alterado para um círculo vazio.
	 ...
	 ![Status de um trabalho do bloco de anotações do Jupyter](./media/hdinsight-apache-spark-ipython-notebook-machine-learning-v1/hdispark.jupyter.job.status.png "Status de um trabalho do bloco de anotações do Jupyter")
...
7. Verifique o documento de treinamento para o ponto de verificação de seu progresso com o aplicativo. Cole o trecho a seguir em uma célula vazia e pressione **SHIFT + ENTER**.
	...
	![Instantâneo de dados de HVAC](./media/hdinsight-apache-spark-ipython-notebook-machine-learning-v1/hdispark.ml.show.data.first.row.png "Instantâneo dos dados de HVAC")
...
11. Agora você pode sair do bloco de anotações reiniciando o kernel. Na barra de menus superior, clique em **Kernel**, clique em **Reiniciar** e em **Reiniciar** novamente no prompt.

	![Reiniciar o kernel do Jupyter](./media/hdinsight-apache-spark-ipython-notebook-machine-learning-v1/hdispark.jupyter.restart.kernel.png "Reiniciar o kernel do Jupyter")
