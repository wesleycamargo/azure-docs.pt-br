...
Este artigo explica como instalar o Presto em clusters do Adobe HDInsight baseados em Hadoop usando ações de script. Você também aprenderá como instalar o Airpal em um cluster Presto do HDInsight existente.
...
3. Execute uma consulta em uma tabela de exemplo, **hivesampletable**, que está disponível em todos os clusters do HDInsight por padrão:
    ...
    Por padrão, os conectores [Apache Hive](https://prestodb.io/docs/current/connector/hive.html) e [TPCH](https://prestodb.io/docs/current/connector/tpch.html) já vêm configurados. O conector Hive é configurado para usar a instalação padrão do Hive. Portanto, todas as tabelas do Hive são automaticamente visíveis no Presto.
