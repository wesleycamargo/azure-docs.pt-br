---
title: Provisionar clusters do HBase em uma Rede Virtual | Microsoft Docs
...
> 
> Este documento também inclui informações sobre como usar o Azure PowerShell. Os trechos de código fornecidos baseiam-se em comandos que usam o ASM (Gerenciamento de Serviço do Azure) para funcionar com o HDInsight e foram **preteridos**. Estes comandos serão removidos até 1º de janeiro de 2017.
> 
> Para obter uma versão deste documento que usa o portal do Azure, juntamente com os trechos de código do PowerShell que usam o ARM (Azure Resource Manager), veja [Provisionar clusters HBase na Rede Virtual do Azure](hdinsight-hbase-provision-vnet.md).
> 
> 
...
**Para criar uma rede virtual usando o Portal Clássico do Azure**
...
> [!NOTE]
> Os clusters do HDInsight usam o armazenamento de blob do Azure para armazenar dados. Para obter mais informações, consulte [Usar o armazenamento de blob do Azure com o Hadoop no HDInsight](hdinsight-hadoop-use-blob-storage.md). Você precisará de uma conta de armazenamento e um contêiner de armazenamento de blob. O local da conta de armazenamento deve corresponder ao local da rede virtual e ao local do cluster.
> 
> 

Como outros clusters do HDInsight, o cluster HBase requer uma conta de armazenamento do Azure e um contêiner de armazenamento de Blob como o sistema de arquivos padrão. O local da conta de armazenamento deve corresponder ao local da rede virtual e ao local do cluster. Para obter mais informações, consulte [Usar o armazenamento de blob do Azure com o Hadoop no HDInsight][hdinsight-storage]. Quando você provisiona um cluster HBase, você tem as opções de criar um novo ou usar os existentes. Este procedimento mostra como criar uma conta de armazenamento e um contêiner de armazenamento de Blobs usando o Portal Clássico do Azure.
...
> [!NOTE]
> Para obter informações sobre como provisionar um novo cluster do HBase usando o PowerShell do Azure, consulte [Provisionar um cluster HBase usando o PowerShell do Azure](#powershell).
> 
> 
...
1. Na página **Configurar Cluster**, digite ou selecione os valores a seguir:
...
2. Insira o nome de usuário e senha do Hadoop a utilizar para este cluster e, em seguida, clique no botão à direita.
...
3. Na página **Conta de Armazenamento**, forneça os seguintes valores:
...
4. Na página **Ações de Script** selecione a marca de seleção no canto inferior direito. Não clique no botão **Adicionar ação de script**, já que este tutorial não requer uma configuração de cluster personalizado.
   
    ![Configurar a Ação de Script para personalizar um cluster HBase do HDInsight][img-provision-cluster-page5]
   
   > [!NOTE]
   > Esta página pode ser usada para personalizar o cluster durante a instalação. Para obter mais informações, consulte [Personalizar clusters HDInsight usando a Ação de Script](hdinsight-hadoop-customize-cluster.md).
   > 
   > 
...
2. Ao usar um aplicativo Java para se conectar ao HBase remotamente, você deve usar o nome de domínio totalmente qualificado (FQDN). Para determiná-lo, é preciso obter o sufixo DNS específico da conexão do cluster do HBase. Para fazer isso, use o Curl para consultar o Ambari, ou use a área de trabalho remota para conectar-se ao cluster.
   ...
   * **PowerShell do Azure** - use o seguinte script do PowerShell do Azure para registrar a função **Get-ClusterDetail**, que pode ser usada para retornar o sufixo DNS:
   ...
   > [!NOTE]
   > Você também pode usar a área de trabalho remota para conectar o cluster do HBase (você estará conectado ao nó principal) e executar **ipconfig** por meio de um prompt de comando para obter o sufixo DNS. Para obter instruções sobre como habilitar o protocolo RDP (Protocolo de Área de Trabalho Remota) e se conectar ao cluster usando o RDP, veja [Gerenciar clusters do Hadoop no HDInsight usando o Portal Clássico do Azure][hdinsight-admin-portal].
   > 
...
4. Para validar o cluster, você pode verificar o cluster no Portal Clássico do Azure ou executar o seguinte cmdlet do Azure PowerShell no painel inferior:
