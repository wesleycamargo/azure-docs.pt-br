...
Para concluir este tutorial e usar as ferramentas do Data Lake no Visual Studio, você precisa dos seguintes itens:

* Um cluster Azure HDInsight: para criar um, confira [Introdução ao uso do Hadoop no Azure HDInsight](apache-hadoop-linux-tutorial-get-started.md). Para saber como executar consultas de Hive interativas, você precisa de um cluster de [Consulta interativa do HDInsight](../interactive-query/apache-interactive-query-get-started.md).
* Uma estação de trabalho com o Visual Studio 2013/2015/2017.
...
## <a name="install-and-upgrade-data-lake-tools-for-visual-studio"></a>Instalar e atualizar as Ferramentas do Azure Data Lake para Visual Studio

As Ferramentas do Data Lake estão instaladas por padrão no Visual Studio 2017. Em versões antigas do Visual Studio, você pode instalá-las usando o [Web Platform Installer](https://www.microsoft.com/web/downloads/). Você deve escolher aquela que corresponde à sua versão do Visual Studio. Se não tiver instalado o Visual Studio, você poderá instalar a mais recente Visual Studio Community e o SDK do Azure usando o [Web Platform Installer](https://www.microsoft.com/web/downloads/):
...
3. Expanda **Atualizações**, atualize **Ferramentas do Azure Data Lake e do Stream Analytics**, se houver alguma.

> [!NOTE]
>
> Somente a versão 2.3.0.0 ou posterior oferece suporte à conexão a clusters de Consulta Interativa e à execução de consultas interativas do Hive.
...
3. Expanda **Azure** e expanda **HDInsight**.
   ...
   > 
   > 
4. Insira as credenciais de assinatura do Azure e clique em **Entrar**. A autenticação só será necessária se você nunca tiver se conectado à assinatura do Azure pelo Visual Studio nesta estação de trabalho.
...
Depois de se conectar à assinatura do Azure, você poderá fazer as seguintes tarefas:
...
A [Consulta Interativa](../interactive-query/apache-interactive-query-get-started.md) aproveita o [Hive no LLAP](https://cwiki.apache.org/confluence/display/Hive/LLAP) no Apache Hive 2.1 e traz a interatividade às suas consultas de estilo de data warehouse complexas em grandes conjuntos de dados armazenados. A execução de consultas Hive na Consulta Interativa é muito mais rápida em comparação aos trabalhos de lote tradicionais de Hive.  Para saber mais sobre como executar trabalhos de lote do Hive, consulte [Executar trabalhos de lote do Hive](#run-hive-batch-jobs).

> [!note]
>
> A execução de consultas interativas do Hive tem suporte apenas quando você se conecta a um cluster de [Consulta Interativa HDInsight](../interactive-query/apache-interactive-query-get-started.md).
...
Todos os clusters HDInsight são fornecidos com uma tabela Hive de exemplo chamada *hivesampletable*. Esta tabela do Hive é usada para mostrar como listar tabelas do Hive, exibir os esquemas de tabela e listar as linhas na tabela Hive.
...
3. Digite as consultas Hive. Observe que o editor Hive dá suporte a IntelliSense. Agora as Ferramentas do Data Lake para Visual Studio dão suporte à obtenção de metadados remotos quando você edita o script do Hive. Por exemplo, quando você digitar "SELECT * FROM", o IntelliSense listará todos os nomes de tabela sugeridos. Quando um nome de tabela for especificado, os nomes de coluna serão listados pelo IntelliSense. As ferramentas dão suporte a quase todas as instruções DML Hive, subconsultas e UDFs internos.
   ...
   > [!NOTE]
   > Apenas os metadados dos clusters selecionados na Barra de Ferramentas de HDInsight são sugeridos.
   > 
   > 
...
Além de melhorar o desempenho, os usuários também podem exibir o Hive em grafos Tez e os detalhes da Tarefa.
...
E você poderá ver os logs transmitidos em tempo real e ver os grafo de trabalho se a consulta do Hive for executada no Tez.

![Ferramentas do Data Lake para Visual Studio, execução por caminho rápido de fast](./media/apache-hadoop-visual-studio-tools-get-started/hdinsight.fast.path.hive.execution.png "Exibir grafos do trabalho")
...
As Ferramentas do Data Lake dão suporte à exibição de grafos de desempenho para os trabalhos Hive executados pelo mecanismo de execução Tez. Para saber mais sobre como habilitar o Tez, consulte [Usar o Hive no HDInsight](hdinsight-use-hive.md). Depois de enviar um trabalho do Hive no Visual Studio, o Visual Studio mostrará o grafo quando o trabalho for concluído.  Talvez seja necessário clicar no botão **Atualizar** para obter o status mais recente do trabalho.

> [!NOTE]
> Esse recurso só está disponível para o cluster HDInsight em versões posteriores a 3.2.4.593 e funciona somente para trabalhos concluídos (Se você enviar seu trabalho por meio do WebHCat. Esse grafo mostra quando você executa a consulta por meio de HiveServer2). 
> 
> 

![grafo de desempenho do hive tez do hadoop](./media/apache-hadoop-visual-studio-tools-get-started/hdinsight.hive.tez.performance.graph.png "Status do trabalho")

Para ajudar você a entender melhor a consulta do Hive, as ferramentas adicionan a Exibição de Operador do Hive nesta versão. Basta clicar duas vezes nos vértices do grafo do trabalho para ver todos os operadores dentro do vértice. Você também pode passar o mouse sobre um operador específico para exibir mais detalhes desse operador.
...
Demora para testar o script do Hive em um cluster HDInsight, com exceção do cluster da Consulta Interativa. Pode levar vários minutos ou mais. As Ferramentas do Data Lake para Visual Studio podem validar o script do Hive localmente sem se conectar a um cluster ativo. Para saber mais sobre como executar consultas interativas, consulte [Executar consultas interativas de Hive](#run-interactive-hive-queries).

As Ferramentas do Data Lake para Visual Studio também permitem que os usuários vejam o que há dentro do trabalho do Hive coletando e identificando os logs do YARN de determinados trabalhos do Hive.

Para saber mais sobre como executar trabalhos de lote do Hive, consulte a seção [Executar consultas interativas de Hive](#run-interactive-hive-queries). As informações nessa seção se aplicam à execução de Trabalhos em Lotes do Hive com mais tempo.
...
* Atualmente, os resultados do HiveServer2 são exibidos na forma de texto simples, o que não é ideal. A Microsoft está trabalhando para corrigir isso.
* Se os resultados forem iniciados com valores NULL, atualmente os resultados não serão mostrados. Podemos corrigir esse problema e, se você estiver preso nesse problema, entre em contato com a equipe de suporte.
