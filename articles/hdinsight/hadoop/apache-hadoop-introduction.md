...
* **[Pré-visualização de clusters de domínio associado](../domain-joined/apache-domain-joined-introduction.md)**: um cluster associado a um domínio do Active Directory para que você possa controlar o acesso e fornecer governança de dados.
...
* [Introdução ao Hadoop no HDInsight](apache-hadoop-linux-tutorial-get-started.md): um tutorial de início rápido para provisionamento de clusters do Hadoop no HDInsight e execução de consultas Hive de exemplo.
* [Introdução ao Spark no HDInsight](../spark/apache-spark-jupyter-spark-sql.md): um tutorial de início rápido para criar um cluster Spark e executar consultas interativas do Spark SQL.
* [Usar Servidor R no HDInsight](../r-server/r-server-get-started.md): comece a usar o Servidor R no HDInsight Premium.
