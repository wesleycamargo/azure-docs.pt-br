...
- **Chaves SSH (Secure Shell)**. Se você quiser acesso remoto em um cluster do Linux usando SSH com uma chave, em vez de senha, o método recomendado é usar uma chave, pois é mais segura. Para obter instruções sobre como gerar chaves SSH, consulte os seguintes artigos:
...
###<a id="cli"></a> Usar a interface de linha de comando do Azure (CLI do Azure)

Outra opção para o provisionamento de um cluster HDInsight é a CLI do Azure. A CLI do Azure é implementada no Node. js. Ela pode ser usada em qualquer plataforma que dá suporte a Node. js, incluindo Windows, Mac e Linux. Você pode instalar a CLI do Azure nos seguintes locais:
...
- Instalar a CLI do Azure usando o NPM (Gerenciador de Pacotes Node.js)
...
Antes de usar a CLI do Azure, você deve configurar a conectividade entre a estação de trabalho e o Azure. As informações da assinatura do Azure são utilizadas pela CLI do Azure para se conectar à sua conta. Essas informações podem ser obtidas no Azure em um arquivo de configurações de publicação. O arquivo de configurações de publicação pode, então, ser importado como uma definição de configuração local persistente que a ferramenta usará para operações subsequentes. Você precisa importar suas configurações de publicação apenas uma vez.
...
Execute os seguintes procedimentos para configurar seu computador Windows para usar a interface de linha de comando do Azure (CLI do Azure).

- Instalar a CLI do Azure usando o NPM ou instalador do Windows
- Baixar e importar as configurações de publicação da conta do Azure


A CLI do Azure pode ser instalada usando o NPM ou o Instalador do Windows. A Microsoft recomenda que você a instale usando apenas uma das duas opções.
...
#### <a id="cliprovision"></a>Provisionar clusters HDInsight usando a CLI do Azure
...
Normalmente, você provisiona um cluster HDInsight, executa os trabalhos e exclui o cluster para reduzir o custo. A CLI do Azure oferece a opção de salvar as configurações em um arquivo, de modo que você possa reutilizá-lo toda vez que provisionar um cluster.
