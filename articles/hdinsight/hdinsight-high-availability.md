...
O HDInsight remove esse ponto único de falha, com a adição de um nó principal secundário (Nó Principal1). Nós [ZooKeeper](http://zookeeper.apache.org/) (ZKs) foram adicionados e são usados para eleição de líder de nós principais, e para assegurar que os nós de trabalho e gateways (GWS) saibam quando fazer failover no nó principal secundário (Nó Principal1) quando o nó principal ativo (Node0) tornar-se inativo.
