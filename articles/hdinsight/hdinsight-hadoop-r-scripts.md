...
> [AZURE.NOTE]As etapas deste documento exigem um cluster HDInsight baseado em Windows. Para obter informações sobre como usar o R com um cluster baseado no Linux, veja [Instalar e usar o R em clusters do Hadoop no HDinsight (Linux)](hdinsight-hadoop-r-scripts-linux.md).
...
Um [exemplo de script](https://hdiconfigactions.blob.core.windows.net/rconfigactionv02/r-installer-v02.ps1) para instalar o R em um cluster HDInsight está disponível em um blob somente leitura no armazenamento do Azure. Esta seção oferece instruções sobre como usar o script de exemplo ao provisionar o cluster usando o portal de visualização do Azure.
...
1. Quando você provisionar um cluster HDInsight no portal de visualização, clique em **Configuração Opcional** e em **Ações de Script**.
...
**Para criar um aplicativo .NET no Visual Studio**
