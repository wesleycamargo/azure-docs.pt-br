...
> [AZURE.NOTE] Você também pode provisionar os clusters Storm usando a [CLI do Azure](../xplat-cli-install.md) ou o [Azure PowerShell](../powershell-install-configure.md).
...
__Para o Storm baseado em Linux nos clusters HDInsight__, você pode conectar o cluster usando o SSH e usar o comando `storm` para iniciar e gerenciar as topologias. Além disso, você pode usar a Ambari para monitorar o serviço Storm e a profusão de IU para monitorar e gerenciar as topologias em execução.
...
Os exemplos storm-starter estão localizados no diretório __ /usr/hdp/current/storm-client/contrib/storm-starter__ nos clusters baseados em Linux e o diretório **%storm_home%\contrib\storm-starter**, nos clusters baseados no Windows.
