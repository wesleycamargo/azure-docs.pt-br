...
**com.microsoft.example.EventHubReader** lê dados do Hub de eventos (os dados gravados pelo EventHubWriter) e os armazena no HDFS (WASB nesse caso, uma vez que isso foi gravado e testado com o Azure HDInsight) no diretório /devicedata.
...
2. Na tela **Adicionar um novo Hub de Eventos**, insira um **Nome do Hub de Eventos**, selecione a **Região** na qual criar o hub e crie um novo namespace ou selecione um existente. Clique na **Seta** para continuar.
   ...
   > 
   > 
3. Na tela **Configurar o Hub de Eventos**, insira os valores de **Contagem de partições** e **Retenção de Mensagem**. Para este exemplo, use uma contagem de partições de 10 e uma retenção de mensagens de 1. Observe a contagem de partições, pois você precisará desse valor posteriormente.
...
2. Use o seguinte para criar e empacotar o projeto:
    ...
    Isso baixará as dependências exigidas, compilará e então empacotará o projeto. A saída será armazenada no diretório **/target** como **EventHubExample-1.0-SNAPSHOT.jar**.
...
2. Após a conclusão do carregamento do arquivo, use o SSH para se conectar ao cluster HDInsight. Substitua o **USERNAME** pelo nome do seu logon SSH. Substitua o **CLUSTERNAME** pelo nome do seu cluster HDInsight:
   ...
   > 
    ...
    Se você estiver usando o PuTTY, insira `CLUSTERNAME-ssh.azurehdinsight.net` no campo **Nome do Host (ou endereço IP)** e clique em **Abrir** para se conectar. Será solicitado que você insira seu nome de conta SSH.
   
   > [!NOTE]
   > Se você tiver usado uma senha para sua conta SSH, a inserção da senha poderá ser solicitada. Se você tiver usado uma chave SSH com a conta, talvez seja necessário usar as etapas a seguir para selecionar a chave:
   > 
...
7. No **Console de Consulta**, selecione **Editor do Hive** e substitua o padrão `select * from hivesampletable` pelo seguinte:
    ...
    Clique em **Selecionar** para executar a consulta. Isso retornará 10 linhas dos dados gravados no Armazenamento do Azure (WASB) pelo EventHubReader. Assim que a consulta for concluída, você deverá ver dados semelhantes aos seguintes:
