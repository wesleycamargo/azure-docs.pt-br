...
> 
> Para obter uma versão deste documento que usa o portal do Azure, juntamente com os trechos de código do PowerShell que usam o ARM (Azure Resource Manager), veja [Instalar Giraph em clusters do HDInsight](hdinsight-hadoop-giraph-install.md).
> 
> 

**Artigos relacionados**

* [Instalar o Giraph em clusters HDInsight](hdinsight-hadoop-giraph-install.md): instale o Giraph usando o Portal do Azure
...
1. Comece a criar um cluster usando a opção **CRIAÇÃO PERSONALIZADA**, conforme descrito em [Criar clusters Hadoop no HDInsight usando opções de personalização](hdinsight-provision-clusters.md#portal).
2. Na página **Ações de Script** do assistente, clique em **Adicionar ação de script** para fornecer detalhes sobre a ação de script, como mostrado abaixo:
    ...
    ![Usar Ação de Script para personalizar um cluster](./media/hdinsight-hadoop-giraph-install-v1/hdi-script-action-giraph.png "Usar Ação de Script para personalizar um cluster")
...
1. Carregue um arquivo de dados de exemplo no armazenamento de Blob do Azure. Em sua estação de trabalho local, crie um novo arquivo chamado **tiny\_graph.txt**. Ele deve conter as seguintes linhas:
    ...
    ![tiny\_graph.txt Desenhado como círculos com linhas de distância variável entre](./media/hdinsight-hadoop-giraph-install-v1/giraph-graph.png)
2. Execute o exemplo SimpleShortestPathsComputation. Use os seguintes cmdlets do PowerShell do Azure para executar o exemplo, usando o arquivo tiny\_graph.txt como entrada. Isso requer que você tenha instalado e configurado o [PowerShell do Azure][powershell-install].
...
3. Exibir os resultados. Concluído o trabalho, os resultados serão armazenados em dois arquivos de saída na pasta **wasbs:///example/out/shotestpaths**. Os arquivos se chamam **part-m-00001** e *_part-m-00002\*\_. Execute as etapas a seguir para baixar e exibir a saída:
    ...
    ![Desenho dos objetos como círculos com os percursos mais curtos entre](./media/hdinsight-hadoop-giraph-install-v1/giraph-graph-out.png)
