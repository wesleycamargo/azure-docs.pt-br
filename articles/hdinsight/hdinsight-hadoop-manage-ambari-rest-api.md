...
## API REST
O URI de base para a API REST do Ambari no HDInsight é https://CLUSTERNAME.azurehdinsight.net/api/v1/clusters/CLUSTERNAME, no qual **NOMEDOCLUSTER** é o nome do cluster.
...
O exemplo a seguir usa cURL para fazer uma solicitação GET na API REST. Substitua **PASSWORD** pela senha do administrador do cluster. Substitua **CLUSTERNAME** pelo nome do cluster:
...
3. Use o comando Carregar para armazenar um arquivo no contêiner:
    ...
    Por exemplo, se você quiser exibir o arquivo no HDInsight em wasbs://example/data/filename.txt, **CAMINHODOBLOB** deve ser `example/data/filename.txt`.
...
2. Recupere a configuração do componente e da marca usando o comando a seguir. Substitua **spark-thrift-sparkconf** e **INITIAL** pelo componente e pela marca cuja configuração você deseja recuperar.
   ...
   * Obtém o conteúdo da matriz `.items[]` e o adiciona sob o elemento **desired\_config**.
