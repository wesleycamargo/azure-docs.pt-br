...
> [!IMPORTANT]
> Os exemplos neste documento foram testados usando o PowerShell no Windows 10 e Bash. Em muitos casos, o mesmo comando funciona em ambos. Nos casos em que há diferença, são fornecidos exemplos tanto do PowerShell quanto do Bash.
...
* [cURL](http://curl.haxx.se/): o cURL é um utilitário de plataforma cruzada que pode ser usado para trabalhar com APIs REST na linha de comando. Neste documento, ele é usado para se comunicar com a API REST do Ambari.
    ...
    > [!WARNING]
    > Se você estiver usando o PowerShell, deverá remover o alias padrão para o comando `curl` usando o comando `remove-item alias:curl`. Se você não remover o alias, receberá erros como “O parâmetro não pode ser processado porque o nome do parâmetro 'u' é ambíguo”.
...
* [CLI do Azure 2.0](https://docs.microsoft.com/cli/azure/install-az-cli2) (visualização): um utilitário de linha de comando de plataforma cruzada usado para trabalhar com serviços do Azure.
...
O PowerShell tem regras ligeiramente diferentes de uso de aspas simples e duplas. Use o seguinte comando do PowerShell:
...
Você pode usar essas informações com a [CLI do Azure 2.0](https://docs.microsoft.com/cli/azure/install-az-cli2) para carregar ou baixar dados do contêiner.

1. Use o seguinte comando para obter a ID exclusiva para a Conta de Armazenamento. No comando a seguir, substitua **NOMEDACONTA** pelo nome da Conta de Armazenamento recuperada do Ambari:

        az storage account list --query "[?name=='ACCOUNTNAME'].id" --out list

2. Use o seguinte para obter uma chave para a Conta de Armazenamento. Substitua **STORAGEID** pela ID da conta de armazenamento:
...
3. Use o comando Carregar para armazenar um arquivo no contêiner:
    ...
    Substitua **NOMEDACONTA** pelo nome da Conta de Armazenamento. Substitua **CHAVEDACONTA** pela chave recuperada anteriormente. Substitua __CONTAINER__ pelo nome do contêiner. **CAMINHODOARQUIVO** é o caminho para o arquivo que você deseja carregar, enquanto **CAMINHODOBLOB** é o caminho no contêiner.
