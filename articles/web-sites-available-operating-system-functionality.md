...
-   [Modos de site][Modos de site]
-   [Estruturas de desenvolvimento][Estruturas de desenvolvimento]
-   [Acesso a arquivos][Acesso a arquivos]
    -   [Unidades locais][Unidades locais]
    -   [Unidades de rede (também conhecidas como compartilhamentos UNC)][Unidades de rede (também conhecidas como compartilhamentos UNC)]
    -   [Arquivo em várias instâncias][Arquivo em várias instâncias]
    -   [Tipos de acesso a arquivos concedido a um aplicativo Web][Tipos de acesso a arquivos concedido a um aplicativo Web]
-   [Acesso à rede][Acesso à rede]
-   [Execução de código, processos e memória][Execução de código, processos e memória]
-   [Logs de diagnóstico e eventos][Logs de diagnóstico e eventos]
-   [Acesso ao Registro][Acesso ao Registro]
...
As areas de registro em log de diagnóstico e rastreamento que não estão disponíveis para aplicativos Web no Azure são eventos ETW Windows e logs de evento do Windows comuns (por exemplo, logs de evento de sistema, aplicativo e segurança). Como as informações de rastreamento ETW podem abranger todo o computador (com as ACLs certas), o acesso de leitura e gravação para eventos ETW permanece bloqueado. Os desenvolvedores podem notar que chamadas à API para ler e gravar eventos ETW e logs de eventos do Windows estão aparentemente funcionando, mas isso é porque os Sites do Azure estão "falsificando" as chamadas para que pareçam ter êxito. Na realidade, o código do aplicativo do site não tem acesso a esses dados de evento.
