<properties linkid="services-linux-cassandra-with-linux" urlDisplayName="Cassandra com Linux" pageTitle="Executar Cassandra com Linux no Azure" metaKeywords="" description="Explica como executar um cluster Cassandra no Linux em máquinas virtuais do Azure." metaCanonical="" services="virtual-machines" documentationCenter="Node.js" title="Executando Cassandra com Linux no Azure e acessando-a do Node.js" authors="" solutions="" manager="" editor="" />
...
## <a id="composite"> </a> Implantação composta ##
...
##<a id="deployment"> </a>Implantação de máquinas virtuais Azure##
...
##<a id="task1"> </a>Tarefa 1: Implantar o Cluster Linux##

Durante a versão de visualização das máquinas virtuais, para que as MVs do Linux façam parte da mesma rede virtual, todas as máquinas precisam ser implantadas para o mesmo serviço de nuvem. A sequência típica para a criação de um cluster é: 
...
<table>
	...
	<tr>
		<td>Nome da Máquina Virtual</td>
		<td>hk-cas1</td>
		<td>Este é o nome do organizador da MV</td>
	</tr>
	<tr>
		<td>Novo nome de usuário</td>
		<td>localadmin</td>
		<td>"admin" é um nome de usuário reservado no Ubuntu 12.xx</td>
	</tr>
	<tr>
		<td>Nova senha</td>
		<td><i>senha de alta segurança</i></td>
		<td></td>
	</tr>
	<tr>
	...
	<tr>
		<td>Tamanho</td>
		<td>Pequena</td>
		<td>Selecione a MV com base nas necessidades de IO. </td>
	</tr>
	<tr>
		<td>Proteja usando a chave SSH para autenticação</td>
		<td>Clique na caixa de seleção</td>
		<td>Verifique se você deseja proteger com uma chave SSH</td>
	</tr>
	<tr>
		<td>Certificado</td>
		<td><i>nome do arquivo do certificado de chave pública</i></td>
		<td>Chave pública SSH codificada em DER ou PEM gerada com OpenSSL ou outras ferramentas</td>
	</tr>
</table>
...
<table>
	...
	<tr>
		<td>MV Virtual autônoma</td>
		<td>caixa de rádio "verificação"</td>
		<td>Isso é para a primeira MV de MVs subsequentes, usaremos a opção "Conectar à MV existente"</td>
	</tr>
	<tr>
		<td>Nome DNS</td>
		<td><i>nome exclusivo</i>.cloudapp.net</td>
		<td>Dê um nome do balanceador de carga independente da máquina</td>
	</tr>
	<tr>
		<td>Conta de armazenamento</td>
		<td><i>conta de armazenamento padrão</i></td>
		<td>Use a conta de armazenamento padrão que você criou</td>
	</tr>
	<tr>
		<td>Região/grupo de afinidade/rede virtual</td>
		<td>Oeste dos EUA</td>
		<td>Selecione uma região na qual seus aplicativos da web acessam o cluster Cassandra</td>
	</tr>
</table>
...
**Etapa 3: Adicionar uma carga balanceada de extremidade final Thrift** 
...
a.	Na tela de detalhes da primeira MV, clique em "Adicionar ponto de extremidade"
...
<table>
	...
	<tr>
		<td>Nome</td>
		<td>cassandra</td>
		<td>Qualquer nome do ponto de extremidade exclusivo estará bem</td>
	</tr>
	<tr>
		<td>Protocolo</td>
		<td>TCP</td>
	...
	<tr>
		<td>Porta pública</td>
		<td>9160</td>
		<td>Porta Thrift padrão. </td>
	</tr>
	<tr>
		<td>Porta privada</td>
		<td>9160</td>
		<td>A menos que você alterou isso em cassandra.yaml</td>
	</tr>
</table>
...
**Etapa 2: Instalação de Cassandra**
...
O acima expandirá o arquivo no diretório da - [versão] apache-cassandra.   
...
5.	Conceda permissões de gravação para a identidade do usuário sob a qual Cassandra será executada	
		...
		Para usar o contexto do usuário atual, substitua os <user> e <group> com $USER e $GROUP
6.	Inicie o Cassandra do diretório de compartimento/[versão]-apache-cassandra usando o seguinte comando: 
...
**Etapa 3: Instalar o mx4j**

	a)	Baixe o mx4j: wget [http://sourceforge.net/projects/mx4j/files/MX4J%20Binary/3.0.2/mx4j-3.0.2.tar.gz/download](http://sourceforge.net/projects/mx4j/files/MX4J%20Binary/3.0.2/mx4j-3.0.2.tar.gz/download) -O mx4j.tar.gz
...
Agora que todas as MVs individuais estão prontas com o software necessário, é hora de estabelecer a comunicação entre os nós por meio da configuração de propagação. Analise as informações localizadas em [http://wiki.apache.org/cassandra/MultinodeCluster](http://wiki.apache.org/cassandra/MultinodeCluster) para obter detalhes sobre a configuração de cluster de vários nós. 

**Etapa 6: Configurar o Cluster de vários nós**
...
Os endereços de IP especificados aqui serão usados por novos nós para saber mais sobre a topologia de anel. Defina os nós mais confiáveis como suas sementes em um formato separado por vírgulas: "*host1*,*host2*". Configuração de exemplo: "hk-ub1,hk-ub2".
...
**Etapa 7: Testar o Cluster de vários nós**
...
##<a id="task3"> </a>Tarefa 3: Acessar o Cluster de Cassandra do Node.js##

Crie uma MV Linux no Azure usando o processo descrito nas tarefas anteriores.  Certifique-se de que essa MV seja uma MV autônoma, pois usaremos isso como um cliente para acessar o cluster do Cassandra. Instalaremos o Node.js, NPM e [cliente cassandra](https://github.com/racker/node-cassandra-client)  do github antes de conectar ao cluster do Cassandra dessa MV: 

**Etapa 1: Instalar o Node.js e NPM**

a)	Instalar os pré-requisitos 
...
**Etapa 2: Instalar o pacote do cliente cassandra**

	npm cassandra-client 

**Etapa 3: Preparar o armazenamento do Cassandra**
...
populateCustomerData() insere duas linhas na COLUMNFAMILY, ou seja, os clientes_cf. Na linguagem de consulta do Cassandra, a atualização irá inserir o registro se o registro já não estiver presente no processo tornando a instrução INSERIR CQL redundante. 
