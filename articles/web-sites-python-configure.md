...
Para obter um exemplo mais complexo, configurando o framework Django em sites do Azure, consulte o tutorial a seguir: 
...
Depois que a publicação Git estiver configurada, um repositório Git será criado e associado ao seu site.  A URL do repositório será exibida e poderá ser usada para enviar dados do ambiente de desenvolvimento local para a nuvem. Para publicar aplicativos via Git, certifique-se de que um cliente Git também esteja instalado e use as instruções fornecidas para enviar por push o conteúdo do site para sites do Azure.
...
Abaixo está o código para o manipulador WSGI básico. É semelhante ao sugerido pela especificação [PEP 3333](http://www.python.org/dev/peps/pep-3333/) como ponto de partida para um aplicativo compatível com WSGI. Salvamos o conteúdo em um arquivo chamado ConfigurePython.py criado em uma pasta ConfigurePython na raiz do site:
...
* *start_response*: um callable fornecido pelo servidor Web para transferência do cabeçalho de status e resposta HTTP
...
* O PYTHONPATH informa o Python sobre o diretório para pesquisar módulos. Os sites do Azure fornecem D:\home\site\wwwroot como um açúcar sintático apontando para a raiz do seu site. 
...
2.3. Adicionar um manipulador para o Python.
> Isso informará aos sites do Azure que o Python deve tratar as solicitações feitas para o caminho handler.fcgi. É importante que a sintaxe do manipulador pareça exatamente com o que temos dentro da marca &lt;handlers&gt; no exemplo abaixo, a menos que você traga seu próprio manipulador FastCGI ou pilha de desenvolvimento do Python.

2.4. Regravar as URLs no handler.fcgi.
