---
title: Entender como o Azure Policy executa auditorias dentro de uma máquina virtual
...
Antes de usar a Configuração de Convidado, você precisa registrar o provedor de recursos. Você pode fazer isso por meio do portal ou por meio do PowerShell.
...
Dentro da máquina virtual, o cliente de Configuração de Convidado usa ferramentas locais para executar a auditoria.
...
A definição de política **DeployIfNotExists** valida e corrige o seguinte:

- Verifique se uma configuração a ser avaliada foi atribuída à máquina virtual. Se não houver nenhuma atribuição no momento, obtenha a atribuição e prepare a máquina virtual:
...
Todas as políticas internas da Configuração de Convidado são incluídas em uma iniciativa para agrupar as definições a serem usadas em atribuições. A iniciativa interna denominada *[Versão prévia]: configurações de segurança de senha de auditoria dentro de Máquinas Virtuais do Linux e do Windows* contém 18 políticas. Há seis pares de **DeployIfNotExists** e **AuditIfNotExists** para o Windows e três para o Linux. Em cada caso, a lógica dentro da definição garante que somente o sistema operacional de destino seja avaliado com base na definição de [regra de política](definition-structure.md#policy-rule).
