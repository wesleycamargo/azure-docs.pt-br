...
Você pode fazer uso da propriedade `runAfter` para especificar que `postToErrorMessageQueue` deverá ser executado somente quando `readData` tiver **Falha**. Isso também poderia ser uma lista de valores possíveis e, portanto, `runAfter` poderia ser `["Succeeded", "Failed"]`.
