...
ms.locfileid: "34795645"
---
# <a name="using-a-partitioned-graph-in-azure-cosmos-db"></a>Usando um gráfico particionado no Azure Cosmos DB

Um dos principais recursos da API do Graph no Azure Cosmos DB é a capacidade de lidar com gráficos em larga escala por meio de escalabilidade horizontal. Esse processo é obtido por meio de [recursos de particionamento no Azure Cosmos DB](partition-data.md#how-does-partitioning-work), que faz uso de coleções, também conhecidas como contêineres, que podem ser dimensionadas de forma independente em termos de armazenamento e a taxa de transferência. O Azure Cosmos DB oferece suporte aos seguintes tipos de contêineres em todas as APIs:

- **Coleção fixa**: essas coleções podem armazenar um banco de dados de gráfico de até 10 GB com um máximo de 10.000 unidades de solicitação por segundo alocadas a ele. Para criar uma coleção fixa não é necessário especificar uma propriedade de chave de partição nos dados.

- **Coleção ilimitada**: essas coleções podem ser dimensionadas automaticamente para armazenar um gráfico além do limite de 10 GB por meio de particionamento horizontal. Cada partição armazenará 10 GB e os dados serão automaticamente balanceados com base na **chave de partição especificada**, que será um parâmetro obrigatório ao usar uma coleção ilimitada. Esse tipo de contêiner pode armazenar um tamanho de dados praticamente ilimitado e pode permitir até 100.000 unidades de solicitação por segundo, ou mais [se entrar em contato com o suporte](https://aka.ms/cosmosdbfeedback?subject=Cosmos%20DB%20More%20Throughput%20Request).
...
- **A configuração do particionamento será necessária** se a coleção tiver que ter mais de 10 GB e/ou se estiver alocando mais de 10.000 unidades de solicitação por segundo (RU/s).
- **Vértices e as bordas são armazenados como documentos JSON** no back-end de um contêiner de API do Graph do Azure Cosmos DB.
- **Vértices exigem uma chave de partição**. Essa chave determina em qual partição o vértice será armazenado por meio de um algoritmo de hash. O nome dessa chave de partição é uma cadeia de caracteres de uma palavra sem espaços ou caracteres especiais e é definido ao se criar uma nova coleção usando o formato `/partitioning-key-name` no portal.
...
A seguir estão as diretrizes que devem ser seguidas para garantir o melhor desempenho e escalabilidade ao usar gráficos particionados em coleções ilimitadas:
...
- **Escolha uma chave de partição que distribuirá uniformemente dados entre as partições**. Essa decisão depende muito do modelo de dados da solução. Leia mais sobre a criação de uma chave de partição apropriada em [Particionamento e dimensionamento no Azure Cosmos DB](partition-data.md).
