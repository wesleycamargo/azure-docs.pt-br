...
Assim como ocorre com outros recursos do modelo de recurso do Cosmos DB, os bancos de dados podem ser criados, substituídos, excluídos, lidos ou enumerados facilmente usando as [APIs REST](/rest/api/documentdb/) ou um dos [SDKs do cliente](documentdb-sdk-dotnet.md). O Cosmos DB garante uma coerência forte para leitura ou consulta dos metadados de um recurso do banco de dados. Excluir um banco de dados automaticamente garante que você não possa acessar qualquer uma das coleções ou usuários contidos nele.   
