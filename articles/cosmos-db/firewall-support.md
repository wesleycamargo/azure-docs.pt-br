...
## <a name="connections-from-azure-paas-service"></a>Conexões do serviço de PaaS do Azure 
No Azure, os serviços de PaaS, como Azure Stream Analytics, Azure Functions são usados em conjunto com o Azure Cosmos DB. Para habilitar o acesso a uma conta de banco de dados do Azure Cosmos DB desse tipo de serviço cujo endereço IP não está prontamente disponível, o endereço IP 0.0.0.0. deve ser adicionado à lista de permissões de endereços IP associados à sua conta de banco de dados do Azure Cosmos DB com a [configuração da política de controle de acesso de IP](#configure-ip-policy).  Isso garante que serviços de PaaS do Azure podem acessar uma conta do Azure Cosmos DB que tem essa regra. 
