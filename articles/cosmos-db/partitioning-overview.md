...
Para saber mais sobre como o Azure Cosmos DB gerencia partições, consulte [partições lógicas](partition-data.md). (Não é necessário entender os detalhes internos para compilar ou executar seus aplicativos, mas adicionado aqui para um leitor curioso.)
...
* Escolha uma chave de partição que distribui a carga de trabalho uniformemente entre todas as partições e também ao longo do tempo. Sua escolha de chave de partição deve equilibrar a necessidade de consultas de partição eficiente e transações em relação à meta de distribuição de itens em várias partições para atingir a escalabilidade.
