...
-   OAuth credenciais (uma ID de cliente e um segredo do cliente) para o Engagement para poder enviar por push seus dispositivos.
...
Se você já usa a [Biblioteca de cliente do ADM] e já tiver [ADM integrado], você pode ir diretamente para o android-sdk-adm-receive.
...
-   Depois de adicionar a marca da amazon, você poderá ter um erro de compilação se o destino da compilação do projeto estiver abaixo da versão 2.1 do Android. Você precisa usar um destino de compilação **Android 2.1 +** (não se preocupe, você ainda pode ter um `minSdkVersion` definido como 4).
-   Integre a chave de API do ADM como um ativo seguindo [esse procedimento].
...
Para se comunicar a ID de registro do dispositivo para o serviço de envio por push do Engagement e receber notificações, adicione o seguinte ao seu arquivo `AndroidManifest.xml` dentro da marca `<application/>` (mesmo se vocÊ usar ADM):
...
Certifique-se de ter as seguintes permissões em seu `AndroidManifest.xml` (antes a marca `</application>`).
...
##Conceda as credenciais de Engagement OAuth

Envie suas credenciais OAuth (ID do Cliente e Segredo do Cliente) em $/\#application/YOUR\_APPID/native-push.

Agora você pode selecionar "Qualquer hora" durante a criação de pesquisas e anúncios do Reach.
