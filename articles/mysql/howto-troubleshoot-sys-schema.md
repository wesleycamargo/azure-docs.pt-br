...
O performance_schema do MySQL, primeiro disponível no MySQL 5.5, fornece instrumentação para muitos recursos de servidor vitais, como alocação de memória, programas armazenados, bloqueio de metadados e etc. No entanto, o performance_schema contém mais de 80 tabelas e, obter as informações necessárias muitas vezes requer junção de tabelas dentro do performance_schema, assim como tabelas do information_schema. Compilando ambos o performance_schema e o information_schema, o sys_schema fornece uma coleção avançada de [exibições amigáveis de usuário](https://dev.mysql.com/doc/refman/5.7/en/sys-schema-views.html) em um banco de dados somente leitura e é totalmente habilitado no Banco de Dados do Azure para MySQL versão 5.7.

![exibições do sys_schema](./media/howto-troubleshoot-sys-schema/sys-schema-views.png)

Há 52 exibições no sys_schema e cada exibição tem um dos prefixos a seguir:
...
E/S é a operação mais cara no banco de dados. É possível localizar a latência média de E/S, consultando a exibição *sys.user_summary_by_file_io*. Com o padrão 125 GB de armazenamento provisionado, a latência de E/S é de aproximadamente 15 segundos.
...
Apesar do planejamento cuidadoso, muitas consultas ainda podem resultar em verificações de tabela completas. Para obter informações adicionais sobre os tipos de índices e sobre como otimizá-los, você pode consultar esse artigo: [Como solucionar problemas de desempenho de consultas](./howto-troubleshoot-query-performance.md). As verificações de tabela completas são tarefas com uso intensivo de recursos e prejudicam o desempenho do banco de dados. A maneira mais rápida de localizar tabelas com verificação de tabela completa é consultar a exibição *sys.schema_tables_with_full_table_scans*.
...
Para solucionar problemas de desempenho do banco de dados, pode ser útil identificar os eventos ocorrendo dentro do banco de dados e, usando a exibição *sys.user_summary_by_statement_type* simplesmente, poderá fazer a rodada.
...
Os índices são excelentes ferramentas para melhorar o desempenho de leitura, mas eles incorrem em custos adicionais para inserções e armazenamento. O *Sys.schema_unused_indexes* e *sys.schema_redundant_indexes* fornecem informações sobre índices duplicados ou não utilizados.
