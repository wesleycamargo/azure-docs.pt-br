...
Instale a extensão CLI para pontos de extremidade de serviço de VNet do Banco de Dados do Azure para MySQL usando o comando `az extension add --name rdbms-vnet`. 
```azurecli-interactive
...
az extension list
```
A saída do comando lista todas as extensões instaladas. A extensão CLI do Banco de Dados do Azure para MySQL é:
