---
title: "Camadas de serviço no Banco de dados do Azure para MySQL | Microsoft Docs"
description: "Camadas de serviço no Banco de Dados do Azure para MySQL"
services: mysql
author: v-chenyh
manager: jhubbard
editor: jasonh
...
---
# <a name="azure-database-for-mysql-options-and-performance-understand-whats-available-in-each-service-tier"></a>Opções e desempenho do Banco de Dados do Azure para MySQL: compreender o que está disponível em cada camada de serviço
O Banco de Dados do Azure para MySQL oferece as camadas de serviço Básico e Standard. A camada Premium ainda não está disponível.
...
Você pode criar um Banco de Dados do Azure para servidor MySQL com recursos dedicados dentro de uma camada de serviço em um nível de desempenho específico. Depois, você pode criar de um a vários bancos de dados no servidor no qual os recursos são compartilhados entre vários bancos de dados. Os recursos disponíveis para um único Banco de Dados do Azure para servidor MySQL são expressos em termos de Unidades de Computação e Unidades de Armazenamento. Para saber mais sobre Unidades de Computação e Unidades de Armazenamento, confira [Explicar a Unidade de Computação e a Unidade de Armazenamento](concepts-compute-unit-and-storage.md)
...
> [!NOTE]
> A camada de serviço Standard, atualmente em modo visualização, oferece suporte a até 800 Unidades de Computação e no máximo 1000 GB de armazenamento.

Depois de determinar a camada de serviço mínima, você está pronto para determinar o nível de desempenho do Banco de Dados do Azure para servidor MySQL (as Unidades de Computação). As Unidades de Computação 200 e 400 padrão costumam ser um bom ponto de partida para aplicativos que exigem maior simultaneidade de usuários para suas cargas de trabalho de análise ou da Web. 
...
O Banco de Dados do Azure para servidores MySQL oferece vários níveis de desempenho dentro de cada camada de serviço. Você tem a flexibilidade de escolher o nível que melhor atenda às suas demandas de carga de trabalho usando uma das opções a seguir:
- [Portal do Azure](quickstart-create-mysql-server-database-using-azure-portal.md)
- [CLI do Azure](quickstart-create-mysql-server-database-using-azure-cli.md)

Independentemente do número de bancos de dados hospedados em cada servidor MySQL, o banco de dados ainda obterá um conjunto garantido de recursos e as características de desempenho esperadas de seu servidor não são afetadas.
...
## <a name="scaling-up-or-down-a-single-server"></a>Expandir ou reduzir um único servidor
...
### <a name="documentation-about-the-steps-for-scaling-up-or-down"></a>Documentação sobre as etapas para escalar verticalmente
[Monitorar e escalar um Banco de dados do Azure para servidor MySQL usando a CLI do Azure](scripts/sample-scale-server.md)
...
- As ofertas de serviço de restauração são diferentes para as várias camadas de serviço. Se estiver fazendo downgrade, talvez você perca a capacidade de fazer uma restauração pontual ou tenha um período menor de retenção do backup. Para saber mais, confira [Como fazer backup e restaurar um Banco de Dados do Azure para servidor MySQL usando o Portal do Azure](howto-restore-server-portal.md)
- As novas propriedades do servidor não serão aplicadas até que as alterações sejam concluídas.

## <a name="next-steps"></a>Próximas etapas
[Explicação da unidade de computação e unidade de armazenamento](concepts-compute-unit-and-storage.md)
