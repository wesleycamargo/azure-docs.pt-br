...
* Como criar um projeto ASP.NET MVC 5 seguro e publicá-lo para [Aplicativos Web do Serviço de Aplicativo](http://go.microsoft.com/fwlink/?LinkId=529714) no Serviço de Aplicativo do Azure.
...
>[AZURE.NOTE] Para concluir este tutorial, você precisa de uma conta do Microsoft Azure. Se não tiver uma conta, você poderá <a href="/pt-br/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F" target="_blank">ativar seus benefícios de assinante MSDN</a> ou <a href="/pt-br/pricing/free-trial/?WT.mc_id=A261C142F" target="_blank">inscrever-se para uma avaliação gratuita</a>.
...
1. Nomeie o aplicativo **ContactManager** e clique em **OK**.
	...
	**Observação:** Certifique-se de inserir "ContactManager". Blocos de código que você vai copiar posteriormente supõem que o nome do projeto seja ContactManager.

1. Na caixa de diálogo **Novo Projeto ASP.NET**, selecione o modelo **MVC**. Verifique se a **Autenticação** está definida como **Contas de Usuário Individuais**, se **Host na nuvem** está marcado e se a opção **Aplicativos Web** está selecionada.

	![New ASP.NET Project dialog box](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/ss1.PNG)

1. O assistente de configuração sugerirá um nome exclusivo baseado em *ContactManager* (consulte a imagem abaixo). Selecione uma região perto de você. Você pode usar o [azurespeed.com](http://www.azurespeed.com/ "AzureSpeed.com") para localizar o datacenter com a menor latência. 
...
1. Verifique se o Internet Explorer é o navegador que inicia o Visual Studio, como mostrado na imagem abaixo:
	...
	Você pode selecionar vários navegadores e fazer com que o Visual Studio atualize cada navegador quando você fizer alterações. Para obter mais informações, consulte [Usando o link do navegador no Visual Studio 2013](http://www.asp.net/visual-studio/overview/2013/using-browser-link).
...
1. O IE mostra a página *Home* e não há nenhum aviso de SSL.
...
2. Na caixa de diálogo **Adicionar Novo Item**, nomeie o novo arquivo de classe como *Contact.cs* e clique em **Adicionar**.
...
7. Pressione CTRL+F5 para executar o aplicativo e, em seguida, clique no link **Demonstração de CM**, ou navegue até http://localhost:(nº da porta)/Cm. 
...
Nesta seção, você vai adicionar um usuário local e a função *canEdit* ao banco de dados de associação. Somente os usuários na função *canEdit* poderão editar os dados. A melhor prática é nomear as funções pelas ações que podem executar, portanto, é preferível usar *canEdit* do que uma função chamada *admin*. Quando seu aplicativo evoluir, você poderá adicionar novas funções, como *canDeleteMembers*, em vez da menos descritiva *superAdmin*.
...
<span></span>
   ...
   Esse código cria uma nova função chamada *canEdit*, cria um novo usuário local *user1@contoso.com* e adiciona *user1@contoso.com* à função *canEdit*. Para obter mais informações, consulte meus [tutoriais do ASP.NET Identity](http://www.asp.net/identity/overview/features-api).

## Usar código temporário para adicionar novos usuários de logon social à função canEdit  ##
Nesta seção, você vai modificar temporariamente o método **ExternalLoginConfirmation** no controlador de conta para adicionar novos usuários com registro com um provedor OAuth na função *canEdit*. Vamos modificar temporariamente o método **ExternalLoginConfirmation** para adicionar automaticamente novos usuários a uma função administrativa. Até que possamos fornecer uma ferramenta para adicionar e gerenciar funções, vamos usar o código de registro automático temporário abaixo. No futuro, esperamos fornecer uma ferramenta semelhante à [WSAT](http://msdn.microsoft.com/library/ms228053.aspx) que permita criar e editar contas de usuário e funções. 
...
Execute o comando **Update-Database**, que executará o método **Seed** e também o **AddUserAndRole** que você acabou de adicionar. O **AddUserAndRole** criará o usuário *user1@contoso.com* e o adicionará à função *canEdit*.
...
1. Clique em *CM Demo* e verifique se você vê os dados. 
...
2. Verifique se você pode editar dados enquanto estiver conectado nessa conta.
 	**Observação:** você não pode fazer logoff do Google neste aplicativo e fazer logon em outra conta do google com o mesmo navegador. Se você estiver usando um navegador, você precisará navegar até o Google e fazer logoff. Você pode fazer logon com outra conta do mesmo autenticador de terceiros (como o Google) usando um navegador diferente.
...
**Observação** se você não pode expandir **Bancos de dados SQL** nem *can't* consultar o **ContactDB** do Visual Studio, você precisará seguir as instruções abaixo para abrir uma porta de firewall ou um intervalo de portas. Siga as instruções em **Configurar regras de firewall do Azure**. Você terá que aguardar alguns minutos para acessar o banco de dados depois de adicionar a regra de firewall.
...
Verifique se os **UserId**s são de *user1@contoso.com* e da conta do Google que você registrou. 
...
2.	[Aplicativo ASP.NET MVC 5 com Autenticação de dois fatores de SMS e email](http://www.asp.net/mvc/overview/getting-started/aspnet-mvc-5-app-with-sms-and-email-two-factor-authentication)
