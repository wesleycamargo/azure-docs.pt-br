...
# Entrega contínua no Azure usando o Visual Studio Online
  ...
  Você pode configurar seus projetos de equipe do Visual Studio Online para compilação e implantação automática em aplicativos Web ou serviços de nuvem do Azure. (Para obter informações sobre como configurar uma compilação contínua e implantar um sistema usando um Team Foundation Server *local*, consulte [Entrega contínua de serviços de nuvem no Azure](cloud-services/cloud-services-dotnet-continuous-delivery.md).)
...
<br/> Observe as opções para incluir ou excluir alterações específicas ao fazer check-in. Se desejar que as alterações sejam excluídas, escolha o link **Incluir Tudo**.<br/>
![][9]
...
3. No Gerenciador de Soluções, abra um arquivo e altere-o. Por exemplo, mude a pasta do arquivo _Layout.cshtml na pasta Views\\Shared em uma função web do MVC.<br/>
![][17]
...
![][27]
<br/>
Especifique valores para as propriedades se você desejar valores diferentes dos padrões. As propriedades de publicação no Azure estão na seção Implantação. A tabela a seguir mostra as propriedades disponíveis na seção de Implantação:
	<table>
