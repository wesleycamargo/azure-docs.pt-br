...
1. Comece [criando uma regra na IoT Central](howto-create-telemetry-rules.md). Depois de salvar as condições da regra, clique na **ação do Microsoft Flow** como uma nova ação. Uma nova janela ou guia deve se abrir no navegador, conduzindo você ao Microsoft Flow.
...
1. Selecione o aplicativo na lista suspensa. Agora, você precisará da ID do dispositivo existente que deseja atualizar. É possível obter a ID do dispositivo da IoT Central na URL do navegador.
