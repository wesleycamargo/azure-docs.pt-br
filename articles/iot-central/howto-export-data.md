---
title: Exportar seus dados do Azure IoT Central | Microsoft Docs
...
ms.locfileid: "57759892"
---
# <a name="export-your-data-from-azure-iot-central"></a>Exportar seus dados do Azure IoT Central

*Este tópico aplica-se aos administradores.*

Este artigo descreve como usar o recurso de exportação contínua de dados no Azure IoT Central para exportar seus dados para suas próprias instâncias do **Armazenamento de Blobs do Azure**, dos **Hubs de Eventos do Azure** e do **Barramento de Serviço do Azure**. Exporte **medidas**, **dispositivos** e **modelos de dispositivo** para seu próprio destino para análise de caminhos warm e cold. Você pode exportar dados para o armazenamento de BLOBs para executar análise de tendências de longo prazo no Microsoft Power BI ou exportar dados para Hubs de eventos e barramento de serviço para transformar e aumentar seus dados em tempo quase real com aplicativos lógicos do Azure ou Azure Functions.
...
3. Escolha uma assinatura.
    ...
    > [!Note]
    > Agora você pode exportar dados para outras assinaturas que **não são a mesma** usada para o aplicativo IoT Central pago conforme o uso. Você se conectar usando uma cadeia de caracteres de conexão neste caso.
...
2. No menu à esquerda, selecione **exportação contínua de dados**.
    ...
    ![Criar um novo Hub de eventos](media/howto-export-data/export_menu.png)

3. Selecione o **+ novo** botão no canto superior direito. Escolha uma das opções **Armazenamento de Blobs do Azure**, **Hubs de Eventos do Azure** ou **Barramento de Serviço do Azure** como o destino da exportação.
...
4. Na caixa de listagem suspensa, selecione a **Conta de Armazenamento/namespace do Barramento de Serviço/namespace dos Hubs de Eventos**. Você também pode escolher a última opção da lista, **Inserir uma cadeia de conexão**. 
    ...
    > [!NOTE]
    > Você só verá o namespace de Hubs de evento/contas de armazenamento namespaces/Service Bus na **mesma assinatura que seu aplicativo IoT Central**. Caso deseje exportar para um destino fora dessa assinatura, escolha **Inserir uma cadeia de conexão** e veja a etapa 5.
...
8. Para ativar a exportação contínua de dados, verifique se a opção **Exportação de dados** está **Ativada**. Clique em **Salvar**.

    ![Configure a exportação contínua de dados](media/howto-export-data/export_list.png)

9. Depois de alguns minutos, os dados aparecerão em seu destino escolhido.
