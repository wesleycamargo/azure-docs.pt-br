...
3. Identifique a biblioteca. jar para a versão atual desse SDK (esta documentação foi preparada para a versão 3.0.0) e copie-a para a área de transferência.
...
3. No código que você acabou de colar, substitua "< Your application name>" no rótulo. Por exemplo:
...
2. Copie os recursos para seu projeto por meio das seguintes etapas:
	1. Navegue de volta para o conteúdo de download do SDK e abra a pasta 'res'.
	2. Selecione as 2 pastas e copie-as para a área de transferência.

		![][23]

	4. Volte ao Android Studio, selecione a parte 'res' do seu projeto e cole para adicionar recursos ao seu projeto.

		![][24]

###Especificar um ícone padrão em notificações
O código a seguir definirá o ícone padrão que será exibido com as notificações. Aqui usamos o ícone fornecido com o projeto criado pelo Android Studio. Este trecho xml deve ser colado em seu Manifest.xml entre as marcas < application > e </application >. Certifique-se de que ic_launcher existe em seu aplicativo ou use outro arquivo de ícone, caso contrário, a notificação não será exibida.
...
3. Adicione o último conjunto de permissões realçadas abaixo ou após a marca < application>. Novamente, usamos este nome de pacote de projeto que você precisará substituir em seu aplicativo de produção.
