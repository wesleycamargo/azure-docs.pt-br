...
O [SDK do Azure para PHP] consiste em vários componentes. Este artigo usará dois deles: O PowerShell do Azure e o Emuladores do Azure. Esses dois componentes podem ser instalados pelo Microsoft Web Platform Installer aqui: [Instale os emuladores do Azure e do PowerShell do Azure][instalar emuladores e ps].
...
> [AZURE.NOTE] O parâmetro  `roleName` é opcional. Se for omitido, o nome da função será gerado automaticamente. A primeira função Web criada será `WebRole1`, a segunda `WebRole2`, e assim por diante. A primeira função de trabalho criada será `WorkerRole1`, a segunda `WorkerRole2`, e assim por diante.
...
> [AZURE.NOTE] Todas as configurações que você não definir explicitamente no arquivo  `php.ini` que você fornecer serão definidas automaticamente para seus valores padrão. No entanto, lembre-se de que você pode adicionar um  `php.ini` completo.
...
> [AZURE.NOTE] A pasta `download.ps1` script (in the `bin` do diretório raiz da função Web) pode ser excluído após a realização das etapas descritas acima para usar seu próprio tempo de execução do PHP.
...
Para publicar seu aplicativo, você precisa primeiro importar suas configurações de publicação  com o cmdlet do **PublishSettingsFile importação**, você pode publicar seu aplicativo com o cmdlet do **Publish-AzureServiceProject**. Detalhes sobre como usar cada um desses cmdlets podem ser encontrados em [Como: Importar configurações de publicação] e [Como: Implantar um serviço de nuvem no Azure], respectivamente.

[sites da web do modelo de execução]: /pt-br/develop/net/fundamentals/compute/#WebSites
[execução do modelo vms]: /pt-br/develop/net/fundamentals/compute/#VMachine
[serviços de nuvem do modelo de execução]: /pt-br/develop/net/fundamentals/compute/#CloudServices
[SDK do Azure para PHP]: /pt-br/develop/php/common-tasks/download-php-sdk/
[instalar emuladores e ps]: http://go.microsoft.com/fwlink/?LinkId=253447&clcid=0x409
