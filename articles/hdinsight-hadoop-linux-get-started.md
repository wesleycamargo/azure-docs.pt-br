...
# Introdução ao uso de Hadoop com o Hive no HDInsight no Linux (visualização)

> [AZURE.SELECTOR]
- [Windows](hdinsight-get-started.md)
- [Linux](hdinsight-hadoop-linux-get-started.md)

Este tutorial faz uma rápida apresentação do Azure HDInsight no Linux, mostrando como provisionar um cluster HDInsight Hadoop no Linux e como executar uma consulta Hive para extrair informações úteis de dados não estruturados.
...
Suponha que você tenha um grande conjunto de dados não estruturado e deseje executar consultas nele para extrair algumas informações importantes. Veja como fazer isso:
   ...
   ![As etapas do tutorial "Introdução ao uso do Hadoop com o Hive no HDInsight" mostraram como: criar uma conta; provisionar um cluster; consultar dados; e analisar no Tableau.](./media/hdinsight-hadoop-linux-get-started/HDI.Linux.GetStartedFlow.png)
...
2. Clique em **NOVO** no canto inferior esquerdo, aponte para **SERVIÇOS DE DADOS**, aponte para **ARMAZENAMENTO** e clique em **CRIAÇÃO RÁPIDA**.
	...
	![Portal do Azure, em que você pode usar a Criação Rápida para configurar uma nova conta de Armazenamento.](./media/hdinsight-hadoop-linux-get-started/HDI.StorageAccount.QuickCreate.png)

3. Insira as informações de **URL**, **LOCAL** e **REPLICAÇÃO** e clique em **CRIAR CONTA DE ARMAZENAMENTO**. Não há suporte para grupos de afinidade. Você verá a nova conta do Armazenamento na lista de armazenamento.

	>[AZURE.NOTE]A opção Criação Rápida para provisionar um cluster HDInsight em Linux, como a que usamos neste tutorial, não solicitará um local ao provisionar o cluster. Por padrão, ela coloca o cluster no mesmo datacenter da conta do Armazenamento. Portanto, lembre-se de criar sua conta do Armazenamento nos locais compatíveis com o cluster, que são: **Ásia Oriental**, **Sudeste da Ásia**, **Norte da Europa**, **Europa Ocidental**, **Leste dos EUA**, **Oeste dos EUA**, **Centro-Norte dos EUA**, **Centro-Sul dos EUA**.
...
2. Clique em **NOVO** no canto inferior esquerdo, depois em **SERVIÇOS DE DADOS**, clique em **HDINSIGHT** e, então, em **HADOOP NO LINUX**.
	...
	![Criação de um cluster Hadoop no HDInsight.](./media/hdinsight-hadoop-linux-get-started/HDI.QuickCreateCluster.png)
...
2. Abra o PuTTY. Em **Categoria**, clique em **Sessão**. Na tela **Opções básicas para sua sessão PuTTY**, insira o endereço SSH do seu servidor HDInsight no campo **Nome de Host (ou Endereço IP)**. O endereço do SSH é o nome do cluster, seguido de **-ssh.azurehdinsight.net**. Por exemplo, **myhdinsightcluster-ssh.azurehdinsight.net**.
	...
	![Conectar-se a um cluster HDInsight no Linux usando PuTTY](./media/hdinsight-hadoop-linux-get-started/HDI.linux.connect.putty.png)
...
4. Para verificar se apenas linhas que contêm [ERROR] na coluna t4 foram armazenadas na tabela **errorLogs**, use a seguinte instrução para retornar todas as linhas de **errorLogs**:
	...
	Os dados retornados devem corresponder, todos, aos logs de [ERROR].


## <a name="nextsteps"></a>Próximas etapas
Neste tutorial, você aprendeu a provisionar um cluster Hadoop em Linux com o HDInsight e a executar nele uma consulta Hive usando SSH. Para saber mais, consulte os seguintes artigos:
