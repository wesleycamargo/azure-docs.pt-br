...
O bloco **Acesso à VM just in time** na folha **Central de Segurança** mostra o número de VMs configuradas para o acesso just in time e o número de solicitações de acesso aprovadas na última semana.

Selecione o bloco **Acesso à VM just in time** e a folha **Acesso à VM just in time** é aberta.

![Bloco acesso à VM just in time][2]

A folha **Acesso à VM just in time** fornece informações sobre o estado das suas VMs:
...
1. Na folha **Acesso à VM just in time**, selecione a guia **Recomendada**.

  ![Habilitar o acesso just in time][3]

2. Em **VMs**, selecione as VMs que você deseja habilitar. Isso coloca uma marca de seleção ao lado de uma VM.
...
2. Em **VMs**, selecione uma VM. Isso coloca uma marca de seleção ao lado da VM e abre a folha **Configuração de acesso à VM JIT**. Esta folha exibe as portas padrão.

### <a name="add-ports"></a>Adicionar portas

Na folha **Configuração de acesso à VM JIT**, você também pode adicionar e configurar uma nova porta na qual deseja habilitar a solução just in time.

1. Na folha **Configuração de acesso à VM JIT**, selecione **Adicionar**. Isso abre a folha **Adicionar configuração de porta**.

  ![Configuração de portas][7]

2. Na folha **Adicionar configuração de porta**, você identifica a porta, o tipo de protocolo, os IPs de origem com permissão e o tempo máximo de solicitação.
...
2. Selecione **Editar** no menu. Isso abre a folha **Configuração de acesso à VM JIT**.

  ![Editar política][8]

3. Na folha **Configuração de acesso à VM JIT**, você pode editar as configurações existentes de uma porta já protegida clicando em sua porta ou você pode selecionar **Adicionar**. Isso abre a folha **Adicionar configuração de porta**.

  ![Adicionar uma porta][7]

4. Na folha **Adicionar configuração de porta**, identifique a porta, o tipo de protocolo, os IPs de origem com permissão e o tempo máximo de solicitação.
5. Selecione **OK**.
6. Selecione **Salvar**.
...
3. Selecione **Log de atividades** no menu. Isso abre a folha **Log de atividades**.

![Selecionar o log de atividades][9]

A folha **Log de atividades** fornece uma exibição filtrada das operações anteriores dessa VM junto com a hora, a data e a assinatura.
...
Depois disso, você precisa instalar a Central de Segurança do Azure [mais recente](https://www.powershellgallery.com/packages/Azure-Security-Center/0.0.12) da galeria do PowerShell.
