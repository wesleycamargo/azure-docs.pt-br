...
---
# <a name="query-data-from-azure-time-series-insights-environment-using-c"></a>Consulte dados do ambiente de Análise de Séries Temporais do Azure usando C#
...
1. Como uma etapa de preparação, o token de acesso é obtido usando a API do Azure Active Directory. Esse token deve ser passado no cabeçalho `Authorization` de cada solicitação de API de consulta.
...
Consulte o documento [API de consulta](/rest/api/time-series-insights/time-series-insights-reference-queryapi) para obter a referência completa de API de consulta.
