---
title: Prova de Autoridade do Consórcio Ethereum
...
Para aqueles que são novos na comunidade de blockchain, o lançamento dessa solução é uma grande oportunidade para saber mais sobre a tecnologia de uma maneira fácil e configurável no Azure. A prova de trabalho é um mecanismo de resistência Sybil que aproveita os custos de computação para autorregular a rede e permitir a participação justa. Isso funciona muito bem em redes de blockchain anônimas abertas onde a competição por criptomoeda promove a segurança na rede. No entanto, em redes privadas/consórcio o Ether subjacente não tem valor. Um protocolo alternativo, prova de autoridade, é mais adequado para redes com permissão em que todos os participantes de consenso são conhecidos e confiáveis. Sem a necessidade de mineração, a prova de autoridade é mais eficiente enquanto ainda retém a tolerância a falhas de Byzantine.

### <a name="consortium-governance"></a>Governança do Consortium

Como a prova de autoridade depende de uma lista com permissão de autoridades de rede para manter a rede íntegra, é importante fornecer um mecanismo razoável para fazer modificações a essa lista de permissão. Cada implantação vem com um conjunto de contratos inteligentes e o portal para governança na cadeia dessa lista que recebeu permissão. Quando uma alteração proposta atinge um voto maioria por membros consortium, a alteração é imposta. Isso permite que novos participantes de consenso sejam adicionados ou comprometidos para serem removidos de forma transparente que incentiva uma rede honesta.

### <a name="admin-account"></a>Conta de administrador

Durante a implantação de nós da prova de autoridade, você será solicitado para um endereço de administrador Ethereum. Você pode usar vários mecanismos diferentes para gerar e proteger esta conta Ethereum. Depois que esse endereço é adicionado como uma autoridade na rede, você pode usar essa conta para participar de governança. Essa conta de administrador também será usada para delegar a participação de consenso para os nós de validador que são criados como parte dessa implantação. Como apenas o endereço Ethereum público é usado, cada administrador tem a flexibilidade de proteger suas chaves privadas de uma maneira que esteja em conformidade com seu modelo de segurança desejado.
...
Uma vez que cada membro terá vários nós de validador em execução simultaneamente e cada nó deve ter uma identidade com permissão, é importante que os validadores possam adquirir com segurança uma identidade exclusiva de Active Directory na rede. Para facilitar isso, criamos um Armazenamento de Identidade e que é implantado na assinatura de cada membro que armazena com segurança as identidades Ethereum geradas. Após a implantação do contêiner de orquestração gerará uma chave privada do Ethereum para cada validador e irá armazená-lo para cada validador e armazená-lo no Azure Key Vault. Antes do nó de paridade ser iniciado, ele primeiro adquire uma concessão em uma identidade não utilizada para garantir que a identidade não seja selecionada por outro nó. A identidade é fornecida ao cliente que fornece a ele a autoridade para iniciar a criação de blocos. Se a VM de hospedagem sofrer uma interrupção, a concessão de identidade será liberada, permitindo que um nó de substituição retome sua identidade no futuro.
...
-   Conjuntos de Dimensionamento de Máquina virtual (VMSS) para executar os validadores de PoA
...
O cerne da prova de autoridade é descentralizado de governança. A governança DApp é um conjunto de contratos inteligentes previamente implantados e um aplicativo web que são usados para controlar as autoridades na rede.
...
![dapp de governança](./media/ethereum-poa-deployment-guide/governance-dapp.png)
...
Esse processo requer uma assinatura do Azure que pode suportar a implantação de vários discos gerenciados e conjuntos de dimensionamento de máquinas virtuais. Se necessário, [crie uma conta gratuita do Azure](https://azure.microsoft.com/free/) para começar.
...
---|---|---|---
Criar uma nova rede ou ingressar em uma rede existente?|Criar uma nova rede ou ingressar em uma rede de consórcio pré-existente|Criar Nova Ingressar em Existente|Criar Novo
...
Uma implantação de exemplo é mostrada abaixo: ![folha básica](./media/ethereum-poa-deployment-guide/basic-blade.png)
...
Uma implantação de exemplo é mostrada abaixo: ![regiões de implantação](./media/ethereum-poa-deployment-guide/deployment-regions.png)
...
Para obter uma descrição detalhada de cada parâmetro:
  ...
  Desempenho do armazenamento de nó do validador|Tipo de disco gerenciado de suporte de cada um de nós de mineração implantado.|Standard ou Premium|Standard
...
Uma implantação de exemplo é mostrada abaixo: ![tamanho e desempenho de rede](./media/ethereum-poa-deployment-guide/network-size-and-performance.png)
...
Contrato de permissão de transação (opções avançadas = habilitar)|Código de bytes para o contrato de permissão de transação. Restringe a implantação inteligente de contrato e a execução para uma lista com permissão de contas Ethereum.|Código de bytes do contrato|ND

Uma implantação de exemplo é mostrada abaixo: ![configurações do ethereum](./media/ethereum-poa-deployment-guide/ethereum-settings.png)

#### <a name="azure-monitor"></a>Azure Monitor

A folha do Azure Monitor permite que você configure um recurso do Azure Monitor para sua rede. O Azure Monitor coletará e revestirá métricas úteis e logs de sua rede, fornecendo a capacidade para verificar rapidamente a integridade da rede ou depurar problemas.

  Nome do Parâmetro|DESCRIÇÃO|Valores Permitidos|Valores padrão
  ---|---|---|---
Monitoramento|Opção para habilitar o Azure Monitor|Habilitar ou Desabilitar|Habilitar
Conecte-se ao Log Analytics existente|Criar uma nova instância do Log Analytics como parte do Azure Monitor ou ingressar em uma instância existente|Criar nova ou ingresar em existente|Criar Novo
Azure Monitor Location (Conectar a Azure Monitor existente = Criar novo)|A região do Azure Monitor onde os recursos serão implantados|Todas as regiões do Azure Monitor|ND
ID de espaço de trabalho do Log Analytics existente (conectar-se ao Azure Monitor existente = associação existente)|ID do espaço de trabalho da instância existente do Log Analytics||ND
Chave primária do Log Analytics existente (conectar-se ao Azure Monitor existente = associação existente)|A chave primária usada para se conectar à instância existente do Azure Monitor||ND


Uma implantação de exemplo é mostrada abaixo: ![folha básica](./media/ethereum-poa-deployment-guide/azure-monitor.png)
...
Depois que a implantação for concluída, você poderá acessar os parâmetros necessários por meio de email de confirmação ou por meio do portal do Azure. Nesses parâmetros você encontrará:
...
-   Id de recurso de Gateway de rede virtual (opcional)
...
![email de implantação](./media/ethereum-poa-deployment-guide/deployment-email.png)

##### <a name="portal"></a>Portal

Depois que a implantação for concluída com êxito e todos os recursos tiverem sido provisionados, você poderá exibir os parâmetros de saída no seu grupo de recursos.
...
Usando a implantação com base em IP público a primeira etapa é perfeita. Se implantar por trás de uma VPN, consulte a seção [Gateway de rede virtual conectando](#connecting-vnet-gateways) para realizar a conexão de rede como parte da [Etapa 2](#step-2-new-admin-deployment).

#### <a name="step-1-add-the-new-admin"></a>Etapa 1: Adicionar o novo administrador

1.  Colete endereço público de Ethereum do novo administrador.

2.  Navegue até o DApp de governança e crie um novo administrador com o endereço de Ethereum e um alias apropriado

3.  Notifique os membros existentes da nova solicitação para que eles possam votar para adicionar esse novo administrador

4.  Depois que o voto atinge 51%, o membro será adicionado como um administrador

![adicionando administrador](./media/ethereum-poa-deployment-guide/adding-admin.png)

#### <a name="step-2-new-admin-deployment"></a>Etapa 2: Implantação do admin novo
...
1.  Compartilhe as informações a seguir com o membro da junção. Essas informações podem ser encontradas em seu email de pós-implantação ou na saída de implantação do portal.
    ...
    -  ID de recurso de Gateway de rede virtual (se usar VPN)
...
2.  O membro de implantação deve usar a [mesma solução](https://portal.azure.com/?pub_source=email&pub_status=success#create/microsoft-azure-blockchain.azure-blockchain-ethereumethereum-poa-consortium) ao implantar sua presença de rede com tendo em mente o seguinte:
    ...
    -  Use o mesmo endereço Ethereum que foi fornecido na [etapa anterior](#step-1-add-the-new-admin)
...
#### <a name="step-3-delegate-validators"></a>Etapa 3: Validadores de delegado

O novo administrador precisará autorizar os validadores para participar em seu nome. Isso não pode ser feito automaticamente, pois somente o membro controla suas chaves.\*

_\*Os nós do primeiro membro na rede podem ser adicionados automaticamente porque podemos pré-compilar seus nós de validador na gênese do bloqueio na rede._

O novo administrador deve executar as seguintes etapas:

1.  Abra o DApp de governança implantado como parte de *sua* implantação

2.  Navegue até a guia Validadores

3.  Selecione cada um dos validadores que foram implantados e clique em **Adicionar**

![adição de validadores](./media/ethereum-poa-deployment-guide/adding-validators.png)

#### <a name="connecting-vnet-gateways"></a>Conectar-se a gateways de rede virtual

No caso de uma rede privada, os membros diferentes são conectados por meio de conexões de gateway de rede virtual. Antes de um membro poder ingressar na rede e ver o tráfego de transação, um membro existente deve executar uma configuração final no seu Gateway de VPN para aceitar a conexão. Isso significa que os nós de Ethereum do membro que ingressa não serão executados até que uma conexão seja estabelecida. É recomendável criar conexões de rede redundantes (malha) em consórcio para reduzir as chances de um ponto único de falha.

Depois que o novo membro implanta, o membro existente deve concluir a conexão bi-direcional, configurando uma conexão de gateway de rede virtual para o novo membro. Para obter isso, o membro existente deverá:

1.  O gateway de rede virtual ResourceId do membro que está se conectando (consulte a saída de implantação)

2.  A chave de conexão compartilhada

O membro existente deve executar o seguinte script do PowerShell para concluir a conexão. Recomendamos que você use o Azure Cloud Shell localizados na barra de navegação direita superior no portal.
...
![status do nó](./media/ethereum-poa-deployment-guide/node-stats.png)
...
#### <a name="sample-log-analytics-queries"></a>Amostra de consultas do Log Analytics
...
Por motivos de segurança, o acesso de porta SSH é negado por uma regra de segurança de grupo de rede por padrão. Para acessar o dimensionamento de máquinas virtuais defina instâncias na rede PoA, você precisará alterar essa regra para \"Permitir\"

1.  Comece na seção Visão geral do grupo de recursos implantados do portal do Azure.

    ![visão geral ssh](./media/ethereum-poa-deployment-guide/ssh-overview.png)
...
3.  Selecione a regra \"permitir-ssh\"
    ...
    ![ssh-permitir](./media/ethereum-poa-deployment-guide/ssh-allow.png)

4.  Alterar \"Ação\" para permitir

    ![habilitar permissão ssh](./media/ethereum-poa-deployment-guide/ssh-enable-allow.png)
...
O comando SSH de execução para acessar o primeiro nó do validador está listado no parâmetro de saída de implantação de modelo como 'SSH\_TO\_FIRST\_VL\_NODE\_REGION1' (para a implantação de exemplo: ssh -p 4000\@leader4vb.eastus.cloudapp.azure.com). Para obter nós de transações adicionais, incremente o número da porta em um (por exemplo, o primeiro nó de transação está na porta 4000).
...
![pesquise o gerenciador de tráfego do azure](./media/ethereum-poa-deployment-guide/traffic-manager-search.png)

Atribua o perfil de um nome exclusivo e selecione o grupo de recursos que foi criado durante a implantação de PoA.

![criar gerenciador de tráfego](./media/ethereum-poa-deployment-guide/traffic-manager-create.png)

Depois de implantado, em seguida, selecione a instância no grupo de recursos. O nome DNS para acessar o gerenciador de tráfego pode ser encontrado na guia Visão geral

![Localize o Gerenciador de tráfego DNS](./media/ethereum-poa-deployment-guide/traffic-manager-dns.png)

Selecione a guia Pontos de extremidade e clique no botão Adicionar. Em seguida, altere o tipo de recurso de destino para o endereço IP público. Em seguida, selecione o endereço IP público da primeira região\'balanceador de carga de s.

![Roteamento do gerenciador de tráfego](./media/ethereum-poa-deployment-guide/traffic-manager-routing.png)

Repita para cada região na rede implantada. Depois que os pontos de extremidade estiverem no status \"habilitado\", eles serão carregados automaticamente e com balanceamento de região no nome de DNS de gerenciador de tráfego. Agora você pode usar esse nome DNS, em vez do parâmetro \[CONSORTIUM\_DATA\_URL\] nas outras etapas do documento.
...
A resposta conterá informações úteis para unir os membros (Gênese do bloqueio, contrato de validador definido ABI, bootnodes), bem como informações úteis para o membro existente (endereços do validador). Recomendamos o uso de padronização para estender o consórcio entre provedores na nuvem. Essa API retornará uma resposta JSON formatado com a seguinte estrutura:
```json
{
  "$id": "", 
  "type": "object", 
...
### <a name="debug-a-smart-contract-truffle-develop"></a>Depurar um contrato inteligente (Desenvolver Truffle)
...
### <a name="i-notice-a-bunch-of-transactions-on-the-network-that-i-didnt-send-where-are-these-coming-from"></a>Observei um monte de transações na rede que eu\' não enviei. De onde elas vêm?

Não é seguro para desbloquear a [API pessoal](https://web3js.readthedocs.io/en/1.0/web3-eth-personal.html). Contas de bot estão ouvindo as contas de Ethereum desbloqueadas e tentam drenar os fundos. O bot pressupõe que essas contas contêm real e tentativa pra ser o primeiro a extrair o saldo. Não habilite a API pessoal na rede. Em vez disso, assine previamente as transações manualmente usando uma carteira como MetaMask ou programaticamente, conforme descrito na seção interagir programaticamente com um contrato inteligente.

### <a name="how-to-ssh-onto-a-vm"></a>Como o SSH em uma VM?

Podemos bloquear a porta SSH por motivos de segurança. Siga [este guia para habilitar a porta SSH](#_Accessing_Nodes_via).
...
Isso funciona bem como um mecanismo para fornecer capacidade de auditoria para reguladores da rede. Para fazer isso simplesmente siga a Etapa 2 de um consórcio de crescimento.
...
### <a name="are-public-ip-deployments-compatible-with-private-network-deployments"></a>Implantações de IP público são compatíveis com as implantações de rede privada?
