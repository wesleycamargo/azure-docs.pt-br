...
- **Uma assinatura do Azure** -- Se você não tiver uma conta, poderá criar uma em apenas alguns minutos. Se você tiver uma assinatura do MSDN, confira [Crédito mensal do Azure para assinantes do Visual Studio](https://azure.microsoft.com/pricing/member-offers/msdn-benefits-details/). Caso contrário, saiba como [criar uma conta de avaliação gratuita](https://azure.microsoft.com/pricing/free-trial/).

- **Ferramentas do Azure PowerShell** -- O módulo do Azure PowerShell deve estar instalado e configurado para a assinatura do Azure poder ser usada. Para baixar o módulo, consulte [Downloads do Azure](https://azure.microsoft.com/downloads/) (a página pode estar em inglês). Um tutorial que descreve como instalar e configurar o módulo está disponível aqui. Use o cmdlet [Downloads do Azure](https://azure.microsoft.com/downloads/) para carregar o VHD.
...
> [AZURE.NOTE] Não há suporte para o formato VHDX mais recente no Azure. Você pode converter o disco em formato VHD usando o Gerenciador do Hyper-V ou o cmdlet [convert-vhd](https://technet.microsoft.com/library/hh848454.aspx). Além disso, há um [tutorial no MSDN sobre como usar o FreeBSD com o Hyper-V](http://blogs.msdn.com/b/kylie/archive/2014/12/25/running-freebsd-on-hyper-v.aspx).

Esta tarefa inclui as cinco etapas a seguir.

## Etapa 1: preparar a imagem para upload
...
6. Instalar o agente do Azure.
    ...
    >[AZURE.IMPORTANT] Depois de instalar o agente do Azure, convém verificar se ele está funcionando:
...
### Usar o método do Azure AD para carregar um arquivo .vhd
...
### Usar o método de certificado para carregar um arquivo .vhd
...
4. Digite: `Import-AzurePublishSettingsFile <PathToFile>`, em que `<PathToFile>` é o caminho completo para o arquivo .publishsettings.
...
## Etapa 5: Criar uma VM com o arquivo .vhd carregado
...
1. Na janela PowerShell do Azure que você usou na etapa anterior, digite:
    ...
    > [AZURE.NOTE]Use o Linux como o tipo de sistema operacional. A versão atual do Azure PowerShell aceita apenas "Linux" ou "Windows" como parâmetro.
...
4. Após a conclusão do provisionamento, você verá sua VM do FreeBSD em execução no Azure.
