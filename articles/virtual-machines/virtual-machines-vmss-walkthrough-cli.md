...
O modelo que você cria neste tutorial é semelhante a um modelo que pode ser encontrado na Galeria de modelos. Para obter mais informações, confira [Implantar um Conjunto de Escalas de VM simples com VMs do Linux e um Jumpbox](https://azure.microsoft.com/documentation/templates/201-vmss-linux-jumpbox/).
...
11.	Adicione o recurso autoscaleSettings, que define como o conjunto de escalas se ajusta com base no uso do processador nas máquinas no conjunto. Para este tutorial, estes são os valores importantes:
 ...
 - **metricName** - Esse é o mesmo que o contador de desempenho que definimos na variável wadperfcounter. Usando essa variável, a extensão de Diagnóstico coleta o contador **Processor\\PercentProcessorTime**.
...
1. Em sua interface de linha de comando (Bash, Terminal, Prompt de comando), execute esses comandos para definir as variáveis de ambiente necessárias para acessar a conta de armazenamento:
	...
	Você pode obter a chave clicando no ícone de chave ao exibir o recurso de conta de armazenamento no Portal do Azure. Ao usar um prompt de comando do Windows, digite **definir** em vez de exportar.
