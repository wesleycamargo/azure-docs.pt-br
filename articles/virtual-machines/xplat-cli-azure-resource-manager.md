<properties
	...
	description="Usando a CLI do Azure para Mac, Linux e Windows com o Gerenciamento de Recursos do Azure"
	editor="tysonn"
	manager="timlt"
	documentationCenter=""
	authors="dlepow"
...
Este tópico descreve como criar, gerenciar e excluir recursos do Azure e máquinas virtuais usando a CLI do Azure para Mac, Linux e Windows usando o modo **arm**.
...
O Gerenciador de Recursos permite que você gerencie um grupo de _recursos_ (entidades gerenciadas pelo usuário, como um servidor de banco de dados, um banco de dados ou um site) como uma unidade lógica única ou um _grupo de recursos_. Você pode criar, gerenciar e excluir esses recursos imperativa na linha de comando, assim como no modo **asm**.

Usando o modo **arm**, você também pode gerenciar os recursos do Azure de maneira _declarativa_ descrevendo a estrutura e as relações de um grupo de implantação de recursos em *modelos* JSON. O modelo descreve os parâmetros que podem ser preenchidos embutidos durante a execução de um comando ou armazenados em arquivo de JSON **azuredeploy parameters.json** separado. Isso permite criar facilmente novos recursos usando-se o mesmo modelo simplesmente fornecendo parâmetros diferentes. Por exemplo, um modelo que cria um site terá parâmetros para o nome do site, a região do site em que estará localizado e outros parâmetros comuns.

Quando um modelo é usado para modificar ou criar um grupo, uma _implantação_ é criada e, então, aplicada ao grupo. Para obter mais informações sobre o Gerenciador de recursos, visite [Visão geral do Gerenciador de recursos do Azure](../resource-group-overview.md).

## Autenticação

Atualmente, o trabalho com o Gerenciador de Recursos através do CLI do Azure exige uma autenticação no Microsoft Azure usando uma conta corporativa ou escolar. A autenticação usando um certificado instalado por meio de um arquivo .publishsettings não funcionará.
...
## Definindo o modo **arm**

Como o modo do Gerenciador de Recursos não está habilitado por padrão, você deve usar o comando a seguir para habilitar os comandos do gerenciador de recursos da CLI do Azure.
...
A maioria dos comandos do **arm** precisam de um local válido para criar ou localizar um recurso de. Você pode encontrar todos os locais disponíveis usando o comando
...
Um grupo de recursos é um agrupamento lógico de rede, armazenamento e outros recursos. Quase todos os comandos no modo **arm** precisam de um grupo de recursos. Você pode criar um grupo de recursos denominado _testrg_, por exemplo, usando o comando
...
Há duas maneiras para criar máquinas virtuais no modo **arm**:
...
Essa é a abordagem básica para configurar e criar uma máquina virtual de acordo com suas necessidades. No modo **arm**, você precisará configurar alguns recursos obrigatórios, como o sistema de rede antes de usar o comando **criar vm**.

>[AZURE.NOTE]Se você estiver criando recursos pela primeira vez na linha de comando para a sua assinatura, você pode ser solicitado a fazer o registro para determinados Provedores de recursos. Se isso acontecer, é fácil de registrar esse provedor e repetir o comando com falha novamente. Por exemplo,
>
> `azure provider register Microsoft.Storage`
>
> Você pode descobrir a lista de provedores registrados para sua assinatura através executando,
>
> `azure provider list`
...
Você precisará criar um IP público para que você possa SSH em sua nova máquina virtual para qualquer trabalho significativo. Criar um IP público é simples. O comando precisa de um grupo de recursos, um nome para o recurso IP público e um local, nessa ordem. Por exemplo,

	azure network public-ip create "testrg" "testip" "westus"

#### Criando um recurso de Placa de interface de rede

A placa de interface de rede ou NIC precisa, primeiro, de uma sub-rede e uma rede virtual para ser criada. Crie uma rede virtual em um local e grupo recursos específicos usando **network vnet create**.

	azure network vnet create "testrg" "testvnet" "westus"

Em seguida, você pode criar uma sub-rede nessa rede virtual usando **network vnet subnet create** como neste exemplo:

	azure network vnet subnet create "testrg" "testvnet" "testsubnet"

Você deve ser capaz de criar uma NIC usando esses recursos com **network nic create**.
...
Para obter mais informações sobre os comandos de **rede**, consulte a ajuda de linha de comando ou [Usando a CLI do Azure com o Gerenciamento de recursos do Azure](azure-cli-arm-commands.md).

#### Localizando a imagem do sistema operacional

Atualmente, você pode localizar um sistema operacional somente com base no editor de imagem. Em outras palavras, você deve executar esse comando para encontrar uma lista de editores de imagem do sistema operacional no local desejado,

	azure vm image list-publishers "westus"

Em seguida, escolha um editor na lista e encontre a lista de imagens por esse editor executando, por exemplo,

	azure vm image list "westus" "CoreOS"

Finalmente, escolha uma imagem de SO da lista que é semelhante a:
...
Você deve ser capaz de iniciar esta máquina virtual executando

	azure vm start "testrg" "testvm"

e fazer SSH no mesmo usando o comando **ssh username@ipaddress**. Para pesquisar rapidamente o endereço IP do seu recurso IP público, use este comando:

	azure network public-ip show "testrg" "testip"

Gerenciar esta máquina virtual é fácil com os comandos **vm**; para obter mais informações, visite [Usando a CLI do Azure com o Gerenciamento de recursos do Azure](azure-cli-arm-commands.md).
...
5. Depois de salvar o arquivo **params.json**, use o comando a seguir para criar um novo grupo de recursos com base no modelo. O parâmetro `-e` especifica o arquivo **params.json** criado na etapa anterior. Substitua **testRG** pelo nome do grupo que deseja usar, e **testDeploy** pelo nome da sua implantação. O local deve ser o mesmo que aquele especificado no seu arquivo de parâmetro do modelo **params. JSON**.
	...
	Se sua implantação foi bem-sucedida, você verá uma saída semelhante ao apresentado abaixo:
...
7. Você também pode usar os modelos mais recentes diretamente do github, em vez de baixar a biblioteca de modelos. Abra [Github.com](http://www.github.com) e procure por AzureRmTemplates. Selecione o repositório AzureRmTemplates e procure por quaisquer modelos que você achar interessantes, por exemplo, _101-simple-vm-from-image_. Se você clicar no modelo, verá que ele contém **azuredeploy.json** entre outros arquivos. Este é o modelo que você deseja usar em seu comando usando uma opção **--template-url**. Abra-o no modo _bruto_ e copie a URL que aparece na barra de endereços do navegador. Você pode usar essa URL diretamente para criar uma implantação, em vez de baixar uma biblioteca de modelos, usando um comando semelhante a
	...
	> [AZURE.NOTE]É importante abrir o modelo json no modo _bruto_. A URL que aparece na barra de endereços do navegador é diferente daquela que aparece no modo normal. Para abrir o arquivo no modo _bruto_, clique no botão chamado _Bruto_ no canto superior direito ao exibir o arquivo no github.
...
2. Para exibir recursos individuais, dentro do grupo, use o comando a seguir.
	...
	Observe o parâmetro **Microsoft.ClassicCompute/virtualMachines**. Isso indica o tipo do recurso sobre o qual você está solicitando informações. Se você examinar o arquivo de modelo baixado anteriormente, você observará que esse mesmo valor é usado para definir o tipo do recurso da máquina virtual descrito no modelo.
...
* Para obter mais informações sobre como configurar a interface de linha de comando do Azure (CLI do Azure), veja [Instalação e configuração a CLI do Azure][clisetup].
* Para obter informações sobre como trabalhar com o Gerenciador de Recursos usando o Azure PowerShell, consulte [Usando o Azure PowerShell com o Gerenciador de Recursos do Azure](../powershell-azure-resource-manager.md)
* Para obter informações sobre como trabalhar com o Gerenciador de recursos do Portal do Azure, consulte [Usando grupos de recursos para gerenciar os recursos do Azure][psrm]
