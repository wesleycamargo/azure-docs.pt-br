...
Nos comandos a seguir, substitua os exemplos por suas próprias escolhas.
...
## <a name="using-sshkeygen"></a>Usando ssh-keygen
...
## <a name="using-sshagent-to-store-your-private-key-password"></a>Usando ssh-agent para armazenar sua senha de chave privada
...
`User myusername` = a conta de usuário remoto a ser usada ao fazer logon no servidor.

`PubKeyAuthentication yes` = instrui o SSH desejado a usar uma chave SSH para fazer logon.

`IdentityFile /home/myusername/.ssh/id_id_rsa` = a chave privada SSH e a chave pública correspondente a serem usadas para autenticação.
