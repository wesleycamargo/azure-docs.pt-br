...
As chaves SSH são mantidas por padrão no diretório `~/.ssh`.  Se você não tiver um diretório `~/.ssh`, o comando `ssh-keygen` o criará para você com as permissões corretas.  O argumento `-N` especifica a senha para criptografar a chave privada do SSH e *não* é a senha do usuário.
