---
title: "SAP NetWeaver em VMs (máquinas virtuais) do Linux – Guia de Implantação | Microsoft Docs"
...
1. Configurações
  ...
  * **Tipo de Armazenamento**: (apenas para o modelo de duas camadas) o tipo de armazenamento que deve ser usado
    * Para sistemas maiores, é altamente recomendável utilizar o Armazenamento Premium. Para obter mais informações sobre os diferentes tipos de armazenamento, leia
      * [Use of Azure Premium SSD Storage for SAP DBMS Instance][2367194] (Uso do Armazenamento SSD Premium do Azure para instância do SAP DBMS)
...
Dependendo da configuração da rede local, poderá ser necessário configurar o proxy em sua máquina virtual se ela estiver conectada à rede local por meio da VPN ou da Rota Expressa. Caso contrário, é possível que a máquina virtual não acesse a Internet e, com isso, não possa baixar as extensões necessárias nem coletar os dados de monitoramento. Veja o capítulo [Configurar proxy][deployment-guide-configure-proxy] deste documento.
...
1. Configurações
  * **ID do Sistema SAP**: a ID do sistema SAP
  * **Tipo de OS**: o tipo de sistema operacional que você deseja implantar, Windows ou Linux
  * **Tamanho do Sistema SAP**: o tamanho do sistema SAP
    * A quantidade de SAPs que o novo sistema fornecerá. Se você não tiver certeza de quantos SAPs o sistema precisará, pergunte ao seu Parceiro de tecnologia SAP ou Integrador de sistemas
  * **Disponibilidade do Sistema**: (apenas para o modelo de três camadas) a disponibilidade do sistema 
    * Selecione HA para uma configuração adequada para uma instalação de alta disponibilidade. Dois servidores de banco de dados e dois servidores para o ASCS serão criados.
...
Supondo que o disco já tenha sido carregado e definido no Azure (veja [Guia de planejamento e implementação][planning-guide]), siga estas etapas:

#### <a name="create-virtual-machine"></a>Criar máquina virtual
Para criar uma implantação usando um disco de sistema operacional privado por meio do Portal do Azure, use o modelo SAP publicado no [repositório azure-quickstart-templates do GitHub][azure-quickstart-templates-github]. Você também pode criar uma máquina virtual manualmente usando o [PowerShell][virtual-machines-windows-create-vm-specialized].
...
Para obter mais detalhes sobre rotas definidas pelo usuário, consulte [este artigo][virtual-networks-udr-overview].
