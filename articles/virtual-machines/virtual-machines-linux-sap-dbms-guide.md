<properties
   pageTitle="SAP NetWeaver em VMs (máquinas virtuais) do Linux – Guia de implantação de DBMS | Microsoft Azure"
...
# SAP NetWeaver em VMs (máquinas virtuais) do Azure – Guia de implantação de DBMS
...
* [SAP NetWeaver em VMs (máquinas virtuais) do Azure – Guia de implantação de DBMS (este documento)][dbms-guide]

As seguintes Notas SAP estão relacionadas ao tópico da SAP no Azure:
...
> ![Linux][Logo_Linux] Linux
>
> Somente o MDADM tem suporte para criar um RAID de software no Linux. Confira [este][virtual-machines-linux-configure-raid] artigo para obter mais informações sobre o uso do RAID de software.
...
* **ZRS (Armazenamento com Redundância de Zona)**: nesse caso, as três réplicas dos dados permanecem na mesma região do Azure. Conforme explicado [neste][deployment-guide-3] capítulo do [Guia de implantação][deployment-guide], uma região do Azure pode ser um número de datacenters nas proximidades. No caso do LRS, as réplicas devem ser distribuídas em diferentes datacenters que compõem uma região do Azure.

Mais informações podem ser encontradas [aqui][storage-redundancy].
 
> [AZURE.NOTE] Para implantações de DBMS, o uso do Armazenamento de Redundância Geográfica não é recomendado
>
...
#### Mapeando VHDs em contas de armazenamento do serviço de máquina virtual do Azure
...
Enquanto usando a série DS de VMs do Azure é possível montar VHDs de Contas de Armazenamento Standard e Contas de Armazenamento Premium do Azure. Casos de uso como a gravação de backups em VHDs apoiados pelo Armazenamento Standard enquanto se tem dados DBMS e arquivos de log no Armazenamento Premium vêm à mente onde tal armazenamento heterogêneo poderia ser aproveitado.
...
Outro artigo que resume as práticas recomendadas sobre o Armazenamento Standard do Azure e Contas de Armazenamento Standard do Azure pode ser encontrado aqui <https://blogs.msdn.com/b/mast/archive/2014/10/14/configuring-azure-virtual-machines-for-optimal-storage-performance.aspx>
 
#### Movendo VMs DBMS implantadas do Armazenamento Standard do Azure para o Armazenamento Premium do Azure
...
### Implantação de VMs para SAP no Azure
...
#### Implantando uma VM do Azure Marketplace
...
### VMs implantadas em nós do Azure
...
Os conjuntos de disponibilidade do Microsoft Azure são um agrupamento lógico de VMs ou serviços que garante que as VMs e outros serviços sejam distribuídos em diferentes domínios de falha e de atualização em um cluster, de modo que haja apenas um desligamento de nó em um determinado momento (leia [este][virtual-machines-manage-availability] artigo para obter mais detalhes).
...
* A exceção é se é feita a atribuição de um endereço IP estático para uma interface de rede conforme descrito [aqui][virtual-networks-reserved-private-ip].
* Nesse caso o endereço IP permanece fixo contanto que a interface de rede não seja excluída.

> [AZURE.IMPORTANT] Para manter toda a implantação simples e gerenciável, a recomendação clara é configurar a parceria de VMs em uma configuração de HA ou DR de DBMS no Azure de uma maneira que haja uma resolução de nome funcionando entre as diferentes VMs envolvidas.
...
Para obter detalhes sobre a implantação de componentes que fornecem dados de host ao SAPOSCOL e ao SAPHostAgent e o gerenciamento de ciclo de vida desses componentes, consulte o [Guia de implantação][deployment-guide]
...
### Diretrizes de configuração do SQL Server para instalações do SQL Server relacionadas ao SAP em VMs do Azure

#### Recomendações sobre a estrutura de VM/VHD para implantações do SQL Server relacionadas ao SAP
...
A compactação de banco de dados funciona bem nas máquinas virtuais do Azure como o faz localmente. Para obter mais detalhes sobre como compactar um banco de dados do SQL Server do SAP existente, consulte aqui: <https://blogs.msdn.com/b/saponsqlserver/archive/2010/10/08/compressing-an-sap-database-using-report-msscompress.aspx>
  
### SQL Server 2014: armazenando arquivos do banco de dados arquivos diretamente no Armazenamento de Blobs do Azure
...
1. O SQL Server 2012 CU4 e superior podem fazer o backup de bancos de dados para uma URL de forma nativa. Isso é detalhado no blog de [New functionality in SQL Server 2014 – Part 5 – Backup/Restore Enhancements](https://blogs.msdn.com/b/saponsqlserver/archive/2014/02/15/new-functionality-in-sql-server-2014-part-5-backup-restore-enhancements.aspx) (Nova funcionalidade no SQL Server 2014, parte 5, aprimoramentos de backup e restauração). Consulte o capítulo [SQL Server 2012 SP1 CU4 e posterior][dbms-guide-5.5.1].
1. Versões do SQL Server anteriores ao SQL 2012 CU4 podem usar uma funcionalidade de redirecionamento para fazer o backup em um VHD e basicamente mover o fluxo de gravação para um local de Armazenamento do Azure que tenha sido configurado. Consulte o capítulo [SQL Server 2012 SP1 CU3 e versões anteriores][dbms-guide-5.5.2].
1. O método final é executar um comando de backup convencional do SQL Server para disco em um dispositivo de disco VHD. Isso é idêntico ao padrão de implantação local e não é discutido em detalhes neste documento.
...
#### Outras possibilidades para fazer o backup de bancos de dados SQL Server
...
#### Considerações de desempenho para backups/restaurações
...
* As versões do SQL Server que não são de avaliação acarretam em custos mais elevados do que uma VM ‘Somente Windows’ implantada do Azure Marketplace. Consulte estes artigos para comparar preços: <https://azure.microsoft.com/pricing/details/virtual-machines/> e <https://azure.microsoft.com/pricing/details/virtual-machines/#Sql>. 
...
#### Alterando o agrupamento do SQL Server de uma VM Microsoft Windows/SQL Server
...
* Execute o comando: Setup.exe /QUIET /ACTION=REBUILDDATABASE /INSTANCENAME=MSSQLSERVER /SQLSYSADMINACCOUNTS=<local_admin_account_name> /SQLCOLLATION=SQL\_Latin1\_General\_Cp850\_BIN2   
...
* Execute o comando sp\_helpsort no banco de dados mestre do SQL Server.

O resultado desejado deve ter uma aparência semelhante a essa:

	Latin1-General, binary code point comparison sort for Unicode Data, SQL Server Sort Order 40 on Code Page 850 for non-Unicode Data

Se esse não for o resultado, INTERROMPA a implantação do SAP e investigue por que o comando de instalação não funcionou conforme o esperado. A implantação de aplicativos SAP NetWeaver na instância do SQL Server com páginas de código do SQL Server diferentes da mencionada acima é **NÃO** tem suporte.
...
[comentário]: <> (O artigo ainda está fazendo referência a ASM) [comentário]: <> (Antes de ler as diferentes tecnologias de alta disponibilidade específicas utilizáveis para o SQL Server no Azure, há um documento muito bom que fornece mais detalhes e ponteiros [aqui][virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions])
...
Um tutorial para configurar o espelhamento de banco de dados no Azure pode ser encontrado aqui: <https://technet.microsoft.com/library/ms189852.aspx>
...
[comentário]: <> (taxa de transferência maior de 2.3 do que o que? De um VHD?)
...
1. Planeje cuidadosamente sua estrutura de sistema da SAP no Azure para balancear o layout do arquivo de dados e as restrições do Azure:
...
1. Nunca instale software ou coloque quaisquer arquivos que exijam persistência na unidade D:\\, uma vez que ela não é permanente e tudo nessa unidade será perdido em uma reinicialização do Windows.
1. Não use o caching de VHD do Azure para o Armazenamento Standard do Azure.
1. Não use contas de armazenamento com replicação geográfica do Azure. Use Localmente Redundante para cargas de trabalho do DBMS.
1. Use a solução de HA/DR do seu fornecedor do DBMS para replicar dados do banco de dados.
1. Sempre use a resolução de nome, não confie em endereços IP.
1. Use a maior compactação de banco de dados possível. Para o SQL Server, essa é a compactação de página.
1. Tenha cuidado ao usar imagens do SQL Server do Azure Marketplace. Se você usar o SQL Server um, deverá alterar o agrupamento de instância antes de instalar qualquer sistema SAP NetWeaver nele.
1. Instale e configure o monitoramento de host do SAP para Azure conforme descrito no [Guia de implantação][deployment-guide].

## Informações específicas para o SAP ASE no Windows
...
Há um SLA para as máquinas virtuais do Azure que pode ser encontrado aqui: <https://azure.microsoft.com/support/legal/sla>
...
### Diretrizes de configuração do SAP ASE para instalações do SAP ASE relacionadas ao SAP em VMs do Azure

#### Estrutura da implantação do SAP ASE
...
No caso de cargas de trabalho específicas do ERP ou todas do BW, faz sentido, em relação ao desempenho, manter os dispositivos de tempdb do tempdb adicionalmente criado (por SWPM ou manualmente) em uma unidade diferente da C:. Caso não exista nenhum tempdb adicional, é recomendável criar um (Nota SAP [1752266]).
...
#### Usando o DBACockpit para monitorar instâncias do banco de dados
...
Definindo o parâmetro de perfil do SAP icm/host\_name\_full para o nome DNS da VM do Azure, o link pode ser semelhante a:
...
#### Considerações sobre backup/recuperação para o SAP ASE
...
A distribuição de um volume em vários VHDs montados foi discutida anteriormente nesse guia. Para obter mais informações sobre o uso de vários diretórios na configuração de despejo do ASE SAP, consulte a documentação sobre o procedimento armazenado sp\_config\_dump que é usado para criar a configuração de despejo no [Infocenter do Sybase](http://infocenter.sybase.com/help/index.jsp).

### Recuperação de desastre com VMs do Azure

#### Replicação de dados com o Servidor de Replicação do SAP Sybase
...
## Informações específicas para o SAP ASE no Linux
...
As informações de dimensionamento do SAP e uma lista dos SKUs de VM certificados do SAP serão fornecidas na Nota SAP [1928533]. Os documentos adicionais sobre o dimensionamento do SAP para máquinas virtuais do Azure podem ser encontrados aqui <http://blogs.msdn.com/b/saponsqlserver/archive/2015/06/19/how-to-size-sap-systems-running-on-azure-vms.aspx> e aqui <http://blogs.msdn.com/b/saponsqlserver/archive/2015/12/01/new-white-paper-on-sizing-sap-solutions-on-azure-public-cloud.aspx>
...
Nunca coloque qualquer diretório do SAP ASE em /mnt ou /mnt/resource da VM. Isso também se aplicará ao tempdb, mesmo se os objetos mantidos no tempdb forem apenas temporários porque /mnt ou /mnt/resource é um espaço temporário da VM do Azure padrão que não é persistente. Mais detalhes sobre o espaço temporário da VM do Azure podem ser encontrados [neste artigo][virtual-machines-linux-how-to-attach-disk]
...
## Informações específicas do Banco de Dados Oracle no Windows
...
### Suporte de versão do Oracle
Todos os detalhes sobre as versões do Oracle e versões de SO correspondentes que têm suporte para execução do SAP no Oracle nas máquinas virtuais do Azure podem ser encontrados na Nota SAP [2039619] a seguir

Informações gerais sobre como executar o SAP Business Suite no Oracle podem ser encontradas no SCN: <https://scn.sap.com/community/oracle>

### Diretrizes de configuração do Oracle para instalações do SAP em VMs do Azure

#### Configuração de armazenamento
Há suporte a apenas uma única instância do Oracle usando discos formatados em NTFS. Todos os arquivos de banco de dados devem ser armazenados no sistema de arquivos NTFS com base em discos VHD. Esses VHDs são montados na VM do Azure e são baseados no Armazenamento de Blobs de páginas do Azure (<https://msdn.microsoft.com/library/azure/ee691964.aspx>). Qualquer variante de unidades de rede ou compartilhamentos remotos como serviços de arquivo do Azure:
...
Usando VHDs do Azure com base no Armazenamento de Blobs de páginas do Azure, as declarações feitas nos capítulos [Cache para VMs e VHDs][dbms-guide-2.1] e [Armazenamento do Microsoft Azure][dbms-guide-2.3] nesse documento também se aplicam às implementações com o Banco de Dados Oracle.

Conforme explicado anteriormente na parte geral do documento, existem cotas na taxa de transferência de IOPS para VHDs do Azure. As cotas exatas dependem do tipo de VM usado. Uma lista de tipos de VM com suas cotas pode ser encontrada [aqui][virtual-machines-size-specs]
...
Se mais IOPS forem necessários, será altamente recomendável usar pools de armazenamento do Windows (disponíveis apenas no Windows Server 2012 e superior) ou distribuição do Windows para o Windows 2008 R2 para criar um grande dispositivo lógico em vários discos VHD montados. Consulte também o capítulo [RAID de Software][dbms-guide-2.2] deste documento. Essa abordagem simplifica a sobrecarga administrativa para gerenciar o espaço em disco e evita o esforço para distribuir manualmente os arquivos entre vários VHDs montados.
...
[comentário]: <> (o link se refere ao ASM) O Oracle Data Guard tem suporte para fins de recuperação de desastre e alta disponibilidade. É possível encontrar detalhes [nessa][virtual-machines-configuring-oracle-data-guard] documentação.
...
### Documentação do SAP MaxDB disponível
...
> [AZURE.IMPORTANT] Como outros bancos de dados, o SAP MaxDB também tem arquivos de log e de dados. No entanto, na terminologia do SAP MaxDB o termo correto é "volume" (não "arquivo"). Por exemplo, há volumes de dados e volumes de log do SAP MaxDB. Não os confunda com volumes de disco do SO.
...
* Defina o caching de arquivo apropriado para cada blob do Azure, dependendo de se você o usa para volumes de log ou de dados (ou seja, arquivos) do SAP MaxDB, e se usa o Armazenamento Standard ou o Armazenamento Premium do Azure, conforme descrito no capítulo [Caching para VMs][dbms-guide-2.1].
...
* Se mais IOPS e/ou espaço forem necessários, será altamente recomendável usar pools de armazenamento do Microsoft Windows (disponíveis apenas no Microsoft Windows Server 2012 e superior) ou distribuição do Microsoft Windows para o Microsoft Windows 2008 R2 para criar um grande dispositivo lógico em vários discos VHD montados. Consulte também o capítulo [RAID de Software][dbms-guide-2.2] deste documento. Essa abordagem simplifica a sobrecarga administrativa para gerenciar o espaço em disco e evita o esforço de distribuir manualmente os arquivos entre vários VHDs montados.
...
A distribuição de um volume em vários VHDs montados foi discutida anteriormente no capítulo [RAID de software][dbms-guide-2.2] desse documento.

#### <a name="f77c1436-9ad8-44fb-a331-8671342de818"></a>Outros
Todos os outros tópicos gerais, como o monitoramento do SAP ou conjuntos de disponibilidade do Azure, também se aplicam como descrito nos três primeiros capítulos desse documento para implantações de VMs com o banco de dados SAP MaxDB. Outras configurações específicas do SAP MaxDB são transparentes para VMs do Azure e são descritas em diferentes documentos listados na Nota SAP [767598] e nestas Notas SAP:

* [826037] 
* [1139904]
* [1173395]

## Informações específicas para o SAP liveCache no Windows
...
#### Tipos de VM do Azure recomendados
...
#### VM do Azure dedicada para o liveCache
...
O backup e restauração, incluindo considerações de desempenho, já estão descritos nos capítulos relevantes do SAP MaxDB [Backup e restauração][dbms-guide-8.4.2] e [Considerações de desempenho para backup e restauração][dbms-guide-8.4.3].

#### Outros
Todos os outros tópicos gerais estão descritos [neste][dbms-guide-8.4.4] capítulo do SAP MaxDB relevante.

## Informações específicas para o SAP Content Server no Windows
...
### Suporte de versão do SAP Content Server
...
É altamente recomendável usar a versão mais recente do SAP Content Server, que no momento da redação deste documento é **6.50 SP4**, e a versão mais recente do **Microsoft IIS 8.5**.

Verifique as versões mais recentes com suporte do SAP Content Server e o Microsoft IIS no [PAM (Matriz de Disponibilidade de Produto) da SAP][sap-pam].
...
1. **O cliente é um navegador da Web local** O SAP Content Server pode ser configurado para ser acessado diretamente pelo navegador da Web. Nesse caso, um navegador da Web em execução no local é um cliente do SAP Content Server. O datacenter local e o datacenter do Azure são colocados em diferentes locais físicos (idealmente próximos entre si). Seu datacenter local é conectado ao Azure por meio da Rota Expressa ou VPN site a site do Azure. Embora as duas opções ofereçam uma conexão de rede VPN segura para o Azure, a conexão de rede site a site não oferece uma largura de banda de rede e SLA de latência entre o datacenter local e o datacenter do Azure. Para acelerar o acesso a documentos, você pode realizar uma das seguintes ações:
    1. Instale o SAP Cache Server localmente, próximo ao navegador da Web de local (opção [nessa][dbms-guide-900-sap-cache-server-on-premises] figura)
    1. Configure a Rota Expressa do Azure, que oferece uma conexão de rede dedicada de alta velocidade e baixa latência entre o datacenter local e o datacenter do Azure.
 
![Opção de instalação local do SAP Cache Server][dbms-guide-figure-900] <a name="642f746c-e4d4-489d-bf63-73e80177a0a8"></a>

#### Backup/restauração
Se você configura o SAP Content Server para armazenar arquivos no banco de dados SAP MaxDB, as considerações de desempenho e o procedimento de backup/restauração já estão descritas no capítulo [Backup e restauração][dbms-guide-8.4.2] do SAP MaxDB e o capítulo [Considerações de desempenho para backup e restauração][dbms-guide-8.4.3].
...
## Informações específicas para o IBM DB2 para LUW no Windows
Com o Microsoft Azure, você pode facilmente migrar seu aplicativo SAP existente em execução no IBM DB2 para LUW (Linux, UNIX e Windows) para máquinas virtuais do Azure. Com o SAP no IBM DB2 para LUW, os administradores e desenvolvedores ainda podem usar as mesmas ferramentas de desenvolvimento e administração que estão disponíveis localmente. As informações gerais sobre como executar o SAP Business Suite no IBM DB2 para LUW podem ser encontradas na SCN (SAP Community Network) em <https://scn.sap.com/community/db2-for-linux-unix-windows>.
...
Se você estiver usando VHDs do Azure com base no Armazenamento de Blobs de páginas do Azure, as declarações feitas no capítulo [Estrutura de uma implantação do RDBMS][dbms-guide-2] também se aplicarão a implantações com o banco de dados do IBM DB2 para LUW.
...
Alternativamente, você pode usar pools de armazenamento do Windows (disponíveis apenas no Windows Server 2012 e superior) ou distribuição do Windows para o Windows 2008 R2 conforme descrito no capítulo [RAID de software][dbms-guide-2.2] desse documento para criar um grande dispositivo lógico em vários discos VHD montados. Para os discos que contém os caminhos de armazenamento do DB2 para os sapdata e saptmp, você deve especificar um tamanho de setor do disco físico de 512 KB. Ao usar pools de armazenamento do Windows, você deve criar os pools de armazenamento manualmente por meio da interface de linha de comando usando o parâmetro “-LogicalSectorSizeDefault”. Para saber mais, confira <https://technet.microsoft.com/library/hh848689.aspx>.
...
Não use a replicação geográfica do armazenamento do Azure. Para obter mais informações, consulte o capítulo [Armazenamento do Microsoft Azure][dbms-guide-2.3] e o capítulo [Alta disponibilidade e recuperação de desastre com VMs do Azure][dbms-guide-3].
