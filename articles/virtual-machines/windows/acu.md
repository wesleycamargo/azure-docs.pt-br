---
title: Visão geral da Unidade de Computação do Azure | Microsoft Docs
description: Visão geral do conceito das unidades computação do Azure para Windows. A ACU é uma maneira de comparar o desempenho da CPU entre SKUs do Azure.
services: virtual-machines-windows
documentationcenter: ''
author: jonbeck7
manager: jeconnoc
editor: ''
tags: azure-resource-manager,azure-service-management
ms.assetid: ''
ms.service: virtual-machines-windows
ms.devlang: na
ms.topic: article
ms.tgt_pltfrm: vm-windows
ms.workload: infrastructure-services
ms.date: 12/21/2018
ms.author: jonbeck
ms.openlocfilehash: ae1d6ddfb96d65a730fd1008f44c663d3b447b17
ms.sourcegitcommit: d2329d88f5ecabbe3e6da8a820faba9b26cb8a02
ms.translationtype: HT
ms.contentlocale: pt-BR
ms.lasthandoff: 02/16/2019
ms.locfileid: "56326112"
---
# <a name="azure-compute-unit-acu"></a>ACU (unidade de computação do Azure)

[!INCLUDE [virtual-machines-common-acu](../../../includes/virtual-machines-common-acu.md)]


Nos links abaixo, você obtém mais informações sobre os diferentes tamanhos:
- [Uso geral](sizes-general.md)
- [Memória otimizada](../virtual-machines-windows-sizes-memory.md)
- [Computação otimizada](sizes-compute.md)
- [GPU otimizada](sizes-gpu.md)
- [Computação de alto desempenho](sizes-hpc.md)
- [Armazenamento otimizado](sizes-storage.md)
