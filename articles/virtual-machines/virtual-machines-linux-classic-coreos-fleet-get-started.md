<properties
	...
	description="Fornece exemplos básicos de como usar o fleet e o Docker em um cluster de VM do Linux no CoreOS criado com o modelo de implantação clássica no Azure."
...
Aqui estão dois exemplos rápidos de como usar o [fleet](https://github.com/coreos/fleet) e o [Docker](https://www.docker.com/) para executar aplicativos em um cluster de máquinas virtuais do [CoreOS] no Azure.

Para usar esses exemplos, primeiro configure um cluster CoreOS de três nós como descrito em [Como usar o CoreOS no Azure]. Feito isso, você entenderá os elementos básicos das implantações de CoreOS e terá um cluster e um computador cliente funcionais. Vamos usar exatamente o mesmo nome de cluster nestes exemplos. Além disso, estes exemplos supõem que você esteja usando o host local do Linux para executar os comandos **fleetctl**. Consulte [Usando o cliente](https://coreos.com/fleet/docs/latest/using-the-client.html) para obter mais informações sobre o cliente **fleetctl**.


## Exemplo 1: Hello World com Docker</a>
...
## Exemplo 2: Servidor nginx altamente disponível</a>
...
>[AZURE.IMPORTANT] Para executar o servidor Web altamente disponível, você precisará configurar um ponto de extremidade HTTP com balanceamento de carga nas máquinas virtuais (porta pública 80, porta privada 80). Você pode fazer isso depois de criar o cluster CoreOS, usando o portal clássico do Azure ou o comando **azure vm endpoint**. Veja [Configurar um conjunto com balanceamento de carga] para obter as etapas.
...
* Você pode tentar algo mais com o cluster CoreOS de três nós no Azure. Explore como criar clusters mais complexos e como usar o Docker para criar aplicativos mais interessantes. Para isso, leia o [Tutorial de Tim Park sobre o CoreOS], o [Tutorial de Patrick Chanezon sobre o CoreOS], a documentação do [Docker] e a [Visão geral do CoreOS].

* Para começar a usar o Fleet e CoreOS no Gerenciador de Recursos do Azure, tente este [modelo de início rápido](https://github.com/Azure/azure-quickstart-templates/tree/master/coreos-with-fleet-multivm).
...
[Configurar um conjunto com balanceamento de carga]: ../load-balancer/load-balancer-get-started-internet-classic-cli.md
...
[Computação de software livre e do Linux no Azure]: virtual-machines-linux-opensource-links.md
