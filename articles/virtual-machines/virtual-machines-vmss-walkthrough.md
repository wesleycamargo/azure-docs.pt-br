...
O modelo que você cria neste tutorial é semelhante a um modelo que pode ser encontrado na Galeria de modelos. Para obter mais informações, confira [Implantar um simples Conjunto de Dimensionamento de VM com VMs do Windows e um Jumpbox](https://azure.microsoft.com/blog/azure-vm-scale-sets-public-preview).
...
2.	**Criar um grupo de recursos** – Todos os recursos devem ser implantados em um grupo de recursos. Neste tutorial, nomeie o grupo de recursos **vmss-test1**. Confira [New-AzureRmResourceGroup](https://msdn.microsoft.com/library/mt603739.aspx).

3.	**Implantar uma conta de armazenamento para o novo grupo de recursos** – Este tutorial usa várias contas de armazenamento para facilitar o conjunto de dimensionamento de máquina virtual. Use [New-AzureRmStorageAccount](https://msdn.microsoft.com/library/mt607148.aspx) para criar uma conta de armazenamento denominada **vmssstore1**. Mantenha a janela do Microsoft Azure PowerShell aberta para etapas posteriormente neste tutorial.
...
2.	Os parâmetros nem sempre são necessários, mas eles facilitam o gerenciamento de modelos. Eles fornecem uma maneira de especificar valores para o modelo, descrevem o tipo do valor, o valor padrão, se necessário, e possivelmente os valores permitidos do parâmetro.
	...
	- Um prefixo para nomes de contas de armazenamento que são usados pelas máquinas virtuais no conjunto de dimensionamento.
...
7.	Adicione o recurso de balanceador de carga que é usado pelo conjunto de dimensionamento. Para obter mais informações, confira [Suporte do Gerenciador de Recursos do Azure para Balanceador de Carga](../load-balancer/oad-balancer-arm.md).
...
8.	Adicione o recurso de interface de rede que é usado pela máquina virtual de jumpbox.
...
9.	Adicione o recurso de máquina virtual na mesma rede que o conjunto de dimensionamento. Como máquinas em um conjunto de dimensionamento de máquina virtual não são diretamente acessíveis usando um endereço IP público, uma máquina virtual de jumpbox é criada na mesma rede virtual como o conjunto de dimensionamento e é usada para acessar remotamente as máquinas no conjunto.
