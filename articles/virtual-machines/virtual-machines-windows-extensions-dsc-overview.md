...
A extensão de DSC do Azure usa a estrutura do Agente do Azure para entregar, aplicar e gerar relatórios sobre configurações da DSC executadas em VMs do Azure. A extensão de DSC espera um arquivo .zip contendo pelo menos um documento de configuração e um conjunto de parâmetros fornecidos por meio do SDK do Azure PowerShell ou da IU Portal.

Quando a extensão é chamada pela primeira vez, ela executa um processo de instalação. Esse processo instala uma versão do Windows Management Framework (WMF) conforme definido abaixo:
...
- ResourceGroupName, VMName, ArchiveStorageAccountName, Version, Location são os novos parâmetros necessários.
...
## Funcionalidade do portal de visualização ##
Navegue até uma VM. Em Configurações -> Gerenciar clique em "extensões". Um novo painel é criado. Clique em "Adicionar" e selecione DSC do PowerShell.
...
**Nome de configuração qualificado por módulo**: os arquivos .ps1 podem ter várias funções de configuração. Insira o nome do script .ps1 de configuração seguido por um '' e o nome da função de configuração. Por exemplo

**Argumentos de configuração**: Se a função de configuração leva argumentos, digite-os aqui no formato `argumentName1=value1,argumentName2=value2`. Observe que esso é um formato diferente daquele como argumentos de configuração são aceitos por meio de cmdlets do PowerShell ou modelos de ARM.
