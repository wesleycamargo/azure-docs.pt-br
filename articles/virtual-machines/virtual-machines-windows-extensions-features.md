...
- Aplique as configurações de Estado Desejado do PowerShell a uma máquina virtual usando a extensão de DSC para Windows. Para saber mais, confira [Extensão de configuração de Estado Desejado do Azure](virtual-machines-windows-extensions-dsc-overview.md).
...
- Configure uma máquina virtual do Azure usando o Chef. Para saber mais, veja [Automatizar a implantação de máquina virtual do Azure com o Chef](virtual-machines-windows-chef-automation.md).

Além de extensões específicas ao processo, uma extensão de Script Personalizado está disponível para máquinas virtuais Windows e Linux. A extensão de Script personalizado para Windows permite a execução de qualquer script do PowerShell em uma máquina virtual. Isso é útil para a criação de implantações do Azure que exigem uma configurações que vão além da capacidade das ferramentas nativas do Azure. Para saber mais, veja [Extensão de Script personalizado de VM do Windows](virtual-machines-windows-extensions-customscript.md).

Para trabalhar em um exemplo no qual uma extensão de VM é usada em uma implantação de aplicativos de ponta a ponta, confira [Automatizar implantações de aplicativos em máquinas virtuais do Azure](virtual-machines-windows-dotnet-core-1-landing.md).
...
O exemplo a seguir usa a extensão de Script personalizado para baixar um script de um repositório do GitHub para a máquina virtual de destino e, em seguida, executa o script. Para saber mais sobre como usar a extensão de Script personalizado, veja [Visão geral da extensão de Script personalizado](virtual-machines-windows-extensions-customscript.md).
...
Neste exemplo, a extensão de acesso à VM é usada para redefinir a senha administrativa de uma máquina virtual Windows. Para saber mais sobre a extensão de acesso à VM, veja [Serviço de redefinição de área de trabalho remota em uma VM do Windows](virtual-machines-windows-reset-rdp.md).
...
![Instalar a extensão de antimalware](./media/virtual-machines-windows-extensions-features/installantimalwareextension.png)
...
Para saber mais, veja [Criação de modelos do Azure Resource Manager com extensões de VM do Windows](virtual-machines-windows-extensions-authoring-templates.md).
