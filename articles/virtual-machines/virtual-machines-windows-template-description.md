...
- [tamanho](virtual-machines-windows-sizes.md)
- [nome](virtual-machines-linux-infrastructure-naming-guidelines.md) e credenciais
- disco e [configurações do sistema operacional](virtual-machines-windows-cli-ps-findimage.md)
- [adaptador de rede](../virtual-network/virtual-networks-multiple-nics.md) 
- diagnóstico de inicialização

## <a name="disks-and-images"></a>Discos e imagens
   
No Azure, arquivos VHD podem representar [discos ou imagens](virtual-machines-windows-about-disks-vhds.md). Quando o sistema operacional em um arquivo VHD é especializado para ser uma VM específica, ele é conhecido como um disco. Quando o sistema operacional em um arquivo VHD é generalizado para ser usado para criar muitas VMs, ele é conhecido como uma imagem.   
...
Embora [extensões](virtual-machines-windows-extensions-features.md) sejam um recurso separado, elas estão estreitamente relacionadas a VMs. As extensões podem ser adicionadas como um recurso filho da VM ou como um recurso separado. O exemplo mostra a [Extensão de diagnóstico](virtual-machines-windows-extensions-diagnostics-template.md) que está sendo adicionada às VMs:
...
Há muitas extensões que podem ser instalados em uma VM, mas a mais útil é provavelmente a [Extensão de Script Personalizado](virtual-machines-windows-extensions-customscript.md). No exemplo, um script do PowerShell chamado start.ps1 é executado em cada VM quando ela é iniciada pela primeira vez:
...
![Obter informações sobre a implantação](./media/virtual-machines-windows-template-description/virtual-machines-deployment-info.png)
