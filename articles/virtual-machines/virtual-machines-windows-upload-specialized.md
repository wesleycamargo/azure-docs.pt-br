...
Se você pretende usar o VHD especializado no estado em que se encontra para criar uma nova VM, não deixe de concluir as etapas a seguir. 
  * Se você pretende usar discos gerenciados, consulte [Como planejar a migração em Managed Disks](virtual-machines-windows-on-prem-to-azure.md#plan-for-the-migration-to-managed-disks).
...
O sistema operacional especializado pode ser um VHD que você [carregou no Azure](virtual-machines-windows-upload-image.md) ou um [VHD copiado de uma VM do Azure existente](virtual-machines-windows-vhd-copy.md). 
...
Opcional: anexe discos gerenciados adicionais como discos de dados. Essa opção pressupõe que você criou os discos de dados gerenciados usando [Como criar discos de dados gerenciados](virtual-machines-windows-create-managed-disk-ps.md). 
