...
Este artigo mostra como habilitar a entrada para usuários usando o ponto de extremidade comum do Azure Active Directory (Azure AD) por meio do uso de [políticas personalizadas](active-directory-b2c-overview-custom.md).
...
Para habilitar a entrada de usuários que usam o ponto de extremidade multilocatário do Azure AD, você precisa ter um aplicativo multilocatário registrado em qualquer um dos seus locatários do Azure AD. Neste artigo, mostraremos como criar um aplicativo multilocatário do Azure AD em seu locatário do Azure AD B2C. Em seguida, habilite a entrada para usuários com o uso do aplicativo multilocatário do Azure AD.

>[!NOTE]
> Se você quer que os usuários do Azure AD **e os usuários com contas da Microsoft** entrem, ignore esta seção e registre um aplicativo no [portal do desenvolvedor da Microsoft](https://apps.dev.microsoft.com).
...
2. Selecione **Mais serviços** no painel esquerdo e pesquise por “Registros do aplicativo”.
3. Selecione **Novo registro de aplicativo**.
4. Insira um nome para seu aplicativo (por exemplo, `Azure AD B2C App`).
5. Selecione **Aplicativo Web/API** como o tipo de aplicativo.
6. Para a **URL de Logon**, insira a URL a seguir, em que `yourtenant` é substituído pelo nome do seu locatário do Azure AD B2C (`fabrikamb2c.onmicrosoft.com`):
...
5. Selecione **Criar**.
6. Confirme que você criou a chave `B2C_1A_AADAppSecret`.
...
> [!NOTE]
> Usando `https://sts.windows.net` como um valor de prefixo permitirá que TODOS os usuários do Azure AD entrem no seu aplicativo.
...
6. Carregue o arquivo `TrustFrameworkExtensions.xml` e o arquivo RP (por exemplo, `SignUpOrSignInWithAAD.xml`) e verifique se eles são aprovados.
