<properties
	...
	description="Os tipos de tokens emitidos na visualização do Azure Active Directory B2C."
...
# Visualização do Azure AD B2C: referência de token
...
O Azure AD B2C dá suporte ao [protocolo de autorização do OAuth 2.0](active-directory-b2c-reference-protocols.md), que usa tokens de acesso e tokens de atualização. Ele também dá suporte à autenticação e à conexão por meio do [OpenID Connect](active-directory-b2c-reference-protocols.md), que introduz um terceiro tipo de token, o token de ID. Cada um desses tokens é representado como um token de portador.
...
Detalhes adicionais sobre as declarações em tokens de ID são listados mais adiante neste guia, junto com o token de ID de exemplo a seguir. Observe que as declarações em tokens de ID não são retornadas em uma ordem específica. Além disso, novas declarações podem ser introduzidas em tokens de ID a qualquer momento. Seu aplicativo não deve ser interrompido à medida que novas declarações são introduzidas. A lista inclui as declarações que o aplicativo pode interpretar confiavelmente no momento em que este documento foi escrito. Para praticar, tente inspecionar as declarações no exemplo de token de ID colando-o em [calebb.net](https://calebb.net). Mais detalhes podem ser encontrados na [especificação do OpenID Connect](http://openid.net/specs/openid-connect-core-1_0.html).

#### Token de ID de exemplo
```
...
O AD B2C do Azure não emite tokens de acesso no momento. Não há suporte para a autenticação entre duas partes no estágio de visualização. No entanto, você pode usar tokens de ID para a comunicação entre componentes do mesmo aplicativo. Saiba mais sobre os [aplicativos e cenários de autenticação com suporte na visualização do Azure AD B2C](active-directory-b2c-apps.md).
...
A única validação de token que seus aplicativos devem precisar realizar atualmente é validar tokens de ID. Para validar um token de ID, o aplicativo deve validar a assinatura e as declarações do token.
...
Um JWT contém três segmentos, separados pelo caractere `.`. O primeiro segmento é o **cabeçalho**, o segundo é o **corpo** e o terceiro é a **assinatura**. O segmento de assinatura pode ser usado para validar a autenticidade do token de ID, de modo que o aplicativo possa confiar nele.

Os tokens de ID são assinados usando algoritmos de criptografia assimétrica padrão do setor, como RSA 256. O cabeçalho do token de ID contém informações sobre o método de criptografia e a chave usados para assinar o token:
...
A declaração `alg` indica o algoritmo que foi usado para assinar o token. A declaração `kid` ou `x5t` indica a chave pública específica que foi usada para assinar o token.
...
`fabrikamb2c.onmicrosoft.com` é o diretório B2C usado para autenticar o usuário e `b2c_1_sign_in` é a política usada para adquirir o token de ID. Para determinar qual política foi usada para assinar um token de ID (e onde buscar os metadados), você tem duas opções. Primeiro, o nome da política é incluído na declaração `acr` no token de ID. Você pode analisar as declarações fora do corpo do JWT decodificando em base 64 o corpo e desserializando a cadeia de caracteres JSON resultante. A declaração `acr` será o nome da política que foi usada para emitir o token de ID. A outra opção é codificar a política no valor do parâmetro `state` quando você emitir a solicitação e, em seguida, decodificá-lo para determinar qual política foi usada. Ambos os métodos são válidos.
...
Quando o aplicativo ou a API recebe um token de ID, também deve fazer algumas verificações em relação às declarações no token de ID. Estão incluídos:
...
Detalhes dos valores esperados para essas declarações estão incluídos na [seção token de ID](#id-tokens) anterior.

## Tempos de vida do token

Os tempos de vida de token a seguir são fornecidos para expandir seus conhecimentos. Eles poderão ajudá-lo quando você desenvolver e depurar aplicativos. Observe que seus aplicativos não devem ser escritos com a esxpectativa de que esses tempos de vida permaneçam constantes. Eles podem e vão ser alterados.
