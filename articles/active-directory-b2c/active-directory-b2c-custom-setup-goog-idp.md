---
title: Adicionar Google+ como um provedor de identidade OAuth2 usando políticas personalizadas no Azure Active Directory B2C | Microsoft Docs
...
Para usar o Google+ como um provedor de identidade no Azure AD (Azure Active Directory B2C), você precisará criar um aplicativo do Google+ e fornecer a ele os parâmetros certos. É possível registrar um aplicativo do Google+ aqui: [https://accounts.google.com/SignUp](https://accounts.google.com/SignUp)
...
13.  Forneça um **Nome** para seu aplicativo, insira `https://{tenant}.b2clogin.com` no campo **Origens de JavaScript autorizadas** e `https://{tenant}.b2clogin.com/te/{tenant}.onmicrosoft.com/oauth2/authresp` no campo **URIs de redirecionamento autorizados**. Substitua **{tenant}** pelo nome do locatário (por exemplo, contosob2c). O valor **{tenant}** diferencia letras maiúsculas de minúsculas. Clique em **Criar**.
