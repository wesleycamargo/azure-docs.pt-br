<properties 
	pageTitle="Aplicativo LOB em sites do Azure com o AD FS" 
	description="Saiba como criar um aplicativo ASP.NET MVC LOB nos sites do Azure que autentica com o STS local. Este tutorial destina-se ao AD FS como STS local." 
	services="web-sites" 
...
# Criar um aplicativo de linha de negócios do ASP.NET MVC em sites do Azure que autentica com o AD FS #

Neste artigo, você aprenderá como criar um aplicativo de linha de negócios (LOB) do ASP.NET MVC nos [sites do Azure](http://azure.microsoft.com/services/websites/) usando [Serviços de Federação do Active Directory](http://technet.microsoft.com/ library/hh831502.aspx) locais como o provedor de identidade. Este cenário poderá funcionar quando você desejar criar aplicativos LOB nos sites do Azure, mas sua organização exigir que todos os dados sejam armazenados no local.
...
Você criará um aplicativo básico do ASP.NET em sites do Azure com os seguintes recursos:

- Autentica usuários no AD FS
- Usa `[Authorize]` para autorizar usuários para ações diferentes
- Configuração estática para depuração no Visual Studio e publicação em sites do Azure (configurar uma vez, depurar e publicar a qualquer momento)  
...
- Uma implantação do AD FS local (para uma explicação de ponta a ponta do laboratório de teste que eu uso, consulte [laboratório de teste: STS autônomo com o AD FS no Azure VM (teste)](tarefas))
...
O aplicativo de exemplo neste tutorial, [WebApp-WSFederation-DotNet)](https://github.com/AzureADSamples/WebApp-WSFederation-DotNet), é criado pela equipe do Active Directory do Azure. Como o AD FS dá suporte a WS-Federation, é possível criá-lo como um modelo para criar novos aplicativos de LOB com facilidade. Ele tem as seguintes vantagens:
...
2.	Clonar ou baixar a solução de exemplo em [WebApp-WSFederation-DotNet](https://github.com/AzureADSamples/WebApp-WSFederation-DotNet) em seu diretório local.
...
Aqui, você publicará o aplicativo em um site do Azure, preservando o ambiente de depuração. Observe que você pretende publicar o aplicativo antes que ele tenha uma relação de confiança RP com o AD FS, para que autenticação ainda não funcione. No entanto, se fizer isso agora, você poderá ter a URL do site que também usará para configurar a relação de confiança RP mais tarde.
...
5. Preencha todos os campos obrigatórios. Você pretende se conectar a dados locais posteriormente, para não criar um banco de dados para este site.
...
11. No Visual Studio, abra **Web.Release.config** em seu projeto. Insira o seguinte XML em `<configuration>` e substitua o valor de chave com sua URL do site de publicação.  
	<pre class="prettyprint">
&lt;appSettings&gt;
   &lt;add key="ida:RPIdentifier" value="<mark>[e.g. https://mylobapp.azurewebsites.net/]</mark>" xdt:Transform="SetAttributes" xdt:Locator="Match(key)" /&gt;
&lt;/appSettings&gt;</pre>

Quando terminar, você terá dois identificadores RP configurados em seu projeto, um para o seu ambiente de depuração no Visual Studio e um para o site publicado no Azure. Você configurará uma relação de confiança RP para cada um dos dois ambientes do AD FS. Durante a depuração, as configurações do aplicativo em Web.config é usado para fazer sua configuração **Depurar** funcionar com AD FS, e quando ele é publicado com (por padrão, a configuração **Versão** é publicada), um Web.config transformado é carregado e incorpora as alterações de configuração do aplicativo em Web.Release.config.

Se você deseja anexar o site publicado ao depurador (ou seja, você deve carregar símbolos de depuração do seu código no site publicado), você pode criar um clone da configuração de depuração para depuração do Azure, mas com sua própria transformação de Web.config (por exemplo, Web.AzureDebug.config) personalizado que usa as configurações de aplicativo de Web.Release.config. Isso permite que você mantenha uma configuração estática em diferentes ambientes.

<a name="bkmk_rptrusts"></a>
## Configurar a terceira parte confiável no gerenciamento do AD FS ##

Agora você precisa configurar uma relação de confiança RP no gerenciamento do AD FS antes de seu aplicativo de exemplo poder realmente autenticar com o AD FS. Você precisará configurar duas relações de confiança RP separadas, uma para o seu ambiente de depuração e outra para seu site publicado.
...
6.	Na página **Especificar nome para exibição**, digite um nome de exibição para o aplicativo e clique em **Próximo**.
7.	Na página **Escolher protocolo**, clique em **Próximo**.
8.	Na página **Configurar certificado**, clique em **Próximo**.
...
6.	De volta no gerenciamento do AD FS, a página **Configurar URL** de **Adicionar assistente de objeto de confiança de terceira parte confiável**, selecione **Habilitar o suporte para o protocolo WS-Federation Passive** e digite a URL do SSL do seu projeto do Visual Studio que você anotou na etapa anterior. Em seguida, clique em **Próximo**.

	![](./media/web-sites-dotnet-lob-application-adfs/4-configure-url.png)

	> [AZURE.NOTE] A URL especifica para onde enviar o cliente após a autenticação bem-sucedida. Para o ambiente de depuração, deve ser <code>https://localhost:&lt;port&gt;/</code>. Para o site publicado, ele deve ser a URL do site.

7.	Na página **Configurar identificadores**, verifique se o seu projeto de URL de SSL já está listado e clique em **Próximo**. Clique em **Próximo** até o final do assistente com as seleções padrão.

	> [AZURE.NOTE] No App_Start\Startup.Auth.cs do projeto do Visual Studio, esse identificador é comparado com o valor de <code>WsFederationAuthenticationOptions.Wtrealm</code> durante a autenticação federada. Por padrão, a URL do aplicativo da etapa anterior é adicionada como um identificador RP.

8.	Agora você concluiu a configuração do aplicativo de RP para seu projeto no AD FS. Em seguida, você vai configurar esse aplicativo para enviar as declarações necessitadas para seu aplicativo. O diálogo **Editar regras de declaração** é aberto por padrão para você no final do assistente para que você possa começar imediatamente. Vamos configurar pelo menos as seguintes declarações (com esquemas entre parênteses):
...
10.	Selecione **Enviar declarações usando uma regra personalizada** e clique em **Próximo**.
...
9.	Clique em **Adicionar regra** novamente.
10.	Selecione **Transformar uma declaração de entrada** e clique em **Próximo**.
...
12.	Clique em **Aplicar** na caixa de diálogo **Editar regras de declaração**. Agora ele deve se parecer com a captura de tela abaixo:
	...
	> [AZURE.NOTE] Novamente, certifique-se de repetir essas etapas para seu ambiente de depuração e o site publicado.
...
Para testar a autenticação no depurador, tudo o que você precisa fazer agora é o tipo `F5`. Se você quiser testar a autenticação no site publicado, navegue até a URL.
...
Depois de fazer logon com um usuário no domínio do AD de implantação do AD FS, você deverá ver a página inicial novamente com **Olá, <nome de usuário>!** no canto. Aqui está o que acontece.
...
Se a declaração de nome estiver ausente, você teria visto **Olá, !**. Se você olhar Views\Shared\_LoginPartial.cshtml, descobrirá que ele usa `User.Identity.Name` para exibir o nome de usuário. Conforme mencionado anteriormente, o ASP.NET alimenta essa propriedade com a declaração de nome de usuário autenticado, se ele estiver disponível no token SAML. Para ver todas as declarações enviadas pelo AD FS, coloque um ponto de interrupção em Controllers\HomeController.cs, no método de ação de Índice. Depois de o usuário ser autenticado, inspecione a coleção `System.Security.Claims.Current.Claims`.
...
Como você incluiu associações de grupo como declarações de função em sua configuração de confiança RP, agora você pode usar eles diretamente na decoração `[Authorize(Roles="...")]` para controladores e ações. Em um aplicativo de linha de negócios com o padrão Criar-Ler-Atualizar-Excluir (CRUD), é possível autorizar funções específicas para acessar cada ação. Por enquanto, você simplesmente testará esse recurso no controlador Início existente.

1. Abra Controllers\HomeController.cs.
2. Decore os métodos de ação `About` e `Contact` similares aos abaixo, usando as associações de grupo de segurança que seu usuário autenticado tem.  
	<pre class="prettyprint">
	...
	Já que adicionei **Testar usuário** ao **Grupo de teste** em meu ambiente de laboratório do AD FS, usarei o Grupo de teste para testar a autorização em `About`. Para `Contact`, irei testar o caso negativo de **Admins. do domínio**, ao qual **Testar usuário** não pertence.

3. Inicie o depurador digitando `F5` e entre, depois clique em **Sobre**. Você deve agora estar visualizando a página `~/About/Index` com êxito, se o usuário autenticado for autorizado para essa ação.
4. Agora clique em **contato**, que, em meu caso, não deve autorizar **Testar usuário** para a ação. No entanto, o navegador é redirecionado para o AD FS, que, por fim, mostra esta mensagem:
	...
	O motivo pelo qual isso acontece é que, por padrão, o MVC retorna um 401 Não autorizado quando uma função do usuário não está autorizada. Isso dispara uma solicitação de nova tentativa de autenticação para seu provedor de identidade (AD FS). Uma vez que o usuário já está autenticado, o AD FS retorna para a mesma página, o que, em seguida, emite outro 401, criando um loop de redirecionamento. Você substituirá o método AuthorizeAttribute `HandleUnauthorizedRequest` com lógica simples para mostrar algo que faça sentido em vez de dar continuidade ao loop de redirecionamento.
...
5. Crie um arquivo no projeto chamado AuthorizeAttribute.cs e cole o código a seguir nele.
	...
	O código de substituição envia um HTTP 403 (proibido) em vez de HTTP 401 (não autorizado) em casos autenticado-mas-não autorizado.
...
7. Publique o aplicativo no seu site do Azure novamente e teste o comportamento do aplicativo ao vivo.

<a name="bkmk_data"></a>
## Conectar-se a dados locais

Um motivo pelo qual você desejaria implementar seu aplicativo de linha de negócios com o AD FS, em vez de Active Directory do Azure, são os problemas de conformidade em manter dados organizacionais fora do local. Isso também pode significar que o seu site do Azure deve acessar bancos de dados locais, pois você não tem permissão para usar o [banco de dados SQL](http://azure.microsoft.com/services/sql-database/) como a camada de dados para seus sites.

Os sites do Azure dão suporte ao acessar bancos de dados locais com duas abordagens: [Conexões híbridas](http://azure.microsoft.com/documentation/articles/integration-hybrid-connection-overview/) e [redes virtuais](http://azure.microsoft.com/documentation/articles/web-sites-integrate-with-vnet/). Para obter mais informações, consulte [Usando integração VNET e conexões híbridas com os sites do Azure](http://azure.microsoft.com/blog/2014/10/30/using-vnet-or-hybrid-conn-with-websites/).
...
- [Criar um aplicativo de linha de negócios do ASP.NET MVC, em sites do Azure, que autentica com o Active Directory do Azure](web-sites-dotnet-lob-application-azure-ad.md)
