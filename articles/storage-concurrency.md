<properties title="Gerenciando a simultaneidade no Armazenamento do Microsoft Azure" pageTitle="obrigat&oacute;rio" description="Como gerenciar a simultaneidade para os servi&ccedil;os Blob, Fila, Tabela e Arquivo" metaKeywords="Optional" services="Optional" solutions="Optional" documentationCenter="Optional" authors="Tamra Myers" videoId="Optional" scriptId="Optional" />
...
2.  Simultaneidade pessimista: um aplicativo procurando realizar uma atualização bloqueará um objeto, impedindo que outros usuários atualizem os dados até que o bloqueio seja liberado. Por exemplo, em um cenário de replicação de dados mestre/subordinado em que apenas o mestre realizará atualizações, normalmente o mestre manterá um bloqueio exclusivo por um período de tempo prolongado nos dados para garantir que nenhuma outra pessoa possa atualizá-los.
...
# Armazenamento do Azure: simplifica o desenvolvimento na nuvem
...
O trecho de C# a seguir (usando a Client Storage Library 4.2.0) mostra um exemplo simples de como criar um **If-Match AccessCondition** com base no valor da ETag acessado nas propriedades de um blob que foi recuperado ou inserido anteriormente. Em seguida, ele usa o objeto **AccessCondition** ao atualizar o blob: o objeto **AccessCondition** adiciona o cabeçalho **If-Match** à solicitação. Se outro processo atualizou o blob, o serviço Blob retorna uma mensagem de status HTTP 412 (Falha de precondição). A amostra completa pode ser baixada [aqui][aqui].
...
Operação |Retorna valor de ETag |Aceita cabeçalhos condicionais|
-----------|-------------------|----------------------------|
Put Blob| Sim| Sim|
Get Blob| Sim| Sim|
...
Copy Blob| Sim| Sim (para o blob de origem e de destino)|
...
Para bloquear um blob para uso exclusivo, é possível obter uma [concessão][concessão] sobre ele. Ao obter uma concessão, você especifica por quanto tempo precisa dela: isso pode ser entre 15 e 60 segundos ou por tempo indeterminado, o que corresponde a um bloqueio exclusivo. Você pode renovar uma concessão finita para estendê-la e pode liberar qualquer concessão quando terminar de trabalhar com ela. O serviço Blob libera as concessões finitas automaticamente quando elas expiram.
...
O trecho de C# a seguir mostra um exemplo de aquisição de uma concessão exclusiva por 30 segundos em um blob, atualizando o conteúdo do blob e liberando a concessão. Se já houver uma concessão válida no blob quando você tentar obter uma nova concessão, o serviço Blob retornará um resultado de status “HTTP (409) Conflito”. O trecho abaixo usa um objeto **AccessCondition** para encapsular as informações da concessão quando ela faz uma solicitação para atualizar o blob no serviço de Armazenamento. A amostra completa pode ser baixada [aqui][aqui].
...
Se você tentar realizar uma operação de gravação em um blob sob concessão sem enviar a ID de concessão, a solicitação falhará com um erro 412. Observe que se a concessão expirar antes de chamar o método **UploadText**, mas você ainda utilizar a ID de concessão, a solicitação também falhará com um erro **412**. Para obter mais informações sobre o gerenciamento de tempos de expiração de concessão e IDs de concessão, consulte a documentação de REST de [Lease Blob][concessão].
...
-   [Especificando cabeçalhos condicionais para operações do serviço Blob][1]
-   [Lease Container][Lease Container]
-   [Lease Blob][concessão]
...
-   [Operações em entidades][Operações em entidades]
...
-   [API REST do serviço Fila][API REST do serviço Fila]
...
-   [Gerenciando a simultaneidade usando o Armazenamento do Azure - Aplicativo de amostra][aqui]

Para obter mais informações sobre Armazenamento do Azure, consulte:

-   [Página inicial do Armazenamento do Microsoft Azure][Página inicial do Armazenamento do Microsoft Azure]
-   [Introdução ao Armazenamento do Azure][Introdução ao Armazenamento do Azure]
-   Introdução ao Armazenamento para [Blob][Blob], [Tabela][Tabela] e [Filas][Filas]
-   Arquitetura de armazenamento – [Armazenamento do Microsoft Azure: Um serviço de armazenamento em nuvem altamente disponível com consistência sólida][Armazenamento do Microsoft Azure: Um serviço de armazenamento em nuvem altamente disponível com consistência sólida]

  [aqui]: http://code.msdn.microsoft.com/windowsazure/Managing-Concurrency-using-56018114
  [Especificando cabeçalhos condicionais para operações do serviço Blob]: http://msdn.microsoft.com/pt-br/library/dd179371.aspx
  [concessão]: http://msdn.microsoft.com/pt-br/library/azure/ee691972.aspx
  [1]: http://msdn.microsoft.com/pt-br/library/azure/dd179371.aspx
  ...
  [Blob]: http://azure.microsoft.com/pt-br/documentation/articles/storage-dotnet-how-to-use-blobs/
  [Tabela]: http://azure.microsoft.com/pt-br/documentation/articles/storage-dotnet-how-to-use-tables/
  [Filas]: http://azure.microsoft.com/pt-br/documentation/articles/storage-dotnet-how-to-use-queues/
  [Armazenamento do Microsoft Azure: Um serviço de armazenamento em nuvem altamente disponível com consistência sólida]: http://blogs.msdn.com/b/windowsazurestorage/archive/2011/11/20/windows-azure-storage-a-highly-available-cloud-storage-service-with-strong-consistency.aspx
