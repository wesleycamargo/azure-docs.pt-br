...
<div class="dev-center-tutorial-selector sublanding"> 
  <a href="/pt-br/develop/mobile/how-to-guides/work-with-net-client-library/" title=".NET Framework">.NET Framework</a><a href="/pt-br/develop/mobile/how-to-guides/work-with-html-js-client/" title="HTML/JavaScript" class="current">HTML/JavaScript</a><a href="/pt-br/develop/mobile/how-to-guides/work-with-ios-client-library/" title="iOS">iOS</a><a href="/pt-br/develop/mobile/how-to-guides/work-with-android-client-library/" title="Android" class="current">Android</a><a href="/pt-br/develop/mobile/how-to-guides/work-with-xamarin-client-library/" title="Xamarin" class="current">Xamarin</a>
</div>

Este guia mostra como executar cenários comuns usando um cliente HTML/JavaScript para os Serviços Móveis do Azure. Os cenários abrangidos incluem consultas de dados, inserção, atualização e exclusão de dados, autenticação de usuários e tratamento de erros. Se você for novo nos Serviços Móveis, deverá considerar primeiro concluir os Serviços Móveis [Início rápido do JavaScript da Windows Store][Início rápido do JavaScript da Windows Store] ou [Início rápido do HTML][Início rápido do HTML]. O tutorial de início rápido ajuda a configurar sua conta e criar seu primeiro serviço móvel.
...
-   [Como: Consultar dados a partir de um serviço móvel][Como: Consultar dados a partir de um serviço móvel]
    ...
    -   [Executar uma operação de consulta OData][Executar uma operação de consulta OData]
...
-   [Como: Exibir dados na interface do usuário][Como: Exibir dados na interface do usuário]
-   [Como: Autenticar usuários][Como: Autenticar usuários]
-   [Como: Tratar erros][Como: Tratar erros]
-   [Como: Usar promessas][Como: Usar promessas]
-   [Como: Personalizar cabeçalhos de solicitações][Como: Personalizar cabeçalhos de solicitações]
...
> [WACOM.NOTE]Para um aplicativo da Windows Store escrito no JavaScript/HTML, é necessário apenas adicionar o pacote NuGet **WindowsAzure.MobileServices.WinJS** para seu projeto.
...
Você deve substituir o espaço reservado `AppUrl` pela URL do aplicativo do seu serviço móvel e pela chave do aplicativo `AppKey`. Para saber como obter a URL e a chave do aplicativo para o serviço móvel, consulte o tutorial [Introdução a dados em JavaScript na Windows Store][Introdução a dados em JavaScript na Windows Store] ou [Introdução a dados em HTML/JavaScript][Introdução a dados em HTML/JavaScript].

## <a name="querying"></a>Como: Consultar dados a partir de um serviço móvel
...
A função `lookup` usa apenas o valor `id` e retorna o objeto do bancos de dados com essa ID. As tabelas de banco de dados são criadas com uma coluna de números inteiros ou `id` de cadeia de caracteres. Uma coluna `id` de cadeia de caracteres é o padrão.
...
> [WACOM.NOTE]Quando você fornece uma cadeia de caracteres de opção de consulta do OData bruta para a função `read`, você também não pode usar os métodos de construção de consultas na mesma consulta. Neste caso, você deve compor toda sua consulta como uma cadeia de caracteres de consulta do OData. Para obter mais informações sobre as opções de consulta do sistema OData, consulte a [referência de opções de consulta do sistema OData][referência de opções de consulta do sistema OData].
...
## <a name="binding"></a><span class="short-header">Exibindo dados</span>Como: Exibir dados na interface do usuário

Esta seção mostra como exibir os objetos de dados retornados usando elementos da interface do usuário. Para consultar itens na `todoItemTable` e exibi-los em uma lista muito simples, você pode executar o seguinte código de exemplo. Nenhuma seleção, filtragem ou classificação de qualquer tipo é feita.
...
Em um aplicativo da Windows Store, os resultados de uma consulta podem ser usados para criar um objeto [WinJS.Binding.List], que pode ser vinculado como a fonte de dados para um objeto [ListView][ListView]. Para obter mais informações, consulte [Associação de dados (aplicativos da Windows Store usando JavaScript e HTML)][Associação de dados (aplicativos da Windows Store usando JavaScript e HTML)].

## <a name="caching"></a><span class="short-header">Autenticação</span>Como: Autenticar usuários
...
Nesse caso, os Serviços Móveis gerenciam o fluxo de autenticação OAuth 2.0 exibindo a página de logon do provedor selecionado e gerando um token de autenticação dos Serviços Móveis depois de um logon bem-sucedido com o provedor de identidade. A função [login][login], quando concluída, retorna um objeto JSON (**user**) que expõe o ID do usuário e o token de autenticação dos Serviços Móveis nos campos **userId** e **authenticationToken**, respectivamente. Esse token pode ser armazenado em cache e reutilizado até que expire. Para obter mais informações, consulte Armazenando o [token de autenticação em cache].

<div class="dev-callout"><b>Aplicativo da Windows Store</b>
<p>Ao usar o provedor de logon da Conta da Microsoft para autenticar usu&aacute;rios de seu aplicativo da Windows Store, voc&ecirc; tamb&eacute;m deve registrar o pacote de aplicativos nos Servi&ccedil;os M&oacute;veis. Ao registrar as informa&ccedil;&otilde;es do pacote de aplicativos da Windows Store com Servi&ccedil;os M&oacute;veis, o cliente &eacute; capaz de reutilizar as credenciais de login da conta da Microsoft para obter uma experi&ecirc;ncia de logon &uacute;nica. Se voc&ecirc; n&atilde;o fizer isso, os usu&aacute;rios de login da conta da Microsoft ser&atilde;o apresentados com uma solicita&ccedil;&atilde;o de logon toda vez que o m&eacute;todo de logon for chamado. Para saber como registrar seu pacote de aplicativos da Windows Store, consulte <a href="/pt-br/develop/mobile/how-to-guides/register-windows-store-app-package/" target="_blank">Registrar seu pacote de aplicativos da Windows Store para a autentica&ccedil;&atilde;o da Microsoft</a>. Ap&oacute;s as informa&ccedil;&otilde;es do pacote serem registradas nos Servi&ccedil;os M&oacute;veis, chame o m&eacute;todo <a href="http://go.microsoft.com/fwlink/p/?LinkId=322050" target="_blank">login</a> fornecendo um valor de <strong>true</strong> para o par&acirc;metro <em>useSingleSignOn</em> para reutilizar as credenciais.</p>
</div>
...
Esse exemplo simplificado obtém um token do Live Connect, que é fornecido aos Serviços Móveis chamando a função [login][login]. Para obter um exemplo mais completo de como usar a Conta da Microsoft para fornecer uma experiência de logon único, consulte [Autenticar seu aplicativo com logon único][Autenticar seu aplicativo com logon único].
...
Em alguns casos, a chamada para o método login pode ser evitada após a primeira vez que o usuário é autenticado. Podemos usar [sessionStorage][sessionStorage] ou [localStorage][sessionStorage] para armazenar em cache a identidade do usuário quando fizerem o login pela primeira vez e todas as vezes subsequentes que verificamos se já temos a identidade do usuário armazenada em cache. Se o cache estiver vazio ou a chamada falhar (ou seja, a sessão de logon atual tiver expirado), ainda precisaremos passar pelo processo de logon.
...
## <a name="promises"></a><span class="short-header">Promessas</span>Como: Usar promessas
...
## <a name="customizing"></a><span class="short-header">Personalizar cabeçalhos de solicitações</span>Como: Personalizar cabeçalhos de solicitações de clientes
...
## <a name="hostnames"></a><span class="short-header">Usar o CORS</span>Como: Usar compartilhamento de recursos entre origens
