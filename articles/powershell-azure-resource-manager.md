<properties pageTitle="Usando o Windows PowerShell com o Gerenciador de Recursos" metaKeywords="ResourceManager, PowerShell, PowerShell do Azure" description="Usar o Windows PowerShell para criar um grupo de recursos" metaCanonical="" services="" documentationCenter="" title="Usando o Windows PowerShell com o Gerenciador de Recursos" authors="juneb" solutions="" manager="mbaldwin" editor="mollybos" />
...
- Windows PowerShell, versão 3.0 ou posterior. Para localizar a versão do Windows PowerShell, digite:`$PSVersionTable` e verifique se o valor de `PSVersion` é 3.0 ou superior. Para instalar uma versão mais recente, consulte [Windows Management Framework 3.0 ](http://www.microsoft.com/pt-br/download/details.aspx?id=34595) ou [Windows Management Framework 4.0](http://www.microsoft.com/pt-br/download/details.aspx?id=40855)
	
- PowerShell do Azure, versão 0.8.0 ou posterior. Para instalar a versão mais recente e associá-la à sua assinatura do Azure, consulte [Como instalar e configurar o PowerShell do Windows Azure (a página pode estar em inglês)](http://www.windowsazure.com/pt-br/documentation/articles/install-configure-powershell/).
...
##<a id="about"></a>Sobre os módulos do PowerShell do Azure ##
A partir da versão 0.8.0, a instalação do PowerShell do Azure inclui três módulos do Windows PowerShell:

- **Azure**: inclui os cmdlets tradicionais para gerenciar recursos individuais, como contas de armazenamento, sites, bancos de dados, máquinas virtuais e serviços de mídia. Para obter mais informações, consulte [Cmdlets do Gerenciamento de Serviços do Azure (a página pode estar em inglês)](http://msdn.microsoft.com/pt-br/library/jj152841.aspx).

- **AzureResourceManager**: inclui cmdlets para criar, gerenciar e implantar os recursos do Azure em um grupo de recursos. Para obter mais informações, consulte [Cmdlets do Gerenciador de Recursos do Azure (a página pode estar em inglês)](http://go.microsoft.com/fwlink/?LinkID=394765).

- **AzureProfile**: inclui cmdlets comuns aos dois módulos, como Add-AzureAccount, Get-AzureSubscription e Switch-AzureMode. Para obter mais informações, consulte [Cmdlets de perfil do Azure (a página pode estar em inglês)](http://go.microsoft.com/fwlink/?LinkID=394766).

> [ WACOM.NOTE] O módulo do Gerenciador de recursos do Azure está no modo de visualização. Ele pode não fornecer os mesmos recursos de gerenciamento que o módulo Gerenciamento de Serviços do Azure. 
...
Para obter uma lista dos cmdlets no módulo AzureResourceManager com uma sinopse da Ajuda, digite: 
	...
	Name                                   Synopsis
	----                                   --------
	Get-AzureLocation                      Obtém os locais dos data centers do Azure e os tipos de recurso com suporte
	Get-AzureResource                      Obtém os recursos do Azure
	Get-AzureResourceGroup                 Obtém os grupos de recursos do Azure
	Get-AzureResourceGroupDeployment       Obtém as implantações em um grupo de recursos.
	Get-AzureResourceGroupGalleryTemplate  Obtém as implantações de grupos de recursos na galeria
	Get-AzureResourceGroupLog              Obtém o log de implantações de um grupo de recursos.
	New-AzureResource                      Cria um novo recurso em um grupo de recursos
	New-AzureResourceGroup                 Cria um grupo de recursos Azure e seus recursos
	New-AzureResourceGroupDeployment       Adicione uma implantação do Azure para um grupo de recursos.
	Remove-AzureResource                   Exclui um recurso
	Remove-AzureResourceGroup              Exclui um grupo de recursos.
	Save-AzureResourceGroupGalleryTemplate Salva um modelo da galeria em um arquivo JSON
	Set-AzureResource                      Altera as propriedades de um recurso do Azure.
	Stop-AzureResourceGroupDeployment      Cancela a implantação de um grupo de recursos
	Test-AzureResourceGroupTemplate        Detecta erros em modelo de grupo de recursos ou nos parâmetros do modelo
...
#<a id="create"></a> Criar um grupo de recursos para um site e um banco de dados#
...
O modelo Microsoft.WebSiteSQLDatabase.0.1.0-preview1.json parece ser interessante. Para obter mais informações sobre um modelo da galeria, use o parâmetro **Identity**. O valor do parâmetro Identity é a identidade do modelo.
...
O cmdlet retorna um objeto com muito mais informações sobre o modelo, incluindo uma descrição.
	...
	<p>Os Sites do Windows Azure oferecem opções seguras e flexíveis de 
	desenvolvimento, implantação e dimensionamento para qualquer tamanho de aplicativo web. Utilize 
	as ferramentas existentes para criar e implantar aplicativos sem a dificuldade 
	de gerenciar a infraestrutura.</p>
...
#<a id="manage"></a> Gerenciar um Grupo de Recursos
...
#<a id="troubleshoot"></a> Solucionar problemas de um grupo de recursos
...
- [Cmdlets do Gerenciador de Recursos do Azure (a página pode estar em inglês)](http://go.microsoft.com/fwlink/?LinkID=394765&clcid=0x409): saiba usar os cmdlets nos módulos AzureResourceManager.
...
- [Blog "Hey, Scripting Guy!" (a página pode estar em inglês)](http://blogs.technet.com/b/heyscriptingguy/): obtenha dicas reais e truques da comunidade.
- [Usando a interface de linha de comando entre plataformas do Azure com o Gerenciador de Recursos (a página pode estar em inglês)](http://www.windowsazure.com/pt-br/documentation/articles/xplat-cli-azure-resource-manager/): conheça maneiras alternativas para automatizar as operações do Gerenciador de Recursos.
