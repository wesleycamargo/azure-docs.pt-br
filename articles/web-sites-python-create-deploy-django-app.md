...
Agora que seu repositório Git no Azure está criado, você começará a preenchê-lo com o site em seu computador local.  A primeira etapa é clonar o site vazio existente utilizando a url fornecida:

![](./media/web-sites-python-create-deploy-django-app/django-ws-007.png)

Daqui, você estará pronto para configurar a inscrição com o site.  Você precisará fazer algumas coisas:
...
Esse comando copia todas as bibliotecas localizadas nos site-packages, incluindo Django.  Se há bibliotecas não usadas pelo site, fique à vontade para removê-las.

![](./media/web-sites-python-create-deploy-django-app/django-ws-008.png)
 
 Em seguida, crie seu aplicativo Django inicial.  Você pode fazer isso exatamente como você criaria qualquer outro aplicativo Django a partir da linha de comando, ou você pode usar [Ferramentas Python para Visual Studio](http://pytools.codeplex.com/) para criar o projeto.  As duas opções são mostradas aqui.

**Opção 1:** 
Para criar o novo projeto na linha de comando, execute o seguinte comando. O comando cria o aplicativo Django na pasta DjangoApplication:
...
Daqui, você pode continuar o desenvolvimento do seu aplicativo Django com as ferramentas que você já está usando. Se você estiver usando [Ferramentas Python para Visual Studio](http://pytools.codeplex.com/) para o desenvolvimento, você provavelmente desejará instalar [VisualGit](http://code.google.com/p/visualgit/) para obter a integração do controle do código-fonte de dentro do Visual Studio.  

Seu aplicativo pode ter dependências além de Python e Django.  Se você instalou o Python usando o instalador em [http://www.windowsazure.com/pt-br/develop/python/](http://www.windowsazure.com/pt-br/develop/python/), o PIP já estará instalado e poderá ser usado para adicionar novas dependências rapidamente.  Por exemplo, para instalar o kit de ferramentas de linguagem natural e todas as suas dependências, digite:
...
Depois de copiar os arquivos, execute o comando **git status** para ver os arquivos recentemente adicionados, e **git add** seguido de **git commit** para confirmar as alterações no repo.  Por fim, você pode fazer um **git push**, que implementa o site atualizado no Azure.
