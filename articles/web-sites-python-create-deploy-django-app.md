<properties urlDisplayName="Websites with Django" pageTitle="Websites Python com Django - Tutorial do Azure" metaKeywords="Azure django, django website" description="Um tutorial que apresenta a execução de um site Python no Azure." metaCanonical="" services="web-sites" documentationCenter="Python" title="Creating Websites with Django" authors="huvalo" solutions="" manager="wpickett" editor="" />
...
Este tutorial usa Python 2.7 e Django 1.4.  Você pode obtê-los usando uma maneira própria ou fazendo a instalação de maneira rápida e fácil usando o link do Windows Installer no [http://www.windowsazure.com/pt-br/develop/python/](http://www.windowsazure.com/pt-br/develop/python/).  
...
> [WACOM.NOTE] Agora você pode selecionar a versão do Python que deseja usar no portal dos Sites do Azure, abrindo a guia Configurar de seu site e alterando a configuração **Versão do Python**.

Você também precisará instalar uma opção de implantação para enviar o site para o Azure. Há várias ferramentas de implantação disponíveis, mas este tutorial usa Git. É recomendável [msysgit](http://code.google.com/p/msysgit/). 
...
Agora que seu repositório Git no Azure está criado, você começará a preenchê-lo com o site em seu computador local.  A primeira etapa é clonar o site vazio existente utilizando o url fornecido:
...
Em seguida, crie seu aplicativo Django inicial.  Você pode fazer isso exatamente como você criaria qualquer outro aplicativo Django por meio da linha de comando, ou você pode usar [Ferramentas Python para Visual Studio](http://pytools.codeplex.com/) para criar o projeto.  As duas opções são mostradas aqui.

**Opção 1:** 
Para criar o novo projeto em uma linha de comando, execute o seguinte comando. O comando cria o aplicativo Django na pasta DjangoApplication:
...
O primeiro comando adicionará seus arquivos não rastreados para que sejam rastreados.  O segundo comando confirmará os arquivos que você acabou de adicionar ao repositório.  O terceiro comando adiciona um remoto com o nome *azure* em seu repositório.  O comando final leva as alterações e as envia para o repositório remoto. Este último comando também iniciará a implantação.  Depois de fazer isso, você verá um resultado semelhante a este:
...
Por fim, volte para o Painel. Vá até a **URL do SITE** no lado esquerdo.Clique no link e abra o novo site Django, que terá a seguinte aparência:

![](./media/web-sites-python-create-deploy-django-app/django-ws-017.png)

## Próximas etapas

A partir daqui, você pode continuar o desenvolvimento do seu aplicativo Django com as ferramentas que você já está usando. Se você estiver usando [Python Tools for Visual Studio](http://pytools.codeplex.com/) para desenvolvimento, provavelmente você desejará instalar [VisualGit](http://code.google.com/p/visualgit/) para obter a integração do controle do código-fonte por meio do Visual Studio.  

Seu aplicativo pode ter dependências além de Python e Django.  Se você instalou o Python usando o instalador em [http://www.windowsazure.com/pt-br/develop/python/](http://www.windowsazure.com/pt-br/develop/python/), você já terá o PIP instalado e poderá usar isso para adicionar rapidamente novas dependências.  Por exemplo, para instalar o Kit de ferramentas de linguagem Natural e todas as suas dependências, digite:
...
Depois de copiar os arquivos, execute o comando **status git** para ver os arquivos recentemente adicionados, e **git add** seguido de **git commit** para confirmar as alterações no repo.  Por fim, você pode fazer um **git push**, que implementa o site atualizado no Azure.
