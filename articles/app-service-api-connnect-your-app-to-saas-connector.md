...
1. Vá para a home page do [Portal de visualização do Azure] e clique em **Marketplace**.
	...
	![Marketplace no portal de visualização do Azure](./media/app-service-api-connect-your-app-to-saas-connector/marketplace.png)

2. Procure pelo Dropbox e clique no ícone **Conector do Dropbox**.

	![Clique em Conector do Dropbox](./media/app-service-api-connect-your-app-to-saas-connector/searchdb.png)
 
3. Clique em **Criar**.

	![Clicar em Criar](./media/app-service-api-connect-your-app-to-saas-connector/clickcreate.png)
...
9. <a id="gateway"></a>Mantenha o padrão "DropboxConnector" como o **Nome** do conector e clique **Criar**.
	...
	![Clique em criar](./media/app-service-api-connect-your-app-to-saas-connector/createdropbox.png)
...
3. Quando o Azure terminar de criar o conector, clique em **Procurar > Grupos de recursos > DropboxRG**.
	...
	![Diagrama do grupo de recursos](./media/app-service-api-connect-your-app-to-saas-connector/rgdiagram.png)
...
1. Vá para o [Portal do desenvolvedor do Dropbox](https://www.dropbox.com/developers/apps), clique em **Console do Aplicativo**, e, em seguida, clique em **Criar Aplicativo**.
	...
	![Criar aplicativo Dropbox](./media/app-service-api-connect-your-app-to-saas-connector/dbappcreate.png)
...
3. Clique em **Criar aplicativo**.
	...
	![Criar aplicativo Dropbox](./media/app-service-api-connect-your-app-to-saas-connector/dbapiapp.png)
	...
	![Criar aplicativo Dropbox](./media/app-service-api-connect-your-app-to-saas-connector/dbappsettings.png)

### Copiar as configurações de aplicativo do Dropbox no conector de Dropbox do Azure e vice-versa 

4. Em outra janela ou guia do navegador, vá para o [Portal de visualização do Azure].
...
4. Clique em **Configurações**, e na folha **Configurações** clique em **Autenticação**.
	...
	![Clique em Configurações](./media/app-service-api-connect-your-app-to-saas-connector/clicksettings.png)

	![Clique em Autenticação](./media/app-service-api-connect-your-app-to-saas-connector/clickauth.png)

5. Na folha Autenticação, insira a ID do cliente e o segredo do cliente do site do Dropbox e, em seguida, clique em **Salvar**.

	![Insira as configurações e clique em Salvar](./media/app-service-api-connect-your-app-to-saas-connector/authblade.png)
...
3. Copie o **URI de Redirecionamento** (a caixa cinza acima da ID do cliente e do segredo do cliente) e adicione o valor à página deixada aberta na etapa anterior.
	...
	![Obter URI de redirecionamento](./media/app-service-api-connect-your-app-to-saas-connector/redirecturi.png)

	![Criar aplicativo Dropbox](./media/app-service-api-connect-your-app-to-saas-connector/dbappsettings2.png)
...
2. Na folha **Configurações do aplicativo**, defina o **Nível de acesso** para **público (autenticado)**e, em seguida, clique em **Salvar**.
	...
	![Definir como Público (autenticado)](./media/app-service-api-connect-your-app-to-saas-connector/pubauth.png)
...
As instruções a seguir mostram como executar essas etapas usando as ferramentas de desenvolvimento do navegador Chrome e a ferramenta de cliente Postman REST. Isso é apenas um exemplo, e você pode fazer os mesmos procedimentos com outros navegadores e ferramentas.
...
2. Vá para a URL de logon para o gateway e o provedor de autenticação que você configurou. A URL segue este padrão: 
	...
	![URL de gateway](./media/app-service-api-connect-your-app-to-saas-connector/gatewayurl.png)
...
Para obter autorização do Dropbox para usar a API do Dropbox, você precisa fornecer a ele as credenciais do usuário. O Azure fará isso por você, mas para disparar o processo que você precisa ir para uma URL especial no navegador. Para obter essa URL você faz uma solicitação HTTP Post para o gateway.
...
4. Em uma nova janela ou guia do navegador, crie e envie uma solicitação HTTP Post para o gateway, para solicitar uma URL de autorização. Inclua o `x-zumo-auth` token como um cabeçalho HTTP.
	...
	![Enviar para a URL de autorização](./media/app-service-api-connect-your-app-to-saas-connector/sendforconsent.png)

	A resposta inclui uma URL a ser usada para autenticar o usuário com Dropbox. (Se você receber uma resposta de erro indicando que o método Get não tem suporte, embora a lista suspensa de métodos esteja definida como **Post**, talvez seja necessário usar outra ferramenta cliente REST. "Cliente REST Avançado" é outro complemento do Chrome que você pode usar.)

	![URL de autorização](./media/app-service-api-connect-your-app-to-saas-connector/getconsenturl.png)

2. Vá para a URL que você recebeu em resposta à solicitação HTTP Post.

	O Dropbox associa sua identidade de usuário com seu aplicativo de API do Dropbox e, em seguida, redireciona o navegador para a URL de redirecionamento especificada (por exemplo, o portal de visualização do Azure se você seguiu o exemplo e usou https://portal.azure.com).

	Como seu aplicativo Dropbox está no modo de desenvolvedor, você pode receber uma página de logon do Dropbox antes que o navegador vá para a URL de redirecionamento. Depois de fazer logon usando suas credenciais do Dropbox, a identidade do usuário usada para efetuar logon no gateway é associada ao aplicativo Dropbox e futuramente essa etapa de logon do Dropbox não será mais necessária para a identidade do usuário.
...
4. Clique no ícone **Definição da API** para ver os métodos de API disponíveis no conector.
	...
	![Folha Aplicativo de API](./media/app-service-api-connect-your-app-to-saas-connector/apiappblade.png)

	![Definição de API](./media/app-service-api-connect-your-app-to-saas-connector/apidef.png)
...
3. Por exemplo, se a URL do conector for https://dropboxrg784237ad3e7.azurewebsites.net e você quiser ver os arquivos na pasta de seu blog, a URL seria:
	...
	![Obter a resposta à solicitação de pasta](./media/app-service-api-connect-your-app-to-saas-connector/dbresponse.png)

## Próximas etapas

Você viu como instalar, configurar e testar um conector de SaaS. Para obter mais informações, consulte [Usando conectores](app-service-logic-use-biztalk-connectors.md).
