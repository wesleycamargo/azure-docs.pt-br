<properties linkid="Install-Config-Windows-Azure-PowerShell" urlDisplayName="PowerShell no Azure" pageTitle="Como instalar e configurar o PowerShell no Azure" description="Saiba como instalar e configurar o PowerShell no Azure." umbracoNaviHide="0" disqusComments="1" writer="kathydav" editor="tysonn" manager="jeffreyg" documentationCenter="" services="" solutions="" authors="" title="Como instalar e configurar o PowerShell no Azure" />

# Como instalar e configurar o PowerShell no Azure#
 ...
 * [Como instalar o PowerShell no Azure](#Install)  
...
### <a id="Prereq"></a>Pré-requisitos para usar o PowerShell no Azure
...
> [WACOM.NOTE] Várias opções de assinatura estão disponíveis. Para obter informações, consulte [Comece a usar o Azure](http://go.microsoft.com/fwlink/p/?LinkId=320795).
...
<h2> <a id="Install"></a>Como instalar o PowerShell no Azure</h2>

Você pode baixar e instalar o módulo PowerShell no Azure executando o [Microsoft Web Platform Installer](http://go.microsoft.com/fwlink/p/?LinkId=320376). Quando solicitado, clique em **Executar**. O Microsoft Web Platform Installer é carregado com o módulo **PowerShell no Azure** disponível para instalação. O Microsoft Web Platform Installer instala todas as dependências dos cmdlets do PowerShell no Azure. Siga os prompts para concluir a instalação.
...
- Em um computador com uma versão anterior ao Windows 8 ou ao Windows Server 2012, você pode usar a menu Iniciar. No menu Iniciar, clique em **Todos os Programas**, clique em **Azure** e, em seguida, em **PowerShell no Azure**.
...
Os cmdlets requerem informações da sua assinatura para que possam ser usados para gerenciar seus serviços. A partir da versão .0.7 do módulo, há duas maneiras de fornecer essas informações. Você pode baixar e usar um certificado de gerenciamento que contenha as informações ou fazer logon no Azure usando sua conta da Microsoft ou uma ID organizacional. Quando você faz logon, o Active Directory do Azure (AD do Azure) autentica as credenciais. 

Para ajudar a escolher o método de autenticação apropriado para suas necessidades, considere o seguinte:

- O método do AD do Azure pode facilitar o gerenciamento do acesso a uma assinatura, mas pode prejudicar a automação. As credenciais estão disponíveis para o PowerShell no Azure por 12 horas. Depois que elas expiram, você precisa fazer logon novamente.  
...
<h3>Usar o método do AD do Azure</h3>

1. Abra o console do PowerShell no Azure, conforme instruído em [como: instalar o PowerShell no Azure](#Install)
...
O módulo do PowerShell no Azure inclui cmdlets que ajudam a baixar e importar o certificado.

- O cmdlet **Get-AzurePublishSettingsFile** abre uma no Portal de Gerenciamento do Azure, do qual você pode baixar as informações da assinatura. As informações estão contidas em um arquivo .publishsettings.
...
sobre suas assinaturas, você poderá obtê-las no <a href="http://manage.windowsazure.com/">Portal de Gerenciamento do Azure</a> ou no <a href="http://go.microsoft.com/fwlink/p/?LinkId=324875">Portal do Cliente do Microsoft Online Services</a>.</p> 
</div>

1. Entre no [Portal de Gerenciamento do Azure](http://manage.windowsazure.com) usando as credenciais da sua conta do Azure.

2. Abra o console do PowerShell no Azure, conforme instruído em [Como instalar o PowerShell no Azure](#Install)
...
os serviços de uma assinatura, consulte [Adicionar e remover coadministradores de suas assinaturas do Azure](http://msdn.microsoft.com/pt-br/library/windowsazure/gg456328.aspx)
...
1. Se ainda não estiver aberto, abra o shell do PowerShell no Azure.

2. Determine um nome para o site. Você precisará usar um nome que esteja de acordo com as convenções de nomenclatura DNS. Os nomes válidos podem conter somente letras 'a' a 'z', os números '0' a '9' e também o hífen ('-'). O nome também deve ser exclusivo dentro do Azure. 
...
-   No console, você pode usar o sistema de Ajuda interno. O cmdlet **Get-Help** fornece acesso a esse sistema. A tabela a seguir fornece alguns exemplos de comandos que você pode usar para obter ajuda. Você pode obter mais informações no console digitando **help**.
    ...
    <tr align="left" valign="top">
		<td>help azure</td>
		<td>Lista todos os cmdlets do módulo do PowerShell no Azure.</td>
    </tr>
	<tr align="left" valign="top">
...
- Informações de referência sobre os cmdlets no módulo PowerShell no Azure também estão disponíveis na biblioteca do Azure. Para obter informações, consulte [Referência de cmdlets do Azure](http://msdn.microsoft.com/pt-br/library/windowsazure/jj554330.aspx).

Para obter ajuda da comunidade, tente estes fóruns populares:

- [Fórum do Azure no MSDN (a página pode estar em inglês)]( http://go.microsoft.com/fwlink/p/?LinkId=320212) 
...
- Para fornecer comentários sobre os cmdlets, relatar problemas ou acessar o código-fonte, consulte [Repositório de código do PowerShell no Azure (a página pode estar em inglês)](https://github.com/WindowsAzure/azure-sdk-tools).
...
- Para obter informações sobre cmdlets do AD do Azure, consulte [Gerenciar o AD do Azure usando o Windows PowerShell (a página pode estar em inglês)](http://go.microsoft.com/fwlink/p/?LinkId=320628).
