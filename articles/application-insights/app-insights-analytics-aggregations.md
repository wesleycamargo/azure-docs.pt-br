...
Localize o número de solicitações de cada nome durante a última hora:

    requests | where timestamp > ago(1h)
    | summarize req_count = sum(itemCount) by name

> [AZURE.NOTE] Em vez de simplesmente contar os pontos de dados de solicitação, use a propriedade itemCount. Isso compensa qualquer [amostragem](app-insights-sampling.md) que possa estar em operação. Por exemplo, se a amostragem estiver definida para 33%, itemCount será 3 em cada ponto de dados.

#### Mín. e Máx.

Localize o carimbo de data/hora mínimo e máximo de todos os registros no fluxo de solicitações. Não há qualquer cláusula group-by e, portanto, há apenas uma linha na saída:
...
Retorna uma contagem de linhas para a qual *Predicado* é avaliado como `true`. Se nenhum *Predicado* for especificado, retornará o número total de registros no grupo.
...
*Precisão*, se for especificada, controlará o equilíbrio entre velocidade e precisão.
...
Consulte também o operador [`mvexpand`](app-insights-analytics-queries.md#mvexpand-operator) para a função oposta.
