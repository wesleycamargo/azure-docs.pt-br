...
Cada filtro prefixado pelo caractere de barra vertical `|` é uma instância de um *operador* com alguns parâmetros. A entrada do operador é a tabela que é o resultado do pipeline anterior. Na maioria dos casos, os parâmetros são [expressões escalares](app-analytics-scalars.md) sobre as colunas da entrada. Em alguns casos, os parâmetros são os nomes das colunas da entrada e, em outros casos, o parâmetro é uma segunda tabela. O resultado de uma consulta é sempre uma tabela, mesmo que ela tenha somente uma coluna e uma linha.
...
[Sobre tipos de junção](app-analytics-samples.md#join-flavors).
...
* *tipo:* `bool`, `int`, `long`, `double`, `string`, `timespan`, `datetime`, `guid`, [`dynamic`](app-analytics-scalars.md#dynamic-type)
...
Confira [Registrar a contagem de atividades em tempo real ao longo do tempo](app-analytics-samples.md#concurrent-activities).
...
[Mais exemplos](app-analytics-samples.md#activities).
...
Os argumentos devem ser valores numéricos, de data ou de período de tempo. Eles não podem referenciar as colunas de nenhuma tabela. (Se quiser calcular o intervalo com base em uma tabela de entrada, use a [função *range*](app-analytics-scalars.md#range), talvez com o [operador mvexpand](#mvexpand-operator).)
...
[Mais exemplos](app-analytics-aggregations.md).
...
* *Predicate:* uma [expressão](app-analytics-scalars.md#boolean) `boolean` nas colunas de *T*. É avaliado para cada linha em *T*.
...
* **Use comparações simples** entre os nomes de coluna e as constantes. (“Constante” significa constante ao longo da tabela. Portanto, `now()` e `ago()` estão OK, bem como valores escalares atribuídos usando uma instrução [`let`](app-analytics-syntax.md#let-statements).)
