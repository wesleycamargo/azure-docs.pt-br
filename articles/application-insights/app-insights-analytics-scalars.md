...
[Application Insights](app-insights-overview.md). Essas páginas descrevem a linguagem de consulta da Análise.
...
As expressões escalares são diferentes de [consultas](app-insights-analytics-queries.md), cujos resultados são tabelas.
...
* *ifTrue:* uma expressão que é avaliada e terá seu valor retornado da função se *predicate* for avaliado como `true`.
* *ifFalse:* uma expressão que é avaliada e terá seu valor retornado da função se *predicate* for avaliado como `false`.
...
Arredonda os valores até um número inteiro múltiplo de um determinado tamanho de compartimentalização. Muito usado na consulta [`summarize by`](app-insights-analytics-queries.md#summarize-operator). Se você tiver um conjunto disperso de valores, eles serão agrupados em um conjunto menor de valores específicos.
...
Literais de cadeia de caracteres ofuscados são cadeias de caracteres que serão obscurecidas pela Análise quando ela produzir a saída da cadeia de caracteres (por exemplo, durante o rastreamento). O processo de ofuscação substitui todos os caracteres ofuscados por um caractere inicial (`*`).

Para formar um literal de cadeia de caracteres ofuscado, acrescente `h` ou 'H' no início. Por exemplo:
...
* *search:* a cadeia de caracteres simples ou a [expressão regular](app-insights-analytics-reference.md#regular-expressions) a ser correspondida em *text*.
...
Obtém uma correspondência para um [expressão regular](app-insights-analytics-reference.md#regular-expressions) em uma cadeia de texto. Opcionalmente, converte a subcadeia de caracteres extraída para o tipo indicado.
...
* *regex:* a [expressão regular](https://github.com/google/re2/wiki/Syntax) para pesquisar *text*. Pode conter grupos de captura entre '('parênteses')'. 
* *rewrite:* o regex de substituição para qualquer correspondência feita por *matchingRegex*. Use `\0` para se referir à correspondência inteira, `\1` para o primeiro grupo de captura, `\2` etc., para grupos de captura subsequentes.
* *text:* uma cadeia de caracteres.
...
* *requestedIndex*: um índice `int` opcional baseado em zero. Se for fornecido, a matriz de cadeias de caracteres retornada conterá a subcadeia de caracteres solicitada, se ela existir. 
...
**mvexpand:** para desmembrar as propriedades de um objeto em linhas separadas, use mvexpand:
...
Observe que `indexer` é usado para marcar onde você deve usar um índice numérico. Nesse esquema, alguns caminhos válidos devem ser (supondo que esses índices de exemplo estejam dentro do intervalo):
...
Observe que, ao contrário do JavaScript, JSON exige o uso de aspas duplas (`"`) ao redor das cadeias de caracteres. Portanto, geralmente é mais fácil colocar entre aspas literais de uma cadeia de caracteres codificada em JSON usando aspas simples (`'`).
...
|[`mvexpand` listColumn](app-insights-analytics-queries.md#mvexpand-operator) | Replica uma linha para cada valor em uma lista em uma célula especificada.
|[`summarize buildschema(`column`)`](app-insights-analytics-queries.md#summarize-operator) |Infere o esquema de tipo a partir do conteúdo da coluna
|[`summarize makelist(`column`)` ](app-insights-analytics-queries.md#summarize-operator)| Mescla os grupos de linhas e coloca os valores da coluna em uma matriz.
|[`summarize makeset(`column`)`](app-insights-analytics-queries.md#summarize-operator) | Mescla os grupos de linhas e coloca os valores da coluna em uma matriz, sem duplicação.

### Objetos dinâmicos em cláusulas let


As [cláusulas let](app-insights-analytics-queries.md#let-clause) armazenam valores dinâmicos como cadeias de caracteres, portanto, essas duas cláusulas são equivalentes, e ambas precisam de `parsejson` (ou `todynamic`) antes de serem usadas:
...
desse modo, o fragmento a seguir recupera o valor do slot `duration` no objeto e, desse valor, recupera dois slots, `duration.value` e `duration.min` (`118.0` e `110.0`, respectivamente).
...
A função `range()` (não deve ser confundida com o operador `range`)
...
Quando [amostragem](app-insights-sampling.md) for uma operação, apenas uma fração dos dados gerados pelo SDK é ingerida no portal do Application Insights. A amostragem pode ser aplicada no SDK ou na ingestão para o ponto de extremidade do portal.

`itemCount` indica quantos eventos foram gerados no SDK para cada evento que aparece no fluxo. Por exemplo, se a taxa de amostragem atual for 25%, itemCount == 4.

Quando você quiser contar eventos, use `sum(itemCount)` em vez de `count()` para obter uma estimativa precisa dos números reais. Por exemplo:

* O número de exceções que ocorreram nas últimas duas horas:
