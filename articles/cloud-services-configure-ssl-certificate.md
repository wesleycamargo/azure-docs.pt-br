<properties linkid="dev-net-commons-tasks-enable-ssl" urlDisplayName="Habilitar o SSL" pageTitle="Configurar SSL para um serviço de nuvem - Azure" metaKeywords="SSL do Azure, HTTPS do Azure, SSL do Azure, HTTPSdo Azure, SSL do Azure .NET, .HTTPS do NET Azure, SSL do Azure C#, HTTPS do C# Azure, SSL do VB Azure, HTTPS do VB Azure" description="Saiba como especificar um ponto de extremidade HTTPS para uma função Web e como carregar um certificado SSL para proteger o aplicativo." metaCanonical="" services="cloud-services" documentationCenter=".NET" title="Configurando SSL para um aplicativo no Azure" authors=""  solutions="" writer="" manager="jeffreyg" editor="mollybos"  />
...
<p>Os procedimentos nesta tarefa se aplicam aos Serviços de Nuvem do Azure; para Sites, consulte <a href="../web-sites-configure-ssl-certificate/">Configurando um certificado SSL para um site do Azure</a>.</p>
</div>
...
<h2><a name="step1"> </a><span class="short-header">Obter um certificado SSL</span>Etapa 1: obter um certificado SSL</h2>
...
<h2><a name="step2"> </a><span class="short-header">Modificar arquivos svc/config</span>Etapa 2: Modificar a definição de serviço e arquivos de configuração</h2>
...
1.  No ambiente de desenvolvimento, abra o arquivo de definição do serviço (CSDEF), adicione uma seção **Certificates**dentro da seção **WebRole** e inclua as seguintes informações sobre o certificado:
    ...
    A seção **Certificates** define o nome do nosso certificado, seu local e o nome do repositório no qual está localizado. Optamos por armazenar o certificado no repositório CA, mas é possível escolher outras opções também. Consulte [Como associar um certificado a um serviço][] para obter mais informações.

2.  No arquivo de definição do serviço, adicione um elemento **InputEndpoint**
dentro da seção **Endpoints** para habilitar HTTPS:
...
3.  No arquivo de definição do serviço, adicione um elemento **Binding** dentro
da seção **Sites**. Isso adiciona uma associação HTTP para mapear o
ponto de extremidade para o site:
...
concluídas, mas você continua precisando adicionar as informações de
certificado ao arquivo de configuração do serviço.

4.  No arquivo de configuração do serviço (CSCFG), ServiceConfiguration.Cloud.cscfg adicione uma seção **Certificates** na seção **Role**, substituindo o valor de exemplo thumbprint mostrado abaixo pelo de seu certificado:
...
(O exemplo acima usa **sha1** para o algoritmo thumbprint. Especifique o valor apropriado ao algoritmo thumbprint do certificado.)

Agora que os arquivos de definição e configuração do serviço foram atualizados, empacote a implantação para carregamento no Azure. Se você estiver usando **cspack**, verifique se você não usou o sinalizador **/generateConfigurationFile**, porque isso substituirá as informações de certificado que acabou de inserir.

<h2><a name="step3"> </a><span class="short-header">Carregar no Azure</span>Etapa 3: Carregar o pacote de implantação e de certificado</h2>

O pacote de implantação foi atualizado para usar o certificado, e um
ponto de extremidade HTTPS foi adicionado. Agora é possível carregar o pacote e o certificado no Azure com o Portal de Gerenciamento.
...
5.  Na caixa de diálogo **Adicionar certificado**, insira o local do arquivo .pfx do certificado SSL, a senha do certificado e clique em **anexar certificado**.  
...
<h2><a name="step4"> </a><span class="short-header">Conectar usando HTTPS</span>Etapa 4: Conectar-se à instância da função usando HTTPS</h2>

Agora que a implantação está em funcionamento no Azure, você pode se conectar a ele usando HTTPS.
...
<h2><a name="additional_resources"> </a><span class="short-header">Recursos adicionais</span>Recursos adicionais</h2>
...
* [Como configurar um certificado SSL em um ponto de extremidade HTTPS][]
  ...
  [Etapa 2: Modificar a definição de serviço e arquivos de configuração]: #step2
