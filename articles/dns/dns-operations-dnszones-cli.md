...
O Azure DNS é um serviço somente do Gerenciador de Recursos do Azure. Ele não tem uma API do ASM. Você precisará verificar se a CLI do Azure está configurada para usar o modo do Gerenciador de Recursos. Você pode fazer isso usando o comando `azure config mode arm`.

Se você vir a mensagem*Erro: 'dns' não é um comando do Azure*, isso provavelmente significa que você está usando a CLI do Azure no modo ASM, não no modo do Gerenciador de Recursos.
