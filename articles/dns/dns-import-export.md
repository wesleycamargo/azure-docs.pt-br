<properties
   pageTitle="Importar e exportar um arquivo de zona de domínio usando o DNS do Azure | Microsoft Azure"
...
Este guia mostra como importar arquivos de zona DNS e exportá-los do DNS do Azure.
...
O DNS do Azure oferece suporte à importação e exportação de arquivos de zona usando a interface de linha de comando (CLI) do Azure. Este artigo explica como usar o recurso.
...
Antes de importar um arquivo de zona DNS para o DNS do Azure, você precisará obter uma cópia do arquivo de zona. A origem do arquivo depende de onde a zona DNS está hospedada no momento. Por exemplo:
...
Se uma zona com esse nome não existir no grupo de recursos, ela será criada para você. Se a zona já existir, conjuntos de registros importados serão mesclados a conjuntos de registros existentes. Para substituir os conjuntos de registros existentes, use a opção `--force`. Mais detalhes sobre [como conjuntos de registro são mesclados](#merge-the-file-with-existing-data) são fornecidos abaixo.

Para verificar o formato de um arquivo de zona sem realmente importá-lo, use a opção `--parse-only`.

Vejamos um exemplo passo a passo. Suponha que você deseje importar um arquivo de zona para a zona **contoso.com**.
...
6. Para importar a zona **contoso.com** do arquivo **contoso.com.txt** para uma nova zona DNS no grupo de recursos **myresourcegroup**, execute o comando `azure network dns zone import`.

	`azure network dns zone import myresourcegroup contoso.com contoso.com.txt`

    Esse comando irá carregar o arquivo de zona e analisá-lo. O comando executará uma série de comandos no serviço DNS do Azure para criar a zona e todos os conjuntos do registro na zona.

    Ele relatará o progresso na janela do console, bem como erros ou avisos. Como os conjuntos de registros são criados em série, pode levar alguns minutos para importar um arquivo de zona grande.

## Verificar a zona DNS após importar o arquivo

Você pode listar os registros usando o seguinte comando CLI do Azure.

	azure network dns record-set list myresourcegroup contoso.com

Você também pode fazer isso por meio do PowerShell usando `Get-AzureRmDnsRecordSet`.
...
## Mesclar o arquivo com dados existentes
...
- Início de parâmetros de autoridade (SOA) (exceto `host`) sempre são extraídos do arquivo de zona importado, independentemente do uso de `--force`. Da mesma forma, para o conjunto do registro de nome do servidor no ápice da zona, o TTL sempre é obtido do arquivo de zona importado.
...
- A diretiva `$TTL` é opcional e tem suporte. Quando nenhuma diretiva `$TTL` for especificada, serão importados registros sem um TTL explícito, definidos para um TTL padrão de 3600 segundos. Quando dois registros no mesmo conjunto de registros especificarem TTLs diferentes, o menor valor será usado.
...
Como acontece com a importação de zona, você primeiro precisa fazer logon, escolher sua assinatura e configurar a CLI do Azure para usar o modo do Gerenciador de Recursos. Vejamos um exemplo passo a passo.
...
4. Para exportar a zona DNS do Azure **contoso.com** existente no grupo de recursos **myresourcegroup** para o arquivo **contoso.com.txt** (na pasta atual), execute `azure network dns zone export`.
