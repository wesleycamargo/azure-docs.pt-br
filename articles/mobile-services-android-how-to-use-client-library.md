...
<div class="dev-center-tutorial-selector sublanding"> 
  <a href="/pt-br/develop/mobile/how-to-guides/work-with-net-client-library/" title=".NET Framework">.NET Framework</a><a href="/pt-br/develop/mobile/how-to-guides/work-with-html-js-client/" title="HTML/JavaScript">HTML/JavaScript</a><a href="/pt-br/develop/mobile/how-to-guides/work-with-ios-client-library/" title="iOS">iOS</a><a href="/pt-br/develop/mobile/how-to-guides/work-with-android-client-library/" title="Android" class="current">Android</a><a href="/pt-br/develop/mobile/how-to-guides/work-with-xamarin-client-library/" title="Xamarin" class="current">Xamarin</a>
</div>

Este guia mostra como executar cenários comuns usando o cliente Android para os Serviços Móveis do Azure. Os cenários abordados incluem consulta de dados, inserção, atualização e exclusão de dados, autenticação de usuários, tratamento de erros e personalização do cliente. Se for novo nos Serviços Móveis, você deve primeiro concluir o [início rápido dos Serviços Móveis][início rápido dos Serviços Móveis]. O tutorial de início rápido ajuda a configurar sua conta e criar seu primeiro serviço móvel.

As amostras são escritas em Java e exigem o [SDK dos Serviços Móveis][SDK dos Serviços Móveis]. Este tutorial também requer o [SDK do Android][SDK do Android], que inclui o ambiente de desenvolvimento integrado (IDE) Eclipse e o plug-in ADT (Android Developer Tools). O SDK dos Serviços Móveis oferece suporte ao Android versão 2.2 ou posterior, mas recomendamos compilar com base no Android versão 4.2 ou posterior.
...
-   [Como: Criar uma referência de tabela][Como: Criar uma referência de tabela]
    ...
    -   [A estrutura API][A estrutura API]
-   [Como: Consultar dados a partir de um serviço móvel][Como: Consultar dados a partir de um serviço móvel]
    ...
    -   [Como: Concatenar métodos de consulta][Como: Concatenar métodos de consulta]
-   [Como: Inserir dados em um serviço móvel][Como: Inserir dados em um serviço móvel]
-   [Como: Atualizar dados em um serviço móvel][Como: Atualizar dados em um serviço móvel]
-   [Como: Excluir dados em um serviço móvel][Como: Excluir dados em um serviço móvel]
-   [Como: Pesquisar um item específico][Como: Pesquisar um item específico]
-   [Como: Trabalhar com dados não tipados][Como: Trabalhar com dados não tipados]
-   [Como: Associar dados à interface com o usuário][Como: Associar dados à interface com o usuário]

    -   [Como: Definir o layout][Como: Definir o layout]
    -   [Como: Definir o adaptador][Como: Definir o adaptador]
    -   [Como: Usar o adaptador][Como: Usar o adaptador]
-   [Como: Autenticar usuários][Como: Autenticar usuários]

    -   [Armazenar tokens de autenticação em cache][Armazenar tokens de autenticação em cache]
-   [Como: Tratar erros][Como: Tratar erros]
-   [Como: Personalizar o cliente][Como: Personalizar o cliente]

    -   [Personalizar cabeçalhos de solicitações][Personalizar cabeçalhos de solicitações]
...
Vamos pressupor que você criou um serviço móvel e uma tabela. Para obter mais informações, consulte [Criar uma tabela][Criar uma tabela]. No código usado neste tópico, supomos que a tabela é denominada *ToDoItem* e tem as seguintes colunas:
...
Quando o esquema dinâmico está habilitado, os Serviços Móveis do Azure geram automaticamente novas colunas com base no objeto da solicitação de inserção ou atualização. Para obter mais informações, consulte [Esquema dinâmico][Esquema dinâmico].
...
O código a seguir cria o objeto [MobileServiceClient][MobileServiceClient] que é usado para acessar seu serviço móvel.
...
A maneira mais fácil de consultar ou modificar dados no serviço móvel é usando o *modelo de programação tipado*, pois o Java é uma linguagem fortemente tipada (posteriormente, discutiremos o modelo *não tipado*). Esse modelo fornece perfeita serialização e desserialização para JSON usando a biblioteca [gson][gson] ao enviar dados entre o cliente e o serviço móvel: o desenvolvedor não precisa fazer nada, a estrutura lida com tudo isso.

A primeira tarefa que você executa para consultar ou modificar dados é criar um objeto [MobileServiceTable][MobileServiceTable], chamando o método **getTable** no [**MobileServiceClient**][MobileServiceClient]. Examinaremos duas sobrecargas desse método:
...
A [primeira sobrecarga][primeira sobrecarga] é usada onde o nome da classe e o nome da tabela são iguais:

        MobileServiceTable<ToDoItem> mToDoTable = mClient.getTable(ToDoItem.class);

A [2ª sobrecarga][2ª sobrecarga] é usada quando o nome da tabela é diferente do nome do tipo.
...
As consultas, como essa, usam o objeto de retorno de chamada [**TableQueryCallback\<E\>**][**TableQueryCallback\<E\>**].
...
Você inicia um filtro com uma chamada do método [**where**][**where**] na referência de tabela. Isso é seguido por uma chamada do método [**field**][**field**] seguida por uma chamada de método que especifica o predicado lógico. Os métodos de predicado possíveis incluem [**eq**][**eq**], [**ne**][**ne**], [**gt**][**gt**], [**ge**][**ge**], [**lt**][**lt**], [**le**][**le**] etc.
...
Por exemplo, você pode filtrar por datas. Você pode comparar todo o campo de data, mas também pode comparar partes da data, com métodos como [**year**][**year**], [**month**][**month**], [**day**][**day**], [**hour**][**hour**], [**minute**][**minute**] e [**second**][**second**]. O código parcial a seguir adiciona um filtro para itens cuja *data de vencimento* é igual a 2013.

        mToDoTable.where().year("due").eq(2013)

Você pode usar uma grande variedade de filtros complexos em campos de cadeia de caracteres com métodos como [**startsWith**][**startsWith**], [**endsWith**][**endsWith**], [**concat**][**concat**], [**subString**][**subString**], [**indexOf**][**indexOf**], [**replace**][**replace**], [**toLower**][**toLower**], [**toUpper**][**toUpper**], [**trim**][**trim**] e [**length**][**length**]. Os filtros de código parcial a seguir são para linhas de tabela onde a coluna *text* começa com "PRI0".

        mToDoTable.where().startsWith("text", "PRI0")

Os campos de número também permitem uma grande variedade de filtros mais complexos com métodos como [**add**][**add**], [**sub**][**sub**], [**mul**][**mul**], [**div**][**div**], [**mod**][**mod**], [**floor**][**floor**], [**ceiling**][**ceiling**] e [**round**][**round**]. Os filtros de código parcial a seguir são para linhas de tabela onde *duration* é um número par.

        mToDoTable.where().field("duration").mod(2).eq(0)

Você pode combinar predicados com métodos como [**and**][**and**], [**or**][**or**] e [**not**][**not**]. Este código parcial combina dois dos exemplos acima.
...
Para obter uma discussão mais detalhada e exemplos de filtragem, consulte [Explorando a riqueza do modelo de consulta de cliente dos Serviços Móveis do Android][Explorando a riqueza do modelo de consulta de cliente dos Serviços Móveis do Android].
...
O primeiro parâmetro do método [**orderBy**][**orderBy**] é uma cadeia de caracteres igual ao nome do campo em que será feita a classificação.

O segundo parâmetro usa a enumeração [**QueryOrder**][**QueryOrder**] para especificar se a classificação será em ordem crescente ou decrescente.
...
O método [**select**][**select**] precisa seguir métodos como [**where**][1] e [**orderBy**][2], se eles estiverem presentes. Ele pode ser seguido por métodos como [**top**][**top**].
...
O que faz isso funcionar é que os métodos de consulta usados retornam objetos [**MobileServiceQuery\<T\>**][**MobileServiceQuery\<T\>**], que, por sua vez, podem ter métodos adicionais invocados neles. Para encerrar a série de métodos e realmente executar a consulta, chame o método [**execute**][**execute**].
...
Em seguida, chame o método [**insert**][**insert**].
...
Para operações **insert**, o objeto de retorno de chamada é um [**TableOperationCallback\<ToDoItem\>**][**TableOperationCallback\<ToDoItem\>**].
...
Observe que no caso *delete*, o objeto de retorno de chamada é [**TableDeleteCallback**][**TableDeleteCallback**] e o método**onCompleted** é um pouco diferente, pois nenhuma linha de tabela é retornada.
...
## <a name="lookup"></a><span class="short-header">Pesquisando dados</span>Como: Pesquisar um item específico
...
Semelhante ao modelo tipado, você começa obtendo uma referência de tabela, mas, nesse caso, é um objeto[MobileServicesJsonTable][MobileServicesJsonTable]. Você obtém a referência chamando o método [getTable()][getTable()] em uma instância do cliente de Serviços Móveis.
...
O código a seguir mostra como fazer uma inserção. A primeira etapa consiste em criar[**JsonObject**][**JsonObject**], que faz parte da biblioteca [gson][gson].
...
A próxima etapa consiste em inserir o objeto. A função de retorno de chamada passada para o método [**insert**][3] é uma instância da classe [**TableJsonOperationCallback**][**TableJsonOperationCallback**]. Observe como o primeiro parâmetro do método *onCompleted* é um JsonObject.
...
O código a seguir mostra como recuperar uma tabela inteira. Observe que o modelo de programação não tipado usa um objeto de retorno de chamada diferente: [**TableJsonQueryCallback**][**TableJsonQueryCallback**].
...
## <a name="binding"></a><span class="short-header">Vinculando dados</span>Como: Associar dados à interface com o usuário
...
Como a fonte de dados da nossa exibição é uma matriz de ToDoItem, podemos criar a subclasse do nosso adaptador de uma classe *ToDoItem*, podemos criar uma subclasse do nosso adaptador de uma classe *ArrayAdapter\<ToDoItem\>*. Esta subclasse produzirá uma exibição para cada *ToDoItem* usando o layout *row\_list\_to\_do*.
...
Os Serviços Móveis oferecem suporte à autenticação e à autorização de usuários de aplicativos usando uma variedade de provedores de identidade externos: Facebook, Google, Conta da Microsoft, Twitter e Active Directory do Azure. Você pode definir permissões em tabelas para restringir o acesso a operações específicas apenas para usuários autenticados. Você também pode usar a identidade de usuários autenticados para implementar regras de autorização em scripts do servidor. Para obter mais informações, consulte [Comece a usar a autenticação][Comece a usar a autenticação].

Dois fluxos de autenticação são suportados: um fluxo de *servidor* e um fluxo *de cliente*. O fluxo de servidor fornece a experiência de autenticação mais simples, pois depende da interface de autenticação da web do provedor. O fluxo de cliente permite uma integração mais profunda com recursos específicos do dispositivo, como logon único, uma vez que depende de provedores específicos e SDKs específicos do dispositivo.

Três etapas são necessárias para habilitar a autenticação no seu aplicativo:

1.  Registrar seu aplicativo para autenticação com um provedor e configurar os Serviços Móveis
2.  Restringir as permissões de tabela somente aos usuários autenticados
3.  Adicionar código de autenticação ao seu aplicativo
...
Essas duas primeiras tarefas são feitas usando o [Portal de Gerenciamento do Azure][Portal de Gerenciamento do Azure]. Para obter mais informações, consulte [Comece a usar a autenticação][Comece a usar a autenticação].
...
2.  No método **onCreate** da classe activity, adicione a seguinte linha de código após o código que cria o objeto `MobileServiceClient`: assumimos que a referência ao objeto `MobileServiceClient` seja *mClient*.
...
<p>Se voc&ecirc; estiver usando um provedor de identidade diferente do Google, altere o valor passado para o m&eacute;todo <strong>logon</strong> acima para um dos seguintes: <i>MicrosoftAccount</i>, <i>Facebook</i>, <i>Twitter</i>, ou <i>windowsazureactivedirectory</i>.</p>
</div>
...
Você poderá ver um exemplo de como fazer a validação e tratar qualquer erro [aqui][aqui], que implementará a validação por meio de scripts de servidor que retornarão exceções de erro e do código do cliente que lida com as exceções.
...
## <a name="customizing"></a><span class="short-header">Personalizando o cliente</span>Como: Personalizar o cliente

### <a name="headers"></a>Como: Personalizar cabeçalhos de solicitações
...
Você precisa serializar os nomes de cliente em nomes JSON que correspondam aos nomes de coluna da tabela*ToDoItem* no servidor. O código a seguir, que usa a biblioteca [gson][gson] faz isso.
...
[getTable()][2ª sobrecarga] conforme mostrado no código a seguir.

        mToDoTable = mClient.getTable("ToDoItemBackup", ToDoItem.class);

### <a name="conversions"></a>Como: Automatizar os mapeamentos de nome de coluna

O mapeamento de nomes de coluna para uma tabela estreita com apenas algumas colunas não é um grande problema, conforme observado na seção anterior. Mas, suponha que a nossa tabela tenha muitas colunas, digamos 20 ou 30. Acontece que podemos chamar a API [gson][gson] e especificar uma estratégia de conversão que se aplicará a todas as colunas e evitar a anotação de cada nome de coluna.

Para isso, usamos a biblioteca [gson][gson] que é usada pela biblioteca de cliente Android de forma oculta para serializar objetos Java para dados JSON, que são enviados aos Serviços Móveis do Azure.
...
Para ver um exemplo de como fazer isso, verifique a postagem de blog [Personalizando a serialização usando a biblioteca][Personalizando a serialização usando a biblioteca][gson][gson] no cliente de Serviços Móveis do Android</a>.
...
<!-- URLs. -->
  ...
  [SDK do Android]: https://go.microsoft.com/fwLink/p/?LinkID=280125&clcid=0x409
  ...
  [primeira sobrecarga]: http://go.microsoft.com/fwlink/p/?LinkId=296839
  [2ª sobrecarga]: http://go.microsoft.com/fwlink/p/?LinkId=296840
  ...
  [Explorando a riqueza do modelo de consulta de cliente dos Serviços Móveis do Android]: http://hashtagfail.com/post/46493261719/mobile-services-android-querying
  ...
  [aqui]: https://www.windowsazure.com/pt-br/develop/mobile/tutorials/validate-modify-and-augment-data-dotnet/
  [Personalizando a serialização usando a biblioteca]: http://hashtagfail.com/post/44606137082/mobile-services-android-serialization-gson
  []: http://go.microsoft.com/fwlink/p/?LinkId=298735 "aqui em"
