...
Para poder usar o Gerenciamento de Logs no Insights Operacionais do Microsoft Azure, você deve ter a solução instalada. Como esse pacote deve ser configurado e não coleta nada por padrão, ele está habilitado por padrão em cada novo espaço de trabalho. Se o tiver removido, você poderá adicioná-lo novamente da Galeria de Soluções. Consulte [Usar a Galeria de Soluções para adicionar ou remover soluções](operational-insights-add-solution.md).

Você pode adicionar novos logs para coletar eventos e escolher quais níveis de evento ou gravidade deseja coletar para os logs. Depois de o gerenciamento de log estar configurado, ele enviará por push sua política de coleta e começará a coletar eventos. Você pode acessar algumas falhas iniciais de eventos de log coletados dos servidores monitorados usando o bloco **Gerenciamento de log** na página **Visão Geral** do Insights Operacionais.

![imagem do bloco Gerenciamento de Log](./media/operational-insights-log-collection/overview-log-mgt.png)

O bloco abre a página **Gerenciamento de Log**, em que é possível exibir um resumo dos eventos contidos nos logs. Ou você pode usar a pesquisa de log diretamente, como para qualquer outro tipo de dados.
...
### Para trabalhar com arquivos de log
...
4. Você pode executar pesquisas de log adicionais com base nos resultados iniciais e então analisar e correlacionar os logs.
