---
title: "Proteger o acesso ao Azure RemoteApp e além | Microsoft Docs"
...
---
# <a name="securing-access-to-azure-remoteapp-and-beyond"></a>Proteger o acesso ao RemoteApp do Azure e além
> [!IMPORTANT]
...
O administrador escolhe os usuários que podem acessar os aplicativos remotos na coleção. Você pode usar as contas corporativas ou de estudante do Azure AD (Azure Active Directory) – anteriormente chamadas de "contas organizacionais" – ou as contas da Microsoft (por exemplo, @outlook.com). A maioria dos cenários empresariais usa as contas do Azure AD. Elas permitem que você use os recursos de acesso condicional analisados posteriormente e também são a única opção para as coleções de domínio. O restante do artigo pressupõe que você esteja usando as contas do AD do Azure com o RemoteApp do Azure.
...
### <a name="how-to-set-up-conditional-access-for-azure-remoteapp"></a>Como configurar o acesso condicional para o RemoteApp do Azure
...
> [!NOTE]
> Supomos que você tenha atualizado o AD do Azure para a camada Premium e criado pelo menos uma coleção do RemoteApp do Azure.
> 
> 
...
2. Clique em **Aplicativos** e, em seguida, clique em **Microsoft Azure RemoteApp** para configurar o acesso condicional. Observe que é possível configurar o acesso condicional para cada aplicativo de “software como serviço” no diretório separadamente.
   ![Configurar o acesso condicional para o RemoteApp do Azure](./media/remoteapp-secureaccess/ra-conditionalaccessscreen.png)
3. Na guia **Configurar**, defina **Habilitar Regras de Acesso** para LIGADO.
...
4. Agora, você pode configurar várias regras e escolher em quem aplicá-las:
   ...
   1. Escolha **Bloquear o acesso quando não estiver no trabalho** para impedir completamente que os usuários acessem o Azure RemoteApp fora do ambiente de rede especificado.
   2. Clique na opção abaixo para definir os intervalos de endereços IP que constituem a “rede confiável”. Tudo que estiver fora será rejeitado.
...
### <a name="how-to-monitor-access-to-azure-remoteapp"></a>Como monitorar o acesso ao RemoteApp do Azure
...
4. Clique no aplicativo para mostrar as informações sobre os usuários que acessam o Azure RemoteApp.
...
* Não ingressado em domínio: os aplicativos terão um “campo visual” dos outros recursos na VNET. Por exemplo, isso pode ser usado para conectar os aplicativos a um banco de dados SQL que usa a autenticação SQL (os aplicativos autenticam o usuário diretamente no banco de dados)
...
### <a name="how-to-create-a-secure-connection-between-azure-and-my-on-premises-environment"></a>Como criar uma conexão segura entre o Azure e meu ambiente local
...
![Proteger Azure RemoteApp](./media/remoteapp-secureaccess/ra-secureoverview.png) No Estágio 1, escolhemos os usuários e criamos regras de acesso que determinam como o ARA pode ser acessado. No exemplo abaixo, apenas permitimos o acesso para os usuários que trabalham na rede corporativa. Os usuários não compatíveis não poderão acessar o ambiente ARA.
No Estágio 2, expomos o recurso de back-end somente por meio da configuração da VNet/VPN que controlamos. O RemoteApp do Azure foi colocado na mesma VNET. O resultado final é que o recurso só pode ser acessado através do ambiente ARA.
