---
title: Perguntas frequentes do RemoteApp do Azure | Microsoft Docs
...
Se você não encontrar as informações necessárias ou se tiver uma pergunta adicional que não estamos abordando aqui, acesse o [fórum do RemoteApp do Azure](http://aka.ms/araforum) e faça sua pergunta. Sempre poderemos adicionar mais respostas aqui.
...
* **Posso usar uma nuvem ou máquina virtual existente como modelo para a Minha coleção de RemoteApp?** Sim! Você pode criar uma imagem com base em uma VM do Azure, usar uma das imagens incluídas com sua assinatura ou criar uma imagem personalizada. Confira as [Opções de imagem do RemoteApp](remoteapp-imageoptions.md).

## <a name="network-options"></a>Opções de rede
* **A coleção híbrida requer uma VNET. Podemos usar nossa VNET existente?** Sim, se a VNET existente for uma VNET do Azure. Veja "Etapa 1: Configurar sua rede virtual" nas [instruções de coleção híbrida](remoteapp-create-hybrid-deployment.md) para obter mais informações.
