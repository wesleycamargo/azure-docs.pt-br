<properties
   pageTitle="Etapas para configuração de um circuito da Rota Expressa usando ARM| Microsoft Azure"
...
Este artigo fornece uma orientação pelas etapas de criação de um circuito da Rota Expressa usando cmdlets do PowerShell e o modelo de implantação ARM. As etapas a seguir também mostrarão a você como verificar o status, atualizar ou excluir e desprovisionar um circuito da Rota Expressa.
...
3. **Criar um circuito da Rota Expressa.**
	...
	Especifique a camada certa de SKU e a família de SKU.
	 ...
	 - A camada de SKU determina se o complemento padrão da Rota Expressa ou da Rota Expressa Premium está habilitado. Você pode especificar *padrão* para obter a SKU padrão ou *premium* para obter o complemento premium
...
5. **Verifique periodicamente o status e o estado da chave do circuito.**
	...
	Isso permite que você saiba quando seu provedor habilitou seu circuito. Após a configuração do circuito, o *ServiceProviderProvisioningState* será exibido como *Provisionado*, como mostrado no exemplo abaixo.
...
6. **Crie sua configuração de roteamento.**
	...
	Veja a página [Configuração de roteamento do circuito da Rota Expressa (criar e modificar emparelhamentos de circuito)](expressroute-howto-routing-arm.md) para obter instruções detalhadas.

7. **Vincule uma Rede Virtual a um circuito da Rota Expressa.**

	Em seguia, vincule uma Rede Virtual ao seu circuito da Rota Expressa. Veja [Vinculando circuitos da Rota Expressa a redes virtuais](expressroute-howto-linkvnet-arm.md) para obter instruções passo a passo. Se precisar criar uma rede virtual para a Rota Expressa, veja [Criando uma rede virtual para a Rota Expressa](expressroute-howto-createvnet-classic.md) para obter instruções.
...
Se o provedor de serviços tiver desprovisionado o circuito (o estado de provisionamento do provedor de serviços é definido como *não provisionado*) antes da execução do cmdlet acima, desprovisionaremos o circuito e interromperemos a cobrança.
