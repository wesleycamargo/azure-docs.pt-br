<properties linkid="manage-services-hdinsight-get-started-hdinsight" urlDisplayName="Introdução" pageTitle="Introdução ao Emulador do HDInsight para Azure" metaKeywords="hdinsight, Azure, hdinsight azure, introdução ao hdinsight, emulador, emulador do hdinsight" description="Saber usar o Emulador do HDInsight para Azure." umbracoNaviHide="0" disqusComments="1" writer="jgao" editor="cgronlun" manager="paulettm" title="Introdução ao Emulador do HDInsight"/>



# Introdução ao Emulador do HDInsight 

Este tutorial apresenta o uso do Emulador do Microsoft HDInsight para Azure (anteriormente Visualização do Desenvolvedor do HDInsight Server). O Emulador do HDInsight é fornecido com os mesmos componentes do ecossistema do Hadoop como Azure HDInsight. Para obter detalhes, incluindo informações sobre as versões implantadas, consulte [Qual versão do Hadoop está no Azure HDInsight?](http://www.windowsazure.com/pt-br/manage/services/hdinsight/howto-hadoop-version/ "HDInsight components and versions") 

O Emulador do HDInsight fornece um ambiente de desenvolvimento local para o Azure HDInsight. Se estiver familiarizado com o Hadoop, você poderá começar a usar o Emulador com o HDFS. Mas, no HDInsight, o sistema de arquivos padrão é o armazenamento de Blob do Azure (WASB, também conhecido como o Armazenamento do Azure – Blobs), portanto, por fim, você desejará desenvolver seus trabalhos usando o WASB. Você pode começar a desenvolver no WASB usando o Emulador de Armazenamento do Azure - provavelmente apenas desejará usar um pequeno subconjunto dos dados (nenhuma alteração de configuração é necessária no Emulador do HDInsight, apenas um nome de conta de armazenamento diferente). Em seguida, você testará seus trabalhos localmente no Armazenamento do Azure - novamente, usando apenas um subconjunto dos dados (requer uma alteração de configuração no Emulador do HDInsight). Finalmente, você estará pronto para mover a parte de computação do seu trabalho para o HDInsight e executar um trabalho em relação aos dados de produção.

> [WACOM.NOTE] O Emulador do HDInsight pode usar somente uma implantação de nó único. 

Para obter um tutorial sobre como usar o HDInsight, consulte [Introdução ao Azure HDInsight][hdinsight-get-started]
...
* [Conecte-se ao armazenamento de Blob do Azure](#blobstorage)
...
> [WACOM.NOTE] No momento, o Emulador do HDInsight só oferece suporte ao sistema operacional em inglês. 


> [WACOM.NOTE] Se você tiver a Visualização do Desenvolvedor do Microsoft HDInsight instalada, desinstale primeiro estes dois componentes do Painel de Controle/Programas e Recursos.
><ul>
<li>Visualização do Desenvolvedor do HDInsight</li>
<li>Visualização do Desenvolvedor do Hortonworks Data Platform</li>
</ul>
...
8. Verifique se o Web Platform Installer exibe a mensagem **os produtos a seguir foram instalados com êxito** e, em seguida, clique em **Concluir**.
9. Clique em **Sair** para fechar a janela do Web Platform Installer 4.6.

	A instalação deve ter inserido três ícones na área de trabalho. Os três ícones estão vinculados a estes itens: 
	
	- **Linha de comando do Hadoop**: o prompt de comando do Hadoop a partir do qual os trabalhos de MapReduce, Pig e Hive são executados no Emulador do HDInsight.
	...
	- **Status de MapReduce do Hadoop**: o controlador de trabalho que aloca tarefas MapReduce para os nós de um cluster.
...
Agora você tem o emulador do HDInsight configurado na estação de trabalho. Você pode executar um trabalho do MapReduce para testar a instalação. Primeiro, você carregará alguns arquivos de texto para o HDFS e, em seguida, executará um trabalho de contagem de palavras do MapReduce para contar as frequências de palavras desses arquivos. 

O programa de contagem de palavra do MapReduce foi compactado em *hadoop-examples.jar*.  O arquivo jar está localizado na pasta *C:\Hadoop\hadoop-1.1.0-SNAPSHOT*.

A sintaxe do comando jar é:

	hadoop jar <jar> [mainClass] args...

Você também usará alguns comandos fs. Para obter mais informações sobre comandos do Hadoop, consulte o [manual de comandos do Hadoop][hadoop-commands-manual].

O trabalho de contagem de palavra do MapReduce usa dois argumentos: uma pasta de entrada e uma pasta de saída.  Você usará *hdfs://localhost/user/HDIUser* como a pasta de entrada e *hdfs://localhost/user/HDIUser/WordCount_Output* como o diretório de saída. A pasta de saída não poderá ser uma pasta existente, caso contrário o trabalho do MapReduce falhará. Se você quiser executar o trabalho do MapReduce pela segunda vez, especifique uma pasta de saída diferente ou exclua a pasta de saída existente. 
...
2. Execute o comando do Hadoop a seguir para criar uma pasta HDFS para armazenar os arquivos de entrada e saída:

		hadoop fs -mkdir /user/HDIUser
	
3. Execute o comando do Hadoop a seguir para copiar alguns arquivos locais para HDFS:
...
As amostras são organizadas com base no processamento dos cenários de dados de log do IIS W3C. Uma ferramenta de geração de dados é fornecida para criar e importar os conjuntos de dados de vários tamanhos para o HDFS ou o WASB (armazenamento de Blob do Azure). Consulte [Usar o armazenamento de Blob do Azure para HDInsight][hdinsight-blob-store] para obter mais informações. Em seguida, os trabalhos do MapReduce, do Pig ou do Hive poderão ser executados nas páginas de dados gerados pelo script de PowerShell. Observe que os scripts do Pig e do Hive usados são ambos compilados para os programas MapReduce. Os usuários podem executar uma série de trabalhos para observar, por eles mesmos, os efeitos de uso dessas tecnologias diferentes e os efeitos do tamanho dos dados na execução das tarefas de processamento. 

### Nesta seção

- [Cenários de dados de log do IIS w3c](#scenarios)
...
- **errors**: calcula o número de erros por página, por hora, para as solicitações cujo status era 404 ou 500 
...
MapReduce é o mecanismo de computação básico para o Hadoop. Por padrão, ele é implementado no Java, mas também há exemplos que aproveitam o .NET e o Hadoop Streaming que usam C#. A sintaxe para executar um trabalho do MapReduce é:
...
3. Execute o comando a seguir:
	...
	<tr><td>w3c_scenarios.jar</td><td>O arquivo jar está localizado na pasta C:\Hadoop\GettingStarted\Java.</td></tr>
...
4. Execute o comando a seguir para exibir o arquivo de saída:
	...
	Portanto, a página Default.aspx terá 3409 visitas e assim por diante. 
...
4. Execute o comando a seguir para criar a **/w3c/hive/input** pasta e copiar o arquivo de dados da estação de trabalho para HDFS:

        hadoop fs -mkdir /w3c/hive/input
        hadoop fs -cp /w3c/input/small/data_w3c_small.txt /w3c/hive/input

5. Execute o comando a seguir para executar o arquivo de script **w3ccreate.hql**:  O script cria uma tabela do Hive e carrega dados para ela:
        
		C:\Hadoop\hive-0.9.0\bin\hive.cmd -f ./Hive/w3c/w3ccreate.hql -hiveconf "input=/w3c/hive/input/data_w3c_small.txt"

	O script de HiveQL é:
...
6. Execute o comando a seguir para processar o arquivo de script do HiveQL **w3ctotalhitsbypate.hql**.  
	...
	<tr><td>C:\Hadoop\hive-0.9.0\bin\hive.cmd</td><td>O script de comando do Hive.</td></tr>
	<tr><td>C:\Hadoop\GettingStarted\Hive\w3c\w3ctotalhitsbypage.hql</td><td> Você pode substituir o arquivo de script do Hive por um destes:
	<ul>
...
3. Execute o comando a seguir para enviar um trabalho do Pig:
	...
	<table border="1">
	<tr><td>Arquivo</td><td>Descrição</td></tr>
	<tr><td>C:\Hadoop\pig-0.9.3-SNAPSHOT\bin\pig.cmd</td><td>O script de comando do Pig.</td></tr>
	<tr><td>C:\Hadoop\GettingStarted\Pig\w3c\TotalHitsForPage.pig</td><td> Você pode substituir o arquivo de script de Pig Latin por um destes:
	<ul>
...
O Azure HDInsight usa o armazenamento de Blob do Azure como o sistema de arquivos padrão. Para obter mais informações, consulte [Usar o armazenamento de Blob do Azure com HDInsight][hdinsight-blob-store]. 
...
- conexão com o emulador de armazenamento
- conexão com o armazenamento de Blob do Azure
- configuração do armazenamento de Blob do Azure como o sistema de arquivos padrão para o Emulador do HDInsight

### Conectar-se ao emulador de armazenamento

O Emulador de Armazenamento do Azure é fornecido com o [SDK do Azure para .NET][azure-sdk]. O emulador de armazenamento não é iniciado automaticamente. Você deve iniciá-lo manualmente.  O nome do aplicativo é *Emulador de Armazenamento do Azure*. Para iniciar/parar os emuladores, clique com botão direito do mouse no ícone azul do Azure na bandeja de sistema do Windows e, em seguida, clique em Mostrar Interface do Usuário do Emulador de Armazenamento.

> [WACOM.NOTE] A mensagem de erro poderá ser exibida quando você iniciar o emulador de armazenamento:

><pre><code>The process cannot access the file because it is being used by another process.
</code></pre>

><p>Isso ocorre porque um dos serviços do Hive no Hadoop também usa a porta 10000. Para resolver o problema, execute o procedimento a seguir:</p>

><ol>
<li>Interrompa os serviços do Hive no Hadoop usando services. msc: Apache Hadoop hiveserver e Apache Hadoop Hiveserver2.</li>
<li>Inicie o emulador de armazenamento de Blob. </li>
<li>Reinicie os dois serviços do Hive no Hadoop. </li>
</ol>
...
Para obter instruções sobre como criar uma conta de armazenamento, consulte [Como criar uma conta de armazenamento][azure-create-storage-account].
...
Antes de acessar uma Conta de Armazenamento do Azure, você deve adicionar o nome e a chave da conta ao arquivo de configuração.

**Para configurar a conexão com uma Conta de Armazenamento do Azure**

1. Abra **C:\Hadoop\hadoop-1.1.0-SNAPSHOT\conf\core-site.xml** no Bloco de Notas.
2. Adicione a marca &lt;property> a seguir ao lado das outras marcas &lt;property>:
	...
	Substitua &lt;StorageAccountName> e &lt;StorageAccountKey> pelos valores que correspondem às informações de sua conta de armazenamento.
...
### Usar um contêiner de armazenamento de Blob do Azure como o sistema de arquivos padrão
...
1. Abra **C:\Hadoop\hadoop-1.1.0-SNAPSHOT\conf\core-site.xml** no Bloco de Notas.
2. Localize a marca &lt;property> a seguir:

		<property>
		  <name>fs.default.name</name>
		  <!-- cluster variant -->
		  <value>hdfs://localhost:8020</value>
		  <description>O nome do sistema de arquivos padrão.  Pode ser a	cadeia de caracteres literal "local" ou um host:porta para NDFS.</description>
		  <final>true</final>
		</property>
...
3. Substitua a marca pelas duas marcas &lt;property> a seguir:
		...
		<property>
		  ...
		  <value>hdfs://localhost:8020</value>
		  <description>Uma base para outros diretórios temporários.</description>
		</property>
...
Neste tutorial, você terá um Emulador do HDInsight instalado e terá executado alguns trabalhos do Hadoop. Para saber mais, consulte os seguintes artigos:

- [Introdução ao Azure HDInsight][hdinsight-get-started]
- [Desenvolver programas Java MapReduce para HDInsight][hdinsight-develop-mapreduce]
- [Desenvolver programas MapReduce de streaming de C# Hadoop para HDInsight][hdinsight-develop-deploy-streaming]
- [Notas de versão do emulador do HDInsight](https://gettingstarted.hadooponazure.com/releaseNotes.html)
- [Fórum do MSDN para discussão do HDInsight](http://social.msdn.microsoft.com/Forums/pt-br/hdinsight)



[azure-sdk]: http://www.windowsazure.com/pt-br/downloads/
[azure-create-storage-account]: /pt-br/manage/services/storage/how-to-create-a-storage-account/
...
[hdinsight-blob-store]: /pt-br/manage/services/hdinsight/howto-blob-store/
...
[hdinsight-develop-deploy-streaming]: /pt-br/manage/services/hdinsight/develop-deploy-hadoop-streaming-jobs/
