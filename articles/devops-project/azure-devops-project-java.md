...
O Azure DevOps Project apresenta uma experiência simplificada que cria recursos do Azure e configura um pipeline de integração contínua (CI) e entrega contínua (CD) para seu aplicativo Java no Azure DevOps Services.  
...
1. Escolha o ícone **Criar um recurso** na barra de navegação à esquerda e procure o **Projeto do DevOps**.  Escolha **Criar**.
    ...
    ![Iniciando a configuração de Entrega Contínua](_img/azure-devops-project-java/fullbrowser.png)
...
1. A estrutura de exemplo padrão é **Spring**. Mantenha a configuração padrão e selecione **Avançar**.  
...
1. Em alguns minutos, o **painel do Azure DevOps Project** é carregado no portal do Azure.  Um aplicativo de exemplo é configurado em um repositório em sua organização do Azure DevOps Services, é executado um build e seu aplicativo é implantado no Azure.  Esse painel oferece visibilidade no seu **repositório de código**, **pipeline CI/CD do Azure** e seu **aplicativo no Azure**.  À direita do painel, selecione **Procurar** para exibir o aplicativo em execução.
    ...
    ![Exibição Painel](_img/azure-devops-project-java/dashboardnopreview.png) 
    
O Azure DevOps Project configura automaticamente um build de CI e libera o gatilho.  Agora você está pronto para colaborar com uma equipe em um aplicativo Java com um processo de CI/CD que implanta automaticamente o trabalho mais recente em seu site da Web.
...
1. À esquerda do navegador, vá para o arquivo **src/main/webapp/index.html**.
