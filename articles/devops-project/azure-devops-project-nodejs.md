...
O Projeto de DevOps do Azure apresenta uma experiência simplificada que cria recursos do Azure e configura um pipeline de integração contínua (CI) e entrega contínua (CD) para seu aplicativo Node.js no Visual Studio Team Services (VSTS), que é a solução de DevOps da Microsoft para o Azure.  
...
1. Crie uma **nova** conta do VSTS gratuita ou use uma conta **existente**.  Escolha um **nome** para seu projeto do VSTS.  Selecione sua **assinatura do Azure**, **local** e escolha um **nome** para seu aplicativo.  Quando terminar, escolha **Concluído**.
    ...
    ![Inserir informações do VSTS](_img/azure-devops-project-nodejs/vstsazureinfo.png)
...
O projeto de DevOps do Azure configura automaticamente um build de CI e libera o acionador.  Agora você está pronto para colaborar com uma equipe em um aplicativo Node.js com um processo de CI/CD que implanta automaticamente o trabalho mais recente em seu site da Web.
