---
title: Criar um pipeline de CI/CD para Python com o projeto de DevOps do Azure | Início rápido
...
ms.locfileid: "37966864"
---
# <a name="create-a-cicd-pipeline-for-python-with-the-azure-devops-project"></a>Criar um pipeline de CI/CD para Python com o projeto de DevOps do Azure

O projeto de DevOps do Azure apresenta uma experiência simplificada que cria recursos do Azure e configura um pipeline de integração contínua (CI) e de entrega contínua (CD) para seu aplicativo Python no Visual Studio Team Services (VSTS), que é a solução de DevOps da Microsoft para o Azure.  
...
1. Escolha o ícone **Criar um recurso** na barra de navegação à esquerda, depois procure o **projeto de DevOps**.  Escolha **Criar**.
    ...
    ![Iniciando a configuração de Entrega Contínua](_img/azure-devops-project-python/fullbrowser.png)
...
1. Crie uma **nova** conta do VSTS gratuita ou use uma conta **existente**.  Escolha um **nome** para seu projeto do VSTS.  Selecione sua **assinatura do Azure**, **local** e escolha um **nome** para seu aplicativo.  Quando terminar, escolha **Concluído**.
    ...
    ![Inserir informações do VSTS](_img/azure-devops-project-python/vstsazureinfo.png)

1. Em alguns minutos, o **painel do projeto** é carregado no portal do Azure.  Um aplicativo de exemplo é configurado em um repositório em sua conta do VSTS, é executada uma compilação e seu aplicativo é implantado no Azure.  Esse painel oferece visibilidade no seu **repositório de código**, **pipeline CI/CD do VSTS** e seu **aplicativo no Azure**.  No lado direito do painel, selecione **Procurar** para exibir seu aplicativo em execução.

    ![Exibição Painel](_img/azure-devops-project-python/dashboardnopreview.png) 
    
O projeto de DevOps do Azure configura automaticamente um build de CI e libera o acionador.  Agora você está pronto para colaborar com uma equipe em um aplicativo do Python com um processo de CI/CD que implanta automaticamente o seu trabalho mais recente em seu site da Web.
...
1. No lado esquerdo do navegador, navegue até o arquivo **app/templates/app/index.html**.
...
1. No lado esquerdo do navegador, selecione **Tarefas**.  As tarefas são as atividades que seu processo de implantação realiza.  Neste exemplo, uma tarefa foi criada para ser implantada no **serviço de Aplicativo do Azure**.
