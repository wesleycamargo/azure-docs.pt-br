...
Este tutorial explica as etapas para instalar um controlador de domínio adicional de sua floresta do Active Directory Corp em uma máquina virtual (VM) na [Rede Virtual do Azure][Rede Virtual do Azure]. Neste tutorial, a rede virtual da VM está conectada à rede de sua empresa. Para obter orientações conceituais sobre a instalação de Serviços de Domínio Active Directory (AD DS) na rede virtual do Azure, consulte [Diretrizes para a implantação do Active Directory do Windows Server em máquinas virtuais do Azure][Diretrizes para a implantação do Active Directory do Windows Server em máquinas virtuais do Azure] (a página pode estar em inglês).

## Sumário

-   [Pré-requisitos][Pré-requisitos]
-   [Etapa 1: verificar o endereço IP estático de YourPrimaryDC][Etapa 1: verificar o endereço IP estático de YourPrimaryDC]
-   [Etapa 2: Instalar a floresta Corp][Etapa 2: Instalar a floresta Corp]
-   [Etapa 3: Criar sub-redes e sites][Etapa 3: Criar sub-redes e sites]
-   [Etapa 4: Instalar um controlador de domínio adicional no CloudSite][Etapa 4: Instalar um controlador de domínio adicional no CloudSite]
-   [Etapa 5: validar a instalação][Etapa 5: validar a instalação]
-   [Etapa 6: provisionar uma máquina virtual que seja associada ao domínio na inicialização][Etapa 6: provisionar uma máquina virtual que seja associada ao domínio na inicialização]
-   [Etapa 7: fazer backup do controlador de domínio][Etapa 7: fazer backup do controlador de domínio]
-   [Etapa 8: Testar a autenticação e a autorização][Etapa 8: Testar a autenticação e a autorização]

## <span id="Prerequisites"></span></a>Pré-requisitos

-   [Configure um VPN site a site no Portal de Gerenciamento][Configure um VPN site a site no Portal de Gerenciamento] configurado entre a rede virtual Azure e a rede Corp.
-   Crie um serviço de nuvem na rede virtual.
-   Implante duas VMs no Serviço de Nuvem que façam parte da rede virtual (especifique a sub-rede onde você deseja colocar a máquina virtual). Para obter mais informações, consulte [Adicionar uma máquina virtual a uma rede virtual (a página pode estar em inglês)][Adicionar uma máquina virtual a uma rede virtual (a página pode estar em inglês)]. Uma VM deve ter tamanho L ou superior para que dois discos de dados possam ser anexados a ela. Os discos de dados são necessários para armazenar:
...
<p>Voc&ecirc; precisa fornecer sua pr&oacute;pria infraestrutura DNS para oferecer suporte ao AD DS na Rede Virtual do Azure. A infraestrutura DNS fornecida pelo Azure para esta vers&atilde;o n&atilde;o d&aacute; suporte a alguns recursos exigidos pelo AD DS, como o registro de registros de recursos SRV. </p>
</div>

<div class="dev-callout"> 
<b>Observa&ccedil;&atilde;o</b>
<p>Se j&aacute; tiver conclu&iacute;do as etapas em <a href="/pt-br/manage/services/networking/active-directory-forest/">Instalar uma nova floresta do Active Directory no Azure</a>, voc&ecirc; poder&aacute; precisar remover o AD DS do controlador de dom&iacute;nio na Rede Virtual do Azure antes de come&ccedil;ar este tutorial. Para obter mais informa&ccedil;&otilde;es sobre como remover o AD DS, consulte <a href="http://technet.microsoft.com/pt-br/library/cc771844(v=WS.10).aspx">Removendo de um dom&iacute;nio o controlador de dom&iacute;nio</a>.</p>
</div>

## <span id="verifystaticip"></span></a>Etapa 1: verificar o endereço IP estático de YourPrimaryDC
...
## <span id="installforest"></span></a>Etapa 2: instalar a floresta Corp
...
## <span id="subnets"></span></a>Etapa 3: criar sub-redes e sites
...
7.  Clique com o botão direito do mouse em **Sites** e clique em **Nova Sub-Rede**.
...
## <span id="cloudsite"></span></a>Etapa 4: instalar um controlador de domínio adicional no CloudSite
...
## <span id="validate"></span></a>Etapa 5: validar a instalação
...
Para obter mais informações sobre como usar o Windows PowerShell, consulte [Introdução ao Azure PowerShell][Introdução ao Azure PowerShell] e [Cmdlets de gerenciamento do Azure (as páginas podem estar em inglês)][Cmdlets de gerenciamento do Azure (as páginas podem estar em inglês)].

## <span id="provisionvm"></span></a>Etapa 6: provisionar uma máquina virtual que seja associada ao domínio na inicialização
...
1.  Para criar uma máquina virtual adicional associada ao domínio durante a primeira inicialização, abra o ISE do PowerShell do Azure, cole o script a seguir, substitua os espaços reservados por seus próprios valores e execute-o.
    ...
    Após o provisionamento das máquinas virtuais, faça logon especificando uma conta de domínio usando o formato de nome UPN, como <administrator@corp.contoso.com>.
...
## <span id="backup"></span></a>Etapa 7: fazer backup do controlador de domínio
...
## <span id="test"></span></a>Etapa 8: Testar a autenticação e a autorização
...
-   [PowerShell do Azure][Introdução ao Azure PowerShell]

-   [Cmdlets gerenciamento do Azure][Cmdlets de gerenciamento do Azure (as páginas podem estar em inglês)]

  [Rede Virtual do Azure]: http://msdn.microsoft.com/pt-br/library/windowsazure/jj156007.aspx
  [Diretrizes para a implantação do Active Directory do Windows Server em máquinas virtuais do Azure]: http://msdn.microsoft.com/pt-br/library/windowsazure/jj156090.aspx
  ...
  [Adicionar uma máquina virtual a uma rede virtual (a página pode estar em inglês)]: http://azure.microsoft.com/pt-br/documentation/articles/virtual-networks-add-virtual-machine/
  [Criar uma delegação de zona]: http://technet.microsoft.com/library/cc753500.aspx
  [Instalar uma nova floresta do Active Directory no Azure]: /pt-br/manage/services/networking/active-directory-forest/
  [Removendo de um domínio o controlador de domínio]: http://technet.microsoft.com/pt-br/library/cc771844(v=WS.10).aspx
  ...
  [Introdução ao Azure PowerShell]: http://msdn.microsoft.com/pt-br/library/windowsazure/jj156055.aspx
  [Cmdlets de gerenciamento do Azure (as páginas podem estar em inglês)]: http://msdn.microsoft.com/pt-br/library/windowsazure/jj152841
