<properties
	pageTitle="Criar, configurar e implantar um aplicativo Web PHP no Azure"
...
# Criar, configurar e implantar um aplicativo Web PHP no Azure
...
## Criar um aplicativo Web do Azure e configurar a implantação do Git

>[AZURE.NOTE] "Espere! E se eu quiser implantar com o FTP?" Há um [tutorial sobre o FTP](web-sites-php-mysql-deploy-use-ftp.md) para suas necessidades.
...
4. Execute o comando para criar o aplicativo Web com a implantação no Git. Quando solicitado, especifique o número da região desejada.
...
4. Defina a versão do PHP que o aplicativo Laravel requer.

        azure site set --php-version 5.6

    Você terminou de definir a versão do PHP.
    
4. Gere uma nova `APP_KEY` para o aplicativo Web do Azure e defina-a como uma configuração de aplicativo para o aplicativo Web do Azure.

        php artisan key:generate --show
        azure site appsetting add APP_KEY="<output_of_php_artisan_key:generate_--show>"

4. Além disso, ative a depuração do Laravel para antecipar-se a qualquer página `Whoops, looks like something went wrong.` complicada.

        azure site appsetting add APP_DEBUG=true

    Você terminou de definir as variáveis de ambiente.
    
    >[AZURE.NOTE] Aguarde: vamos desacelerar um pouco e explicar o que o Laravel faz e o que o Azure faz aqui. O Laravel usa o arquivo `.env` no diretório raiz para fornecer variáveis de ambiente ao aplicativo, em que você encontrará a linha `APP_DEBUG=true` (e também `APP_KEY=...`). Essa variável é acessada no `config/app.php` pelo código `'debug' => env('APP_DEBUG', false),`. [env()](https://laravel.com/docs/5.2/helpers#method-env) é um método auxiliar do Laravel que usa o PHP [getenv()](http://php.net/manual/en/function.getenv.php) nos bastidores.
    >
...
4. No menu à esquerda, clique em **Serviços de Aplicativos** > **&lt;nome\_do\_aplicativo>** > **Ferramentas**.
    ...
    >[AZURE.TIP] Se clicar em **Configurações** em vez de **Ferramentas**, você poderá acessar a folha **Configurações de Aplicativo**, que permite definir versões do PHP, configurações de aplicativo e diretórios virtuais, como acabou de fazer.
    
4. Clique em **Extensões** > **Adicionar** para adicionar uma extensão.

4. Selecione **Composer** na **folha** [Escolher extensão](../azure-portal-overview.md) (*folha*: uma página do portal que é aberta horizontalmente).

4. Clique em **OK** na folha **Aceitar os termos legais**.

5. Clique em **OK** na folha **Adicionar extensão**.
...
4. De volta à folha do aplicativo Web, clique em **Configurações** > **Configurações do Aplicativo**.
...
4. Role até a parte inferior da folha e altere o diretório virtual raiz para apontar para **site\\wwwroot\\public** em vez de **site\\wwwroot**.
...
## Implantar o aplicativo Web com o Git (e definir variáveis de ambiente)

Você está pronto para implantar o código agora. Você fará isso no prompt de comando ou terminal.

4. Confirme todas as alterações e implante o código no aplicativo Web da maneira como faria em qualquer repositório Git:
...
5. Vamos ver isso em execução no navegador executando este comando:
...
<a name="clierror"></a>
### A CLI do Azure mostra "'site' não é um comando do azure"
...
<a name="http403"></a>
### O aplicativo Web mostra o erro HTTP 403
...
<a name="whoops"></a>
### O aplicativo Web mostra "Opa, parece que algo deu errado".
...
<a name="encryptor"></a>
### O aplicativo Web mostra "Nenhum criptografador com suporte encontrado".

Você implantou o aplicativo Web no Azure com êxito, mas, ao navegar para o aplicativo Web, recebe a mensagem de erro abaixo:

![APP\_KEY ausente no aplicativo PHP (Laravel) no Azure](./media/app-service-web-php-get-started/laravel-error-APP_KEY.png)
    
Esse é um erro desagradável, mas, pelo menos, não é indecifrável, pois você ativou a depuração do Laravel. Uma pesquisa superficial da cadeia de caracteres de erro nos fóruns do Laravel mostrará que isso ocorreu porque a APP\_KEY não foi definida em `.env` ou, no seu caso, `.env` não está presente no Azure. Você pode corrigir isso adicionando a configuração `APP_KEY` como uma configuração de aplicativo do Azure (confira [Configurar o aplicativo Web do Azure](#configure)).
