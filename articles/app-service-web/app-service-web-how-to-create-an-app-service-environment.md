<properties 
	...
	description="Descrição do fluxo de criação para ambientes de serviço de aplicativo" 
...
Os preços para ambientes de serviço de aplicativo dependem dos recursos de computação atribuídos. Você paga pelos recursos de computação alocados para seu ambiente de serviço de aplicativo independentemente de eles estarem hospedando cargas de trabalho ou não.



### Criação de VNET ###
Embora haja uma opção de criação rápida que cria automaticamente uma nova VNET, o recurso também oferece suporte à seleção de uma VNET existente e à criação manual de uma VNET. Você pode selecionar uma VNET existente se ela é grande o suficiente para dar suporte à implantação de um ambiente de serviço de aplicativo. A VNET deve ter 512 endereços ou mais. Se você selecionar uma VNET pré-existente, você também terá que especificar uma sub-rede para usar ou então criar uma nova. A sub-rede deve ter 256 endereços ou mais.
...
### Definição de tamanho de ambiente de serviço de aplicativo ###
...
Conforme observado anteriormente, em um ASE há 3 pools de trabalho que um cliente pode definir. O tamanho dos recursos de computação pode ser de P1 a P4. Por padrão existem somente 2 processadores P1 configurados no pool de trabalho 1. Isso é o suficiente para dar suporte a um plano de serviço de aplicativo único com 1 instância.

Os controles deslizantes se ajustam automaticamente para refletir a capacidade de computação total disponível no ambiente de serviço de aplicativo. Conforme os controles deslizantes são ajustados em qualquer um dos pools, os outros controles deslizantes mudam para refletir a quantidade de recursos de computação disponíveis restante antes de atingir 55.
...
### Após a criação de um Ambiente de Serviço de Aplicativo ###
...
Há dependências adicionais que não estão disponíveis para personalização, como o banco de dados e o armazenamento. Esses são gerenciados pelo Azure e fornecidos com o sistema. O armazenamento do sistema oferece suporte a até 500 GB para todo o ambiente de serviço de aplicativo.
