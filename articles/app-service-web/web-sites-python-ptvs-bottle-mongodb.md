<properties 
	...
	description="Aprenda a usar o Python Tools para Visual Studio para criar um aplicativo Bottle que armazena dados em uma instância do MongoDB e o implanta em Aplicativos Web do Serviço de Aplicativo do Azure." 
...
> **Observação:** o fluxo de trabalho do MongoLab não tem suporte no Portal de Visualização

Neste tutorial, usaremos o [Python Tools para Visual Studio] para criar um aplicativo Web de votação simples usando um dos modelos de exemplo de PTVS. Este tutorial também está disponível como um [vídeo](https://www.youtube.com/watch?v=8hQMyf8p_Jo).
...
Aprenderemos como usar um dos serviços MongoDB hospedados no Azure, como configurar o aplicativo para usar o MongoDB e como publicar o aplicativo Web nos [Aplicativos Web do Serviço de Aplicativo](http://go.microsoft.com/fwlink/?LinkId=529714).
...
1.  Clique em **MARKETPLACE**, em **MongoLab** e, em seguida, clique em **Avançar**.
...
1.  Escolha uma **Região** na qual localizar o serviço do banco de dados. Se você estiver usando o banco de dados de seu aplicativo do Azure, selecione a mesma região na qual você implantará seu aplicativo.

  	<!-- ![Personalize Add-on Dialog](./media/web-sites-python-ptvs-bottle-mongodb/PollsCommonMongoLabAddon2.png) -->

1.  Clique em **Avançar** e em **COMPRAR**.
...
1.  No [Portal de Gerenciamento do Azure], clique em **MARKETPLACE** e, em seguida, clique no serviço MongoLab que você criou anteriormente.
...
1.  No Gerenciador de Soluções, clique com o botão direito do mouse no projeto e selecione **Propriedades**. Clique na guia **Depurar**.
...
1.  Clique em `F5` para executar o aplicativo. As votações criadas com **Criar Votações de Exemplo** e os dados enviados por voto serão serializados no MongoDB.
...
1.  Crie uma nova conexão. Será necessário o **MONGOLAB_URI** recuperado na seção anterior.
    ...
    Observe o formato do URI: `mongodb://<name>:<password>@<address>:<port>/<name>`
...
1.  Na página de conexão, defina o **Nome** como qualquer nome que deseja usar para a conexão. Configure os campos **Endereço** e **Porta** de acordo com o *endereço* e a *porta* do **MONGOLAB_URI**.
...
1.  Na página de autenticação, configure o **Banco de dados** e **Nome de usuário** de acordo com o *nome* do **MONGOLAB_URI**. Também configure a **Senha** de acordo com a *senha* do **MONGOLAB_URI**.
...
## Publicar o aplicativo Web no Azure

O SDK .NET do Azure fornece uma forma fácil de implantar seu aplicativo Web no Azure.
...
2.  Clique em **Novo** para criar um novo aplicativo Web.
...
1.  Seu navegador da Web será aberto automaticamente para o aplicativo Web publicado. Se navegar até página Sobre, você verá que ela usa o repositório **Na memória**, não o repositório **MongoDB**.
...
1.  No [Portal de Gerenciamento do Azure], clique no aplicativo Web criado na seção anterior.

1.  No menu superior, clique em **CONFIGURAR**.

  	<!-- ![Top Menu](./media/web-sites-python-ptvs-bottle-mongodb/PollsCommonWebSiteTopMenu.png) -->

1.  Role para baixo até a seção de **configurações do aplicativo** e defina os valores para **REPOSITORY_NAME**, **MONGODB_HOST** e **MONGODB_DATABASE** conforme descrito na seção acima.
