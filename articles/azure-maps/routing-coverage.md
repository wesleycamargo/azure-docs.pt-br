---
title: Cobertura de roteamento nos Mapas do Azure | Microsoft Docs
description: Saiba mais sobre a cobertura de rota nos Mapas do Azure
author: jingjing-z
ms.author: jinzh
ms.date: 03/22/2019
ms.topic: conceptual
ms.service: azure-maps
services: azure-maps
manager: ''
ms.openlocfilehash: 9337a8fcc7aa097c94515bc2dc64d0f059be5e0a
ms.sourcegitcommit: 49c8204824c4f7b067cd35dbd0d44352f7e1f95e
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 03/22/2019
ms.locfileid: "58369455"
---
# <a name="azure-maps-routing-coverage"></a>Cobertura de rotas nos Mapas do Azure

Este artigo fornece informações de cobertura para rota nos Mapas do Azure. Quando você pesquisa a rota ideal do local A para B, mapas do Azure pode fornecer tempos de viagem altamente precisos, live instruções de informações e a rota de viagem atualizado. A rota levará em termos de pesquisa adicionais de conta, como tráfego atual, o tipo de veículo e condições para evitar. A capacidade de otimizar a rota depende da região, como mapas do Azure tem vários níveis de informações e a precisão para diferentes regiões. A tabela a seguir lista as regiões e que tipo de informação que você pode solicitar para eles.
 
Fazer check-out de cobertura para [ **Geocodificação**](geocoding-coverage.md).       
Fazer check-out de cobertura para [ **Tráfego**](traffic-coverage.md).  
Fazer check-out de cobertura para [ **Renderização**](render-coverage.md).
<br>  
**Legenda**

| Símbolo             | Significado                                |
|--------------------|----------------------------------------|
| ✓                  | País fornecido com dados detalhados.   |
| Ø                  | País fornecido com dados simplificados. |
| País/região faltando | Os dados do país/região não foram fornecidos.          |

<br>  

A tabela a seguir fornece informações de cobertura para rota nos Mapas do Azure.
<br>  


## <a name="africa"></a>África

| País/Região                 | Roteamento         | Roteamento de tráfego | Roteamento de caminhão |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Argélia                        |        ✓       |                       |             |
| Angola                         |        ✓       |                       |             |
| Benin                          |        ✓       |                       |             |
| Botsuana                       |        ✓       |                       |             |
| Burkina Faso                   |        ✓       |                       |             |
| Burundi                        |        ✓       |                       |             |
| Camarões                       |        ✓       |                       |             |
| Cabo Verde                     |        ✓       |                       |             |
| Congo - Brazzaville Kinshasa    |        ✓       |                       |             |
| Congo - Kinshasa                |        ✓       |                       |             |
| Côte d'Ivoire                  |        ✓       |                       |             |
| Egito                          |        ✓       |         ✓            |             |
| Gabão                          |        ✓       |                       |             |
| Gâmbia                         |        ✓       |                       |             |
| Gana                          |        ✓       |                       |             |
| Quênia                          |        ✓       |         ✓            |             |
| Lesoto                        |        Ø        |         ✓            |             |
| Malaui                         |        ✓       |                       |              |
| Mali                           |        ✓       |                       |             |
| Mauritânia                     |        ✓       |                       |             |
| Maurício, Mayotte e Réunion |        ✓      |                       |             |
| Marrocos                        |        ✓       |                       |             |
| Moçambique                     |        ✓       |         ✓            |             |
| Namíbia                        |        ✓       |                       |             |
| Níger                          |        ✓       |                       |             |
| Nigéria                        |        ✓       |         ✓            |             |
| Ruanda                         |        ✓       |                       |             |
| Senegal                        |        ✓       |                       |             |
| Seychelles                     |        ✓       |                       |             |
| África do Sul                   |        ✓       ||         ✓            |             |
| Suazilândia                      |        ✓       |                       |             |
| Tanzânia                       |        ✓       |                       |             |
| Togo                           |        ✓       |                       |             |
| Tunísia                        |        ✓       |                       |             |
| Uganda                         |        ✓       |                       |             |
| Saara Ocidental                 |        ✓       |                       |             |
| Zâmbia                         |        ✓       |                       |             |
| Zimbábue                       |        ✓       |                       |             |


## <a name="americas"></a>Américas

| País/Região                 | Roteamento         | Roteamento de tráfego | Roteamento de caminhão |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Argentina                      |        ✓       |         ✓            |     ✓       |
| Antígua e Barbuda            |        ✓       |                      |             |
| Bahamas                        |        ✓       |                      |             |
| Barbados                       |        ✓       |                      |             |
| Belize                         |        ✓       |                      |             |
| Brasil                         |        ✓       |         ✓            |     ✓       | 
| Canadá                         |        ✓       |         ✓            |     ✓      |
| Chile                          |        ✓       |         ✓            |     ✓      |
| Colômbia                       |        ✓       |         ✓            |             |
| Costa Rica                     |        ✓       |                      |             |
| Cuba                           |        ✓       |                      |             |
| Dominica                       |        ✓       |                      |             |
| República Dominicana             |        ✓       |                      |             |
| Equador                        |        ✓       |                      |             |
| El Salvador                    |        ✓       |                      |             |
| Guiana Francesa                  |        ✓       |                      |             |
| Granada                        |        ✓       |                      |             |
| Guatemala                      |        ✓       |                      |             |
| Guiana                         |        ✓       |                      |             |
| Haiti                          |        ✓       |                      |             |
| Honduras                       |        ✓       |                      |             |
| Jamaica                        |        ✓       |                      |             |
| México                         |        ✓       |         ✓            |     ✓      | 
| Nicarágua                      |        ✓       |                      |             |
| Panamá                         |        ✓       |                      |             |
| Paraguai                       |        ✓       |                      |             |
| Peru                           |        ✓       |         ✓            |             |
| São Cristóvão e Névis            |        ✓       |                      |             |
| Santa Lúcia                      |        ✓       |                      |             |
| São Vicente e Granadinas       |        ✓       |                      |             |
| Suriname                       |        ✓       |                      |             |
| Trinidad e Tobago              |        ✓       |                      |             |
| Estados Unidos                  |        ✓       |         ✓            |     ✓      | 
| + American Samoa                |        ✓       |                      |             |
| + Os Ilhas Marianas do Norte      |        ✓       |                      |             |
| +Porto Rico                   |        ✓       |                      |             |
| + DOS EUA Ilhas Virgens           |        ✓       |                      |             |
| Uruguai                        |        ✓       |         ✓            |     ✓      | 
| Venezuela                      |        ✓       |                      |             |


## <a name="asia-pacific"></a>Pacífico Asiático

| País/Região                 | Roteamento         | Roteamento de tráfego | Roteamento de caminhão |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Austrália                      |        ✓       |         ✓            |     ✓       |
| + Ilha de Natal              |        ✓       |                      |             |
| Brunei                         |        ✓       |         ✓            |             |
| Camboja                       |        ✓       |                      |             |
| Fiji                           |        ✓       |                      |             |
| Guam                           |        ✓       |                      |             |
| RAE de Hong Kong                      |        ✓       |         ✓            |             |
| Índia                          |        ✓       |         ✓            |             |
| Indonésia                      |        ✓       |         ✓            |             |
| Coreia do Sul                          |        Ø       |                      |             |
| Laos                           |        ✓       |                      |             |
| Macau                          |        ✓       |         ✓            |             |
| Malásia                       |        ✓       |         ✓            |             |
| Myanmar                        |        ✓       |                      |             |
| Nova Zelândia                    |        ✓       |         ✓            |     ✓       | 
| Filipinas                    |        ✓       |         ✓            |             |
| Singapura                      |        ✓       |         ✓            |             |
| Taiwan                         |        ✓       |         ✓            |             |
| Tailândia                       |        ✓       |         ✓            |             |
| Vietnã                        |        ✓       |         ✓            |             |


## <a name="europe"></a>Europa

| País/Região                 | Roteamento         | Roteamento de tráfego | Roteamento de caminhão |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Albânia                        |        ✓       |                      |             |
| Andorra                        |        ✓       |         ✓            |             |
| Áustria                        |        ✓       |         ✓            |     ✓       | 
| Belarus                        |        ✓       |                      |             |
| Bélgica                        |        ✓       |         ✓            |     ✓      | 
| Bolívia                        |        ✓       |                      |             |
| Bósnia e Herzegovina             |        ✓       |                      |             |
| Bulgária                       |        ✓       |         ✓            |     ✓      | 
| Croácia                        |        ✓       |         ✓            |     ✓      |
| Chipre                         |        ✓       |                      |             |
| República Tcheca                 |        ✓       |         ✓            |     ✓      |
| Dinamarca                        |        ✓       |         ✓            |     ✓      |
| + Ilhas de Faroés                 |        ✓       |                      |             |
| Estônia                        |        ✓       |         ✓            |     ✓      |
| Finlândia                        |        ✓       |         ✓            |     ✓      |
| França                         |        ✓       |         ✓            |     ✓      | 
| + Guadalupe                    |        ✓       |                      |             |
| + Martinica                    |        ✓       |                      |             |
| + São Bartolomeu                |        ✓       |                      |             |
| + St. Martin                    |        ✓       |                      |             |
| + São Pedro e Miquelon         |        ✓       |                      |             |
| Geórgia                        |        ✓       |                      |             |
| Alemanha                        |        ✓       |         ✓            |     ✓      | 
| Grécia                         |        ✓       |         ✓            |     ✓      |  
| Guernesey                       |        ✓       |                      |             |
| Hungria                        |        ✓       |         ✓            |     ✓      | 
| Islândia                        |        ✓       |         ✓            |             |
| Irlanda                        |        ✓       |         ✓            |     ✓      | 
| Itália                          |        ✓       |         ✓            |     ✓      | 
| Jersey                         |        ✓       |                      |             |
| Letônia                         |        ✓       |         ✓            |     ✓      | 
| Liechtenstein                  |        ✓       |         ✓            |             |
| Lituânia                      |        ✓       |         ✓            |     ✓      | 
| Luxemburgo                     |        ✓       |         ✓            |     ✓      | 
| Macedônia                      |        ✓       |                      |             |
| Malta                          |        ✓       |         ✓            |             |
| Moldova                        |        ✓       |                      |             |
| Mônaco                         |        ✓       |         ✓            |             |
| Montenegro                     |        ✓       |                      |     ✓      | 
| Países Baixos                    |        ✓       |         ✓            |     ✓      | 
| +Aruba                         |        ✓       |                      |             | 
| + Países baixos Caribe         |        ✓       |                      |             |
| +Curaçao                       |        ✓       |                      |             |
| + Sint Maarten                  |        ✓       |                      |             |
| Noruega                         |        ✓       |         ✓            |     ✓      | 
| Polônia                         |        ✓       |         ✓            |     ✓      | 
| Portugal                       |        ✓       |         ✓            |     ✓      |  
| Romênia                        |        ✓       |         ✓            |     ✓      | 
| Federação Russa             |        ✓       |         ✓            |     ✓      | 
| San Marino                     |        ✓       |         ✓            |             |
| + Sint Maarten                  |        ✓       |                      |             |
| Sérvia                         |        ✓       |                      |     ✓      | 
| Eslováquia                       |        ✓       |         ✓            |     ✓      | 
| Eslovênia                       |        ✓       |         ✓            |     ✓      |
| Espanha                          |        ✓       |         ✓            |     ✓      | 
| Suécia                         |        ✓       |         ✓            |     ✓      | 
| Suíça                    |        ✓       |         ✓            |     ✓      | 
| Turquia                         |        ✓       |         ✓            |     ✓      | 
| Ucrânia                        |        ✓       |         ✓            |             | 
| Reino Unido                 |        ✓       |         ✓            |     ✓      | 
| +Anguilla                      |        ✓       |                      |             |
| + Bermuda                       |        ✓       |                      |             |
| + Ilhas Virgens Britânicas.        |        ✓       |                      |             |
| + Ilhas de Cayman                |        ✓       |                      |             |
| +Gibraltar                      |        ✓       |         ✓            |             | 
| +Montserrat                    |        ✓       |                      |             |
| + Ilhas Turcos e Caicos        |        ✓       |                      |             |
| Cidade do Vaticano                   |        ✓       |         ✓            |             |
| Ilha de Man                    |        ✓       |                      |             |


## <a name="middle-east"></a>Oriente Médio

| País/Região                 | Roteamento         | Roteamento de tráfego | Roteamento de caminhão |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Bahrein                        |        ✓       |         ✓            |             |
| Iraque                           |        ✓       |                      |             |
| Israel                         |                |         ✓            |             |
| Jordânia                         |        ✓       |                      |             |
| Kuwait                         |        ✓       |         ✓            |             |
| Líbano                        |        ✓       |                      |             |
| Omã                           |        ✓       |         ✓            |             |
| Catar                          |        ✓       |         ✓            |             |
| Arábia Saudita                   |        ✓       |         ✓            |             |
| Emirados Árabes Unidos           |        ✓       |         ✓            |             |
| Iêmen                          |        ✓       |                      |             |


## <a name="next-steps"></a>Próximas etapas

Para obter mais informações sobre o roteamento de mapas do Azure, consulte a [roteamento](https://docs.microsoft.com/rest/api/maps/route) as páginas de referência.