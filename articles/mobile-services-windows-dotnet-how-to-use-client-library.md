...
Este guia mostra como executar cenários comuns usando um cliente .NET para os Serviços Móveis do Azure, em aplicativos da Windows Store e em aplicativos do Windows Phone. Os cenários abrangidos incluem consultas de dados, inserção, atualização e exclusão de dados, autenticação de usuários e tratamento de erros. Se você for novo nos Serviços Móveis, deverá considerar primeiro concluir o tutorial "Início rápido dos Serviços Móveis " ([Tutorial de início rápido da Windows Store][Tutorial de início rápido da Windows Store]/[Tutorial de início rápido do Windows Phone][Tutorial de início rápido do Windows Phone]) e o tutorial "Introdução aos dados no .NET" ([Tutorial de dados da Windows Store][Tutorial de dados da Windows Store]/[Tutorial de dados do Windows Phone][Tutorial de dados do Windows Phone]). O tutorial de início rápido requer o[SDK dos Serviços Móveis][SDK dos Serviços Móveis] e ajuda a configurar sua conta e a criar seu primeiro serviço móvel.
...
-   [Como: Chamar uma API personalizada][Como: Chamar uma API personalizada]
-   [Como: Usar Simultaneidade Otimista][Como: Usar Simultaneidade Otimista]
-   [Como: Vincular dados à interface do usuário em um serviço móvel][Como: Vincular dados à interface do usuário em um serviço móvel]
...
Você pode exibir o URI da solicitação enviado ao serviço móvel usando software de inspeção de mensagem, como as ferramentas de desenvolvedor do navegador ou o [Fiddler][Fiddler]. Se você examinar o URI abaixo, observe que estamos modificando a própria cadeia de caracteres da consulta:
...
Por padrão, o servidor retorna apenas as primeiras 50 linhas. Você pode aumentar o número de linhas retornadas chamando o método [Take][Take]. Use `Take` juntamente com o método [Skip][Skip] para solicitar uma "página" específica do conjunto de dados total retornado pela consulta. A consulta a seguir, quando executada, retorna os três itens principais na tabela.
...
<div class="dev-callout"><strong>Observa&ccedil;&atilde;o</strong> <p>Se voc&ecirc; desejar executar opera&ccedil;&otilde;es de inser&ccedil;&atilde;o, pesquisa, exclus&atilde;o ou atualiza&ccedil;&atilde;o em um tipo, ser&aacute; necess&aacute;rio criar um membro chamado <strong>Id</strong>. &Eacute; por isso que a classe de exemplo <strong>TodoItem</strong> tem um membro de nome <strong>Id</strong>. Um valor de id v&aacute;lido deve sempre estar presente em opera&ccedil;&otilde;es de atualiza&ccedil;&atilde;o e exclus&atilde;o.</p> </div>
...
Para obter um exemplo mais completo de como usar a simultaneidade otimista nos Serviços Móveis, consulte o [Tutorial de simultaneidade otimista][Tutorial de simultaneidade otimista].

## <a name="binding"></a><span class="short-header">Exibindo dados</span>Como: Vincular dados à interface do usuário em um serviço móvel
...
Alguns controles no Tempo de Execução do Windows oferecem suporte a uma interface chamada [ISupportIncrementalLoading (a página pode estar em inglês)][ISupportIncrementalLoading (a página pode estar em inglês)]. Essa interface permite que os controles solicitem dados adicionais quando o usuário rola. Há suporte interno para essa interface para aplicativos da Windows Store via `MobileServiceIncrementalLoadingCollection`, que manipula automaticamente as chamadas dos controles. Para usar `MobileServiceIncrementalLoadingCollection` em aplicativos da Windows Store, proceda da seguinte maneira:
...
<p>Ao usar o provedor de logon da Conta da Microsoft para autenticar usu&aacute;rios de seu aplicativo da Windows Store, voc&ecirc; tamb&eacute;m deve registrar o pacote de aplicativos nos Servi&ccedil;os M&oacute;veis. Ao registrar as informa&ccedil;&otilde;es do pacote de aplicativos da Windows Store com Servi&ccedil;os M&oacute;veis, o cliente &eacute; capaz de reutilizar as credenciais de login da conta da Microsoft para obter uma experi&ecirc;ncia de logon &uacute;nica. Se voc&ecirc; n&atilde;o fizer isso, os usu&aacute;rios de login da conta da Microsoft ser&atilde;o apresentados com uma solicita&ccedil;&atilde;o de logon toda vez que o m&eacute;todo de logon for chamado. Para saber como registrar seu pacote de aplicativos da Windows Store, consulte <a href="/pt-br/develop/mobile/how-to-guides/register-windows-store-app-package/" target="_blank">Registrar seu pacote de aplicativos da Windows Store para a autentica&ccedil;&atilde;o da Microsoft</a>. Depois que as informa&ccedil;&otilde;es do pacote estiverem registradas nos Servi&ccedil;os M&oacute;veis, chame o m&eacute;todo <a href="http://go.microsoft.com/fwlink/p/?LinkId=311594" target="_blank">LoginAsync</a> fornecendo um valor de <strong>verdadeiro</strong> para o par&acirc;metro <em>useSingleSignOn</em> para reutilizar as credenciais.</p>
</div>
...
Para obter um exemplo de como usar a Conta da Microsoft para fornecer uma experiência de logon único, consulte "Autenticar seu aplicativo com logon único ([Windows Store][1]/[Windows Phone][2].

### <a name="caching"></a>Armazenando o token de autenticação em cache

Em alguns casos, a chamada para o método login pode ser evitada após a primeira vez que o usuário é autenticado. Você pode usar o [PasswordVault][PasswordVault] para aplicativos da Windows Store para armazenar a identidade do usuário atual em cache na primeira vez que fizer logon e em todas as vezes subsequentes em que você verificar se já tem a identidade do usuário em nosso cache. Quando o cache estiver vazio, ainda será necessário enviar o usuário pelo processo de logon.
...
Para aplicativos do Windows Phone, você pode criptografar e armazenar os dados em cache usando a classe [ProtectedData][ProtectedData] e armazenar informações confidenciais em um armazenamento isolado.
...
A classe [MobileServiceClient][MobileServiceClient] expõe uma propriedade `SerializerSettings` do tipo [JsonSerializerSettings][JsonSerializerSettings]
...
<!-- URLs. -->
  ...
  [MobileServiceClient]: http://msdn.microsoft.com/pt-br/library/microsoft.windowsazure.mobileservices.mobileserviceclient.aspx
  ...
  [Começar a trabalhar com dados]: /pt-br/develop/mobile/tutorials/get-started-with-data-dotnet/
