<properties linkid="obile-services-how-to-dotnet-client" urlDisplayName="Biblioteca de Cliente .NET" pageTitle="Trabalhando com a Biblioteca de Cliente .NET dos Serviços Móveis" metaKeywords="Serviços Móveis do Azure, cliente .NET dos Serviços Móveis, cliente .NET" description="Saiba como usar um cliente .NET para os Serviços Móveis do Azure." metaCanonical="" services="" documentationCenter="Mobile" title="Como usar um cliente .NET para os Serviços Móveis do Azure" authors=""  solutions="" writer="krisragh" manager="" editor=""  />
...
- [Como personalizar o cliente]
	- [Personalizar cabeçalhos de solicitações]
...
Quando o esquema dinâmico está habilitado, os Serviços Móveis do Azure geram automaticamente novas colunas com base no objeto nas solicitações de inserção ou de atualização. Para obter mais informações, consulte [Esquema dinâmico](http://go.microsoft.com/fwlink/?LinkId=296271).
...
No código acima, substitua `AppUrl` e `AppKey` pela URL e a chave do aplicativo do serviço móvel, nessa ordem. Ambas estão disponíveis no Portal de Gerenciamento do Azure selecionando o serviço móvel e, em seguida, clicando em "Painel".
...
A cláusula `where` oferece suporte a operações que são traduzidas para o subconjunto OData dos Serviços Móveis. Isso inclui operadores relacionais (==, !=, <, <=, >, >=), operadores aritméticos (+, -, /, *, %), precisão de número (Math.Floor, Math.Ceiling), funções de cadeias de caracteres (Length, Substring, Replace, IndexOf, StartsWith, EndsWith), propriedades de data (Year, Month, Day, Hour, Minute, Second), propriedades de acesso de um objeto e expressões que combinam todos eles.

### <a name="sorting"></a>Como classificar dados retornados

O código a seguir ilustra como classificar dados incluindo uma função `OrderBy` ou `OrderByDescending` na consulta. Ele retorna itens da `todoTable` classificada em ordem crescente pelo campo `Text`. 
...
<div class="dev-callout"><strong>Observação</strong> <p>se você desejar executar operações de inserção, pesquisa, exclusão ou atualização em um tipo, será necessário criar um membro chamado <strong>Id</strong>. É por isso que a classe de exemplo <strong>TodoItem</strong> tem um membro de nome <strong>Id</strong>. Um valor de id válido deve sempre estar presente em operações de atualização e exclusão.</p> </div>
...
<h2><a name="optimisticconcurrency"></a><span class="short-header">Simultaneidade otimista</span>Como usar simultaneidade otimista</h2>
...
Aplicativos que usam tabelas não tipadas habilitam a simultaneidade otimista definindo o sinalizador `Version` em `SystemProperties` da tabela da seguinte maneira.

	//Enable optimistic concurrency by retrieving __version 
	todoTable.SystemProperties |= MobileServiceSystemProperties.Version;


O código a seguir mostra como resolver um conflito de gravação quando detectado. O valor de `__version` correto deve ser incluído na chamada de `UpdateAsync()` para confirmar uma resolução.
...
Para usar a nova coleção no Windows Phone, use os métodos de extensão `ToCollection` em `IMobileServiceTableQuery<T>` e `IMobileServiceTable<T>`. Para realmente carregar dados, chame `LoadMoreItemsAsync()`. 

	MobileServiceCollection<TodoItem, TodoItem> items = todoTable.Where(todoItem => todoItem.Complete==false).ToCollection(); 
	await items.LoadMoreItemsAsync();         

Quando usa a coleção criada chamando `ToCollectionAsync` ou `ToCollection`, você obtém uma coleção que pode ser vinculada a controles da interface do usuário. Essa coleção tem reconhecimento de paginação, ou seja, um controle pode pedir que a coleção "carregue mais itens", e a coleção fará isso para o controle. Nesse momento não há nenhum código de usuário envolvido, o controle iniciará o fluxo. No entanto, como a coleção está carregando dados da rede, é esperado que haja falhas nesse carregamento algumas vezes. Para tratar essas falhas, é possível substituir o método `OnException` na `MobileServiceIncrementalLoadingCollection` para tratar as exceções resultantes de chamadas para `LoadMoreItemsAsync` executadas pelos controles. 
...
Depois de registrar seu provedor de identidade, basta chamar o [método LoginAsync] com o valor [MobileServiceAuthenticationProvider] de seu provedor. Por exemplo, o código a seguir inicia um logon de fluxo de servidor usando o Facebook. 
...
Se você estiver usando um provedor de identidade que não seja o Facebook, altere o valor de [MobileServiceAuthenticationProvider] acima para o valor do seu provedor.
...
<p>Ao usar o provedor de logon da Conta da Microsoft para autenticar usuários de seu aplicativo da Windows Store, você também deve registrar o pacote de aplicativos nos Serviços Móveis. Ao registrar as informações do pacote de aplicativos da Windows Store com Serviços Móveis, o cliente é capaz de reutilizar as credenciais de login da conta da Microsoft para obter uma experiência de logon única. Se você não fizer isso, os usuários de login da conta da Microsoft serão apresentados com uma solicitação de logon toda vez que o método de logon for chamado. Para saber como registrar seu pacote de aplicativos da Windows Store, consulte <a href="/pt-br/develop/mobile/how-to-guides/register-windows-store-app-package/" target="_blank">Registrar seu pacote de aplicativos da Windows Store para a autenticação da Microsoft</a>. Depois que as informações do pacote estiverem registradas nos Serviços Móveis, chame o método <a href="http://go.microsoft.com/fwlink/p/?LinkId=311594" target="_blank">LoginAsync</a> fornecendo um valor de <strong>true</strong> para o parâmetro <em>useSingleSignOn</em> para reutilizar as credenciais.</p>
</div>
...
Em alguns casos, a chamada para o método login pode ser evitada após a primeira vez que o usuário é autenticado. Você pode usar [PasswordVault] para aplicativos da Windows Store para armazenar a identidade do usuário atual em cache na primeira vez que ele fizer logon e em todas as vezes subsequentes que verificar se você já tem a identidade do usuário em seu cache. Quando o cache estiver vazio, ainda será necessário enviar o usuário pelo processo de logon. 
...
O valor retornado por `MobileServiceClient.GetTable` e as consultas são interfaces. Isso os torna "mockable" facilmente para fins de teste, portanto, você pode criar um `MyMockTable : IMobileServiceTable<TodoItem>` que implementa a lógica do teste.
...
Você pode desejar anexar um cabeçalho personalizado a cada solicitação de saída ou alterar os códigos de status de respostas. Você pode fazer isso configurando um DelegatingHandler, como abaixo:
...
A classe [MobileServiceClient](http://msdn.microsoft.com/pt-br/library/microsoft.windowsazure.mobileservices.mobileserviceclient.aspx) expõe uma propriedade `SerializerSettings` do tipo [JsonSerializerSettings](http://james.newtonking.com/projects/json/help/?topic=html/T_Newtonsoft_Json_JsonSerializerSettings.htm)

Usando essa propriedade, você pode definir propriedades de Json.NET (há muitas), incluindo uma, por exemplo, para converter todas as propriedades em letras minúsculas:
...
[Tutorial de dados do Windows Phone]: http://www.windowsazure.com/pt-br/develop/mobile/tutorials/get-started-with-data-wp8/
[Autenticação da Windows Store]: http://www.windowsazure.com/pt-br/develop/mobile/tutorials/get-started-with-users-dotnet/
[Autenticação do Windows Phone]: http://www.windowsazure.com/pt-br/develop/mobile/tutorials/get-started-with-users-wp8/
[PasswordVault]: http://msdn.microsoft.com/pt-br/library/windows/apps/windows.security.credentials.passwordvault.aspx
[SDK dos Serviços Móveis]: http://go.microsoft.com/fwlink/?LinkId=257545
[ProtectedData]: http://msdn.microsoft.com/pt-br/library/system.security.cryptography.protecteddata%28VS.95%29.aspx
...
[IncludeTotalCount]: http://msdn.microsoft.com/pt-br/library/windowsazure/dn250560.aspx 
[Skip]: http://msdn.microsoft.com/pt-br/library/windowsazure/dn250573.aspx
[Take]: http://msdn.microsoft.com/pt-br/library/windowsazure/dn250574.aspx 
