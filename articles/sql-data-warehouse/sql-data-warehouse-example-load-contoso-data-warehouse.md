<properties
   pageTitle="Carregar o Contoso Retail Data Warehouse no SQL Data Warehouse | Microsoft Azure"
   description="Saiba como carregar o Contoso Retail Data Warehouse no SQL Data Warehouse usando o PolyBase para obter o desempenho máximo"
...
# Carregando o Contoso Retail Data Warehouse

Neste artigo, você aprenderá a carregar dados rapidamente no esquema do Contoso Retail Data Warehouse.

Esse conjunto de dados fornece uma fonte de dados alternativa que você pode usar para aprender tudo sobre o Azure SQL Data Warehouse.

O conjunto de dados da contoso tem aproximadamente GB. Os dados brutos são mantidos em um armazenamento de blobs do Azure, que acessaremos diretamente para carregar no SQL Data Warehouse.

Neste tutorial, você irá:

1. Configurar o PolyBase para carregar por meio de um contêiner de armazenamento de blobs público
2. Importar os dados usando [CTAS][]
3. [RECOMPILAR][] as tabelas para garantir que os índices columnstore sejam otimizados
4. Criar estatísticas em colunas de chave

## Configurando o PolyBase

A primeira etapa ao carregar dados é configurar os scripts de pré-requisito do PolyBase:

### Criar a fonte de dados externa

Como os dados são mantidos em um contêiner público, a primeira tarefa é criar a fonte de dados externa:
...
Neste caso, não é necessário criar uma chave mestra ou uma credencial com escopo de banco de dados porque o contêiner de armazenamento de blobs está configurado como "público", o que significa que qualquer pessoa pode ler e enumerar esse contêiner.

> [AZURE.IMPORTANT] Se você optar por tornar seus contêineres do armazenamento de blobs do Azure públicos, tenha em mente que você será responsável por encargos de saída de dados quando os dados deixarem o data center.

### Criar um objeto de formato de arquivo externo

O formato de arquivo fornece as informações necessárias para analisar arquivos de texto mantidos no armazenamento de blobs.
...
Em nosso caso, os dados são descompactados e delimitados por barra vertical.

## Criar as tabelas externas

Agora que a conectividade com o armazenamento de blobs do Azure está configurada, podemos criar as tabelas externas.

Para manter tudo organizado, primeiro vamos criar um esquema:
...
Agora, vamos criar as tabelas externas. Tenha em mente que tudo o que estamos fazendo aqui é fornecer o mapeamento dos tipos de dados, ou seja, estamos associando essa definição aos metadados para localizar os arquivos.

Esteja avisado de que este é um script longo!
...
## Carregar os dados com CTAS

Para carregar os dados, convém aproveitar as tabelas externas fortemente tipadas que acabamos de criar. Portanto, instruções [CTAS][] simples serão suficientes.

No entanto, primeiro vamos criar um novo esquema para carregar nossas tabelas.
...
Para carregar os dados, usamos a instrução [CTAS][]. Observe que há um [CTAS][] por tabela.
...
Você pode acompanhar o progresso do carregamento usando a DMV (exibição de gerenciamento dinâmico) `[sys].[dm_pdw_exec_requests]`. Como as instruções CTAS acima têm um `label`, você também pode acompanhar o progresso usando o `label`. Para obter mais detalhes, consulte o artigo sobre [rótulos][].

## Recompilar os índices

Por fim, para garantir que as tabelas estejam em um formato columnstore compactado, podemos executar uma operação de recompilação na tabela. Isso converte quaisquer grupos de linhas abertos ou fechados no formato columnstore compactado.
...
Para obter mais informações sobre como verificar o estado do seu columnstore clusterizado, consulte o artigo [gerenciar índices columnstore][].

## Criar alguns objetos de estatística

Para criar estatísticas em todas as colunas, você pode aproveitar o código de procedimento armazenado `prc_sqldw_create_stats` no artigo sobre [estatística][].

Abaixo, temos um subconjunto da saída que abrange as dimensões por completo e todas as colunas de junção no Contoso Retail Data Warehouse. Este é um bom ponto de partida.

Você pode adicionar mais estatísticas a outras colunas de fatos posteriormente.

Mais uma vez, o script é bem longo.
...
Agora, você tem todos os dados carregados no Azure SQL Data Warehouse. Bom trabalho!

Agora você pode começar a consultar as tabelas usando consultas como a mostrada abaixo:
...
## Próximas etapas
Para obter uma visão geral do carregamento, consulte [Carregar dados no SQL Data Warehouse][]. Para saber mais sobre como trabalhar com índices columnstore, consulte [gerenciar índices columnstore][].
...
[estatística]: ./sql-data-warehouse-develop-statistics.md
[CTAS]: ./sql-data-warehouse-develop-ctas.md
[rótulos]: ./sql-data-warehouse-develop-label.md

<!--MSDN references-->
[sys.dm_pdw_exec_requests]: https://msdn.microsoft.com/pt-BR/library/mt203887.aspx
[RECOMPILAR]: https://msdn.microsoft.com/pt-BR/library/ms188388.aspx
