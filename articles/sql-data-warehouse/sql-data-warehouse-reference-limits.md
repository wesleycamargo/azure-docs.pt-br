<properties
   pageTitle="Especificações de capacidade do SQL Data Warehouse | Microsoft Azure"
   description="Especificações de capacidade mínima e máxima do SQL Data Warehouse."
...
# Especificações de capacidade do SQL Data Warehouse

O SQL Data Warehouse tem limites para o processamento de consultas e exibições do sistema que são muito semelhantes aos do SQL Server. Alguns limites são diferentes devido à arquitetura paralela e escalonável subjacente. Esse é um resumo dos limites do SQL Data Warehouse.
...
| Logons | Logons por servidor. | 500\.000 |
...
| Tabela | Bytes por linha, tamanho do buffer interno para a movimentação de dados | 32\.768<br/><br/>O SQL Data Warehouse usa um buffer interno para mover as linhas dentro do sistema SQL Data Warehouse distribuído. O serviço que move linhas é chamado de DMS (Serviço de Movimentação de Dados) e armazena linhas em um formato diferente do SQL Server.<br/><br/>Se uma linha não couber no buffer interno, você obterá um erro de compilação de consulta ou um erro de movimentação de dados internos. Para evitar esse problema, confira [Detalhes sobre o tamanho de buffer do DMS](#details-about-the-dms-buffer-size).|
| Tabela | Partições por tabela | 15\.000<br/><br/>Para alto desempenho, recomendamos minimizar o número de partições necessárias e, ao mesmo tempo, dar suporte a seus requisitos de negócios. À medida que o número de partições vai aumentando, a sobrecarga de operações de DDL e de DML também aumenta e faz com que o desempenho fique mais lento.|
...
Para melhorar o desempenho de consulta paralela, o DMS preenche todos os dados de comprimento variável com o tamanho máximo definido pelo banco de dados SQL. Por exemplo, o valor "hello" para um `nvarchar(2000) NOT NULL` usará 4002 bytes no buffer do DMS. Ele usa dois bytes para cada um dos 2000 caracteres, mais dois bytes para o terminador NULL.
...
;
````
A próxima etapa mostra que podemos usar INSERT para inserir dados na tabela com êxito. Essa instrução carrega os dados diretamente para o SQL Server sem usar DMS e, portanto, não causa uma falha de excedente de buffer DMS. Os serviços de integração também carregam essa linha com êxito.</para>
