<properties title="A extens&atilde;o de m&aacute;quina virtual Docker para Linux no Azure" pageTitle="A extens&atilde;o de m&aacute;quina virtual Docker para Linux no Azure" description="Descreve o Docker, os cont&ecirc;ineres e as extens&otilde;es de m&aacute;quina virtual do Azure, al&eacute;m de indicar mais recursos para criar cont&ecirc;ineres do Docker por meio da xplat-cli e do portal." metaKeywords="linux, virtual machines, vm, azure, docker, linux containers,  lxc, virtualization" services="virtual-machines" solutions="dev-test" documentationCenter="virtual-machines" authors="rasquill" videoId="" scriptId="" manager="timlt" />
...
-   [Como usar a extensão de máquina virtual Docker na interface de plataforma cruzada (xplat-cli) do Azure][Como usar a extensão de máquina virtual Docker na interface de plataforma cruzada (xplat-cli) do Azure]
-   [Como usar a extensão de máquina virtual Docker com o Portal do Azure][Como usar a extensão de máquina virtual Docker com o Portal do Azure]

## <span id="Docker and Linux Containers"></span>Contêineres Docker e Linux</a>

O [Docker][Docker] é uma das abordagens de virtualização mais populares que usa [contêineres Linux][contêineres Linux] em vez de máquinas virtuais como uma forma de isolar dados e computar recursos compartilhados, e oferece outros serviços que lhe permitem criar ou compilar aplicativos rapidamente e distribuí-los entre outros contêineres Docker.

Contêineres Docker e Linux não são [Hipervisores][Hipervisores] como o Windows Hyper-V e o [KVM][KVM] no Linux (há muitos outros exemplos). Hipervisores virtualizam o sistema operacional subjacente para permitir que sistemas operacionais completos sejam executados dentro do hipervisor como se fossem um aplicativo.
...
| Tempo necessário para iniciar  | Consideravelmente mais tempo: inicialização do SO e carregamento dos aplicativos | Consideravelmente menos tempo: é necessário iniciar apenas os aplicativos, pois o kernel já está em execução |
| Automação do contêiner         | Varia muito de acordo com o SO e os aplicativos                                  | [Galeria de imagens do Docker][Galeria de imagens do Docker]; outros                                                                     |

Para participar de um debate de alto nível sobre contêineres e suas vantagens, confira o [Quadro de comunicações de alto nível do Docker][Quadro de comunicações de alto nível do Docker].

Para obter mais informações sobre o Docker e seu funcionamento, confira [O que é o Docker?][O que é o Docker?]

#### Práticas recomendadas para a segurança do contêiner do Linux e do Docker

Como os contêineres compartilham o acesso ao kernel do computador hóspede, se códigos mal-intencionados conseguirem acessar a raiz, eles poderão também acessar não só o computador hóspede, mas todos os outros contêineres. Para ir além da segurança proporcionada ao sistema de contêineres pela configuração padrão, a [Docker recomenda][Docker recomenda] o uso de uma política de grupo para adição ou [segurança baseada em funções][segurança baseada em funções], como [SELinux][SELinux] ou [AppArmor][AppArmor], por exemplo, além de recomendar a redução ao máximo das funcionalidades do kernel concedidas aos contêineres. Além disso, há muitos outros documentos na Internet que descrevem abordagens de segurança ao usar contêineres como o Docker.
...
## <span id="Virtual Machine Extensions for Linux and Windows"></span>Extensões de Máquina Virtual para Linux e Windows</a>

A extensão de VM do Docker para Azure é apenas uma das diversas extensões que oferecem comportamentos especiais, e outras estão sendo desenvolvidas. Por exemplo, diversos recursos da [Extensão do Agente de VM Linux][Agente Linux do Azure] permitem que você modifique e gerencie a imagem, incluindo recursos de segurança, recursos de kernel e rede e assim por diante. A extensão VMAccess para imagens do Windows permite que você redefina ou modifique as configurações de Acesso à Área de trabalho remota e redefina a senha do administrador.

Para obter uma lista completa, consulte [Extensões de VM do Azure][Extensões de VM do Azure].
...
<!--Anchors-->
  ...
  [Galeria de imagens do Docker]: https://registry.hub.docker.com/
  ...
  [segurança baseada em funções]: http://en.wikipedia.org/wiki/Role-based_access_control
