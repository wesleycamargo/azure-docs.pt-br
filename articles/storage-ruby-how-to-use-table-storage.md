<properties linkid="dev-ruby-how-to-table-services" urlDisplayName="Serviço Tabela" pageTitle="Como usar o armazenamento da tabela (Ruby) | Microsoft Azure" metaKeywords="serviço de armazenamento da tabela do Azure, serviço tabela do Azure com Ruby, Ruby de armazenamento da tabela" description="Aprenda a usar o serviço de armazenamento Tabela no Azure. Os exemplos de código foram escritos usando a API do Ruby." metaCanonical="" services="storage" documentationCenter="Ruby" title="Como usar o serviço Tabela com Ruby" authors="guayan" solutions="" manager="" editor="" />





# Como usar o serviço Tabela com Ruby

Este guia mostra como executar cenários comuns usando o
serviço Tabela do Azure. Os exemplos foram escritos usando a API do Ruby. Os cenários abordados incluem **criar e excluir uma
tabela, inserir e consultar entidades em uma tabela**. Para obter mais informações sobre tabelas, consulte a seção [Próximas etapas](#next-steps).
...
* [Como consultar um subconjunto de propriedades de entidade](#how-to-query-a-subset-of-entity-properties)
...
## <a id="how-to-add-an-entity-to-a-table"></a>Como adicionar uma entidade a uma tabela

Para adicionar uma entidade, primeiro crie um objeto hash que defina as propriedades da entidade. Observe que é necessário especificar um **PartitionKey** e um **RowKey** para cada entidade. Estes são os identificadores exclusivos das entidades e são os valores que podem ser consultados muito mais rápido que as outras propriedades. O serviço Armazenamento do Azure usa **PartitionKey** para distribuir automaticamente as entidades da tabela entre vários nós de armazenamento. As entidades com o mesmo **PartitionKey** são armazenadas no mesmo nó. O **RowKey** é o ID exclusivo da entidade na partição à qual pertence. 
...
* **update\_entity():** Atualiza uma entidade existente ao substituí-la.
...
## <a id="how-to-query-a-subset-of-entity-properties"></a>Como consultar um subconjunto de propriedades de entidade
