<properties 
	pageTitle="Introdução aos dados (HTML 5) | Centro de Desenvolvimento de Serviços Móveis" 
	description="Saiba como começar a usar os serviços móveis para utilizar dados em seu aplicativo HTML." 
...
Você pode hospedar o aplicativo GetStartedWithData em qualquer servidor web. Mas, para sua conveniência, foram fornecidos scripts para permitir que você execute o aplicativo em `http://localhost:8000`.
...
<h2><a name="download-app"></a>Baixe o GetStartedWithData projeto</h2>
...
1. [Baixar os arquivos de projeto do aplicativo][GetStartedWithData HTML].
...
4. Abra a URL <a href="http://localhost:8000/" target="_blank">http://localhost:8000/</a> em um navegador Web para iniciar o aplicativo.
...
6. Na guia **Configurar** verifique se `localhost` já está relacionado na lista **Permitir solicitações de nomes de host** em **Compartilhamento de Recursos entre Origens (CORS)**. Se não estiver, digite `localhost` no campo **Nome do host** e clique em **Salvar**.
...
6. Remova os comentários da linha de código a seguir:
   	...
   	Esse código cria um objeto proxy (**todoItemTable**) para o Banco de Dados SQL **TodoItem**. 
...
6. No arquivo de projeto app.js, localize o método **RefreshTodoItems** e substitua a linha de código que define `query` pelo seguinte:
