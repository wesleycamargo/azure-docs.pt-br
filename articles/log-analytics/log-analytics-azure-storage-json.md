<properties
    pageTitle="Analisar logs de diagnóstico do Azure usando o Log Analytics | Microsoft Azure"
...
O Log Analytics consegue coletar os logs para os seguintes serviços do Azure que gravam [logs de diagnóstico do Azure](../azure-portal/monitoring-overview-of-diagnostic-logs.md) no armazenamento de blobs no formato JSON:
...
Para alguns recursos, é possível executar a configuração anterior do portal do Azure usando as etapas descritas em [Visão geral dos logs de diagnóstico de do Azure](../azure-portal/monitoring-overview-of-diagnostic-logs.md#how-to-enable-collection-of-diagnostic-logs).
...
>[AZURE.NOTE] Se o espaço de trabalho e os recursos estiverem em diferentes assinaturas do Azure, alterne entre eles, conforme necessário usando `Select-AzureRmSubscription -SubscriptionId <Subscription the resource is in>`
...
>[AZURE.NOTE] A configuração não está visível no portal do Azure. Você pode verificar a configuração usando o cmdlet `Get-AzureRmOperationalInsightsStorageInsight` .  
