<properties
    pageTitle="Conectar o Configuration Manager ao Log Analytics | Microsoft Azure"
...
Você pode conectar o System Center Configuration Manager ao Log Analytics no OMS para sincronizar os dados de coleção de dispositivos. Isso disponibiliza os dados da sua implantação do Configuration Manager no OMS.
...
4. No Configuration Manager, você poderá [atualizar as propriedades de conexão](#update-oms-connection-properties) se a chave secreta do cliente ou a senha expirar ou for perdida.
...
6. No OMS, [importe coleções do Configuration Manager](#import-collections) como grupos de computadores.
7. No OMS, exiba dados do Configuration Manager como [grupos de computadores](log-analytics-computer-groups.md).
...
O procedimento a seguir concede ao Portal de Gerenciamento do Azure permissões para o OMS. Especificamente, você deve conceder a *função Colaborador* aos usuários no grupo de recursos. Por sua vez, isso permite que o Portal de Gerenciamento do Azure conecte o Configuration Manager ao OMS.

>[AZURE.NOTE] Você deve especificar permissões para o OMS do Configuration Manager. Caso contrário, você receberá uma mensagem de erro ao usar o assistente de configuração do Configuration Manager.
...
3. Na folha **Espaço de Trabalho do OMS**, forneça as seguintes informações e clique em **OK**.
  ...
  - **Tipo de preço**  
    ...
    >[AZURE.NOTE] O exemplo acima cria um novo grupo de recursos. O grupo de recursos é usado apenas para conceder ao Configuration Manager as permissões para o espaço de trabalho do OMS nesse exemplo.
...
3. Na tela **Azure Active Directory**, configure suas definições de conexão para o OMS fornecendo seu **Locatário**, **ID do Cliente** e **Chave Secreta do Cliente** e selecione **Avançar**.  
...
>[AZURE.NOTE] Você deve conectar o OMS ao site de nível superior na hierarquia. Se você conectar o OMS a um site primário autônomo e, em seguida, adicionar um site de administração central ao seu ambiente, precisará excluir e recriar a conexão do OMS na nova hierarquia.
...
2. Nessa página, clique na guia **Azure Active Directory** para exibir seu **Locatário**, **ID do Cliente**, **Client secret key expiration (Expiração da chave secreta do cliente)**. **Verifique** sua **Chave secreta do cliente** caso ela tenha expirado.
...
Quando você clica em qualquer um, uma Pesquisa se abre, mostrando todos os grupos importados ou todos os computadores que pertencem a cada grupo. Usando a [Pesquisa de Log](log-analytics-log-searches.md), você pode iniciar uma análise detalhada de dados do Configuration Manager.
