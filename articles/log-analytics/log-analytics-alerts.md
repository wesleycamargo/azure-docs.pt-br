...
Ações de runbook iniciam o runbook usando um [webhook](../automation/automation-webhooks.md). Ao criar a regra de alerta, ela criará automaticamente um novo webhook para o runbook com o nome **OMS Alert Remediation (Remediação de Alerta do OMS)** seguido por um GUID. A regra de alerta não envia uma carga e não é possível popular nenhum parâmetro do runbook, por isso você deve usar um runbook que não exija parâmetros.

Você também pode iniciar um runbook de um alerta com uma ação de webhook conforme descrito na seção anterior. Ações de webhook têm a vantagem de fornecer uma carga para enviar dados para o runbook. A vantagem das ações de runbook é que você não precisa criar o webhook por conta própria ou entender como usá-lo.
