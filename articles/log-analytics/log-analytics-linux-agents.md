...
>[AZURE.NOTE] Os arquivos de configuração de edição para contadores de desempenho e syslog serão substituídos se a Configuração do Portal do OMS estiver habilitada. Você pode desabilitar a configuração no Portal do OMS (para todos os nós) ou para nós únicos ao executar o seguinte:
