...
Crie um sistema operacional do Linux VM para receber todos os dados syslog de hosts ESXi. O [agente do Log Analytics para Linux](log-analytics-quick-collect-linux-computer.md) é o ponto de coleta para todos os dados syslog de hosts ESXi. Você pode usar vários hosts ESXi para encaminhar logs para um único servidor Linux, como no exemplo a seguir.
...
Desse ponto em diante, você pode editar a consulta de pesquisa para modificá-lo para algo específico. Para obter detalhes sobre a criação de pesquisas de log, consulte [Localizar dados usando as pesquisas de logs no Log Analytics](log-analytics-queries.md).
...
* Usar [Pesquisas de Log](log-analytics-queries.md) no Log Analytics para exibir dados detalhados de hosts do VMware.
* [Criar seus próprios painéis](log-analytics-dashboards.md) mostrando os dados de host do VMware.
* [Criar alertas](../monitoring-and-diagnostics/monitoring-overview-unified-alerts.md) quando ocorrem eventos específicos de host do VMware.
