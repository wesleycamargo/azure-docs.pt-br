<properties 
   pageTitle="Logs personalizados no Log Analytics | Microsoft Azure"
...
## Definindo um log personalizado
...
### Etapa 1. Abrir o Custom Log Wizard (Assistente de Log Personalizado)
...
2.	Clique em **Dados** e em **Custom logs** (Logs personalizados).
...
### Etapa 2. Carregar e analisar um log de exemplo
...
>[AZURE.NOTE]O Log Analytics atualmente trata a data/hora coletadas de um log usando um delimitador de carimbo de data/hora como UTC. Isso em breve será alterado para usar o fuso horário no agente.
...
### Etapa 3. Adicionar caminhos de coleta de log
...
### Etapa 4. Fornecer um nome e descrição para o log
...
### Etapa 5. Validar que os logs personalizados estão sendo coletados
...
>[AZURE.NOTE] Se a propriedade RawData estiver ausente da pesquisa, você precisará fechar e reabrir o navegador.

### Etapa 6. Analisar as entradas do log personalizado
...
## Pesquisas de log com registros de log personalizados
...
| Consultar | Descrição |
|:--|:--|
| Type=MyApp\_CL | Todos os eventos de um log personalizado chamado MyApp\_CL. |
| Type=MyApp\_CL Severity\_CF=error | Todos os eventos de um log personalizado chamado MyApp\_CL com um valor de *erro* em um campo personalizado chamado *Severity\_CF*. |
...
Usamos um nome de *MyApp\_CL* e digitamos uma **Descrição**.
...
Usamos uma consulta de *Type=MyApp\_CL* para retornar todos os registros do log coletado.
