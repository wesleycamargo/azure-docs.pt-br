<properties
    pageTitle="Recursos do Log Analytics para provedores de serviços | Microsoft Azure"
    description="O Log Analytics pode ajudar provedores de serviços gerenciados (MSPs), grandes empresas, fornecedores de software independentes (ISVs) e provedores de serviço de hospedagem a gerenciar e monitorar servidores na infraestrutura de nuvem ou local do cliente."
...
O Log Analytics pode ajudar provedores de serviços gerenciados (MSPs), grandes empresas, fornecedores de software independentes (ISVs) e provedores de serviço de hospedagem a gerenciar e monitorar na infraestrutura de nuvem ou local do cliente. 
...
Para o Log Analytics, os seguintes recursos estão habilitados nas assinaturas do *Provedor de soluções de nuvem*.
...
Para fazer logon em uma assinatura de locatário, você precisa especificar o identificador do locatário. O identificador do locatário costuma ser a última parte do endereço de email usado para entrar.
...
>[AZURE.NOTE] As soluções Backup e Site Recovery do Log Analytics não são capazes de se conectar a um cofre dos Serviços de Recuperação e não podem ser configuradas em uma assinatura do CSP.
...
Mais detalhes sobre quando e por que criar vários espaços de trabalho estão descritos em [gerenciar acesso ao log analytics] (log-analytics-manage-access.md#determine-the-number-of-workspaces-you-need).
