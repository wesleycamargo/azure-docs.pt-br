...
description: Descreva os sintomas, as causas e a resolução dos problemas mais comuns com o agente do Log Analytics para Linux.
...
ms.locfileid: "49404572"
---
# <a name="how-to-troubleshoot-issues-with-the-log-analytics-agent-for-linux"></a>Como solucionar problemas com o agente Log Analytics para Linux 

Este artigo fornece ajuda para a solução de erros que podem ocorrer com o agente Log Analytics para Linux e sugere possíveis soluções para resolvê-los.
...
* Diagnostique os problemas do OMI com o [guia de solução de problemas da OMI](https://github.com/Microsoft/omi/blob/master/Unix/doc/diagnose-omi-problems.md).
* Arquivo de um [problema do GitHub](https://github.com/Microsoft/OMS-Agent-for-Linux/issues).
* Visite a página de Comentários do Log Analytics para revisar ideias e bugs enviados [http://aka.ms/opinsightsfeedback](http://aka.ms/opinsightsfeedback) ou arquivar um novo.  
...
## <a name="important-configuration-files"></a>Arquivos de configuração importantes
 ...
 >[!NOTE]
 >A edição de arquivos de configuração para contadores de desempenho e Syslog será sobrescrita se a coleção for configurada no [menu de dados Configurações Avançadas do Log Analytics](log-analytics-data-sources.md#configuring-data-sources) no portal do Azure para sua área de trabalho. Para desativar a configuração para todos os agentes, desative a coleta em **Configurações avançadas** do Log Analytics ou, para um único agente, execute o seguinte:  
...
| --- | --- |
| NOT_DEFINED | Como as dependências necessárias não estão instaladas, o plug-in auditoms do auoms não será instalado | A instalação dos auoms falhou, instale o pacote auditd. |
...
| 4 | Tipo de pacote inválido OU configurações de proxy inválidas; Pacotes omsagent- *rpm* .sh só podem ser instalados em sistemas baseados em RPM, e pacotes omsagent- *deb* .sh só podem ser instalados em sistemas baseados em Debian. É recomendado que você use o instalador universal da [última versão](log-analytics-quick-collect-linux-computer.md#install-the-agent-for-linux). Também [examine](#issue:-unable-to-connect-through-proxy-to-log-analytics) para verificar as configurações de proxy. |
...
| 55 | Versão openssl não suportada OU Não é possível conectar-se ao serviço do Log Analytics OU o dpkg está bloqueado OU faltando o programa de curl. |
...
| 4 | Proxy inválido fornecido ao script omsadmin. Verifique o proxy e consulte nossa [documentação para usar um proxy HTTP ](log-analytics-concept-hybrid.md#network-firewall-requirements). |
| 5 | 403 Erro HTTP recebido do serviço Log Analytics. Ver a saída completa do script omsadmin para obter detalhes. |
| 6 | Erro HTTP não-200 recebido do serviço Log Analytics. Ver a saída completa do script omsadmin para obter detalhes. |
| 7 | Não é possível se conectar ao serviço Log Analytics. Ver a saída completa do script omsadmin para obter detalhes. |
...
O log de depuração permite que você veja uploads em lote para o serviço Log Analytics separados por tipo, número de itens de dados e tempo gasto para enviar:
...
1. Reintegrar-se ao Log Analytics com o agente Log Analytics para Linux usando o seguinte comando com a opção `-v` habilitada. Isso permite a saída detalhada do agente que está se conectando por meio do proxy ao Log Analytics. 
...
1. Reonboard para o serviço Log Analytics com o agente Log Analytics para Linux usando o seguinte comando com a opção `-v` ativada. Ele permite que a saída detalhada do agente seja conectada por meio do proxy ao serviço do Log Analytics. 
...
* Verifique uma segunda vez se os seguintes pontos de extremidade do Log Analytics estão na lista de permissões:
...
<<<<<<< HEAD
3. Reintegre usando a ID do Workspace e a Chave do Workspace corretas seguindo as instruções de instalação descritas anteriormente neste tópico.
=======
...
<<<<<<< HEAD
- A integração ao Log Analytics falhou
- A conexão ao Log Analytics está bloqueada
- O agente do Log Analytics para dados do Linux é submetido a backup

### <a name="resolutions"></a>Resoluções
1. Verifique se a integração ao Log Analytics foi bem-sucedida, verificando se o seguinte arquivo existe: `/etc/opt/microsoft/omsagent/<workspace id>/conf/omsadmin.conf`
...
### <a name="resolution"></a>Resolução
* Verifique se a configuração no espaço de trabalho Log Analytics para o Syslog tem todos os recursos e os níveis de log corretos. Revise [configurar a coleção do Syslog no portal do Azure](log-analytics-data-sources-syslog.md#configure-syslog-in-the-azure-portal)
* Verifique se os daemons de mensagens de syslog nativos (`rsyslog`, `syslog-ng`) são capazes de receber as mensagens encaminhadas
...
### <a name="probable-causes"></a>Causas prováveis
* A integração ao serviço do Log Analytics falhou
* A conexão com o serviço Log Analytics está bloqueada
...
8. Em alguns casos, quando o agente do Log Analytics para Linux não pode se comunicar com o serviço do Log Analytics, é feito backup dos dados no agente para o tamanho total do buffer: 50 MB. O agente deve ser reiniciado executando o seguinte comando `/opt/microsoft/omsagent/bin/service_control restart`.
...
* Em alguns casos, o agente de configuração do Log Analytics Agent for Linux pode não conseguir se comunicar com o serviço de configuração do portal, fazendo com que a configuração mais recente não seja aplicada.
  ...
  2. Verifique se o agente `omsconfig` pode se comunicar com o serviço Log Analytics executando o seguinte comando `sudo su omsagent -c 'python /opt/microsoft/omsconfig/Scripts/GetDscConfiguration.py'`. Esse comando retorna a configuração que o agente recebe do serviço, incluindo configurações de Syslog, contadores de desempenho do Linux e logs personalizados. Se esse comando falhar, execute o seguinte comando `sudo su omsagent -c 'python /opt/microsoft/omsconfig/Scripts/PerformRequiredConfigurationChecks.py'`. Esse comando retorna a configuração que o agente recebe do serviço, incluindo configurações de Syslog, contadores de desempenho do Linux e logs personalizados.
...
### <a name="resolution"></a>Resolução
1. Verifique se o onboarding do Log Analytics foi bem-sucedido, verificando se o seguinte arquivo existe: `/etc/opt/microsoft/omsagent/<workspace id>/conf/omsadmin.conf`. Se não, qualquer um:  

  1. Reintegração usando a linha de comando omsadmin.sh [instruções](https://github.com/Microsoft/OMS-Agent-for-Linux/blob/master/docs/OMS-Agent-for-Linux.md#onboarding-using-the-command-line).
...
## <a name="issue-you-are-trying-to-reonboard-to-a-new-workspace"></a>Problema: você está tentando reingressar em um novo espaço de trabalho
...
2. Instale o agente seguindo as [instruções](log-analytics-quick-collect-linux-computer.md).
