...
# Divisão e mesclagem com dimensionamento Elástico
...
-    Tamanho do lote: O tamanho do lote controla o número de shardlets que ficarão offline por vez durante a movimentação de dados. Isso é um valor inteiro em que pode usar valores menores quando for sensível a períodos longos de inatividade para shardlets. Valores maiores aumentarão o tempo que um determinado shardlet fica offline mas podem melhorar o desempenho.
...
* O serviço de ﻿Divisão/Mesclagem atualmente não dá suporte a várias instâncias de função para esta função de trabalho. Isso precede configurações de alta disponibilidade no Azure usando domínios de falha ou de atualização que dependam da capacidade de executar várias instâncias de função. Além disso, o banco de dados de metadados de ﻿Divisão/Mesclagem não deve ser compartilhado entre instâncias diferentes. Por exemplo, uma instância do serviço de ﻿Divisão/Mesclagem executando em preparo precisa apontar para um banco de dados de metadados diferente da instância em execução na produção.
...
* **Carimbo de data/hora**: A hora e data em que a solicitação foi iniciada.
