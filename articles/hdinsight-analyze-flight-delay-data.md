<properties linkid="manage-services-hdinsight-analyze-flight-delay-data" urlDisplayName="Analisar dados de atraso de voo com o HDInsight" pageTitle="Analisar dados de atraso de voo usando o HDInsight | Azure" metaKeywords="" description="Saiba como atualizar dados no HDInsight, como processar os dados usando o Hive e como exportar os resultados para o Banco de Dados SQL usando o Sqoop." metaCanonical="" services="hdinsight" documentationCenter="" title="Analisar os dados de atraso de voo usando o HDInsight" authors="jgao" solutions="" manager="paulettm" editor="cgronlun" />

#Analisar os dados de atraso de voo usando o HDInsight

O Hive fornece um meio de executar o trabalho do MapReduce por meio de uma linguagem de script semelhante ao SQL, chamada *[HiveQL][hadoop-hiveql]*, que pode ser aplicada para sumarização, consultas e análise de grandes volumes de dados. Este tutorial mostra como usar o Hive para calcular a média de atrasos entre aeroportos e como usar o Sqoop para exportar os resultados para o Banco de Dados SQL. 

**Pré-requisitos:**

Antes de começar este tutorial, você deve ter o seguinte:

* Um cluster Azure HDInsight. Para obter informações sobre como provisionar um cluster HDInsight, consulte [Introdução ao HDInsight][hdinsight-get-started] ou [Provisionar clusters HDInsight][hdinsight-provision].
...
Este tutorial usa o desempenho pontual de dados de voos de companhias aéreas do [Research and Innovative Technology Administration, Bureau of Transportation Statistics][rita-website] (RITA) para sua estação de trabalho. Você irá executar o seguinte:
...
<table border="1">
<tr><td><strong>Arquivos</strong></td><td><strong>Descrição</strong></td></tr>
<tr><td>\tutorials\flightdelays\data</td><td>dados de voos de entrada para o Hive</td></tr>
...
- O comando CREATE EXTERNAL TABLE não permite pastas no LOCAL. Essa é a razão pela qual o tutorial faz uma cópia do arquivo sample.log.
...
1. Vá para [Research and Innovative Technology Administration, Bureau of Transportation Statistics][rita-website] (RITA).
...
3. Clique em **Baixar**. Cada arquivo pode levar até 15 minutos para baixar.
4. Descompacte o arquivo da pasta **C:\Tutorials\FlightDelays\Data**.  Cada arquivo é um arquivo CSV e tem aproximadamente 60 GB de tamanho.
5.	Renomeie o arquivo para o nome do mês do qual ele contém dados. Por exemplo, o arquivo que contém os dados de janeiro seria nomeado *January.csv*.
...
2. Defina as primeiras três variáveis e execute os comandos:
	...
	<table border="1">
	<tr><td><strong>Nome da variável</strong></td><td><strong>Descrição</strong></td></tr>
	<tr><td>$subscriptionName</td><td>Nome da sua assinatura do Azure.</td></tr>
	<tr><td>$storageAccountName</td><td>A conta de armazenamento do Azure usada para armazenar os arquivos de dados de voo. É recomendável usar a conta de armazenamento padrão.</td></tr>
	<tr><td>$containerName</td><td>O contêiner de armazenamento de Blob do Azure usado para armazenar arquivos de dados de voo. É recomendável usar o contêiner de Blob do sistema de arquivos do cluster HDInsight padrão.  Por padrão, o contêiner tem o mesmo nome do cluster HDInsight.</td></tr>
	<tr><td>$localFolder</td><td>Essa é a pasta em sua estação de trabalho onde estão armazenados os arquivos de atraso de voo.</td></tr>
	<tr><td>$destFolder</td><td>Esse é o caminho WASB onde os dados de atraso de voo serão carregados. O caminho do Hadoop (HDInsight) diferencia maiúsculas de minúsculas.</td></tr>
	<tr><td>$month</td><td>se você não baixou todos os 12 arquivos, precisará atualizar esta variável</td></tr>
	</table>

3. Execute os seguintes comandos para carregar e listar os arquivos:		
...
3. Defina as primeiras seis variáveis no script a seguir e, em seguida, execute os comandos:
	...
	<tr><td><strong>Nome da variável</strong></td><td><strong>Descrição</strong></td></tr>
	<tr><td>$subscriptionName</td><td>Nome da sua assinatura do Azure</td></tr>
	<tr><td>$sqlDatabaseServer</td><td>O nome do servidor de Banco de Dados SQL usado pelo Sqoop para o qual exportar dados. Se você deixá-lo como está, o script criará um para você. Caso contrário, especifique um Banco de Dados SQL ou um servidor SQL existente.</td></tr>
	<tr><td>$sqlDatabaseUsername</td><td>nome de usuário do Banco de Dados SQL/Servidor SQL.</td></tr>
	<tr><td>$sqlDatabasePassword</td><td>senha de usuário do Banco de Dados SQL/Servidor SQL.</td></tr>
	<tr><td>$sqlDatabaseLocation</td><td>Isso só será usado se você quiser que o script crie um servidor de Banco de Dados SQL para você.</td></tr>
	<tr><td>$sqlDatabaseName</td><td>O nome do Banco de Dados SQL usado pelo Sqoop para o qual exportar dados. Se você deixá-lo como está, o script criará um para você. Caso contrário, especifique um Banco de Dados SQL ou um servidor SQL existente.</td></tr>
	<tr><td>$sqlDatabaseMaxSizeGB</td><td>Isso só será usado se você quiser que o script crie um Banco de Dados SQL para você.</td></tr>
	</table>

4. Execute os seguintes comandos para criar o server/database/table do Banco de Dados SQL.
...
##<a id="createscript"></a>Criar e carregar o script HiveQL
...
4. **Crie a tabela de atrasos**. É útil limpar os dados antes de continuar o processamento. Esta consulta cria uma nova tabela *delays* a partir da tabela *delays_raw*. Observe que as colunas TEMP (conforme mencionado anteriormente) não são copiadas e que a função *substring* é usada para remover marcas de aspas dos dados. 
5. **Calcula a média de atraso de tempo e agrupa os resultados por nome de cidade.** Ela também retornará os resultados para o WASB. Observe que a consulta irá remover apóstrofos dos dados e excluirá linhas onde o valor de *weather_deal*y for *null*, o que é necessário porque, por padrão, o Sqoop usado posteriormente neste tutorial, não trata desses valores normalmente.
...
2. Defina as primeiras duas variáveis e execute os comandos.
	...
	<table border="1">
	<tr><td><strong>Nome da variável</strong></td><td><strong>Descrição</strong></td></tr>
	<tr><td>$storageAccountName</td><td>A conta de armazenamento do Azure usada para armazenar os arquivos de script HiveQL. Os scripts do PowerShell fornecidos neste tutorial requerem os arquivos de dados de voo e o arquivo de script localizados na mesma conta de armazenamento do Azure e no contêiner de armazenamento de Blob.</td></tr>
	<tr><td>$containerName</td><td>O contêiner de armazenamento de Blob usado para armazenar o arquivo de script HiveQL. Os scripts do PowerShell fornecidos neste tutorial requerem os arquivos de dados de voo e o arquivo de script localizados na mesma conta de armazenamento do Azure e no contêiner de armazenamento de Blob.</td></tr>
	<tr><td>$hqlLocalFileName</td><td>Os nomes dos arquivos locais para o script HiveQL antes de serem carregados no WASB. Para simplificar o script do PowerShell, você irá gravar o arquivo localmente e, em seguida, usar o cmdlet Set-AzureStorageBlobContent para carregar o arquivo de script no HDInsight.</td></tr>
	<tr><td>$hqlBlobName</td><td>Esse é o nome do arquivo de script com o caminho no WASB</td></tr>
	<tr><td>$srcDataFolder</td><td>Essa é a pasta no WASB onde o script HiveQL efetua pull dos dados.</td></tr>
	<tr><td>$dstDataFolder </td><td>Essa é a pasta no WASB para a qual o script HiveQL envia a saída. Mais adiante no tutorial, você usará o Sqoop para exportar os dados dessa pasta para o Banco de Dados SQL do Azure.</td></tr>
	</table>

3. Execute os comandos a seguir para definir as instruções HiveQL:
...
4. Execute os seguintes comandos para gravar o arquivo de script do Hive na estação de trabalho e carregá-lo no WASB:
...
##<a id="executehqlscript"></a>Executar o script HiveQL 

Existem vários cmdlets do PowerShell do Azure, que você pode usar para executar o Hive. Este tutorial usa o Invoke-Hive. Para obter outros métodos, consulte [Usar o Hive com o HDInsight][hdinsight-hive]. Usando o Invoke-Hive, você pode executar uma instrução HiveQL ou um script HiveQL. Você irá usar o script HiveQL, que você criou e carregou no armazenamento de Blob do Azure.

Há um erro conhecido do caminho do Hive. As instruções para corrigir o problema podem ser encontradas no [TechNet Wiki][technetwiki-hive-error].
...
2. Defina as primeiras três variáveis e, em seguida, execute-as:
	...
	<table border="1">
	<tr><td><strong>Nome da variável</strong></td><td><strong>Descrição</strong></td></tr>
	<tr><td>$clusterName</td><td>O cluster HDInsight que executará o script do Hive e a exportação do Sqoop.</td></tr>
	<tr><td>$storageAccountName</td><td>A conta de armazenamento do Azure usada para armazenar o script HiveQL. Consulte <a href = "#createScript">Criar e carregar o script HiveQL</a>.</td></tr>
	<tr><td>$containerName</td><td>O contêiner de armazenamento de Blob usado para armazenar o script HiveQL. Consulte <a href = "#createScript">Criar e carregar o script HiveQL</a>.</td></tr>
	<tr><td>$hqlScriptFile</td><td>Esse é o URI do arquivo de script HiveQL.</td></tr>
	<tr><td>$outputBlobName</td><td>Esse é o arquivo de saída do script HiveQL. O valor padrão é *000000_0*.</td></tr>
	</table>

3. Execute o comando a seguir para invocar o hive
		
		Use-AzureHDInsightCluster $clusterName
				
		Invoke-Hive –File $hqlScriptFile

4. Execute o comando a seguir para verificar os resultados:
...
##<a id="exportdata"></a>Exportar a saída de trabalho do Hive para o Banco de Dados SQL do Azure
...
3. Defina as primeiras cinco variáveis e execute os comandos:
	...
	Estas são as variáveis e suas descrições:
	<table border="1">
	<tr><td><strong>Nome da variável</strong></td><td><strong>Observação</strong></td></tr>
	<tr><td>$clusterName</td><td>Nome do cluster HDInsight</td></tr>
	<tr><td>$sqlDatabaseServer</td><td>O nome do servidor de Banco de Dados SQL para o qual o Sqoop irá exportar dados.</td></tr>
	<tr><td>$sqlDatabaseUsername</td><td>Nome de usuário do Banco de Dados SQL.</td></tr>
	<tr><td>$sqlDatabasePassword</td><td>Senha de usuário do Banco de Dados SQL.</td></tr>
	<tr><td>$sqlDatabaseName</td><td>O Banco de Dados SQL para o qual o Sqoop irá exportar os dados. O nome padrão é *HDISqoop". </td></tr>
	<tr><td>$sqlDatabaseTableName</td><td>Banco de Dados SQL para o qual o Sqoop irá exportar os dados O nome do arquivo padrão é AvgDelays. Esta é a tabela que você criou anteriormente no tutorial.</td></tr>
	<tr><td>$exportDir</td><td>Esse é o local do arquivo de saída do Hive. O Sqoop exportará os arquivos neste local para o Banco de Dados SQL.</td></tr>
	</table>
...
* [Usar o Sqoop com o HDInsight][hdinsight-sqoop]
