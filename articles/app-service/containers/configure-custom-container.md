...
No seu *docker-Compose. yml* do arquivo, mapeie o `volumes` opção `${WEBAPP_STORAGE_HOME}`. `WEBAPP_STORAGE_HOME` é uma variável de ambiente no Serviço de Aplicativo mapeada para armazenamento persistente para o aplicativo. Por exemplo: 
...
> [Tutorial: Implantar a partir do repositório de contêiner privado](tutorial-custom-docker-image.md)

> [!div class="nextstepaction"]
> [Tutorial: Aplicativo do WordPress de vários contêineres](tutorial-multi-container-app.md)
