...
* Novos recursos\cenários relacionados a **Extensões de Ferramentas da Web**. 
  ...
  * As caixas de diálogos de publicação de Aplicativo Web e de Aplicativo de API foram aprimoradas para dar suporte ao conceito de Portal do Azure do agrupamento de recursos e a seleção/criação de Grupos de Recursos do Azure e de Planos de Serviço de Aplicativo são representadas na nova caixa de diálogo de provisionamento do Aplicativo Web e do Aplicativo de API. 
  * Os nós do Gerenciador de Servidores do Aplicativo de API do Azure fornecem links para o link profundo de Aplicativos de API no Portal do Azure, bem como outros recursos, como Streaming de Log e Depuração Remota.
...
* Novo recursos\cenários relacionados a **Ferramentas do HDInsight** no Visual Studio estão habilitados nesta versão. 
  ...
  * Depuração de trabalhos de Hive aprimorada. Você pode depurar trabalhos de Hive acessando os logs de Yarn no Visual Studio. Se seu aplicativo tiver problemas de desempenho, a investigação de logs YARN oferecerá informações úteis.
  ...
  * Suporte para Storm. Agora você pode usar as ferramentas HDInsight para Visual Studio para desenvolver topologias do Storm/Spouts/Bolts em C#. Você pode enviar a topologia desenvolvida para um cluster Storm e ver o status da topologia/bolt/spout. Você pode usar logs do sistema e logs de cliente para solucionar Bolts/Spouts/topologias Storm. Você também pode usar os ativos JAVA existentes no Storm no HDInsight.
    ...
    Para obter mais informações, consulte [Introdução ao uso das ferramentas do HDInsight Hadoop para Visual Studio](../hdinsight/hadoop/apache-hadoop-visual-studio-tools-get-started.md).
...
* Os projetos de Aplicativo Móvel devem ser publicados em aplicativos Web, mas é necessário escolher um site criado como um Aplicativo Móvel no Portal do Azure, já que projetos de Aplicativo Móvel exigem um banco de dados. 
