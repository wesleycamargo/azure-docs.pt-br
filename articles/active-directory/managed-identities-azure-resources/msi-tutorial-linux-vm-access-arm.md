...
Atribuir a identidade gerenciada atribuída pelo usuário à sua VM do Linux usando [az vm assign-identity](/cli/azure/vm#az-vm-assign-identity). Substitua os valores de parâmetro `<RESOURCE GROUP>` e `<VM NAME>` pelos seus próprios valores. Use a propriedade `id` retornada na etapa anterior para o valor do parâmetro `--identities`.
