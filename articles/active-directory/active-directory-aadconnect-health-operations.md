...
- ***Diretório***: caminho recomendado para a maioria dos clientes e controla o acesso para todas as instâncias de serviço (por exemplo, um farm do AD FS) em todos os tipos de função que estão sendo monitorados pelo Azure AD Connect Health.
...
Para permitir acesso a um usuário no nível do *diretório* no Azure AD Connect Health, abra a folha principal no Azure AD Connect Health.<br> [//]\: # (RBAC\_MainBladeSS)
#### Etapa 2: Adicione usuários, grupos e atribua funções
1. Clique na parte "Usuários" na seção Configurar.<br> [//]\: # (RBAC\_MainBladeUserPart)
2. Selecione "Adicionar"
3. Selecione a “Função”<br> [//]\: # (RBAC\_AddRole)
...
5. Selecione "Ok".<br> [//]\: # (RBAC\_LastStep)
6. Uma vez concluída a atribuição de função, os usuários e/ou grupos aparecerão na lista.<br> [//]\: # (RBAC\_ListofUsersAndGroups)
...
2. Uma vez na folha, o usuário pode fixar a folha ou partes diferentes no painel clicando em "Fixar no painel"<br> [//]\: # (RBAC\_PinBlade)
...
Você pode remover um usuário ou grupo adicionado à parte de controle de acesso baseado em função do Azure AD Connect Health clicando com botão direito e selecionando “Remover”.<br> [/ /]\: # (RBAC\_RemoveUser)
