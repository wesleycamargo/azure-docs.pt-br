...
> [AZURE.TIP] Clique no link a seguir para executar essa solicitação! Depois de entrar, seu navegador deverá ser redirecionado para `https://localhost/myapp/` com um `id_token` na barra de endereços.  Observe que essa solicitação usa `response_mode=query` (para fins de tutorial apenas).  É recomendável usar `response_mode=form_post`.
...
> [AZURE.TIP] Clique no link a seguir para executar essa solicitação! Depois de entrar, seu navegador deverá ser redirecionado para `https://localhost/myapp/` com um `id_token` e um `code` na barra de endereços.  Observe que essa solicitação usa `response_mode=query` (para fins de tutorial apenas).  É recomendável usar `response_mode=form_post`.
