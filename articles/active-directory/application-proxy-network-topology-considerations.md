...
1. O usuário se conecta ao ponto de extremidade público do Proxy de Aplicativo do Azure AD no Azure
2. O Proxy de Aplicativo se conecta ao conector
3. O conector se conecta automaticamente ao aplicativo de destino
...
O serviço Proxy de Aplicativo escolhe o local das instâncias para você, com base no local de seu locatário. Você pode decidir onde instalar o conector, permitindo que você defina as características de latência de seu tráfego de rede.
...
O conector tem se comunicar com o Azure e seus aplicativos, de modo que o posicionamento do conector afeta a latência dessas duas conexões. Ao avaliar o posicionamento do Conector, considere o seguinte:

* Se você quiser usar a delegação restrita de Kerberos (KCD) para logon único, o conector precisará de uma linha de visão para um datacenter. 
...
Você pode tentar minimizar a latência do tráfego de ponta a ponta otimizando cada uma das conexões de rede. Cada conexão pode ser otimizada com:
...
Não há muito a fazer para controlar a conexão entre os usuários e o Proxy de Aplicativo do Azure AD; eles podem acessar seus aplicativos de uma rede doméstica, de uma cafeteria ou de um país diferente. Em vez disso, você pode otimizar as conexões do Proxy de aplicativo para os conectores e sucessivamente para os aplicativos. Considere a possibilidade de incorporar os padrões a seguir em seu ambiente.

### <a name="pattern-1-put-the-connector-close-to-the-application"></a>Padrão 1: colocar o conector perto do aplicativo

Coloque o conector perto do aplicativo de destino na rede do cliente. A vantagem desse padrão é que o conector provavelmente precisa de uma linha de visão para o controlador de domínio. Essa abordagem é suficiente para a maioria dos cenários, motivo pelo qual a maioria de nossos clientes segue esse padrão. Esse padrão também pode ser combinado com o padrão 2 para otimizar o tráfego entre o Proxy de Aplicativo e o conector.
...
Considere usar [grupos de conectores](active-directory-application-proxy-connectors.md) para aplicativos de destino que estão em localizações e redes diferentes. Se você estiver considerando essa opção, consulte [Serviços de Domínio do Azure AD](../active-directory-domain-services/active-directory-ds-overview.md).
...
- **Salto 1**: do usuário para o Proxy de Aplicativo do Azure AD
- **Salto 2**: do Proxy de Aplicativo do Azure AD para o conector
- **Salto 3**: do conector para o aplicativo de destino 
...
**Recomendação:** siga padrão 2, explicado na seção anterior. 
