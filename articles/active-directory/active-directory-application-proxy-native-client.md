...
O método recomendado para publicar aplicativos é usar a Biblioteca de Autenticação do AD do Azure que se encarrega de todo o incômodo da autenticação e dá suporte a muitos ambientes de cliente diferentes. O Proxy de Aplicativo se encaixa no [cenário de Aplicativo Nativo para API Web](https://azure.microsoft.com/documentation/articles/active-directory-authentication-scenarios/#native-application-to-web-api). O processo para realizar isso é o seguinte:
...
![](./media/active-directory-application-proxy-native-client/new_native_app.png) </br> </br>Para saber mais sobre o fluxo do aplicativo nativo, consulte [Aplicativo nativo para API Web](https://azure.microsoft.com/pt-BR/documentation/articles/active-directory-authentication-scenarios/#native-application-to-web-api).
