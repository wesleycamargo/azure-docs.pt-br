...
Sim.  Para operações em andamento, você pode configurar o Agente de integridade para encaminhar solicitações http de saída usando um HTTP Proxy. Para saber mais, veja [Configurar agentes do Azure AD Connect Health para usar HTTP Proxy](active-directory-aadconnect-health-agent-install.md#configure-azure-ad-connect-health-agents-to-use-http-proxy)
...
Quando você remove um agente de um servidor, o servidor não é automaticamente removido do Portal do Azure AD Connect.  Se você tiver removido um agente manualmente de um servidor ou o próprio servidor, precisará excluir manualmente a entrada do servidor no portal do Azure AD Connect Health. Para saber mais, acesse [Excluir uma instância de serviço ou servidor.](active-directory-aadconnect-health-operations.md#delete-a-server-or-service-instance)
