...
description: Usando id_tokens emitidos pelos pontos de extremidade v1 e v2 do Azure AD.
...
## <a name="validating-idtokens"></a>Validando `id_tokens`

A validação de um id_token é muito semelhante à primeira etapa de [validar um token de acesso](access-tokens.md#validating-tokens) – seu cliente deve validar que o emissor correto enviou de volta o token e que ele não foi adulterado.  Como `id_tokens` sempre são um JWT, há muitas bibliotecas para validar esses tokens – recomendamos que você use uma delas em vez de fazer isso por conta própria.  
