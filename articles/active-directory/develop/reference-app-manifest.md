...
| `availableToOtherTenants` | Boolean | True se o aplicativo for compartilhado com outros locatários; Caso contrário, false. <br><br> _Observação: Isso é substituído por `signInAudience` no [registros do aplicativo (visualização)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experiência._ | |
...
| `displayName` | Cadeia de caracteres | O nome de exibição do aplicativo. <br><br> _Observação: Isso é substituído por `name` no [registros do aplicativo (visualização)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experiência._ | `"MyRegisteredApp"` |
...
| `homepage` | Cadeia de caracteres | A URL da home page do aplicativo. <br><br> _Observação: Isso é substituído por `signInUrl` no [registros do aplicativo (visualização)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experiência._ | `"https://MyRegisteredApp"` |
| `objectId` | Cadeia de caracteres | O identificador exclusivo do aplicativo no diretório. <br><br> _Observação: Isso é substituído por `id` no [registros do aplicativo (visualização)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experiência._ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `publicClient` | Boolean | Especifica se este aplicativo é um cliente público (por exemplo, um aplicativo instalado em execução em um dispositivo móvel). <br><br> _Observação: Isso é substituído por `allowPublicClient` no [registros do aplicativo (visualização)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experiência._ | |
...
1. Se seu fluxo de trabalho exige que você salve os manifestos em seu repositório de origem para uso posterior, sugerimos que os manifestos salvos em seu repositório com aquele que você vê na troca de base de **registros do aplicativo (visualização)** experiência.
