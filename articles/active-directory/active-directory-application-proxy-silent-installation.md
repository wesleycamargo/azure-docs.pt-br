---
title: Como instalar silenciosamente o Conector de Proxy de Aplicativo do Azure AD | Microsoft Docs
...
1. Quando tiver o token, crie uma SecureString usando o token: <br> `$SecureToken = $Token | ConvertTo-SecureString -AsPlainText -Force`
2. Execute o seguinte comando do Windows PowerShell, onde SecureToken é o nome do token criado acima e tenantID é o GUID do seu locatário: <br> `RegisterConnector.ps1 -modulePath "C:\Program Files\Microsoft AAD App Proxy Connector\Modules" -moduleName "AppProxyPSModule" -Authenticationmode Token -Token $SecureToken -TenantId <tenant GUID>`
