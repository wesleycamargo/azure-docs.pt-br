...
> *Pule diretamente para o [guia de início rápido da Graph API](active-directory-graph-api-quickstart.md) e a [documentação de referência da Graph API interativa](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/api-catalog).*
...
Para começar a usar a Graph API do Active Directory do Azure, consulte o [Guia de início rápido da Graph API](active-directory-graph-api-quickstart.md) ou a [documentação de referência da Graph API interativa](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/api-catalog).
...
- **Pontos de extremidade da API REST**: a Graph API é um serviço RESTful composto por pontos de extremidade acessados por meio de solicitações HTTP padrão. A Graph API dá suporte a tipos de conteúdo XML ou JavaScript Object Notation (JSON) para solicitações e respostas. Para saber mais, consulte [Referência da Graph API REST do Azure AD](https://msdn.microsoft.com/library/azure/hh974478.aspx).

- **Autenticação com o Azure AD**: cada solicitação à Graph API deve ser autenticada com o anexo de um JWT (Token Web JSON) no cabeçalho de Autorização da solicitação. Esse token é adquirido ao fazer uma solicitação ao ponto de extremidade do AD do Azure e fornecer credenciais válidas. Você pode usar o fluxo de credenciais de cliente OAuth 2.0 ou o fluxo de concessão do código de autorização para adquirir um token para chamar o Graph. Para saber mais, [OAuth 2.0 no Azure AD](https://msdn.microsoft.com/library/azure/dn645545.aspx).

- **Autorização baseada em função (RBAC)**: grupos de segurança são usados para executar a RBAC na Graph API. Por exemplo, se você quiser determinar se um usuário tem acesso a um recurso específico, o aplicativo pode chamar a operação [Verificar associação de grupo (transitiva)](https://msdn.microsoft.com/library/azure/dn151601.aspx) que retorna true ou false.

- **Consulta diferencial**: se quiser verificar alterações em um diretório em dois períodos de tempo sem precisar fazer consultas frequentes à Graph API, você pode fazer uma solicitação de consulta diferencial. Este tipo de solicitação retornará apenas as alterações feitas entre a solicitação de consulta diferencial anterior e a solicitação atual. Para saber mais, leia [Consulta diferencial da Graph API do Azure AD](https://msdn.microsoft.com/library/azure/jj836245.aspx).

- **Extensões de diretório**: se estiver desenvolvendo um aplicativo que precisa ler ou gravar propriedades exclusivas para objetos de diretório, você pode registrar e usar valores de extensão usando a Graph API. Por exemplo, se seu aplicativo requer uma propriedade Skype ID para cada usuário, você pode registrar a nova propriedade no diretório e ela estará disponível para todos os objetos de usuário. Para saber mais, consulte [Extensões de esquema do diretório da Graph API do Azure AD](https://msdn.microsoft.com/library/azure/dn720459.aspx).
