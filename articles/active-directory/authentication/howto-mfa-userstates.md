...
As duas opções solicitam que os usuários se registrem na Autenticação Multifator do Azure na primeira vez que entrarem após a ativação dos requisitos. As duas opções também funcionam com as [definições configuráveis de Autenticação Multifator do Azure](howto-mfa-mfasettings.md).
...
Depois de habilitar os usuários, notifique-os por email. Informe que eles receberão uma solicitação de registro na próxima vez que entrarem. Além disso, se sua organização usar aplicativos sem navegador que não dão suporte à autenticação moderna, eles precisam criar senhas de aplicativo. Você também pode incluir um link para o [Guia do usuário final da MFA do Azure](./../../multi-factor-authentication/end-user/multi-factor-authentication-end-user.md) para ajudá-los a começar.
...
O _acesso condicional_ é um recurso pago do Azure Active Directory, com várias opções de configuração. Estas etapas explicam uma maneira de criar uma política. Para saber mais, leia sobre o [Acesso condicional no Azure Active Directory](../active-directory-conditional-access-azure-portal.md).
...
- Obtenha dicas sobre as [Práticas recomendadas para acesso condicional](../active-directory-conditional-access-best-practices.md).

- Gerenciar configurações de Autenticação Multifator do Azure para [seus usuários e os dispositivos deles](howto-mfa-userdevicesettings.md).
