...
* **É compatível com a redefinição de senhas de usuários que usam a** [sincronização de hash de senha](./../connect/active-directory-aadconnectsync-implement-password-hash-synchronization.md): quando o serviço de redefinição de senha detecta que uma conta de usuário sincronizado está habilitada para a sincronização de hash de senha, redefinimos a senha local e na nuvem dessa conta simultaneamente. O write-back de senha não depende da sincronização de hash de senha.
