...
Essa propriedade indica a relação entre o usuário e o locatário do host. Ele pode assumir dois valores:
...
- Convidado: esse valor indica um usuário que não é considerado interno na empresa. Pode ser um colaborador externo, parceiro, cliente ou usuário semelhante que não precisa receber o memorando interno do CEO, por exemplo, tampouco receber os benefícios da empresa.
...
Em alguns casos, talvez você queira dar privilégios mais altos aos usuários convidados. Para isso, você pode adicionar um usuário convidado a qualquer função e até mesmo remover as restrições do usuário convidado padrão no diretório a fim de fornecer os mesmos privilégios como membros.

É possível desativar as limitações do usuário convidado padrão para que os usuários convidados no diretório da empresa recebam as mesmas permissões de diretório que um usuário normal, que é um membro.
