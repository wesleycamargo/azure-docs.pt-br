...
O escopo deste artigo é fornecer um exemplo de código para a **API de auditoria**.

Consulte:

* [Logs de auditoria](overview-reports.md#activity-reports) para obter mais informações conceituais
* [Introdução à API de relatório do Azure Active Directory](concept-reporting-api.md) para saber mais sobre a API de relatório.
...
Antes de usar os exemplos deste artigo, você precisará atender aos [pré-requisitos para acessar a API de relatório do Azure AD](howto-configure-prerequisites-for-reporting-api.md).  
...
O script retorna a saída do relatório de auditoria no formato JSON. Ele também cria um arquivo `Audits.json` com a mesma saída. Você pode experimentar ao modificar o script para retornar dados de outros relatórios, além de comentar os formatos de saída de que você não precisa.
...
* Gostaria de personalizar os exemplos deste artigo? Confira os [Referência da API de auditoria do Azure Active Directory](https://developer.microsoft.com/graph/docs/api-reference/beta/resources/directoryaudit). 
* Se você quiser uma visão geral de como usar a API de relatório do Azure Active Directory, confira [Introdução à API de relatório do Azure Active Directory](concept-reporting-api.md).
* Se você quiser saber mais sobre os relatórios do Azure Active Directory, confira o [Guia de relatórios do Azure Active Directory](overview-reports.md).  
