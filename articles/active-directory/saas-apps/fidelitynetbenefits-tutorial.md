...
3. Na seção **URLs e Domínio do Fidelity NetBenefits**, siga as etapas abaixo:
    ...
    b. Na caixa de texto **Reply URL**, insira uma URL a ser fornecida pela Fidelity no momento da implementação ou entre em contato com o Fidelity Client Service Manager atribuído.
...
7. Na seção **Configuração do Fidelity NetBenefits**, clique em **Configurar Fidelity NetBenefits** para abrir a janela **Configurar logon**. Copie a **ID da Entidade SAML e a URL do Serviço de Logon Único SAML** da **seção de Referência Rápida.**

    ![Configuração do Fidelity NetBenefits](./media/fidelitynetbenefits-tutorial/tutorial_fidelitynetbenefits_configure.png)

8. Para configurar o single sign-on no lado do **Fidelity NetBenefits**, você precisa enviar o **XML de metadados**, **URL SAML Single Sign-On Service** e **SAML ID de entidade** a seu **Fidelity Client Service Manager designado**. Eles definem essa configuração para ter a conexão de SSO de SAML definida corretamente em ambos os lados.
