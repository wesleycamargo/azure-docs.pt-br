...
1. Na caixa de pesquisa, digite **SignalFx**, selecione **SignalFx** no painel de resultados e, em seguida, clique no botão **Adicionar** para adicionar o aplicativo.
...
1. **[Criar de um usuário de teste do SignalFx](#create-a-signalfx-test-user)** - para ter um equivalente de Brenda Fernandes no SignalFx que esteja vinculado à representação de usuário no Azure Active Directory.
...
1. Na seção **Domínio e URLs do SignalFx**, execute as seguintes etapas:
...
1. O aplicativo SignalFx espera as asserções SAML em um formato específico. Configure as declarações a seguir para este aplicativo. Você pode gerenciar os valores desses atributos da seção **Atributos de Usuário** na página de integração de aplicativos. A captura de tela a seguir mostra um exemplo disso.   
...
1. Na seção **Configuração do SignalFx**, clique em **Configurar SignalFx** para abrir a janela **Configurar logon**. Copie a **ID da Entidade SAML** da **seção Referência Rápida.**

    ![Configuração do SignalFx](./media/signalfx-tutorial/tutorial_signalfx_configure.png) 

1. Faça logon no site da empresa SignalFx como administrador.
