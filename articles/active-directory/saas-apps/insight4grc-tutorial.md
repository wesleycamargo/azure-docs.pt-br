...
3. **[Criar um usuário de teste do IInsight4GRC](#create-an-insight4grc-test-user)** – para ter um equivalente de Brenda Fernandes no Insight4GRC vinculado à representação do usuário no Microsoft Azure Active Directory.
...
3. Na seção **Domínio e URLs do Insight4GRC**, realize as seguintes etapas se desejar configurar o aplicativo no modo iniciado pelo **IDP**:
...
7. Para configurar o logon único no lado do **Insight4GRC**, é necessário enviar a **URL de metadados de federação do aplicativo** copiada para a [equipe de suporte do Insight4GRC](mailto:support.ss@rsmuk.com). Eles definem essa configuração para ter a conexão de SSO de SAML definida corretamente em ambos os lados.
