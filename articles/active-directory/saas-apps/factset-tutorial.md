...
3. **[Criar um usuário de teste do FactSet](#create-a-factset-test-user)** – para ter um equivalente de Britta Simon no FactSet vinculado à representação de usuário no Azure AD.
...
3. Na seção **URLs e Domínio do FactSet**, realize as seguintes etapas:
...
6. Para configurar o logon único no lado do **FactSet**, é necessário enviar o **XML de metadados** baixado para a [equipe de suporte do FactSet](mailto:support@factset.com). Eles definem essa configuração para ter a conexão de SSO de SAML definida corretamente em ambos os lados.
