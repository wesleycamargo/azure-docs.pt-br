...
1. Na caixa de pesquisa, digite **Degreed**.

    ![Criação de um usuário de teste do AD do Azure](./media/degreed-tutorial/tutorial_degreed_search.png)

1. No painel de resultados, selecione **Degreed** e clique no botão **Adicionar** para adicionar o aplicativo.
...
1. **[Como criar um usuário de teste do Degreed](#creating-a-degreed-test-user)** – para ter um equivalente de Brenda Fernandes no Degreed que esteja vinculado à representação do usuário no Azure AD.
...
1. Na seção **URLs e Domínio do Degreed**, execute as seguintes etapas:
...
1. Para configurar o logon único no lado do **Degreed**, é necessário enviar o **XML de metadados** baixado para a [equipe de suporte do Degreed](mailTo:admin@degreed.com). Eles definem essa configuração para ter a conexão de SSO de SAML definida corretamente em ambos os lados.
