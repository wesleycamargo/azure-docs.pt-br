...
4. Na caixa de pesquisa, digite **Kontiki**.

    ![Criação de um usuário de teste do AD do Azure](./media/kontiki-tutorial/tutorial_kontiki_search.png)

5. No painel de resultados, selecione **Kontiki** e clique no botão **Adicionar** para adicionar o aplicativo.
...
3. **[Criar um usuário de teste do Kontiki](#creating-a-kontiki-test-user)** – para ter um equivalente de Brenda Fernandes no Kontiki que esteja vinculado à representação do usuário no Azure AD.
...
3. Na seção **URLs e Domínio do Kontiki**, execute as seguintes etapas:
    ...
    > [!NOTE] 
    > Esse valor não é real. Atualize o valor com a URL de Logon real. Contate a [equipe de suporte ao cliente do Kontiki](http://customersupport.kontiki.com/enterprise/contactsupport.html) para obter o valor. 
...
6. Para configurar o logon único no lado do **Kontiki**, é necessário enviar o **XML de metadados** baixado para a [equipe de suporte Kontiki](http://customersupport.kontiki.com/enterprise/contactsupport.html). Eles definem essa configuração para ter a conexão de SSO de SAML definida corretamente em ambos os lados.
