...
3. **[Criar um usuário de teste do AwardSpring](#create-an-awardspring-test-user)** – para ter um equivalente de Brenda Fernandes no AwardSpring que esteja vinculado à representação de usuário do Microsoft Azure Active Directory.
...
3. Na seção **Domínio e URLs do AwardSpring**, realize as seguintes etapas se desejar configurar o aplicativo no modo iniciado pelo **IDP**:
...
9. Para configurar o lado do **AwardSpring** de logon único, você precisa enviar o **XML de Metadados** baixado para [a equipe de suporte do AwardSpring](mailto:support@awardspring.com). Eles definem essa configuração para ter a conexão de SSO de SAML definida corretamente em ambos os lados.
