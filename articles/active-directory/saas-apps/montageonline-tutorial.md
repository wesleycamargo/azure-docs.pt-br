...
3. **[Criar um usuário de teste do Montage Online](#create-a-montage-online-test-user)** – para ter um equivalente de Brenda Fernandes no Montage Online que esteja vinculado à representação de usuário do Azure AD.
...
3. Na seção **Domínio e URLs do Montage Online**, realize as seguintes etapas:
...
6. Na seção **Configuração do Montage Online**, clique em **Configurar o Montage Online** para abrir a janela **Configurar logon**. Copie a **URL de saída, a ID da Entidade SAML e a URL do Serviço de Logon Único SAML** da **seção de Referência Rápida.**

    ![Configuração do Montagem Online](./media/montageonline-tutorial/tutorial_montageonline_configure.png) 

7. Para configurar o logon único no lado do **Montage Online**, é necessário enviar o **Certificado (Base64) baixado, a URL de Saída, a ID da Entidade SAML e a URL do Serviço de Logon Único do SAML** para a [equipe de suporte do Montage Online](https://www.montage.co.nz/contact-us/). Eles definem essa configuração para ter a conexão de SSO de SAML definida corretamente em ambos os lados.
