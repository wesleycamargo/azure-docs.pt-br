...
1. Na caixa de pesquisa, digite **FiscalNote**, selecione **FiscalNote** no painel de resultados e clique no botão **Adicionar** para adicionar o aplicativo.
...
1. **[Criar de um usuário de teste do FiscalNote](#create-a-fiscalnote-test-user)** – para ter um equivalente de Brenda Fernandes no FiscalNote vinculado à representação do usuário no Azure AD.
...
1. Na seção **URLs e Domínio do FiscalNote**, execute as seguintes etapas:
...
1. Na seção **Configuração do FiscalNote**, clique em **Configurar o FiscalNote** para abrir a janela **Configurar logon**. Copie a **URL de saída, a ID da Entidade SAML e a URL do Serviço de Logon Único SAML** da **seção de Referência Rápida.**

    ![Configuração do FiscalNote](./media/fiscalnote-tutorial/tutorial_fiscalnote_configure.png) 

1. Para configurar o logon único no lado do **FiscalNote**, é necessário enviar o **Certificado (Bruto) baixado, a URL de Saída, ID da Entidade SAML e URL do Serviço de Logon Único SAML**para a [equipe de suporte do FiscalNote](mailto:support@fiscalnote.com). Eles definem essa configuração para ter a conexão de SSO de SAML definida corretamente em ambos os lados.
...
1. Na lista de aplicativos, escolha **FiscalNote**.
