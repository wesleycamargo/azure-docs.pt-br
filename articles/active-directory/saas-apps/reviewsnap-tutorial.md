...
3. **[Criar um usuário de teste do Reviewsnap](#create-a-reviewsnap-test-user)** – para ter um equivalente de Brenda Fernandes no Reviewsnap que esteja vinculado à representação de usuário no Azure AD.
...
3. Na seção **Domínio e URLs do Reviewsnap**, realize as seguintes etapas se desejar configurar o aplicativo no modo iniciado pelo **IDP**:
...
7. Na seção **Configuração do Reviewsnap**, clique em **Configurar Reviewsnap** para abrir a janela **Configurar logon**. Copie a **URL de saída, a ID da Entidade SAML e a URL do Serviço de Logon Único SAML** da **seção de Referência Rápida.**

    ![Configuração do Reviewsnap](./media/reviewsnap-tutorial/tutorial_reviewsnap_configure.png) 

8. Para configurar o logon único no lado do **Reviewsnap**, é necessário enviar o **Certificado (Base64) baixado, a URL de Saída, a ID da Entidade SAML e a URL do Serviço de Logon Único do SAML** para a [equipe de suporte do Reviewsnap](mailto:support@reviewsnap.com). Eles definem essa configuração para ter a conexão de SSO de SAML definida corretamente em ambos os lados.
...
Nesta seção, você criará uma usuária chamada Britta simon no Reviewsnap. Trabalhe com a [Equipe de suporte do Reviewsnap](mailto:support@reviewsnap.com) para adicionar os usuários na plataforma do Reviewsnap. Os usuários devem ser criados e ativados antes de você usar o logon único
