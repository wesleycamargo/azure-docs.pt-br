<properties pageTitle="Tutorial: Integração do Active Directory do Azure com a Plataforma de Nuvem HANA SAP | Microsoft Azure" description="Saiba como usar a Plataforma de Nuvem HANA SAP com o Active Directory do Azure para habilitar o logon único, o provisionamento automatizado e muito mais!" services="active-directory" authors="MarkusVi"  documentationCenter="na" manager="stevenpo"/>
<tags ms.service="active-directory" ms.devlang="na" ms.topic="article" ms.tgt_pltfrm="na" ms.workload="identity" ms.date="08/01/2015" ms.author="markvi" />
#Tutorial: Integração do Active Directory do Azure com a Plataforma de Nuvem HANA SAP
>[AZURE.TIP]Para ver comentários, clique [aqui](http://go.microsoft.com/fwlink/?LinkId=529793).
...
Depois de concluir este tutorial, os usuários do AD do Azure que você atribuiu à Plataforma de Nuvem HANA SAP poderão fazer um logon único no aplicativo usando a [Introdução ao Painel de Acesso](https://msdn.microsoft.com/library/dn308586)
...
O objetivo desta seção é descrever como permitir que os usuários se autentiquem na Plataforma de Nuvem HANA SAP com sua conta do AD do Azure usando federação baseada no protocolo SAML. Como parte deste procedimento, será necessário carregar um certificado codificado de base 64 no locatário da Plataforma de Nuvem HANA SAP. Se você não estiver familiarizado com este procedimento, confira [Como converter um certificado binário em um arquivo de texto](http://youtu.be/PlgrzUZ-Y1o).

###Para configurar o logon único, execute as seguintes etapas:

1.  No portal do AD do Azure, na página de integração do aplicativo **Plataforma de Nuvem HANA SAP**, clique em **Configurar logon único** para abrir a caixa de diálogo **Configurar logon único**.
...
2.  Na página **Como você deseja que os usuários façam logon na Plataforma de Nuvem HANA SAP**, selecione **Logon Único do AD do Microsoft Azure** e clique em **Avançar**.
...
3.  Em outra janela do navegador da Web, faça logon na ferramenta Cockpit da Plataforma de Nuvem HANA SAP em https://account.\<landscape host>.ondemand.com/cockpit (por exemplo: **https://account.hanatrial.ondemand.com/cockpit*).
...
5.  Na seção de gerenciamento de confiança, execute as seguintes etapas:
    ...
    1.  Clique na guia **Provedor de Serviço Local**.
    2.  Para baixar o arquivo de metadados da Plataforma de Nuvem HANA SAP, clique em **Obter Metadados**.

6.  No portal do Active Directory do Azure, na página **Configurar URL do Aplicativo**, execute as etapas a seguir e clique em **Avançar**.
    ...
    1.  Na caixa de texto **URL de Logon**, digite a URL usada pelos usuários para entrar no aplicativo **Plataforma de Nuvem HANA SAP**. Esta é a URL específica da conta de um recurso protegido em seu aplicativo Plataforma de Nuvem HANA SAP. A URL se baseia no seguinte padrão: *https://\<nomedoAplicativo><nomedaConta>.<landscape host>.ondemand.com/<caminho\_para\_recurso\_protegido>* (por exemplo: **https://xleavep1941203872trial.hanatrial.ondemand.com/xleave*)

		>[AZURE.NOTE]Esta é a URL em seu aplicativo Plataforma de Nuvem HANA SAP que exige que o usuário seja autenticado.

    2.  Abra o arquivo de metadados da Plataforma de Nuvem HANA SAP baixado e localize a marca **ns3:AssertionConsumerService**.
    3.  Copie o valor do atributo **Local** e cole-o na caixa de texto **URL de Resposta da Plataforma de Nuvem HANA SAP**.

7.  Na página **Configurar logon único na Plataforma de Nuvem HANA SAP**, para baixar seus metadados, clique em **Baixar metadados** e salve o arquivo do certificado em seu computador.
...
8.  Na ferramenta Cockpit da Plataforma de Nuvem HANA SAP, na seção **Provedor de Serviço Local**, execute as seguintes etapas:
...
9.  Clique na guia **Provedor de Identidade Confiável** e clique em **Adicionar Provedor de Identidade Confiável**.
...
10. Clique na guia **Geral** e clique em **Procurar** para carregar o arquivo de metadados baixado.
    ...
    >[AZURE.NOTE]Depois de carregar o arquivo de metadados, os valores de **URL de Logon Único**, **URL de Logoff Único** e o **Certificado de Assinatura** serão automaticamente preenchidos.
...
12. Na guia **Atributos**, execute as seguintes etapas:
    ...
    1.  Ao clicar em **Adicionar Atributo Baseado em Asserção**, adicione os atributos baseados em asserção a seguir:
    ...
    >  
    >a. O **Atributo Padrão** na captura de tela é apenas para fins de ilustração. Ele não é necessário para que o cenário funcione.
    >
...
13. No portal do Azure, na página de diálogo **Configurar logon único na Plataforma de Nuvem HANA SAP**, selecione a confirmação de configuração de logon único e clique em **Concluir**.
...
>[AZURE.NOTE]Usar grupos na Plataforma de Nuvem HANA SAP permite que você atribua dinamicamente um ou mais usuários a uma ou mais funções em seus aplicativos da Plataforma de Nuvem HANA SAP, determinados pelos valores de atributos na asserção SAML 2.0. Por exemplo, se a asserção contém o atributo "*contract=temporary*", você pode querer que todos os usuários afetados sejam adicionados ao grupo "*TEMPORÁRIO*". O grupo "*TEMPORÁRIO*" pode conter uma ou mais funções de um ou mais aplicativos implantados em sua conta da Plataforma de Nuvem HANA SAP.
>  
...
1.  Faça logon na sua ferramenta cockpit da **Plataforma de Nuvem HANA SAP**.
...
2.  Na página de integração de aplicativos da **Plataforma de Nuvem HANA SAP**, clique em **Atribuir usuários**.
