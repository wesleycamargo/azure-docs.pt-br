...
Usar a mesma credencial (nome de usuário e senha) para acessar seus recursos corporativos e serviços com base em nuvem garante que os usuários não precisem se lembrar de credenciais diferentes e reduz as chances de que eles se esqueçam de como fazer logon.  Isso também tem a vantagem de reduzir o envolvimento da equipe de assistência técnica para eventos de redefinição de senha.  

Embora muitas organizações se sintam à vontade com o uso da [Sincronização de hash da senha](active-directory-aadconnectsync-implement-password-synchronization.md) do Azure AD para fornecer aos usuários uma única credencial para acessar serviços locais e de nuvem, outras organizações exigem que as senhas, mesmo em um formulário de hash, não deixem seus limites organizacionais internos.  

A autenticação de passagem do Azure AD fornece uma solução simples para esses clientes, garantindo que a validação de senha para serviços do Azure AD seja executada no Active Directory local, sem a necessidade de infraestrutura de rede complexa ou existência das senhas locais na nuvem de qualquer forma.  

Quando combinado com o [Logon Único](active-directory-aadconnect-sso.md), os usuários também não precisam digitar a senha para fazer logon no Azure AD ou em outros serviços de nuvem, proporcionando a esses clientes uma experiência realmente integrada em seus computadores corporativos.

![Autenticação de Passagem](./media/active-directory-aadconnect-pass-through-authentication/pta1.png)

A autenticação de passagem pode ser configurada por meio do AAD Connect e utiliza um agente local simples que aceita solicitações de validação de senha.  O agente pode ser implantado facilmente em várias máquinas para fornecer alta disponibilidade e balanceamento de carga.  Como todas as comunicações são apenas de saída, não há requisitos para uma DMZ ou para o conector para ser instalado em uma DMZ.  Os requisitos da máquina para o conector são os seguintes:

- Windows Server 2012 R2 ou posterior
- Associado a um domínio na floresta em que os usuários serão validados

>[!NOTE]
>Ambientes de várias florestas poderão ser compatíveis se houver relações de confiança entre as florestas e o roteamento de sufixo de nome estiver corretamente configurado.

## <a name="supported-clients-in-the-preview"></a>Clientes com suporte na visualização
A autenticação de passagem tem suporte por meio de clientes baseados em navegador da Web e clientes do Office que dão suporte à [autenticação moderna](https://aka.ms/modernauthga).  Para clientes sem suporte, como clientes herdados do Office, Exchange ActiveSync (ou seja, clientes de email nativos em dispositivos móveis), os clientes são incentivados a usar a autenticação moderna equivalente.  Isso não só permite a autenticação de passagem, mas também permite que o acesso condicional seja aplicado, como a autenticação multifator.

Para clientes que usam o Windows 10 associado ao Azure AD, a autenticação de passagem não tem suporte atualmente.  No entanto, os clientes podem utilizar a sincronização de hash de senha como um fallback automático para Windows 10, além dos clientes herdados mencionados acima.

>[!NOTE]
>Durante a visualização, a sincronização de hash de Senha está habilitada por padrão quando a autenticação de passagem for selecionada como a opção de entrada no Azure AD Connect. Isso pode ser desabilitado na página Opções do Azure AD Connect.

## <a name="how-azure-ad-pass-through-authentication-works"></a>Como funciona a autenticação de passagem do Azure AD
Quando um usuário insere seu nome de usuário e senha na página de logon do Azure AD, o Azure AD coloca o nome de usuário e a senha na fila do conector no local apropriado para validação.  Um dos conectores disponíveis no local recupera o nome de usuário e a senha e as valida em relação ao Active Directory.  A validação ocorre em APIs padrão do Windows de maneira semelhante à forma como os Serviços de Federação do Active Directory validam a senha.

O Controlador de Domínio local avalia a solicitação e retorna uma resposta para o conector, que, por sua vez, a retorna ao Azure AD. O Azure AD avalia a resposta e responde ao usuário conforme apropriado, por exemplo, emitindo um token ou pedindo a Autenticação Multifator.  O diagrama a seguir mostra as diversas etapas.
...
- Software do Azure AD Connect
- Um diretório do Azure AD para o qual você é um administrador global.  
...
- Um servidor executando o Windows Server 2012 R2 ou superior no qual executar a ferramenta AAD Connect.  Essa máquina deve ser membro da mesma floresta em que o usuário será validado.
- Observe que, se você tiver mais de uma floresta que contém usuários a serem sincronizados com o Azure AD, as florestas deverão ter relações de confiança entre elas.
...
A autenticação de passagem do Azure AD é habilitada por meio do Azure AD Connect.  Habilitar a autenticação de passagem implantará o primeiro conector no mesmo servidor que o Azure AD Connect.  Ao instalar o Azure Connect AD, selecione uma instalação personalizada e a Autenticação de passagem na página de opções de entrada. Para obter mais detalhes, confira [Instalação personalizada do Azure AD Connect](active-directory-aadconnect-get-started-custom.md).
...
Informações adicionais podem ser encontradas nos logs de segurança dos controladores de domínio, se o log de auditoria está habilitado.  Uma consulta simples para solicitações de autenticação pelo conector seria o seguinte:
...
Outros erros relatados na tela de logon do Azure AD são detalhados abaixo, junto com as etapas de resolução apropriadas.
