...
O objetivo desse tutorial é mostrar como integrar o Trakstar ao Azure AD (Azure Active Directory). A integração do Trakstar ao Azure AD oferece os seguintes benefícios:
...
Para que o logon único funcione, o AD do Azure precisa saber qual usuário do Trakstar é equivalente a um usuário do AD do Azure. Em outras palavras, é necessário estabelecer uma relação de vinculação entre um usuário do AD do Azure e o usuário relacionado no Trakstar. Essa relação de vinculação é estabelecida com a atribuição do valor de **nome de usuário** no Azure AD como o valor de **Nome de usuário** no Trakstar.
...
4. Na página **Configurar logon único no Trakstar**, execute as seguintes etapas:
...
1. Para que o SSO seja configurado para o aplicativo, contate a equipe de suporte do Trakstar pelo email [integrations@trakstar.com](mailto:integrations@trakstar.com) e forneça o seguinte:
...
O objetivo desta seção é testar sua configuração de logon único do Azure AD usando o Painel de Acesso. Ao clicar no bloco do Trakstar no Painel de Acesso, você deverá ser conectado automaticamente ao seu aplicativo Trakstar.
