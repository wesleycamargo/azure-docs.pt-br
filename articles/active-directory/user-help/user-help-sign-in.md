---
title: Entrar com informações de segurança ou de verificação em duas etapas – Azure Active Directory | Microsoft Docs
...
Se por alguma razão você não puder usar o método de entrada principal, você pode usar outro configurado anteriormente o método de verificação.

1. Entre sua conta normalmente e, em seguida, escolha o **entre de outra forma** no link a **verificação em duas etapas** página.

    ![Alterar o método de verificação de entrada](media/security-info/two-factor-auth-signin-another-way.png)

    >[!Note]
    >Se você não vir as **entre de outra forma** link, isso significa que você não configurou quaisquer outros métodos de verificação e que você terá que contate o administrador para entrar em sua conta de Ajuda. Depois que o administrador ajuda você a entrar, verifique se você adicionar mais métodos de verificação. Para obter mais informações sobre como adicionar métodos de verificação, consulte o artigo [gerenciar as configurações de verificação em duas etapas](multi-factor-authentication-end-user-manage-settings.md).
    > 
    >Se você vir as **entre de outra forma** vincular, mas ainda não vir quaisquer outros métodos de verificação, você terá de entrar em contato com seu administrador para a Ajuda para entrar na sua conta.
