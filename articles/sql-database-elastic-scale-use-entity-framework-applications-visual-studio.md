<properties 
	pageTitle="Usando a Escala Elástica com o Entity Framework" 
	description="A escala elástica facilita o dimensionamento. O Entity Framework é fácil de usar para codificar bancos de dados" 
...
* **Model First**: O desenvolvedor cria o modelo no EF designer e, em seguida, o EF cria o banco de dados por meio do modelo.
* **Database First**: O desenvolvedor usa ferramentas do EF para inferir o modelo de banco de dados existente. 
...
* O banco de dados já existe e foi registrado no mapa do fragmento da Escala Elástica. 
* O esquema do aplicativo já foi implantado no banco de dados (explicado abaixo). 
* Conexões de roteamento dependentes de dados no banco de dados são arranjados pelo mapa do fragmento da Escala Elástica. 
...
Use o novo construtor para sua subclasse DbContext em vez do construtor padrão em seu código. Este é um exemplo: 
...
A equipe Microsoft Patterns & Practices publicou o [Bloco de aplicativos de manipulação de falhas transitórias](http://msdn.microsoft.com/library/dn440719(v=pandp.60.aspx)). A biblioteca é usada com a Escala Elástica em combinação com o EF. No entanto, certifique-se de que qualquer exceção transitória retorne para um local onde podemos garantir que o novo construtor está sendo usado após uma falha temporária para que qualquer nova tentativa de conexão seja feita usando os construtores que podemos ter ajustado. Caso contrário, uma conexão com o fragmento correto não é garantida e não há nenhuma garantia que a conexão seja mantida enquanto ocorrem alterações no mapa do fragmento. 
